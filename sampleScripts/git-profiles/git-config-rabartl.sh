...
eg config --global alias.log-short1 "log --pretty=format:'%Cgreen%H%Creset%nAuthor: %an <%ae>%nDate: %cd%n%n%s%n'"
eg config --global alias.log-short2 "log --pretty=format:'%Cgreen%h%Creset \"%s\"%nAuthor: %an <%ae>%nDate: %ad%n'"

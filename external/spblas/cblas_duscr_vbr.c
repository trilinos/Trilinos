...
void cblas_duscr_vbr(int n, double *val, int *indx, int *bindx, int *rpntr, 
...

{
  ...
  for (i=0; i<n; i++) 
	...
	minblocksize = MIN(minblocksize,blocksize);
	maxblocksize = MAX(maxblocksize,blocksize);
      }
    ncolvec[i] = buffersize;
    maxbuffersize = MAX(maxbuffersize, buffersize);
    minblocksize = MIN(minblocksize,rpntr[i+1]-rpntr[i]);
    maxblocksize = MAX(maxblocksize,rpntr[i+1]-rpntr[i]);

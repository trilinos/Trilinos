 \documentclass[12pt,strict]{TrilinosDevGuide}
\usepackage{array}
...
example, a user must append \InlineCommand{--disable-komplex} to the configure 
invocation  line.  Similarly, to include a package that is not currently built 
by default, NOX for example, users must append \InlineCommand{--enable-nox} to 
...
the BLAS and LAPACK) are located in standard places, try configuring without
...
and \InlineCommand{--with-mpi-incdir=MPIROOT/include}.
...
\item Pavel Bochev has discovered the the build process will fail on OSX if 
``DropZip'' is used to unzip the Trilinos tarball.  This utility truncates 
long file names.
...
in which the new files are located.  When adding a new directory that will
require a Makefile (for example, when adding a new test directory), create a 
new Makefile.am.  Usually a developer can find a Makefile.am to use as a 
template in the new\_package package.  To remove files from the build process, 
delete the file names from the appropriate Makefile.am.
...
committed makes sense. A cvs update will check to 
...
Next verify the list of files that appears and enter an appropritate log 
...
scripts can be found in \InlineDirectory{Trilinos/config}.  The names of all 
...
Bonsai gives a developer the ability to view the changes made to the files in 
...
%Possibly add a bit about where the whole release process can be found
...
a branch.  A developer must checkout a new working copy, create the branch, 
...
We want to tag the working copy to mark the point at which the new branch 
will diverge from the existing branch.  To do this, cd to the top Trilinos 
directory and type
...
to be incremented in the \InlineDirectory{Trilinos/configure.ac} file.  First, 
...
a many tests need to be run.  This step simply refers to running a few 
simple sanity tests (for example, configure and build Trilinos and run the 

...
\InlineCommand{arch_comm_machine}.
For example \InlineCommand{sgi64_mpi_atlantis}.  Note that these scripts are examples 
...
which runs nightly on a variety of supported platforms.
\end{tabular}
\end{center}
...
Table~\ref:RequirementsAndPractices}, we have structured the
integration
...
defined:
\begin{enumerate}
\item \InlineCommand{Package_name-Announce@software.sandia.gov}: 
Announcements such
...
\item \InlineCommand{Package_name-Checkins@software.sandia.gov} List
...
\item \InlineCommand{Package_name-Developers@software.sandia.gov}
...
\item \InlineCommand{Package_name-Regression@software.sandia.gov} List
to which all automated regression test results are sent for archival purposes.
\item \InlineCommand{Package_name-Users@software.sandia.gov} User
...
Figure~\ref{Figure:PackageDependencies} shows how the present
\begin{figure}
%\begin{center}
\includegraphics[height=6in,angle=270]{TrilinosPackagesDependencies}
%\end{center}
\label{Figure:TrilinosPackages}

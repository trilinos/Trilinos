...
package.  We outline the steps for adding or removing source files below.  
The below outline assumes the simple addition or removal of files.  Special 
situations such as adding header file or library dependencies to 
a Trilinos package require a more complicated process.
\begin{enumerate}
...
Automake can always be found on software.sandia.gov.
...
\item List new files in/remove obsolete files from Makefile.am
(describe)
...
both the new code works and existing code has not been broken.

\item Update source code from Trilinos repository
Didn't we just do this?  Yes, but there are two good reasons to do it again.

\bullet Other developers could have committed changes during the past several 
...
\bullet 
\item Commit the changes to the Trilinos repository (FIX-UP)
Once all of the above steps are completed, the final step is to commit the 
changes to the repository by typing...
\begin{verbatim}
cvs commit
\begin{verbatim}
Next, look at the list of files that are to be added or modified.  It is 
easy to inadvertantly commit files.  Perhaps a developer is working on two 
changes to files in the same package, or has forgotten about an abandoned 
effort that was never cleaned up.  Finally, enter an appropriate log message, 
save the file, and exit the CVS editor.  It is recommended that a developer 
consider testing the new code...

\end{enumerate}

ADDING FILES PRELIMINARILY!!!

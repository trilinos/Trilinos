...
rules for raw C++ pointers (e.g.~from derived to base classes, from
non-{}\texttt{const} to {}\texttt{const}) is just as easy as with raw
C++ pointers.  For example, the compiler will implicitly cast a raw
C++ pointer up its type's inheritance hierarchy (e.g.~{}\texttt{B1}
...
The most common use of {}\texttt{RefCountPtr<>} is to manage the

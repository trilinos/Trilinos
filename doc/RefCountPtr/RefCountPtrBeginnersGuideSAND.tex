...
tools package {}\texttt{Teuchos} (means ``tool'' in Latin), that
combines the concepts of smart pointers and reference counting to
build a low-overhead but effective means for simplifying dynamic
memory management in C++.  Here we describe why the raw use of
pointers and raw calls to operator {}\texttt{new} and operator
...
easily be modified to use {}\texttt{Ref\-Count\-Ptr<>}.  In its most
basic usage, {}\texttt{Ref\-Count\-Ptr<>} automatically determines
when operator {}\texttt{delete} should be called to free an object
...
{}\texttt{Ref\-Count\-Ptr<>} is described in the companion document
...
Effective C++" {}\cite{ref:meyers_1996}.  In short, using this class
allows one client to dynamically create an object (using operator
...
to it.  This is very similar to the type of garbage collection that is
in languages like Perl and Java.  There are, although, some
...
rules are partly spelled out in the form of commandments (see Appendix
{}\ref{rcpqs:apdx:commandments}).

The use of {}\texttt{Ref\-Count\-Ptr<>} is critical in the development
and maintenance of complex object-oriented programs composed of many
separately-developed pieces (such as Trilinos).  This discussion
assumes that the reader has a basic familiarity and some programming
...
{}\cite{ref:gama_et_al_1995} and {}\cite{ref:stroustrup_2000}) and is
comfortable with the use of C++ pointers and references.
...
the template class {}\texttt{Ref\-Count\-Ptr<>} and some important
...
{}\ref{rcpqs:apdx:passing-args} gives tables for recommended idioms
for how to pass raw C++ objects and
...
{}\texttt{Ref\-Count\-Ptr<>} is all about.  This example together with
the reference material in the appendices should be enough for
semi-experienced C++ developers to get up and going with
...
{}\texttt{Ref\-Count\-Ptr<>} objects or to manage the lifetime of such
array class objects.
...
for the development of {}\texttt{Ref\-Count\-Ptr<>}.  OO programs are
...
known until runtime.  The ``Abstract Factory''
{}\cite{ref:gama_et_al_1995} is a popular design pattern that allows
the flexible runtime selection of what concrete subclasses to create.
...
that defines an interface that provides some useful capability.
...
when new objects should be created anew or when objects should be
reused and passed around is very common in complex OO programs.  The
above client classes show two different types of associations between
objects: {}\textit{transitive} and {}\textit{non-transitive}.
...
{}\texttt{ClientC} object and a {}\texttt{Utility\-Base\-Factory} and
a {}\texttt{Utility\-Base} object through the the private C++ pointer
...
transitive relationships since in many programs a non-transitive
relationship implies that some objects must be responsible for the
lifetime of other objects.  This is never the case in a transitive
...
Appendix {}\ref{rcpqs:apdx:ex-prg} has several memory management
...
logic would enlarge the program, complicate maintenance, and would
...
{}\texttt{Ref\-Count\-Ptr<>} is to replace {}\texttt{T*} with
...
requires that the operator {}\texttt{new} calls be wrapped in a
templated function call to {}\texttt{Teuchos\-::rcp(...)} (see
...
This appendix presents a short but fairly comprehensive quick-start for
the use of {}\texttt{Ref\-Count\-Ptr<>}.  The use cases described here
should cover 99\% or more of the use instances of
{}\texttt{Ref\-Count\-Ptr<>} in a program.
...
to be in the namespace {}\texttt{Teuchos} or has appropriate
{}\texttt{using Teuchos::...} declarations.  This removes the need to
explicitly use {}\texttt{Teuchos::} to qualify classes and non-member
functions.
...
function prototypes as self documenting as possible, avoid coding
errors and increase readability of C++ code.  In general,
...
int main( int argc, char* argv[] )
{
  ...
  RefCountPtr<UtilityBaseFactory>
    utilityFactory = ( useA ? rcp(new UtilityAFactory()) : rcp(new UtilityBFactory()) );
  // Create cleints
  ClientA a;
  ClientB b1, b2;
  ClientC c(utilityFactory);

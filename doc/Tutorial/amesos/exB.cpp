...
int main(int argc, char *argv[])
{
  ...
  // create a new MPI Communicator which contains the first 3 processes
  // The others will do anything

  MPI_Comm NewMpiComm;
  MPI_Group OrigGroup, NewGroup;
  
  int NewNumProcs = 2;
  int members[NewNumProcs]; 
  for (i=0; i<NewNumProcs; i++) {
    members[i] = i;
  };

  MPI_Comm_group(Comm.GetMpiComm(), &OrigGroup);
  MPI_Group_incl(OrigGroup, NewNumProcs, members, &NewGroup);
  
  MPI_Comm_create(Comm.GetMpiComm(), NewGroup, &NewMpiComm);

  int NumMyElements;
  
  if( MyPID < 2 ) NumMyElements = 10;
  else            NumMyElements = 0;
  ...
  if( NumMyElements != 0 ) {
    ...
    Epetra_MpiComm NewComm(NewMpiComm);

    // create a map
    Epetra_Map NewMap(-1,NumMyElements,0,NewComm);

    double * pointer;
    x.ExtractView(&pointer);

    Epetra_Vector xNew(View,NewMap,pointer);

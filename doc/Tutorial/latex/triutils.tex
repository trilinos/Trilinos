...
\verb!Trilinos_Util::CommandLineParser! is a class to parse the command
...
The main options are here reported.
...
{\tt box} & {Used for problems defined on Cartesian grids over a
  square. The domain subdivided into {\tt mx x my} subdomains.  {\tt mx}
  and {\tt my} are automatically computed if the total number or
  processes is a square number. Alternatively, {\tt
...
\hline
  \end{tabular}
  \caption{Available options for {\tt map\_type}.}
...
{\tt quad\_xy} & Nodes are supposed to be distributed over the square
  ...
  computed as $x(1-x)y(1-y)$. \\
    \hline
  \end{tabular}
  \caption{Available options for {\tt exact\_solution}.}

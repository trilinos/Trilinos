...
\chapter{Multilevel Methods with ML}
\label{chap:ml}

The ML package defines a class of preconditioner based on multilevel
...
provides multilevel solvers, as well as multilevel preconditioners.

In this Chapter we present:
\begin{itemize}
\item Outline the basic issued of multilevel schemes (in
  Section~\ref{ml:theoretical});
\item How to use ML objects as a preconditioner for an AztecOO solver
  (in Section~\ref{sec:ml_prec});
\item How to implement two-level domain decomposition methods, with a
  coarse grid defined using aggregation procedures (in
  Section~\ref{sec:ml_DD}).
\end{itemize}
...
methods, to establish a notation. The Section is not supposed to be
exhaustive, nor complete on this subject. The reader is referred to the
existing literature for a rigorous presentation.

\medskip

Multigrid methods require the operator to be defined on a sequence of
coarser grids, an iterative method that evolves the solution (called a
smoother) and interpolation operators that transfer information between
the grids. The principle behind the algorithm is that the high-frequency
errors can be efficiently solved on the fine grid, while the
low-frequency are treated on the coarser one, where there frequencies
manifest themselves as high-frequencies.  Geometric multigrid (GMG)
...
\section{ML as a Preconditioner for AztecOO}
\label{sec:ml_prec}
...
Let us suppose that we intend to use ML as a ``black-box'' (or gray-box)
multilevel preconditioner, using aggregation procedures to define the
multilevel hierarchy. The variable 
\begin{verbatim}
int N_levels = 10;
\end{verbatim}
...
aim to construct a preconditioner.)
...
We still have to define the smoother, which is a one-level domain
decomposition, as implemented in the AztecOO library:
\begin{verbatim}
FIXME...
\end{verbatim}

################################################################################
#
# Set up env on skybridge for ATMD builds of Trilinos
#
# This source script gets the settings from the ATDM_CONFIG_BUILD_NAME var.
#
################################################################################

# Skybridge and serrano jobs all use the same environmnet changes to the
...
if [ "$ATDM_CONFIG_KOKKOS_ARCH" != "SNB" ] ; then
  ...
  echo "*** on Skybridge 'SNB' is the only valid KOKKOS_ARCH. If no KOKKOS_ARCH is specified then"

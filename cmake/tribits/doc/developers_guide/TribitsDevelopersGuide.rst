...
configuration by setting the configure option  (see `Outputting package dependency information`), or the CMake
-P script ``TribitsDumpDepsXmlScript.cmake`` can be used to create this file
on the fly without having to configure a TriBITS project.  To create this file
outside of configuration, one can run::

  cmake \
    -D PROJECT_SOURCE_DIR=<projectSourceDir> \
    [-D <Project>_TRIBITS_DIR=<tribitDir> ] \
...
``<projectDir>/TriBITS`` and (one does not want to include info from extra
repositories), then one can just run::
...
``<build-dir>/Testing/Temporary/LastTestsFailed.log`` file (which is used in
the `TRIBITS_CTEST_DRIVER()`_ function).

The tool `filter-packages-list.py`_ takes in a list of TriBITS SE package
names and the filters the list according the `Test Test Category`_ of the
packages.  This is used in testing workflows that only test a subset of
packages at different stages in the workflow.  For example, the
`checkin-test.py`_ tool uses this functionality to only test Primary Tested
(PT) or Secondary Tested (ST) packages for a given set of changed files in a

...
{}\textbf{Defined: PT, ST, and EX tested code:}

{}\textbf{PT} (Primary Tested) code has minimal outside dependencies, PT TPLs, and represents critical functionality such that if broken would hamper core development efforts for many developers. (Examples of PT TPLs include BLAS, LAPACK, and MPI.)

{}\textbf{ST} (Secondary Tested) code has dependencies on more than just the minimal set of PT TPLs (such as depending on Boost) or is code that does not represent critical functionality such that if broken would significantly hamper the work of other developers.  Most ST code is tested by the asynchronous CI server for the primary development platform after commits are pushed to the main development repository.  ST code is also tested nightly on many platforms.

{}\textbf{EX} (Experimental) (non-)tested code is not PT or ST code and not tested in any official testing process by by the project.  Developers of EX code do their own testing on the side.

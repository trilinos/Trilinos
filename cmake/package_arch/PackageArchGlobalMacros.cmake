...
FUNCTION(PACKAGE_ARCH_DUMP_DEPS_XML_FILE)
  ...
  FOREACH(PACKAGE ${${PROJECT_NAME}_SE_PACKAGES})
    ...
    APPEND_STRING_VAR(DEPS_XML
      "  <Package name=\"${PACKAGE}\" dir=\"${PACKAGE_DIR}\" type=\"${${PACKAGE}_CLASSIFICATION}\">\n")
    ...
    APPEND_STRING_VAR(DEPS_XML
      "    <ParentPackage value=\"${${PACKAGE}_PARENT_PACKAGE}\"/>\n"
      )
...
#
FUNCTION(PACKAGE_ARCH_PRINT_ENABLED_SE_PACKAGE_LIST DOCSTRING ENABLED_FLAG INCLUDE_EMPTY)
  PACKAGE_ARCH_GET_ENABLED_LIST( ${PROJECT_NAME}_SE_PACKAGES ${PROJECT_NAME} ${ENABLED_FLAG}
...
FUNCTION(PACKAGE_ARCH_DETERMINE_IF_PROCESS_PACKAGE  PACKAGE_NAME
  ...
  # Process the package if any of the the subpackages are enabled
  FOREACH(SUBPACKAGE ${${PACKAGE_NAME}_SUBPACKAGES})
    ...
    IF (${PROJECT_NAME}_ENABLE_${SUBPACKAGE_FULLNAME})
      ...
      APPEND_STRING_VAR(PACKAGE_ENABLE_STR "${SUBPACKAGE}, ")
    ENDIF()
  ...
  IF (${PROJECT_NAME}_ENABLE_${PACKAGE_NAME})
    SET(PROCESS_PACKAGE TRUE)
    APPEND_STRING_VAR(PACKAGE_ENABLE_STR "Libs, ")
    IF (${PACKAGE_NAME}_ENABLE_TESTS)
      APPEND_STRING_VAR(PACKAGE_ENABLE_STR "Tests, ")
    ENDIF()
    IF (${PACKAGE_NAME}_ENABLE_EXAMPLES)
      APPEND_STRING_VAR(PACKAGE_ENABLE_STR "Examples, ")
    ENDIF()
  ENDIF()
...
MACRO(PACKAGE_ARCH_CONFIGURE_ENABLED_PACKAGES)
  ...
  FOREACH(PACKAGE ${${PROJECT_NAME}_PACKAGES})
    ...
    PACKAGE_ARCH_DETERMINE_IF_PROCESS_PACKAGE(${PACKAGE}
      PROCESS_PACKAGE  PACKAGE_ENABLE_STR)

    IF (PROCESS_PACKAGE)

      MESSAGE("Processing enabled package: ${PACKAGE} (${PACKAGE_ENABLE_STR})")

      SET(PACKAGE_NAME ${PACKAGE}) # Used in CMake code in downstream package
      SET(PARENT_PACKAGE_NAME ${PACKAGE})

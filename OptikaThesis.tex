...
using a technology that will not work on their user's platform of choice. Cocao, for example, only works on
...
This hierarchy is made up of Parameters and ParameterLists. A Parameter can simply be thought of as single key-value input pair for
...
\caption{The supporting C++ source code needed utilize the XML in Figure~\ref{basicXMLXML}}
\label{basicXMLC++}
\end{figure}
\begin{figure}
{\footnotesize
...
in Figure~\ref{custSig}. The parameter list that is given in the argument is the current values user has specified. To use a custom function, pass a pointer to the

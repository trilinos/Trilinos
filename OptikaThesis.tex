...
best to look at an example of the problem. Currently, the program Tramonto has
a large input file filled with complex, non-intuitive inputs. Figure
...
modify them for their needs. Optika aims to solve this problem by allowing the developers
of Tramonto to specify their required inputs, and let Optika handle the actual retrieval of
input values. The method of gathering input is completely abstracted away from the application developer.
...
of a GUI to an application, a developer doesn't want to accidentally exclude some users by
...
This hierarchy is made up of Parameters and ParameterLists. Parameters can simply be thought of as single key-value input pair for
...
it's own set of unique capabilities. Optika is the Trilnos GUI package.
...
reasons:
	...
	\end{itemize}
Because Trilinos is itself a library, relying on an additional third-party library had many potential risks. But
...
While examining the example input file section we see it deals with ``Dimension Control Parameters.'' These are just a few parameters 
used to define what kind of dimensions are going to be used in the rest of the input file. Code listing~\ref{basicTramonto} shows how we would create
...
Figure~\ref{basicTramontoXML}. The XML code is rather straight forward. As in the C++ code we specify the name of the
...
not accept invalid parameter values. Validators were also already part of the Teuchos package
...
``prototypeId'' XML attribute as in code listing~\ref{protoAttributeArrayXML}. This second option allows the developer to reuse the prototype validator in 
other instances.
...
Code listing~\ref{StringValiDepXML} shows an example of how to use a String Validator Dependency. In this example we have two parameters, 
``Favorite Food Type''
...
Code listing~\ref{simpleNumberCon} shows an example of a number condition used to evaluate a parameter with an id of 3. Like the other number based entities in Optika the number 
...
There are a number of ways customize the look and feel of an Optika based GUI. All of them involve creating an Optika GUI object. Once the object is created a call to the
...
The initial task for creating a GUI solution for Trilinos was proposed by Dr. Michael Heroux of Sandia National Laboratories. All code for Optika was written by Kurtis Nusbaum, 
including the algorithm for evaluating dependencies. Roscoe Bartlett of Sandia National Laboratories helped design the mechanisms by which Optika interprets XML. Jim Willenbring of Sandia National Laboratories

...
parameter, it's default value, and a documentation string (the documentation attribute is optional, just like when using 
...
to the ParameterList into which user input should be stored.
	\begin{figure}
		\centering
		\subfigure[C++ Code]{
      \lstinputlisting[language=C++, basicstyl=\scriptsize]{basicXMLTramonto/main}
			\label{basicXMLC++}
		}
		\subfigure[XML Code]{
      \lstinputlisting[language=XML, basicstyl=\scriptsize]{basicXMLTramonto/inputs}
			\label{basicXMLXML}
		}
	...
	\end{figure}
\section{Advanced Optika Usage}
In this section we discuss some of the advanced features of Optika. All off the examples provided here will be done using
the XML style of input parameter specification in order to avoid messy C++ code.
...
not allow the user to input invalid values for a parameter. 
...
least it's type and an ID. This ID can then be used anywhere in the rest of the XML file. To apply the validator to
a parameter, simply add the validatorID attribute to the <Parameter> tag. The same validator can be used on multiple
parameters.

Optika supports several different validators which can be found in the Teuchos package. Below we will discuss each
in detail.

\subsubsection{Enhanced Number Validators}
...
\begin{figure}
  ...
  \begin{lstlisting}
  <ParameterList>
    <Parameter name="Double Parameter" type="double" docString="A double parameter"
      validatorId="1" />
      ...Other Parameters ....
    <Validators>
      <Validator type="EnhancedNumberValidator(int)" min="0" max="10" step=".5" precision="2"/
        validatorId="1"/>
    </Validators>
 </ParameterList>
 \end{lstlisting}

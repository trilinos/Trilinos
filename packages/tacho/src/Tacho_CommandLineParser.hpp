...
namespace Tacho {
    ...
    template<> Option<bool>::Option(std::string desc, bool val)
      :  _desc(desc), _type("bool"), _val(std::to_string(val)) {}
    template<> Option<int>::Option(std::string desc, int val)
      :  _desc(desc), _type("int"), _val(std::to_string(val)) {}
    template<> Option<std::string>::Option(std::string desc, std::string val)
      :  _desc(desc), _type("string"), _val(val) {}
    ...
    public:
      ...
      bool parse(int argc, char **argv) {
        ...
        for (int i=1;i<argc;++i) {
          std::string s = argv[i];
          if (s == "--help") help = true;
          if (s == "--echo-command-line") echo = true;
        }

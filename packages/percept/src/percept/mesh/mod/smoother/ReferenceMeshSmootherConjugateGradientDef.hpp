...
//            m_coord_updater->coord_field_lagged);
  ...
  run_one_iteration()
  {
    ...
    PerceptMesh *local_eMesh = Base::m_eMesh;

    bool total_valid=false;

    if (Base::m_iter == 0)
      {
        get_edge_lengths(local_eMesh);
        local_eMesh->nodal_field_set_value("cg_r", 0.0);
        local_eMesh->nodal_field_set_value("cg_d", 0.0);
        local_eMesh->nodal_field_set_value("cg_s", 0.0);
      }
    ...
    /// r = -g
    local_eMesh->nodal_field_axpby(-1.0, "cg_g", 0.0, "cg_r");
    Base::m_dold = local_eMesh->nodal_field_dot("cg_d", "cg_d");
    Base::m_dmid = local_eMesh->nodal_field_dot("cg_r", "cg_d");
    Base::m_dnew = local_eMesh->nodal_field_dot("cg_r", "cg_r");
    if (Base::m_iter == 0)
      {
    ...
    if (Base::m_iter % N == 0 || cg_beta <= 0.0)
      {
        /// s = r
        local_eMesh->copy_field("cg_s", "cg_r");
      }
    else
      {
        /// s = r + beta * s
        local_eMesh->nodal_field_axpby(1.0, "cg_r", cg_beta, "cg_s");
      }

    local_eMesh->copy_field("cg_d", "cg_r");
    bool restarted = false;
    Double alpha = line_search(restarted);
    Double snorm = local_eMesh->nodal_field_dot("cg_s", "cg_s");
    ...
    if (local_eMesh->get_smooth_surfaces())
      {
        snap_nodes();
        if (Base::m_stage != 0)
          {

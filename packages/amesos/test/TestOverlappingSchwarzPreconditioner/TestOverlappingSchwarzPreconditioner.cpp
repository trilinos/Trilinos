...
#include "Amesos_OverlappingSchwarzPreconditioner.h"
#include "AztecOO.h"
...
int main(int argc, char *argv[])
{
  ...
  // need square number of processors becauseof "map_type" == "box"
  switch (Comm.NumProc()) {
  case 1:
  case 4:
  case 9:
  ...
  default:
    if (Comm.MyPID() == 0) 
      cout << "This test must be run with N^2 processors" << endl;
    AMESOS_CHK_ERR(-1);
  }
  ...
  // define AZLU to use the Aztec's LU factorization.
  // Note that this requires AztecOO to be configure with option
  // --enable-aztecoo-azlu
...
#define AZLUz
  ...
  // ============================= //
  // build the Y12M preconditioner //
  // ============================= //
 
  AztecOOSolver.SetAztecOption(AZ_precond, AZ_dom_decomp);
  AztecOOSolver.SetAztecOption(AZ_subdomain_solve, AZ_lu);
  AztecOOSolver.SetAztecOption(AZ_overlap,OverlapLevel);

#else

  // =============================== //
  // build the Amesos preconditioner //
  // =============================== //

  Epetra_Operator* AmesosPrec;
  int NumLocalDomains = 4;

  Teuchos::ParameterList List;
//  List.set("OutputLevel",2);
  Amesos_AmesosInverseFactory InverseFactory;

  AmesosPrec = 
    new Amesos_OverlappingSchwarzPreconditioner("Amesos_Klu", CrsA, 
						InverseFactory,
						OverlapLevel, true,
						NumLocalDomains,
						List);

  AztecOOSolver.SetPrecOperator(AmesosPrec);
#endif
  ...
  // one processor must converge in one iteration only
  if (Comm.NumProc() == 1) {
    if (AztecOOSolver.NumIters() == 1)
      return(EXIT_SUCCESS);
    else
      return(EXIT_FAILURE);
  }
  else {
    if (AztecOOSolver.NumIters() < 100)
      return(EXIT_SUCCESS);
    else
      return(EXIT_FAILURE);
  }

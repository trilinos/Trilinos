...
#
#  AmesosKlu.exe, the Direct Sparse Solver Regresion Test, tests and times 
#  Klu on a MPI build.
#
...
#  suggest an error:
#
grep -v OK SST.summary | grep -v COMMENT | grep " " > /dev/null || echo "End Result: TEST PASSED - Klu test passed on all" $expected_lines[1] " tests"
#
#  This should not generaly print anything as errors should have been caught in the if test above
#
grep -v OK SST.summary  | grep -v COMMENT | grep " " && echo "AmesosKlu Test FAILED" 
exit($status == 0)

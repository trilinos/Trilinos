...
#
#  AmesosTaucs.exe, tests Taucs on an MPI build.
#
#  RETURNS 0, and outputs "End Result: TEST PASSED - Taucs test passed on all nn tests" if the test succeeds and 1 if the test fails 
#
...
#COMMENT      $mpigo  1 amesos_test.exe TAUCS SuperLU.rua 0 1 1 0 1e-14 1e-13
#  where:
#     TAUCS SuperLU.rua - The solver to use and the matrix to solve
...
echo "COMMENT Start AmesosTaucs.exe " > SST.summary 
...
$mpigo  3 amesos_test.exe TAUCS 662_bus_out.rsa 0 1 -3 0 1e-10 1e-12 >>SST.stdout 
$mpigo  2 amesos_test.exe TAUCS nos5.mtx 0 1 3 0 1e-12 1e-9 >>SST.stdout 
$mpigo  2 amesos_test.exe TAUCS nos1.mtx 0 1 3 0 1e-9 2e-6 >>SST.stdout 
$mpigo  2 amesos_test.exe TAUCS bcsstk01.mtx 0 1 3 0 1e-12 2e-5 >>SST.stdout 
$mpigo  2 amesos_test.exe TAUCS bcsstk02.mtx 0 1 3 0 2e-12 2e-10 >>SST.stdout 
$mpigo  2 amesos_test.exe TAUCS bcsstk04.mtx 0 1 3 0 1e-11 2e-8 >>SST.stdout 
$mpigo  2 amesos_test.exe TAUCS KheadK.mtx 0 1 3 0 1e-12 2e-9 >>SST.stdout 
$mpigo  2 amesos_test.exe TAUCS KheadSorted.mtx 0 1 3 0 1e-12 2e-9 >>SST.stdout 
$mpigo  2 amesos_test.exe TAUCS Diagonal.mtx 0 1 3 0 1e-15 2e-15 >>SST.stdout 
...
#
set expected_lines = `grep mpigo AmesosTaucs.csh | grep -v COMMENT | wc`
...
#  suggest an error:
#
grep -v OK SST.summary | grep -v COMMENT | grep " " > /dev/null || echo "End Result: TEST PASSED - Taucs test passed on all" $expected_lines[1] " tests"
#
#  This should not generaly print anything as errors should have been caught in the if test above
#
grep -v OK SST.summary  | grep -v COMMENT | grep " " && echo "AmesosTaucs Test FAILED" 
exit($status == 0)

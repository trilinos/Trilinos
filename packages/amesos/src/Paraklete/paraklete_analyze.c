...

{
	...
	*LUsymbolicHandle = NULL ;
	...
	PR0 ((cm->file, "proc %d root process analyze fails\n", Common->myid)) ;
	return (FALSE) ;
    }

    PR1 ((cm->file, "proc %d in bcast symbolic: cm->status %d header %d %d\n",
	...
	*LUsymbolicHandle = LUsymbolic ;
	...
	PR0 ((cm->file, "proc %d everybody fails in analyze\n", Common->myid)) ;
...
)
{
	...
	*Leaves, *Merged, *Nchildren, *NewNode, *Cparent2, *W ;
    ...
    cm = &(Common->cm) ;
    cholmod_allocate_work (n, 2*n, n, sizeof (double), cm) ;
    ...
    {
	/* out of memory; inform all processes */
	PR0 ((cm->file, "oops, proc 0 ran out\n")) ;
	paraklete_free_symbolic (&LUsymbolic, Common) ;
	MPI (paraklete_bcast_symbolic (&LUsymbolic, Common)) ;
	return (NULL) ;
    }
    ...
    AT = cholmod_transpose (A, FALSE, NULL, NULL, 0, cm) ;
    C = cholmod_add (A, AT, one, one, FALSE, FALSE, cm) ;
    cholmod_free_sparse (&AT, cm) ;
    cholmod_band_inplace (C, 0, n, 0, cm) ;

    if (cm->status != CHOLMOD_OK)
    {
	/* out of memory; inform all processes */
	PR0 ((cm->file, "oops, proc 0 ran out here2\n")) ;
    ...
    {
	/* out of memory; inform all processes */
	PR0 ((cm->file, "oops, proc 0 ran out here3\n")) ;
	paraklete_free_symbolic (&LUsymbolic, Common) ;
	cholmod_free_sparse (&C, cm) ;
	MPI (paraklete_bcast_symbolic (&LUsymbolic, Common)) ;
	return (NULL) ;
    ...
    Elo = cholmod_transpose (C, FALSE, Cperm, NULL, 0, cm) ;
    cholmod_free_sparse (&C, cm) ;
    Eup = cholmod_transpose (Elo, FALSE, NULL, NULL, 0, cm) ;
    ...
    {
	/* out of memory; inform all processes */
	PR0 ((cm->file, "oops, proc 0 ran out here4\n")) ;
	paraklete_free_symbolic (&LUsymbolic, Common) ;
	cholmod_free_sparse (&Eup, cm) ;
	cholmod_free_sparse (&Elo, cm) ;
	cholmod_free (W, 5*(n+1), sizeof (int), cm) ;
	MPI (paraklete_bcast_symbolic (&LUsymbolic, Common)) ;
	return (NULL) ;
    }
    ...
    cholmod_postorder (Parent, n, Post, cm) ;
	...
	PR0 ((cm->file, "oops, proc 0 ran out here5\n")) ;
	paraklete_free_symbolic (&LUsymbolic, Common) ;
	cholmod_free (W, 5*(n+1), sizeof (int), cm) ;
	MPI (paraklete_bcast_symbolic (&LUsymbolic, Common)) ;
	return (NULL) ;
    ...
    for (c = 0 ; c < ncomponents ; c++)
    {
	PR1 ((cm->file, "Node %d work %g parent %d\n",
		    c, Cwork [c], Cparent [c])) ;
    }
#endif
    ...
    for (c = 0 ; c < ncomponents ; c++)
    {
	PR1 ((cm->file, "Node %d has %d children\n", c, Nchildren [c])) ;
	if (Nchildren [c] == 0)
	{
	    PR1 ((cm->file, "Leaf: %d\n", c)) ;
	    Leaves [nleaves++] = c ;
	...
	ASSERT (c != EMPTY) ;
	PR2 ((cm->file, "Lightest leaf is %d with work %g\n", c, Cwork [c])) ;
	...
	PR1 ((cm->file, "merge %d into %d, parent %d\n", c, cmerge, parent)) ;
	...
	if (Nchildren [parent] == 0 && parent != ncomponents)
	{
	    ...
	    PR1 ((cm->file, "parent is new leaf: %d\n", parent)) ;
	...
	if (Merged [c] == EMPTY)
	{
	    PR1 ((cm->file, "Live node %d becomes node %d\n", c, ncomp2)) ;
	    NewNode [c] = ncomp2++ ;
	}
    }
    for (c = 0 ; c < ncomponents ; c++)
	...
	if (Merged [c] != EMPTY)
	{
	    ...
	    PR1 ((cm->file, "Dead node %d becomes part of node %d\n",
			c, NewNode [c])) ;
	}
    }
    ...
    for (c = 0 ; c < ncomponents ; c++)
    {
	PR1 ((cm->file, "New node: %d new parent %d\n", c, Cparent [c])) ;
    }
    ...
    for (c = 0 ; c < ncomponents ; c++)
    {
	PR1 ((cm->file, "Node %d children: ", c)) ;
	for (cp = Childp [c] ; cp < Childp [c+1] ; cp++)
	{
	    PR1 ((cm->file, "%d ", Child [cp])) ;
	}
	PR1 ((cm->file, "\n")) ;
    }
#endif
	...
	if (nchild == 0)
	{
	    ...
	    {
		PR1 ((cm->file, "  node %d to process %d\n", cc, proc)) ;
		Sched [cc] = proc ;
	    }
	    proc++ ;
	}
    ...
    for (c = 0 ; c < ncomponents ; c++)
    {
	PR0 ((cm->file, "    node %d Sched %d : Cparent %d proc %d\n",
		    c, Sched [c], Cparent [c],
...
)
{
    ...
    cholmod_free (LUsymbolic->Mem_n, 3*n, sizeof (int), cm) ;
    LUsymbolic->Cperm = NULL ; 
    LUsymbolic->Cinv = NULL ; 
    LUsymbolic->Cparent = NULL ; 

    cholmod_free (LUsymbolic->Mem_c, 7*ncomponents + 2, sizeof (int), cm);
    LUsymbolic->Cstart = NULL ;
    LUsymbolic->Child = NULL ;
    LUsymbolic->Childp = NULL ;
    LUsymbolic->Clnz = NULL ;
    ...
    *LUsymbolicHandle = cholmod_free ((*LUsymbolicHandle),
	    1, sizeof (paraklete_symbolic), cm) ;
}

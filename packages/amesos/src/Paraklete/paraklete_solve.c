...
)
{
	...
	if (myid == 0)
	{
	    PR1 ((cm->file, "get soln, node c = %d, nfound %d\n", c, nfound)) ;
	    /* get X from node c */
	    if (Sched [c] != myid)
	    {
		PR1 ((cm->file, "recv node %d from %d\n", c, Sched [c])) ;
	    ...
	    {
		PR1 ((cm->file, "I own it already\n")) ;
		X2 = X ;
	    }
	    PR1 ((cm->file, "got it from Sched [c] = %d\n", Sched [c])) ;
	    for (i = 0 ; i < nfound ; i++)
	    {
		B [Q [k]] = X2 [i] ;
		PR2 ((cm->file, "X [%d] is global B [%d] %g\n", i, k, X2 [i])) ;
		k++ ;
	    }
	}
	else
	{
	    if (Sched [c] == myid)
	    {
		PR1 ((cm->file, "send soln, node c = %d, myid %d nfound %d\n",
			    c, myid, nfound)) ;
		MPI (MPI_Send (X, nfound, MPI_DOUBLE, 0,
			    TAG0, MPI_COMM_WORLD)) ;
	    }
	}
    }

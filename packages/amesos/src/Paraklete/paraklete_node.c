...
)
{
	...
	LU->LUnode [child]->sx = cholmod_free (LU->LUnode [child]->sx,
		LU->LUnode [child]->PK_SSIZE, sizeof (double), cm) ;
	LU->LUnode [child]->sp = cholmod_free (LU->LUnode [child]->sp,
		sn, sizeof (int), cm) ;
	LU->LUnode [child]->slen = cholmod_free (LU->LUnode [child]->slen,
		sn, sizeof (int), cm) ;
    }
}
...
)
{
    ...
    LUnode->W2 = cholmod_free (LUnode->W2, 2*(LUnode->nlost_in), sizeof (int),
	    cm) ;
	...
	PR1 ((cm->file, "proc %d sends header parent proc %d\n",
	...
	PR1 ((cm->file, "proc %d wait for parent %d\n",
		Common->myid, parent_id)) ;
	...
	LUnode->sx = cholmod_free (LUnode->sx, LUnode->PK_SSIZE,
		sizeof (double), cm) ;
	LUnode->sp = cholmod_free (LUnode->sp, LUnode->PK_SN,
		sizeof (int), cm) ;
	LUnode->slen = cholmod_free (LUnode->slen, LUnode->PK_SN,
		sizeof (int), cm) ;
    }
...
)
{
	...
	*Sched, *Pinv ;
...
#ifndef NDEBUG
    PR1 ((cm->file, "proc %d node %d nchild %d\n", myid, c, nchild)) ;
    {
	int cc ;
	for (cc = 0 ; cc < LUsymbolic->ncomponents ; cc++)
	{
	    ...
	    {
		PR2 ((cm->file, "node %d: child node %d\n",
    ...
    PR1 ((cm->file, "proc %d posting recv at node %d, nchild %d status %d\n",
	    myid, c, nchild, cm->status)) ;
    nmessages = 0 ;
    for (cp = 0 ; cp < nchild ; cp++)
    {
	child = Child [Childp [c] + cp] ;
	PR2 ((cm->file, "proc %d child %d owned by %d\n",
		    myid, child, Sched [child])) ;
	if (Sched [child] != myid)
	{
	    PR2 ((cm->file, "parent proc %d awaits header from %d\n",
		    myid, Sched [child])) ;
	...
	PR1 ((cm->file, "parent proc %d replies to proc %d\n",
		    myid, Sched [child])) ;
    ...
    PR1 ((cm->file, "proc %d node %d arrowhead again\n", myid, c)) ;
    ...
    if (!ok)
    {
	PR0 ((cm->file, "proc %d, node %d, report failure to parent: %d\n",
		myid, c, parent_id)) ;
    ...
    /* get workspace */
    cholmod_allocate_work (cn, 3*cn, cn, sizeof (double), cm) ;
	...
	PR0 ((cm->file, "node %d proc %d fails here\n", c, myid)) ;
	paraklete_send_to_parent (c, PK_OUT_OF_MEMORY, parent_id,
		LU, LUsymbolic, Common) ;
	return (FALSE) ;
    }
	...
	PR1 ((cm->file, "Cn [c] is %d\n", Cn [c])) ;
	PR1 ((cm->file, "child is %d\n", child)) ;
	PR1 ((cm->file, "Lost[cp] is %d\n", Lost [cp])) ;
	PR1 ((cm->file, "sn %d  Lost[cp]+Cn[cn] %d\n", sn, Lost [cp] + Cn [c]));
	...
	for (j = 0 ; j < Lost [cp] ; j++)
	{
	     ...
	     * k of C */
	    PR2 ((cm->file, "lost child col %d becomes col %d of C\n", j, k)) ;
	    Cp [k] = cnz ;
		...
		Ci [cnz] = ci ;
		Cx [cnz] = Sx [p] ;
		PR3 ((cm->file, "  Lost child col: row %d newrow %d value %g\n",
			i, ci, Sx [p])) ;
		cnz++ ;
	    }
	...
	for (p = Ap [j] ; p < Ap [j+1] ; p++)
	{
	    i = Ai [p] ;
	    ci = i + nlost_in ;
	    PR3 ((cm->file, "scatter original (%d,%d) %g to (%d,%d)\n",
			i, j, Ax [p], ci, cj)) ;
	    Flag [ci] = mark ;
	    Ci [cnz++] = ci ;
	    W [ci] = Ax [p] ;
    ...
    cholmod_reallocate_sparse (C, cnz, cm) ;
    ...
    PR1 ((cm->file, "allocated LUnode->ix: size %d\n", LUnode->lusize)) ;
    ...
    PR0 ((cm->file, "============= NODE %d NPIV %d FOUND %d LOST %d\n", c,

...

{
	...
	*Cn, *Pinv, *Li, *Cparent, *Sched ;
    ...
    X = LU->LUnode [c]->X ;
    PR1 ((cm->file, "Lsolve at Node %d, cn %d\n", c, cn)) ;
    for (i = 0 ; i < cn ; i++)
    {
	X [i] = 0 ;
    }
	...
	ci = i + nlost_in ;
	k = Pinv [ci] ;
	PR2 ((cm->file, "original B [%d] = %g goes to X [%d]\n", i, B [i], k)) ;
	ASSERT (k >= 0 && k < cn) ;
	X [k] = B [i] ;
    }
	...
	for (cp = 0 ; cp < nchild ; cp++)
	{
		...
		PR2 ((cm->file, "Xchild [%d] = %g goes to X [%d] (lost)\n",
			i, Xchild [i], k)) ;
		ASSERT (k >= 0 && k < cn) ;
		X [k] += Xchild [i] ;
	    }
		...
		k = Pinv [ci] ;		/* kth local pivot row */
		PR2 ((cm->file, "Xchild [%d] = %g goes to X [%d] (cand)\n",
			i, Xchild [i], k)) ;
		ASSERT (k >= 0 && k < cn) ;
		X [k] += Xchild [i] ;
	    }
		...
		PR2 ((cm->file, "Xchild [%d] = %g goes to X [%d] (anc)\n",
			i, Xchild [i], k)) ;
		ASSERT (k >= 0 && k < cn) ;
		X [k] += Xchild [i] ;
	    }
    ...
    {
	MPI (MPI_Isend (X, cn, MPI_DOUBLE, Sched [parent], TAG0, MPI_COMM_WORLD, &req)) ;
    }

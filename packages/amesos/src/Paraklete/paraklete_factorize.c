...
)
{
    ...
    {
	/* out of memory */
	PR0 ((cm->file, "proc %d LU header failed\n", myid)) ;
	return (NULL) ;
    }
    ...
    {
	/* out of memory */
	PR0 ((cm->file, "proc %d LU contents failed\n", myid)) ;
...
)
{
    ...
    cholmod_free (LU->P,    LU->n, sizeof (int), cm) ;
    cholmod_free (LU->Q,    LU->n, sizeof (int), cm) ;
    cholmod_free (LU->Pinv, LU->n, sizeof (int), cm) ;
    cholmod_free (LU->Qinv, LU->n, sizeof (int), cm) ;
    cholmod_free (LU->W,    LU->n, sizeof (double), cm) ;
    cholmod_free (LU->Ep2,  LU->n+1, sizeof (int), cm) ;
	...
	for (c = 0 ; c < LU->ncomponents ; c++)
	{
		...
		PR2 ((cm->file, "proc %d node %d free numeric, nk %d B %p\n",
			Common->myid, c, LUnode->nk, (void *) (LUnode->B))) ;
		cholmod_free (LUnode->B, LUnode->nk, sizeof (double), cm) ;
		cholmod_free (LUnode->X, LUnode->PK_NN, sizeof (double), cm) ;

		/* LU factors at this node */
		cholmod_free (LUnode->llen, LUnode->PK_NPIV, sizeof (int), cm) ;
		cholmod_free (LUnode->lp, LUnode->PK_NPIV, sizeof (int), cm) ;
		cholmod_free (LUnode->ulen, LUnode->PK_NN, sizeof (int), cm) ;
		cholmod_free (LUnode->up, LUnode->PK_NN, sizeof (int), cm) ;
		cholmod_free (LUnode->ix, LUnode->lusize, sizeof (double), cm) ;
		cholmod_free (LUnode->Lost, LUnode->nchild, sizeof (int), cm) ;
		cholmod_free (LUnode->Lostp, LUnode->nchild+1, sizeof (int),
			cm) ;
		MPI (cholmod_free (LUnode->Req, LUnode->nchild,
			    sizeof (MPI_Request), cm)) ;

		/* P and Q at this node */
		cholmod_free (LUnode->Pglobal, LUnode->PK_NPIV, sizeof (int),
			cm) ;
		cholmod_free (LUnode->Qglobal, LUnode->PK_NPIV, sizeof (int),
			cm) ;
		cholmod_free (LUnode->Plocal, LUnode->PK_NPIV, sizeof (int),
			cm) ;
		cholmod_free (LUnode->Qlocal, LUnode->PK_NPIV, sizeof (int),
			cm) ;
		cholmod_free (LUnode->Pinv, LUnode->PK_NPIV, sizeof (int), cm) ;
		cholmod_free (LUnode->Qinv, LUnode->PK_NPIV, sizeof (int), cm) ;

		/* Schur complement of this node */
		cholmod_free (LUnode->sp, LUnode->PK_SN, sizeof (int), cm) ;
		cholmod_free (LUnode->slen, LUnode->PK_SN, sizeof (int), cm) ;
		cholmod_free (LUnode->sx, LUnode->PK_SSIZE, sizeof (double),
			cm) ;
		...
		cholmod_free (LUnode->W2, 2*(LUnode->nlost_in), sizeof (int),
			cm) ;

		/* free the LUnode itself */
		cholmod_free (LUnode, 1, sizeof (paraklete_node), cm) ;
	    }
	}
    }

    cholmod_free (LU->LUnode, LU->ncomponents, sizeof (paraklete_node *), cm) ;
    *LUHandle = cholmod_free ((*LUHandle), 1, sizeof (paraklete_numeric), cm) ;
}
...
)
{
    ...
    {
	/* out of memory */
	PR0 ((cm->file, "failed to allocate E\n")) ;
	return (NULL) ;
    }
    ...
    {
	/* out of memory */
	PR0 ((cm->file, "out of memory to sort E\n")) ;
...
)
{
	...
	*Cstart, *P, *Q, *Cp, *Ci, *Pc, *Qc, *Pinv, *Qinv, *Sched ;
    ...
    PR0 ((cm->file, "in factor: proc %d my_tries %d\n", myid, my_tries)) ;

#ifndef NDEBUG
    for (c = 0 ; c < ncomponents ; c++)
    {
	PR1 ((cm->file, "proc: %d node %d: %d to %d, parent %d Sched %d\n",
    ...
    ok = (LU != NULL) ;
    PR1 ((cm->file, "factor proc %d alloc ok: %d\n", myid, ok)) ;
	...
	for (c = 0 ; c < ncomponents ; c++)
	{
	    PR1 ((cm->file, "distribute node c = %d\n", c)) ;
	    cn = Cn [c] ;
	    ...
	    {
		PR2 ((cm->file, "ancestor %d, for c %d, ncomp %d\n",
			a, c, ncomponents)) ;
		ASSERT (a >= c && a < ncomponents) ;
		k1 = Cstart [a] ;
		k2 = Cstart [a+1] ;
		PR2 ((cm->file, "k1 %d k2 %d\n", k1, k2)) ;
		for (k = k1 ; k < k2 ; k++)
		{
		    /* global index k becomes local index j */
		    PR3 ((cm->file, "  global: %d local %d\n", k, cj)) ;
		    Map [k] = cj++ ;
		}
	    }
	    ASSERT (cj == cn) ;
    ...
    if (myid == 0)
    {
	LU->Ep2 = cholmod_free (LU->Ep2, n+1, sizeof (int), cm) ;
	cholmod_free_sparse (&(LU->E), cm) ;
    }
    ...
    PR1 ((cm->file, "proc %d everybody OK so far2\n", Common->myid)) ;
    cm->print = 5 ;
    if (myid == 0)
    {
	for (c = 0 ; c < ncomponents ; c++)
	...
	if (Sched [c] == myid)
	{
	    PR1 ((cm->file, "proc %d Node %d original matrix:\n", myid, c)) ;
	...
	if (Sched [c] == myid)
	{
	    PR1 ((cm->file, "proc %d doing node %d\n", myid, c)) ;
    ...
    PR1 ((cm->file, "proc %d here in PQ: ok %d\n", Common->myid, ok)) ;
    all_ok = ok ;
    MPI (MPI_Allreduce (&ok, &all_ok, 1, MPI_INT, MPI_LAND, MPI_COMM_WORLD)) ;
    if (!all_ok)
    {

...
{ 
	...
	*Lprune ;
    ...
    cholmod_allocate_work (n, 3*n, n, sizeof (double), cm) ;
	...
	*/
	...
	if (lup + ngrow > (int) lusize)
	{
	    ...
	    PR1 ((cm->file, "growing LU from %d to %d\n", lusize, new)) ;
	    LU = cholmod_realloc (LU, &lusize, new, sizeof (double), cm) ;
	    LUnode->ix = LU ;
	...
	while (!found && qhead != qtail)
	{
	    ...
	    PR1 ((cm->file, "k %d, diagrow = %d, UNFLIP(diagrow) = %d\n",
		...
		if (!second_try)
		{
		    PR1 ((cm->file, "requeue kcol %d for 2nd try\n", kcol)) ;
		    Queue [qtail] = FLIP (kcol) ;
		    qtail = (qtail + 1) % (npiv + 1) ;
		}
		else
		{
		    PR1 ((cm->file, "discarding kcol %d\n", kcol)) ;
		}
		PR1 ((cm->file, "restore lup from %d to %d\n", lup_orig, lup)) ;
		lup = lup_orig ;
	    }
	    else
	    {
		...
		Q [nfound++] = kcol ;
		PR1 ((cm->file, "found kcol: %d nfound %d\n", kcol, nfound)) ;
	    }
	...
	GET_COLUMN (Uip, Ulen, LU, k, Ui, Ux, ulen) ;
	PR1 ((cm->file, "k %d Ulen[k] %d\n", k, Ulen [k])) ;
	ASSERT (ulen > 0) ;
	Ui [ulen-1] = k ;
	Ux [ulen-1] = pivot ;
	PR1 ((cm->file, "pivot: Ui %d Ux %g\n", Ui [ulen-1], Ux [ulen-1])) ;
	...
	if (pivrow != diagrow)
	{
	    ...
	    noffdiag++ ;
	    PR1 ((cm->file, ">>>>>>>>>>>>>>>>> pivrow %d k %d off-diagonal\n",
			pivrow, k)) ;
	    if (Pinv [diagrow] < 0)
	    {
	...
	for (p = 0 ; p < ulen ; p++)
	{
	    PR2 ((cm->file, "Column %d of U: %d : %g\n", k, Ui [p], Ux [p])) ;
	}
	GET_COLUMN (Lip, Llen, LU, k, Li, Lx, llen) ;
	for (p = 0 ; p < llen ; p++)
	{
	    PR2 ((cm->file, "Column %d of L: %d : %g\n", k, Li [p], Lx [p])) ;
	}
#endif
    ...
    for (i = 0 ; i < npiv ; i++)
    {
	if (i == nfound) PR1 ((cm->file, "----------- nfound = %d\n", nfound)) ;
	if (i == npiv  ) PR1 ((cm->file, "----------- npiv   = %d\n", npiv  )) ;
	PR2 ((cm->file, "P [%d] = %d Pinv[%d] = %d\n", i, P [i], i, Pinv [i])) ;
	PR2 ((cm->file, "Q [%d] = %d Qinv[%d] = %d\n", i, Q [i], i, Qinv [i])) ;
    }
#endif
	...
	PR2 ((cm->file, "lup is now %d\n", lup)) ;

#ifndef NDEBUG
	GET_COLUMN (Uip, Ulen, LU, k, Ui, Ux, ulen) ;
	for (p = 0 ; p < ulen ; p++)
	{
	    PR2 ((cm->file, "Column %d of U2: %d : %g\n", k, Ui [p], Ux [p])) ;
	}
#endif
    ...
    LU = cholmod_realloc (LU, &lusize, lup, sizeof (double), cm) ;
    LUnode->ix = LU ; 
    LUnode->lusize = lusize ;
    ...
    PR1 ((cm->file, "allocate S, : ssize %d sn %d \n", ssize, sn)) ;
	...
	if (sp + ngrow > (int) ssize)
	{
	    ...
	    S = cholmod_realloc (S, &ssize, new, sizeof (double), cm) ;
	    LUnode->sx = S ;
	    LUnode->PK_SSIZE = ssize ;
	    if (cm->status != CHOLMOD_OK)
	    {
	...
	GET_COLUMN (Uip, Ulen, LU, k, Ui, Ux, ulen) ;
	PR2 ((cm->file, "Multiply L2 by U2(:,%d) :\n", k)) ;
	...
	for (p = 0 ; p < ulen ; p++)
	{
		...
		if (Flag [i] < mark)
		{
		    PR3 ((cm->file, "    new entry in Si, slen = %d\n", slen)) ;
		    Si [slen++] = i - nfound ;
		    Flag [i] = mark ;
		}
		X [i] -= Lx [p2] * ujk ;
	...
	PR2 ((cm->file, "snz so far: %d.  Final col of S(:,%d) = \n", snz, k)) ;
	for (p = 0 ; p < slen ; p++)
	{
	    i = Si [p] + nfound ;
	    Sx [p] = X [i] ;
    ...
    PR0 ((cm->file, "Final ssize = %d sn = %d snz: %d\n", sp, sn, snz)) ;
    S = cholmod_realloc (S, &ssize, sp, sizeof (double), cm) ;
    LUnode->sx = S ; 
    LUnode->PK_SNZ = snz ; 
    LUnode->PK_SSIZE = ssize ;

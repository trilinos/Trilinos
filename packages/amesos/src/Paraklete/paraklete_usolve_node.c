...
)
{
	...
	*Cn, *Qinv, *Ui, *Sched, *Cparent ;
    ...
    PR1 ((cm->file, "Usolve node %d cn %d nfound %d\n", c, cn, nfound)) ;

    if (parent != EMPTY && Sched [parent] != myid)
    {
	PR1 ((cm->file, "Recving usolve from %d, size %d\n", Sched [parent],
		cn - nfound)) ;
	...
	PR1 ((cm->file, "child %d cn %d nfound %d\n",
	...
	for (i = 0 ; i < Lost [cp] ; i++)
	{
	    ...
	    PR2 ((cm->file, "Xchild [%d] = %g from X [%d] (lost)\n",
		    i, Xchild [i], k)) ;
	    Xchild [i] = X [k] ;
	}
	...
	for ( ; i < Lost [cp] + (k2-k1) ; i++)
	{
	    ...
	    k = Qinv [ci] ;		/* kth local pivot col */
	    PR2 ((cm->file, "Xchild [%d] = %g from X [%d] (cand)\n",
		    i, Xchild [i], k)) ;
	    Xchild [i] = X [k] ;
	}
	...
	for ( ; i < cn_child - cn_nfound ; i++)
	{
	    ...
	    PR2 ((cm->file, "Xchild [%d] = %g from X [%d] (anc)\n",
		    i, Xchild [i], k)) ;
	    Xchild [i] = X [k] ;
	}

	PR1 ((cm->file, "Usolve: Sending from %d to child %d\n", c, child)) ;
	if (Sched [child] != myid)
	{
	    PR1 ((cm->file, "Sending to %d, size %d\n", Sched [child], 
		    cn_child - cn_nfound)) ;

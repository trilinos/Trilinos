 ...
 *
 * If inplace is TRUE, then the matrix A is modified in place.  In this case,
 * C is returned as a pointer to A.  Be careful regarding memory leaks/faults in
 * this case: either A or C should should be freed with cholmod_free_sparse,
 * but not both.  Only packed matrices can be converted in place.
 *
...
(
    /* input, or both input/output if inplace is TRUE */
    cholmod_sparse *A,
...
)
{
	...
	cholmod_error (CHOLMOD_INVALID,
		"cholmod_band: cannot operate on unpacked matrix in-place",
		Common) ;
	return (NULL) ;
    }
     ...
     * avoid possible int overflow if k1 and k2 are huge */
    k1 = MAX (-nrow, k1) ;
    k1 = MIN (k1, ncol) ;
    k2 = MAX (-nrow, k2) ;
    k2 = MIN (k2, ncol) ;
	...
	C = cholmod_allocate_sparse (A->nrow, ncol, nz, sorted, TRUE,
		A->stype, values, Common) ;
	...
	(void) cholmod_reallocate_sparse (A, nz, Common) ;
    ...
    ASSERT (Common->malloc_count == orig + (inplace) ? 0 : ((values) ? 4 : 3)) ;
    DEBUG (nnzdiag = cholmod_dump_sparse (C, "band", Common)) ;
...
cholmod_sparse *cholmod_band	/* returns C, or NULL if failure */
(
...
int cholmod_band_inplace    /* returns TRUE if successful, FALSE otherwise */
(
    /* input/output */
    cholmod_sparse *A,

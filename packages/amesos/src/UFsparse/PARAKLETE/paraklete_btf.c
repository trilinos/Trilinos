...
)
{
	...
	*LU_btf_symbolic_handle = NULL ;
	...
	LU_btf_symbolic = paraklete_btf_alloc_symbolic (n, nblocks, Common) ;
	*LU_btf_symbolic_handle = LU_btf_symbolic ;
	...
	paraklete_btf_free_symbolic (&LU_btf_symbolic, Common) ;
	*LU_btf_symbolic_handle = NULL ;
	return (FALSE) ;
    }
...
paraklete_btf_symbolic *paraklete_btf_alloc_symbolic
(
    Int n,
    Int nblocks,
    paraklete_common *Common
...
paraklete_btf_symbolic *paraklete_btf_analyze
(
...
)
{
	...
	*Ai = NULL, *Cp = NULL, *Ci = NULL, *Pbinv = NULL ;
	...
	LU_btf_symbolic = paraklete_btf_alloc_symbolic (n, n, Common) ;
        if (LU_btf_symbolic == NULL)
        {
	...
	else
	{
	    ...
	    LUsymbolic [block] = (void *) paraklete_analyze (C, Common) ;
...
paraklete_btf_numeric *paraklete_btf_factorize
(
    cholmod_sparse *A,				/* matrix to analyze */ 
...
)
{
	...
	*Ci = NULL, *Pbinv = NULL, *Fp = NULL, *Fi = NULL ;
	...
	{
            /* parallel factorization of a large block */
	    LUnumeric [block] = (void *) paraklete_factorize (C,
	...
	paraklete_btf_free_numeric (&LU_btf_numeric, Common) ;
    }
...
Int paraklete_btf_solve                 /* TRUE if OK, FALSE otherwise */
(
...
void paraklete_btf_free_symbolic
(
    paraklete_btf_symbolic **LU_btf_symbolic_handle,
    paraklete_common *Common
)
	...
	*LU_btf_symbolic_handle = NULL ;
	...
	nk = k2 - k1 ;
        ...
        else
        {
            ...
            paraklete_free_symbolic (&S, Common) ;
        }
        LUsymbolic [block] = NULL ;
    }
...
void paraklete_btf_free_numeric
(
    ...
    for (block = 0 ; block < nblocks ; block++)
    {
        ...
        if (N != NULL)
        {
            if (N->magic == PARAKLETE_MAGIC)
            {
                ...
                paraklete_free_numeric (&N, Common) ;
            }
            else
            {

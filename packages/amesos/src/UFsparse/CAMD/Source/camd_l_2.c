...
*/
    ...
    Int curC, pBucket, pBucket2, degreeListCounter, *Bucket, *Perm ;
...
/* CountingSort BucketSet[] based on C[], It is O(n) linear time */
	...
	*/
	...
	if (deg > dense || deg == 0)
	{
	    ...
	    if (deg > dense)
	    {
		CAMD_DEBUG1 (("Dense node "ID" degree "ID"\n", i, deg)) ;
		ndense++ ;
	    }
	    else
	    {
		CAMD_DEBUG1 (("Empty node "ID" degree "ID"\n", i, deg)) ;
		nnull++ ;
	    }
	    Pe [i] = FLIP (n) ;
...
#ifndef NDEBUG
	CAMD_DEBUG3 (("The postordering with dense/empty nodes are :\n")) ;
	for (i = 0 ; i < n ; i++)
	{
	    ASSERT (Perm [i] >= 0 && Perm [i] < n) ;
	    CAMD_DEBUG3 (("Perm ["ID"] ="ID"\n", i, Perm [i])) ;
	}
#endif

	/* merge postordering and dense/empty nodes */
	if (C != NULL)
	{
	    /* dense (or empty) nodes exist, AND C also exists */
	    p1 = 0 ;
	    p2 = k ;
	    p3 = 0 ;
	    while (p1 < k && p2 < n)
	    {
		if (C [Perm [p1]] <= C [Perm [p2]])
		{
		    Last [p3++] = Perm [p1++] ;
		}
		else

...
user will still have the {\em same} Amesos interface to all of them.
\end{remark}
...
\verb!SetParameters()!, and use the default values.
...
changed.
%\footnote{The values of {\tt A} must
...
they must call \verb!SymbolicFactorization()!. 
...
to configure \amesos\ with KLU support. KLU and LAPACK are the only interface
that are
...
of the given Epetra\_LinearProblem. If the single precision interface is 
enabled, this automatically disables the double-precision one.

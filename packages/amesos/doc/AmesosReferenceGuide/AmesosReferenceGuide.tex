...
\begin{table}[tbhp]
  ...
  \caption{Supported interfaces. ``serial'' means that the supported
    direct solver will be used on process 0 (also for distributed linear
    systems; Amesos will take care of data redistribution), and
    ``parallel'' that a subset or all the processes in the current
    communicator will be used by the solver. ``unsym'' means general unsymmetric
...
\vskip .1in \choicebox{\tt MatrixType}{Set it to {\tt SPD} if the matrix
  ...
  to {\tt general} is the matrix is general unsymmetric. Only the MUMPS
  interface can take advantage of {\tt SPD} and {\tt symmetric}. }

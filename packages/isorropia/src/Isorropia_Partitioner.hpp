...
public:
  ...
  */
  int newNumMyElements() const;

  /** Fill user-allocated list (of length len) with the
      global element ids to be located on the local processor
      in the new partitioning.
  */
  void myNewElements(int* elementList, int len) const;

private:
  Teuchos::RefCountPtr<Epetra_Map> new_map_;

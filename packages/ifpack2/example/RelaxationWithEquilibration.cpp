...
int getRankSafelyFromTpetraDistObject(const TpetraDistObjectType& X) {
...
deep_copy(Epetra_Vector& X_e,
  ...
  if (X_e_lcl_raw == nullptr && lclNumRows != 0) {
    ...
    errStrm << "Proc " << myRank << ": deep_copy(Vector): "
                                    ...
                                    "even though the object's Map reports nonzero ("
            << lclNumRows << ") "
                             "rows on the calling process."
            << std::endl;
    return {lclErrCode, errStrm.str()};
  }
...
deep_copy(Epetra_MultiVector& X_e,
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(X_t.getNumVectors() != std::size_t(numCols), std::invalid_argument,
...
tpetraToEpetraVector(const Tpetra::MultiVector<double, LO, GO, NT>& X_t,
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(X_t.getNumVectors() != std::size_t(1), std::invalid_argument,
                             "This function requires that X_t have only one column.");

  const int lclNumRows = map_e.NumMyElements();
  TEUCHOS_TEST_FOR_EXCEPTION(lclNumRows != static_cast<int>(X_t.getLocalLength()),
                             std::runtime_error, "X_t.getLocalLength() = " << X_t.getLocalLength() << " != map_e.NumMyElements() = " << lclNumRows << ".");
...
tpetraToEpetraCrsMatrix(const Tpetra::CrsMatrix<double, LO, GO, NT>& A_t,
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(gblErrCode != 0, std::runtime_error,
                             "tpetraToEpetraCrsMatrix: "
...
tpetraToEpetraLinearSystem(const Tpetra::CrsMatrix<double, LO, GO, NT>& A_t,
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(!A_t.isFillComplete(), std::invalid_argument,
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(comm_t.is_null(), std::runtime_error, "Tpetra's Comm is null.");

  const bool domMapsSame = A_t.getDomainMap()->isSameAs(*(x_t.getMap()));
  const bool ranMapsSame = A_t.getRangeMap()->isSameAs(*(b_t.getMap()));
  TEUCHOS_TEST_FOR_EXCEPTION(!domMapsSame, std::invalid_argument,
                             "Domain Map of input matrix A "
                             "is not the same as the Map of input vector x.");
  TEUCHOS_TEST_FOR_EXCEPTION(!ranMapsSame, std::invalid_argument,
                             "Range Map of input matrix A "
...
solveEpetraLinearSystemWithAztecOO(std::tuple<Epetra_CrsMatrix, Epetra_Vector, Epetra_Vector>& sys,
  ...
  if (upperSolverType == "BICGSTAB") {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(true, std::invalid_argument, "Linear solver type \"" << solverType << "\" not currently supported for AztecOO.");
  }
...
template <class MV>
bool AZ_breakdown_f(MV& v, MV& w, const typename MV::dot_type v_dot_w) {
  using STS = Teuchos::ScalarTraits<typename MV::scalar_type>;
...
template <class SC, class LO, class GO, class NT>
LO gatherAndDensify(HostDenseMatrix<SC, LO, GO, NT>& A_dense,
...
void solveLeastSquaresProblemAndReport(DenseMatrixType A,
                                       DenseMatrixType B,
...
template <class DenseMatrixType>
void findEigenvaluesAndReport(DenseMatrixType A) {
  using std::cerr;
  using std::cout;
  using std::endl;
...
template <class MultiVectorViewType,
          ...
          class IndexType>
void elementWiseMultiply(const MultiVectorViewType& X,
...
void elementWiseMultiplyMultiVector(MultiVectorType& X,
                                    const ScalingFactorsViewType& scalingFactors,
                                    const bool takeSquareRootsOfScalingFactors,
                                    const bool takeAbsoluteValueOfScalingFactors =
                                        !std::is_same<
...
class BelosIfpack2Solver {
 private:
  ...
  void equilibrateMatrix() {
    TEUCHOS_TEST_FOR_EXCEPTION(A_.get() == nullptr, std::runtime_error,
                               "Solver: You must call "
...
void solveAndReport(BelosIfpack2Solver<CrsMatrixType>& solver,
  ...
  if (args.useAztecOO) {
    if (comm->getRank() == 0) {
      cerr << "In order to solve AztecOO, you must have built with the AztecOO "
              ...
              "and Trilinos_ENABLE_Epetra:BOOL=ON set."
           << endl;
    }
    return EXIT_FAILURE;
  }

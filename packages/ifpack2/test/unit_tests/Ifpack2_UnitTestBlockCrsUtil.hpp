...
struct BlockCrsMatrixMaker {
  ...
  struct StructuredBlock {
    ...
    KOKKOS_INLINE_FUNCTION GO ijk2id(const Int i, const Int j, const Int k) const {
      return (keep_same_gids_ || contiguous_ ? (i * nj + j) * nk + k : (k * nj + j) * ni + i);
    }
  ...
  static StructuredBlockPart make_StructuredBlockPart(
    ...
    TEUCHOS_TEST_FOR_EXCEPT_MSG(isplit > sb.ni || jsplit > sb.nj,
                                "splits must be <= dimension; isplit jsplit ni nj " << isplit << " " << jsplit << " " << sb.ni << " " << sb.nj);
  ...
  template <typename T>
  static KOKKOS_INLINE_FUNCTION T signof(const T& v) { return v >= 0 ? 1 : -1; }
  template <typename T>
  static KOKKOS_INLINE_FUNCTION Kokkos::complex<T> signof(const Kokkos::complex<T>& v) { return v == 0 ? 1 : v / Kokkos::abs(v); }
...
#pragma omp parallel for
  ...
  template <typename T>
  static void make_entry(const GO& gid, const LO& col, T& e) {
    e = std::sin(0.1 * (1 + gid + col));
  }
#ifdef HAVE_TEUCHOS_COMPLEX
  template <typename T>
  static void make_entry(const GO& gid, const LO& col, std::complex<T>& e) {
    T re, im;
  ...
  template <typename T>
  static void make_entry(const GO& gid, const LO& col, Kokkos::complex<T>& e) {
    std::complex<T> se;

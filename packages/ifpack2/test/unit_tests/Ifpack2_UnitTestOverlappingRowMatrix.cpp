...
// do it for debugging.
#define IFPACK2OVERLAPPINGROWMATRIX_REPORT_GLOBAL_ERR(WHAT_STRING)          \
  do {                                                                      \
...
void localReducedMatvec(const MatrixClass& A_lcl,
  ...
  if (is_vector) {
    ...
    Kokkos::parallel_for(
        "reduced-mv-vector", policy, KOKKOS_LAMBDA(const team_member& dev) {
          Kokkos::parallel_for(Kokkos::TeamThreadRange(dev, 0, rows_per_team), [&](const LO loop) {
            ...
            Kokkos::parallel_reduce(
                Kokkos::ThreadVectorRange(dev, row_length), [&](const LO iEntry, residual_value_type& lsum) {
    ...
    // Kernel interior shamelessly horked from Ifpack2_Details_ScaledDampedResidual_def.hpp
    Kokkos::parallel_for(
        "reduced-mv-multivector", policy, KOKKOS_LAMBDA(const team_member& dev) {

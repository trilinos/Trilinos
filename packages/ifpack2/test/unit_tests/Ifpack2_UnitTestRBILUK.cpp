...
#define IFPACK2RBILUK_REPORT_GLOBAL_ERR(WHAT_STRING)                        \
  do {                                                                      \
...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(RBILUK, BlockMatrixOps, Scalar, LocalOrdinal, GlobalOrdinal) {
  ...
  typedef typename KokkosKernels::ArithTraits<Scalar>::val_type impl_scalar_type;
#else
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      "lapackInfo = "
          << lapackInfo << " which indicates an error in the factorization GETF2.");
...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(RBILUK, DiagonalBlockCrsMatrix, Scalar, LocalOrdinal, GlobalOrdinal) {
  ...
  if (useOut) {
    ...
    newOutPtr = comm->getRank() == 0 ? Teuchos::getFancyOStream(Teuchos::rcpFromRef(std::cerr)) : Teuchos::getFancyOStream(blackHole);
  }

...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(Ifpack2AdditiveSchwarz, Test2, Scalar, LO, GO) {
  ...
  if (rowmap->getComm()->getSize() == 1) {
    out << endl
        << "This test must be run on more than one process." << endl
        << endl;
    return;
  }
...
#error "This test requires Amesos2 to have support for either SuperLU or KLU."
  ...
  out << endl
      << "solve using AdditiveSchwarz with sparse direct method as the subdomain solve" << endl;
  ...
  out << endl
      << "solve using AdditiveSchwarz with dense direct method as the subdomain solve" << endl;

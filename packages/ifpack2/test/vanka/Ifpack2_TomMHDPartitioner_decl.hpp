...
#ifndef IFPACK2_TOMMHDPARTITIONER_DECL_HPP
#define IFPACK2_TOMMHDPARTITIONER_DECL_HPP

#include "Ifpack2_ConfigDefs.hpp"
#include "Ifpack2_Partitioner.hpp"
#include "Tpetra_RowGraph.hpp"

/* \brief Ifpack2_TomMHDPartitioner: A class to create overlapping
    partitions of a local graph.

Class Ifpack2_TomMHDPartitioner enables the extension of 
...
class TomMHDPartitioner : public Partitioner<GraphType> {
...
public:
  ...
  //! Constructor.
  TomMHDPartitioner(const Teuchos::RCP<const Tpetra::RowGraph<LocalOrdinal,GlobalOrdinal,Node> >& Graph);

  //! Destructor.
  virtual ~TomMHDPartitioner();
  ...
  //! Returns an ArrayRCP to the integer vector containing the non-overlapping partition ID of each local row.
   Teuchos::ArrayView<const LocalOrdinal>  nonOverlappingPartition() const;
  ...
  //! Computes the partitions. Returns 0 if successful.
   void computeOverlappingPartitions();
...
protected:
  ...
  GlobalOrdinal NV_;
  //! Number of pressure rows in A
  GlobalOrdinal NP_;

}; // class Ifpack2::TomMHDPartitioner

}// namespace Ifpack2

#endif // IFPACK2_TOMMHDPARTITIONER_DECL_HPP

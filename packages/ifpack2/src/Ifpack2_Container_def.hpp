...
Container<MatrixType>::getBlockRows(int blockIndex) const {
  return Teuchos::ArrayView<const LO>(&blockRows_[blockOffsets_[blockIndex]], blockSizes_[blockIndex]);
}
          ...
          for (int localR = 0; localR < this->bcrsBlockSize_; localR++) {
            for (int localC = 0; localC < this->bcrsBlockSize_; localC++) {
              Resid(row * this->bcrsBlockSize_ + localR, m) -=
                  values[k * this->bcrsBlockSize_ * this->bcrsBlockSize_ + localR + localC * this->bcrsBlockSize_] * Y2(col * this->bcrsBlockSize_ + localC, m);
            }
          }
        }
      }
...
ContainerImpl<MatrixType, LocalScalarType>::
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      "On process "
          << this->inputMatrix_->getRowMap()->getComm()->getRank() << ", at least one row index in the set of local "
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      "Ifpack2::Container::translateRowToCol: "
      "On process "
          << this->inputMatrix_->getRowMap()->getComm()->getRank() << ", "
                                                                      ...
                                                                      "singular. The invalid global column index is "
          << diagGID << ".");

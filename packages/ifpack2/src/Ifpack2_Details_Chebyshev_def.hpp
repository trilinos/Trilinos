...
void reciprocal_threshold(Tpetra::Vector<S, L, G, N>& V, const S& minVal) {
...
template <class V>
void computeInitialGuessForCG(const V& diagonal, V& x) {
  ...
  Kokkos::parallel_for(
      "computeInitialGuessforCG::zero_bcs", range_policy(0, N), KOKKOS_LAMBDA(const size_t& i) {
        if (d_view(i, 0) == ONE)
          x_view(i, 0) = ZERO;
      });
}
...
void Chebyshev<ScalarType, MV>::checkInputMatrix() const {
  ...
  if (debug_ && !A_.is_null()) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        ...
        "the same (in the sense of isSameAs())."
            << std::endl
            << "We only check "
...
void Chebyshev<ScalarType, MV>::
  ...
  // (preferred) or double.  We check both.
  {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(boostFactor < Teuchos::ScalarTraits<MT>::one(), std::invalid_argument,
                               "Ifpack2::Chebyshev::setParameters: \"" << paramName << "\" parameter "
                                                                                       "must be >= 1, but you supplied the value "
                                                                       << boostFactor << ".");
  }
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      "nonnegative integer.  You gave a value of "
          << numIters << ".");
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(plist.isType<bool>("chebyshev: use block mode") &&
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(plist.isType<bool>("chebyshev: solve normal equations") &&
...
template <class ScalarType, class MV>
void Chebyshev<ScalarType, MV>::reset() {
  ck_                = Teuchos::null;
  D_                 = Teuchos::null;
...
template <class ScalarType, class MV>
void Chebyshev<ScalarType, MV>::compute() {
  using std::endl;
...
Chebyshev<ScalarType, MV>::apply(const MV& B, MV& X) {
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(A_.is_null(), std::runtime_error, prefix << "The input matrix A is null. "
                                                                         " Please call setMatrix() with a nonnull input matrix, and then call "
                                                                         "compute(), before calling this method.");
  TEUCHOS_TEST_FOR_EXCEPTION(STS::isnaninf(lambdaMaxForApply_), std::runtime_error,
                             prefix << "There is no estimate for the max eigenvalue."
                                    << std::endl
                                    << computeBeforeApplyReminder);
  TEUCHOS_TEST_FOR_EXCEPTION(STS::isnaninf(lambdaMinForApply_), std::runtime_error,
                             prefix << "There is no estimate for the min eigenvalue."
                                    << std::endl
                                    << computeBeforeApplyReminder);
  TEUCHOS_TEST_FOR_EXCEPTION(STS::isnaninf(eigRatioForApply_), std::runtime_error,
                             prefix << "There is no estimate for the ratio of the max "
                                       "eigenvalue to the min eigenvalue."
                                    << std::endl
                                    << computeBeforeApplyReminder);
  TEUCHOS_TEST_FOR_EXCEPTION(D_.is_null(), std::runtime_error, prefix << "The vector of inverse "
  ...
  if (debug_) {
    ...
    if (gblSuccess == 1) {
      *out_ << "makeInverseDiagonal: The matrix's diagonal entries all have "
               "positive real part (this is good for Chebyshev)."
            << std::endl;
    } else {
      *out_ << "makeInverseDiagonal: The matrix's diagonal has at least one "
               ...
               "matrix's communicator.  This is bad for Chebyshev."
            << std::endl;
    }
  }

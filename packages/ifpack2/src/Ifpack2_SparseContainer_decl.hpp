...
Note: A somehwhat dubious decision has been made in this initial implementation
to insiste that the matrices generated by the SparseContainer have GlobalOrdinal==LocalOrdinal.
This means that the InverseType has to do this as well.  This will eventually be remedied by modifying
...
public:
  ...
  virtual const Teuchos::RCP<Tpetra::MultiVector<Scalar,LocalOrdinal,GlobalOrdinal,Node> > & getX();

  //! Get the Y vector ( y = apply * x )
  virtual const Teuchos::RCP<Tpetra::MultiVector<Scalar,LocalOrdinal,GlobalOrdinal,Node> > & getY();
   ...
   */
  virtual LocalOrdinal & ID(const LocalOrdinal i);
  ...
  virtual void compute(const Teuchos::RCP<const Tpetra::RowMatrix<Scalar,LocalOrdinal,GlobalOrdinal,Node> >& Matrix);
...
private:
  ...
  virtual void extract(const Teuchos::RCP<const Tpetra::RowMatrix<Scalar,LocalOrdinal,GlobalOrdinal,Node> >& Matrix);
  ...
  Teuchos::RCP<Tpetra::Map<LocalOrdinal,GlobalOrdinal,Node> > Map_;
  //! Pointer to the local matrix.
  Teuchos::RCP<Tpetra::CrsMatrix<Scalar,LocalOrdinal,GlobalOrdinal,Node> > Matrix_;
  //! Solution vector.
  Teuchos::RCP<Tpetra::MultiVector<Scalar,LocalOrdinal,GlobalOrdinal,Node> > Y_;
  //! Input vector for local problems
  Teuchos::RCP<Tpetra::MultiVector<Scalar,LocalOrdinal,GlobalOrdinal,Node> > X_;
  //! Contains the subrows/subcols of A that will be inserted in Matrix_.
  std::vector<LocalOrdinal> GID_;

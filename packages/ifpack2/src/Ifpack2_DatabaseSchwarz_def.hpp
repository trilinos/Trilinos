...
template <class MatrixType>
void DatabaseSchwarz<MatrixType>::setParameters(const Teuchos::ParameterList& params) {
  // GH: Copied from CAG and others. Yes, const_cast bad.
  this->setParametersImpl(const_cast<Teuchos::ParameterList&>(params));
}

template <class MatrixType>
void DatabaseSchwarz<MatrixType>::setParametersImpl(Teuchos::ParameterList& params) {
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      "must be a nonnegative integer.  You gave a value of "
          << PatchSize_ << ".");
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      "must be a positive double.  You gave a value of "
          << PatchTolerance_ << ".");
...
template <class MatrixType>
void DatabaseSchwarz<MatrixType>::setZeroStartingSolution(bool zeroStartingSolution) {
    ...
    for (unsigned int i = 0; i < index_count.size(); ++i) {
      TEUCHOS_TEST_FOR_EXCEPTION(index_count[i] == 0.0, std::logic_error,
                                 "Ifpack2::DatabaseSchwarz::compute: DOF " << i << " has no corresponding patch! "
                                                                                   ...
                                                                                   "Have you considered adjusting the patch size? Currently it is "
                                                                           << PatchSize_ << ".");

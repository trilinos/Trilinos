...
//==============================================================================
Tifpack_NodeFilter::Tifpack_NodeFilter(const RefCountPtr<const Tpetra_RowMatrix>& Matrix,int nodeID) :
  Matrix_(Matrix),
  NumMyRows_(0),
  NumGlobalRows_(0),
  NumMyNonzeros_(0),
  ...
  MaxNumEntriesA_(0)
{
  sprintf(Label_,"%s","Tifpack_NodeFilter");
  ...
  ovA_ = dynamic_cast<const Tifpack_OverlappingRowMatrix*>(&*Matrix_);
  assert(ovA_ != 0);
  ...
  try {
    ...
    printf("** * Tifpack_NodeFilter ctor: problem creating row map * **\n\n");
  }
  ...
  try {
    ...
    printf("** * Tifpack_NodeFilter ctor: problem creating col map * **\n\n");
  }
  ...
  if (!(RowMatrixRowMap().SameAs(OperatorRangeMap()))) {
    try{Exporter_ = rcp(new Tpetra_Export(RowMatrixRowMap(), OperatorRangeMap()));}
      ...
      printf("** * gpid %d: Tifpack_NodeFilter ctor: problem creating Exporter_ * **\n\n",gpid);
    }
  }
  //if (gpid > 1) sleep(8);
/*
...
/*
  if (!(RowMatrixColMap().SameAs(OperatorDomainMap()))) {
    ...
    try{Importer_ = new Tpetra_Import(RowMatrixColMap(), OperatorDomainMap());}
      ...
      printf("** * gpid %d: Tifpack_NodeFilter ctor: problem creating Importer_ * **\n\n",gpid);
    }
*/
...
/*
    if (lpid == 0)
    printf("=======================================\nTifpack_NodeFilter Importer_ on node %d\n=======================================\n",(gpid == 0 ? 0: 1)); fflush(stdout);
    cout << *Importer_ << endl;
    if (lpid == 0)
    printf("=======================================\nTifpack_NodeFilter colmap on node %d\n=======================================\n",(gpid == 0 ? 0: 1)); fflush(stdout);
    cout << *colMap_ << endl;
*/
  }

} //Tifpack_NodeFilter() ctor

//==============================================================================
int Tifpack_NodeFilter::
...
//==============================================================================
int Tifpack_NodeFilter::ExtractDiagonalCopy(Tpetra_Vector & Diagonal) const
{
...
int Tifpack_NodeFilter::Apply(const Tpetra_MultiVector<Scalar,LocalOrdinal,GlobalOrdinal,Node>& X, Tpetra_MultiVector<Scalar,LocalOrdinal,GlobalOrdinal,Node>& Y) const {
  ...
  if(Acrs_){
    ...
    TIFPACK_CHK_ERR(Acrs_->ExtractCrsDataPointers(MyRows,MyIndices,MyValues));
  ...
  TIFPACK_CHK_ERR(ovA_->B().ExtractCrsDataPointers(MyRows,MyIndices,MyValues));
  //special case NumVectors==1
  if (NumVectors==1) {
...
void Tifpack_NodeFilter::UpdateImportVector(int NumVectors) const {
  if(Importer() != 0) {
    if(ImportVector_ != 0) {
...
void Tifpack_NodeFilter::UpdateExportVector(int NumVectors) const {
  if(Exporter() != 0) {
    if(ExportVector_ != 0) {
...
//==============================================================================
const Tpetra_BlockMap& Tifpack_NodeFilter::Map() const
{
  return(*Map_);

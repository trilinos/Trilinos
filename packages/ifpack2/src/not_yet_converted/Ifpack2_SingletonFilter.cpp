...
//==============================================================================
Ifpack2_SingletonFilter::Ifpack2_SingletonFilter(const Teuchos::RCP<Tpetra_RowMatrix>& Matrix) :
  A_(Matrix),
  NumSingletons_(0),
  NumRows_(0),
  NumRowsA_(0),
  ...
  NumNonzeros_(0)
{
  ...
  if (A_->Comm().NumProc() != 1) {
    cerr << "Ifpack2_SingletonFilter can be used with Comm().NumProc() == 1" << endl;
  ...
  for (int i = 0 ; i < NumRowsA_ ; ++i) {
    int Nnz;
    IFPACK2_CHK_ERRV(A_->ExtractMyRowCopy(i,MaxNumEntriesA_,Nnz,
		 ...
		 double *Values, int * Indices) const
{
  ...
  IFPACK2_CHK_ERR(A_->ExtractMyRowCopy(Row,MaxNumEntriesA_,Nnz,
				     &Values_[0],&Indices_[0]));
  NumEntries = 0;
  for (int i = 0 ; i < Nnz ; ++i) {
...
{
  IFPACK2_CHK_ERR(Multiply(false,X,Y));
  return(0);
}
		 ...
		 Tpetra_MultiVector<Scalar,LocalOrdinal,GlobalOrdinal,Node>& ReducedRHS)
{
  ...
  for (int i = 0 ; i < NumRows_ ; ++i) {
    int ii = InvReorder_[i];
    int Nnz;
    IFPACK2_CHK_ERR(A_->ExtractMyRowCopy(ii,MaxNumEntriesA_,Nnz,
					&Values_[0], &Indices_[0]));

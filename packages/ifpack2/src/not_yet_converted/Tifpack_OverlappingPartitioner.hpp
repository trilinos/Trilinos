...
#ifndef TIFPACK_OVERLAPPINGPARTITIONER_HPP
#define TIFPACK_OVERLAPPINGPARTITIONER_HPP
...
/* \brief Tifpack_OverlappingPartitioner: A class to create overlapping
    partitions of a local graph.

Class Tifpack_OverlappingPartitioner enables the extension of 
...
for derived classes Tifpack_LinearPartitioner, Tifpack_GreedyPartitioner,
Tifpack_METISPartitioner, and Tifpack_EquationPartitioner. Graphs in
...
that has been filtered using Tifpack_SingletonFilter.

\author Michael Heroux, SNL 9214.

\date Last update: Oct-04.
*/  
class Tifpack_OverlappingPartitioner : public Tifpack_Partitioner {
...
public:
  ...
  //! Destructor.
  virtual ~Tifpack_OverlappingPartitioner();
  ...
  int operator() (int MyRow) const
  {
    if ((MyRow < 0) || (MyRow > NumMyRows()))
      TIFPACK_CHK_ERR(-1); // input value not valid
...
#endif // TIFPACK_OVERLAPPINGPARTITIONER_HPP

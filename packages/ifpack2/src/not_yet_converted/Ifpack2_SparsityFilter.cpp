...
//==============================================================================
Ifpack2_SparsityFilter::Ifpack2_SparsityFilter(const Teuchos::RCP<Tpetra_RowMatrix>& Matrix,
					     int AllowedEntries, 
  ...
  NumRows_(0)
{
  ...
  if (A_->Comm().NumProc() != 1) {
    cerr << "Ifpack2_SparsityFilter can be used with Comm().NumProc() == 1" << endl;

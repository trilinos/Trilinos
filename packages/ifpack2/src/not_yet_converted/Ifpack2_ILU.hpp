...
void ILU::computeSetup() {
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(((L_.get() == 0) || (U_.get() == 0) || (D_.get() == 0)), std::runtime_error,
                             "Ifpack2::ILU::computeSetup(): Local column value of " << k << " for user matrix at local row " << i << " and index " << j << " less than zero on node " << getGraph().getL_Graph().getRowMap().getComm()->getRank());
    ...
    for (j = 0; j < numIn; j++) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION((k < 0 || k > getNumMyRows()), std::runtime_error,
                                 "Ifpack2::ILU::computeSetup(): Local column value of " << k
                                                                                        ...
                                                                                        << izero << " and " << getNumMyRows() << " on node " << getGraph().getL_Graph().getRowMap().getComm()->getRank());
      if (k == i) {
        diagFound = true;
        if (InV[j] >= zero)
  ...
  GlobalOrdinal temp              = (GlobalOrdinal)numNonzeroDiags;
        ...
        TEUCHOS_TEST_FOR_EXCEPTION(numNonzeroDiags != getNumMyRows(), std::runtime_error,
                                   "Ifpack2::ILU::computeSetup(): Number of nonzero diagonal values " << numNonzeroDiags << " for factorization is less than the number of rows " << getNumMyRows() << " in factor node " << getGraph().getL_Graph().getRowMap().getComm()->getRank());  // Diagonals are not right, warn user
}

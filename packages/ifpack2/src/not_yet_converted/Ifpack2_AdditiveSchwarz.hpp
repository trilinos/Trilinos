...
#ifndef IFPACK2_ADDITIVESCHWARZ_HPP
#define IFPACK2_ADDITIVESCHWARZ_HPP
...
#include "EpetraExt_BlockMapOut.hpp"
  ...
  #include "Ifpack2_AMDReordering.hpp"
#endif


//! Ifpack2_AdditiveSchwarz: a class to define Additive Schwarz preconditioners of Tpetra_RowMatrix's.

/*!
  Class  Ifpack2_AdditiveSchwarz enables the construction of Additive
  Schwarz (one-level overlapping domain decomposition) preconditioners, 
  for a given Tpetra_RowMatrix.
  Ifpack2_AdditiveSchwarz is derived from Ifpack2_Preconditioner,
  ...
  from Ifpack2_Preconditioner. This can be easily accomplished, as
  Ifpack2_AdditiveSchwarz is templated with the solver for each subdomain.
...
template<typename T>
class Ifpack2_AdditiveSchwarz : public virtual Ifpack2_Preconditioner {
      
public:

  //@{ \name Constructors/Destructors
  //! Ifpack2_AdditiveSchwarz constructor with given Tpetra_RowMatrix.
  /*! Creates an Ifpack2_AdditiveSchwarz preconditioner with overlap.
   ...
   */
  Ifpack2_AdditiveSchwarz(Tpetra_RowMatrix* Matrix_in,
...
protected:
  ...
  Ifpack2_AdditiveSchwarz(const Ifpack2_AdditiveSchwarz& RHS)
  { }
  ...
  Teuchos::RCP<Ifpack2_OverlappingRowMatrix> OverlappingMatrix_;
  //! Localized version of Matrix_ or OverlappingMatrix_.
# ifdef IFPACK2_NODE_AWARE_CODE
  Teuchos::RCP<Ifpack2_NodeFilter> LocalizedMatrix_;
# else
  Teuchos::RCP<Ifpack2_LocalFilter> LocalizedMatrix_;
# endif
  ...
  Teuchos::RCP<Ifpack2_Reordering> Reordering_;
  //! Pointer to the reorderd matrix.
  Teuchos::RCP<Ifpack2_ReorderFilter> ReorderedLocalizedMatrix_;
  //! Filter for singletons.
  bool FilterSingletons_;
  //! filtering object.
  Teuchos::RCP<Ifpack2_SingletonFilter> SingletonFilter_;
...
}; // class Ifpack2_AdditiveSchwarz<T>

//==============================================================================
template<typename T>
Ifpack2_AdditiveSchwarz<T>::
Ifpack2_AdditiveSchwarz(Tpetra_RowMatrix* Matrix_in,
		       int OverlapLevel_in) :
...
//==============================================================================
template<typename T>
int Ifpack2_AdditiveSchwarz<T>::Setup()
{
...
*/
  ...
  catch(...){fprintf(stderr,"Ifpack2_AdditiveSchwarz<T>::Setup(): no parameter \"ML node id\"\n\n");
             cout << List_ << endl;}
# endif
...
#   ifdef IFPACK2_NODE_AWARE_CODE
    ...
    //LocalizedMatrix_ = Teuchos::rcp( new Ifpack2_LocalFilter(OverlappingMatrix_) );
#   else
    LocalizedMatrix_ = Teuchos::rcp( new Ifpack2_LocalFilter(OverlappingMatrix_) );
#   endif
  }
  ...
  {
#   ifdef IFPACK2_NODE_AWARE_CODE
    Ifpack2_NodeFilter *tt = new Ifpack2_NodeFilter(Matrix_,nodeID); //FIXME
    LocalizedMatrix_ = Teuchos::rcp(tt);
    //LocalizedMatrix_ = Teuchos::rcp( new Ifpack2_LocalFilter(Matrix_) );
#   else
    LocalizedMatrix_ = Teuchos::rcp( new Ifpack2_LocalFilter(Matrix_) );
#   endif
  }
  ...
  if (FilterSingletons_) {
    SingletonFilter_ = Teuchos::rcp( new Ifpack2_SingletonFilter(LocalizedMatrix_) );
  ...
  if (UseReordering_) {
    ...
    if (ReorderingType_ == "rcm")
      Reordering_ = Teuchos::rcp( new Ifpack2_RCMReordering() );
    else if (ReorderingType_ == "metis")
      Reordering_ = Teuchos::rcp( new Ifpack2_METISReordering() );
#ifdef HAVE_IFPACK2_AMESOS	
    else if (ReorderingType_ == "amd" )
      Reordering_ = Teuchos::rcp( new Ifpack2_AMDReordering() );
#endif
    else {
      cerr << "reordering type not correct (" << ReorderingType_ << ")" << endl;
      exit(EXIT_FAILURE);
    }
    if (Reordering_ == Teuchos::null) IFPACK2_CHK_ERR(-5);

    IFPACK2_CHK_ERR(Reordering_->SetParameters(List_));
    IFPACK2_CHK_ERR(Reordering_->Compute(*MatrixPtr));

    // now create reordered localized matrix
    ReorderedLocalizedMatrix_ = 
      Teuchos::rcp( new Ifpack2_ReorderFilter(Teuchos::rcp( MatrixPtr, false ), Reordering_) );

    if (ReorderedLocalizedMatrix_ == Teuchos::null) IFPACK2_CHK_ERR(-5);
...
//==============================================================================
template<typename T>
int Ifpack2_AdditiveSchwarz<T>::SetParameters(Teuchos::ParameterList& List_in)
{
...
//==============================================================================
template<typename T>
int Ifpack2_AdditiveSchwarz<T>::Initialize()
{
  IsInitialized_ = false;
...
/*
      cout << "pid " << Comm().MyPID()
           << ": calling Ifpack2_OverlappingRowMatrix with myNodeID = "
           << myNodeID << ", OverlapLevel_ = " << OverlapLevel_ << endl;
*/
      OverlappingMatrix_ = Teuchos::rcp( new Ifpack2_OverlappingRowMatrix(Matrix_, OverlapLevel_, myNodeID) );
#   else
      OverlappingMatrix_ =
        Teuchos::rcp( new Ifpack2_OverlappingRowMatrix(Matrix_, OverlapLevel_) );
#   endif
...
*/
  ...
  IFPACK2_CHK_ERR(Inverse_->SetUseTranspose(UseTranspose()));
  IFPACK2_CHK_ERR(Inverse_->SetParameters(List_));
  IFPACK2_CHK_ERR(Inverse_->Initialize());

  // Label is for Aztec-like solvers
  Label_ = "Ifpack2_AdditiveSchwarz, ";
  if (UseTranspose())
    Label_ += ", transp";
  Label_ += ", ov = " + Ifpack2_toString(OverlapLevel_)
...
//==============================================================================
template<typename T>
int Ifpack2_AdditiveSchwarz<T>::Compute()
{
  ...
  IsComputed_ = true; // need this here for Condest(Ifpack2_Cheap)
  ++NumCompute_;
  ...
  if (ComputeCondest_)
    Condest(Ifpack2_Cheap);
  
  // add Condest() to label
  Label_ = "Ifpack2_AdditiveSchwarz, ov = " + Ifpack2_toString(OverlapLevel_)
    + ", local solver = \n\t\t***** `" + string(Inverse_->Label()) + "'"
    + "\n\t\t***** " + R + "Condition number estimate = "
    + Ifpack2_toString(Condest());
...
//==============================================================================
template<typename T>
int Ifpack2_AdditiveSchwarz<T>::SetUseTranspose(bool UseTranspose_in)
{
  ...
  if (Inverse_!=Teuchos::null)
    IFPACK2_CHK_ERR(Inverse_->SetUseTranspose(UseTranspose_in));
  return(0);
}

//==============================================================================
template<typename T>
int Ifpack2_AdditiveSchwarz<T>::
...
//==============================================================================
template<typename T>
double Ifpack2_AdditiveSchwarz<T>::NormInf() const
{
  return(-1.0);
}
...
//==============================================================================
template<typename T>
const char * Ifpack2_AdditiveSchwarz<T>::Label() const
{
  return(Label_.c_str());
}
...
//==============================================================================
template<typename T>
bool Ifpack2_AdditiveSchwarz<T>::UseTranspose() const
{
...
//==============================================================================
template<typename T>
bool Ifpack2_AdditiveSchwarz<T>::HasNormInf() const
{
  return(false);
}
...
//==============================================================================
template<typename T>
const Tpetra_Comm & Ifpack2_AdditiveSchwarz<T>::Comm() const
{
  return(Matrix_->Comm());
}
...
//==============================================================================
template<typename T>
const Tpetra_Map & Ifpack2_AdditiveSchwarz<T>::OperatorDomainMap() const
{
...
//==============================================================================
template<typename T>
const Tpetra_Map & Ifpack2_AdditiveSchwarz<T>::OperatorRangeMap() const
{
...
ApplyInverse(const Tpetra_MultiVector<Scalar,LocalOrdinal,GlobalOrdinal,Node>& X, Tpetra_MultiVector<Scalar,LocalOrdinal,GlobalOrdinal,Node>& Y) const
{
  ...
  if (IsOverlapping()) {
    ...
    OverlappingY = Teuchos::rcp( new Tpetra_MultiVector(OverlappingMatrix_->RowMatrixRowMap(),
							Y.NumVectors()) );
    if (OverlappingY == Teuchos::null) IFPACK2_CHK_ERR(-5);

    OverlappingY->PutScalar(0.0);
    OverlappingX->PutScalar(0.0);
    IFPACK2_CHK_ERR(OverlappingMatrix_->ImportMultiVector(X,*OverlappingX,Insert));
    // FIXME: this will not work with overlapping and non-zero starting
    // solutions. The same for other cases below.
    // IFPACK2_CHK_ERR(OverlappingMatrix_->ImportMultiVector(Y,*OverlappingY,Insert));
  }
  else {
  ...
  if (FilterSingletons_) {
    ...
    IFPACK2_CHK_ERR(SingletonFilter_->SolveSingletons(*OverlappingX,*OverlappingY));
    IFPACK2_CHK_ERR(SingletonFilter_->CreateReducedRHS(*OverlappingY,*OverlappingX,ReducedX));

    // process reordering
    if (!UseReordering_) {
      IFPACK2_CHK_ERR(Inverse_->ApplyInverse(ReducedX,ReducedY));
    }
    else {
      Tpetra_MultiVector ReorderedX(ReducedX);
      Tpetra_MultiVector ReorderedY(ReducedY);
      IFPACK2_CHK_ERR(Reordering_->P(ReducedX,ReorderedX));
      IFPACK2_CHK_ERR(Inverse_->ApplyInverse(ReorderedX,ReorderedY));
      IFPACK2_CHK_ERR(Reordering_->Pinv(ReorderedY,ReducedY));
    }

    // finish up with singletons
    IFPACK2_CHK_ERR(SingletonFilter_->UpdateLHS(ReducedY,*OverlappingY));
  }
  else {
    // process reordering
    if (!UseReordering_) {
      IFPACK2_CHK_ERR(Inverse_->ApplyInverse(*OverlappingX,*OverlappingY));
    }
    else {
      Tpetra_MultiVector ReorderedX(*OverlappingX);
      Tpetra_MultiVector ReorderedY(*OverlappingY);
      IFPACK2_CHK_ERR(Reordering_->P(*OverlappingX,ReorderedX));
      IFPACK2_CHK_ERR(Inverse_->ApplyInverse(ReorderedX,ReorderedY));
      IFPACK2_CHK_ERR(Reordering_->Pinv(ReorderedY,*OverlappingY));
    }
  }

  if (IsOverlapping()) {
    IFPACK2_CHK_ERR(OverlappingMatrix_->ExportMultiVector(*OverlappingY,Y,
							 CombineMode_));
  }
...
//==============================================================================
template<typename T>
std::ostream& Ifpack2_AdditiveSchwarz<T>::
Print(std::ostream& os) const
{
  ...
  os << endl;
  os << "================================================================================" << endl;
  os << "Ifpack2_AdditiveSchwarz, overlap level = " << OverlapLevel_ << endl;
...
//==============================================================================
template<typename T>
double Ifpack2_AdditiveSchwarz<T>::
Condest(const Ifpack2_CondestType CT, const int MaxIters, 
...
#endif // IFPACK2_ADDITIVESCHWARZ_HPP

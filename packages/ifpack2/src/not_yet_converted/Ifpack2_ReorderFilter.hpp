...
#ifndef IFPACK2_REORDERFILTER_HPP
#define IFPACK2_REORDERFILTER_HPP
...
//! Ifpack2_ReorderFilter: a class for light-weight reorder of local rows and columns of an Tpetra_RowMatrix.

/*!
Class Ifpack2_ReorderFilter enables a light-weight construction of 
reordered matrices. 

This class is used in Ifpack2_AdditiveSchwarz to reorder (if required
...
To improve the performances of Ifpack2_AdditiveSchwarz, some
...
class Ifpack2_ReorderFilter : public virtual Tpetra_RowMatrix {

public:
  // Constructor.
  Ifpack2_ReorderFilter(const Teuchos::RCP<Tpetra_RowMatrix>& Matrix_in,
		       const Teuchos::RCP<Ifpack2_Reordering>& Reordering_in);

  //! Copy constructor.
  Ifpack2_ReorderFilter(const Ifpack2_ReorderFilter& RHS);

  //! Destructor.
  virtual ~Ifpack2_ReorderFilter() {};
  ...
  //! Returns a reference-counted pointer to the internally stored pointer to Ifpack2_Reordering..
  inline Teuchos::RCP<Ifpack2_Reordering> Reordering() const {
    return(Reordering_);
  }

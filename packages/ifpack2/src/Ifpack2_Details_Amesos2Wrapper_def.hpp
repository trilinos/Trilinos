...
void Amesos2Wrapper<MatrixType>::initialize() {
  ...
  {  // Start timing here.
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        A_local.is_null(), std::logic_error,
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(solver_.is_null(), std::runtime_error,
                               "Ifpack2::Details::"
...
void Amesos2Wrapper<MatrixType>::
  ...
  {  // Start timing here.
    ...
    // alias Y_temp to Y.
    RCP<MV> Y_temp = (alpha != STS::one() || beta != STS::zero()) ? rcp(new MV(Y.getMap(), Y.getNumVectors())) : rcpFromRef(Y);

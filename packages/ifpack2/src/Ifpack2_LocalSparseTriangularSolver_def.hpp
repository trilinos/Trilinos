...
LocalSparseTriangularSolver<MatrixType>::
  ...
  if (!A.is_null()) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(A_crs.is_null(), std::invalid_argument,
...
void LocalSparseTriangularSolver<MatrixType>::
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(reverseStorage_ && (trisolverType == Details::TrisolverType::HTS || trisolverType == Details::TrisolverType::KSPTRSV),
                             std::logic_error,
...
void LocalSparseTriangularSolver<MatrixType>::
  ...
  if (!isKokkosKernelsStream_) {
    TEUCHOS_TEST_FOR_EXCEPTION(A_.is_null(), std::runtime_error, prefix << "You must call "
    ...
    if (A_crs_.is_null()) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(A_crs.get() == nullptr, std::invalid_argument,
                                 prefix << "The input matrix A is not a Tpetra::CrsMatrix.");
      A_crs_ = A_crs;
    }
    auto G = A_crs_->getGraph();
    TEUCHOS_TEST_FOR_EXCEPTION(G.is_null(), std::logic_error, prefix << "A_ and A_crs_ are nonnull, "
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(!G->isFillComplete(), std::runtime_error,
    ...
    for (int i = 0; i < num_streams_; i++) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(G.is_null(), std::logic_error, prefix << "A_crs_ are nonnull, "
                                                                          "but A_crs_'s RowGraph G is null.  "
                                                                          "Please report this bug to the Ifpack2 developers.");
      // At this point, the graph MUST be fillComplete.  The "initialize"
      // (symbolic) part of setup only depends on the graph structure, so
      ...
      if (i > 0) {
        TEUCHOS_TEST_FOR_EXCEPTION((this->diag_ != prev_diag) || (this->uplo_ != prev_uplo),
  ...
  if (this->isKokkosKernelsSptrsv_ && ksptrsv_valid_uplo && this->diag_ != "U") {
    if (!isKokkosKernelsStream_) {
      ...
      for (int i = 0; i < num_streams_; i++) {
        ...
        KokkosSparse::sptrsv_symbolic(kh_v_[i].getRawPtr(), ptr_i, ind_i, val_i);
      }
      kh_v_nonnull_ = true;
    }
  }
...
void LocalSparseTriangularSolver<MatrixType>::
  ...
  if (!isKokkosKernelsStream_) {
    TEUCHOS_TEST_FOR_EXCEPTION(A_.is_null(), std::runtime_error, prefix << "You must call "
                                                                           "setMatrix() with a nonnull input matrix before you may call "
                                                                           "initialize() or compute().");
    TEUCHOS_TEST_FOR_EXCEPTION(A_crs_.is_null(), std::logic_error, prefix << "A_ is nonnull, but "
                                                                             "A_crs_ is null.  Please report this bug to the Ifpack2 developers.");
    // At this point, the matrix MUST be fillComplete.
    TEUCHOS_TEST_FOR_EXCEPTION(!A_crs_->isFillComplete(), std::runtime_error,
                               "If you call this "
    ...
    for (int i = 0; i < num_streams_; i++) {
      TEUCHOS_TEST_FOR_EXCEPTION(A_crs_v_[i].is_null(), std::runtime_error, prefix << "You must call "
                                                                                      ...
                                                                                      "initialize() or compute().");
      // At this point, the matrix MUST be fillComplete.
      TEUCHOS_TEST_FOR_EXCEPTION(!A_crs_v_[i]->isFillComplete(), std::runtime_error,
                                 "If you call this "
                                 "method, the matrix must be fill complete.  It is not.");
    }
  }
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(!isInitialized_, std::logic_error, prefix << "initialize() should have "
...
// This would all be much better handled via a KokkosSparse::sptrsv_numeric(...)
...
void LocalSparseTriangularSolver<MatrixType>::
    ...
    if (!isKokkosKernelsStream_) {
      if (A_crs_.is_null()) {
        *out_ << "A_crs_ is null!" << std::endl;
        ...
        const std::string trans = (mode == Teuchos::CONJ_TRANS) ? "C" : (mode == Teuchos::TRANS ? "T" : "N");
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(!isComputed(), std::runtime_error, prefix << "If compute() has not yet "
  ...
  if (!isKokkosKernelsStream_) {
    TEUCHOS_TEST_FOR_EXCEPTION(A_.is_null(), std::logic_error, prefix << "A_ is null.  "
                                                                         "Please report this bug to the Ifpack2 developers.");
    TEUCHOS_TEST_FOR_EXCEPTION(A_crs_.is_null(), std::logic_error, prefix << "A_crs_ is null.  "
                                                                             "Please report this bug to the Ifpack2 developers.");
    ...
    for (int i = 0; i < num_streams_; i++) {
      TEUCHOS_TEST_FOR_EXCEPTION(A_crs_v_[i].is_null(), std::logic_error, prefix << "A_crs_ is null.  "
                                                                                    "Please report this bug to the Ifpack2 developers.");
      // However, it _is_ possible that the user called resumeFill() on
      // the matrix, after calling compute().  This is NOT allowed.
      TEUCHOS_TEST_FOR_EXCEPTION(!A_crs_v_[i]->isFillComplete(), std::runtime_error,
  ...
  if (!isKokkosKernelsStream_) {
    ...
    X_cur = importer.is_null() ? rcpFromRef(X) : Teuchos::rcp_const_cast<const MV>(X_colMap_);
...
void LocalSparseTriangularSolver<MatrixType>::
  ...
  if (!isKokkosKernelsStream_) {
    TEUCHOS_TEST_FOR_EXCEPTION_CLASS_FUNC(!A_crs_->isFillComplete(), std::runtime_error,
                                          "The matrix is not fill complete.");
    TEUCHOS_TEST_FOR_EXCEPTION_CLASS_FUNC(A_crs_->getLocalNumRows() > 0 && this->uplo_ == "N", std::runtime_error,
    ...
    for (int i = 0; i < num_streams_; i++) {
      TEUCHOS_TEST_FOR_EXCEPTION_CLASS_FUNC(!A_crs_v_[i]->isFillComplete(), std::runtime_error,
                                            "The matrix is not fill complete.");
      TEUCHOS_TEST_FOR_EXCEPTION_CLASS_FUNC(A_crs_v_[i]->getLocalNumRows() > 0 && this->uplo_ == "N", std::runtime_error,
                                            "The matrix is neither upper triangular or lower triangular.  "
                                            "You may only call this method if the matrix is triangular.  "
                                            "Remember that this is a local (per MPI process) property, and that "
                                            "Tpetra only knows how to do a local (per process) triangular solve.");
  ...
  TEUCHOS_TEST_FOR_EXCEPTION_CLASS_FUNC(!X.isConstantStride() || !Y.isConstantStride(), std::invalid_argument,
                                        "X and Y must be constant stride.");
  using STS = Teuchos::ScalarTraits<scalar_type>;
  TEUCHOS_TEST_FOR_EXCEPTION_CLASS_FUNC(STS::isComplex && mode == TRANS, std::logic_error,
                                        "This method does "
...
template <class MatrixType>
int LocalSparseTriangularSolver<MatrixType>::
...
void LocalSparseTriangularSolver<MatrixType>::
  ...
  if (vl != Teuchos::VERB_NONE) {
    ...
    auto comm = A_.is_null() ? Tpetra::getDefaultComm() : A_->getComm();
...
void LocalSparseTriangularSolver<MatrixType>::
  ...
  if (A.getRawPtr() != A_.getRawPtr() || isInternallyChanged_) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(!A.is_null() && A->getComm()->getSize() == 1 &&
                               ...
                               std::runtime_error, prefix << "If A's communicator only contains one "
                                                          ...
                                                          << A->getLocalNumRows() << " rows and " << A->getLocalNumCols() << " columns.");
    ...
    if (A.is_null()) {
      ...
      Teuchos::RCP<const crs_matrix_type> A_crs =
          Teuchos::rcp_dynamic_cast<const crs_matrix_type>(A);
      TEUCHOS_TEST_FOR_EXCEPTION(A_crs.is_null(), std::invalid_argument, prefix << "The input matrix A is not a Tpetra::CrsMatrix.");
      A_crs_ = A_crs;
      A_     = A;
    }
...
void LocalSparseTriangularSolver<MatrixType>::
  ...
  for (int i = 0; i < num_streams_; i++) {
    ...
    if (A_crs_v[i].getRawPtr() != A_crs_v_[i].getRawPtr() || isInternallyChanged_) {
      // Check in serial or one-process mode if the matrix is square.
      TEUCHOS_TEST_FOR_EXCEPTION(!A_crs_v[i].is_null() && A_crs_v[i]->getComm()->getSize() == 1 &&
                                     A_crs_v[i]->getLocalNumRows() != A_crs_v[i]->getLocalNumCols(),
                                 std::runtime_error, prefix << "If A's communicator only contains one "
                                                               "process, then A must be square.  Instead, you provided a matrix A with "
                                                            << A_crs_v[i]->getLocalNumRows() << " rows and " << A_crs_v[i]->getLocalNumCols() << " columns.");

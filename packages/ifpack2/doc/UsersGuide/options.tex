...
\cccc{relaxation: container}
    ...
    {Containers are used to store and solve block matrices. These container
     types are always available: ``Dense'', ``TriDi'', ``Banded'' and ``SparseILUT''.
     If Amesos2 is enabled, ``SparseAmesos'' is available. If experimental kokkos-kernels
     features are enabled (true by default), ``BlockTriDi'' is availabe.}

...
bool
testAlltoallv (bool& success, std::ostream& out,
               const Teuchos::Comm<int>& comm)
{
  using Teuchos::alltoAllv;
  ...
  // Process 0 anyway.
  out << "Testing Teuchos::alltoAllv<int, " << TypeNameTraits<packet_type>::name ()
  ...
  for (int i = 0; i < total_count; ++i) {
    ...
    recvBuf2[i] = ZERO;
    send_counts2[i] = send_count; send_displs2[i+1] = send_displs2[i] + send_counts2[i];
    recv_counts2[i] = recv_count; recv_displs2[i+1] = recv_displs2[i] + recv_counts2[i];
  }
...
TEUCHOS_UNIT_TEST_TEMPLATE_1_DECL( Comm, AlltoAllv, PacketType )
{
  using Teuchos::Comm;
  using Teuchos::RCP;
  typedef PacketType packet_type;
  ...
  const bool curSuccess = testAlltoallv<packet_type> (success, out, *comm);
...
//
//
TEUCHOS_UNIT_TEST_TEMPLATE_1_INSTANT( Comm, AlltoAllv, int )
TEUCHOS_UNIT_TEST_TEMPLATE_1_INSTANT( Comm, AlltoAllv, double )

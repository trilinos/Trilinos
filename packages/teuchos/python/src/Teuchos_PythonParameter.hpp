...
namespace Teuchos {
  ...
  bool synchronizeParameters(PyObject * dict, ParameterList & plist) {
	...
	if (plist.isSublist(name)) {
	  if (!PyDict_Check(value)) value = PyDict_New();
	  result = result and synchronizeParameters(value, plist.sublist(name));
	...
	// If the parameter exists, it will have been synchronized
	// above.  We are only interested in entries not in the
	// parameter list.
	name = string(PyString_AsString(key));

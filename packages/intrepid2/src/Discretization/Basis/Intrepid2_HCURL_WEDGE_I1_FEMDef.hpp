  ...
  namespace Impl {
    ...
    getValues(       OutputViewType output,
      ...
      if constexpr (OpType == OPERATOR_VALUE) {
        ...
        output(2, 0) = y*(z - 1.0);
        output(2, 1) = (1.0 - x)*(z - 1.0);
        output(2, 2) = 0.0;

        output(3, 0) = (1.0 - y)*(1.0 + z);
        output(3, 1) = x*(1.0 + z);
        output(3, 2) = 0.0;
        ...
        output(1, 0) = x;
        output(1, 1) = y;
        output(1, 2) = 2.0*(1.0 - z);

        output(2, 0) = (x - 1.0);
        output(2, 1) = y;
        output(2, 2) = 2.0*(1.0 - z);

        output(3, 0) = -x;
        output(3, 1) = (1.0 - y);
        output(3, 2) = 2.0*(1.0 + z);
...
#define HCURL_WEDGE_I1_FEM_INSTANT(DEVICE, OUTPUT_TYPE, POINT_TYPE, EXTERN)    \
  EXTERN template class Intrepid2::Basis_HCURL_WEDGE_I1_FEM<                   \
      DEVICE, OUTPUT_TYPE, POINT_TYPE>;

INTREPID2_ETI_DEVICE_DEF(HCURL_WEDGE_I1_FEM_INSTANT);

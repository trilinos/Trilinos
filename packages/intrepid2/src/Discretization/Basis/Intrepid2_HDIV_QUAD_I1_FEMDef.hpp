...
namespace Intrepid2 {
  ...
  namespace Impl {
    ...
    getValues(       OutputViewType output,
      ...
      if constexpr (OpType == OPERATOR_VALUE) {      // outputValues is a rank-3 array with dimensions (basisCardinality_, dim0, spaceDim)
  ...
  Basis_HDIV_QUAD_I1_FEM<DT,OT,PT>::
  Basis_HDIV_QUAD_I1_FEM() {
...
#define HDIV_QUAD_I1_FEM_INSTANT(DEVICE, OUTPUT_TYPE, POINT_TYPE, EXTERN)       \
  EXTERN template class Intrepid2::Basis_HDIV_QUAD_I1_FEM<DEVICE, OUTPUT_TYPE,  \
                                                         POINT_TYPE>;

INTREPID2_ETI_DEVICE_DEF(HDIV_QUAD_I1_FEM_INSTANT);

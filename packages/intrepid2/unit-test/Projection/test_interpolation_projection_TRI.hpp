...
#define ConstructWithLabel(obj, ...) obj(#obj, __VA_ARGS__)
  ...
  ordinal_type cells_orig[numCells][numElemVertexes] = {{0,1,2},{1,2,3}};
  ...
  shards::CellTopology cellTopo(shards::getCellTopologyData<shards::Triangle<3> >());
  ...
  try {
    ...
    do {
      ...
      for (ordinal_type shift=0; shift<3; ++shift) {
        std::rotate_copy(&cells_orig[0][0], &cells_orig[0][0]+shift, &cells_orig[0][0]+3, &cells_rotated[0][0]);
          ...
          for (auto basisPtr:basis_set) {
            ...
            *outStream << " " << name <<  ": " << degree << std::endl;
            ...
            //compute Lagrangian Interpolation of fun
            {
              ...
              Basis_HGRAD_TRI_C1_FEM<DeviceType,ValueType,ValueType> linearBasis;
      ...
      for (ordinal_type shift=0; shift<3; ++shift) {
        ...
        *outStream << "]\n";
        ...
        for (ordinal_type degree=1; degree <= max_degree; degree++) {
          ...
          basis_set.clear();
          if(degree==1)
            basis_set.push_back(new Basis_HCURL_TRI_I1_FEM<DeviceType,ValueType,ValueType>());
          ...
          for (auto basisPtr:basis_set) {
            ...
            *outStream << " " << name <<  ": " << degree << std::endl;
            ...
            DynRankView ConstructWithLabel(basisCoeffsHCurl, numCells, basisCardinality);
            {
              ...
              if(numCurlPoints>0) {
                DynRankView ConstructWithLabel(curlOfHCurlBasisAtEvaluationPointsNonOriented, basisCardinality , numCurlPoints);
      ...
      for (ordinal_type shift=0; shift<3; ++shift) {
        ...
        *outStream << "]\n";
        ...
        for (ordinal_type degree=1; degree <= max_degree; degree++) {
          ...
          basis_set.clear();
          if(degree==1)
            basis_set.push_back(new Basis_HDIV_TRI_I1_FEM<DeviceType,ValueType,ValueType>());

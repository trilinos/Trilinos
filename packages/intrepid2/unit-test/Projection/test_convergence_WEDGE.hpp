...
/** \file
    \brief  Test for checking accuracy of interpolation-based projections for wedge elements

    The test considers a uniform and structured wedge mesh of the cube [-1,1]^3, formed by first
    building an hexahedral mesh with N^3 hexas and then splitting each hexa in 2 wedges.
...
int ConvergenceWedge(const bool verbose) {
...
#define ConstructWithLabel(obj, ...) obj(#obj, __VA_ARGS__)
  ...
  *outStream << "\n";
  ...
  ValueType hgrad_errors[4] = {4.90336, 1.23434, 0.201003, 0.0266877};
  ValueType hcurl_errors[4] = {5.63876, 1.53668, 0.248837, 0.0335233};
  ValueType hdiv_errors[4] = {4.64172, 0.956411, 0.150255, 0.0201462};
  ValueType hvol_errors[4] = {0.624463, 0.116153, 0.0181689, 0.00237163};

  ValueType hgrad_errors_L2[4] = {4.84081, 1.22096, 0.203772, 0.0271797};
  ValueType hcurl_errors_L2[4] = {5.71563, 1.63339, 0.290667, 0.0506111};
  ValueType hdiv_errors_L2[4] = {4.56499, 1.07584, 0.210319, 0.0426091};
  ValueType hvol_errors_L2[4] = {0.624463, 0.116153, 0.0181689, 0.00237163};
  ...
  for(int iter= 0; iter<numRefinements; iter++, NX *= 2) {
    ...
    *outStream
    ...
    try {
      ...
      typename CG_DNBasis::HGRAD_WEDGE warpBasis(basisDegree,POINTTYPE_WARPBLEND); //used only for computing reference points 
      ...
      basis_set.push_back(new typename  CG_DNBasis::HGRAD_WEDGE(basisDegree));
      basis_set.push_back(new typename  CG_HBasis::HGRAD_WEDGE(basisDegree));
      ...
      for (auto basisPtr:basis_set) {
        ...
        *outStream << " " << basis.getName() << std::endl;
        ...
        {
          Basis_HGRAD_WEDGE_C1_FEM<DeviceType,ValueType,ValueType> linearBasis; //used for computing physical coordinates
          ...
          DynRankView ConstructWithLabel(physEvalGradPoints, numElems, numGradPoints, dim);
          {
            ...
            KOKKOS_LAMBDA (const int &i) {
              ...
              for(ordinal_type j=0; j<numGradPoints; ++j) {
                ...
                Impl::Basis_HGRAD_WEDGE_C1_FEM::template Serial<OPERATOR_VALUE>::getValues(basisValuesAtEvalGradPoint, evalGradPoint);
                for(ordinal_type k=0; k<numNodesPerElem; ++k)
                  for(ordinal_type d=0; d<dim; ++d)
    ...
    *outStream
    ...
    try {
      ...
      std::vector<basisType*> basis_set;
      basis_set.push_back(new typename  CG_DNBasis::HCURL_WEDGE(basisDegree));
      basis_set.push_back(new typename  CG_HBasis::HCURL_WEDGE(basisDegree));
          ...
          DynRankView ConstructWithLabel(physEvalCurlPoints, numElems, numCurlPoints, dim);
          {
            ...
            KOKKOS_LAMBDA (const int &i) {
              ...
              for(ordinal_type j=0; j<numCurlPoints; ++j) {
                ...
                Impl::Basis_HGRAD_WEDGE_C1_FEM::template Serial<OPERATOR_VALUE>::getValues(basisValuesAtEvalCurlPoint, evalGradPoint);
                for(ordinal_type k=0; k<numNodesPerElem; ++k)
                  for(ordinal_type d=0; d<dim; ++d)
    ...
    *outStream
    ...
    try {
      ...
      basis_set.push_back(new typename  CG_HBasis::HDIV_WEDGE(basisDegree));
      basis_set.push_back(new typename  CG_DNBasis::HDIV_WEDGE(basisDegree));
          ...
          DynRankView ConstructWithLabel(physEvalDivPoints, numElems, numDivPoints, dim);
          {
            ...
            KOKKOS_LAMBDA (const int &i) {
              ...
              for(ordinal_type j=0; j<numDivPoints; ++j) {
                ...
                Impl::Basis_HGRAD_WEDGE_C1_FEM::template Serial<OPERATOR_VALUE>::getValues(basisValuesAtEvalDivPoint, evalGradPoint);
                for(ordinal_type k=0; k<numNodesPerElem; ++k)
                  for(ordinal_type d=0; d<dim; ++d)
    ...
    *outStream
    ...
    try {
      ...
      for (auto useL2Projection:useL2Proj) { //
      std::vector<basisType*> basis_set;
      basis_set.push_back(new typename  CG_DNBasis::HVOL_WEDGE(basisDegree-1));
      basis_set.push_back(new typename  CG_HBasis::HVOL_WEDGE(basisDegree-1));

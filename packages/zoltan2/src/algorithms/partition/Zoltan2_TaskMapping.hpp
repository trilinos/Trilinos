...
public:
	...
	virtual void getMapping(
		...
		AlgMJ<pcoord_t, procId_t, procId_t> mj_partitioner;
...
public:
	...
	CoordinateTaskMapper(
			...
			ArrayRCP<procId_t>task_comm_xadj,
			ArrayRCP<procId_t>task_comm_adj,
			pcoord_t *task_communication_edge_weight_,
			int recursion_depth,
			procId_t *part_no_array,
...
void coordinateTaskMapperInterface(
		...
		procId_t num_tasks,
		tcoord_t **task_coords,
		procId_t *task_comm_xadj,
		procId_t *task_comm_adj,
...
)
{
	...
	for (procId_t i = 0; i < num_processors; ++i){
		proc_to_task_xadj[i] = proc_to_task_xadj_[i];
	}
	for (procId_t i = 0; i < num_tasks; ++i){

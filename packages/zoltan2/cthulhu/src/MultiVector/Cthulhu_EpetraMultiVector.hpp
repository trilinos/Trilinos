...
namespace Cthulhu {
  ...
  */
  ...
  public:
    ...
    EpetraMultiVector(const EpetraMultiVector<double,int,int> &source){ CTHULHU_DEBUG_ME; } 
    ...
    inline void putScalar(const double &value) { CTHULHU_DEBUG_ME; vec_->PutScalar(value); }
     ...
     */
    inline EpetraMultiVector<double,int,int>& operator=(const EpetraMultiVector<double,int,int> &source) { CTHULHU_DEBUG_ME; return vec_->(source); }
#endif // CTHULHU_NOT_IMPLEMENTED
    ...
    */
    ...
    inline Teuchos::RCP<MultiVector<double,int,int> > subCopy(const Teuchos::Range1D &colRng) const { CTHULHU_DEBUG_ME; return vec_->subCopy(colRng); }

    //! Returns a EpetraMultiVector with copies of selected columns.
    inline Teuchos::RCP<MultiVector<double,int,int> > subCopy(const Teuchos::ArrayView<const size_t> &cols) const { CTHULHU_DEBUG_ME; return vec_->subCopy(cols); }

    //! Returns a const MultiVector with const views of selected columns.
    inline Teuchos::RCP<const MultiVector<double,int,int> > subView(const Teuchos::Range1D &colRng) const { CTHULHU_DEBUG_ME; return vec_->subView(colRng); }

    //! Returns a const MultiVector with const views of selected columns.
    inline Teuchos::RCP<const MultiVector<double,int,int> > subView(const Teuchos::ArrayView<const size_t> &cols) const { CTHULHU_DEBUG_ME; return vec_->subView(cols); }

    //! Returns a MultiVector with views of selected columns.
    inline Teuchos::RCP<MultiVector<double,int,int> > subViewNonConst(const Teuchos::Range1D &colRng) { CTHULHU_DEBUG_ME; return vec_->subViewNonConst(colRng); }

    //! Returns a MultiVector with views of selected columns.
    inline Teuchos::RCP<MultiVector<double,int,int> > subViewNonConst(const Teuchos::ArrayView<const size_t> &cols) { CTHULHU_DEBUG_ME; return vec_->subViewNonConst(cols); }
    ...
    */
    inline Teuchos::RCP<const MultiVector<double,int,int> > offsetView(const Teuchos::RCP<const Map<int,int> > &subMap, size_t offset) const { CTHULHU_DEBUG_ME; return vec_->offsetView(subMap, offset); }
    ...
    /** 
        ...
        \pre  <tt>subMap->getNodeNumElements() + offset < this->getLocalLength()</tt>
    */
    inline Teuchos::RCP<MultiVector<double,int,int> > offsetViewNonConst(const Teuchos::RCP<const Map<int,int> > &subMap, size_t offset) { CTHULHU_DEBUG_ME; return vec_->offsetViewNonConst(subMap, offset); }

    //! Const Vector access function.
    inline Teuchos::RCP<const Vector<double,int,int> > getVector(size_t j) const { CTHULHU_DEBUG_ME; return vec_->getVector(j); }
    ...
    inline const Kokkos::MultiVector<double> & getLocalMV() const { CTHULHU_DEBUG_ME; return vec_->getLocalMV(); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA

#ifdef CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    //! Return a non-const reference to the underlying Kokkos::MultiVector object (advanced use only)
    inline Kokkos::MultiVector<double> & getLocalMVNonConst() { CTHULHU_DEBUG_ME; return vec_->getLocalMVNonConst(); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
...
scalarThis) {
    ...
    inline size_t getNumVectors() const { CTHULHU_DEBUG_ME; return vec_->NumVectors(); }
  ...
  */
  ...
  createEpetraMultiVector(const Teuchos::RCP< const Map<int,int> > &map, size_t numVectors) { CTHULHU_DEBUG_ME; }
#endif // CTHULHU_NOT_IMPLEMENTED

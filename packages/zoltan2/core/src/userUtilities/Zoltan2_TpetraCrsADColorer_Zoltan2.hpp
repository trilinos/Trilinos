...
public:
  ...
  // Constructor
  Zoltan2CrsColorer(const Teuchos::RCP<matrix_type> &matrix_) : base_type(matrix_) {}
  ...
  computeColoring(Teuchos::ParameterList &coloring_params) override
  {
    ...
    matrix_type C(this->matrix->getRowMap(), nzpr * nzpr); // Check this
    Tpetra::MatrixMatrix::Multiply(*(this->matrix), true, *(this->matrix), false, C);
    ...
    // Extract colors
    Zoltan2::ColoringSolution<Z2Adapter_t> *z2_solution = z2_problem.getSolution();
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        len != this->graph->getColMap()->getNodeNumElements(), std::logic_error, "Incorrect length of color list!");
    list_of_colors_host_type list_of_colors_tmp(local_list_of_colors.getRawPtr(), len);
    this->list_of_colors      = list_of_colors_type("list_of_colors", len);
    this->list_of_colors_host = Kokkos::create_mirror_view(this->list_of_colors);
...
class Zoltan2CrsColorer<BlockCrsMatrix<SC, LO, GO, NO>> : public CrsColorer<BlockCrsMatrix<SC, LO, GO, NO>>
{
public:
  ...
  computeColoring(Teuchos::ParameterList &coloring_params) override
  {
    TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error, "Zoltan2 colorer does not support Tpetra::BlockCrsMatrix!");
  }
};
} // namespace Tpetra

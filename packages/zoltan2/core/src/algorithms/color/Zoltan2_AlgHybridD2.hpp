  ...
  public:
	...
	for(size_t vid_d1_adj = vid_d1_adj_begin; vid_d1_adj < vid_d1_adj_end; vid_d1_adj++){
	  ...
	  if(vid_d1 < n_local){
	    ...
	    vid_d1_degree = ghost_degrees_dev(vid_d1-n_local);
	  }
	  if( vid_d1 != curr_lid && femv_colors(vid_d1) == curr_color){
	    if(curr_degree < vid_d1_degree && recolor_degrees){
	      ...
	      recoloring_size++;
	    } else if(rand_dev(curr_lid) < rand_dev(vid_d1)){
	      femv_colors(curr_lid) = 0;
	      recoloring_size++;
	      break;
	...
	for(int i = 0; i <boundary_verts_size ; i++){
	  ...
	  for(size_t vid_d1_adj = vid_d1_adj_begin; vid_d1_adj < vid_d1_adj_end; vid_d1_adj++){
		...
		break;
	        ...
	        femv_colors(vid_d1) = 0;
		recoloringSize(0)++;
	      } else if(rand_host(curr_lid) < rand_host(vid_d1)){
	        femv_colors(curr_lid) = 0;
		recoloringSize(0)++;
		break;
	      } else if(rand_host(vid_d1) < rand_host(curr_lid)){
	        femv_colors(vid_d1) = 0;
		recoloringSize(0)++;
	      } else {
	        if(gid_host(curr_lid) >= gid_host(vid_d1)){
		  femv_colors(curr_lid) = 0;
		  recoloringSize(0)++;
		  break;
		} else {

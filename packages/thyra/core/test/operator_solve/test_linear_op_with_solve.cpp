...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    if (!Thyra::run_linear_op_with_solve_tests<float>(
            n, as<float>(epsScale * ScalarTraits<float>::eps()), showAllTests, dumpAll, *out)) success = false;
#endif
    if (!Thyra::run_linear_op_with_solve_tests<double>(
            n, as<double>(epsScale * ScalarTraits<double>::eps()), showAllTests, dumpAll, *out)) success = false;

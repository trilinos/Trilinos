...
class SimpleDenseLinearOp : public virtual LinearOpDefaultBase<Scalar>,
 ...
 public:
   ...
   */
  void initialize(const RCP<MultiVectorBase<Scalar> > &mv) { mv_ = mv; }

  /** \brief . */
  RCP<MultiVectorBase<Scalar> > getNonconstMultiVector() { return mv_; }

 protected:
  // Overridden from LinearOpBase

  /** \brief . */
  virtual Teuchos::RCP<const Thyra::VectorSpaceBase<Scalar> > range() const { return mv_->range(); }

  /** \brief . */
  virtual Teuchos::RCP<const Thyra::VectorSpaceBase<Scalar> > domain() const { return mv_->domain(); }

  /** \brief . */
  virtual bool opSupportedImpl(Thyra::EOpTransp /* M_trans */) const { return true; }

...
class VectorSpaceConverterBase {
 public:
  ...
  createVectorSpaceTo(
      const VectorSpaceBase<ScalarFrom> &vecSpc) const = 0;
  ...
  createVectorSpaceFrom(
      const VectorSpaceBase<ScalarTo> &vecSpc) const = 0;
  ...
  virtual void convert(
      const MultiVectorBase<ScalarFrom> &mv_from, MultiVectorBase<ScalarTo> *mv_to) const = 0;
};

...
class SpmdVectorBase : virtual public VectorBase<Scalar>,
 ...
 public:
   ...
   */
  RTOpPack::SubVectorView<Scalar> getNonconstLocalSubVector() { return getNonconstLocalSubVectorImpl(); }

  /** \brief Get a const generalized view of local vector data.
   */
  RTOpPack::ConstSubVectorView<Scalar> getLocalSubVector() const { return getLocalSubVectorImpl(); }
   ...
   */
  void getNonconstLocalData(const Ptr<ArrayRCP<Scalar> > &localValues) { this->getNonconstLocalVectorDataImpl(localValues); }
   ...
   * all of the <tt>ArrayRCP</tt> objects (or setting them to null).
   */
  void getLocalData(const Ptr<ArrayRCP<const Scalar> > &localValues) const { this->getLocalVectorDataImpl(localValues); }

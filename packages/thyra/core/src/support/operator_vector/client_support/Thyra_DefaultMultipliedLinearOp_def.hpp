...
inline void DefaultMultipliedLinearOp<Scalar>::assertInitialized() const {
#ifdef TEUCHOS_DEBUG
...
template <class Scalar>
inline std::string DefaultMultipliedLinearOp<Scalar>::getClassName() const {
  return Teuchos::Describable::description();
}

template <class Scalar>
inline Ordinal DefaultMultipliedLinearOp<Scalar>::getRangeDim() const {
  return (numOps() > 0 ? this->range()->dim() : 0);
}

template <class Scalar>
inline Ordinal DefaultMultipliedLinearOp<Scalar>::getDomainDim() const {
    ...
    for (int k = 0; k < nOps; ++k) {
      ...
      if (k < nOps - 1) {
        THYRA_ASSERT_LINEAR_OP_TIMES_LINEAR_OP_SPACES_NAMES(
            getClassName() + "::initialize(...)", *Ops_[k], NOTRANS, ("Ops[" + toString(k) + "]"), *Ops_[k + 1], NOTRANS, ("Ops[" + toString(k + 1) + "]"));
      }
    }
  } catch (...) {
    uninitialize();

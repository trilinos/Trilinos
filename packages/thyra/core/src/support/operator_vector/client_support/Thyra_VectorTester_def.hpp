...
VectorTester<Scalar>::VectorTester(
    const ScalarMag warning_tol_in, const ScalarMag error_tol_in, const int num_random_vectors_in, const bool show_all_tests_in, const bool dump_all_in)
...
bool VectorTester<Scalar>::check(
    const VectorBase<Scalar> &v, Teuchos::FancyOStream *out_arg) const {
  using std::endl;
  ...
  if (out.get()) *out << endl
                      << "*** Entering Thyra::VectorTester<" << ST::name() << ">::check(v,...) ...\n";
  ...
  Teuchos::RCP<Thyra::VectorBase<Scalar> >
      t1 = createMember(vs),
      t2 = createMember(vs), t3 = createMember(vs), t4 = createMember(vs);
  ...
  if (out.get()) *out << endl
                      << "*** Leaving Thyra::VectorTester<" << ST::name() << ">::check(v,...) ...\n";

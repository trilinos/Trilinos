...
bool DefaultMultiVectorProductVectorSpace<Scalar>::isCompatible(
  ...
  const DefaultMultiVectorProductVectorSpace<Scalar> *multiVecProdVecSpc = dynamic_cast<const DefaultMultiVectorProductVectorSpace<Scalar> *>(&vecSpc);
...
bool DefaultMultiVectorProductVectorSpace<Scalar>::hasInCoreView(
    const Range1D &rng_in, const EViewType viewType, const EStrideType strideType) const {

...
void DefaultMultiVectorProductVector<Scalar>::absImpl(
  ...
  const RCP<const ProductVectorBase<Scalar> > pv = Teuchos::rcp_dynamic_cast<const ProductVectorBase<Scalar> >(Teuchos::rcpFromRef(x));
  if (nonnull(pv)) {
    for (int k = 0; k < numBlocks_; ++k) {
...
DefaultMultiVectorProductVector<Scalar>::norm2WeightedImpl(
    const VectorBase<Scalar>& x) const {
#ifdef TEUCHOS_DEBUG
  TEUCHOS_ASSERT(productSpace_->isCompatible(*x.space()));
#endif
  // Apply operation block-by-block if mv is also a ProductVector
  ...
  if (nonnull(pv)) {
    ...
    for (int k = 0; k < numBlocks_; ++k) {
      typename ST::magnitudeType subNorm = multiVec_.getConstObj()->col(k)->norm_2(*pv->getVectorBlock(k));
...
void DefaultMultiVectorProductVector<Scalar>::acquireDetachedVectorViewImpl(
    const Range1D& rng_in, RTOpPack::ConstSubVectorView<Scalar>* sub_vec) const {

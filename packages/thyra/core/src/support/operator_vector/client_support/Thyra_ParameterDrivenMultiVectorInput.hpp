...
class ParameterDrivenMultiVectorInput
 ...
 public:
  ...
  bool readMultiVector(
      const std::string &mvName, Thyra::MultiVectorBase<Scalar> *mv) const;
   ...
   */
  bool readVector(
      const std::string &vName, Teuchos::RCP<Thyra::VectorBase<Scalar> > *v) const;
...
template <class Scalar>
inline const Teuchos::Array<Scalar> &
...
template <class Scalar>
void copy(
    const Teuchos::Array<Scalar> &array, VectorBase<Scalar> *vec) {
...
void ParameterDrivenMultiVectorInput<Scalar>::setParameterList(
  ...
  explicitArray_ = Teuchos::getArrayFromStringParameter<Scalar>(
      *paramList_, ExplicitArray_name_, -1  // An array of any size will do here
      ,

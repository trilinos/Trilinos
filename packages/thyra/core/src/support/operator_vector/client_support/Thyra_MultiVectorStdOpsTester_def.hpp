...
MultiVectorStdOpsTester<Scalar>::MultiVectorStdOpsTester(
    const ScalarMag &warning_tol_in, const ScalarMag &error_tol_in, const int num_mv_cols_in)
...
bool MultiVectorStdOpsTester<Scalar>::checkStdOps(
    const VectorSpaceBase<Scalar> &vecSpc, std::ostream *out, const bool & /* dumpAll */
) {
  using Teuchos::as;
  using Teuchos::ptr;
  using Teuchos::tuple;
  ...
  ++tc;
  {
    ...
    if (!testRelErrors<Scalar, Scalar, ScalarMag>(
            "sums(*V1)", scalars1(),
            "-2.0*n", scalars2(),
            "error_tol", error_tol(), "warning_tol", warning_tol(), ptr(out))) success = false;
  }

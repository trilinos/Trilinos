...
VectorSpaceTester<Scalar>::VectorSpaceTester(
    const ScalarMag warning_tol_in, const ScalarMag error_tol_in, const int num_random_vectors_in, const int num_mv_cols_in, const bool show_all_tests_in, const bool dump_all_in)
...
bool VectorSpaceTester<Scalar>::check(
    const VectorSpaceBase<Scalar> &vs, Teuchos::FancyOStream *out_arg) const {
  using std::endl;
  ...
  if (nonnull(out)) *out << endl
                         << "*** Entering Thyra::VectorSpaceTester<" << ST::name() << ">::check(vs,...) ...\n";
  ...
  if (nonnull(out))
    *out
        ...
        << "vs.dim() = " << vs.dim()
        << endl
        << "vs.hasInCoreView() = " << vs.hasInCoreView() << std::endl;
  ...
  if (nonnull(out)) {
    ...
    *out << endl
         << "*** Leaving VectorSpaceTester<" << ST::name() << ">::check(vs,...)\n";
  }

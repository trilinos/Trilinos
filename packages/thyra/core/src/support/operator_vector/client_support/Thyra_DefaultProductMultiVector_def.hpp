...
void DefaultProductMultiVector<Scalar>::assignMultiVecImpl(
  ...
  const RCP<const ProductMultiVectorBase<Scalar> > pv = Teuchos::rcp_dynamic_cast<const ProductMultiVectorBase<Scalar> >(Teuchos::rcpFromRef(mv));
  if (nonnull(pv)) {
    for (int k = 0; k < numBlocks_; ++k) {
...
void DefaultProductMultiVector<Scalar>::mvMultiReductApplyOpImpl(
    ...
    const Ordinal primary_global_offset_in) const {

...
inline void DefaultAddedLinearOp<Scalar>::assertInitialized() const {
#ifdef TEUCHOS_DEBUG
...
template <class Scalar>
inline std::string DefaultAddedLinearOp<Scalar>::getClassName() const {
  return Teuchos::Describable::description();
}

template <class Scalar>
inline Ordinal DefaultAddedLinearOp<Scalar>::getRangeDim() const {
  return (numOps() > 0 ? this->range()->dim() : 0);
}

template <class Scalar>
inline Ordinal DefaultAddedLinearOp<Scalar>::getDomainDim() const {
    ...
    for (int k = 0; k < l_numOps; ++k) {
      ...
      if (k > 0) {
        THYRA_ASSERT_LINEAR_OP_PLUS_LINEAR_OP_SPACES_NAMES(
            getClassName() + "::initialize(...)", *Ops_[0], NOTRANS, ("Ops[0]"), *Ops_[k], NOTRANS, ("Ops[" + toString(k) + "]"));
      }
    }
  } catch (...) {
    uninitialize();

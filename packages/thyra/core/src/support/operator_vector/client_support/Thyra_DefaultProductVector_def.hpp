...
DefaultProductVector<Scalar>::norm2WeightedImpl(
  ...
  if (nonnull(pv)) {
    ...
    for (int k = 0; k < numBlocks_; ++k) {
      typename ST::magnitudeType subNorm = vecs_[k].getConstObj()->norm_2(*pv->getVectorBlock(k));
...
void DefaultProductVector<Scalar>::applyOpImpl(
  ...
  for (int k = 0; k < num_vecs; ++k) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        test_failed, Exceptions::IncompatibleVectorSpaces, "DefaultProductVector::applyOp(...): Error vecs[" << k << "]->space() = " << vecs[k]->space()->description() << "\' is not compatible with this "
                                                                                                             << "vector space = " << this->space()->description() << "!");
  }
  for (int k = 0; k < num_targ_vecs; ++k) {
    test_failed = !this->space()->isCompatible(*targ_vecs[k]->space());
    TEUCHOS_TEST_FOR_EXCEPTION(
        test_failed, Exceptions::IncompatibleVectorSpaces, "DefaultProductVector::applyOp(...): Error targ_vecs[" << k << "]->space() = " << targ_vecs[k]->space()->description() << "\' is not compatible with this "
                                                                                                                  << "vector space = " << this->space()->description() << "!");
  }
#endif
...
#endif
  if (
      rng.lbound() + rng.size() <= kth_global_offset + vecs_[kth_vector_space].getConstObj()->space()->dim()) {
...
#endif
  if (
      sub_vec->globalOffset() + sub_vec->subDim() <= kth_global_offset + vecs_[kth_vector_space].getConstObj()->space()->dim()) {
...
void DefaultProductVector<Scalar>::norms2Impl(
  ...
  norms[0] = ST::magnitude(ST::zero());
  for (int k = 0; k < numBlocks_; ++k) {
    typename ST::magnitudeType subNorm = vecs_[k].getConstObj()->norm_2();

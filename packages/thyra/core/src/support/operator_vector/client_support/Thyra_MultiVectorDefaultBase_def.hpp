...
MultiVectorDefaultBase<Scalar>::nonContigSubViewImpl(
  ...
  for (int k = 0; k < numCols; ++k) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        !(0 <= col_k && col_k < dimDomain), std::invalid_argument, msg_err << " col[" << k << "] = " << col_k << " is not in the range [0," << (dimDomain - 1) << "]!");
#endif
...
void MultiVectorDefaultBase<Scalar>::mvMultiReductApplyOpImpl(
    ...
    const Ordinal prim_global_offset_in) const {
  using Teuchos::as;
  using Teuchos::Workspace;
  Teuchos::WorkspaceStore *wss = Teuchos::get_default_workspace_store().get();
...
#ifdef TEUCHOS_DEBUG
  TEUCHOS_TEST_FOR_EXCEPTION(
      !(rowRng.ubound() < rangeDim), std::out_of_range, "MultiVectorDefaultBase<Scalar>::acquireDetachedMultiVectorViewImpl(...): Error, rowRng = [" << rowRng.lbound() << "," << rowRng.ubound() << "] is not in the range = [0," << (rangeDim - 1) << "]!");
  TEUCHOS_TEST_FOR_EXCEPTION(
      !(colRng.ubound() < domainDim), std::out_of_range, "MultiVectorDefaultBase<Scalar>::acquireDetachedMultiVectorViewImpl(...): Error, colRng = [" << colRng.lbound() << "," << colRng.ubound() << "] is not in the range = [0," << (domainDim - 1) << "]!");
#endif

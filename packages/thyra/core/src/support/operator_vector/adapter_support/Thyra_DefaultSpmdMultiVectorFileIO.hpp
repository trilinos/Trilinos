...
class DefaultSpmdMultiVectorFileIO
 ...
 public:
  ...
  DefaultSpmdMultiVectorFileIO(
      const std::string &extensionTagName = "", const int numProcs = -1, const int procRank = -1);
  ...
  void readMultiVectorFromFile(
      const std::string &fileNameBase, Thyra::MultiVectorBase<Scalar> *mv) const;
  /** \brief . */
  void writeMultiVectorToFile(
      const Thyra::MultiVectorBase<Scalar> &mv, const std::string &fileNameBase) const;
  //@}
...
DefaultSpmdMultiVectorFileIO<Scalar>::DefaultSpmdMultiVectorFileIO(
    const std::string &extensionTagName, const int numProcs, const int procRank)
...
bool DefaultSpmdMultiVectorFileIO<Scalar>::isCompatible(
    const MultiVectorBase<Scalar> &mv) const {
...
void DefaultSpmdMultiVectorFileIO<Scalar>::readMultiVectorFromFile(
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      in_file.eof(), std::logic_error, "Error, the file \"" << fileName << "\" could not be opened for input!");

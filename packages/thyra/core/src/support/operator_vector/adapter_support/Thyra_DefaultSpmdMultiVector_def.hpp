...
class CopyBackSpmdMultiVectorEntries {
 public:
  CopyBackSpmdMultiVectorEntries(
    ...
    : cols_(cols)
    , localValuesView_(localValuesView)
...
template <class Scalar>
DefaultSpmdMultiVector<Scalar>::DefaultSpmdMultiVector(
    const RCP<const SpmdVectorSpaceBase<Scalar> > &spmdRangeSpace, const RCP<const ScalarProdVectorSpaceBase<Scalar> > &domainSpace) {
  initialize(spmdRangeSpace, domainSpace);
}

template <class Scalar>
DefaultSpmdMultiVector<Scalar>::DefaultSpmdMultiVector(
    const RCP<const SpmdVectorSpaceBase<Scalar> > &spmdRangeSpace, const RCP<const ScalarProdVectorSpaceBase<Scalar> > &domainSpace, const ArrayRCP<Scalar> &localValues, const Ordinal leadingDim) {
...
void DefaultSpmdMultiVector<Scalar>::uninitialize(
    RCP<const SpmdVectorSpaceBase<Scalar> > *spmdRangeSpace, RCP<const ScalarProdVectorSpaceBase<Scalar> > *domainSpace, ArrayRCP<Scalar> *localValues, Ordinal *leadingDim) {
...
DefaultSpmdMultiVector<Scalar>::contigSubViewImpl(
    const Range1D &col_rng_in) const {
  ...
  return Teuchos::rcp(
      new DefaultSpmdMultiVector<Scalar>(
          ...
          Teuchos::rcp_dynamic_cast<const ScalarProdVectorSpaceBase<Scalar> >(
              spmdRangeSpace_->smallVecSpcFcty()->createVecSpc(colRng.size()), true),

...
template <class Scalar>
DefaultStateEliminationModelEvaluator<Scalar>::DefaultStateEliminationModelEvaluator(
    const Teuchos::RCP<ModelEvaluator<Scalar> > &thyraModel, const Teuchos::RCP<NonlinearSolverBase<Scalar> > &stateSolver) {
...
void DefaultStateEliminationModelEvaluator<Scalar>::initialize(
  ...
  wrappedThyraModel_ = Teuchos::rcp(
      new DefaultNominalBoundsOverrideModelEvaluator<Scalar>(
          Teuchos::rcp_const_cast<ModelEvaluator<Scalar> >(thyraModel), Teuchos::null));
  stateSolver_->setModel(wrappedThyraModel_);
}

template <class Scalar>
void DefaultStateEliminationModelEvaluator<Scalar>::uninitialize(
    Teuchos::RCP<ModelEvaluator<Scalar> > *thyraModel, Teuchos::RCP<NonlinearSolverBase<Scalar> > *stateSolver) {
...
void DefaultStateEliminationModelEvaluator<Scalar>::evalModelImpl(
  ...
  VOTSNSB statSolver_outputTempState(
      stateSolver_, out, static_cast<int>(verbLevel) >= static_cast<int>(Teuchos::VERB_LOW) ? Teuchos::VERB_LOW : Teuchos::VERB_NONE);
  ...
  if (solveStatus.solveStatus == SOLVE_STATUS_CONVERGED) {
    ...
    for (int l = 0; l < Np; ++l) {
      for (int j = 0; j < Ng; ++j) {
        if (
            outArgs.supports(MEB::OUT_ARG_DgDp, j, l).none() == false && outArgs.get_DgDp(j, l).isEmpty() == false) {
    ...
    for (int l = 0; l < Np; ++l) {
      if (
          wrappedOutArgs.supports(MEB::OUT_ARG_DfDp, l).none() == false && wrappedOutArgs.get_DfDp(l).isEmpty() == false) {
        //

...
class DampenedNewtonNonlinearSolver : public NonlinearSolverBase<Scalar> {
 public:
  ...
  DampenedNewtonNonlinearSolver(
      const ScalarMag defaultTol = 1e-2, const int defaultMaxNewtonIterations = 1000, const bool useDampenedLineSearch = true, const Scalar armijoConstant = 1e-4, const int maxLineSearchIterations = 20);
...
DampenedNewtonNonlinearSolver<Scalar>::DampenedNewtonNonlinearSolver(
    const ScalarMag my_defaultTol, const int my_defaultMaxNewtonIterations, const bool my_useDampenedLineSearch, const Scalar my_armijoConstant, const int my_maxLineSearchIterations)
...
DampenedNewtonNonlinearSolver<Scalar>::solve(
    VectorBase<Scalar> *x_inout, const SolveCriteria<Scalar> *solveCriteria, VectorBase<Scalar> *delta) {
  using std::endl;
  using Teuchos::as;
  ...
  for (newtonIter = 1; newtonIter <= maxIters; ++newtonIter) {
    ...
    if (lineSearchIter > maxLineSearchIterations()) {
      ...
      solveStatus.message = oss.str();
      if (out.get() && (showNewtonIters || showNewtonDetails)) *out << endl
                                                                    << oss.str() << endl;
      goto exit;
    }

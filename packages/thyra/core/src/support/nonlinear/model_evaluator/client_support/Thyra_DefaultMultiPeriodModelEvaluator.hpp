...
class DefaultMultiPeriodModelEvaluator
 ...
 private:
  ...
  static RCP<ProductVectorBase<Scalar> >
  createProductVector(
...
DefaultMultiPeriodModelEvaluator<Scalar>::createOutArgsImpl() const {
  ...
  for (int l = 0; l < Np_; ++l) {
    ...
    const MEB::DerivativeSupport period_DfDp_l_support = periodOutArgs.supports(MEB::OUT_ARG_DfDp, period_l);
  ...
  for (int l = 0; l < Np_; ++l) {
    const int period_l                                 = this->period_l(l);
    const MEB::DerivativeSupport period_DgDp_l_support = periodOutArgs.supports(MEB::OUT_ARG_DgDp, g_index_, period_l);

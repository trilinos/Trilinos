...
ModelEvaluatorBase::DerivativeMultiVector<Scalar>
get_dmv(
    const ModelEvaluatorBase::Derivative<Scalar> &deriv, const std::string &derivName);

/** \relates ModelEvaluatorDefaultBase */
template <class Scalar>
RCP<MultiVectorBase<Scalar> >
get_mv(
    const ModelEvaluatorBase::Derivative<Scalar> &deriv, const std::string &derivName, ModelEvaluatorBase::EDerivativeMultiVectorOrientation orientation);
...
template <class Scalar>
void eval_f(
    const ModelEvaluator<Scalar> &model, const VectorBase<Scalar> &x, VectorBase<Scalar> *f);

/** \brief Evaluate <tt>f(x)</tt> and <tt>W(x) = DfDx(x)</tt>. */
template <class Scalar>
void eval_f_W(
    const ModelEvaluator<Scalar> &model, const VectorBase<Scalar> &x, VectorBase<Scalar> *f, LinearOpWithSolveBase<Scalar> *W);

/** \brief Evaluate <tt>f(x,t)</tt>. */
template <class Scalar>
void eval_f(
    const ModelEvaluator<Scalar> &model, const VectorBase<Scalar> &x, const Scalar &t, VectorBase<Scalar> *f);
...
template <class Scalar>
void eval_f(
    const ModelEvaluator<Scalar> &model, const VectorBase<Scalar> &x_dot, const VectorBase<Scalar> &x, const typename ModelEvaluatorBase::InArgs<Scalar>::ScalarMag &t, VectorBase<Scalar> *f);

/** \brief Evaluate <tt>f(x_dot,x,t)</tt> and <tt>W(x_dot,x,t,alpha,beta) =
 * alpha*DfDx_dot(x_dot,x,t) + beta*DfDx(x_dot,x,t)</tt>. */
template <class Scalar>
void eval_f_W(
    const ModelEvaluator<Scalar> &model, const VectorBase<Scalar> &x_dot, const VectorBase<Scalar> &x, const typename ModelEvaluatorBase::InArgs<Scalar>::ScalarMag &t, const Scalar &alpha, const Scalar &beta, VectorBase<Scalar> *f, LinearOpWithSolveBase<Scalar> *W);
...
template <class Scalar>
void eval_f_poly(
    const ModelEvaluator<Scalar> &model, const Teuchos::Polynomial<VectorBase<Scalar> > &x_poly, const typename ModelEvaluatorBase::InArgs<Scalar>::ScalarMag &t, Teuchos::Polynomial<VectorBase<Scalar> > *f_poly);

/** \brief . */
template <class Scalar>
void eval_f_poly(
    const ModelEvaluator<Scalar> &model, const Teuchos::Polynomial<VectorBase<Scalar> > &x_dot_poly, const VectorBase<Scalar> &x_poly, const typename ModelEvaluatorBase::InArgs<Scalar>::ScalarMag &t, Teuchos::Polynomial<VectorBase<Scalar> > *f_poly);
  ...
  switch (orientation) {
    case MEB::DERIV_MV_BY_COL:
      return MEB::DerivativeMultiVector<Scalar>(
          createMembers(model.get_g_space(j), model.get_x_space()->dim()), MEB::DERIV_MV_BY_COL);
    case MEB::DERIV_TRANS_MV_BY_ROW:
      return MEB::DerivativeMultiVector<Scalar>(
          createMembers(model.get_x_space(), model.get_g_space(j)->dim()), MEB::DERIV_TRANS_MV_BY_ROW);
    default:
...
Thyra::create_DgDp_mv(
  ...
  switch (orientation) {
    case MEB::DERIV_MV_BY_COL:
      return MEB::DerivativeMultiVector<Scalar>(
          createMembers(model.get_g_space(j), model.get_p_space(l)->dim()), MEB::DERIV_MV_BY_COL);
    case MEB::DERIV_TRANS_MV_BY_ROW:
      return MEB::DerivativeMultiVector<Scalar>(
          createMembers(model.get_p_space(l), model.get_g_space(j)->dim()), MEB::DERIV_TRANS_MV_BY_ROW);
    default:
      TEUCHOS_TEST_FOR_EXCEPT(true);
  }
  TEUCHOS_UNREACHABLE_RETURN(MEB::DerivativeMultiVector<Scalar>());
...
Thyra::get_dmv(
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      deriv.getLinearOp().get() != NULL, std::logic_error, "Error, LinearOpBase type not expected for " << derivName << "!");
...
Thyra::get_mv(
  ...
  if (mv.get()) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        dmv.getOrientation() != orientation, std::logic_error, "Error, the orientation " << toString(dmv.getOrientation()) << " is not the"
                                                                                                                              " expected orientation of "
                                                                                         << toString(orientation) << " for " << derivName << "!");
  }
  return mv;
}
...
void Thyra::assertInArgsOutArgsSetup(
  ...
  for (int l = 0; l < Np; ++l) {
    ...
    for (int j = 0; j < Ng; ++j) {
      // DgDx_dot(j)
      TEUCHOS_TEST_FOR_EXCEPTION(
          (!outArgs.supports(MEB::OUT_ARG_DgDx_dot, j).none() && !inArgs.supports(MEB::IN_ARG_x_dot)),
          std::logic_error,
          "Error: The underlying model " << modelEvalDescription << " says that\n"
                                                                    "it supports DgDx_dot("
                                         << j << ") but it does not support x_dot!");

      // DgDx(j)
      TEUCHOS_TEST_FOR_EXCEPTION(
          (!outArgs.supports(MEB::OUT_ARG_DgDx, j).none() && !inArgs.supports(MEB::IN_ARG_x)),
          std::logic_error,
          "Error: The underlying model " << modelEvalDescription << " says that\n"
                                                                    "it supports DgDx("
                                         << j << ") but it does not support x!");

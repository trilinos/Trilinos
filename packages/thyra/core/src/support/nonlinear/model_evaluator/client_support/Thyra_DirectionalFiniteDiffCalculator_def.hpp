...
class OutArgsCreator : public StateFuncModelEvaluatorBase<Scalar> {
 public:
  ...
  ModelEvaluatorBase::InArgs<Scalar> createInArgs() const {
    TEUCHOS_TEST_FOR_EXCEPT(true);
    TEUCHOS_UNREACHABLE_RETURN(ModelEvaluatorBase::InArgs<Scalar>());
  }
  ModelEvaluatorBase::OutArgs<Scalar> createOutArgs() const {
    TEUCHOS_TEST_FOR_EXCEPT(true);
    TEUCHOS_UNREACHABLE_RETURN(ModelEvaluatorBase::OutArgs<Scalar>());
  }
  void evalModel(
      const ModelEvaluatorBase::InArgs<Scalar> &inArgs,
      const ModelEvaluatorBase::OutArgs<Scalar> &outArgs) const { TEUCHOS_TEST_FOR_EXCEPT(true); }
...
DirectionalFiniteDiffCalculator<Scalar>::fdMethodValidator() {
  static RCP<Teuchos::StringToIntegralParameterEntryValidator<EFDMethodType> >
      loc_fdMethodValidator = Teuchos::rcp(
          new Teuchos::StringToIntegralParameterEntryValidator<Thyra::DirectionalFiniteDiffCalculatorTypes::EFDMethodType>(
              Teuchos::tuple<std::string>(
                  "order-one", "order-two", "order-two-central", "order-two-auto", "order-four", "order-four-central", "order-four-auto"),
              Teuchos::tuple<std::string>(
                  "Use O(eps) one sided finite differences (cramped bounds)", "Use O(eps^2) one sided finite differences (cramped bounds)", "Use O(eps^2) two sided central finite differences", "Use \"order-two-central\" when not cramped by bounds, otherwise use \"order-two\"", "Use O(eps^4) one sided finite differences (cramped bounds)", "Use O(eps^4) two sided central finite differences", "Use \"order-four-central\" when not cramped by bounds, otherwise use \"order-four\""),
...
DirectionalFiniteDiffCalculator<Scalar>::fdStepSelectTypeValidator() {
  static const RCP<Teuchos::StringToIntegralParameterEntryValidator<EFDStepSelectType> >
      loc_fdStepSelectTypeValidator = Teuchos::rcp(
          new Teuchos::StringToIntegralParameterEntryValidator<Thyra::DirectionalFiniteDiffCalculatorTypes::EFDStepSelectType>(
              Teuchos::tuple<std::string>(
                  "Absolute", "Relative"),
              Teuchos::tuple<std::string>(
                  "Use absolute step size \"" + FDStepLength_name() + "\"", "Use relative step size \"" + FDStepLength_name() + "\"*||xo||inf"),
              Teuchos::tuple<Thyra::DirectionalFiniteDiffCalculatorTypes::EFDStepSelectType>(
                  Thyra::DirectionalFiniteDiffCalculatorTypes::FD_STEP_ABSOLUTE, Thyra::DirectionalFiniteDiffCalculatorTypes::FD_STEP_RELATIVE),
              ""));
...
DirectionalFiniteDiffCalculator<Scalar>::DirectionalFiniteDiffCalculator(
    EFDMethodType fd_method_type_in, EFDStepSelectType fd_step_select_type_in, ScalarMag fd_step_size_in, ScalarMag fd_step_size_min_in)
...
void DirectionalFiniteDiffCalculator<Scalar>::calcDerivatives(
  ...
  for (int l = 0; l < Np; ++l) {
    ...
    // DfDp(l)
    if (
        !deriv.supports(MEB::OUT_ARG_DfDp, l).none() && !deriv.get_DfDp(l).isEmpty()) {
      hasDerivObject = true;

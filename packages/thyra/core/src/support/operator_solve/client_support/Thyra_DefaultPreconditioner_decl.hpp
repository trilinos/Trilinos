...
class DefaultPreconditioner : virtual public PreconditionerBase<Scalar> {
 public:
  ...
  void initializeLeftRight(
      const Teuchos::RCP<LinearOpBase<Scalar> > &leftPrecOp, const Teuchos::RCP<LinearOpBase<Scalar> > &rightPrecOp);

  /** \brief Initialize a const-only split left/right preconditioner.
   */
  void initializeLeftRight(
      const Teuchos::RCP<const LinearOpBase<Scalar> > &leftPrecOp, const Teuchos::RCP<const LinearOpBase<Scalar> > &rightPrecOp);

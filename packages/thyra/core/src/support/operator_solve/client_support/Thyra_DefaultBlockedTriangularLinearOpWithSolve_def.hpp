...
template <class Scalar>
bool DefaultBlockedTriangularLinearOpWithSolve<Scalar>::solveSupportsImpl(
    EOpTransp M_trans) const {
  assertBlockFillIsActive(false);
  for (int k = 0; k < numDiagBlocks_; ++k) {
...
template <class Scalar>
bool DefaultBlockedTriangularLinearOpWithSolve<Scalar>::solveSupportsSolveMeasureTypeImpl(
...
void DefaultBlockedTriangularLinearOpWithSolve<Scalar>::assertBlockFillIsActive(
    bool blockFillIsActive_in) const {
#ifdef THYRA_DEBUG

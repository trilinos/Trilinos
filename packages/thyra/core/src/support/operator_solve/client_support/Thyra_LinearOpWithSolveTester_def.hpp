...
template <class Scalar>
void LinearOpWithSolveTester<Scalar>::set_all_solve_tol(
...
template <class Scalar>
void LinearOpWithSolveTester<Scalar>::set_all_slack_warning_tol(
...
template <class Scalar>
void LinearOpWithSolveTester<Scalar>::set_all_slack_error_tol(
...
bool LinearOpWithSolveTester<Scalar>::check(
  ...
  if (out.get()) {
    *out << endl
         << "*** Entering LinearOpWithSolveTester<" << ST::name() << ">::check(op,...) ...\n";
    if (show_all_tests()) {
      *out << endl
           << "describe forward op:\n"
      ...
      *testOut
      ...
      for (int rand_vec_i = 1; rand_vec_i <= num_random_vectors(); ++rand_vec_i) {
        ...
        *testOut << endl
        ...
        norms(*v4, norms_v4());
        std::transform(
            norms_v4.begin(), norms_v4.end(), norms_v1.begin(), norms_v4_norms_v1.begin(), std::divides<ScalarMag>());
        ...
        norms(*v5, norms_v5());
        std::transform(
            norms_v5.begin(), norms_v5.end(), norms_v2.begin(), norms_v5_norms_v2.begin(), std::divides<ScalarMag>());
      ...
      for (int rand_vec_i = 1; rand_vec_i <= num_random_vectors(); ++rand_vec_i) {
        ...
        *testOut << endl
        ...
        SolveCriteria<Scalar> solveCriteria(
            SolveMeasureType(SOLVE_MEASURE_NORM_RESIDUAL, SOLVE_MEASURE_NORM_RHS), forward_residual_solve_tol());
        assign(v3.ptr(), ST::zero());
        SolveStatus<Scalar> solveStatus;
        {
          VOTS lowsTempState(Teuchos::rcp(&op, false), testOut, verbLevel);
        ...
        *testOut
            << endl
            << "check: solveStatus = " << toString(solveStatus.solveStatus) << " == SOLVE_STATUS_CONVERGED : "
        ...
        *testOut << endl
        ...
        norms(*v4, norms_v4());
        std::transform(
            norms_v4.begin(), norms_v4.end(), norms_v2.begin(), norms_v4_norms_v2.begin(), std::divides<ScalarMag>());
      ...
      *testOut
      ...
      for (int rand_vec_i = 1; rand_vec_i <= num_random_vectors(); ++rand_vec_i) {
        ...
        *testOut << endl
        ...
        Teuchos::RCP<MultiVectorBase<Scalar> > v3 = createMembers(range, l_num_rhs);
        SolveCriteria<Scalar> solveCriteria(
            SolveMeasureType(SOLVE_MEASURE_NORM_RESIDUAL, SOLVE_MEASURE_NORM_RHS), adjoint_residual_solve_tol());
        assign(v3.ptr(), ST::zero());
        SolveStatus<Scalar> solveStatus;
        {
          VOTS lowsTempState(Teuchos::rcp(&op, false), testOut, verbLevel);
  ...
  if (out.get()) {
    ...
    *out << endl
         << "*** Leaving LinearOpWithSolveTester<" << ST::name() << ">::check(...)\n";
  }

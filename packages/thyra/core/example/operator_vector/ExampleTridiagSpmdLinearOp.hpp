...
class ExampleTridiagSpmdLinearOp : public Thyra::LinearOpDefaultBase<Scalar> {
 public:
  ...
  ExampleTridiagSpmdLinearOp(
      ...
      const Teuchos::ArrayView<const Scalar> &upper) { this->initialize(comm, localDim, lower, diag, upper); }
...
void ExampleTridiagSpmdLinearOp<Scalar>::applyImpl(
  ...
  for (Ordinal col_j = 0; col_j < m; ++col_j) {
    ...
    if (beta == zero) {
      // First local row
      y[k] = alpha * ((first ? zero : lower_[lk] * x_km1) + diag_[k] * x[k] + upper_[k] * x[k + 1]);
      if (!first) ++lk;
      // Middle local rows
      ...
      // First local row
      y[k] = alpha * ((first ? zero : lower_[lk] * x_km1) + diag_[k] * x[k] + upper_[k] * x[k + 1]) + beta * y[k];
      if (!first) ++lk;
      // Middle local rows
      for (k = 1; k < localDim - 1; ++lk, ++k)

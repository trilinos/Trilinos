...
#ifndef THYRA_MPI_VECTOR_SPACE_FACTORY_STD_DECL_HPP
#define THYRA_MPI_VECTOR_SPACE_FACTORY_STD_DECL_HPP
 ...
 * serial locally-replicated distributed <tt>DefaultMPIVectorSpace</tt>
 * objects.
 *
 ...
 * domain space of <tt>DefaultMPIMultiVector</tt>.  In addition, an object
...
class DefaultMPIVectorSpaceFactory : public VectorSpaceFactoryBase<Scalar> {
public:

  /** \brief Construct with an <tt>MPI_Comm</tt> object.
   *
   ...
   * </ul>
   */
  DefaultMPIVectorSpaceFactory( MPI_Comm  mpiComm );
  ...
  /** \brief Create a new locally-replicated <tt>DefaultMPIVectorSpace</tt> object given its dimension!
   *
   ...
   *
   \code
   return Teuchos::rcp(new DefaultMPIVectorSpace(this->mpiComm(),dim,dim))</tt>
   \endcode
   *
...
private:
  ...
  DefaultMPIVectorSpaceFactory(); // Not defined and not to be called!
    
}; // end class DefaultMPIVectorSpaceFactory
...
template<class Scalar>
inline
MPI_Comm DefaultMPIVectorSpaceFactory<Scalar>::mpiComm() const
{
  return mpiComm_;
}
...
#endif  // THYRA_MPI_VECTOR_SPACE_FACTORY_STD_DECL_HPP

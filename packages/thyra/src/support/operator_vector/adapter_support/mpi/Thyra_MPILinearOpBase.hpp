  ...
  ) const
{
  ...
  const ConstDetachedMultiVectorView<Scalar>         local_X_ev(X,Range1D(localOffsetDomain,localOffsetDomain+localDimDomain-1));
  const DetachedMultiVectorView<Scalar>  local_Y_ev(*Y,Range1D(localOffsetRange,localOffsetRange+localDimRange-1));
...
#endif
  Teuchos::RefCountPtr<const DefaultMPIVectorSpace<Scalar> >
    range  = Teuchos::rcp(new DefaultMPIVectorSpace<Scalar>(mpiComm,localDimRange,-1)),
    domain = Teuchos::rcp(new DefaultMPIVectorSpace<Scalar>(mpiComm,localDimDomain,-1));
  range_  = range;
  domain_ = domain;
  sp_range_  = range;
  sp_domain_ = domain;
  ...
  ) const
{
  for(Index j = 0; j < local_X.numSubCols(); ++j ) {
    const RTOpPack::ConstSubVectorView<Scalar>         local_X_j = local_X.col(j);
    const RTOpPack::SubVectorView<Scalar>  local_Y_j = local_Y->col(j);

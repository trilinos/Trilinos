 ...
 * <tt>DefaultMPIMultiVector</tt> objects.
 *
 * This is a simple but yet fully general and functional concrete
 * subclass of <tt>MPIVectorSpace</tt> that creates <tt>DefaultMPIMultiVector</tt>
 * objects from <tt>createMembers()</tt> and <tt>DefaultVectorMultiVector</tt>
...
class DefaultMPIVectorSpace : public MPIVectorSpaceDefaultBase<Scalar> {
public:
  ...
  DefaultMPIVectorSpace();

  /// Calls <tt>initialize()</tt>.
  DefaultMPIVectorSpace( MPI_Comm mpiComm, const Index localSubDim, const Index globalDim );
  ...
  //@{
  /** \brief Returns true if all the elements in <tt>rng</tt> are on this
   * processor.
   */
...
}; // end class DefaultMPIVectorSpace

...
SerialVectorSpaceConverterStd<ScalarFrom,ScalarTo>::createVectorSpaceTo(
  const VectorSpaceBase<ScalarFrom>  &vecSpc
  ) const
{
  return Teuchos::rcp(new SerialVectorSpaceStd<ScalarTo>(vecSpc.dim()));
}

template<class ScalarFrom, class ScalarTo>
Teuchos::RefCountPtr<const VectorSpaceBase<ScalarFrom> >
SerialVectorSpaceConverterStd<ScalarFrom,ScalarTo>::createVectorSpaceFrom(
  const VectorSpaceBase<ScalarTo>  &vecSpc
  ) const
{
  return Teuchos::rcp(new SerialVectorSpaceStd<ScalarFrom>(vecSpc.dim()));
}

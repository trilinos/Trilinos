...
class NewtonNonlinearSolver : public NonlinearSolverBase<Scalar> {
public:
...
template <class Scalar>
NewtonNonlinearSolver<Scalar>::NewtonNonlinearSolver(
...
template <class Scalar>
SolveStatus<Scalar> NewtonNonlinearSolver<Scalar>::solve(
  const ModelEvaluator<Scalar>          &model
  ,VectorBase<Scalar>                   *x
  ,const SolveCriteria<Scalar>          *solveCriteria
  ) const
{
  ...
  for( ; iter <= maxIters; ++iter ) {
    ...
    if(linearSolveStatus.solveStatus != SOLVE_STATUS_CONVERGED) {
      warningOut()
        << "Thyra::NewtonNonlinearSolver<Scalar>::solve(...), Warning, linear solve did not converge with solve status:\n"

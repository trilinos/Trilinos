  ...
  *c3 = num_objs = lb->Get_Num_Local_Obj(lb->Object_Type);
  ...
  if(num_objs > 0) {
...
#if 0
    /* processor 0 creates the global root */
    if(msg_mypid == 0) {
      root = POC_new();
    ...
    if(msg_nprocs > 1) {
      n = msg_nprocs;
      remainder = 8;
      for(; remainder >= 8; level++) {
	int pr;

	pr = (int)pow(8, level);
	/* fprintf(stderr,"n = %d, pow = %d, level = %d\n", n, pr, level); */
	remainder = n - pr;
      }
      if(remainder == 0)
	level--;
    ...
    /* POC_setparent(root, NULL, -1); */
    for(i=0; i<level; i++) {
      cursor = root;
      while(cursor != NULL) {
	if(POC_isTerminal(cursor)) {
	  cursor2 = POC_nextDfs(cursor);
	  /* fprintf(stderr,"refining octant\n"); */
    ...
    /* ignoring the z+ octants */
    /*hold = count = (int)pow(dimensions, level) / 2;*/
    hold = count = (int)pow(8, level);

    part = count / msg_nprocs;
    if(msg_mypid == -1)
      fprintf(stderr,"level = %d, part = %d, count = %d\n", 
	      level, part, count);
    remainder = count % msg_nprocs;
    extra = msg_nprocs - remainder;
    proc = 0;
    array = (Map *)malloc(count * sizeof(Map));
    if(array == NULL) {
    ...
    if(msg_mypid == 0) {
      cursor = root;
      while(cursor != NULL) {
	fprintf(stderr, "looking at octant %d\n", POC_id(cursor));
	cursor = POC_nextDfs(cursor);
      }
    }
    msg_sync();
...
#if 0
    {
      ...
      rlist = POC_localroots();
      while(rlist != NULL) {
	double min[3], max[3];
	POC_bounds(rlist->oct, min, max);
	fprintf(stderr, "(%d) octant %d (min %lf %lf %lf, max %lf %lf %lf)\n",
		msg_mypid, POC_id(rlist->oct), min[0], min[1], min[2],
		max[0], max[1], max[2]);
	rlist = rlist->next;
      }
      ...
      if(msg_mypid == 0) {
	for(s=0; s<i; s++)
	  fprintf(stderr,"i = %d, [%d]: proc %d\n",
		  i, s, array[s].npid);		  
	/*
	 * (min %lf %lf %lf, max %lf %lf %lf)\n 
	 * , array[s].min[0], array[s].min[1],
	 * array[s].min[2], array[s].max[0], array[s].max[1], 
	 * array[s].max[2]);
	 */
      }
      msg_sync();
  ...
  fprintf(stderr,"number of extra regions %d\n", num_extra);
...
int oct_global_insert_object(pRegion Region_list, int num_objs) {
  ...
  for(i=0; i<num_objs; i++) {
    ...
    if (!oct_global_insert(region)) {
      ...
      count++;
      fprintf(stderr,"(%d) region %d not attached\n", 
	      msg_mypid, region->Tag.Local_ID);
      region->attached = 0;
    }
    else
      region->attached = 1;

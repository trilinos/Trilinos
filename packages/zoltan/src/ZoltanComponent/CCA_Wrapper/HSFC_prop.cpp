...
int ZoltanSpace::HSFC_LB::SetParameter(char *key, char *val)
{
  ...
  /* 
     Perhaps the key is common to all partitioners and is kept in the
     only base class ? If it isn't, the call will return a -1
  */
  int err = -1 ;
  if ( prop.find(name) == prop.end() ) err = BaseLB::SetCommonParameter(key, val)  ;

  if (err != 0 ) // the key was found in the call to common properties
  {
    string value(val) ;
    prop[ name ] = value ;
...
int ZoltanSpace::HSFC_LB::GetParameter(char *key, char **val)
{
...
int ZoltanSpace::HSFC_LB::SetParameter(char *key, double d)
{
...
int ZoltanSpace::HSFC_LB::GetParameter(char *key, double *d)
{
...
int ZoltanSpace::HSFC_LB::SetParameter(char *key, bool b)
{
...
int ZoltanSpace::HSFC_LB::GetParameter(char *key, bool *b)
{
...
int ZoltanSpace::HSFC_LB::SetParameter(char *key, int i)
{
...
int ZoltanSpace::HSFC_LB::GetParameter(char *key, int *i)
{
...
int ZoltanSpace::HSFC_LB::PrintKeywordsToScreen()
{

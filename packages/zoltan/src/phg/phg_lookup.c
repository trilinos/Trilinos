...
static phg_gid_list seen_gids;

void phg_initialize_gid_list(int gid_len, int nvtx)
{
  seen_gids.top = (phg_gid_node *)ZOLTAN_MALLOC(sizeof(phg_gid_node) * nvtx);
  seen_gids.gn = (phg_gid_node **)ZOLTAN_CALLOC(sizeof(phg_gid_node *) , nvtx);
  seen_gids.size = nvtx;
  seen_gids.next_slot = 0;
  seen_gids.lenGID = gid_len;
}
...
phg_gid_node *gl;
  ...
  j = Zoltan_Hash((ZOLTAN_ID_PTR)gid, gidlen, seen_gids.size);

...
)
{
  ...
  Zoltan_PHG_heap_init(zz, &h[0], hg->nVtx);
  Zoltan_PHG_heap_init(zz, &h[1], 0);       /* Dummy heap, not used. */
  for(i=0; i<hg->nVtx; i++)
    Zoltan_PHG_heap_input(h, i, gain[i]);
  Zoltan_PHG_heap_make(h);

  while (bfsnumber < hg->nVtx ) {

    /* Get next vertex from heap */
    vtx = Zoltan_PHG_heap_extract_max(h);
    ...
    if (p && (pnumber+1)<p && part_sum > cutoff) {
      ...
      if (priority_mode > 0) {
        ...
        for (i=0; i<hg->nVtx; i++){
          if (i != j) gain[i] = 0.0;
          if (rank[i] < 0) Zoltan_PHG_heap_change_value(h, i, gain[i]);
        }
        /* Reset counters. */
        if (vtx_count)
        ...
        for (j=hg->vindex[vtx]; j<hg->vindex[vtx+1]; j++) {
          ...
          for (i=hg->hindex[edge]; i<hg->hindex[edge+1]; i++) {
            ...
            if (rank[nbor] < 0) {
               ...
               Zoltan_PHG_heap_change_value(h, nbor, gain[nbor]);
            }
          }
        }
      }
...
error:
  ...
  Zoltan_PHG_heap_free (&h[0]);
  Zoltan_PHG_heap_free( &h[1]);
  return err;
}

    ...
    )   
{
  ...
  *LevelSndCnt = count;
  ...
  if (hg->nEdge && (
    ...
    || !(ids     = (int *) ZOLTAN_MALLOC(hg->nEdge*sizeof(int)))
          ))
      MEMORY_ERROR;
  ...
  *LevelCnt   = 0;
  ...
  if (c_hg->nEdge) {
      if (!(c_ewgt =(float*)ZOLTAN_MALLOC(c_hg->nEdge*c_hg->EdgeWeightDim*sizeof(float))))
          MEMORY_ERROR;
      if (hg->EdgeWeightDim)
  ...
  if (!(c_hindex = (int*)ZOLTAN_MALLOC((c_hg->nEdge+1)*sizeof(int))))
      MEMORY_ERROR;

  if (c_hg->nPins>0 && !(c_hvertex = (int*)ZOLTAN_MALLOC (c_hg->nPins*sizeof(int))))
      MEMORY_ERROR;
  ...
  for (i=0; i < hg->nEdge; ++i) { /* loop over edges */
      ...
      uqsorti(idx-sidx, &c_hvertex[sidx]);
  }
  c_hindex[c_hg->nEdge] = c_hg->nPins = idx;
  ...
  for (i=0; i < c_hg->nEdge; ++i) { /* compute size and hashvalue */
      hlsize[i] = c_hindex[i+1]-c_hindex[i];
      lhash[i] = hashValue(hg, hlsize[i], &c_hvertex[c_hindex[i]]);
      /* UVC: TODO CHECK
         following Zoltan_Hash call didn't work well (couple other n values also didn't work great)
         we need to use global vertex numbers in the hash! may be can implement modified version
         of the Zoltan_Hash for that  */
      /* lhash[i] = Zoltan_Hash((ZOLTAN_ID_PTR) &c_hvertex[c_hindex[i]], hlsize[i], 0xffffffffu); */
      ids[i] = i;
  }
  ...
  if (c_hg->nEdge) {
      MPI_Allreduce(lhash, hash, c_hg->nEdge, MPI_INT, MPI_BXOR, hgc->row_comm);
      MPI_Allreduce(hlsize, hsize, c_hg->nEdge, MPI_INT, MPI_SUM, hgc->row_comm);      
  }

  Zoltan_quicksort_pointer_inc_int_int (ids, (int *)hash, hsize, 0, c_hg->nEdge-1);

  /* UVC: TODO: REMOVE THE FOLLOWING debug output*/
#ifdef _DEBUG2  
  idx = me = 0;
  for (size=j=0; j<hg->nEdge; ++j) {
      int n1=ids[j];
      for (i = j+1; i<c_hg->nEdge && hash[n1] == hash[ids[i]]; ++i) {
          int n2=ids[i];
          ++idx;
          if (hsize[n1]!=hsize[n2] || hlsize[n1]!=hlsize[n2]) 
              ++me;
      }
  }
#ifndef _DEBUG2
  ...
  for (j=0; j<c_hg->nEdge; ++j) {
      ...
      if (!iden[n1]) {
          ...
          for (i = j+1; i<c_hg->nEdge && hash[n1] == hash[ids[i]]
                   && hsize[n1]==hsize[ids[i]]; ++i) {
              int n2=ids[i];
              ...
              if (!iden[n2] && hlsize[n1]==hlsize[n2]
                  && !memcmp(&c_hvertex[c_hindex[n1]], &c_hvertex[c_hindex[n2]],
...
#ifdef _DEBUG2
  uprintf(hgc, "#Loc.Iden= %7d   (Computed= %d)   #Comp.PerNet= %.2lf     ElapT= %.3lf\n", count+me, count, (double) idx / (double) c_hg->nEdge, MPI_Wtime()-starttime);
  count += me;
#endif
  ...
  if (c_hg->nEdge) {
      ...
      if (!(idenOperandBuf = (int *) ZOLTAN_MALLOC(c_hg->nEdge * sizeof(int))))
          MEMORY_ERROR;
      MPI_Allreduce(iden, keep, c_hg->nEdge, MPI_INT, idenOp, hgc->row_comm);
      MPI_Op_free(&idenOp);
...
#ifdef _DEBUG1
      if (keep[i]==-1 && hlsize[i])
          errexit("keep[%d]==-1 but hlsize[%d] = %d", i, i, hlsize[i]);
#endif
      if (keep[i]>0) { /* identical net */
          int n1=keep[i]-1; /* i is identical to n1 */
          ...
          c_hg->nPins += hlsize[i];
          ++c_hg->nEdge;
          keep[i] = 1; /* Don't ignore */
      } else {
          /* ignoring net i with size hsize[i] */
          keep[i] = 0; /* ignore size 0/1 nets*/
#ifdef _DEBUG1
          ++me;
#endif
      }
...
#ifdef _DEBUG1
#ifdef _DEBUG2 
  uprintf(hgc, "#GlobIden= %7d    SuccessRate= %.1lf%%  %d size-0/1 nets  ElapT= %.3lf\n", idx, 100.0 * idx / (double) count, me, MPI_Wtime()-starttime);
#endif
...
#ifdef _DEBUG2
  uprintf(hgc, "Even when identical nets are removed there are %d same hash-val pairs but %d doesn't have same sizes\n", idx, me);
#endif
  MPI_Allreduce(&idx, &totiden, 1, MPI_INT, MPI_SUM, hgc->col_comm);
  MPI_Allreduce(&me, &totsize1, 1, MPI_INT, MPI_SUM, hgc->col_comm);
  if (!hgc->myProc)
      uprintf(hgc, "Level %d - After Local Iden rem Same Hash pairs=%d but %d has diff size (local/global)\n", hg->info, totiden, totsize1);
  if (size!=c_hg->nEdge)
      errexit("hey size(%d)!=c_hg->nEdge(%d)\n", size, c_hg->nEdge);
#endif

  
#ifdef FIND_GLOBAL_IDENTICAL_NETS
  /* UVC TODO the code to find globally identical nets will go here */
#endif
  ...
  for (idx=ni=i=0; i<hg->nEdge; ++i)
      if (keep[i]) {
          c_hg->hindex[ni] = idx;
          memcpy(&c_hg->ewgt[ni], &c_ewgt[i], c_hg->EdgeWeightDim*sizeof(float));
          memcpy(&c_hg->hvertex[idx], &c_hvertex[c_hindex[i]], hlsize[i]*sizeof(int));
          ++ni;
          idx += hlsize[i];
      }
  ...
  Zoltan_Multifree(__FILE__, __LINE__, 4, &c_hindex, &c_hvertex, &c_ewgt, &ids);
 ...
 End:
  Zoltan_Multifree (__FILE__, __LINE__, 18,
                    ...
                    &hlsize, &hsize, &lhash, &hash, &c_ewgt, &ids,

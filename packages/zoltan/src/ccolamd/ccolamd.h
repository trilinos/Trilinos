 ...
 * Zoltan_ to the original function names. UFconfig.h is no longer used.
 * -------------------------------------------------------------------------- */
...
/* ----------- Zoltan : Use UF_long definition from UFconfig.h -------------- */
/* ========================================================================== */
/* === UF_long ============================================================== */
/* ========================================================================== */
...
size_t Zoltan_ccolamd_recommended	/* returns recommended value of Alen, */
...
size_t Zoltan_ccolamd_l_recommended	/* returns recommended value of Alen, */
				/* or 0 if input arguments are erroneous */
(
...
void Zoltan_ccolamd_set_defaults	/* sets default parameters */
(				/* knobs argument is modified on output */
    double knobs [CCOLAMD_KNOBS]	/* parameter settings for ccolamd */
) ;

void Zoltan_ccolamd_l_set_defaults	/* sets default parameters */
(				/* knobs argument is modified on output */
    double knobs [CCOLAMD_KNOBS]	/* parameter settings for ccolamd */
) ;

int Zoltan_ccolamd			/* returns (1) if successful, (0) otherwise*/
...
UF_long Zoltan_ccolamd_l		/* same as ccolamd, but with UF_long integers */
(
    UF_long n_row,
    UF_long n_col,
    UF_long Alen,
...
int Zoltan_csymamd			/* return (1) if OK, (0) otherwise */
(
...
UF_long Zoltan_csymamd_l		/* same as csymamd, but with UF_long integers */
(
    UF_long n,
    UF_long A [ ],
    UF_long p [ ],
...
void Zoltan_ccolamd_report
(
    int stats [CCOLAMD_STATS]
) ;

void Zoltan_ccolamd_l_report
(
    UF_long stats [CCOLAMD_STATS]
) ;

void Zoltan_csymamd_report
(
    int stats [CCOLAMD_STATS]
) ;

void Zoltan_csymamd_l_report
(
    UF_long stats [CCOLAMD_STATS]
) ;
...
UF_long Zoltan_ccolamd2_l	    /* same as ccolamd2, but with UF_long integers */
(
    UF_long n_row,
    UF_long n_col,
    UF_long Alen,
...
void Zoltan_ccolamd_apply_order
(
    int Front [ ],
    const int Order [ ],
    int Temp [ ],
...
void Zoltan_ccolamd_l_apply_order
(
    UF_long Front [ ],
    const UF_long Order [ ],
    UF_long Temp [ ],
...
void Zoltan_ccolamd_l_postorder
(
    UF_long nn,
    UF_long Parent [ ],
    UF_long Npiv [ ],
...
UF_long Zoltan_ccolamd_l_post_tree
(
    UF_long root,
    UF_long k,
    UF_long Child [ ],
...
EXTERN int (*Zoltan_ccolamd_printf) (const char *, ...) ;

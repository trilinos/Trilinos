...
#define MAX_ITERATIONS 100    /* Max. no. of iterations in main bisection loop */
#define TOL_FACTOR 0.1        /* Balance tolerance for each cut = TOL_FACTOR * lb->Imbal_Tol */
...
struct bisector {          /* bisector cut info */
  ...
  double  totalhi[MAX_BISECT_WGTS]; /* weight in lower half of active partition */
  double  wtlo[MAX_BISECT_WGTS];    /* total weight of dot(s) on boundary */
  double  wthi[MAX_BISECT_WGTS];    /* total weight of dot(s) on boundary */
};
...
/* Local declarations. */
  ...
  double  *weightlo, *weighthi;      /* weight in lower/upper half of non-active */
  ...
  int     breakflag;                 /* for breaking out of bisector iteration */
...
/***************************** BEGIN EXECUTION ******************************/
...
#ifdef DEBUG
  printf("[%3d] Debug: Entering Zoltan_find_bisection, fraclo[0] = %lf \n", proc, fraclo[0]);
#endif
  ...
  for (j=0; j<nwgts; j++){
    ...
    invfraclo[j] = 1.0/temp;
    invfrachi[j] = 1.0/(1.0 - temp);
  }
  ...
  structsize = sizeof(struct bisector) + 4*nwgts*sizeof(double);
  med   = (struct bisector *) ZOLTAN_MALLOC(structsize);
  ...
  if (num_procs > 1) { /* don't need to go through if only one proc.  This
    ...
    while (iteration++ < MAX_ITERATIONS){
...
#ifdef DEBUG
      printf("[%3d] Debug: Iteration %d,  tmp_half = %lf\n", proc, iteration, tmp_half);
#endif
...
#ifdef DEBUG
      printf("[%3d] Debug: medme->totallo = (%lf, %lf), medme->totalhi = (%lf, %lf)\n", 
              proc, medme->totallo[0], medme->totallo[1], medme->totalhi[0], medme->totalhi[1]);
      printf("[%3d] Debug: med->totallo = (%lf, %lf), med->totalhi = (%lf, %lf)\n", 
              proc, med->totallo[0], med->totallo[1], med->totalhi[0], med->totalhi[1]);
      printf("[%3d] Debug: weightlo = (%lf, %lf), weighthi = (%lf, %lf)\n", 
              proc, weightlo[0], weightlo[1], weighthi[0], weighthi[1]);
      printf("[%3d] Debug: normlo = %lf, normhi = %lf, eps = %lf\n", 
      ...
      if (normlo < normhi - eps) {                      /* lower half TOO SMALL */
        ...
        /* tmplo = weightlo + med->wthi */
        Zoltan_daxpy(nwgts, 1., weightlo, med->wthi, tmplo);
        /* tmphi = tmphi - med->wthi */
        Zoltan_daxpy(nwgts, -1., tmphi, med->wthi, tmphi);
        if (med->counthi == 1) {                 /* only one dot to move */
          ...
          /* tmphi += med->wthi */
          Zoltan_daxpy(nwgts, 1.0, med->wthi, tmphi, tmphi);
          for (j = 0; j < numlist; j++) {  /* move dots until balanced or move all  */
            i = dotlist[j];
...
#ifdef DEBUG
          printf("[%3d] Debug: bisect value too small, breakflag = %d\n", proc, breakflag);
#endif
        ...
        /* tmphi = weighthi + med->wtlo */
        Zoltan_daxpy(nwgts, 1., weighthi, med->wtlo, tmphi);
        /* tmplo = tmplo - med->wtlo */
        Zoltan_daxpy(nwgts, -1., tmplo, med->wtlo, tmplo);
...
#ifdef DEBUG
          printf("[%3d] Debug: bisect value too big, breakflag = %d\n", proc, breakflag);
#endif
          if (breakflag) break;                   /* done if moved enough */
        }
...
/* Compute the appropriate norm of a vector.
   ...
   Assume for efficiency that x and scal are non-negative.
*/
static double Zoltan_norm(int norm, int n, double *x, double *scal)
{
  int i;

...
{ int  i;
  ...
  if (p == 1) {            /* everything goes in the one partition */
   ...
   || (hgp->matching == NULL && hgp->packing == NULL && hgp->grouping == NULL)){
     ...
     if      (hgp->packing) ierr =Zoltan_HG_Packing (zz, hg, pack, hgp, &limit);
     else if (hgp->grouping)ierr =Zoltan_HG_Grouping(zz, hg, pack, hgp, &limit);
     else if (hgp->matching)ierr =Zoltan_HG_Matching(zz, hg, pack, hgp, &limit);

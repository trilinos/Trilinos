...
int Zoltan_HG_Coarsening (ZZ *zz,  /* the Zoltan data structure */
  ...
  int    *LevelMap)
{
int i, j, old, vertex, new_vertex, *used_vertices=NULL, *c_hindex=NULL, *c_hvertex=NULL;
float *c_ewgt=NULL;
char *yo = "Zoltan_HG_Coarsening";
  ...
  if (!(used_vertices = (int*)  ZOLTAN_CALLOC (c_hg->nVtx,    sizeof(int)))
   ...
   || !(c_hvertex     = (int*)  ZOLTAN_MALLOC (hg->nPins   * sizeof(int))) ) {
  ...
  c_hindex[0] = c_hindex[1] = c_hg->nEdge = c_hg->nPins = 0;
  for (i = 0; i < hg->nEdge; i++) {
     for (j = hg->hindex[i]; j < hg->hindex[i+1]; j++) {
        new_vertex = LevelMap[hg->hvertex[j]];
        if (used_vertices[new_vertex] <= i) {
           used_vertices[new_vertex] = i + 1;
           c_hvertex[(c_hg->nPins)++] = new_vertex;
           }
        }
     if (c_hg->nPins > c_hindex[c_hg->nEdge] + 1) {  /* a new hyperedge */
        c_ewgt[c_hg->nEdge++] = hg->ewgt ? hg->ewgt[i] : 1.0;
        c_hindex[c_hg->nEdge] = c_hg->nPins;
        }
...
#define EPS (1e-8)
  ...
  c_hg->hvertex =(int*)  ZOLTAN_REALLOC(c_hvertex, c_hg->nPins   * sizeof(int));
#else
  c_hg->ewgt    = c_ewgt;

...
We follow this with three more realistic examples, still fairly
simple.  These examples use the Zoltan library to apply a geometric method,
a graph method and a then hypergraph method to a small mesh, graph
or hypergraph.

If you have the Zoltan source code, you can find these
examples in the \textbf{examples/C} directory.  The examples display
the partitioning after computing it.  (We omitted that part of the
code in this tutorial.)  You can try changing parameters and then
recompiling and rerunning the examples to see how the partitioning
is affected.

These four examples are all written in C.  At the end of this section,
...
It uses the Zoltan method called SIMPLE.  This method partitions the
weighted objects across processors, balancing the total weight in
the final distribution.

The SIMPLE method really exists as an aid to developers
...
We must define two query functions.  The function prototyes named 
below are defined in the Zoltan file \textbf{include/zoltan.h}.

\begin{itemize}
\item A function (get\_number\_of\_objects) of type ZOLTAN\_NUM\_OBJ\_FN which provides the number of objects belonging to this process .
\item A function (get\_object\_list) of type ZOLTAN\_OBJ\_LIST\_FN which supplies the global IDs and optional weights for the objects belonging to the process.
\end{itemize}
...
the rectangular mesh shown in Figure~\ref{fig:simpleGraph}.
...
Your query functions could also be global C or C++ functions.

The example below can also be found in the Zoltan source in 

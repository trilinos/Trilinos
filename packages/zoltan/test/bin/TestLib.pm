...
sub run_experiments {
  ...
  while (($i <= $ntrials) && ($executionstatus eq "Pass")) {
    ...
    print OUTPUT "$cmdline (\$seed = $seed)\n";
    print OUTPUT "</CommandLine>\n";
    print OUTPUT "<Output>\n";
    close(OUTPUT);
    #
    ...
    #
    $lcmd = "\$status = 0xffff & system(\"( testlib_exec $cmdline ) >> $outname 2>&1\")";
    $t0 = mytime();
    eval($lcmd);
    $elapsed = time_diff($t0, mytime());
    #
    ...
    if ($status != 0) {
       ...
       if (! (-e "$outname")) {
          ...
          #
          $executionstatus  = "Fail";
          $explanation = "Missing output file $outname";
       } elsif ((get_tokens(`wc $outname`))[1] eq "0") {
          #
          ...
          close (OUTPUT);
          &$analysisSub("$outname",$executionstatus);
          if ($executionstatus eq "Fail") {
             $explanation = "Bad output format in file $outname";
          }
          open (OUTPUT, ">>$outname");
          print OUTPUT "</Output>\n";
          print OUTPUT "</TrialOutput>\n";
...
# make sure the coverage directory exists and is cleaned up
  ...
  #
  # Execute experiments in *.exp.xml files
  #
  $result = `(find packages -name '*.exp.xml') 2>&1`;
  chomp($result);
  foreach $testfile (get_tokens($result)) {
    ...
    system("(python ../../../bin/runexp --scenario=$software $testtags $tmp[-1]) 2>&1");
    #system("(cp $testname*.results.xml $outdir) 2>&1");
    #system("(cp $testname*.test.xml $outdir) 2>&1");
    chdir $pwd;
  }
  system("(find packages -name '*.results.xml' -exec /bin/cp {} $outdir \\; 2>&1) > /dev/null");
  system("(find packages -name '*.test.xml' -exec /bin/cp {} $outdir \\; 2>&1) > /dev/null");
  #

...
int main(int argc,char * argv[])
{
  ...
  { // Put main inside braces to delete all views prior to finalize of device
    ...
    int x_elements=10,y_elements=10,z_elements=2,basis_order=1;
    double cfl=4.0;
    ...
    // Add fields to the mesh data base (this is a peculiarity of how STK classic requires the
        // fields to be setup)
    createExodusFile(physicsBlocks, Teuchos::rcpFromRef(mesh_factory), mesh);
...
std::vector<panzer::BC> homogeneousBoundaries()
{
  ...
  std::string sidesets[4] = {"left","right","top","bottom"};
...
std::vector<panzer::BC> auxiliaryBoundaries()
{
  ...
  std::size_t bc_id = 0;
  for (int s = 0; s < 4; s++) // TODO: fix Tpetra auxiliary BCs so Dirichlet conditions can be used
    for (int d = 0; d < 0; d++)
    {
      panzer::BCType bctype = panzer::BCT_Dirichlet;
      std::string sideset_id = sidesets[s];
  ...
  pl->set("Linear Solver Type","Belos");
  {
    Teuchos::ParameterList& ls_types = pl->sublist("Linear Solver Types");
    {
      ...
      belos.set("Solver Type","Block GMRES");
      {
        ...
        {
          Teuchos::ParameterList& gmres = solver_types.sublist("Block GMRES");

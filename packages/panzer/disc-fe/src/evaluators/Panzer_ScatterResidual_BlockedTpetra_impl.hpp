...
ScatterResidual_BlockedTpetra(const Teuchos::RCP<const BlockedDOFManager> & indexer,
                              const Teuchos::ParameterList& p)
   : ScatterResidual_BlockedTpetra(indexer,std::vector<Teuchos::RCP<const panzer::GlobalIndexer> >(),p)
{ }
		      ...
		      PHX::FieldManager<TRAITS>& /* fm */)
{
  ...
  // separately from the rows they need their own offsets and LIDs. When they
  // are not, both alias the row-side objects and everything below is unchanged.
  auto hostColBlockOffsets = hostBlockOffsets;
  if (!hasColIndexers_) {
    ...
    colBlockOffsets_ = PHX::View<LO*>("ScatterResidual_BlockedTpetra(Jacobian):colBlockOffsets_",
                                      numColBlocks+1);
    ...
    colWorksetLIDs_ = Kokkos::View<LO**, Kokkos::LayoutRight, PHX::Device>(
                        "ScatterResidual_BlockedTpetra(Jacobian):colWorksetLIDs_",
  ...
  for (int blk=0;blk<static_cast<int>(colGlobalIndexers_.size());blk++) {
    const int blockDerivativeSize = hostColBlockOffsets(blk+1) - hostColBlockOffsets(blk);
      ...
      for(int basis=0; basis < static_cast<int>(fieldOffsets.size()); ++basis) {
        ...
        for (int blockColIndex=0; blockColIndex < numColFieldBlocks; ++blockColIndex) {
          if (blockExistsInJac(blockRowIndex,blockColIndex)) {
            ...
            jacTpetraBlocks(blockRowIndex,blockColIndex).sumIntoValues(rowLID, &colWorksetLIDs(cell,start), sensSize, &workset_vals(cell,0), true,true);
          }
        }
      }
    });
    ...
    Kokkos::parallel_for(Kokkos::RangePolicy<PHX::Device>(0,workset.num_cells), KOKKOS_LAMBDA (const int& cell) {
      for(int basis=0; basis < static_cast<int>(fieldOffsets.size()); ++basis) {
        ...
        for(std::size_t i_param=0; i_param<num_params; i_param++)
          Kokkos::atomic_add(&kokkosTangents(i_param)(lid,0), fieldValues(cell,basis).fastAccessDx(i_param));
      }
    });
  }

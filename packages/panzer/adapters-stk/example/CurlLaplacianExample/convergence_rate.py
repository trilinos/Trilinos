  ...
  for cnt, res in enumerate(mesh_res):
    ...
    if cnt>0:
      ...
      if rate > 0.95*basis_order:
        print('%s: Convergence order of %f is within 5 percent of %f: PASSED' % (filename, rate, basis_order)) 
      else:
        print('%s: Convergence order of %f is not within 5 percent of %f: FAILED' % (filename, rate, basis_order)) 

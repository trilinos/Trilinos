...
// ************************************************************
TEUCHOS_UNIT_TEST(GeneralizedAlpha, BallParabolic)
{
...
TEUCHOS_UNIT_TEST(GeneralizedAlpha, ConstructingFromDefaults)
{
  ...
  RCP<Tempus::StepperGeneralizedAlpha<double> > stepper =
...
#ifdef TEST_SIN_COS
// ************************************************************
TEUCHOS_UNIT_TEST(GeneralizedAlpha, SinCos_SecondOrder)
{
  ...
  for (int n=0; n<nTimeStepSizes; n++) {
    ...
    if (n == nTimeStepSizes-1) {
      ...
      RCP<const SolutionHistory<double> > solutionHistory =
        integrator->getSolutionHistory();
      RCP<const Thyra::VectorBase<double> > x_exact_plot;
	  RCP<const Thyra::VectorBase<double> > x_exact_dot_plot;
	  RCP<const Thyra::VectorBase<double> > x_exact_dot_dot_plot;
      for (int i=0; i<solutionHistory->getNumStates(); i++) {
        RCP<const SolutionState<double> > solutionState = (*solutionHistory)[i];
        double time = solutionState->getTime();
        RCP<const Thyra::VectorBase<double> > x_plot = solutionState->getX();
        RCP<const Thyra::VectorBase<double> > x_dot_plot = solutionState->getXDot();
		RCP<const Thyra::VectorBase<double> > x_dot_dot_plot = solutionState->getXDotDot();
        x_exact_plot = model->getExactSolution(time).get_x();
		x_exact_dot_plot = model->getExactSolution(time).get_x_dot();
		x_exact_dot_dot_plot = model->getExactSolution(time).get_x_dot_dot();
        ...
        ftmp << time << "   "
			 ...
			 << get_ele(*(x_exact_dot_plot), 0) << "   "
			 << get_ele(*(x_dot_dot_plot), 0) << "   "
			 << get_ele(*(x_exact_dot_dot_plot), 0) << "   "
             << te << std::endl;
      }
      ftmp.close();
    }
  ...
  std::cout << "  Stepper = GeneralizedAlpha" << std::endl;
  std::cout << "  =========================" << std::endl;

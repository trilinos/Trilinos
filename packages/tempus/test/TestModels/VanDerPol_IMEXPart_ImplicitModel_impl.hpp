...
VanDerPol_IMEXPart_ImplicitModel<Scalar>::
VanDerPol_IMEXPart_ImplicitModel(Teuchos::RCP<Teuchos::ParameterList> pList_)
{
  ...
  ) const
{
  ...
  if (inArgs.get_x_dot().is_null()) {
    ...
    if (!is_null(f_out)) {
      ...
      f_out_view[0] = (1.0-x0*x0)*x1/epsilon_;
    }
  } else {
    ...
    if (!is_null(f_out)) {
      ...
      f_out_view[0] = x_dot_in_view[0] - (1.0-x0*x0)*x1/epsilon_;
    }
    ...
    if (!is_null(W_out)) {
      ...
      W_view(0,0) = alpha - beta*(1.0 - x0*x0)/epsilon_;    // d(f0)/d(x0_n)

...
#ifndef TEMPUS_TEST_SINCOSXDOTDOT_MODEL_IMPL_HPP
#define TEMPUS_TEST_SINCOSXDOTDOT_MODEL_IMPL_HPP
...
SinCosXdotdotModel<Scalar>::
SinCosXdotdotModel(Teuchos::RCP<Teuchos::ParameterList> pList_)
{
  ...
  Thyra::put_scalar(b_*std::sin(phi_), x_vec_.ptr());
  x_dot_vec_ = createMember(x_space_);
  Thyra::put_scalar(b_*std::cos(phi_), x_dot_vec_.ptr());
  x_dot_dot_vec_ = createMember(x_space_);
  Thyra::put_scalar(-b_*std::sin(phi_), x_dot_dot_vec_.ptr());
  ...
  *out_ << "\n Problem parameters: b, phi = " << b_ << ", " << phi_ << "\n";
}
...
getExactSolution(double t) const
{
  ...
  { // scope to delete DetachedVectorView
    ...
    exact_x_view[0] = b_*std::sin(t+phi_);
  }
  ...
  { // scope to delete DetachedVectorView
    ...
    exact_x_dot_view[0] = b_*std::cos(t+phi_); 
  }
  ...
  { // scope to delete DetachedVectorView
    ...
    exact_x_dot_dot_view[0] = -b_*std::sin(t+phi_); 
  }
  ...
  ) const
{
  ...
  if (!x_in.get()) {
    TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error,
      "\n ERROR: SinCosXdotdotModel requires x as InArgs.\n");
  }
  ...
  if (W_out != Teuchos::null) {
    ...
    if (omega == 0.0) {
      TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error,
        "\n ERROR: omega = 0 in SinCosXdotdotModel!\n");
    }
...
{
  TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error,
                     "\n Error!  SinCosXdotdotModel::get_p_space() is not supported!\n"); 
  return Teuchos::null;
}
...
{
  TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error,
                     "\n Error!  SinCosXdotdotModel::get_p_names() is not supported!\n"); 
  return Teuchos::null;
}
...
get_g_space(int j) const
{
  TEUCHOS_TEST_FOR_EXCEPTION(j != 0, std::logic_error,
                     "\n Error!  SinCosXdotdotModel::get_g_space() only " <<
...
setParameterList(Teuchos::RCP<Teuchos::ParameterList> const& paramList)
{
  ...
  RCP<ParameterList> tmpPL = Teuchos::rcp(new ParameterList("SinCosXdotdotModel"));
...
getValidParameters() const
{
  ...
  if (is_null(validPL)) {
    ...
    Teuchos::setDoubleParameter(
        "Coeff phi", 0.0, "Coefficient phi in model", &*pl);
  }
  return validPL;
}

} // namespace Tempus_Test
#endif // TEMPUS_TEST_SINCOSXDOTDOT_MODEL_IMPL_HPP

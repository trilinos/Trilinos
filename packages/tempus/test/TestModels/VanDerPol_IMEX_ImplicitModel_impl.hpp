...
VanDerPol_IMEX_ImplicitModel<Scalar>::VanDerPol_IMEX_ImplicitModel(
...
VanDerPol_IMEX_ImplicitModel<Scalar>::get_x_space() const
{
  return x_space_;
}
...
template <class Scalar>
Teuchos::RCP<const Thyra::VectorSpaceBase<Scalar> >
VanDerPol_IMEX_ImplicitModel<Scalar>::get_f_space() const
{
  return f_space_;
}
...
VanDerPol_IMEX_ImplicitModel<Scalar>::getNominalValues() const
{
...
VanDerPol_IMEX_ImplicitModel<Scalar>::create_W() const
{
  using Teuchos::RCP;
...
VanDerPol_IMEX_ImplicitModel<Scalar>::create_W_op() const
{
...
VanDerPol_IMEX_ImplicitModel<Scalar>::get_W_factory() const
{
...
template <class Scalar>
Thyra::ModelEvaluatorBase::InArgs<Scalar>
VanDerPol_IMEX_ImplicitModel<Scalar>::createInArgs() const
{
  setupInOutArgs_();
  return inArgs_;
}
...
VanDerPol_IMEX_ImplicitModel<Scalar>::createOutArgsImpl() const
{
  setupInOutArgs_();
  return outArgs_;
}
...
template <class Scalar>
void VanDerPol_IMEX_ImplicitModel<Scalar>::evalModelImpl(
    ...
    const Thyra::ModelEvaluatorBase::OutArgs<Scalar> &outArgs) const
{
  ...
  if (inArgs.get_x_dot().is_null()) {
    ...
    if (!is_null(DfDp_out)) {
      ...
      DfDp_out_view(1, 0) =
          -(1.0 - x_in_view[0] * x_in_view[0]) * x_in_view[1] / (eps * eps);
    ...
    if (!is_null(W_out)) {
      ...
      W_view(1, 0) =
          2.0 * beta * x_in_view[0] * x_in_view[1] / eps;  // d(f1)/d(x0_n)
      W_view(1, 1) = alpha - beta * (1.0 - x_in_view[0] * x_in_view[0]) /
                                 eps;  // d(f1)/d(x1_n)
      // Note: alpha = d(xdot)/d(x_n) and beta = d(x)/d(x_n)
    }
  }
...
template <class Scalar>
Teuchos::RCP<const Thyra::VectorSpaceBase<Scalar> >
VanDerPol_IMEX_ImplicitModel<Scalar>::get_p_space(int l) const
{
  if (!acceptModelParams_) {
    return Teuchos::null;
  }
...
VanDerPol_IMEX_ImplicitModel<Scalar>::get_p_names(int l) const
{
  if (!acceptModelParams_) {
    return Teuchos::null;
  }
...
template <class Scalar>
Teuchos::RCP<const Thyra::VectorSpaceBase<Scalar> >
VanDerPol_IMEX_ImplicitModel<Scalar>::get_g_space(int j) const
{
...
template <class Scalar>
void VanDerPol_IMEX_ImplicitModel<Scalar>::setupInOutArgs_() const
{
  if (isInitialized_) {
    return;
  }
  ...
  if (haveIC_) {
    ...
    const Teuchos::RCP<Thyra::VectorBase<Scalar> > x_ic =
...
template <class Scalar>
void VanDerPol_IMEX_ImplicitModel<Scalar>::setParameterList(
...
VanDerPol_IMEX_ImplicitModel<Scalar>::getValidParameters() const
{

...
// ************************************************************
TEUCHOS_UNIT_TEST(NewmarkImplicit, BallParabolicDamped)
{
  ...
  // Read params from .xml file
  RCP<ParameterList> pList =
    getParametersFromXmlFile("Tempus_Newmark_BallParabolicDamped.xml");
  ...
  for (int n=0; n<nTimeStepSizes; n++) {
    ...
    if (n == nTimeStepSizes-1) {
      std::ofstream ftmp("Tempus_Newmark_BallParabolicDamped.dat");
      RCP<SolutionHistory<double> > solutionHistory =
        integrator->getSolutionHistory();
      RCP<const Thyra::VectorBase<double> > x_exact_plot;
      for (int i=0; i<solutionHistory->getNumStates(); i++) {
  ...
  std::ofstream ftmp("Tempus_Newmark-Error_BallParabolicDamped.dat");

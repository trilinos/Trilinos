              ...
              bool& success)
{
  ...
  for (int n = 0; n < nTimeStepSizes; n++) {
    ...
    RCP<ParameterList> pList =
        getParametersFromXmlFile("Tempus_EPI3_CDR.xml");
    ...
    std::cout << "W factory is set" << std::endl;
    // Set the step size
    dt /= 2;
    ...
    std::cout << "integrator sublist is read" << std::endl;
    integrator = Tempus::createIntegratorBasic<double>(pl, model);
    std::cout << "integrator is created" << std::endl;
    // Integrate to timeMax
    bool integratorStatus = integrator->advanceTime();
    TEST_ASSERT(integratorStatus)
    std::cout << "integrator status is asserted" << std::endl;
    ...
    std::cout << "something equatility" << std::endl;
    ...
    std::cout << "solution is created" << std::endl;

    // Output finest temporal solution for plotting
    // This only works for ONE MPI process
    if ((n == nTimeStepSizes - 1) && (commSize == 1)) {
      std::ofstream ftmp("Tempus_EPI3_CDR.dat");
      ftmp << "TITLE=\"EPI3 Solution to CDR\"\n"
           << "VARIABLES=\"z\",\"T\"\n";
      const double dx =
  ...
  //writeOrderError("Tempus_EPI3_CDR-Error.dat", stepper, StepSize,
  ...
  if (commSize == 1) {
    ...
    std::ofstream ftmp("Tempus_EPI3_CDR-Solution.dat");
...
// ************************************************************
// ************************************************************
TEUCHOS_UNIT_TEST(EPI3, CDR)
{
  std::cout << "Running EPETRA beginning" << std::endl;
...
// ************************************************************
// ************************************************************
TEUCHOS_UNIT_TEST(EPI3, CDR_Tpetra)
{
  std::cout << "Running TPETRA beginning" << std::endl;

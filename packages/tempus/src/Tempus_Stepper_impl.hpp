    ...
    const
{
  ...
  pl->template set<bool>(
      ...
      "incorrect solutions.\n"
      "\n"
      "The exception to this rule are the ForwardEuler and Exponential Steppers:\n"
      "For them, useFSAL=true means that the function f(x^{n},t^{n}) is\n"
      "evaluted at the end of of the step from t^{n-1} to t^n, and saved in the\n"
      "SolutionHistory in xDot, to be used at the beginning of the next step."
      "useFSAL only controls the order and cost of operations, but not correctness.\n"
      "\n"

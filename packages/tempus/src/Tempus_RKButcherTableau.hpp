...
class RKButcherTableau
 ...
 public:
  RKButcherTableau(std::string stepperType,
    ...
    : description_(stepperType)
  {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(
        std::abs(ST::one() - sumb) > 1.0e-08, std::runtime_error,
  ...
  bool operator==(const RKButcherTableau& t) const
  {
    ...
    if (A_->numRows() != t.A_->numRows() || A_->numCols() != t.A_->numCols()) {
      return false;
    }
    else {
      int i, j;
      for (i = 0; i < A_->numRows(); i++) {
        for (j = 0; j < A_->numCols(); j++) {
          if (std::abs((t.A_(i, j) - A_(i, j)) / A_(i, j)) > relTol)
            return false;
        }
      }
    }

    if (b_->length() != t.b_->length() || b_->length() != t.c_->length() ||
      ...
      for (i = 0; i < A_->numRows(); i++) {
        ...
        if (std::abs((t.bstar_(i) - bstar_(i)) / bstar_(i)) > relTol)
          return false;
      }
    }
    return true;
  ...
  bool operator!=(const RKButcherTableau& t) const { return !((*this) == t); }

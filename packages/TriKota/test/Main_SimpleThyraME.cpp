...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    *out << "\nStarting TriKota Example!" << endl;
    ...
    // Wrap EpetraExt::ModelEvaluator in a Thyra::ModelEvaluator 
    // AGS: Try responseOnly version in Thyra!
    Teuchos::RCP<Thyra::EpetraModelEvaluator>
        thyraApp(new Thyra::EpetraModelEvaluator()); // Sets default options!

    thyraApp->initialize(App, Teuchos::null);
    ...
    // Get the final solution and check it! Only on rank==0
    if (dakota.rankZero()) {

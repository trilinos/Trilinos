/*
 * MueLu_RAPFactoryEx.hpp
 *
 *  Created on: Oct 25, 2011
 *      Author: wiesner
 */

#ifndef MUELU_RAPFACTORYEX_HPP_
#define MUELU_RAPFACTORYEX_HPP_
...
#include "MueLu_UseShortNames.hpp"
  ...
  /*RAPFactory(RCP<FactoryBase> PFact = Teuchos::null, RCP<FactoryBase> RFact = Teuchos::null)
    ...
    RAPFactoryEx(RCP<FactoryBase> PFact, RCP<FactoryBase> RFact, RCP<FactoryBase> PtentFact, RCP<FactoryBase> AFact = Teuchos::null)
    : PFact_(PFact), RFact_(RFact), PtentFact_(PtentFact), AFact_(AFact), implicitTranspose_(false) {}

    virtual ~RAPFactoryEx() {}
    //@}
    ...
    void DeclareInput(Level &fineLevel, Level &coarseLevel) const {
      ...
      coarseLevel.DeclareInput("P",PFact_.get());  // transfer operators
...
MemUtils::ReportTimeAndMemory(*apTimer, *(P->getRowMap()->getComm()));
      ...
      Monitor m(*this, "Computing Ac = RAP (experimental)");
...
private:
  ...
  //! Factory which generated tentative prolongator
  RCP<FactoryBase> PtentFact_;  // TODO move this to user defined transfer factories
...
#define MUELU_RAPFACTORYEX_SHORT

#endif /* MUELU_RAPFACTORYEX_HPP_ */

...
Level::Level(RCP<FactoryManagerBase>& factoryManager)
...
std::string Level::GetTypeName(const std::string& ename, const FactoryBase* factory) {
...
void Level::Delete(const std::string& ename, const FactoryBase* factory) {  // Note: do not add default value for input parameter 'factory'
...
bool Level::IsKey(const FactoryBase* factory, const std::string& ename) const {
  TwoKeyMap::const_iterator it = map_.find(factory);
  return (it != map_.end()) ? (it->second).count(ename) : false;
}

bool Level::IsAvailableFactory(const FactoryBase* factory) const {
  TwoKeyMap::const_iterator it = map_.find(factory);
  if (it == map_.end())
    return false;
...
bool Level::IsRequested(const Value& v) const {
...
bool Level::IsRequestedBy(const FactoryBase* factory, const std::string& ename, const FactoryBase* requestedBy) const {
  if (!IsKey(factory, ename))
    return false;
...
bool Level::IsRequestedBy(const Value& v, const FactoryBase* requestedBy) const {
  TEUCHOS_TEST_FOR_EXCEPTION(v->NumAllRequests() == 0 && v->GetKeepFlag() == 0, Exceptions::RuntimeError,
                             "Internal logic error: if counter == 0, the entry in countTable_ should have been deleted");
  return v->IsRequested(requestedBy);
}

bool Level::IsRequestedFactory(const FactoryBase* factory) const {
  TwoKeyMap::const_iterator it = map_.find(factory);
  if (it == map_.end())
    return false;
  for (SubMap::const_iterator sit = it->second.begin(); sit != it->second.end(); sit++)
...
const RCP<VariableContainer>& Level::Get(const FactoryBase* factory, const std::string& ename) const {
  TwoKeyMap::const_iterator it = map_.find(factory);
...
int Level::NumRequests(const FactoryBase* factory, const std::string& ename) const {
...
int Level::CountRequestedFactory(const FactoryBase* factory) const {
  TwoKeyMap::const_iterator it = map_.find(factory);
  if (it == map_.end())
    return 0;

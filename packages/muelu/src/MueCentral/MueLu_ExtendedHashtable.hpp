...
namespace MueLu
{
  ...
  namespace UTILS
  {
    class ExtendedHashtable : MueLu::BaseClass
    {
      ...
      template<typename Value> Value& Get(const string& ename, RCP<const FactoryBase> factory)
      {
        if(!dataTable_.count(ename) > 0)
        {
        ...
        std::map<const MueLu::FactoryBase*,Teuchos::any> mapData = dataTable_[ename];
        Value retData = Teuchos::any_cast<Value>( mapData[fact]);
        Value& refData = retData;
        return refData;
      }

      template<typename Value> void Get(const string& ename, Value& value, RCP<const FactoryBase> factory = Teuchos::null)
      {
        if(!dataTable_.count(ename) > 0)
        {
          std::stringstream str; str << "key" << ename << " does not exist in Hashtable.";
        ...
        std::map<const MueLu::FactoryBase*,Teuchos::any> mapData = dataTable_[ename];
        value = Teuchos::any_cast<Value>( mapData[fact]);
      }

      void Remove(const string& ename, RCP<const FactoryBase> factory)
      {
        if(!dataTable_.count(ename) > 0)
        {
          std::stringstream str; str << "key" << ename << " does not exist in Hashtable.";
      ...
      inline bool isKey(const string& ename, RCP<const FactoryBase> factory)
      {
        ...
        if (dataTable_[ename].count(fact) > 0) return true;
      ...
      void describe(Teuchos::FancyOStream &out, const Teuchos::EVerbosityLevel verbLevel=Teuchos::Describable::verbLevel_default) const
      {
        ...
        {
        	out << "ExtendedHashtable: " << dataTable_.size() << " objects" << std::endl;
        }
      }

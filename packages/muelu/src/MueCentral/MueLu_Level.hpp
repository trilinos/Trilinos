  ...
  public:
    ...
    void Release(const std::string& ename, const FactoryBase* factory = NoFactory::get()) {
      ...
      if(IsAvailable(ename,fac) == false && IsRequested(ename,fac) == true) {
        ...
        //TEUCHOS_TEST_FOR_EXCEPTION(IsAvailable(ename,fac) == false && IsRequested(ename,fac) == false, Exceptions::RuntimeError,
        //    "MueLu::Level::Release(): Data not requested and not available. Call of Release does not make sense. You should check this!");
        TEUCHOS_TEST_FOR_EXCEPTION(IsAvailable(ename,fac) == true && IsRequested(ename,fac) == false, Exceptions::RuntimeError,
            "MueLu::Level::Release(): Data is available but not requested. We have to check this. Maybe data is kept??");
      }
    }

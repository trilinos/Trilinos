...
class Level : public BaseClass {
 public:
  ...
  void Set(const std::string& ename, const T& entry, const FactoryBase* factory = NoFactory::get()) {
    ...
    if (IsRequested(ename, factory) || GetKeepFlag(ename, factory) != 0) {
      ...
      GetOStream(Runtime1) << "Level::Set: Not storing \"" << ename << "\" generated by factory " << factory->ShortClassName() << "[" << factory->GetID() << "]"
                           << " on level " << Teuchos::toString(GetLevelID()) << ", as it has not been requested and no keep flags were set for it" << std::endl;
    }
  }  // Set
  ...
  T& Get(const std::string& ename, const FactoryBase* factory = NoFactory::get()) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(!IsKey(fac, ename), Exceptions::RuntimeError, "\"" + ename + "\" generated by factory \"" + fac->description() + "\" not found on level " + Teuchos::toString(GetLevelID()) + ".");

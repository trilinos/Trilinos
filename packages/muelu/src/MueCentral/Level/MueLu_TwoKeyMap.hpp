    ...
    public:
      ...
      void Set(const Key1 & key1, const Key2 & key2, const Value & entry) {
       ...
       const Value & Get(const Key1 & key1, const Key2 & key2) const {
        // Get SubMap
        TEST_FOR_EXCEPTION(map_.count(key1) != 1, Exceptions::RuntimeError, "MueLu::TwoKeyMap::GetEntry(): Key (" << key1 << ", *) does not exist.");
        const SubMap & subMap = map_.find(key1)->second;
        
        // Get Value
        TEST_FOR_EXCEPTION(subMap.count(key2) != 1, Exceptions::RuntimeError, "MueLu::TwoKeyMap::GetEntry(): Key (" << key1 << ", " << key2 << ") does not exist.");
      ...
      void Remove(const Key1 & key1, const Key2 & key2) {
        ...
        TEST_FOR_EXCEPTION(nElementErased != 1, Exceptions::RuntimeError, "MueLu::TwoKeyMap::Get(): Key (" << key1 << ", " << key2 << ") does not exist.");

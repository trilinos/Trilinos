...
namespace MueLu {
  ...
  */
  ...
  public:
    ...
    // JG: FIXME:
    //        - The TEUCHOS_TEST_FOR_EXCEPTION is now wrong (cf. following error messge). Behavior of Delete() have been incorrectly changed.
    //        - Do not remove data if counter > 0!
    //        - Add an option Keep(ename,factory, false)
    void Delete(const std::string & ename, const FactoryBase* factory) {
      TEUCHOS_TEST_FOR_EXCEPTION(!keepTable_.IsKey(ename, factory), Exceptions::RuntimeError, "MueLu::Needs::Delete(): " + ename + " not found in keepTable_");

      if (keepTable_.Get(ename, factory) == false) {
      ...
      if (dataTable_.IsKey(ename, factory)) {
        ...
        keepTable_.Remove(ename, factory);
      }
    }

...
void StructuredRAPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::GetStructuredGraph(
  ...
  if (graphSpec.numDimensions == 1) {
    ...
    if (procGrid[0] * procGrid[1] * procGrid[2] != numRanks) {
      ...
      // Keep this factorization limit synchronized with Galeri_XpetraMaps_def.hpp.
      const int galeriFactorSearchLimit    = 50;
      int factors[galeriFactorSearchLimit] = {};
      for (int factor = 2; factor < galeriFactorSearchLimit; ++factor) {
      ...
      procGrid[0] = procTemp;
      for (int factor = galeriFactorSearchLimit - 1; factor > 0; --factor) {

...
void RepartitionFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::Build(Level& currentLevel) const {
  ...
  using Util = RepartitionUtilities<Scalar, LocalOrdinal, GlobalOrdinal, Node>;
  ...
  Array<GO> myGIDs;
  {
    SubFactoryMonitor m1(*this, "Compute GIDs", currentLevel);
    GO numLocalKept;
    std::tie(myGIDs, numLocalKept) = Util::ConstructGIDs(decomposition);

    if (IsPrint(Statistics2)) {
      GO numGlobalKept, numGlobalRows = A->getGlobalNumRows();

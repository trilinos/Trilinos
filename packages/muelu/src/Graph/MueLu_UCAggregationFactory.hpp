...
typedef LO my_size_t; //TODO
  ...
  public:
    ...
    //! Default constructor.
    UCAggregationFactory(const AggregationOptions& options) :
    ...
    //! Constructor.
    UCAggregationFactory(const AggregationOptions& options, RCP<CoalesceDropFactory> const &cdFact) :
  ...
  {
    LocalAggregationFactory localAggregationFactory(options_, coalesceDropFact_); //TODO move to constructor
    RCP<Aggregates> aggregates = localAggregationFactory.Build(graph);

...
class AbsDropFunctor {
 ...
 public:
  AbsDropFunctor(local_matrix_type& A_, magnitudeType threshold, diag_view_type diag_, boundary_nodes_view boundaryNodes_, results_view& results_)
    : A(A_)
    , diag(diag_)
    , eps(threshold)
...
class SignedClassicalSADropFunctor {
 ...
 public:
  SignedClassicalSADropFunctor(local_matrix_type& A_, magnitudeType threshold, diag_view_type diag_, boundary_nodes_view boundaryNodes_, results_view& results_)
    : A(A_)
    , diag(diag_)
    , eps(threshold)
    , boundaryNodes(boundaryNodes_)
  ...
  bool operator()(const local_ordinal_type rlid) {
    ...
    for (local_ordinal_type k = 0; k < row.length; ++k) {
      ...
      auto val                  = row.value(k);
      auto aiiajj               = ATS::magnitude(diag(rlid)) * ATS::magnitude(diag(clid));  // |a_ii|*|a_jj|
      const bool is_nonpositive = ATS::real(val) <= ATS::zero();
      auto aij2                 = ATS::magnitude(val) * ATS::magnitude(val);  // |a_ij|^2
      if (is_nonpositive)
        aij2 = -aij2;
      results(offset + k) = (aij2 <= eps * eps * aiiajj) ? DROP : KEEP;

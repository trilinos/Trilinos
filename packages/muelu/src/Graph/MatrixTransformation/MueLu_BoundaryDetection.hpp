...
class DirichletFunctor {
 ...
 public:
  DirichletFunctor(local_matrix_type& A_, boundary_nodes_view boundaryNodes_, magnitudeType dirichletThreshold_, local_ordinal_type dirichletNonzeroThreshold_)
    : A(A_)
  ...
  void operator()(const local_ordinal_type rlid) const {
    ...
    for (local_ordinal_type k = 0; k < row.length; ++k) {
      ...
      if ((rlid != clid) && (ATS::magnitude(val) > dirichletThreshold)) {
        ++nnz;
...
class RowSumFunctor {
 ...
 public:
  ...
  KOKKOS_FORCEINLINE_FUNCTION
  void operator()(const local_ordinal_type rlid) const {
    magnitudeType rowsum  = magATS::zero();
    magnitudeType diagval = magATS::zero();
    auto row              = A.row(rlid);
    for (local_ordinal_type k = 0; k < row.length; ++k) {
      auto clid = row.colidx(k);
      auto val  = row.value(k);

...
class DropFunctor {
 ...
 public:
  DropFunctor(local_matrix_type& A_, magnitudeType threshold, diag_view_type diag_, DistanceFunctorType& dist2_, boundary_nodes_view boundaryNodes_, results_view& results_)
    : A(A_)
    , eps(threshold)
    , diag(diag_)
    , dist2(dist2_)

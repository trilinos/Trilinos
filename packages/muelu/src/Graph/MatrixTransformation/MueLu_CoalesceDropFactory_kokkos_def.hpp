...
#define runCountingFunctor(...)                                                                                                                                  \
  {                                                                                                                                                              \
    auto criteria        = std::make_tuple(__VA_ARGS__);                                                                                                         \
    auto countingFunctor = MatrixConstruction::PointwiseCountingFunctor<local_matrix_type, decltype(criteria), false>(lclA, results, filtered_rowptr, criteria); \
...
#define runCountingFunctor(...)                                                                                                                                  \
  {                                                                                                                                                              \
    ...
    auto regular_criteria = std::make_tuple(__VA_ARGS__);                                                                                                        \
    auto debug_criteria   = std::make_tuple(debug);                                                                                                              \
    auto criteria         = std::tuple_cat(regular_criteria, debug_criteria);                                                                                    \
    auto countingFunctor  = MatrixConstruction::PointwiseCountingFunctor<local_matrix_type, decltype(criteria), true>(lclA, results, filtered_rowptr, criteria); \
    Kokkos::parallel_scan(functorLabel, range, countingFunctor, nnz_filtered);                                                                                   \
  }
#endif
    ...
    if (threshold != zero) {
      ...
      if (algo == "classical" || algo == "block diagonal classical") {
        ...
        auto lclDiag2d = diag->getDeviceLocalView(Xpetra::Access::ReadOnly);
        auto lclDiag1d = Kokkos::subview(lclDiag2d, Kokkos::ALL(), 0);

        if (algo == "block diagonal classical") {
          auto BlockNumbers      = GetBlockNumberMVs(currentLevel);
          auto block_diagonalize = Misc::BlockDiagonalizeFunctor(lclA,
                                                                 std::get<0>(BlockNumbers)->getDeviceLocalView(Xpetra::Access::ReadOnly),
                                                                 std::get<1>(BlockNumbers)->getDeviceLocalView(Xpetra::Access::ReadOnly), results);
          if (classicalAlgoStr == "default") {
            auto classical_dropping = ClassicalDropping::AbsDropFunctor(lclA, threshold, lclDiag1d, results);
        ...
        auto diag = Utilities::GetMatrixMaxMinusOffDiagonal(*A);
        if (GetVerbLevel() & Statistics1)
          GetOStream(Statistics1) << "Calculated max point off-diagonal" << std::endl;
        auto lclDiag2d = diag->getDeviceLocalView(Xpetra::Access::ReadOnly);
        auto lclDiag1d = Kokkos::subview(lclDiag2d, Kokkos::ALL(), 0);

        auto signed_classical_rs_dropping = ClassicalDropping::SignedClassicalRSDropFunctor(lclA, threshold, lclDiag1d, results);
        ...
        auto signed_classical_sa_dropping = ClassicalDropping::SignedClassicalSADropFunctor(lclA, threshold, lclDiag1d, results);
        ...
        if (!importer.is_null()) {
          ghostedCoords = Xpetra::MultiVectorFactory<typename Teuchos::ScalarTraits<Scalar>::magnitudeType, LO, GO, NO>::Build(importer->getTargetMap(), coords->getNumVectors());
          ghostedCoords->doImport(*coords, *importer, Xpetra::INSERT);
        } else
          ghostedCoords = coords;
        auto lclCoords             = coords->getDeviceLocalView(Xpetra::Access::ReadOnly);
        auto lclGhostedCoordinates = ghostedCoords->getDeviceLocalView(Xpetra::Access::ReadOnly);
        auto dist2                 = DistanceLaplacian::DistanceFunctor<decltype(lclCoords), LocalOrdinal>(lclCoords, lclGhostedCoordinates);
        // Construct ghosted distance Laplacian diagonal
        auto diag      = DistanceLaplacian::getDiagonal(A, dist2);
        auto lclDiag2d = diag->getDeviceLocalView(Xpetra::Access::ReadOnly);
        auto lclDiag1d = Kokkos::subview(lclDiag2d, Kokkos::ALL(), 0);
        ...
        if (algo == "block diagonal distance laplacian") {
          ...
          if (distanceLaplacianAlgoStr == "default") {
            auto dist_laplacian_dropping = DistanceLaplacian::DropFunctor(lclA, threshold, lclDiag1d, dist2, results);
...
#undef runBoundaryFunctor
  ...
  // Number of nonzeros of filtered A
  std::tuple<LocalOrdinal, LocalOrdinal> nnz = {0, 0};
...
#define runCountingFunctor(...)                                                                                                                                                                          \
  {                                                                                                                                                                                                      \
    auto criteria        = std::make_tuple(__VA_ARGS__);                                                                                                                                                 \
    auto countingFunctor = MatrixConstruction::VectorCountingFunctor<local_matrix_type, decltype(criteria), false>(lclA, blkPartSize, colTranslation, results, filtered_rowptr, graph_rowptr, criteria); \
...
#undef runCountingFunctor
  ...
  LocalOrdinal nnz_filtered = std::get<0>(nnz);
  LocalOrdinal nnz_graph    = std::get<1>(nnz);

...
#define SET_VALID_ENTRY(name) validParamList->setEntry(name, MasterList::getEntry(name))
  ...
  // SET_VALID_ENTRY("tentative: constant column sums"); // Needs to be implemented to be able to replace the non-Kokkos TentativePFactory.
...
void TentativePFactory_kokkos<Scalar, LocalOrdinal, GlobalOrdinal, Node>::BuildP(Level& fineLevel, Level& coarseLevel) const {
  ...
  // No coarse DoFs so we need to bail by setting Ptentative to null and returning
...
void TentativePFactory_kokkos<Scalar, LocalOrdinal, GlobalOrdinal, Node>::
  ...
  if (NSDim == 1) {
...

    {
      ...
      using LocalQrFunctorType = LocalQR::LocalQRDecompFunctor<LocalOrdinal, GlobalOrdinal, Scalar, DeviceType, decltype(fineNSRandom),

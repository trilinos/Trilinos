...
void SaPFactory<Scalar, LocalOrdinal, GlobalOrdinal, Node>::BuildP(Level& fineLevel, Level& coarseLevel) const {
  ...
  if (!specialMaxwell1Smoothing) {
    if (dampingFactor != Teuchos::ScalarTraits<SC>::zero()) {
      ...
      GetOStream(Statistics1) << "Prolongator damping factor = " << dampingFactor / lambdaMax << " (" << dampingFactor << " / " << Teuchos::ScalarTraits<SC>::magnitude(lambdaMax) << ")" << std::endl;
    ...
    if (nodalAndEdgeSmoothing) {
      ...
      GetOStream(Statistics1) << "Maxwell1 prolongator nodal damping factor = " << nodalDampingFactor / lambdaMaxNodal << " (" << nodalDampingFactor << " / " << Teuchos::ScalarTraits<SC>::magnitude(lambdaMaxNodal) << ")" << std::endl;
      ...
      SC beta = nodalDampingFactor / Teuchos::ScalarTraits<SC>::magnitude(lambdaMaxNodal);
      TEUCHOS_TEST_FOR_EXCEPTION(!std::isfinite(Teuchos::ScalarTraits<SC>::magnitude(beta)), Exceptions::RuntimeError, "Prolongator damping factor needs to be finite.");
    ...
    if (edgeSmoothing) {
      ...
      SC alpha = dampingFactor / Teuchos::ScalarTraits<SC>::magnitude(lambdaMax);
      TEUCHOS_TEST_FOR_EXCEPTION(!std::isfinite(Teuchos::ScalarTraits<SC>::magnitude(alpha)), Exceptions::RuntimeError, "Prolongator damping factor needs to be finite.");

      GetOStream(Statistics1) << "Maxwell1 prolongator edge damping factor = " << alpha << " (" << dampingFactor << " / " << Teuchos::ScalarTraits<SC>::magnitude(lambdaMax) << ")" << std::endl;

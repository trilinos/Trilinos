...
template <class Scalar, class LocalOrdinal, class GlobalOrdinal, class Node>
typename Xpetra::CrsGraph<LocalOrdinal, GlobalOrdinal, Node>::local_graph_type DenseConstraint<Scalar, LocalOrdinal, GlobalOrdinal, Node>::FindBlocks(RCP<const Xpetra::CrsGraph<LocalOrdinal, GlobalOrdinal, Node>>& /*XXt*/) {
  ...
  const auto numConstraints = lclX.numRows();
  const auto NSDim          = B_->getNumVectors();
  const auto numBlocks      = numConstraints / NSDim;
  TEUCHOS_ASSERT_EQUALITY(numBlocks * NSDim, (decltype(numBlocks))numConstraints);

  using graph_type = typename CrsGraph::local_graph_type;
  typename graph_type::row_map_type::non_const_type rowptr(Kokkos::ViewAllocateWithoutInitializing("blocks_rowptr"), numBlocks + 1);
  typename graph_type::entries_type::non_const_type indices(Kokkos::ViewAllocateWithoutInitializing("blocks_indices"), numConstraints);

  Kokkos::parallel_for(
      "MueLu::DenseConstraint::FindBlocks", range_type(0, numBlocks), KOKKOS_LAMBDA(const LocalOrdinal blockId) {
        const auto start = blockId * NSDim;
        const auto end   = (blockId + 1) * NSDim;
        if (blockId == 0)
          rowptr(0) = start;
        rowptr(blockId + 1) = end;
        for (auto constraintId = start; constraintId < end; ++constraintId)

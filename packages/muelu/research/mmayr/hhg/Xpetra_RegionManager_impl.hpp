    ...
    const std::string& mappingFileName)
{
  ...
  while (std::getline(inputFile, line))
  {
    ...
    {
      while(is >> number)
        compositeInfo.push_back(number);

      TEUCHOS_TEST_FOR_EXCEPTION(compositeInfo.size() != 2, Exceptions::RuntimeError,
          "The composite information must be a couple of integers: nTotal of nodes + nTotal of regions \n");
      numNodes_ = compositeInfo[0];
      numRegions_ = compositeInfo[1];
    }
    else if (lineIndex > 2)
    {
      while (is >> number)
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(node.size() != 2, Exceptions::RuntimeError,
          "The node information must be a couple of integers: Node index + Region index \n");

      // create the tuple and append it to the collection of all tuples
      nodeRegionPair = std::make_tuple(node[0], node[1]);
      nodeRegionPairs_.push_back(nodeRegionPair);

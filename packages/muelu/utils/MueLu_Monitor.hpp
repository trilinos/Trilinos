...
namespace MueLu {
  ...
  public:
    ...
    void monitor() {
      //    TODO:  check timer stopped and totalElapsedTime==0
  ...
  private:
    ...
    static std::stack<MutuallyExclusiveTimer<TagName>*> timerStack_;
  ...
  public:
    ...
    MutuallyExclusiveTimerMonitor(const BaseClass& object, const std::string& msg, MsgType timerLevel = Timings0)
    {
      // Inherit verbosity from 'object'
      SetVerbLevel(object.GetVerbLevel());
      setOStream(object.getOStream());
      ...
      if (IsPrint(timerLevel) && 
        ...
        if (!IsPrint(NoTimeReport)) {
          // Start the timer
          timer_ = MutuallyExclusiveTimer<TagName>::getNewTimer("MueLu: " + msg + " (ME)");

	  //          timeMonitor_ = rcp(new Teuchos::TimeMonitor(*timer_));
        } else {
          timer_ = rcp(new MutuallyExclusiveTimer<TagName>("MueLu: " + msg + " (ME)"));
          timer_->start();
        }
      }
    }

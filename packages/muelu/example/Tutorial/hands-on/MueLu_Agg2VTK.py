...
def main(argv=None):
  ...
  #while check_files_for_next_level(0,numprocs, "aggs_level%LEVEL_proc%PROC.txt") == True:
  #  numprocs = numprocs + 1
  #numprocs = numprocs - 1
  #print "Aggregtaion information for " + str(numprocs) + " processors found"

  # process all multigrid levels
  while check_files_for_next_level(level,numprocs,"aggs_level%LEVEL_proc%PROC.txt"):
    global_nodecoords = []
    ...
    # read aggregates
    aggid2nodes, aggid2procs = readin_aggregates("aggs_level%LEVEL_proc%PROC.txt",numprocs,level)

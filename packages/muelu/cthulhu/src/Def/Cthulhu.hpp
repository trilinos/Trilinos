...
// GO nx = 4;
// GO ny = 4;
// RCP<const Map> map = rcp( new MyMap(nx*ny, 0, comm) ); // MyMap = TpetraMap or EpetraMap according to the macro USE_EPETRA and USE_TPETRA.
//
...
// You can optionally define which implementation of the linar algebra layer you want to use (ie: Epetra or Tpetra) by defining the macro USE_EPETRA or USE_TPETRA.
// Instead of using directly TpetraMap or EpetraMap when allocating a Cthulhu::Map, you can then use the type 'MyMap'. This type is define according to USE_EPETRA and USE_TPETRA.
...
#if defined (USE_TPETRA) && !defined(USE_EPETRA)
...
#elif defined(USE_EPETRA) && !defined(USE_TPETRA)

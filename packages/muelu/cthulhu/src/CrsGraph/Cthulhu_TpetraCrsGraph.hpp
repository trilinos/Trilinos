...
namespace Cthulhu {
  ...
  TpetraCrsGraph(const RCP<const Map<LocalOrdinal,GlobalOrdinal,Node> > &rowMap, const RCP<const Map<LocalOrdinal,GlobalOrdinal,Node> > &colMap, const ArrayRCP<const size_t> &NumEntriesPerRowToAlloc, ProfileType pftype = DynamicProfile)
    ...
    */
    inline void insertGlobalIndices(GlobalOrdinal globalRow, const ArrayView<const GlobalOrdinal> &indices) { CTHULHU_DEBUG_ME; graph_->insertGlobalIndices(globalRow, indices); };
    ...
    /**
       ...
       \note If the graph row already contains entries at the indices corresponding to values in \c indices, then the redundant indices will be eliminated; this may happen at insertion or during the next call to fillComplete().
    */
    inline void insertLocalIndices(LocalOrdinal localRow, const ArrayView<const LocalOrdinal> &indices) { CTHULHU_DEBUG_ME; graph_->insertLocalIndices(localRow, indices); };
    ...
    /**
       ...
       \post <tt>isLocallyIndexed() == true</tt>
    */
    inline void removeLocalIndices(LocalOrdinal localRow) { CTHULHU_DEBUG_ME; graph_->removeLocalIndices(localRow); };
    ...
    */
    ...
    inline const RCP<const Map<LocalOrdinal,GlobalOrdinal,Node> > getRowMap() const { CTHULHU_DEBUG_ME; return rcp( new TpetraMapClass(graph_->getRowMap())); };

    //! \brief Returns the Map that describes the column distribution in this graph.
    inline const RCP<const Map<LocalOrdinal,GlobalOrdinal,Node> > getColMap() const { CTHULHU_DEBUG_ME; return rcp( new TpetraMapClass(graph_->getColMap())); };

    //! Returns the Map associated with the domain of this graph.
    inline const RCP<const Map<LocalOrdinal,GlobalOrdinal,Node> > getDomainMap() const { CTHULHU_DEBUG_ME; return rcp( new TpetraMapClass(graph_->getDomainMap())); };

    //! Returns the Map associated with the domain of this graph.
    inline const RCP<const Map<LocalOrdinal,GlobalOrdinal,Node> > getRangeMap() const { CTHULHU_DEBUG_ME; return rcp( new TpetraMapClass(graph_->getRangeMap())); };
     ...
     */
    inline global_size_t getGlobalNumRows() const { CTHULHU_DEBUG_ME; return graph_->getGlobalNumRows(); };

    //! \brief Returns the number of global columns in the graph.
    /** Undefined if isFillActive().
     */
    inline global_size_t getGlobalNumCols() const { CTHULHU_DEBUG_ME; return graph_->getGlobalNumCols(); };

    //! Returns the number of graph rows owned on the calling node.
    inline size_t getNodeNumRows() const { CTHULHU_DEBUG_ME; return graph_->getNodeNumRows(); };

    //! Returns the number of columns connected to the locally owned rows of this graph.
    /** Throws std::runtime_error if <tt>hasColMap() == false</tt>
     */
    inline size_t getNodeNumCols() const { CTHULHU_DEBUG_ME; return graph_->getNodeNumCols(); };

    //! Returns the index base for global indices for this graph. 
    inline GlobalOrdinal getIndexBase() const { CTHULHU_DEBUG_ME; return graph_->getIndexBase(); };

    //! Returns the global number of entries in the graph.
    /** Undefined if isFillActive().
     */
    inline global_size_t getGlobalNumEntries() const { CTHULHU_DEBUG_ME; return graph_->getGlobalNumEntries(); };

    //! Returns the local number of entries in the graph.
    inline size_t getNodeNumEntries() const { CTHULHU_DEBUG_ME; return graph_->getNodeNumEntries(); };

    //! \brief Returns the current number of entries on this node in the specified global row.
    /*! Returns OrdinalTraits<size_t>::invalid() if the specified global row does not belong to this graph. */
    inline size_t getNumEntriesInGlobalRow(GlobalOrdinal globalRow) const { CTHULHU_DEBUG_ME; return graph_->getNumEntriesInGlobalRow(globalRow); };

    //! Returns the current number of entries on this node in the specified local row.
    /*! Returns OrdinalTraits<size_t>::invalid() if the specified local row is not valid for this graph. */
    inline size_t getNumEntriesInLocalRow(LocalOrdinal localRow) const { CTHULHU_DEBUG_ME; return graph_->getNumEntriesInLocalRow(localRow); };
    ...
    */
    inline size_t getNodeAllocationSize() const { CTHULHU_DEBUG_ME; return graph_->getNodeAllocationSize(); };
    ...
    inline size_t getNumAllocatedEntriesInGlobalRow(GlobalOrdinal globalRow) const { CTHULHU_DEBUG_ME; return graph_->getNumAllocatedEntriesInGlobalRow(globalRow); };

    //! Returns the current number of allocated entries on this node in the specified local row.
    /** Throws exception std::runtime_error if the specified local row is not valid for this node. */
    inline size_t getNumAllocatedEntriesInLocalRow(LocalOrdinal localRow) const { CTHULHU_DEBUG_ME; return graph_->getNumAllocatedEntriesInLocalRow(localRow); };

    //! \brief Returns the number of global diagonal entries, based on global row/column index comparisons. 
    /** Undefined if isFillActive().
     */
    inline global_size_t getGlobalNumDiags() const { CTHULHU_DEBUG_ME; return graph_->getGlobalNumDiags(); };

    //! \brief Returns the number of local diagonal entries, based on global row/column index comparisons. 
    /** Undefined if isFillActive().
     */
    inline size_t getNodeNumDiags() const { CTHULHU_DEBUG_ME; return graph_->getNodeNumDiags(); };

    //! \brief Returns the maximum number of entries across all rows/columns on all nodes. 
    /** Undefined if isFillActive().
     */
    inline size_t getGlobalMaxNumRowEntries() const { CTHULHU_DEBUG_ME; return graph_->getGlobalMaxNumRowEntries(); };

    //! \brief Returns the maximum number of entries across all rows/columns on this node. 
    /** Undefined if isFillActive().
     */
    inline size_t getNodeMaxNumRowEntries() const { CTHULHU_DEBUG_ME; return graph_->getNodeMaxNumRowEntries(); };

    //! \brief Indicates whether the graph has a well-defined column map. 
    inline bool hasColMap() const { CTHULHU_DEBUG_ME; return graph_->hasColMap(); }; 

    //! \brief Indicates whether the graph is lower triangular.
    /** Undefined if isFillActive().
     */
    inline bool isLowerTriangular() const { CTHULHU_DEBUG_ME; return graph_->isLowerTriangular(); };

    //! \brief Indicates whether the graph is upper triangular.
    /** Undefined if isFillActive().
     */
    inline bool isUpperTriangular() const { CTHULHU_DEBUG_ME; return graph_->isUpperTriangular(); };

    //! \brief If graph indices are in the local range, this function returns true. Otherwise, this function returns false. */
    inline bool isLocallyIndexed() const { CTHULHU_DEBUG_ME; return graph_->isLocallyIndexed(); };

    //! \brief If graph indices are in the global range, this function returns true. Otherwise, this function returns false. */
    inline bool isGloballyIndexed() const { CTHULHU_DEBUG_ME; return graph_->isGloballyIndexed(); };

    //! Returns \c true if fillComplete() has been called and the graph is in compute mode.
    inline bool isFillComplete() const { CTHULHU_DEBUG_ME; return graph_->isFillComplete(); };
    ...
    */
    inline void getGlobalRowCopy(GlobalOrdinal GlobalRow, 
                                 ...
                                 size_t &NumIndices
                                 ) const { CTHULHU_DEBUG_ME; graph_->getGlobalRowCopy(GlobalRow, Indices, NumIndices); };
    ...
    */
    inline void getLocalRowView(LocalOrdinal LocalRow, ArrayView<const LocalOrdinal> &indices) const { CTHULHU_DEBUG_ME; graph_->getLocalRowView(LocalRow, indices); };
    ...
    inline std::string description() const { CTHULHU_DEBUG_ME; return graph_->description(); };

    /** \brief Print the object with some verbosity level to an FancyOStream object. */
    inline void describe(Teuchos::FancyOStream &out, const Teuchos::EVerbosityLevel verbLevel=Teuchos::Describable::verbLevel_default) const { CTHULHU_DEBUG_ME; graph_->describe(out, verbLevel); };
    ...
    inline bool checkSizes(const DistObject<GlobalOrdinal,LocalOrdinal,GlobalOrdinal,Node>& source) { CTHULHU_DEBUG_ME; return graph_->checkSizes(source); };

    inline void copyAndPermute(const DistObject<GlobalOrdinal,LocalOrdinal,GlobalOrdinal,Node> & source,
                               size_t numSameIDs,
                               const ArrayView<const LocalOrdinal> &permuteToLIDs,
                               const ArrayView<const LocalOrdinal> &permuteFromLIDs) { CTHULHU_DEBUG_ME; graph_->copyAndPermute(sourcem numSameIDs, permuteToLIDs, permuteFromLIDs); };
    ...
    */
    ...
    /*! Returns null if optimizeStorage() hasn't been called.
      The returned buffer exists in host-memory.
    */
    inline ArrayRCP<const LocalOrdinal> getNodePackedIndices() const { CTHULHU_DEBUG_ME; return graph_->getNodePackedIndices(); };
    ...
    */
    ...
    CTHULHU_DEPRECATED inline ArrayRCP<const GlobalOrdinal> getGlobalRowView(GlobalOrdinal GlobalRow) const { CTHULHU_DEBUG_ME; return graph_->getGlobalRowView(GlobalRow); };

    //! Deprecated. Get a persisting const view of the elements in a specified local row of the graph.
    CTHULHU_DEPRECATED inline ArrayRCP<const LocalOrdinal> getLocalRowView(LocalOrdinal LocalRow) const { CTHULHU_DEBUG_ME; return graph_->getLocalRowView(LocalRow); };

    //@}
#endif

    RCP< const Tpetra::CrsGraph<LocalOrdinal, GlobalOrdinal, Node, LocalMatOps> > getTpetra_CrsGraph() const { CTHULHU_DEBUG_ME; return graph_; }

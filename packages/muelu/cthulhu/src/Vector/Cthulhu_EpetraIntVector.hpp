...
namespace Cthulhu {
  ...
  public:
     ...
     */
    inline void replaceGlobalValue(int globalRow, const int &value) {  this->EpetraMultiVector::replaceGlobalValue(globalRow,0,value); };

    //! Adds specified value to existing value at the specified location.
    /** \pre \c globalRow must be a valid global element on this node, according to the row map.
     */
    inline void sumIntoGlobalValue(int globalRow, const int &value) {  this->EpetraMultiVector::sumIntoGlobalValue(globalRow, 0, value); };

    //! Replace current value at the specified location with specified values.
    /** \pre \c localRow must be a valid local element on this node, according to the row map.
     */
    inline void replaceLocalValue(int myRow, const int &value) {  this->EpetraMultiVector::replaceLocalValue(myRow, 0, value); };

    //! Adds specified value to existing value at the specified location.
    /** \pre \c localRow must be a valid local element on this node, according to the row map.
     */
    inline void sumIntoLocalValue(int myRow, const int &value) {  this->EpetraMultiVector::sumIntoLocalValue(myRow, 0, value); };
#endif
    //@}
    ...
    Teuchos::ScalarTraits<int>::magnitudeType norm1() const {  TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "TODO"); return -1; }

    //using EpetraMultiVector::norm2; // overloading, not hiding
    //! Compute 2-norm of this Vector.
    Teuchos::ScalarTraits<int>::magnitudeType norm2() const {  TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "TODO"); return -1; }

    //using EpetraMultiVector::normInf; // overloading, not hiding
    //! Compute Inf-norm of this Vector.
    Teuchos::ScalarTraits<int>::magnitudeType normInf() const {  TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "TODO"); return -1; }
    ...
    int meanValue() const {  TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "TODO"); return -1; }

    //using EpetraMultiVector::maxValue; // overloading, not hiding
    //! Compute max value of this Vector.
    int maxValue() const {  TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "TODO"); return -1; }
    //@} 
    ...
    //TODO wrap RCP etc.   RCP<Epetra_Vector> getEpetra_Vector() const {  this->EpetraMultiVector::getEpetra_MultiVector()->getVector(0); }
    ...
    /** \pre \c globalRow must be a valid global element on this node, according to the row map.
     */
    inline void replaceGlobalValue(int globalRow, size_t vectorIndex, const int &value) {  vec_->replaceGlobalValue(globalRow, vectorIndex, value); }
    ...
    /** \pre \c globalRow must be a valid global element on this node, according to the row map.
     */
    inline void sumIntoGlobalValue(int globalRow, size_t vectorIndex, const int &value) {  vec_->sumIntoGlobalValue(globalRow, vectorIndex, value); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    ...
    /** \pre \c localRow must be a valid local element on this node, according to the row map.
     */
    inline void replaceLocalValue(int myRow, size_t vectorIndex, const int &value) {  vec_->replaceLocalValue(myRow, vectorIndex, value); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    ...
    /** \pre \c localRow must be a valid local element on this node, according to the row map.
     */
    inline void sumIntoLocalValue(int myRow, size_t vectorIndex, const int &value) {  vec_->sumIntoLocalValue(myRow, vectorIndex, value); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA

    //! Initialize all values in a multi-vector with specified value.
    inline void putScalar(const int &value) {  vec_->PutValue(value); }

    //! Set multi-vector values to random numbers.
    inline void randomize() {   TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "Cthulhu::EpetraIntVector::randomize(): Functionnality not available in Epetra"); }
     ...
     */
    inline EpetraMultiVector<int,int,int>& operator=(const EpetraMultiVector<int,int,int> &source) {  return vec_->(source); }
#endif // CTHULHU_NOT_IMPLEMENTED
    ...
    */
    ...
    inline Teuchos::RCP<MultiVector<int,int,int> > subCopy(const Teuchos::Range1D &colRng) const {  return vec_->subCopy(colRng); }

    //! Returns a EpetraMultiVector with copies of selected columns.
    inline Teuchos::RCP<MultiVector<int,int,int> > subCopy(const Teuchos::ArrayView<const size_t> &cols) const {  return vec_->subCopy(cols); }

    //! Returns a const MultiVector with const views of selected columns.
    inline Teuchos::RCP<const MultiVector<int,int,int> > subView(const Teuchos::Range1D &colRng) const {  return vec_->subView(colRng); }

    //! Returns a const MultiVector with const views of selected columns.
    inline Teuchos::RCP<const MultiVector<int,int,int> > subView(const Teuchos::ArrayView<const size_t> &cols) const {  return vec_->subView(cols); }

    //! Returns a MultiVector with views of selected columns.
    inline Teuchos::RCP<MultiVector<int,int,int> > subViewNonConst(const Teuchos::Range1D &colRng) {  return vec_->subViewNonConst(colRng); }

    //! Returns a MultiVector with views of selected columns.
    inline Teuchos::RCP<MultiVector<int,int,int> > subViewNonConst(const Teuchos::ArrayView<const size_t> &cols) {  return vec_->subViewNonConst(cols); }
    ...
    */
    inline Teuchos::RCP<const MultiVector<int,int,int> > offsetView(const Teuchos::RCP<const Map<int,int> > &subMap, size_t offset) const {  return vec_->offsetView(subMap, offset); }
    ...
    /** 
        ...
        \pre  <tt>subMap->getNodeNumElements() + offset < this->getLocalLength()</tt>
    */
    inline Teuchos::RCP<MultiVector<int,int,int> > offsetViewNonConst(const Teuchos::RCP<const Map<int,int> > &subMap, size_t offset) {  return vec_->offsetViewNonConst(subMap, offset); }

    //! Const Vector access function.
    inline Teuchos::RCP<const Vector<int,int,int> > getVector(size_t j) const {  return vec_->getVector(j); }

    //! Vector access function.
    inline Teuchos::RCP<Vector<int,int,int> > getVectorNonConst(size_t j) {  return vec_->getVectorNonConst(j); }
#endif // CTHULHU_NOT_IMPLEMENTED

    //! Const Local vector access function.
    //! View of the local values in a particular vector of this multi-vector.
    inline Teuchos::ArrayRCP<const int> getData(size_t j) const {  
...
#ifdef CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    //! Return multi-vector values in user-provided two-dimensional array (using Teuchos memory management classes).
    inline void get1dCopy(Teuchos::ArrayView<int> A, size_t LDA) const {  vec_->get1dCopy(A, LDA); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA

#ifdef CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    //! Return multi-vector values in user-provided array of pointers (using Teuchos memory management classes).
    inline void get2dCopy(Teuchos::ArrayView<const Teuchos::ArrayView<int> > ArrayOfPtrs) const {  vec_->get2dCopy(ArrayOfPtrs); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    ...
    inline Teuchos::ArrayRCP<const int> get1dView() const {  return vec_->get1dView(); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA

#ifdef CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    //! Return const persisting pointers to values.
    inline Teuchos::ArrayRCP<Teuchos::ArrayRCP<const int> > get2dView() const {  return vec_->get2dView(); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA

#ifdef CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    //! Return non-const persisting view of values in a one-dimensional array. Throws std::runtime_error if the underlying data is non-contiguous.  Teuchos::ArrayRCP<int> get1dViewNonConst() {  return vec_->(); }
    inline Teuchos::ArrayRCP<int> get1dViewNonConst() {  return vec_->get1dViewNonConst(); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA

#ifdef CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    //! Return non-const persisting pointers to values.
    inline Teuchos::ArrayRCP<Teuchos::ArrayRCP<int> > get2dViewNonConst() {  return vec_->get2dViewNonConst(); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA

#ifdef CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    //! Return a const reference to the underlying Kokkos::MultiVector object (advanced use only)
    inline const Kokkos::MultiVector<int> & getLocalMV() const {  return vec_->getLocalMV(); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA

#ifdef CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    //! Return a non-const reference to the underlying Kokkos::MultiVector object (advanced use only)
    inline Kokkos::MultiVector<int> & getLocalMVNonConst() {  return vec_->getLocalMVNonConst(); }
#endif // CTHULHU_NOT_IMPLEMENTED_FOR_EPETRA
    ...
    inline void scale(const int &alpha) {  TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "TODO"); }

//     //! Scale the current values of a multi-vector, this[j] = alpha[j]*this[j].
//     inline void scale(Teuchos::ArrayView<const int> alpha) {  vec_->Scale(alpha.getRawPtr()); }
...
//     }
    ...
    inline void norm1(const Teuchos::ArrayView<Teuchos::ScalarTraits<int>::magnitudeType> &norms) const {  TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "TODO"); }

    //! Compute 2-norm of each vector in multi-vector.
    inline void norm2(const Teuchos::ArrayView<Teuchos::ScalarTraits<int>::magnitudeType> &norms) const {  TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "TODO"); }

    //! Compute Inf-norm of each vector in multi-vector.
    inline void normInf(const Teuchos::ArrayView<Teuchos::ScalarTraits<int>::magnitudeType> &norms) const {  TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "TODO"); }
    ...
    inline void meanValue(const Teuchos::ArrayView<int> &means) const {        TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "TODO"); }

    //! Compute max value of each vector in multi-vector.
    inline void maxValue(const Teuchos::ArrayView<int> &maxs) const {        TEST_FOR_EXCEPTION(1, Cthulhu::Exceptions::NotImplemented, "TODO"); }
    ...
    inline size_t getNumVectors() const {  return 1; }

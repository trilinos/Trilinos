...
\cbb{aggregation: greedy Dirichlet}{bool}{false}{Force the node to be Dirichlet if any DOFs on the node are Dirichlet (default is nodes are Dirichlet only if all DOFs on node are Dirichlet).}

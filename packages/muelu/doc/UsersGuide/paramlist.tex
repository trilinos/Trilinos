...
\cbb{aggregation: symmetrize graph after dropping}{string}{"no symmetrization"}{Symmetrize the graph used for aggregation.}
          
\cbb{aggregation: symmetrize color graph}{string}{"strong wins"}{Symmetrize graph used for coloring when "aggregation: coloring: use color graph" is set. Possible values: "strong wins" ( (i,j) kept if unsymmetrized (j,i) is kept) or "weak wins" ( (i,j) dropped if unsymmetrized (j,i) dropped),}
...
\cbb{aggregation: strength-of-connection: matrix}{string}{"A"}{The matrix that is used for constructing the graph used for aggregation. Possible values: "A" (meaning the level matrix itself), "distance laplacian", and "MinvA".}
...
\cbb{aggregate qualities: mode}{string}{eigenvalue}{Whether to compute eigenvalues (``eigenvalue''),
...
\cbb{rap: prebuild coarse graph}{bool}{false}{Prebuild the coarse graph and pass it to the triple matrix product kernel.}
...
\cbb{emin: iterative method}{string}{"cg"}{Iterative method to use for energy minimization of initial prolongator in energy-minimization. Possible values: "cg" (conjugate gradient), "gmres" (generalized minimum residual).}
...
\cba{maxwell1: 11list}{\parameterlist}{Specifies the multigrid solver for the 11 block}
          
\cba{maxwell1: 22list}{\parameterlist}{Specifies the multigrid solver for the 22 block}

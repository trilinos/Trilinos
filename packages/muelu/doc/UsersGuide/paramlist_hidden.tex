...
\cbb{filtered matrix: spreadlumping diag dom growth factor}{double}{1.1}{Determines how much a measure of diagonal dominance is allowed to grow when using spread lumping. Only used when filtered matrix: use lumping and filtered matrix: use lumping are both set to true. }
        
\cbb{filtered matrix: spreadlumping diag dom cap}{double}{2.0}{For matrix rows that are very diagonally dominant, we ignore the diag dom growth factor and instead use this simple cap as to how large the diagonal dominance measure should allowed to be after dropping. Only used when filtered matrix: use lumping and filtered matrix: use lumping are both set to true. }

...
Choose the parameters from the \texttt{n1\_easy.xml} file instead. If you run the example you might find that the GMRES method did not converge within 50 iterations.
...
\begin{exercise}
Use \texttt{multigrid algorithm = pg} and compare the results with \texttt{multigrid algorithm = unsmoothed}. What is the difference in the number of GMRES iterations? What is changing in the multigrid setup?
\end{exercise}
\begin{exercise}
...
Note, that the \verb|RAPFactory| accepts a sublist \verb|TransferFactories| which contains all the additional helper transfer factories which are called after the Galerkin product. Figure \ref{fig:simpledesignaggregatesvis} shows that the helper transfer factories are registered in the \verb|RAPFactory| using a small circle at the beginning of the arrow.
...
In Part III the \verb|TogglePFactory| is defined. It contains a \verb|TransferFactories| sublist where all different coarsening branches (i.e., the semi-coarsening and aggregation-based coarsening) are defined. The corresponding prolongation operators are listed using the variable name \verb|P| with a number between 1 and 9. In addition to the prolongation operator factories one has also to declare the factories providing the coarse level null space. In case of semi-coarsening this is the \verb|SemiCoarsenPFactory| and for the aggregation-based coarsening this is the \verb|TentativePFactory|. These factories are declared for generating the \verb|Nullspace| variable with a number between 1 and 9 corresponding to the associated transfer operator branch.

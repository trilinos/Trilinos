...
#include "MueLu_UseShortNames.hpp"
  ...
  // We choose random problem dimensions of nx in [5,15] and ny in [14,32]
  GO nx, ny;
  if(rank == 0) {
    nx = 5 + ST::random() % 8;
    ny = 14 + ST::random() % 19;
  ...
  RCP<Matrix> A11 = Teuchos::make_rcp<Xpetra::CrsMatrixWrap<SC, LO, GO, NO>>
    (Xpetra::CrsMatrixFactory<SC, LO, GO, NO>::Build(dofRowMapDual, 0));
  ...
  RCP<MultiVector> nullspace1 = MultiVectorFactory::Build(dofRowMapPrimal, NS1dim);

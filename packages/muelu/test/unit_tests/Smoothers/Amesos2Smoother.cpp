...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(Amesos2Smoother, Apply_CorrectnessSingular, Scalar, LocalOrdinal, GlobalOrdinal, Node) {
#include "MueLu_UseShortNames.hpp"
  MUELU_TESTING_SET_OSTREAM;
  MUELU_TESTING_LIMIT_SCOPE(Scalar, GlobalOrdinal, NO);
  out << "version: " << MueLu::Version() << std::endl;
...
#ifdef HAVE_AMESOS2_KLU2
    smoother = Teuchos::rcp(new Amesos2Smoother("Klu"));
    testDirectSolverSingular(*smoother, out, success);
#endif
...
#define MUELU_ETI_GROUP(SC, LO, GO, Node)                                                            \
  ...
  TEUCHOS_UNIT_TEST_TEMPLATE_4_INSTANT(Amesos2Smoother, Apply_CorrectnessSingular, SC, LO, GO, Node)

...
#include "branchHelperK.hpp"

#include "opfactoryK.hpp"
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (!useQP) {
      ...
      factory = ROL::makePtr<BinaryAdvDiffFactory<RealT,DeviceT>>(*parlist,comm,outStream);
      bHelper = ROL::makePtr<AdvDiffBranchHelper<RealT>>(intTol,method);
    }
    else {
      factory = ROL::makePtr<BinaryAdvDiffQPFactory<RealT,DeviceT>>(*parlist,comm,outStream);

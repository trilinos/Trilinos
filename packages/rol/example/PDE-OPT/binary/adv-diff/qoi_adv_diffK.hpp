...
#ifndef PDEOPT_QOI_ADV_DIFFK_HPP
#define PDEOPT_QOI_ADV_DIFFK_HPP

#include "../../TOOLS/qoiK.hpp"
#include "pde_adv_diffK.hpp"

template <class Real, class DeviceType>
class QoI_State_Cost_adv_diff : public QoI<Real,DeviceType> {
public:
...
public:
  QoI_State_Cost_adv_diff(const ROL::Ptr<fe_type> &fe) : fe_(fe) {}
...
template <class Real, class DeviceType>
class QoI_Control_Cost_adv_diff : public QoI<Real,DeviceType> {
public:
  using scalar_view = Kokkos::DynRankView<Real, DeviceType>;
  using basis_ptr   = Intrepid2::BasisPtr<DeviceType, Real, Real>;
  using fe_type     = FE<Real,DeviceType>;
...
public:
  ...
  Real value(scalar_view & val,
             ...
             const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) {
    Real sum(0);
    for (const auto z : (*z_param)) sum += z;
    val = scalar_view();
    return (sum-budget_);
  }

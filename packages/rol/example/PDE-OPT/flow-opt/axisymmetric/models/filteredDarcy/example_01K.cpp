...
#include "pde_filterK.hpp"
#include "filtered_objK.hpp"
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    int dim = 2;
    auto f_ptr = assembler->createControlVector();  f_ptr->randomize();
    ...
    if (useParamVar) {
      ...
      f1p = ROL::makePtr<PDE_PrimalOptVector<RealT,DeviceT>>(f_ptr,pde,assembler,*parlist);
      fp = ROL::makePtr<PDE_OptVector<RealT>>(f1p,z0p,myRank);
    }
    else {
      fp = ROL::makePtr<PDE_PrimalOptVector<RealT,DeviceT>>(f_ptr,pde,assembler,*parlist);
    }

    // Create combined vectors
    auto xp = ROL::makePtr<ROL::Vector_SimOpt<RealT>>(up,fp);

    // Initialize quadratic objective function.
    //auto qoi  = ROL::makePtr<QoI_Velocity_Darcy<RealT,DeviceT>>(*parlist,
    //                                                            pde->getPressureFE(),
    //                                                            pde->getControlFE(),
    //                                                            pde->getPressureBdryFE(4),
    //                                                            pde->getControlBdryFE(4),
    ...
    auto robj = ROL::makePtr<ROL::Reduced_Objective_SimOpt<RealT>>(obj,con,up,fp,pp,true,false);
    auto fobj = ROL::makePtr<FilteredObjective<RealT,DeviceT>>(robj,pdeFilter,meshMgr,comm,*parlist,*outStream);

    // Build density vector
    ROL::Ptr<ROL::TpetraMultiVector<RealT>> z1p;
    ROL::Ptr<ROL::Vector<RealT>> zp;
    auto z_ptr = fobj->getAssembler()->createControlVector();  z_ptr->randomize();
    if (useParamVar) {
      z1p = ROL::makePtr<PDE_PrimalOptVector<RealT,DeviceT>>(z_ptr,pdeFilter,fobj->getAssembler(),*parlist);
      zp = ROL::makePtr<PDE_OptVector<RealT>>(z1p,z0p,myRank);
    }
    else {
      zp = ROL::makePtr<PDE_PrimalOptVector<RealT,DeviceT>>(z_ptr,pdeFilter,fobj->getAssembler(),*parlist);
    }
    ...
    if (useParamVar) {
      ...
      auto l1_ptr = fobj->getAssembler()->createControlVector();
      auto h1_ptr = fobj->getAssembler()->createControlVector();
      auto l1p = ROL::makePtr<PDE_PrimalOptVector<RealT,DeviceT>>(l1_ptr,pdeFilter,fobj->getAssembler(),*parlist);
      auto h1p = ROL::makePtr<PDE_PrimalOptVector<RealT,DeviceT>>(h1_ptr,pdeFilter,fobj->getAssembler(),*parlist);
      l1p->setScalar(0.0);
      h1p->setScalar(1.0);
    ...
    auto optProb = ROL::makePtr<ROL::Problem<RealT>>(fobj, zp);
    ...
    con->outputTpetraVector(f_ptr,"filtered_control.txt");
    ...
    *outStream << "Residual Norm: " << res[0] << std::endl;
    ...
    auto conFilter = ROL::makePtr<PDE_Constraint<RealT,DeviceT>>(pdeFilter,meshMgr,comm,*parlist,*outStream);
    auto assemblerFilter = conFilter->getAssembler();
    auto filteru_ptr = assemblerFilter->createStateVector();
    auto filterz_ptr = assemblerFilter->createControlVector();
    auto filteru = ROL::makePtr<PDE_PrimalSimVector<RealT,DeviceT>>(filteru_ptr,pdeFilter,assemblerFilter,*parlist);
    auto filterz = ROL::makePtr<PDE_PrimalOptVector<RealT,DeviceT>>(filterz_ptr,pdeFilter,assemblerFilter,*parlist);
    filteru->setScalar(0.0);
    filterz->setScalar(0.0);
    conFilter->solve(*filteru, *filteru, *filterz, tol);
    conFilter->outputTpetraData();

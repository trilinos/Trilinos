...
public:
  PDE_Darcy(ROL::ParameterList &parlist) : Patm_(101.325 /* kg/mm-s^2 */) {
    ...
    if (cubDegree == -1) {  // nodal cubature
      bdryCub_ = ROL::makePtr<Intrepid2::CubatureNodal<DeviceType,Real,Real>>(bdryCellType);
    }
    else {                  // default cubature
      bdryCub_ = cubFactory.create<DeviceType,Real,Real>(bdryCellType, bdryCubDegree); // create default cubature
    }

...
#ifndef IMPERMEABILITYK_HPP
#define IMPERMEABILITYK_HPP
...
private:
  ...
  Real value(const Real z) const {
    if (type_==1) {
      const Real a(viscosity_/minPerm_), b(viscosity_/maxPerm_);
      return b + z * (a - b);
    }
    else if (type_==2) {
      const Real a(-std::log(minPerm_)), b(-std::log(maxPerm_)-a);
      return viscosity_ * std::exp(a + b * z);
    }
    else {
      const Real a(minPerm_), b(maxPerm_ - minPerm_);
      return viscosity_ / (a + b * z);
    }
  }
  ...
  Real deriv1(const Real z) const {
    if (type_==1) {
      ...
      const Real a(-std::log(minPerm_)), b(-std::log(maxPerm_)-a);
      return viscosity_ * b * std::exp(a + b * z);
    }
    else {
      const Real two(2), a(minPerm_), b(maxPerm_ - minPerm_);
      return -viscosity_ * b / std::pow(a + b * z, two);
    }
  }
  ...
  Real deriv2(const Real z) const {
    if (type_==1) {
      ...
      const Real a(-std::log(minPerm_)), b(-std::log(maxPerm_)-a);
      return viscosity_ * b * b * std::exp(a + b * z);
    }
    else {
      const Real two(2), three(3), a(minPerm_), b(maxPerm_ - minPerm_);
      return two * viscosity_ * std::pow(b, two) / std::pow(a + b * z, three);
    }
  }

public:
  Impermeability(ROL::ParameterList &list) {
    ...
    // type_ = 1 ... K = 1/(z/kmin + (1-z)/kmax)
    // type_ = 2 ... K = exp((1-z) log(kmin) + z log(kmax))
  }

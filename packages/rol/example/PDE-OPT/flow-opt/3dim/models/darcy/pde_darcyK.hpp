...
private:
  ...
  void multiplyByRadius(scalar_view &input,
                        const scalar_view cubPts,
...
public:
  PDE_Darcy(ROL::ParameterList &parlist) : Patm_(101.325 /* kg/mm-s^2 */) {
    ...
    std::string elemType = parlist.sublist("Problem").get("Element Type","TET");
    if (elemType == "TET") {
      if (basisDegPres == 2) {
        basisPtrPrs_ = ROL::makePtr<Intrepid2::Basis_HGRAD_TET_C2_FEM<DeviceType,Real,Real>>();
      }
      else {
        std::cout << "\nFirst-order TET basis selected.\n";
        basisPtrPrs_ = ROL::makePtr<Intrepid2::Basis_HGRAD_TET_C1_FEM<DeviceType,Real,Real>>();
      }
    }
    else {
      if (basisDegPres == 2) {
        basisPtrPrs_ = ROL::makePtr<Intrepid2::Basis_HGRAD_HEX_C2_FEM<DeviceType,Real,Real>>();
      }
      else {
    ...
    if (basisDegCtrl == 1) {
      if (elemType == "TET")
        ...
        basisPtrCtrl_ = ROL::makePtr<Intrepid2::Basis_HGRAD_TET_C1_FEM<DeviceType,Real,Real>>();
    }
    else {
  ...
  void residual(scalar_view & res,
    ...
    if (numSideSets > 0) {
      ...
      for (int i = 0; i < numSideSets; ++i) {
        ...
        else if (useNoSlip_ && i==2) /* do-nothing condition */ {}
      }
    }
  }
  ...
  void Hessian_22(scalar_view & hess,
    ...
    // Apply Dirichlet conditions
    scalar_view l0_coeff("l0_coeff", c, p);
    Kokkos::deep_copy(l0_coeff,l_coeff);
    int numSideSets = bdryCellLocIds_.size();
    if (numSideSets > 0) {
      for (int i = 0; i < numSideSets; ++i) {
  ...
  void RieszMap_1(scalar_view & riesz) override {
    ...
    Kokkos::deep_copy(riesz, fePrs_->stiffMat());
    rst::add(riesz, fePrs_->massMat());
    // Evaluate/interpolate finite element fields on cells.
  ...
  void RieszMap_2(scalar_view & riesz) override {
    ...
    // Initialize Jacobians.
    riesz = scalar_view("riesz", c, f, f);
    rst::add(riesz, feCtrl_->massMat());
    // Evaluate on FE basis.
    //scalar_view valPsi;
    //valPsi  = scalar_view(c, f, p);
    //*valPsi = *feCtrl_->N();
    ///*** Evaluate weak form of the residual. ***/
  ...
  void printCellAverages(std::string tag,
                 const scalar_view u_coeff,
                 const scalar_view z_coeff = scalar_view(),
                 const ROL::Ptr<const std::vector<Real>> & z_param = ROL::nullPtr) override {
    // Retrieve dimensions.
    ...
    scalar_view velZ("velZ", c, p);
    scalar_view valCtrl("valCtrl", c, p);
    scalar_view alpha("alpha", c, p);
    scalar_view onesVec("onesVec", c, p);
    scalar_view cellVol("cellVol", c);
    scalar_view avgX("avgX", c);
    scalar_view avgY("avgY", c);
    scalar_view avgZ("avgZ", c);
    scalar_view avgP("avgP", c);
    fePrs_->evaluateGradient(gradP, u_coeff);
    feCtrl_->evaluateValue(valCtrl, z_coeff);
    Kokkos::deep_copy(onesVec, static_cast<Real>(1));
    perm_->compute(alpha, valCtrl, fePrs_->cubPts(), 0);
    fst::scalarMultiplyDataData(AgradP,alpha,gradP);
    // Extract velocity components.
    for (int i = 0; i < c; ++i) {
      for (int j = 0; j < p; ++j) {
        velX(i,j) = -AgradP(i,j,0);
        velY(i,j) = -AgradP(i,j,1);
        velZ(i,j) = -AgradP(i,j,2);
      }
    }
    // Compute cell volumes.
    fst::integrate(cellVol,onesVec,onesVec,false);
    // Compute velocity X cell average.
    fst::integrate(avgX,velX,onesVec,false);
    for (int i = 0; i < c; ++i) avgX(i) /= cellVol(i);
    // Compute velocity Y cell average.
    fst::integrate(avgY,velY,onesVec,false);
    for (int i = 0; i < c; ++i) avgY(i) /= cellVol(i);
    // Compute velocity Z cell average.
    fst::integrate(avgZ,velZ,onesVec,false);
    for (int i = 0; i < c; ++i) avgZ(i) /= cellVol(i);
    // Compute permeability cell average.
    fst::integrate(avgP,alpha,onesVec,false);
    for (int i = 0; i < c; ++i) avgP(i) /= cellVol(i);
    rst::scale(avgP, dynvisco_);
    ...
    for (int i = 0; i < c; ++i) {
      fileVel << std::scientific << std::setprecision(15);
      fileVel << std::setw(25) << avgX(i);
      fileVel << std::setw(25) << avgY(i);
      fileVel << std::setw(25) << avgZ(i);
      fileVel << std::endl;
    }
    fileVel.close();
    // Print to permeability file
  ...
  const std::vector<ROL::Ptr<fe_type>> getPressureBdryFE(const int sideset = -1) const {
    int side = (sideset < 0 || sideset > 3) ? 0 : sideset;

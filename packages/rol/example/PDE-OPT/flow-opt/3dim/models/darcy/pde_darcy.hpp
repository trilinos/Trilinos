...
public:
  PDE_Darcy(Teuchos::ParameterList &parlist) : Patm_(101.325 /* kg/mm-s^2 */) {
    ...
    if (elemType == "TET") {
      if (basisDegPres == 2) {
        basisPtrPrs_ = ROL::makePtr<Intrepid::Basis_HGRAD_TET_C2_FEM<Real, Intrepid::FieldContainer<Real>>>();
      }
      else {
        std::cout << "\nFirst-order TET basis selected.\n";
        basisPtrPrs_ = ROL::makePtr<Intrepid::Basis_HGRAD_TET_C1_FEM<Real, Intrepid::FieldContainer<Real>>>();
      }
      if (basisDegCtrl == 1) {
        basisPtrCtrl_ = ROL::makePtr<Intrepid::Basis_HGRAD_TET_C1_FEM<Real, Intrepid::FieldContainer<Real>>>();
      }
      else {
      ...
      if (basisDegPres == 2) {
        basisPtrPrs_ = ROL::makePtr<Intrepid::Basis_HGRAD_HEX_C2_FEM<Real, Intrepid::FieldContainer<Real>>>();
      }
      else {
  ...
  void printCellAverages(std::string tag,
    ...
    const int d = fePrs_->gradN()->dimension(3);
    // Evaluate/interpolate finite element fields on cells.
    ROL::Ptr<Intrepid::FieldContainer<Real>> gradP, AgradP, velX, velY, velZ, valCtrl, alpha;
    ROL::Ptr<Intrepid::FieldContainer<Real>> onesVec, cellVol, avgX, avgY, avgZ, avgP;
    gradP   = ROL::makePtr<Intrepid::FieldContainer<Real>>(c, p, d);
    AgradP  = ROL::makePtr<Intrepid::FieldContainer<Real>>(c, p, d);
    velX    = ROL::makePtr<Intrepid::FieldContainer<Real>>(c, p);
    velY    = ROL::makePtr<Intrepid::FieldContainer<Real>>(c, p);
    velZ    = ROL::makePtr<Intrepid::FieldContainer<Real>>(c, p);
    valCtrl = ROL::makePtr<Intrepid::FieldContainer<Real>>(c, p);
    alpha   = ROL::makePtr<Intrepid::FieldContainer<Real>>(c, p);
    onesVec = ROL::makePtr<Intrepid::FieldContainer<Real>>(c, p);
    cellVol = ROL::makePtr<Intrepid::FieldContainer<Real>>(c);
    avgX    = ROL::makePtr<Intrepid::FieldContainer<Real>>(c);
    avgY    = ROL::makePtr<Intrepid::FieldContainer<Real>>(c);
    avgZ    = ROL::makePtr<Intrepid::FieldContainer<Real>>(c);
    avgP    = ROL::makePtr<Intrepid::FieldContainer<Real>>(c);
    fePrs_->evaluateGradient(gradP, u_coeff);
    feCtrl_->evaluateValue(valCtrl, z_coeff);
    onesVec->initialize(1.0);
    perm_->compute(alpha, valCtrl, fePrs_->cubPts(), 0);
    Intrepid::FunctionSpaceTools::scalarMultiplyDataData<Real>(*AgradP,*alpha,*gradP);
    ...
    Intrepid::FunctionSpaceTools::integrate<Real>(*cellVol,
                                                  *onesVec,   // 1
                                                  *onesVec,   // 1
                                                  Intrepid::COMP_CPP,
                                                  false);
    // Compute velocity X cell average.
    Intrepid::FunctionSpaceTools::integrate<Real>(*avgX,
                                                  *velX,      // scalar data
                                                  *onesVec,   // 1
                                                  Intrepid::COMP_CPP,
                                                  false);
    ...
    Intrepid::FunctionSpaceTools::integrate<Real>(*avgY,
                                                  *velY,      // scalar data
                                                  *onesVec,   // 1
                                                  Intrepid::COMP_CPP,
                                                  false);
    ...
    Intrepid::FunctionSpaceTools::integrate<Real>(*avgZ,
                                                  *velZ,      // scalar data
                                                  *onesVec,   // 1
                                                  Intrepid::COMP_CPP,
                                                  false);
    ...
    Intrepid::FunctionSpaceTools::integrate<Real>(*avgP,
                                                  *alpha,     // scalar data
                                                  *onesVec,   // 1
                                                  Intrepid::COMP_CPP,
                                                  false);
    for (int i = 0; i < c; ++i) {(*avgP)(i) /= (*cellVol)(i);}
    Intrepid::RealSpaceTools<Real>::scale(*avgP, dynvisco_);

...
private:
  ...
  ROL::Ptr<IntegralObjective<Real,DeviceType>> obj_;
...
public:
  IntegralConstraint(const ROL::Ptr<QoI<Real,DeviceType>> &qoi,
    ...
    obj_ = ROL::makePtr<IntegralObjective<Real,DeviceType>>(qoi,assembler);
  }
...
private:
  const ROL::Ptr<QoI<Real,DeviceType>> qoi_;
  const ROL::Ptr<Assembler<Real,DeviceType>> assembler_;
  ROL::Ptr<IntegralOptObjective<Real,DeviceType>> obj_;
  ROL::Ptr<ROL::Vector<Real>> dualZVector_;
  bool isZvecInitialized_;

public:
  IntegralOptConstraint(const ROL::Ptr<QoI<Real,DeviceType>> &qoi,
                        const ROL::Ptr<Assembler<Real,DeviceType>> &assembler)
    : qoi_(qoi), assembler_(assembler), isZvecInitialized_(false) {
    obj_ = ROL::makePtr<IntegralOptObjective<Real,DeviceType>>(qoi,assembler);
  }

...
#ifndef ROL_PDEOPT_PDECONSTRAINTK_H
#define ROL_PDEOPT_PDECONSTRAINTK_H
...
public:
  PDE_Constraint(const ROL::Ptr<PDE<Real,DeviceType>> &pde,
    ...
    assembler_ = ROL::makePtr<Assembler<Real,DeviceType>>(pde_->getFields(),pde_->getFields2(),meshMgr,comm,parlist,outStream);
  ...
  const ROL::Ptr<PDE<Real,DeviceType>> getPDE(void) const {
    return pde_;
  }

...
private:
  ...
  std::vector<Real> loadMagnitude_, loadPolar_, loadAzimuth_;
...
public:
  ...
  virtual Real evaluate(int dir, int sideset, const std::vector<Real> &param) const {
    const int paramSize = param.size();
    Real val(0), loadMagNoise(0), loadAngNoise0(0);

...
private:
  ...
  std::vector<scalar_view> BMat_, BdetJMat_, NMat_, NdetJMat_, CBdetJMat_;
...
public:
  ...
  void computeDensity(scalar_view & rho, const scalar_view Z, int deriv = 0) const {
    // Retrieve dimensions.
  ...
  const scalar_view B(int i) const {
    return BMat_[i];
  }

  const scalar_view BdetJ(int i) const {
    return BdetJMat_[i];
  }

  const scalar_view N(int i) const {
    return NMat_[i];
  }

  const scalar_view NdetJ(int i) const{
    return NdetJMat_[i];
  }

  const scalar_view CBdetJ(int i) const {
    return CBdetJMat_[i];
  }

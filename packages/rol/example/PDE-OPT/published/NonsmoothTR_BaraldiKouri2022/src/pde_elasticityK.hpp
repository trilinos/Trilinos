...
#ifndef PDE_TOPO_OPT_ELASTICITYK_HPP
#define PDE_TOPO_OPT_ELASTICITYK_HPP
...
public:
  PDE_Elasticity(ROL::ParameterList &parlist) : getFields2called_(false) {
    ...
    basisPtrs_.clear(); basisPtrs_.resize(probDim,basisPtr_);  // Displacement component
    ...
    traction_  = ROL::makePtr<Traction<Real,DeviceType>>(probDim);
    dirichlet_ = ROL::makePtr<Dirichlet<Real,DeviceType>>();
  ...
  void Jacobian_2(scalar_view &jac,
    ...
    std::vector<std::vector<scalar_view>> J(d);
    for (int i=0; i<d; ++i)
      J[i].push_back(scalar_view("jac",c,f,fd));
  ...
  void Hessian_12(scalar_view &hess,
    ...
    for (int i=0; i<d; ++i)
      J[0].push_back(scalar_view("jac",c,fd,f));
  ...
  void Hessian_22(scalar_view &hess,
    ...
    // Initialize Hessians.
    std::vector<std::vector<scalar_view>> J(1);
    J[0].push_back(scalar_view("jac",c,fd,fd));

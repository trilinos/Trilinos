...
#ifndef TOPOPT_FILTERED_COMPLIANCE_ROBJK_H
#define TOPOPT_FILTERED_COMPLIANCE_ROBJK_H
...
private:
  ...
  ROL::Ptr<TopOptComplianceObjective<Real,DeviceType>> comp_;

  // FIlter PDE
  const ROL::Ptr<PDE<Real,DeviceType>> pde_filter_;
  ROL::Ptr<Assembler<Real,DeviceType>> assembler_filter_;
...
public:
  TopOptFilteredComplianceObjective(
      ...
      const ROL::Ptr<PDE<Real,DeviceType>>         &filter,
      const ROL::Ptr<MeshManager<Real,DeviceType>> &mesh,
    ...
    : pde_filter_(filter),
      ...
      nasFi_(0), napFi_(0) {
    // Compliance objective function
    comp_ = ROL::makePtr<TopOptComplianceObjective<Real,DeviceType>>(pde,mesh,comm,list,stream,"state","filtered_density");

    // Filter PDE
    assembler_filter_ = ROL::makePtr<Assembler<Real,DeviceType>>(pde_filter_->getFields(),
    ...
    Fz_      = ROL::makePtr<PDE_PrimalSimVector<Real,DeviceType>>(assembler_filter_->createStateVector(),pde_filter_,assembler_filter_,list);
    Fzcache_ = ROL::makePtr<PDE_PrimalSimVector<Real,DeviceType>>(assembler_filter_->createStateVector(),pde_filter_,assembler_filter_,list);
    Fv_      = ROL::makePtr<PDE_PrimalSimVector<Real,DeviceType>>(assembler_filter_->createStateVector(),pde_filter_,assembler_filter_,list);
    dctrl_   = ROL::makePtr<PDE_DualSimVector<Real,DeviceType>>(assembler_filter_->createStateVector(),pde_filter_,assembler_filter_,list);
  ...
  const ROL::Ptr<Assembler<Real,DeviceType>> getFilterAssembler(void) const {

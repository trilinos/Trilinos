  ...
  void computeDensity(scalar_view & rho,
    ...
    for (int i=0; i<c; ++i) {
      for (int j=0; j<p; ++j) {
        ...
        if (!usePhaseField_) {
          if (!useProjection_) {
            if (deriv==0)      rho(i,j) = minDensity_ + diff*std::pow(z, powerSIMP_);
            else if (deriv==1) rho(i,j) = powerSIMP_*diff*std::pow(z, powerSIMP_-one);
            else if (deriv==2) rho(i,j) = powerSIMP_*(powerSIMP_-one)*diff*std::pow(z, powerSIMP_-two);
          }
          else {
            if (deriv==0) {
              Real pz = project(z,0);
          ...
          z += one;
          z *= half;
          if (deriv==0)      rho(i,j) = minDensity_ + diff*std::pow(z, 2);
          else if (deriv==1) rho(i,j) = diff*z;
          else if (deriv==2) rho(i,j) = half*diff;
        }
      }
    }
  }

...
#ifndef TOPOT_COMPLIANCE_ROBJK_H
#define TOPOT_COMPLIANCE_ROBJK_H
...
public:
  TopOptComplianceObjective(
    ...
    // Elasticity PDE
    assembler_ = ROL::makePtr<Assembler<Real,DeviceType>>(pde_->getFields(),
                                               pde_->getFields2(),
    ...
    state_            = ROL::makePtr<PDE_PrimalSimVector<Real,DeviceType>>(assembler_->createStateVector(),pde_,assembler_,list);
    dctrl_            = ROL::makePtr<PDE_DualOptVector<Real,DeviceType>>(dctrl_data_,pde_,assembler_,list);
    ...
    if (hessAppr_ == "Secant") {
      ...
      s_      = ROL::makePtr<PDE_PrimalOptVector<Real,DeviceType>>(s_data_,pde_,assembler_,list);
      g_      = ROL::makePtr<PDE_DualOptVector<Real,DeviceType>>(assembler_->createControlVector(),pde_,assembler_,list);
      g_old_  = ROL::makePtr<PDE_DualOptVector<Real,DeviceType>>(assembler_->createControlVector(),pde_,assembler_,list);
      g_new_  = ROL::makePtr<PDE_DualOptVector<Real,DeviceType>>(assembler_->createControlVector(),pde_,assembler_,list);
  ...
  void gradient( ROL::Vector<Real> &g, const ROL::Vector<Real> &z, Real &tol ) {
    ngrad_++;
    std::cout << "Gradient: " << tol << std::endl;
    if (hessAppr_ == "Secant") {
  ...
  void hessVec( ROL::Vector<Real> &hv, const ROL::Vector<Real> &v, const ROL::Vector<Real> &z, Real &tol ) {
    ...
    if (hessAppr_ == "Zero") {
      ...
      ROL::Ptr<Tpetra::MultiVector<>>       u_data = getField(*state_);
      // Solve state equation
      std::cout << "HessVec 1: " << tol_ << std::endl;
      solve_state_equation(*state_,z,tol_);
      // Solve state sensitivity equation
      assembleJ2(u_data, z_data);
      std::cout << "HessVec 2: " << tol_ << std::endl;
      solve_state_sensitivity(state_sens_data_, v_data, tol_);
      // Apply density hessian to direction
      assembleH22(u_data, z_data);
      applyHessian22(h_data, v_data);
...
private:
  ...
  void solve_state_equation(ROL::Vector<Real> &u, const ROL::Vector<Real> &z, Real &tol) {
    ...
    std::cout << "Tolerance for state: " << mytol << std::endl;

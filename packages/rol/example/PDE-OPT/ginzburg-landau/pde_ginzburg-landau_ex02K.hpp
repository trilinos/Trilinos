...
#ifndef PDE_GINZBURGLANDAU_EX02K_HPP
#define PDE_GINZBURGLANDAU_EX02K_HPP

#include "pde_ginzburg-landauK.hpp"


template <class Real, class DeviceType>
class PDE_GinzburgLandau_ex02 : public PDE_GinzburgLandau<Real,DeviceType> {
public:
  PDE_GinzburgLandau_ex02(ROL::ParameterList &parlist) : PDE_GinzburgLandau<Real,DeviceType>(parlist) {}
  ...
  Real evaluateNeumann(const std::vector<Real> &x, int component) const override {
    return static_cast<Real>(0);
  }

  Real evaluateForce(const std::vector<Real> &x, int component) const override {

...
#include "../src/pde_helmholtzK.hpp"
#include "../src/obj_helmholtzK.hpp"
...
//  feenableexcept(FE_DIVBYZERO | FE_INVALID | FE_OVERFLOW);
  ...
  auto outStream = ROL::makeStreamPtr( std::cout, iprint );
  ...
  try {
    ...
    /*************************************************************************/
    /*** Read in XML input ***/
    auto parlist = ROL::getParametersFromXmlFile("input.xml");
    int   numSpeakers = 8;
    ...
    for (int i = 0; i < 5; ++i) {
      ...
      pde = ROL::makePtr<PDE_Helmholtz_OCT<RealT,DeviceT>>(*parlist);
      cons[i] = ROL::makePtr<Linear_PDE_Constraint<RealT,DeviceT>>(pde,meshMgr,dcomm,*parlist,*outStream);
    }
    auto assembler = cons[0]->getAssembler();
    ...
    auto obs = ROL::makePtr<PDE_Objective<RealT,DeviceT>>(qoi,assembler);
    ...
    auto model = ROL::makePtr<Model<RealT>>(objs);

    // Build prediction function
    auto vdata = ROL::makePtr<std::vector<RealT>>(5);
    ...
    auto vec = ROL::makePtr<ROL::StdVector<RealT>>(vdata);
    auto predfun = ROL::makePtr<PredFun<RealT>>(model,vec);
    ...
    *outStream << std::endl;
    auto dbman  = ROL::makePtr<ROL::StdTeuchosBatchManager<RealT,int>>(dcomm);
    auto obman  = ROL::makePtr<ROL::StdTeuchosBatchManager<RealT,int>>(ocomm);
    auto dsampler  = ROL::makePtr<ROL::UserInputGenerator<RealT>>(pd,wd,nsampd,2,dbman);
    auto osampler  = ROL::makePtr<ROL::UserInputGenerator<RealT>>(po,wo,nsampo,2,obman);

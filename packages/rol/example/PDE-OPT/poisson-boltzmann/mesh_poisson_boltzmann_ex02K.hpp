...
class MeshManager_Example02 : public MeshManager_Rectangle<Real, DeviceType> {
...
public:
  ...
  MeshManager_Example02(Teuchos::ParameterList &parlist) : MeshManager_Rectangle<Real, DeviceType>(parlist)
  {
  ...
  ROL::Ptr<std::vector<std::vector<std::vector<int>>>> getSideSets(
    ...
    if ( verbose ) {
      outStream << "Mesh_Example02: getSideSets called" << std::endl;
      outStream << "Mesh_Example02: numSideSets = " << meshSideSets_->size() << std::endl;
    }
    return meshSideSets_;
  }

}; // MeshManager_Example02

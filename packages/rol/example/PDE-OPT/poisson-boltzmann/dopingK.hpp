...
private:
  ...
  scalar_view cellNodes_;
  int_view cellDofs_;
  const Teuchos::Array<GO> cellIds_;
  Real a_, b_, wx_, wy_;

public:
  Doping(const ROL::Ptr<fe_type> & fe,
         const scalar_view cellNodes,
         const int_view cellDofs,
  ...
  void build(const ROL::Ptr<Tpetra::MultiVector<>> &dpVec) const {
    ...
    scalar_view dofPoints("dofPoints", c ,f, d);
...
public:
  DopingBounds(const ROL::Ptr<fe_type> &fe,
               const scalar_view cellNodes,
               const int_view cellDofs,
               const Teuchos::Array<GO> & cellIds,
               Teuchos::ParameterList &parlist)
  ...
  void build(const ROL::Ptr<Tpetra::MultiVector<>> & loVec,
    ...
    int f = fe_->gradN().extent_int(1);
    int d = fe_->gradN().extent_int(3);
    scalar_view dofPoints("dofPoint", c, f, d);
    fe_->computeDofCoords(dofPoints, cellNodes_);

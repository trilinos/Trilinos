...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    auto dy_ptr = assembler->createStateVector();     dy_ptr->randomize();
    ...
    auto dyp = ROL::makePtr<PDE_PrimalSimVector<RealT,DeviceT>>(dy_ptr,pde,assembler);
    ...
    std::vector<RealT> lambda = ROL::getArrayFromStringParameter<RealT>(parlist->sublist("Problem"),"Convex Combination Parameters");
    ...
    if ( lambdaZero ) {
      ...
      opt = ROL::makePtr<ROL::StochasticProblem<RealT>>(robj,zp);

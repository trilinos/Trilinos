...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    auto yzbc_ptr = assembler->createControlVector();
    ...
    auto yzp_ptr = ROL::makePtr<std::vector<RealT>>(controlDim);
    ...
    auto dzparam = ROL::makePtr<ROL::StdVector<RealT>>(dzp_ptr);
    auto yzparam = ROL::makePtr<ROL::StdVector<RealT>>(yzp_ptr);
    // Create OptVectors
    auto  zp = ROL::makePtr<PDE_OptVector<RealT>>(zbc, zparam);
    auto dzp = ROL::makePtr<PDE_OptVector<RealT>>(dzbc, dzparam);
    auto yzp = ROL::makePtr<PDE_OptVector<RealT>>(yzbc, yzparam);

...
template <class Real>
class MultiMat_Filtered_Compliance_Objective : public ROL::Objective<Real> {
private:
  // Compliance objective function
  ROL::Ptr<MultiMat_Compliance_Objective<Real>> comp_;
...
public:
  MultiMat_Filtered_Compliance_Objective(
    ...
    : pde_filter_(filter),
      ...
      nasFi_(0), napFi_(0) {
    // Compliance objective function
    comp_ = ROL::makePtr<MultiMat_Compliance_Objective<Real>>(pde,mesh,comm,list,stream,"state","filtered_density");

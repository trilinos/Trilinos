...
public:
  ...
  Traction(ROL::ParameterList & parlist, std::string ex = "Default") {
    if (ex == "2D Wheel") {
      dim_ = 2;
      offset_ = 0;
      ...
      if (parlist.isSublist("Traction")) {
        ...
        if (parlist.isSublist("Load")) {
          std::vector<Real> loadMag
            = ROL::getArrayFromStringParameter<double>(parlist.sublist("Load"), "Magnitude");
          offset_ = dim_*static_cast<int>(loadMag.size());
        }

        // Grab Sidesets
        sidesets_ = ROL::getArrayFromStringParameter<int>(parlist.sublist("Traction"), "Sidesets");

        // Grab Side IDs
        sideids_ = ROL::getArrayFromStringParameter<int>(parlist.sublist("Traction"), "Side IDs");

        // Grab Magnitudes
        loadMagnitude_ = ROL::getArrayFromStringParameter<double>(parlist.sublist("Traction"), "Magnitude");

        // Grab Polar Angle
        loadPolar_ = ROL::getArrayFromStringParameter<double>(parlist.sublist("Traction"), "Polar Angle");

        if (dim_ == 3) {
          // Grab Azimuth Angle
          loadAzimuth_ = ROL::getArrayFromStringParameter<double>(parlist.sublist("Traction"), "Azimuth Angle");
        }
      }
    }
...
//    const int numSideSets = sidesets_.size();
  ...
  void compute(std::vector<std::vector<std::vector<scalar_view>>> &traction,
               const std::vector<std::vector<ROL::Ptr<FE<Real,DeviceType>>>> &fe,
          ...
          if (numCellsSide > 0) {
            ...
            for (int k = 0; k < dim_; ++k) {
              traction[sidesets_[i]][j][k]
                = scalar_view("traction", numCellsSide, numCubPerSide);

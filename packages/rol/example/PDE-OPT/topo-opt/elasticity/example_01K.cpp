...
#include "src/mesh_topo-optK.hpp"
...
#include "../src/volume_objK.hpp"
#include "../src/obj_volumeK.hpp"
#include "../src/obj_phasefieldK.hpp"
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (probDim == 2) meshMgr = ROL::makePtr<MeshManager_TopoOpt<RealT,DeviceT>>(*parlist);
    else              meshMgr = ROL::makePtr<MeshReader<RealT,DeviceT>>(*parlist);
    ...
    if (useFilter && !usePhaseField) {
      ...
      robj_com = ROL::makePtr<TopOptFilteredComplianceObjective<RealT,DeviceT>>(
                 pde,pdeFilter,meshMgr,comm,*parlist,*outStream);
      assembler = ROL::dynamicPtrCast<TopOptFilteredComplianceObjective<RealT,DeviceT>>(robj_com)->getAssembler();
      assemblerFilter = ROL::dynamicPtrCast<TopOptFilteredComplianceObjective<RealT,DeviceT>>(robj_com)->getFilterAssembler();
    }
    else {
      robj_com = ROL::makePtr<TopOptComplianceObjective<RealT,DeviceT>>(
                 pde,meshMgr,comm,*parlist,*outStream);
      assembler = ROL::dynamicPtrCast<TopOptComplianceObjective<RealT,DeviceT>>(robj_com)->getAssembler();
    ...
    if (minType == "Compliance" && volEq) {
      if (useFilter && !usePhaseField)
        qoi_vol = ROL::makePtr<QoI_Volume_TopoOpt<RealT,DeviceT>>(
                    ROL::dynamicPtrCast<PDE_Filter<RealT,DeviceT>>(pdeFilter)->getDensityFE(),volFraction);
      else
        qoi_vol = ROL::makePtr<QoI_Volume_TopoOpt<RealT,DeviceT>>(pde->getDensityFE(),volFraction);
    }
    else {
      if (useFilter && !usePhaseField)
        qoi_vol = ROL::makePtr<QoI_Volume_TopoOpt<RealT,DeviceT>>(ROL::dynamicPtrCast<PDE_Filter<RealT,DeviceT>>(pdeFilter)->getDensityFE());
      else
        qoi_vol = ROL::makePtr<QoI_Volume_TopoOpt<RealT,DeviceT>>(pde->getDensityFE());
    }
    auto obj_vol = ROL::makePtr<TopOptVolumeObjective<RealT,DeviceT>>(qoi_vol,assemblerFilter,zp);
    ...
    if (normalizeObj) {
      if (useFilter && !usePhaseField)
        cs = ROL::dynamicPtrCast<TopOptFilteredComplianceObjective<RealT,DeviceT>>(robj_com)->normalize(*zp,tol);
      else
        cs = ROL::dynamicPtrCast<TopOptComplianceObjective<RealT,DeviceT>>(robj_com)->normalize(*zp,tol);
    }
    ...
    *outStream << std::endl;
    ...
    auto prob = ROL::makePtr<ROL::Problem<RealT>>(obj,zp);
    ...
    *outStream << "Total optimization time = " << algoTimer.totalElapsedTime() << " seconds.\n";
    // Output.
    if (useFilter && !usePhaseField) {
      ROL::dynamicPtrCast<TopOptFilteredComplianceObjective<RealT,DeviceT>>(robj_com)->summarize(*outStream);
      ROL::dynamicPtrCast<TopOptFilteredComplianceObjective<RealT,DeviceT>>(robj_com)->printToFile(*zp,*outStream);
    }
    else {
      ROL::dynamicPtrCast<TopOptComplianceObjective<RealT,DeviceT>>(robj_com)->summarize(*outStream);
      ROL::dynamicPtrCast<TopOptComplianceObjective<RealT,DeviceT>>(robj_com)->printToFile(*zp,*outStream);
    }
    if (minType == "Compliance")
      con_vol->summarize(*outStream);
    else if (minType == "Volume")
      obj_vol->summarize(*outStream);

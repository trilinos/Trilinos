...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    *outStream << std::endl;
    ...
    auto prob = ROL::makePtr<ROL::StochasticProblem<RealT>>(obj,zp);
    ...
    *outStream << "Total optimization time = " << algoTimer.totalElapsedTime() << " seconds.\n";
    ...
    for (int i = 0; i < sampler->numMySamples(); ++i) {
      ...
      if (useFilter && !usePhaseField)
        ROL::dynamicPtrCast<TopOptFilteredComplianceObjective<RealT,DeviceT>>(robj_com)->solveState(*up,*zp);
      else
        ROL::dynamicPtrCast<TopOptComplianceObjective<RealT,DeviceT>>(robj_com)->solveState(*up,*zp);

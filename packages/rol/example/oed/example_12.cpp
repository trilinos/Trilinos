...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    parlist->sublist("OED").sublist("Binary Penalty Transformation").set("Strength Value",8.0);
    parlist->sublist("OED").sublist("Binary Penalty Transformation").set("Type",2);
    ...
    *outStream << "  " << ocType << "-optimal design time:      "
    ...
    auto x = problem->getPrimalOptimizationVector();
    auto xbin = x->clone();
    xbin->set(*x);
    xbin->applyUnary(rnd);
    xbin->axpy(-1.0,*x);
    *outStream << std::endl;
    *outStream << "  Distance to Binary: " << xbin->norm();
    *outStream << std::endl;

    std::stringstream dname;
    dname << ocType << "_optimal_design_ex12";

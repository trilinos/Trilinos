...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // Setup factory
    unsigned numTheta = parlist->sublist("Problem").get("Number of Theta Samples",10);
    bool     homNoise = true;
    ...
    if (numTheta == 1u) {
      ...
      for (unsigned i = 0u; i < numTheta; ++i) {
        thetas[i] = theta->clone();
        thetas[i]->randomize(static_cast<RealT>(-1),static_cast<RealT>(1));
        weights[i] = static_cast<RealT>(1) / static_cast<RealT>(numTheta);
      }
    }
    ...
    *outStream << "  " << ocType << "-optimal design time:      "
    ...
    dname << ocType << "_optimal_design_ex2";
    ...
    // Generate optimization problem
    problem = factory->get(thetas,weights,*parlist,sampler);
    problem->setProjectionAlgorithm(*parlist);
    problem->finalize(false,true,*outStream);
    test->randomize(1,2);
    problem->check(true,*outStream,test,0.1);
    ...
    *outStream << "  " << ocType << "-optimal design time:      "
    ...
    dname2 << "robust_" << ocType << "_optimal_design_ex2";

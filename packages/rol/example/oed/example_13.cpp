...
#include "ROL_OED_BinaryConstraint.hpp"
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    factory->setBudgetConstraint(cost,budget,true);
    ...
    // Finalize fractional optimization problem
    ...
    auto obj  = problem1->getObjective();
    auto x    = problem1->getPrimalOptimizationVector();
    auto proj = problem1->getPolyhedralProjection();
    auto lcon = proj->getLinearConstraint();
    auto lmul = proj->getMultiplier();
    auto lres = proj->getResidual();
    ROL::Ptr<ROL::Constraint<RealT>> bincon = ROL::makePtr<ROL::OED::BinaryConstraint<RealT>>();
    auto res  = x->clone();
    auto mul  = x->dual().clone();

    // Solve fractional problem
    std::clock_t timer = std::clock();
    auto solver = ROL::makePtr<ROL::Solver<RealT>>(problem0,*parlist);
    ...
    *outStream << "  " << ocType << "-optimal design time:      "
    ...
    // Compute distance to binary
    x->set(*problem0->getPrimalOptimizationVector());
    auto xbin = x->clone();
    xbin->set(*x);
    xbin->applyUnary(ROL::Elementwise::Round<RealT>());
    xbin->axpy(-1.0,*x);
    ...
    *outStream << "  Partitioned Constraint Residual: " << pvec->norm() << std::endl;
    ...
    auto test = factory->createDesignVector();
    test->randomize(0,1);
    ...
    *outStream << "  " << ocType << "-optimal design time:      "
    ...
    dname << ocType << "_optimal_design_ex13";

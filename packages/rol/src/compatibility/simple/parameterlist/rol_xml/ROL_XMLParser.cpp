...
#include "ROL_XMLParser.hpp"
#include "ROL_TreeBuildingXMLHandler.hpp"
...
Teuchos::ParameterList ROL::XMLParser::parse()
{

  RCP<ROL::TreeBuildingXMLHandler> handler = rcp(new ROL::TreeBuildingXMLHandler());
  ...
  ROL::XMLParameterListReader pl2xml;

  return (pl2xml.toParameterList(handler->getObject()));

}


void ROL::XMLParser::getETag(std::string &tag)
{
...
void ROL::XMLParser::getSTag(unsigned char lookahead, std::string &tag, std::map<std::string,std::string> &attrs, bool &emptytag)
{
...
void ROL::XMLParser::getComment(long startLine)
{
  /* Recall from the specification:
        Comment   ::= '<!--' ((Char - '-') | ('-' (Char - '-')))* '-->'
                      that is, '<!--' txt '-->', where txt does not contain '--'
...
void ROL::XMLParser::getReference(std::string &refstr) {
...
int ROL::XMLParser::getSpace(unsigned char &lookahead) {
  // if space, consume the whitespace
  do {
    if (lookahead == '\n') ++_lineNo;
...
bool ROL::XMLParser::isLetter(unsigned char c) {
...
bool ROL::XMLParser::isNameChar(unsigned char c) {
...
bool ROL::XMLParser::isSpace(unsigned char c) {
...
bool ROL::XMLParser::isChar(unsigned char c) {
...
int ROL::XMLParser::assertChar(unsigned char cexp)
{
...
void ROL::XMLParser::ignoreXMLDeclaration()
{

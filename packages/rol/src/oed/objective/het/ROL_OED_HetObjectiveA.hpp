...
#ifndef ROL_OED_HETOBJECTIVEA_HPP
#define ROL_OED_HETOBJECTIVEA_HPP
...
private:
  ...
  const Ptr<VectorController<Real,unsigned>> stateStore_, MstateStore_, adjointStore_;
  const Ptr<Vector<Real>> u_, Mu_, p_, r1_, r2_, s_, q_;
  const bool storage_;
  const unsigned dim_;
  Ptr<Vector<Real>> g_;

  void solveState(Vector<Real>& u, Vector<Real>& Mu, const Vector<Real>& z, unsigned i);
  void solveAdjoint(Vector<Real>& p, const Vector<Real>& Mu, const Vector<Real>& z, unsigned i);

public:
  ObjectiveA( const Ptr<MomentOperator<Real>>& M0,
...
#include "ROL_OED_HetObjectiveA_Def.hpp"

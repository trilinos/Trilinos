...
#ifndef ROL_OED_HETOBJECTIVEIBASIC_DEF_HPP
#define ROL_OED_HETOBJECTIVEIBASIC_DEF_HPP

namespace ROL::OED::Het {

template<typename Real>
ObjectiveIbasic<Real>::ObjectiveIbasic( const Ptr<MomentOperator<Real>>& M1,
                                        const Ptr<MomentOperator<Real>>& M2,
                                        const Ptr<Factors<Real>>& F,
                                        const Ptr<SampleGenerator<Real>>& sampler,
                                        bool storage)
  : obj_(makePtr<ObjectivePV<Real>>(M1,M2,F,storage)),
    sampler_(sampler) {}

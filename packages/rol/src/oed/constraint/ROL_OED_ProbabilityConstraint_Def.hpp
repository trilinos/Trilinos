...
void ProbabilityConstraint<Real>::value(Vector<Real> &c,
  ...
  static_cast<ProbabilityConstraintRangeVector<Real>&>(c).setValue(gval-static_cast<Real>(1));
...
void ProbabilityConstraint<Real>::applyJacobian(Vector<Real> &jv,
  ...
  sumAll(&mval,&gval,1,x);
  //jdata[0] = gval;
  static_cast<ProbabilityConstraintRangeVector<Real>&>(jv).setValue(gval);
...
void ProbabilityConstraint<Real>::applyAdjointJacobian(Vector<Real> &ajv,
                          ...
                          Real &tol) {
  //const std::vector<Real> &vdata = getConstData(v);
  ajv.setScalar(static_cast<const ProbabilityConstraintRangeVector<Real>&>(v).getValue());

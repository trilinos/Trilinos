...
int main(int argc, char **argv)
{
  ...
  for (i = 0; i < num_nodes; i += 11) {
    error = ex_put_partial_coord(exoid, i + 1, 11, &x[i], &y[i], &z[i]);
  ...
  for (i = 0; i < num_nodes; i += 11) {
    error = ex_put_partial_one_attr(exoid, EX_NODAL, 0, i + 1, 11, 1, &x[i]);
    if (error) {
      ex_close(exoid);
      exit(-1);
    }

    error = ex_put_partial_one_attr(exoid, EX_NODAL, 0, i + 1, 11, 2, &y[i]);
    if (error) {
      ex_close(exoid);
      exit(-1);
    }
  ...
  for (i = 0; i < num_time_steps; i++) {
    ...
    for (k = 1; k <= num_nod_vars; k++) {
      ...
      error = ex_put_var(exoid, whole_time_step, EX_NODAL, k, 0, num_nodes, nodal_var_vals);

...
int ex_put_map_param(int exoid, int num_node_maps, int num_elem_maps)
{
  ...
  if ((num_node_maps > 0 && ((nc_inq_dimid(exoid, DIM_NUM_NM, &dimid)) == EX_NOERR)) ||
      (num_elem_maps > 0 && ((nc_inq_dimid(exoid, DIM_NUM_EM, &dimid)) == EX_NOERR))) {
  ...
  if ((num_node_maps > 0) || (num_elem_maps > 0)) {
    ...
    if (num_node_maps > 0) {
      ...
      if ((status = nc_def_dim(exoid, DIM_NUM_NM, num_node_maps, &dimid)) != EX_NOERR) {
      ...
      /* node maps id array: */
      dim[0] = dimid;
      if ((status = nc_def_var(exoid, VAR_NM_PROP(1), id_type, 1, dim, &var_nm_id)) != EX_NOERR) {
        snprintf(errmsg, MAX_ERR_LENGTH,
      ...
      if ((status = nc_put_att_text(exoid, var_nm_id, ATT_PROP_NAME, 3, "ID")) != EX_NOERR) {
        snprintf(errmsg, MAX_ERR_LENGTH,
      ...
      if (nc_def_var(exoid, VAR_NAME_NM, NC_CHAR, 2, dim, &varid) != EX_NOERR) {
        snprintf(errmsg, MAX_ERR_LENGTH,
      ...
      if ((status = nc_inq_dimid(exoid, DIM_NUM_NODES, &dimid)) != EX_NOERR) {
    ...
    /* element maps: */
    if (num_elem_maps > 0) {
      if ((status = nc_def_dim(exoid, DIM_NUM_EM, num_elem_maps, &dimid)) != EX_NOERR) {
        snprintf(errmsg, MAX_ERR_LENGTH,
      ...
      dim[0] = dimid;
      if ((status = nc_def_var(exoid, VAR_EM_PROP(1), id_type, 1, dim, &var_em_id)) != EX_NOERR) {
        snprintf(errmsg, MAX_ERR_LENGTH,
      ...
      /*   store property name as attribute of property array variable */
      if ((status = nc_put_att_text(exoid, var_em_id, ATT_PROP_NAME, 3, "ID")) != EX_NOERR) {
        snprintf(errmsg, MAX_ERR_LENGTH,
      ...
      if ((status = nc_def_var(exoid, VAR_NAME_EM, NC_CHAR, 2, dim, &varid)) != EX_NOERR) {
        snprintf(errmsg, MAX_ERR_LENGTH,
      ...
      if ((status = nc_inq_dimid(exoid, DIM_NUM_ELEM, &dimid)) != EX_NOERR) {
        snprintf(errmsg, MAX_ERR_LENGTH,
    ...
    /* Fill the id arrays with EX_INVALID_ID */
    {
      ...
      if (num_node_maps > 0) {
        if ((status = nc_put_var_int(exoid, var_nm_id, invalid_ids)) != EX_NOERR) {
      ...
      if (num_elem_maps > 0) {
        if ((status = nc_put_var_int(exoid, var_em_id, invalid_ids)) != EX_NOERR) {

...
int ex_put_init_info(int exoid, int num_proc, int num_proc_in_f, const char *ftype)
{
  ...
  if (nc_inq_dimid(exoid, DIM_NUM_PROCS, &dimid) != EX_NOERR) {
    ltempsv = num_proc;
    if ((status = nc_def_dim(exoid, DIM_NUM_PROCS, ltempsv, &dimid)) != EX_NOERR) {
  ...
  if (nc_inq_dimid(exoid, DIM_NUM_PROCS_F, &dimid) != EX_NOERR) {
    ltempsv = num_proc_in_f;
    if ((status = nc_def_dim(exoid, DIM_NUM_PROCS_F, ltempsv, &dimid)) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to dimension \"%s\" in file ID %d",
  ...
  if (nc_inq_varid(exoid, VAR_FILE_TYPE, &varid) != EX_NOERR) {
    if ((status = nc_def_var(exoid, VAR_FILE_TYPE, NC_INT, 0, NULL, &varid)) != EX_NOERR) {
    ...
    if ((status = nc_put_var1_int(exoid, varid, NULL, &lftype)) != EX_NOERR) {

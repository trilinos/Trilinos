...
int nei_id_lkup(int exoid, const char *ne_var_name, int64_t *idx, ex_entity_id ne_var_id)
{
  int status;
  int varid;
  if ((status = nc_inq_varid(exoid, ne_var_name, &varid)) != EX_NOERR) {
  ...
  if (idx[1] == -1) {
    ...
    int     dimid[1];
    nc_type var_type;
    if ((status = nc_inq_var(exoid, varid, NULL, &var_type, &ndims, dimid, NULL)) != EX_NOERR) {
      char errmsg[MAX_ERR_LENGTH];
    ...
    size_t length;
    if ((status = nc_inq_dimlen(exoid, dimid[0], &length)) != EX_NOERR) {
      char errmsg[MAX_ERR_LENGTH];
      snprintf(errmsg, MAX_ERR_LENGTH,
...
int exi_get_file_type(int exoid, char *ftype)
{
  ...
  int status;
  int lftype;
  if ((status = nc_get_var1_int(exoid, varid, NULL, &lftype)) != EX_NOERR) {
    char errmsg[MAX_ERR_LENGTH];
...
int exi_put_nemesis_version(int exoid)
{
  ...
  if (nc_get_att_float(exoid, NC_GLOBAL, "nemesis_file_version", &file_ver) != EX_NOERR) {
...
int nei_check_file_version(int exoid)
{
  ...
  if ((status = nc_get_att_float(exoid, NC_GLOBAL, "nemesis_file_version", &file_ver)) != EX_NOERR) {
    snprintf(errmsg, MAX_ERR_LENGTH,
...
int ex_get_idx(int exoid, const char *ne_var_name, int64_t *my_index, int pos)
{
   ...
   */
  if (nc_inq_varid(exoid, ne_var_name, &varid) == EX_NOERR) {

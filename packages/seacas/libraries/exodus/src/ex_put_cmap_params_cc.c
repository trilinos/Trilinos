                          ...
                          const void_int *elem_proc_ptrs)
{
  ...
  if ((status = nc_inq_dimid(exoid, DIM_NUM_PROCS_F, &n_dimid[0])) != EX_NOERR) {
  ...
  if ((status = nc_inq_dimlen(exoid, n_dimid[0], &num_procs_in_file)) != EX_NOERR) {
  ...
  if (nc_inq_dimid(exoid, DIM_NUM_N_CMAPS, &n_dimid[0]) != EX_NOERR) {
    num_n_comm_maps = 0;
  }
  else {
    if ((status = nc_inq_dimlen(exoid, n_dimid[0], &num_n_comm_maps)) != EX_NOERR) {
...
file ID %d",
  ...
  if (num_n_comm_maps > 0) {
    ...
    if ((status = nc_inq_varid(exoid, VAR_N_COMM_INFO_IDX, &n_varid_idx)) != EX_NOERR) {
  ...
  if (nc_inq_dimid(exoid, DIM_NUM_E_CMAPS, &e_dimid[0]) != EX_NOERR) {
    num_e_comm_maps = 0;
  }
  else {
    if ((status = nc_inq_dimlen(exoid, e_dimid[0], &num_e_comm_maps)) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to find length of dimension \"%s\" in \
file ID %d",
  ...
  if (num_e_comm_maps > 0) {
    /* Get the variable ID for the comm map index vector */
    if ((status = nc_inq_varid(exoid, VAR_E_COMM_INFO_IDX, &e_varid_idx)) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to find variable ID for \"%s\" in file ID %d",
  ...
  if (num_n_comm_maps > 0) {
    ...
    if ((status = nc_def_dim(exoid, DIM_NCNT_CMAP, ncnt_cmap, &n_dimid[0])) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH,
    ...
    if ((status = nc_def_var(exoid, VAR_N_COMM_NIDS, bulk_type, 1, n_dimid, &varid)) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to add variable \"%s\" in file ID %d",
    ...
    if ((status = nc_def_var(exoid, VAR_N_COMM_PROC, NC_INT, 1, n_dimid, &varid)) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to add variable \"%s\" in file ID %d",
  ...
  if (num_e_comm_maps > 0) {
    ...
    if ((status = nc_def_dim(exoid, DIM_ECNT_CMAP, ecnt_cmap, &e_dimid[0])) != EX_NOERR) {
    ...
    if ((status = nc_def_var(exoid, VAR_E_COMM_EIDS, bulk_type, 1, e_dimid, &varid)) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to add variable \"%s\" in file ID %d",
    ...
    if ((status = nc_def_var(exoid, VAR_E_COMM_PROC, NC_INT, 1, e_dimid, &varid)) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to add variable \"%s\" in file ID %d",
    ...
    if ((status = nc_def_var(exoid, VAR_E_COMM_SIDS, bulk_type, 1, e_dimid, &varid)) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to add variable \"%s\" in file ID %d",
  ...
  if (num_n_comm_maps > 0) {
    ...
    if ((status = nc_inq_varid(exoid, VAR_N_COMM_STAT, &n_varid[0])) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to find variable ID for \"%s\" in file ID %d",
    ...
    if ((status = nc_inq_varid(exoid, VAR_N_COMM_IDS, &n_varid[1])) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to find variable ID for \"%s\" in file ID %d",
    ...
    for (iproc = 0; iproc < num_procs_in_file; iproc++) {
      ...
      for (int icm = 0; icm < num_icm; icm++) {
        ...
        if ((status = nc_put_var1_int(exoid, n_varid[0], start, &nmstat)) != EX_NOERR) {
  ...
  if (num_e_comm_maps > 0) {
    ...
    if ((status = nc_inq_varid(exoid, VAR_E_COMM_STAT, &e_varid[0])) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to find variable ID for \"%s\" in file ID %d",
    ...
    if ((status = nc_inq_varid(exoid, VAR_E_COMM_IDS, &e_varid[1])) != EX_NOERR) {
      snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: failed to find variable ID for \"%s\" in file ID %d",
    ...
    for (iproc = 0; iproc < num_procs_in_file; iproc++) {
      ...
      for (int icm = 0; icm < num_icm; icm++) {
        ...
        if ((status = nc_put_var1_int(exoid, e_varid[0], start, &nmstat)) != EX_NOERR) {
          snprintf(errmsg, MAX_ERR_LENGTH, "ERROR: unable to output variable in file ID %d", exoid);
          ex_err_fn(exoid, __func__, errmsg, status);
          EX_FUNC_LEAVE(EX_FATAL);
        }

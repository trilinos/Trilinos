...
#define CATCH_INTERNAL_CHECK_UNIQUE_TYPES(...)                                                     \
  static_assert(Catch::is_unique<__VA_ARGS__>,                                                     \
                "Duplicate type detected in declaration of template test case");
#else
#define CATCH_INTERNAL_CHECK_UNIQUE_TYPES(...)                                                     \
  static_assert(Catch::is_unique<__VA_ARGS__>::value,                                              \
                "Duplicate type detected in declaration of template test case");
#endif
      ...
      {                                                                                            \
        ...
        (void)expander{                                                                            \
            (Catch::AutoReg(Catch::makeTestInvoker(&TestName<Types>::test),                        \
                            CATCH_INTERNAL_LINEINFO, #ClassName, Catch::NameAndTags{names, Tags}), \
             0)...}; /* NOLINT */                                                                  \
      }                                                                                            \
    };                                                                                             \

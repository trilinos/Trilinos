...
TEST(Iofx, test_constructor)
{
    ...
    std::vector<double> gold_coordinates{
    ...
    // in exodus file: sideset 1 is elements 1 and 3, sideset 2 is 3 and 2 (however it is reversed in results below)

    std::vector<std::vector<int64_t> > gold_element_ids = { {1, 3}, {2, 3} };
    std::vector<std::vector<int64_t> > gold_side_ids = { {6, 2}, {5, 1} };
    std::vector<std::vector<int64_t> > gold_sideset_conn = {
            {5, 6, 7, 8, 5, 8, 7, 6},
            {5, 8, 7, 6, 5, 6, 7, 8}
    };
    ...
    for (size_t i=0;i<sidesets.size();++i)
    {
        ...
        for (size_t j=0;j<sidesets[i]->block_count();++j)
        {
            ...
            connectivity.insert(connectivity.end(), connectivity_per_block.begin(), connectivity_per_block.end());
        }
        ...
        {
            EXPECT_EQ(gold_sideset_conn[i][j], connectivity[j]) << sidesets[i]->name() << " conn index " << j << " for sideset " << i;
        }
    }
}
...
TEST(Iofx, test_writing_of_file)
{
    ...
    Ioss::NodeBlock *output_node_block = new Ioss::NodeBlock(db_out, NodeBlockName, num_nodes, spatial_dim);
    output_region.add(output_node_block);

    const std::vector<Ioss::ElementBlock*> &input_element_blocks = input_region.get_element_blocks();
    for (size_t blk=0; blk<input_element_blocks.size(); ++blk)
    {
        ...
        Ioss::ElementBlock *output_element_block = new Ioss::ElementBlock(db_out, name, exotype, num_elements);
    ...
    for (size_t i=0;i<nodesets_input.size();++i)
    {
        ...
        Ioss::NodeSet * const nodeset = new Ioss::NodeSet(db_out, nodeset_name, number_nodes_in_nodeset);
    ...
    for (size_t i=0;i<sidesets_input.size();++i)
    {
        ...
        for (size_t k=0;k<side_blocks.size();++k)
        {
            ...
            Ioss::SideBlock * side_block = new Ioss::SideBlock(db_out, topo_name, topo_name,
                    parent_topo_name, side_count);
    ...
    {
        const std::vector<Ioss::SideBlock*> &side_blocks_input = sidesets_input[i]->get_side_blocks();
        const std::vector<Ioss::SideBlock*> &side_blocks_output = sidesets_output[i]->get_side_blocks();

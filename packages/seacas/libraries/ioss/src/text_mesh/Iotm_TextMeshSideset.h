...
namespace Iotm {
  namespace text_mesh {
    ...
    struct SidesetData : public EntityGroupData<SidesetDataType<EntityId>>
    {
      ...
      {
        return m_sidesetSplitter.get_side_block_info();
      }
    ...
    public:
      ...
      // "name=<name>; data=elem_1,side_1,elem_2,side_2,....,elem_n,side_n;
      // split=<block|topology|none>; skin=<{block list}|all>;" Cannot specify skin and data at the
      // same time
    ...
    private:
      ...
      {
        convert_to_lowercase(splitName);

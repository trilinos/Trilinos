...
namespace internal {
  ...
  public:
    ...
    bool operator>=(null_terminating_iterator other) const { return ptr_ >= other.ptr_; }
  ...
  template <typename T> FMT_CONSTEXPR const T *pointer_from(const T *p) { return p; }
  ...
  template <typename T, typename Context> class arg_converter : public function<void>
  {
  private:
  ...
  public:
    ...
    template <typename U> typename std::enable_if<!std::is_integral<U>::value>::type operator()(U)
    {
  ...
  public:
    ...
    template <typename T> typename std::enable_if<!std::is_integral<T>::value>::type operator()(T)
    {
      // No coversion needed for non-integral types.
    }
  };
...
public:
  ...
  /**
    ...
    *buffer* is a reference to the output buffer and *spec* contains format
    ...
    \endrst
   */
  printf_arg_formatter(internal::basic_buffer<char_type> &buffer, format_specs &my_spec,
                       context_type &ctx)
      : base(back_insert_range<internal::basic_buffer<char_type>>(buffer), &my_spec, ctx.locale()),
        context_(ctx)
  {
  }
...
class basic_printf_context :
    ...
    public internal::context_base<OutputIt, basic_printf_context<OutputIt, Char, ArgFormatter>,
                                  Char>
{
public:
...
void basic_printf_context<OutputIt, Char, AF>::parse_flags(format_specs &spec, iterator &it)
{
  for (;;) {
    ...
    case '+': spec.flags |= SIGN_FLAG | PLUS_FLAG; break;
    case '0': spec.fill_ = '0'; break;
    case ' ': spec.flags |= SIGN_FLAG; break;
    case '#': spec.flags |= HASH_FLAG; break;
    default: --it; return;
    }
  }
}
...
void basic_printf_context<OutputIt, Char, AF>::format()
{
  ...
  while (*it) {
    ...
    if (spec.has(HASH_FLAG) && visit_format_arg(internal::is_zero_int(), fmt_arg))
      spec.flags =
          static_cast<uint_least8_t>(spec.flags & (~internal::to_unsigned<int>(HASH_FLAG)));
    if (spec.fill_ == '0') {
    ...
    case 'l':
      ...
      break;
    case 'j': convert_arg<intmax_t>(fmt_arg, *it); break;
    case 'z': convert_arg<std::size_t>(fmt_arg, *it); break;
    case 't': convert_arg<std::ptrdiff_t>(fmt_arg, *it); break;
    case 'L':
      // printf produces garbage when 'L' is omitted for long double, no
      // need to do the same.
      break;
    default: --it; convert_arg<void>(fmt_arg, *it);
    }
    ...
    if (fmt_arg.is_integral()) {
      ...
      switch (spec.type) {
      case 'i':
      case 'u': spec.type = 'd'; break;
      case 'c':
        // TODO: handle wchar_t better?
        visit_format_arg(internal::char_converter<basic_printf_context>(fmt_arg), fmt_arg);
        break;
      }
    }

...
namespace internal {
  ...
  template <typename Format, typename PreparedPartsProvider, typename... Args> class prepared_format
  {
  public:
      ...
      for (auto part_it = parts.begin(); part_it != parts.end(); ++part_it) {
        ...
        case format_part_t::kind::specification: {
          ...
          handle_dynamic_spec<internal::width_checker>(specs.width, specs.width_ref, ctx,
                                                       format_view.begin());
          handle_dynamic_spec<internal::precision_checker>(specs.precision, specs.precision_ref,
                                                           ctx, format_view.begin());
...
// Workaround for old compilers. Compile time parts preparation will not be
// performed with them anyway.
  ...
  template <typename Format, typename PartsContainer> struct compiletime_parts_provider
  {
    compiletime_parts_provider() = delete;
    template <typename Char> FMT_CONSTEXPR compiletime_parts_provider(basic_string_view<Char>) {}
...
FMT_CONSTEXPR auto compile(S format_str) -> internal::prepared_format<
    ...
    internal::compiletime_parts_provider<
        S, typename internal::compiletime_prepared_parts_type_provider<S>::type>,
    Args...>
{
  return format_str;
}
...
format_to_n_result<OutputIt> format_to_n(OutputIt out, size_t n, const CompiledFormat &cf,
                                         const Args &... args)
{
...
{
  return fmt::format_to(internal::counting_iterator<typename CompiledFormat::char_type>(), cf,
                        args...)
      .count();
}

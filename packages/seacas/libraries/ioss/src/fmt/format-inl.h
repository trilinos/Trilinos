...
#define FMT_POWERS_OF_10(factor)                                                                   \
  ...
  FMT_ALWAYS_INLINE digits::result grisu_gen_digits(fp value, uint64_t error, int &exp,
                                                    Handler &handler)
  {
    ...
    auto result =
        handler.on_start(data::powers_of_10_64[exp - 1] << -one.e, value.f / 10, error * 10, exp);
    ...
    do {
      ...
      result             = handler.on_digit(static_cast<char>('0' + digit),
                                data::powers_of_10_64[exp] << -one.e, remainder, error, exp, true);
      if (result != digits::more)
        return result;
    } while (exp > 0);
  ...
  struct fixed_handler
  {
    ...
    digits::result on_digit(char digit, uint64_t divisor, uint64_t remainder, uint64_t error, int,
                            bool integral)
    {
  ...
  struct grisu_shortest_handler
  {
    ...
    digits::result on_digit(char digit, uint64_t divisor, uint64_t remainder, uint64_t error,
  ...
  int format_float(T value, int precision, float_specs specs, buffer<char> &buf)
  {
    ...
    if (precision != -1) {
      ...
      fp   fp_value;
      auto boundaries = specs.binary32 ? fp_value.assign_float_with_boundaries(value)
                                       : fp_value.assign_with_boundaries(value);
      fp_value = normalize(fp_value);
      // Find a cached power of 10 such that multiplying value by it will bring
      // the exponent in the range [min_exp, -32].
      const fp cached_pow =
          get_cached_power(min_exp - (fp_value.e + fp::significand_size), cached_exp10);
      ...
      auto                   result = grisu_gen_digits(fp(boundaries.upper, fp_value.e),
  ...
  FMT_TRY
  {
    ...
    for (;;) {
      ...
      int      result = FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                                  ...
                                  system_message, static_cast<uint32_t>(buf.size()), nullptr);
      if (result != 0) {

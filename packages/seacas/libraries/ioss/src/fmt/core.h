...
#ifndef FMT_OVERRIDE
#if FMT_HAS_FEATURE(cxx_override) || (FMT_GCC_VERSION >= 408 && FMT_HAS_GXX_CXX11) ||              \
    FMT_MSC_VER >= 1900
...
inline basic_string_view<Char> to_string_view(const std::basic_string<Char, Traits, Allocator> &s)
{
...
namespace internal {
  ...
  protected:
    ...
    buffer(T *p = nullptr, std::size_t sz = 0, std::size_t cap = 0) FMT_NOEXCEPT : ptr_(p),
                                                                                   size_(sz),
                                                                                   capacity_(cap)
    {
    }
...
#define FMT_TYPE_CONSTANT(Type, constant)                                                          \
  ...
  private:
    ...
    static void format_custom_arg(const void *arg, basic_parse_context<char_type> &parse_ctx,
                                  Context &ctx)
    {
      Formatter f;
...
FMT_CONSTEXPR auto visit_format_arg(Visitor &&vis, const basic_format_arg<Context> &arg)
    -> decltype(vis(0))
{
  using char_type = typename Context::char_type;
  switch (arg.type_) {
...
using buffer_context =
    basic_format_context<std::back_insert_iterator<internal::buffer<Char>>, Char>;
...
namespace internal {
  ...
  template <typename Char>
  typename buffer_context<Char>::iterator vformat_to(buffer<Char> &          buf,
...
inline std::back_insert_iterator<Container> format_to(std::back_insert_iterator<Container> out,

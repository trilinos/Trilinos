...
CppPipeClip::CppPipeClip(vtkDoubleArray* globalBounds) : CppPipeObject()
{
  this->SetDefaults();
  this->SetGlobalBounds(globalBounds);
}

CppPipeClip::CppPipeClip(const Json::Value& jv,
...
CppPipeClip::~CppPipeClip()
{
  if(this->gb)
    this->gb->Delete();
}

void CppPipeClip::SetDefaults()
{
  ...
  this->defaults["side to keep"] = "positive";
}

void CppPipeClip::SetGlobalBounds(vtkDoubleArray* globalBounds)
{
...
void CppPipeClip::InitializeProxy(vtkSMProxy* proxy)
{
  if( proxy &&
    ...
    vtkSMProperty* planeProperty = sp->GetProperty("ClipFunction");
    ...
    if(this->settings.isMember("side to keep"))
      {
      if(this->settings["side to keep"].asString() == "negative")
      ...
      {
      if(this->defaults["side to keep"].asString() == "negative")
        vtkSMPropertyHelper(sp, "InsideOut").Set(1);
      else
        vtkSMPropertyHelper(sp, "InsideOut").Set(0);
      }

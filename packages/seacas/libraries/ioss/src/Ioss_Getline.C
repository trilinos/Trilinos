...
namespace {
  int         io_gl_init_done = -1;               /* terminal mode flag  */
  int         io_gl_termw     = 80;               /* actual terminal width */
  int         io_gl_scroll    = 27;               /* width of EOL scrolling region */
  int         io_gl_width     = 0;                /* net size available for input */
  int         io_gl_extent    = 0;                /* how far to redraw, 0 means all */
  int         io_gl_overwrite = 0;                /* overwrite mode */
  int         io_gl_pos, io_gl_cnt = 0;           /* position and size of input */
  char        io_gl_killbuf[IO_GL_BUF_SIZE] = ""; /* killed text */
  const char *io_gl_prompt;                       /* to save the prompt string */
  int         io_gl_search_mode = 0;              /* search mode flag */

  void io_gl_init(void);         /* prepare to edit a line */
  void io_gl_cleanup(void);      /* to undo io_gl_init */
  void io_gl_char_init(void);    /* get ready for no echo input */
  ...
  void io_gl_kill(int pos);               /* delete to EOL */
  void io_gl_newline(void);               /* handle \n or \r */
  void io_gl_putc(int c);                 /* write one char to terminal */
  void io_gl_puts(const char *const buf); /* write a line to terminal */
  void io_gl_redraw(void);                /* issue \n and redraw all */
  void io_gl_transpose(void);             /* transpose two chars */
  void io_gl_yank(void);                  /* yank killed text */
  ...
  void io_gl_beep(void);            /* try to play a system beep sound */
...
namespace {
  ...
  void io_gl_puts(const char *const buf)
  {
    if (buf) {
      int len = strlen(buf);
      write(1, buf, len);
...
namespace {
  ...
  void io_gl_del(int loc, int killsave)
  ...
  void io_gl_fixup(const char *prompt, int change, int cursor)

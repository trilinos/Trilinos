    ...
    template <typename U, FMT_ENABLE_IF(std::is_integral<U>::value)> void operator()(U value)
    {
      ...
      if (const_check(sizeof(target_type) <= sizeof(int))) {
        ...
        if (is_signed) {
          ...
          arg_ =
              internal::make_arg<Context>(static_cast<unsigned>(static_cast<unsigned_type>(value)));
        }
      }
      else {
        if (is_signed) {
...
void basic_printf_context<OutputIt, Char>::parse_flags(format_specs &specs, const Char *&it,
                                                       const Char *end)
{
  for (; it != end; ++it) {
    switch (*it) {
...
OutputIt basic_printf_context<OutputIt, Char>::format()
{
  ...
  while (it != end) {
    ...
    case 'z': convert_arg<std::size_t>(arg, t); break;
    ...
    if (arg.is_integral()) {
      ...
      switch (specs.type) {
      case 'i':
      case 'u': specs.type = 'd'; break;
      case 'c': visit_format_arg(internal::char_converter<basic_printf_context>(arg), arg); break;
      }
    }

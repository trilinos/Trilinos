...
public:
  ...
  FMT_CONSTEXPR basic_string_view(FMT_STRING_VIEW<Char> s) FMT_NOEXCEPT : data_(s.data()),
                                                                          size_(s.size())
  {
  }
#endif
...
#ifdef FMT_STRING_VIEW
template <typename Char> inline basic_string_view<Char> to_string_view(FMT_STRING_VIEW<Char> s)
{
  return s;
}
#endif
...
struct convert_to_int : std::integral_constant<bool, !std::is_arithmetic<T>::value &&
...
namespace internal {
  ...
  struct is_string
      : std::integral_constant<
            bool, !std::is_same<dummy_string_view, decltype(to_string_view(declval<S>()))>::value>
  {
  };
...
#define FMT_MAKE_VALUE(TAG, ArgType, ValueType)                                                    \
  template <typename C> FMT_CONSTEXPR init<C, ValueType, TAG> make_value(ArgType val)              \
  {                                                                                                \
...
#define FMT_MAKE_VALUE_SAME(TAG, Type)                                                             \
  ...
  FMT_MAKE_VALUE((sizeof(unsigned long) == sizeof(unsigned) ? uint_type : ulong_long_type),
  ...
  FMT_CONSTEXPR typename std::enable_if<std::is_same<typename C::char_type, Char>::value,
  ...
  template <typename C, typename T>
  inline typename std::enable_if<std::is_enum<T>::value &&
  ...
  inline typename std::enable_if<is_constructible<basic_string_view<Char>, T>::value &&
  ...
  inline typename std::enable_if<!convert_to_int<T, Char>::value && !std::is_same<T, Char>::value &&
                                     ...
                                     !internal::is_string<T>::value,
                                 // Implicit conversion to std::string is not handled here because
                                 // it's unsafe: https://github.com/fmtlib/fmt/issues/729
...
namespace internal {
  ...
  protected:
    ...
    context_base(OutputIt it_out, basic_string_view<char_type> format_str,
                 basic_format_args<Context> ctx_args, locale_ref loc = locale_ref())
...
class basic_format_context
    : public internal::context_base<OutputIt, basic_format_context<OutputIt, Char>, Char>
{
public:
...
namespace internal {
  ...
  vformat_to(internal::basic_buffer<Char> &buf, basic_string_view<Char> format_str,
             basic_format_args<typename buffer_context<Char>::type> args);
} // namespace internal
...
/**
  ...
  **Example**::
  ...
  \endrst
 */
template <typename T> inline internal::named_arg<T, char> arg(string_view name, const T &arg)
{
  return {name, arg};
}

template <typename T> inline internal::named_arg<T, wchar_t> arg(wstring_view name, const T &arg)
{
  return {name, arg};
}

...
C     Select type of desired output. Ten options are
...
c lsw = 0... set all the bits to 0
c       1... set the bit (ixset/mscale,iyset/mscale) to 1
c       2... write raster in epsf format to the postscript file

c number of bits in preview raster per postscript coordinate
      parameter (mscale=2)
c maximum postscript coordinates
      parameter (ixps=792)
      parameter (iyps=612)
c maximum preview raster coordinates
      parameter (ixmax=ixps/mscale)
      parameter (iymax=iyps/mscale)
c maximum 4-bit hex words in x-direction
      parameter (ihmax=ixmax/4)
      common /raster/ lras(0:ihmax,0:iymax)
      character*130 line
C COMPUTER DEPENDENT COMMON VARIABLES AND CONSTANTS.
      include 'vcpstc.blk'
C     mopoly controls polygon fill =0, on ;  =1, off
...
c set a bit
      if(lsw.eq.1) then
         ixs = min(ixmax,max(0,ixset/mscale))
         iys = min(iymax,max(0,iyset/mscale))
         ih = ixs/4
         ibit = ixs-4*ih
         iword = lras(ih,iys)
         ib8 = iword/8
         ...
         iword = iword-ib2*2
         ib1 = iword
         if(ibit.eq.0.and.ib8.eq.0) then
            lras(ih,iys) = lras(ih,iys)+8
         elseif(ibit.eq.1.and.ib4.eq.0) then
            lras(ih,iys) = lras(ih,iys)+4
         elseif(ibit.eq.2.and.ib2.eq.0) then
            lras(ih,iys) = lras(ih,iys)+2
         elseif(ibit.eq.3.and.ib1.eq.0) then
            lras(ih,iys) = lras(ih,iys)+1
         endif
      endif

c write out the raster to the scratch file
      if(lsw.eq.2) then
         open(9,file='cgiout.epsi')
90090    format(a, ' ', 4i5)
         write(9,90090) '%!PS-Adobe-2.0 EPSF-2.0'
         write(9,90090) '%%Creator: SEACAS vdieps $Revision: 1.22 $'
         write(9,90090) '%%BoundingBox:', 0,0,ixps,iyps
         write(9,90090) '%%EndComments'
         write(9,90090) '%%BeginPreview:', ixmax, iymax, 1, iymax+1
         do 90 iy=iymax,0,-1
         write(9,90095) (lras(ih,iy),ih=0,ihmax)
90095    format('% ',130z1)
   90    continue
         write(9,90090) '%%EndPreview'
         write(9,90090) '%%BeginProcSet'
         if (mocolr .eq. 0) then
            write(9,90090) '% This file contains color commands'
            write(9,90090) '300 dict begin'
         end if
         write(9,90090) '/y {/Courier findfont} def '
         write(9,90090) '/x {scalefont setfont} def '
         write(9,90090) '/m {moveto} def /l {lineto} def '
         write(9,90090) '/c {closepath} def /v {save} def '
         write(9,90090) '/r {restore} def '
         write(9,90090) '/f {eofill} def /s {stroke} def '
         write(9,90090) '/w {setlinewidth} def '
         write(9,90090) '/h {setdash} def /t {show} def '
         write(9,90090) '/d {gsave} def /u {grestore} def '
         if (dev(23) .ge. 799.75 .and. dev(23) .le. 799.85) then
            write(9,90090) '/q {exch pop exch dup 2 exp 1 exch sub '
            write(9,90090) 'setgray add /brsum exch def brsum 0 le '
            write(9,90090) '{ 0 setgray } if brsum 2 ge{ 1 setgray }'
            write(9,90090) ' if } def '
         else if (dev(23) .ge. 799.05 .and. dev(23) .le. 799.15) then
            write(9,90090) '/q {exch pop exch dup 2 exp '
            write(9,90090) 'setgray add /brsum exch def brsum 0 le '
            write(9,90090) '{ 0 setgray } if brsum 2 ge{ 1 setgray }'
            write(9,90090) ' if } def '
         else
            write(9,90090) '/q {setrgbcolor} def '
         end if
         write(9,90090) '/o {newpath} def '
         write(9,90090) '/p {showpage} def 1 setlinecap v o '
         write(9,90090) '1 setlinejoin '
c         IF (PGFORM.EQ.0) THEN
c            CALL PSTBUF(4,'/o {')
c            CALL PSTBUF(10,'90 rotate ')
c            CALL PSTI2C(0,4,XCOORD)
c            CALL PSTI2C(INT(YDEVIC+YDEVIC*3./32.),4,YCOORD)
c            COORD = ' '//XCOORD(1:3)//'.'//XCOORD(4:4)//' -'//
c     $           YCOORD(1:3)//'.'//YCOORD(4:4)
c            CALL PSTBUF( 13,COORD)
c            CALL PSTBUF(11,' translate ')
c            CALL PSTBUF(6,'} def ')
c            YPAD = -YPAD
c         ELSE
c            CALL PSTBUF(17,'/o {newpath} def ')
c         ENDIF
         write(9,90090) '%%EndProcSet'
         write(9,90090) '%%EndProlog'
         write (*,*) 'Please Wait, updating Postscript file...'
         rewind koutfl
         do 460 l=1,999999
         read(koutfl,90460,end=461) line
90460    format(a130)
         last = 1
         do 450 i=130,1,-1
         if(line(i:i).ne.' ') then
  ...
  450    continue
  451    continue
         write(9,90461) line(1:last)
90461    format(a)
  460    continue
  461    close(koutfl)
C ...Clear out our dictionary
         if (mocolr .eq. 0) then
            write(9,90090) 'end'
         end if
         write(9,90090) '%%EOF'
         close(9)
         write(*,*) ' File cgiout.epsi created.'
         stop
      endif
...
c>>sas
c draw a line segment on the preview raster
      if(ipen.ne.0) then
         nstep = max(abs(ixdc-ixras),abs(iydc-iyras))/10
         do 90 j=0,nstep
         ix = (ixras+((j+1.0e-5)*(ixdc-ixras))/(nstep+1.0e-5))/10
         iy = (iyras+((j+1.0e-5)*(iydc-iyras))/(nstep+1.0e-5))/10
         call setras(1,ix,iy)
   90    continue
      endif
      ixras = ixdc
...
C C C C C C C C C C C C C C C C C C C C C C C C C C C C C C C C C C C C
      ...
      integer*4 koutfl, koutff
c>>sas
      DATA PSTNAM /'popscratch'/
...
*- INCLUDE PSTOPEN
c>>sas
        OPEN(77,FORM='FORMATTED',STATUS='scratch',
...
c>>sas
      width=max(pstmlw*wmag,linwth)
c     WIDTH=MAX(PSTMLW,LINWTH)
...
C     Otherwise, code will interactively ask for device type, if
...
c >>sas
c initialize the preview raster
      call setras(0,0,0)
      RETURN
      END
...
c$$$      IF(KPSTBG.NE.0) THEN
c$$$        CALL PSTBBG
c$$$      END IF

c>>sas
c stick in the preview raster and stop --- NOTE: Does not return
      call setras(2,0,0)
      GO TO 370
...
C     void frame
  ...
  350 CALL PSTBUF(0,' ')
      CALL PSTBUF(35,'%  VOID PAGE #'//KPAGE//' SUPPRESSED')
      CALL PSTBUF(0,' ')
      GO TO 380
...
C     write end of data message
      CALL PSTBUF(20,'%   END of PLOT DATA')
      CALL PSTBUF(0,' ')
C CLOSE OUTPUT FILE

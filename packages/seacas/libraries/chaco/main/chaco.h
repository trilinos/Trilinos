 ...
 *      3. adjacency. Type int *. As indicated in the description of start, this array con- 
 *         tains a list of edges for all vertices in the graph. Note that if the FREE GRAPH 
 *         parameter from x6.8 is set to TRUE, then after converting to a new data struc- 
 *         ture, both start and adjacency are freed. If this is inappropriate for your ap- 
 *         plication (e.g. you want to keep the graph, or you didn't dynamically allocate 
 *         these arrays), then you should set FREE GRAPH to FALSE. 
 *      4. vwgts. Type int *. This array of length nvtxs speci es weights for all the 
 *         vertices. If you pass in a NULL pointer, then all vertices are given unit weight. 
 *         Vertex weights should be positive. 
 *
 *      5. ewgts. Type oat * (Fortran type real*4). This array
 ...
 *         as those on the \Global method" menu when running Chaco in stand{alone 
 ...
 *
 *      18. rqi ag. Type int. If you requested spectral partitioning and wish to use the 
 ...
 *         the recursive coarsening is nished. 
 ...
 *      21. eigtol. Type double. If you are using a spectral method or multilevel{ 
 *         KL, this argument specifies the tolerance you request for the eigensolver. A 

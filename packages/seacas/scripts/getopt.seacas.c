...
void add_longopt(const char *name, int has_arg)
{
  ...
  if (long_options_nr == long_options_length) {
    ...
    long_options = our_realloc(long_options, sizeof(struct option) * long_options_length);
  }
...
void print_help(void)
{
  ...
  fputs("  -u, --unqote                 Do not quote the output\n", stderr);
...
int main(int argc, char *argv[])
{
  ...
  while ((opt = getopt_long(argc, argv, shortopts, longopts, NULL)) != EOF) {
    ...
    case 'n':
      ...
      break;
    case 'q': quiet_errors = 1; break;
    case 'Q': quiet_output = 1; break;
    case 's': set_shell(optarg ? optarg : ""); break;
    case 'T': exit(4);

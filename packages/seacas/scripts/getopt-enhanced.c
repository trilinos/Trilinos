 ...
 *  Copyright (c) 1997-2014, 2022 Frodo Looijaard <frodo@frodo.looijaard.name>
 *
...
static int generate_output(struct getopt_control *ctl, char *argv[], int argc)
{
  ...
  while ((opt = (getopt_long_fp(argc, argv, ctl->optstr, (const struct option *)ctl->long_options,
                                &longindex))) != EOF) {
    if (opt == '?' || opt == ':')
      ...
      case LONG_OPT:
        ...
        break;
      case NON_OPT: print_normalized(ctl, optarg ? optarg : ""); break;
      default:
        printf(" -%c", opt);
...
int main(int argc, char *argv[])
{
  ...
  static const struct option longopts[] = {
      {"options", required_argument, NULL, 'o'}, {"longoptions", required_argument, NULL, 'l'},
      {"quiet", no_argument, NULL, 'q'},         {"quiet-output", no_argument, NULL, 'Q'},
      {"shell", required_argument, NULL, 's'},   {"test", no_argument, NULL, 'T'},
      {"unquoted", no_argument, NULL, 'u'},      {"help", no_argument, NULL, 'h'},
      {"alternative", no_argument, NULL, 'a'},   {"name", required_argument, NULL, 'n'},
  ...
  while ((opt = getopt_long(argc, argv, shortopts, longopts, NULL)) != EOF)
    switch (opt) {
    case 'a': getopt_long_fp = getopt_long_only; break;
    case 'o': add_short_options(&ctl, optarg); break;
    case 'l': add_long_options(&ctl, optarg); break;
    case 'n':
      free(ctl.name);
      ctl.name = xstrdup(optarg);
      break;
    case 'q': ctl.quiet_errors = 1; break;
    case 'Q': ctl.quiet_output = 1; break;
    case 's': ctl.shell = shell_type(optarg); break;
    case 'T': free(ctl.long_options); return TEST_EXIT_CODE;
    case 'u': ctl.quote = 0; break;

    case 'V': print_version(EXIT_SUCCESS);
    case '?':
    case ':': parse_error(NULL);
    case 'h': usage();
    default: parse_error(_("internal error, contact the author."));
    }

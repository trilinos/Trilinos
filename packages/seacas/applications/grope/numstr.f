...
C=======================================================================
C$Log: numstr.f,v $
CRevision 1.2  2008/03/14 13:31:24  gdsjaar
CAdded copyright information to top of all files.
C
CGROPE2 is now open-sourced under BSD license
C
CRevision 1.1  2005/08/17 19:59:36  gdsjaar
CAdded error checking to precision command.  Now checks that precision
Cis positive and less than 17.
C
CAdded precision-based control of times printed via 'steps' command.
C
CThere was a problem in numstr for a number which would round up based
Con the chosen format and would then overrun the format.  Added a check
Cfor '*' in the number and if found then modified the format and
Cprinted again.  Added numstr to grope2 source instead of using suplib
Cversion (which should still be fixed at sometime...)
C

...
C   $Id: arcxyz.f,v 1.1 1990/08/20 12:21:02 gdsjaar Exp $
C   $Log: arcxyz.f,v $
C   Revision 1.1  1990/08/20 12:21:02  gdsjaar
C   Initial revision
C
...
C      --Get the coordinates for the non-center nodes 
      ...
      DIM3RD = DIM3 * PI / 180.0
      CALL INIGRD (0.0, -DIM3RD, RGRAD, NEREPL, NNREPL, COSANG)
         DO 30 NR = 1, NNREPL
            ANG = COSANG(NR)
            SINANG(NR) = SIN (ANG)
            COSANG(NR) = COS (ANG)

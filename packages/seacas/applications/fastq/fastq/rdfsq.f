...
C
  ...
  110          CONTINUE
            ...
            END IF
C
C  INPUT AN ATTRIBUTE INTO THE DATABASE
C
C         ELSE IF (CIN(1)(1:6) .EQ. 'ATTRIB') THEN
C            NODATA = .FALSE.
C            JJ = IIN(2)
C            IF ((JJ .LE. 0) .OR. (JJ .GE. 10000)) THEN
C               WRITE(*, 10180) JJ
C               GO TO 120
C            ELSE IF (IFOUND .LT. 3) THEN
C               WRITE(*, 10190) JJ
C               GO TO 120
C            END IF
C            CALL INATTR (MS, MR, MA, N(17), N(23), RIN(3), IFOUND - 2,
C     &         ATTRIB, LINKM, NOROOM)
C            IF (NOROOM) GO TO 310
C
...
10170 FORMAT (' FOR FACTORS TO BE INPUT ON LINE: ', I5, /,
     &   ' NOT ENOUGH INFORMATION IS SUPPLIED', /,
     &   ' THIS INTERVAL WILL NOT BE ASSIGNED')
10180 FORMAT (' A MATERIAL NO. OF:', I7, ' IS NOT ALLOWED', /,
     &   ' THIS ATTRIBUTE WILL NOT BE INPUT INTO DATABASE')
10190 FORMAT (' FOR ATTRIBUTES FOR MATERIAL NO.:', I5, /,
     &   ' NOT ENOUGH INFORMATION IS SUPPLIED', /,
     &   ' THIS ATTRIBUTE WILL NOT BE INPUT INTO DATABASE')

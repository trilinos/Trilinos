...
C  A CIRCULAR ROW THAT ENDS AT A ROW END (TEAR DROP SHAPE) NEEDS STARTED

         IF (LNODES (1, N1) .EQ. 1) THEN
            NEND = N0
            NADJ1 = NNN + 1

C  A CIRCULAR ROW THAT HAS NO END, CORNER, OR REVERSAL (CIRCLE SHAPE)
C  NEEDS STARTED
...
C  A CIRCULAR ROW THAT ENDS AT A ROW CORNER NEEDS STARTED
...
C  A CIRCULAR ROW THAT ENDS AT A ROW REVERSAL NEEDS STARTED
     ...
     &         IDIVIS, SIZMIN, EMAX, EMIN)
         ...
         ELSE
            CALL MESSAGE('PROBLEMS IN ADDROW - THE CIRCULAR ROW')

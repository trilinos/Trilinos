C $Id: rmesh.f,v 1.1 1990/11/30 11:15:05 gdsjaar Exp $
C $Log: rmesh.f,v $
C Revision 1.1  1990/11/30 11:15:05  gdsjaar
C Initial revision
C
C
...
C
      CALL CROSS (4, KKK, 2, LLL, LXK, KXL, KKKOLD + 1, LLLOLD + 1,
     &   NOROOM, ERR)
      IF (ERR) RETURN
      ERR = .TRUE.
C
...
C
      CALL CROSS (2, LLL, 4, NNN, NXL, LXN, LLLOLD + 1, NNNOLD + 1,
     &   NOROOM, ERR)
      IF (ERR) RETURN
      ERR = .TRUE.
C

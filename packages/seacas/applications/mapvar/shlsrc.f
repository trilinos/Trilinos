...
C 
     ...
     *  ISRCHR,   RSRCHR,   IDP,      IDS,      CTRCL,    TOLSRCH,
     *  IERR    )
C     
C-----------------------------------------------------------------------
C     
...
C  RECOMPUTE LOCAL COORDS
          ...
          CTRCL(3,I) = ECOORD
          CTRCL(4,I) = PROJN
          CTRCL(5,I) = VCS12X/DISPS
          CTRCL(6,I) = VCS12Y/DISPS
          CTRCL(7,I) = VCS12Z/DISPS
        ENDIF
  201 CONTINUE
C
      DO 202 I = 1, KOUNTS
...
C      
      DO 300 I = 1, KOUNTS
        NN = IDP(I)
        IF( NINT(CTRCL(1,I)) .NE. 0 )THEN
C
C STORE CURRENT SEARCH RESULTS
          IF( ISRCHR(1,NN) .EQ. 0 )THEN
C STORE INTEGER SEARCH RESULTS
            ISRCHR(1,NN) = NINT(ABS(CTRCL(1,I)))
            ITYPE = 1
            IF( NINT(CTRCL(1,I)) .LT. 0 ) ITYPE = 2
            IF( NISR .GE. 2 ) ISRCHR(2,NN) = ITYPE
C STORE INTEGER SEARCH RESULTS
            RSRCHR(1,NN) = CTRCL(5,I)
            RSRCHR(2,NN) = CTRCL(6,I)
            RSRCHR(3,NN) = CTRCL(7,I)
            RSRCHR(4,NN) = CTRCL(4,I)
            IF( NRSR .GE. 6 ) THEN
               RSRCHR(5,NN) = CTRCL(2,I)
               RSRCHR(6,NN) = CTRCL(3,I)
            ENDIF
          ELSE
C KEEP CURRENT SEARCH RESULTS ONLY IF CLOSER
            IF( ABS(CTRCL(4,I)) .LT. ABS(RSRCHR(4,NN)) )THEN
C STORE INTEGER SEARCH RESULTS
              ISRCHR(1,NN) = NINT(ABS(CTRCL(1,I)))
              ITYPE = 1
              IF( CTRCL(1,I) .LT. 0 ) ITYPE = 2
              IF( NISR .GE. 2 ) ISRCHR(2,NN) = ITYPE
C STORE INTEGER SEARCH RESULTS
              RSRCHR(1,NN) = CTRCL(5,I)
              RSRCHR(2,NN) = CTRCL(6,I)

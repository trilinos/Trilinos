...
namespace {
  ...
  int matPutStr(const std::string &name, char *str)
  {
    ...
    matvar =
        Mat_VarCreate(name.c_str(), MAT_C_CHAR, MAT_T_UINT8, 2, dims, str, MAT_F_DONT_COPY_DATA);
    if (matvar != nullptr) {
  ...
  int matPutInt(const std::string &name, int n1, int n2, int *pd)
  {
    ...
    matvar =
        Mat_VarCreate(name.c_str(), MAT_C_INT32, MAT_T_INT32, 2, dims, pd, MAT_F_DONT_COPY_DATA);
    if (matvar != nullptr) {
      error = Mat_VarWrite(mat_file, matvar, MAT_COMPRESSION_ZLIB);
      Mat_VarFree(matvar);
    }
  ...
  void get_put_user_names(int exo_file, ex_entity_type type, int num_blocks,

#ifndef ML_TETCUBEGRID_H
#define ML_TETCUBEGRID_H

/*!
 * \file ml_TetCubeGrid.h
 */
...
public:
  ...
  virtual double FaceArea(const int LocalFace) const
  {
    ...
    if (patch == ML_BOTTOM || patch == ML_TOP)
      return(0.5 * DeltaX() * DeltaY());
    else if (patch == ML_LEFT || patch == ML_RIGHT)

   ...
   *Amat_f = AZ_matrix_create(data_org[AZ_N_internal] + data_org[AZ_N_border]);
   ...
   if (flag == 1) {
      ...
      for (level = N_levels-1; level > coarsest_level; level--) {
	   ...
	   ML_Gen_Smoother_Jacobi(ml , level, ML_BOTH, nsmooth,.6);
         }
         ...
         else if (ML_strcmp(context->smoother,"ilu") == 0) {
          /* This is the Aztec domain decomp/ilu smoother that we */
          /* usually use for this problem.                        */

	   save1 = options[AZ_precond];
	   save2 = options[AZ_subdomain_solve];
	   save3 = options[AZ_reorder];
	   options[AZ_precond] = AZ_dom_decomp;
      ...
      if (ML_strcmp(context->coarse_solve,"Parasails") == 0) {
        ...
        ML_Gen_Blocks_Aggregates(*ml_ag, coarsest_level, &nblocks, &block_list);
        ML_Gen_Smoother_VBlockSymGaussSeidel(ml , coarsest_level, ML_BOTH,
                                             nsmooth,1., nblocks, block_list);
      }
        ...
        nblocks = 250;
        ML_Gen_Blocks_Metis(ml, coarsest_level, &nblocks, &block_list);
        ML_Gen_Smoother_VBlockSymGaussSeidel(ml , coarsest_level, ML_BOTH,
                                             nsmooth,1., nblocks, block_list);
      }
...
*/
  ...
  int        NP;
  int        nx, ny;
  double     dx, dy, rootN, h, eps = 100000000.;
  double north = 0., south = 0., east = 0., west = 0., center = 0.,aa,bb;
  ...
  *n = m;
  ...
  rootN = sqrt( ((double) N_grid_pts)+0.01);
  h = 1/rootN;

  nx = (row/NP)%m;
  ny = (row/(NP*m))%m;

  /* dx,dy are double precision coordinates */
  dx = (((double) nx)+.5)/ ((double) rootN );
  ...
  /* figure out which direction is upwind and stick */
  /* in stencil for first derrivative terms.        */
  ...
  if ( dx*dx+dy*dy  >  -.2 ) { /* if statement is currently turned off   */
                                /* but it could be used to make Dirichlet */
                                /* points in the center                   */

      bindx[k] = row + NP;   if ((row/NP)%m !=     m-1) val[k++] =  east;
      bindx[k] = row - NP;   if ((row/NP)%m !=       0) val[k++] =  west;
      bindx[k] = row + m*NP; if ((row/(NP*m))%m != m-1) val[k++] =  north;
      bindx[k] = row - m*NP; if ((row/(NP*m))%m !=   0) val[k++] =  south;

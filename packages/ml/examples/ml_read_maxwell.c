...
#    endif
  ...
  AZ_input_update(context->partition_file,&Nlocal_edges, &global_edge_inds, 
  ...
  printf("adding something to the diagonal\n");
  for (i = 0; i < Nlocal_edges; i++) Ke_val[i] += 1.;

  AZ_transform(proc_config, &global_edge_externs, Ke_bindx, Ke_val, 
               global_edge_inds, &reordered_glob_edges, 
  ...
  AZ_input_update(context->partition_file,&Nlocal_nodes, &global_node_inds, 
  ...
  AZ_transform(proc_config, &global_node_externs, Kn_bindx, Kn_val,
	       global_node_inds, &reordered_glob_nodes, 
  ...
  AZ_input_msr_matrix("Tmat.az", global_edge_inds, &Tmat_val, &Tmat_bindx, 
		      Nlocal_edges, proc_config);
  ...
  ML_Operator_Set_ApplyFuncData( Tmat, Ncols, Nlocal_edges, 
  ...
  ML_Operator_AmalgamateAndDropWeak(&(ml_edges->Amat[N_levels-1]), num_PDE_eqns, 0.0);
  ML_Gen_Blocks_Metis(ml_edges, N_levels-1, &nblocks, &block_list);

  for (i = 0; i < nblocks; i++) {
     count = 0;
     for (j = 0; j < ml_edges->Amat[N_levels-1].outvec_leng; j++) {
  ...
  fclose(fp2);
  ML_Operator_UnAmalgamateAndDropWeak(&(ml_edges->Amat[N_levels-1]),num_PDE_eqns,0.0);
  ...
  csr_data = (struct ML_CSR_MSRdata *) Tmat_trans->data;
  /*
  csr_data->rowptr
  csr_data->columns
  csr_data->values */
  for (i = 0; i < 540; i++) printf("block %d %d\n",i,block_list[i]);
  proc_assignment = (int *) AZ_allocate( (Ncols+1)*sizeof(int));
  for (i = 0; i < Ncols ; i++ ) {
    pcount = 0;
    for (j = (csr_data->rowptr)[i]; j < (csr_data->rowptr)[i+1]; j++ ) {
      col2 = (csr_data->columns)[j];
      p2   = block_list[col2];
      printf("col2 is %d %d\n",col2,block_list[col2]);
      if (p2 > 0) printf("for proc 1\n");
      for (kk = 0 ; kk < pcount ; kk++) {
	if (procs_cols[kk] == p2) break;
      }
      if (kk == pcount) {procs_cols[pcount] = p2; proc_count[pcount++] = 1;}
      else {proc_count[kk]++;}
    }
    printf("before %d\n",proc_count[0]);
    /* find the largest proc_count() */
    col1 = -1;
    for (kk = 0; kk < pcount; kk++) {
      if (proc_count[kk] > col1) {col1 = proc_count[kk]; p1 = procs_cols[kk]; }
    }
    printf("processor is %d\n",p1);

    /*
    if ((csr_data->rowptr)[i+1] - (csr_data->rowptr)[i] > 2) 
      pr_error("Too many nonzeros per row in Tmat   %d\n",
	       (csr_data->rowptr)[i+1] - (csr_data->rowptr)[i]);
    if ((csr_data->rowptr)[i+1] - (csr_data->rowptr)[i] == 1) {
      col1 = (csr_data->columns)[(csr_data->rowptr)[i]];
      proc_assignment[i] = block_list[col1];
    }
    else if ((csr_data->rowptr)[i+1] - (csr_data->rowptr)[i] == 2) {
      col1 = (csr_data->rowptr)[(csr_data->rowptr)[i]];
      col2 = (csr_data->rowptr)[(csr_data->rowptr)[i]+1];
      p1   = block_list[col1];
      p2   = block_list[col2];
      if ( (col1*col2)%2 == 0) {
    ...
    else pr_error("No column entries in Tmat???\n");
    */
  }
  exit(1);
#endif
  ...
  /* read in the rigid body modes */
   ...
   if (fp == NULL) {
      ...
      AZ_input_msr_matrix("rhsfile", global_edge_inds, &rhs, &garbage, Nlocal_edges, 
                          proc_config);
   }
  ...
  */
  ...
  ML_rap(ml_edges, Tmat, &(ml_nodes->Pmat[N_levels-2]), Tcoarse_trans, 
	 &(ml_edges->Pmat[N_levels-2]), N_levels-2, Tcoarse_trans->invec_leng);
...
/*
  coarsest_level = ML_Gen_MGHierarchy_UsingAggregation(ml_edges, N_levels-1, 
                                            ML_DECREASING, NULL);
*/
...
/*
      ...
      for (i = 0; i < Ke_data_org[AZ_N_internal]+Ke_data_org[AZ_N_border]; i++)
         fscanf(fp,"%lf",&(xxx[i]));
*/
...
*/
      ...
      options[AZ_keep_info] = 1;
      /*      ML_Operator_Print(&(ml_edges->Amat[0]) ,"Ke_mat"); */
      options[AZ_conv] = AZ_noscaled;
      printf("options %d\n",options[AZ_conv]);
      options[AZ_output] = 1;
      /*
      ...
      */
      printf("norm of rhs is %e\n",sqrt(ML_gdot(Nlocal_edges,rhs,rhs,ml_edges->comm)));

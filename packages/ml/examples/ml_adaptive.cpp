...
int main(int argc, char *argv[])
{
  ...
  Gallery.Set("problem_size", 90000);
  ...
  MLList.set("adaptive: enable", true);
  MLList.set("adaptive: use symmetric cycle", false);
  MLList.set("adaptive: eigensolver", "ARPACK");
  MLList.set("adaptive: eigenvalues to compute", 10);
  MLList.set("adaptive: type", "enhanced");
  MLList.set("adaptive: local aggregates", 1);

  // require to tune eigen-solution (here with Anasazi)
  MLList.set("eigen-analysis: tolerance", 1e-2);
  MLList.set("eigen-analysis: block-size", 20);

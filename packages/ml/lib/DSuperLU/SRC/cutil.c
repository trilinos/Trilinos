		...
		SuperMatrix *A, SuperMatrix *B)
{
    ...
    sp_cgemm(trans, "N", A->nrow, nrhs, A->ncol, one, A,
	     x, ldx, zero, rhs, ldc);
...
void cinf_norm_error(int nrhs, SuperMatrix *X, complex *xtrue)
{
	...
	err = MAX(err, c_abs(&temp));
	xnorm = MAX(xnorm, c_abs(&soln_work[i]));
      }
      err = err / xnorm;
      printf("||X - Xtrue||/||X|| = %e\n", err);
    }
...
cPrintPerf(SuperMatrix *L, SuperMatrix *U, mem_usage_t *mem_usage,
	       complex rpg, complex rcond, complex *ferr,
	       complex *berr, char *equed)
{
    SCformat *Lstore;
    NCformat *Ustore;

...
#if   BLAS&&r8
  *work = 0.0;
  cblas_dcopy(size,work,0,buf,1);
#elif BLAS
  *work = 0.0;
  cblas_scopy(size,work,0,buf,1);
#else
  rvec_zero(buf,size);
#endif
...
gs_gop_vec_tree_plus(register gs_id *gs, register REAL *vals, register int step) 
{
  ...
  *work = 0.0;
  ...
  while (*in >= 0)
    { 
#if   BLAS&&r8
      cblas_dcopy(step,vals + *in++*step,1,buf + *out++*step,1);
#elif BLAS
      cblas_scopy(step,vals + *in++*step,1,buf + *out++*step,1);
#else
  ...
  while (*in >= 0)
    {
#if   BLAS&&r8
      cblas_dcopy(step,buf + *out++*step,1,vals + *in++*step,1);
#elif BLAS
      cblas_scopy(step,buf + *out++*step,1,vals + *in++*step,1);
#else

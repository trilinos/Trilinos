 ...
 */
{
	...
	if ( !(rb_marker = intCalloc(k)) )
	    ABORT("Calloc fails for rb_marker[].");
	if ( !(Urb_length = intCalloc(k)) )
	    ABORT("Calloc fails for Urb_length[].");
	if ( !(Urb_indptr = intMalloc(k)) )
	    ABORT("Malloc fails for Urb_indptr[].");
	if ( !(Urb_fstnz = intCalloc(k)) )
	...
	if ( !(Lrb_length = intCalloc(k)) )
	    ABORT("Calloc fails for Lrb_length[].");
	if ( !(Lrb_number = intMalloc(k)) )
	    ABORT("Malloc fails for Lrb_number[].");
	if ( !(Lrb_indptr = intMalloc(k)) )
	...
	for (jb = 0; jb < nsupers; ++jb) {
		...
		if ( nrbl ) { /* Do not ensure the blocks are sorted! */
		    ...
		    mybufmax[0] = MAX( mybufmax[0], len1 );
		    mybufmax[1] = MAX( mybufmax[1], len*nsupc );
		    mybufmax[4] = MAX( mybufmax[4], len );

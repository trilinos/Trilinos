...
#define ITMAX 20
    ...
    *info = 0;
    if ( n < 0 ) *info = -1;
	      ...
	      A->Stype != NCP || A->Dtype != _Z || A->Mtype != GE )
	*info = -2;
    ...
    pzgsmv_setup(A, Glu_persist, grid, &N_update, &update,
    ...
    lwork = N_update         /* For ax and R */
	  ...
	  + ii               /* For x_trs */
          + k                /* For b */
	  + jj;              /* for temp */
    if ( !(work = doublecomplexMalloc(lwork)) )
...
#if ( DEBUGlevel>=2 )
    {
	doublecomplex *dwork = doublecomplexMalloc(n);
	for (i = 0; i < n; ++i) {
	    if ( i & 1 ) dwork[i].r = 1.;
	    else dwork[i].r = 2.;
	    dwork[i].i = 0.;
	...
	pzgsmv(N_update, update, val, bindx, dwork, ax);
	...
	while (1) { /* Loop until stopping criterion is satisfied. */
	    ...
	    /* Matrix-vector multiply. */
	    pzgsmv(N_update, update, val, bindx, X_col, ax);
	    
	    /* Compute residual. */
	    for (i = 0; i < N_update; ++i) z_sub(&R[i], &b[i], &ax[i]);

	    /* Compute abs(op(A))*abs(X) + abs(B). */
	    pzgsmv_abs(N_update, update, val, bindx, X_col, rwork);
	    for (i = 0; i < N_update; ++i) rwork[i] += z_abs1(&b[i]);
	    
	    s = 0.0;
	    for (i = 0; i < N_update; ++i) {
		if ( rwork[i] > safe2 )
		    s = MAX(s, z_abs1(&R[i]) / rwork[i]);
		else
		    s = MAX(s, (z_abs1(&R[i]) + safe1) / (rwork[i] + safe1) );
	    }

...
static void zcreate_msr_matrix(SuperMatrix *, int_t [], int_t,
			doublecomplex **, int **);
static void zPrintMSRmatrix(int, doublecomplex [], int [], gridinfo_t *);
static void PrintInt(char *, int, int []);
 ...
 )
{
     ...
     * so that subsequent calls to AZ_find_index() will function properly.
     */
    n = A->nrow;
...
)
{
    ...
    if ( !(rowcnt = (int_t *) intCalloc(N_update)) )
    ...
    if ( !(*val = (doublecomplex *) doublecomplexMalloc(nnz_local+1)) )
	ABORT("Malloc fails for val[]");
    if ( !(*bindx = (int *) SUPERLU_MALLOC((nnz_local+1) * sizeof(int))) )
 ...
 */
void
pzgsmv(int_t m, int_t update[], doublecomplex val[], int bindx[],
 ...
 */
void
pzgsmv_abs(int_t m, int_t update[], doublecomplex val[], int bindx[],
...
)
{
    ...
    PrintInt("bindx", nnzp1, bindx);
}

static void PrintInt(char *name, int len, int *x)
{
    register int i;

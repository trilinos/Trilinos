...
#define ITMAX 20
    ...
    *info = 0;
    if ( n < 0 ) *info = -1;
	      ...
	      A->Stype != NCP || A->Dtype != _D || A->Mtype != GE )
	*info = -2;
    else if ( ldb < MAX(0, n) ) *info = -10;
    else if ( ldx < MAX(0, n) )	*info = -12;
...
#if ( DEBUGlevel>=2 )
    {
	double *dwork = doubleMalloc(n);
	for (i = 0; i < n; ++i) 
	    if ( i & 1 ) dwork[i] = 1.;
	    else dwork[i] = 2.;
	/* Check correctness of matrix-vector multiply. */
	pdgsmv(N_update, update, val, bindx, dwork, ax);
	...
	while (1) { /* Loop until stopping criterion is satisfied. */
	    ...
	    /* Matrix-vector multiply. */
	    pdgsmv(N_update, update, val, bindx, X_col, ax);
	    
	    /* Compute residual. */
	    for (i = 0; i < N_update; ++i) R[i] = b[i] - ax[i];

	    /* Compute abs(op(A))*abs(X) + abs(B). */
	    pdgsmv_abs(N_update, update, val, bindx, X_col, temp);
	    for (i = 0; i < N_update; ++i) temp[i] += fabs(b[i]);
	    
	    s = 0.0;
	    for (i = 0; i < N_update; ++i) {
		if ( temp[i] > safe2 )
		    s = MAX(s, fabs(R[i]) / temp[i]);
		else

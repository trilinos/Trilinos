 ...
 */
    ...
    if ( !(marker = (int_t*) SUPERLU_MALLOC( (MAX(m,n)+1) * sizeof(int_t)) ) )
...
void
get_perm_c(int_t pnum, int_t ispec, SuperMatrix *A, int_t *perm_c)
/*
 * Purpose
 * =======
 *
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(pnum, "Enter get_perm_c()");
#endif
...
#if ( DEBUGlevel>=1 )
    CHECK_MALLOC(pnum, "Exit get_perm_c()");
#endif
}

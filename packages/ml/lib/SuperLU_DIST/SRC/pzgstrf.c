 ...
 */
{
    ...
    int   msgcnt[4]; /* Count the size of the message xfer'd in each buffer:
		      ...
		      */
    int_t  msg0, msg2;
    doublecomplex *lusup, *lusup1, *uval, *Lval_buf, *Uval_buf;
    ...
    *info = 0;
	...
	for (pj = 0; pj < Pc; ++pj) {
		...
		MPI_Send( lusup, msgcnt[1], SuperLU_MPI_DOUBLE_COMPLEX, 
			 pj, 1, scp->comm );
#if ( DEBUGlevel>=2 )
...
/*#pragma _CRI cache_bypass nzval,tempv*/
	...
	if ( k+1 < nsupers && mycol == PCOL( k+1, grid ) ) {
		...
		if ( ToSendR[lk][pj] != EMPTY ) {
		    ...
		    MPI_Send( lusup1, msgcnt[1], SuperLU_MPI_DOUBLE_COMPLEX,
...
/* This is worse than blocking send. */
		    ...
		    MPI_Isend( lusup1, msgcnt[1], SuperLU_MPI_DOUBLE_COMPLEX,
			      pj, (4*(k+1)+1)%NTAGS, scp->comm, &request[1] );
#endif
#if ( PROFlevel>=1 )
		    TOC(t2, t1);
		    stat->utime[COMM] += t2;

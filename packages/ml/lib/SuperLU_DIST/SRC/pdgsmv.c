...
static void create_msr_matrix(SuperMatrix *, int_t [], int_t,
			      double **, int **);
static void PrintMSRmatrix(int, double [], int [], gridinfo_t *);
...
)
{
    ...
    if ( !(*val = (double *) doubleMalloc(nnz_local+1)) )
 ...
 */
void
pdgsmv(int_t m, int_t update[], double val[], int bindx[],
       double X[], double ax[])
{
    int_t i, j, k;
 ...
 */
void
pdgsmv_abs(int_t m, int_t update[], double val[], int bindx[],
	   double X[], double ax[])
{
    int_t i, j, k;

 ...
 */
    ...
    *info = 0;
	...
	for (k = 0; k < nsupers; ++k) {
		...
		jj = X_BLK( lk );
		x_col = &x[jj];
		MPI_Bcast( x_col, knsupc*nrhs, MPI_DOUBLE,
			  diag, grid->comm );
		RHS_ITERATE(j) {
		    for (i = 0; i < knsupc; ++i) { /* X stored in blocks */
			B_col[i] = x_col[i];
#if ( PRNTlevel>=2 )
			if ( k < 1 ) printf("\t(%d)\t%4d\t%.10f\n",
					    iam, xsup[k]+i, B_col[i]);
#endif
		    }
		    B_col += ldb;  x_col += knsupc;
		}
	    } else { /* All other processes. */
		MPI_Bcast( recvbuf, knsupc*nrhs, MPI_DOUBLE,
			  diag, grid->comm );
		x_col = recvbuf;
		RHS_ITERATE(j) {
		    for (i = 0; i < knsupc; ++i) /* X stored in blocks. */

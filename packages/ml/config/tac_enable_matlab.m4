...
if test -n "$MATLAB_EXE" -a -n "$MATLAB_ROOT_USE" -a -n "MATLAB_USE"; then
  ...
  AC_CHECK_HEADER([engine.h], [],
	[
	 ...
	 echo "Use --with-matlab-root=<DIR> to add the matlab root directory"
         echo "(such that <DIR>/extern/include contains engine.h)"
     	 echo "-----"
  ...
  AC_CHECK_HEADER([mex.h], [],  
	[
	 ...
	 echo "Use --with-matlab-root=<DIR> to add the matlab root directory "
         echo "(such that <DIR>/extern/include contains mex.h)"
     	 echo "-----"
     	 AC_MSG_ERROR(Matlab header file mex.h not found)
     	])
  # Restore CPPFLAGS
  CPPFLAGS=$save_CPPFLAGS
  # Since there's no reliable way to figure out the location of the mex libraries, let's just not check for them.  
  # If you've managed to get matlab and mex installed and specified the matlab root so we can find mexext and the 

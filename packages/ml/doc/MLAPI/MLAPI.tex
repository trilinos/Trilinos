...
both serial and (massively) parallel environments. 
...
computers, to show that the overload required by the object-oriented approach
is very modest, therefore allowing developers of new algorithms to focus on
the method itself, rather than the implementation details, yet obtaining a
...
$n$ & {\tt const}  & {\tt appl}  & {\tt total}  & {\tt constr}  
    & {\tt appl}   & {\tt total} & {\tt diff}  \\
\hline
$32^3$ &   0.74 &   1.21 &  1.86  &  0.79 &   1.23 &  1.91 & 2.68  \\
$64^3$ &   3.76 &  12.03 & 15.79  &  4.28 &  12.92 & 17.20 & 8.92  \\
$96^3$ &  11.73 &  53.20 & 64.93  & 13.53 &  56.02 & 69.55 & 7.11  \\
\hline
\end{tabular}
\caption{LINUX: 3D-Laplacian on LINUX.}
\label{tab:linux-3d}
\end{center}
\end{table}
...
\hline
\end{tabular}
\caption{SUN: 2D-Laplacian on SUN.}
\label{tab:sun-2d}
\end{center}
\end{table}
...
\hline
\end{tabular}
\caption{CPLANT: 2D-Laplacian problem.  The matrix has size $512^2 \times p$.}
\label{tab:cplant-2d}
\end{center}
\end{table}

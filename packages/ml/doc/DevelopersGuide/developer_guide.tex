...
established in the Trilinos Developer's Guide~\ref{Trilinos-Dev-Guide}.
\end{abstract}
...
Trilinos Developer's Guide~\ref{Trilinos-Dev-Guide}, and to the GNU CVS Home
Page.
...
\item When creating a configure {\bf huh??}
...
\item The best way to write comment would be to write the code so that
  ...
  written code.
\item \ML uses Doxygen for the comments~\cite[doxygen] in C++ code. Most
  of the C code is not Doxygen-compliant. (However, the most important
 ...
 */
\end{verbatim}
\item Document functions/methods in Doxygen style:
\begin{verbatim}
...
for instance, in the NOX guide, the Epetra Developer's Coding

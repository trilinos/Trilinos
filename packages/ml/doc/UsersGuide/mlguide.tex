...
described. This class only requires the linear system matrix, and a list
...
standalone package without or with {\sc Aztec 2.1} support (as described
...
{Enables support for the \amesos~package.
 This package is necessary to use the \amesos\ interface in \ML\ to direct
 solvers.  The solvers currently available are
  \umfpack~\cite{umfpack-home-page}, \klu, \superludist~(1.0 and 2.0),
  \superlu~$3.0$~\cite{superlu-home-page}, and \mumps~\cite{MUMPS}.}
%
...
%
\choicebox{\tt --with-ml\_parmetis2x}{ Enables interface for
\parmetis~\cite{ParMETIS}, version $2.x$.}
%
\choicebox{\tt --with-ml\_zoltan}{ Enables interface for \zoltan~\cite{zoltan-home-page}.}
%
...
\begin{verbatim}
--with-incdirs="-I/include-locations"
\end{verbatim}
...
\amesos. \\
\verb!Amesos-Superlu! & Use \superlu\ through \amesos. \\
...
\choicebox{\tt repartition: edge min per proc}{[{\tt int}] Maxwell-specific option.

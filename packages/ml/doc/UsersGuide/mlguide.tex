...
\ref{all possible parameters}.  More advanced uses of \ML\ are presented in
...
\begin{verbatim}
  ...
  // now re-compute the preconditioner
  MLPrec->ComputePreconditioner();
...
\choicebox{\tt output}{[{\tt int}] Output level, from 0 to 10 (10 being verbose). 
  Default: 10.}

\choicebox{\tt print unused}{[{\tt int}] If non-negative, will print all the
  unused parameter on the specified processor. If -1, the unused parameters
...
order for {\tt MLS} sub-smoother.  Default: 3.}
...
\choicebox{\tt smoother: Hiptmair SGS edge sweeps $\star$}{[{\tt int}] Number of iterations
 of SGS sub-smoothing to perform on edge problem. Default: 1.}
...
\choicebox{\tt coarse: sweeps}{[{\tt int}] (Only for {\tt Jacobi} and {\tt
                                            Gauss-Seidel}.) Number of
...
the use of 
matrix~(\ref{eq:discrete_laplacian}). Ignored if the input matrix is an
        Epetra\_VbrMatrix. Default: {\tt false}.}

\choicebox{\tt aggregation: use symmetric pattern}{[{\tt bool}]. If
{\tt true}, matrix~(\ref{eq:discrete_laplacian}) will have a symmetric
pattern. Ignored if the input matrix is an Epetra\_VbrMatrix. Default: {\tt
false}.}

\choicebox{\tt aggregation: theta}{[{\tt double}]. If $\theta$ is 
different from zero, the
code create the aggregates using matrix $\theta A + (1 - \theta)L$. Ignored if the input matrix is an Epetra\_VbrMatrix. Default:
{\tt 0.0}.}

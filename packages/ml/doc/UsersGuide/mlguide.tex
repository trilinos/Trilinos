...
\begin{center}
{\Large {\bf ML 3.1 Smoothed Aggregation User's Guide}}
...
It's assumed that you've already have a local copy of Trilinos.
...
\item Configure Trilinos:
  \be
  \item   \verb!../configure --enable-teuchos ! if you do not want to use MPI.
  \item   \verb!../configure  --enable-teuchos --with-mpi-compilers=/usr/local/mpich/bin! (or wherever your MPI compilers are
located) to use MPI.
  \ee
\item Build Trilinos: \verb!make!.\footnote{If you are using GNU's make on a
...
Directory \verb!\test! is intended primarily for developers and can be ignored.
...
TPLs. By default, all the {\tt --with-ml-}{\sl TPL} options are disabled.
\smallskip
...
\verb!$ML_HOME/examples/ml_preconditioner.cpp!.
...
destroyed using \verb!MLPrec->DestroyPreconditioner()!\footnote{We suggest to create
...
\item \ifpack~smoother requires \verb!--enable-ifpack --enable-epetra!.
...
Note that some parameters (e.g., {\tt Uncoupled-MIS} aggregation) correspond to
quantities that must be the same at all levels.
Descriptions follow the general format:\\\\
\choicebox{\tt parameter}{[{\tt Type}] Description. \{[Value1], Value2, \dots\}}
where $[\cdot]$ denotes the default value.
...
second leg. \{[true],false\}.}

\choicebox{\tt smoother: Hiptmair subsmoother type $\star$}{[{\tt string}] Smoother to be used
as a subsmoother at each step of Hiptmair smoothing.  \{[{\tt MLS}],
{\tt symmetric Gauss-Seidel}\}.}

\choicebox{\tt smoother: Hiptmair MLS polynomial order $\star$}{[{\tt int}] Polynomial
order for {\tt MLS} sub-smoother.  [3].}

\choicebox{\tt smoother: Hiptmair SGS damping factor $\star$}{[{\tt double}]
Damping factor for {\tt symmetric Gauss-Seidel}. Automatically calculated in
the range $(0,2)$.}

\choicebox{\tt smoother: Hiptmair SGS edge sweeps $\star$}{[{\tt int}] Number of iterations
 of SGS sub-smoothing to perform on edge problem. [1].}

\choicebox{\tt smoother: Hiptmair SGS node sweeps $\star$}{[{\tt int}] Number of iterations
 of SGS sub-smoothing to perform on nodal projection problem.
[1].}
...
to use. {Zoltan, ParMETIS}.}

\choicebox{\tt repartition: node min max ratio}{[{\tt double}] Specifies ratio
of maximum
...
\choicebox{\tt repartition: edge min max ratio}{[{\tt double}] Maxwell-specific option.  Specifies ratio of maximum
...
edges that should be on a processor once repartitioning is done.  [20].}

\choicebox{\tt repartition: Zoltan dimensions}{[{\tt int}] Dimension of the problem.  $\{2,3\}$.}

\choicebox{\tt repartition: Zoltan node coordinates}{[{\tt double*}] Pointer to array of
double node coordinates.  The array is one-dimensional, containing $x$-,$y$-, and
$z$- coordinates, in that order.}

\choicebox{\tt repartition: Zoltan edge coordinates}{[{\tt double*}] Maxwell-specific option.
Pointer to array of
double edge coordinates.  The array is one-dimensional, containing $x$-,$y$-, and
$z$- coordinates, in that order.}
...
from each level's smoother, to the aggregation scheme, and a lot of others 
others.
...
the preconditioned finest-level matrix. In the following fragment of code,
...
spectrum. This method requires \ML\ to be configured with support for
\anasazi\
(see Section~\ref{sec:configure:3pl}). The following options, that must be set
{\sl before} the construction of the preconditioner, can be used to
...
\choicebox{\tt analysis: print anasazi status}{[{\tt bool}]
  If {\tt true}, prints out the status of the eigensolver after
...
The syntax is \verb!AnalyzeSmoothersDense(NumPre, NumPost)!. \verb!NumPre! is the number of pre-smoother applications, and
...
be used. This
function reports on the standard output the following information:
\begin{verbatim}
...
%% FIXME add paper references
For more theoretical background and algorithm descriptions, please see \cite{}.
...
the mesh.
There are two ways to contruct \Kn.
...
There are two ways to contruct $T$.
In the first method, 
...
%FIXME
\comm{JJH}{What about directionality?  As I recall, it doesn't matter.}
The second method assumes that \Kn\ is already constructed.
Each off-diagonal nonzero $(i,j)$ in the upper triangular
...
%
%%%%%%%%%%%%%%
\subsection{Prolongation options}\label{maxwell coarsening}
%%%%%%%%%%%%%%
%
%
%%%%%%%%%%%%%%

...
\end{verbatim}
The last two commands set the \teuchos\ reference-counted pointers to
...
\choicebox{\tt smoother: Aztec options $\star$}{[{\tt int*}] \teuchos\
reference-counted pointer to {\sc Aztec}'s options vector (only for {\tt Aztec}
  smoother) .}

\choicebox{\tt smoother: Aztec params $\star$}{[{\tt double*}] \teuchos\

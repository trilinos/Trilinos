...
int ML_Aggregate_ComposeExpandedCommInfo(ML_GetrowFunc *getrow_obj, 
           int Nrows, int num_PDEs, ML_Comm *comm, 
       ...
       ML_Operator *Amatrix, ML_Operator **Pmatrix, ML_Comm *comm)
{
   ...
   ML_Aggregate_ComposeExpandedCommInfo(getrow_obj, Nrows, num_PDE_eqns, 
                ...
                int *bc_array)
{
   ...
   if ( nbytes > 0 ) dbuf = (double *) malloc( nbytes );
   else              dbuf = NULL;
   ...
   row = 0;
   col_entered = (char *) malloc(sizeof(char)*(total_recv_leng+nblocks) );
   if (col_entered == NULL) 
   {
                              ...
                              int **bc_array_out)
{
   ...
   col_ind = (int *) malloc( nbytes );
   nbytes  = maxnnz_per_row * sizeof( double );
   col_val = (double *) malloc( nbytes );
   Nrows   = Amatrix->outvec_leng;
   ...
   if ( epsilon != 0.0 && exp_Nrows > 0 )
   {
      ...
      if ( nbytes > 0 ) diagonal = (double *) malloc(nbytes);
      else              diagonal = NULL;

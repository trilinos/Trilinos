...
int ML_Aggregate_ComposeVBlockCommInfo(ML_GetrowFunc *getrow_obj, 
           int Nrows, int nvblocks, int *vblock_info, ML_Comm *comm, 
       ...
       ML_Operator *Amatrix, ML_Operator **Pmatrix, ML_Comm *comm)
{
   ...
   double  *dble_buf2=NULL, largest, thesign;
   ...
   ML_Aggregate_ComposeVBlockCommInfo(getrow_obj, Nrows, nvblocks, 
       ...
       int *send_list, int *recv_list, int **cnt_array)
{
   ...
   for ( i = 0; i < Nrows; i++ ) 
   {
      ...
      if ( length == 1 && (mat_indx[mat_indx[i]] == i )) trackbc[i] = 1; 
      else                                               trackbc[i] = 0;
                ...
                int **send_leng, int **send_list, int **recv_list)
{
   ...
   int   total_recv_leng, *Lsend_count, mcount, chk_bdry;
   char  *col_entered;
   ...
   for ( i = 0; i < nvblk_local; i++) 
   {
      ...
      if ( chk_bdry == vblock_info[i] &&
           amal_count == amal_mat_indx[i] ) amal_mat_indx[amal_count++] = i;
      if ( amal_count > nz_cnt )
         printf("WARNING in Compress_Matrix : not enough memory.\n");

...
int ML_Epetra::RefMaxwellPreconditioner::ComputePreconditioner(const bool CheckFiltering)
{
  ...
  if(solver22=="multilevel") NodePC=new MultiLevelPreconditioner(*TMT_Matrix_,List22,true);
...
void cms_residual_check(const char * tag, const Epetra_Operator * op,const Epetra_MultiVector& rhs, const Epetra_MultiVector& lhs){
  ...
  double norm_old[NumVectors], norm_new[NumVectors];
  rhs.Norm2(norm_old);
  temp.Norm2(norm_new);
  if(op->Comm().MyPID()==0)
...
double cms_compute_residual(const Epetra_Operator * op,const Epetra_MultiVector& rhs, const Epetra_MultiVector& lhs){
  ...
  op->Apply(lhs,temp);
  temp.Update(1.0,rhs,-1.0);
  double norm_new[rhs.NumVectors()];
  temp.Norm2(norm_new);

  double norm_old[rhs.NumVectors()];
  rhs.Norm2(norm_old);  
  return norm_new[0] / norm_old[0];
}
...
int ML_Epetra::RefMaxwellPreconditioner::ApplyInverse_Implicit_212(const Epetra_MultiVector& B, Epetra_MultiVector& X) const
{
  ...
  /* Setup Temps */  
  Epetra_MultiVector node_sol1(*NodeMap_,NumVectors,false);
  ...
  Epetra_MultiVector edge_sol(*DomainMap_,NumVectors,false);
  ...
  /* Build Edge RHS  (erhs = b - Ms *D0 * yn1) */
  ...
  /* Build Nodal RHS  (nrhs = D0'* (erhs - Ms * ye)) */
  ...
  if(Comm_->MyPID()==0)
    printf("Residual Reduction: %6.4e / %6.4e / %6.4e / %6.4e\n",r1,r2,r3,r4);

...
int Epetra_CrsGraph::RemoveRedundantIndices() {
  ...
  for(i = 0; i < numMyBlockRows; i++) {
    ...
    if(NumIndices > 0) {
      ...
      int j0 = 0;
      jj = Indices[0];
      if(jj > i) CrsGraphData_->LowerTriangular_ = false;
      if(jj < i) CrsGraphData_->UpperTriangular_ = false;
				    ...
				    Epetra_Distributor& Distor) 
{
  ...
  CrsGraphData_->ReAllocateAndCast(Exports, LenExports, IntPacketSize*Nsend);
  CrsGraphData_->ReAllocateAndCast(Imports, LenImports, IntPacketSize*Nrecv);

...
/** \file   Intrepid_HCURL_HEX_In_FEM.hpp
    \brief  Header file for the Intrepid::HCURL_HEX_In_FEM class.
...
/** \class  Intrepid::Basis_HCURL_HEX_In_FEM
...
class Basis_HCURL_HEX_In_FEM : public Basis<Scalar, ArrayScalar> {
private:
...
public:
  ...
  /** \brief  Constructor.
    ...
    */
  Basis_HCURL_HEX_In_FEM( int order , 
			  const ArrayScalar &ptsClosed ,
			  const ArrayScalar &ptsOpen );

  /** \brief Constructor
      \param order     [in] - order of the basis
      \param pointType [in] - provides equispaced lattice points (POINTTYPE_EQUISPACED) or tensor products
                              of Gauss-Lobatto and Gauss points (POINTTYPE_SPECTRAL)
  */
  Basis_HCURL_HEX_In_FEM( int order , const EPointType &pointType );

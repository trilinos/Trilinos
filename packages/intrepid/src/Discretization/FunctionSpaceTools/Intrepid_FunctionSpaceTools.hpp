...
class FunctionSpaceTools {
  ...
  */
  ...
  /** \brief Applies left (row) signs, stored in the user-provided container
      ...
      \f]
             ...
             finite element basis functions; see Sections \ref sec_pullback and Section 
  ...
  */
  ...
  /** \brief Computes point values \a <b>outPointVals</b> of a discrete function
      ...
      \f[
             outPointValues(C,P,*) = \sum_{f=0}^{F-1} \sigma_{f,c} u_{f,c}(x_p)
      \f]
             where \f$\{u_{f,c}\}_{f=0}^{F-1} \f$ is scalar, vector or tensor valued finite element
             basis defined on physical cell \f$\mathcal{C}\f$ and \f$\{\sigma_{f,c}\}_{f=0}^{F-1} \f$

...
/** \file   Intrepid_F0_TET_C2_FEM_FIATDef.hpp
    \brief  Definition file for FEM basis functions of degree 2 for 0-forms on TET cells.
...
void Basis_F0_TET_C2_FEM_FIAT<Scalar>::initialize() {
  ...
  int tags[] = { 0 , 0 , 0 , 1 , 0 , 1 , 0 , 1 , 0 , 2 , 0 , 1 , 0 , 3 , 0 , 1 , 1 , 0 , 0 , 1 , 1 , 1 , 0 , 1 , 1 , 2 , 0 , 1 , 1 , 3 , 0 , 1 , 1 , 4 , 0 , 1 , 1 , 5 , 0 , 1 };
...
template<class Scalar> 
void Basis_F0_TET_C2_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  // Complete polynomial basis of degree 2 (C2) has 10 basis functions on a tetrahedron that are 0-forms in 3D
  int    numFields = 10;
  ...
  for (countPt=0; countPt<numPoints; countPt++) {
    ...
    TEST_FOR_EXCEPTION( !MultiCell<Scalar>::inReferenceCell(CELL_TET, inputPoints[countPt]),
                        ...
                        ">>> ERROR (Basis_F0_TET_C2_FEM_FIAT): Evaluation point is outside the TET reference cell");
  }
#endif
  switch(operatorType) {
    case OPERATOR_VALUE:
    {
      ...
      Teuchos::SerialDenseMatrix<int,Scalar> result(10,numPoints);
    ...
    case OPERATOR_D1:
      ...
      OrthogonalExpansions<Scalar>::tabulate(CELL_TET,2,inputPoints,expansions);
      Teuchos::SerialDenseMatrix<int,Scalar> result(10,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> Dres(10,numPoints);
    ...
    case OPERATOR_D2:
      ...
      OrthogonalExpansions<Scalar>::tabulate(CELL_TET,2,inputPoints,expansions);
      Teuchos::SerialDenseMatrix<int,Scalar> result(10,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> dtmp0(10,10);
      Teuchos::SerialDenseMatrix<int,Scalar> dtmp1(10,10);
      Teuchos::SerialDenseMatrix<int,Scalar> tmp2(10,numPoints);
...
void Basis_F0_TET_C2_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  TEST_FOR_EXCEPTION( (true),
                      std::logic_error,
                      ">>> ERROR (Basis_F0_TET_C2_FEM_FIAT): FEM Basis calling an FV/D member function");
}



template<class Scalar>
int Basis_F0_TET_C2_FEM_FIAT<Scalar>::getNumLocalDof() const {
    return numDof_;   
}
...
template<class Scalar>
int Basis_F0_TET_C2_FEM_FIAT<Scalar>::getLocalDofEnumeration(const LocalDofTag dofTag) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
LocalDofTag Basis_F0_TET_C2_FEM_FIAT<Scalar>::getLocalDofTag(int id) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
const Teuchos::Array<LocalDofTag> & Basis_F0_TET_C2_FEM_FIAT<Scalar>::getAllLocalDofTags() {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
ECell Basis_F0_TET_C2_FEM_FIAT<Scalar>::getCellType() const {
  return CELL_TET;
}
...
template<class Scalar>
EBasis Basis_F0_TET_C2_FEM_FIAT<Scalar>::getBasisType() const {
...
template<class Scalar>
ECoordinates Basis_F0_TET_C2_FEM_FIAT<Scalar>::getCoordinateSystem() const {
...
template<class Scalar>
int Basis_F0_TET_C2_FEM_FIAT<Scalar>::getDegree() const {
  return 1;
}

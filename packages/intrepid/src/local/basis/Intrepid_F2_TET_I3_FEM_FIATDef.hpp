...
/** \file   Intrepid_F2_TET_I3_FEM_FIATDef.hpp
    \brief  Definition file for FEM basis functions of degree 2 for 2-forms on TET cells.
...
void Basis_F2_TET_I3_FEM_FIAT<Scalar>::initialize() {
  ...
  int tags[] = { 2 , 0 , 0 , 6 , 2 , 0 , 1 , 6 , 2 , 0 , 2 , 6 , 2 , 0 , 3 , 6 , 2 , 0 , 4 , 6 , 2 , 0 , 5 , 6 , 2 , 1 , 0 , 6 , 2 , 1 , 1 , 6 , 2 , 1 , 2 , 6 , 2 , 1 , 3 , 6 , 2 , 1 , 4 , 6 , 2 , 1 , 5 , 6 , 2 , 2 , 0 , 6 , 2 , 2 , 1 , 6 , 2 , 2 , 2 , 6 , 2 , 2 , 3 , 6 , 2 , 2 , 4 , 6 , 2 , 2 , 5 , 6 , 2 , 3 , 0 , 6 , 2 , 3 , 1 , 6 , 2 , 3 , 2 , 6 , 2 , 3 , 3 , 6 , 2 , 3 , 4 , 6 , 2 , 3 , 5 , 6 , 3 , 0 , 0 , 12 , 3 , 0 , 1 , 12 , 3 , 0 , 2 , 12 , 3 , 0 , 3 , 12 , 3 , 0 , 4 , 12 , 3 , 0 , 5 , 12 , 3 , 0 , 6 , 12 , 3 , 0 , 7 , 12 , 3 , 0 , 8 , 12 , 3 , 0 , 9 , 12 , 3 , 0 , 10 , 12 , 3 , 0 , 11 , 12 };
...
template<class Scalar> 
void Basis_F2_TET_I3_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  // Incomplete polynomial basis of degree 2 (I2) has 36 basis functions on a tetrahedron that are 2-forms in 3D
  int    numFields = 36;
  ...
  for (countPt=0; countPt<numPoints; countPt++) {
    ...
    TEST_FOR_EXCEPTION( !MultiCell<Scalar>::inReferenceCell(CELL_TET, inputPoints[countPt]),
                        ...
                        ">>> ERROR (Basis_F2_TET_I3_FEM_FIAT): Evaluation point is outside the TET reference cell");
  }
#endif
  switch(operatorType) {
    case OPERATOR_VALUE:   
    ...
    case OPERATOR_D10:
    {
      TEST_FOR_EXCEPTION( true , std::invalid_argument, ">>>ERROR F2_TET_I3_FEM_FIAT: operator not implemented" );
    }
    break; 
    case OPERATOR_CURL:
    {
    ...
    case OPERATOR_DIV:
    {
      ...
      Teuchos::SerialDenseMatrix<int,Scalar> div(36,numPoints);
      div.putScalar(0.0);
      OrthogonalExpansions<Scalar>::tabulate(CELL_TET,3,inputPoints,expansions);
    ...
    default:
      TEST_FOR_EXCEPTION( ( (operatorType != OPERATOR_VALUE) &&
                          ...
                          std::invalid_argument,
                          ">>> ERROR (Basis_F2_TET_I3_FEM_DEFAULT): Invalid operator type");
  }
...
void Basis_F2_TET_I3_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  TEST_FOR_EXCEPTION( (true),
                      std::logic_error,
                      ">>> ERROR (Basis_F2_TET_I3_FEM_FIAT): FEM Basis calling an FV/D member function");
}



template<class Scalar>
int Basis_F2_TET_I3_FEM_FIAT<Scalar>::getNumLocalDof() const {
    return numDof_;   
}
...
template<class Scalar>
int Basis_F2_TET_I3_FEM_FIAT<Scalar>::getLocalDofEnumeration(const LocalDofTag dofTag) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
LocalDofTag Basis_F2_TET_I3_FEM_FIAT<Scalar>::getLocalDofTag(int id) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
const Teuchos::Array<LocalDofTag> & Basis_F2_TET_I3_FEM_FIAT<Scalar>::getAllLocalDofTags() {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
ECell Basis_F2_TET_I3_FEM_FIAT<Scalar>::getCellType() const {
  return CELL_TET;
}
...
template<class Scalar>
EBasis Basis_F2_TET_I3_FEM_FIAT<Scalar>::getBasisType() const {
...
template<class Scalar>
ECoordinates Basis_F2_TET_I3_FEM_FIAT<Scalar>::getCoordinateSystem() const {
...
template<class Scalar>
int Basis_F2_TET_I3_FEM_FIAT<Scalar>::getDegree() const {
  return 1;
}

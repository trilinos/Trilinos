...
/** \file   Intrepid_F2_TRI_I7_FEM_FIATDef.hpp
    \brief  Definition file for FEM basis functions of degree 6 for 2-forms on TRI cells.
...
void Basis_F2_TRI_I7_FEM_FIAT<Scalar>::initialize() {
  ...
  int tags[] = { 1 , 0 , 0 , 7 , 1 , 0 , 1 , 7 , 1 , 0 , 2 , 7 , 1 , 0 , 3 , 7 , 1 , 0 , 4 , 7 , 1 , 0 , 5 , 7 , 1 , 0 , 6 , 7 , 1 , 1 , 0 , 7 , 1 , 1 , 1 , 7 , 1 , 1 , 2 , 7 , 1 , 1 , 3 , 7 , 1 , 1 , 4 , 7 , 1 , 1 , 5 , 7 , 1 , 1 , 6 , 7 , 1 , 2 , 0 , 7 , 1 , 2 , 1 , 7 , 1 , 2 , 2 , 7 , 1 , 2 , 3 , 7 , 1 , 2 , 4 , 7 , 1 , 2 , 5 , 7 , 1 , 2 , 6 , 7 , 2 , 0 , 0 , 42 , 2 , 0 , 1 , 42 , 2 , 0 , 2 , 42 , 2 , 0 , 3 , 42 , 2 , 0 , 4 , 42 , 2 , 0 , 5 , 42 , 2 , 0 , 6 , 42 , 2 , 0 , 7 , 42 , 2 , 0 , 8 , 42 , 2 , 0 , 9 , 42 , 2 , 0 , 10 , 42 , 2 , 0 , 11 , 42 , 2 , 0 , 12 , 42 , 2 , 0 , 13 , 42 , 2 , 0 , 14 , 42 , 2 , 0 , 15 , 42 , 2 , 0 , 16 , 42 , 2 , 0 , 17 , 42 , 2 , 0 , 18 , 42 , 2 , 0 , 19 , 42 , 2 , 0 , 20 , 42 , 2 , 0 , 21 , 42 , 2 , 0 , 22 , 42 , 2 , 0 , 23 , 42 , 2 , 0 , 24 , 42 , 2 , 0 , 25 , 42 , 2 , 0 , 26 , 42 , 2 , 0 , 27 , 42 , 2 , 0 , 28 , 42 , 2 , 0 , 29 , 42 , 2 , 0 , 30 , 42 , 2 , 0 , 31 , 42 , 2 , 0 , 32 , 42 , 2 , 0 , 33 , 42 , 2 , 0 , 34 , 42 , 2 , 0 , 35 , 42 , 2 , 0 , 36 , 42 , 2 , 0 , 37 , 42 , 2 , 0 , 38 , 42 , 2 , 0 , 39 , 42 , 2 , 0 , 40 , 42 , 2 , 0 , 41 , 42 };
...
template<class Scalar> 
void Basis_F2_TRI_I7_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  // Incomplete polynomial basis of degree 6 (I6) has 63 basis functions on a triangle that are 2-forms in 2D
  int    numFields = 63;
  ...
  for (countPt=0; countPt<numPoints; countPt++) {
    ...
    TEST_FOR_EXCEPTION( !MultiCell<Scalar>::inReferenceCell(CELL_TRI, inputPoints[countPt]),
                        ...
                        ">>> ERROR (Basis_F2_TRI_I7_FEM_FIAT): Evaluation point is outside the TRI reference cell");
  }
#endif
  switch(operatorType) {
    case OPERATOR_VALUE:   
    {
      ...
      Teuchos::SerialDenseMatrix<int,Scalar> result(63,numPoints);
    ...
    case OPERATOR_D10:
    {
      TEST_FOR_EXCEPTION( true , std::invalid_argument, ">>>ERROR F2_TRI_I7_FEM_FIAT: operator not implemented" );
    }
    break; 
    case OPERATOR_CURL:
    {
    ...
    {
      Teuchos::SerialDenseMatrix<int,Scalar> expansions(36,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> tmp(36,36);
      Teuchos::SerialDenseMatrix<int,Scalar> div(63,numPoints);
      div.putScalar(0.0);
      OrthogonalExpansions<Scalar>::tabulate(CELL_TRI,7,inputPoints,expansions);
    ...
    default:
      TEST_FOR_EXCEPTION( ( (operatorType != OPERATOR_VALUE) &&
                          ...
                          std::invalid_argument,
                          ">>> ERROR (Basis_F2_TRI_I7_FEM_DEFAULT): Invalid operator type");
  }
...
void Basis_F2_TRI_I7_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  TEST_FOR_EXCEPTION( (true),
                      std::logic_error,
                      ">>> ERROR (Basis_F2_TRI_I7_FEM_FIAT): FEM Basis calling an FV/D member function");
}



template<class Scalar>
int Basis_F2_TRI_I7_FEM_FIAT<Scalar>::getNumLocalDof() const {
    return numDof_;   
}
...
template<class Scalar>
int Basis_F2_TRI_I7_FEM_FIAT<Scalar>::getLocalDofEnumeration(const LocalDofTag dofTag) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
LocalDofTag Basis_F2_TRI_I7_FEM_FIAT<Scalar>::getLocalDofTag(int id) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
const Teuchos::Array<LocalDofTag> & Basis_F2_TRI_I7_FEM_FIAT<Scalar>::getAllLocalDofTags() {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
ECell Basis_F2_TRI_I7_FEM_FIAT<Scalar>::getCellType() const {
  return CELL_TRI;
}
...
template<class Scalar>
EBasis Basis_F2_TRI_I7_FEM_FIAT<Scalar>::getBasisType() const {
...
template<class Scalar>
ECoordinates Basis_F2_TRI_I7_FEM_FIAT<Scalar>::getCoordinateSystem() const {
...
template<class Scalar>
int Basis_F2_TRI_I7_FEM_FIAT<Scalar>::getDegree() const {
  return 1;
}

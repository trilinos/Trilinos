...
/** \file   Intrepid_F0_TET_C4_FEM_FIATDef.hpp
    \brief  Definition file for FEM basis functions of degree 4 for 0-forms on TET cells.
...
void Basis_F0_TET_C4_FEM_FIAT<Scalar>::initialize() {
  ...
  int tags[] = { 0 , 0 , 0 , 1 , 0 , 1 , 0 , 1 , 0 , 2 , 0 , 1 , 0 , 3 , 0 , 1 , 1 , 0 , 0 , 3 , 1 , 0 , 1 , 3 , 1 , 0 , 2 , 3 , 1 , 1 , 0 , 3 , 1 , 1 , 1 , 3 , 1 , 1 , 2 , 3 , 1 , 2 , 0 , 3 , 1 , 2 , 1 , 3 , 1 , 2 , 2 , 3 , 1 , 3 , 0 , 3 , 1 , 3 , 1 , 3 , 1 , 3 , 2 , 3 , 1 , 4 , 0 , 3 , 1 , 4 , 1 , 3 , 1 , 4 , 2 , 3 , 1 , 5 , 0 , 3 , 1 , 5 , 1 , 3 , 1 , 5 , 2 , 3 , 2 , 0 , 0 , 3 , 2 , 0 , 1 , 3 , 2 , 0 , 2 , 3 , 2 , 1 , 0 , 3 , 2 , 1 , 1 , 3 , 2 , 1 , 2 , 3 , 2 , 2 , 0 , 3 , 2 , 2 , 1 , 3 , 2 , 2 , 2 , 3 , 2 , 3 , 0 , 3 , 2 , 3 , 1 , 3 , 2 , 3 , 2 , 3 , 3 , 0 , 0 , 1 };
...
template<class Scalar> 
void Basis_F0_TET_C4_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  // Complete polynomial basis of degree 4 (C4) has 35 basis functions on a tetrahedron that are 0-forms in 3D
  int    numFields = 35;
  ...
  for (countPt=0; countPt<numPoints; countPt++) {
    ...
    TEST_FOR_EXCEPTION( !MultiCell<Scalar>::inReferenceCell(CELL_TET, inputPoints[countPt]),
                        ...
                        ">>> ERROR (Basis_F0_TET_C4_FEM_FIAT): Evaluation point is outside the TET reference cell");
  }
#endif
  switch(operatorType) {
    case OPERATOR_VALUE:
    ...
    case OPERATOR_D1:
      ...
      OrthogonalExpansions<Scalar>::tabulate(CELL_TET,4,inputPoints,expansions);
      Teuchos::SerialDenseMatrix<int,Scalar> result(35,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> Dres(35,numPoints);
    ...
    case OPERATOR_D2:
      ...
      OrthogonalExpansions<Scalar>::tabulate(CELL_TET,4,inputPoints,expansions);
      Teuchos::SerialDenseMatrix<int,Scalar> result(35,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> dtmp0(35,35);
      Teuchos::SerialDenseMatrix<int,Scalar> dtmp1(35,35);
      Teuchos::SerialDenseMatrix<int,Scalar> tmp2(35,numPoints);
...
void Basis_F0_TET_C4_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  TEST_FOR_EXCEPTION( (true),
                      std::logic_error,
                      ">>> ERROR (Basis_F0_TET_C4_FEM_FIAT): FEM Basis calling an FV/D member function");
}



template<class Scalar>
int Basis_F0_TET_C4_FEM_FIAT<Scalar>::getNumLocalDof() const {
    return numDof_;   
}
...
template<class Scalar>
int Basis_F0_TET_C4_FEM_FIAT<Scalar>::getLocalDofEnumeration(const LocalDofTag dofTag) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
LocalDofTag Basis_F0_TET_C4_FEM_FIAT<Scalar>::getLocalDofTag(int id) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
const Teuchos::Array<LocalDofTag> & Basis_F0_TET_C4_FEM_FIAT<Scalar>::getAllLocalDofTags() {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
ECell Basis_F0_TET_C4_FEM_FIAT<Scalar>::getCellType() const {
  return CELL_TET;
}
...
template<class Scalar>
EBasis Basis_F0_TET_C4_FEM_FIAT<Scalar>::getBasisType() const {
...
template<class Scalar>
ECoordinates Basis_F0_TET_C4_FEM_FIAT<Scalar>::getCoordinateSystem() const {
...
template<class Scalar>
int Basis_F0_TET_C4_FEM_FIAT<Scalar>::getDegree() const {
  return 1;
}

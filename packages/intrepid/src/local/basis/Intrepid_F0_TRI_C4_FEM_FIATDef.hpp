...
/** \file   Intrepid_F0_TRI_C4_FEM_FIATDef.hpp
    \brief  Definition file for FEM basis functions of degree 4 for 0-forms on TRI cells.
...
void Basis_F0_TRI_C4_FEM_FIAT<Scalar>::initialize() {
  ...
  int tags[] = { 0 , 0 , 0 , 1 , 0 , 1 , 0 , 1 , 0 , 2 , 0 , 1 , 1 , 0 , 0 , 3 , 1 , 0 , 1 , 3 , 1 , 0 , 2 , 3 , 1 , 1 , 0 , 3 , 1 , 1 , 1 , 3 , 1 , 1 , 2 , 3 , 1 , 2 , 0 , 3 , 1 , 2 , 1 , 3 , 1 , 2 , 2 , 3 , 2 , 0 , 0 , 3 , 2 , 0 , 1 , 3 , 2 , 0 , 2 , 3 };
...
template<class Scalar> 
void Basis_F0_TRI_C4_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  // Complete polynomial basis of degree 4 (C4) has 15 basis functions on a triangle that are 0-forms in 2D
  int    numFields = 15;
  ...
  for (countPt=0; countPt<numPoints; countPt++) {
    ...
    TEST_FOR_EXCEPTION( !MultiCell<Scalar>::inReferenceCell(CELL_TRI, inputPoints[countPt]),
                        ...
                        ">>> ERROR (Basis_F0_TRI_C4_FEM_FIAT): Evaluation point is outside the TRI reference cell");
  }
#endif
  switch(operatorType) {
    case OPERATOR_VALUE:
    {
      ...
      OrthogonalExpansions<Scalar>::tabulate(2,4,inputPoints,expansions);
    ...
    case OPERATOR_D1:
      ...
      OrthogonalExpansions<Scalar>::tabulate(2,4,inputPoints,expansions);
      Teuchos::SerialDenseMatrix<int,Scalar> result(15,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> Dres(15,numPoints);
    ...
    case OPERATOR_D2:
      ...
      OrthogonalExpansions<Scalar>::tabulate(2,4,inputPoints,expansions);
      Teuchos::SerialDenseMatrix<int,Scalar> result(15,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> dtmp0(15,15);
      Teuchos::SerialDenseMatrix<int,Scalar> dtmp1(15,15);
      Teuchos::SerialDenseMatrix<int,Scalar> tmp2(15,numPoints);
...
void Basis_F0_TRI_C4_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  TEST_FOR_EXCEPTION( (true),
                      std::logic_error,
                      ">>> ERROR (Basis_F0_TRI_C4_FEM_FIAT): FEM Basis calling an FV/D member function");
}



template<class Scalar>
int Basis_F0_TRI_C4_FEM_FIAT<Scalar>::getNumLocalDof() const {
    return numDof_;   
}
...
template<class Scalar>
int Basis_F0_TRI_C4_FEM_FIAT<Scalar>::getLocalDofEnumeration(const LocalDofTag dofTag) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
LocalDofTag Basis_F0_TRI_C4_FEM_FIAT<Scalar>::getLocalDofTag(int id) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
const Teuchos::Array<LocalDofTag> & Basis_F0_TRI_C4_FEM_FIAT<Scalar>::getAllLocalDofTags() {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
ECell Basis_F0_TRI_C4_FEM_FIAT<Scalar>::getCellType() const {
  return CELL_TRI;
}
...
template<class Scalar>
EBasis Basis_F0_TRI_C4_FEM_FIAT<Scalar>::getBasisType() const {
...
template<class Scalar>
ECoordinates Basis_F0_TRI_C4_FEM_FIAT<Scalar>::getCoordinateSystem() const {
...
template<class Scalar>
int Basis_F0_TRI_C4_FEM_FIAT<Scalar>::getDegree() const {
  return 1;
}

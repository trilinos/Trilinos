...
/** \file   Intrepid_F0_TRI_C9_FEM_FIATDef.hpp
    \brief  Definition file for FEM basis functions of degree 9 for 0-forms on TRI cells.
...
void Basis_F0_TRI_C9_FEM_FIAT<Scalar>::initialize() {
  ...
  int tags[] = { 0 , 0 , 0 , 1 , 0 , 1 , 0 , 1 , 0 , 2 , 0 , 1 , 1 , 0 , 0 , 8 , 1 , 0 , 1 , 8 , 1 , 0 , 2 , 8 , 1 , 0 , 3 , 8 , 1 , 0 , 4 , 8 , 1 , 0 , 5 , 8 , 1 , 0 , 6 , 8 , 1 , 0 , 7 , 8 , 1 , 1 , 0 , 8 , 1 , 1 , 1 , 8 , 1 , 1 , 2 , 8 , 1 , 1 , 3 , 8 , 1 , 1 , 4 , 8 , 1 , 1 , 5 , 8 , 1 , 1 , 6 , 8 , 1 , 1 , 7 , 8 , 1 , 2 , 0 , 8 , 1 , 2 , 1 , 8 , 1 , 2 , 2 , 8 , 1 , 2 , 3 , 8 , 1 , 2 , 4 , 8 , 1 , 2 , 5 , 8 , 1 , 2 , 6 , 8 , 1 , 2 , 7 , 8 , 2 , 0 , 0 , 28 , 2 , 0 , 1 , 28 , 2 , 0 , 2 , 28 , 2 , 0 , 3 , 28 , 2 , 0 , 4 , 28 , 2 , 0 , 5 , 28 , 2 , 0 , 6 , 28 , 2 , 0 , 7 , 28 , 2 , 0 , 8 , 28 , 2 , 0 , 9 , 28 , 2 , 0 , 10 , 28 , 2 , 0 , 11 , 28 , 2 , 0 , 12 , 28 , 2 , 0 , 13 , 28 , 2 , 0 , 14 , 28 , 2 , 0 , 15 , 28 , 2 , 0 , 16 , 28 , 2 , 0 , 17 , 28 , 2 , 0 , 18 , 28 , 2 , 0 , 19 , 28 , 2 , 0 , 20 , 28 , 2 , 0 , 21 , 28 , 2 , 0 , 22 , 28 , 2 , 0 , 23 , 28 , 2 , 0 , 24 , 28 , 2 , 0 , 25 , 28 , 2 , 0 , 26 , 28 , 2 , 0 , 27 , 28 };
...
template<class Scalar> 
void Basis_F0_TRI_C9_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  // Complete polynomial basis of degree 9 (C9) has 55 basis functions on a triangle that are 0-forms in 2D
  int    numFields = 55;
  ...
  for (countPt=0; countPt<numPoints; countPt++) {
    ...
    TEST_FOR_EXCEPTION( !MultiCell<Scalar>::inReferenceCell(CELL_TRI, inputPoints[countPt]),
                        ...
                        ">>> ERROR (Basis_F0_TRI_C9_FEM_FIAT): Evaluation point is outside the TRI reference cell");
  }
#endif
  switch(operatorType) {
    case OPERATOR_VALUE:
    {
      Teuchos::SerialDenseMatrix<int,Scalar> expansions(55,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> result(55,numPoints);
      OrthogonalExpansions<Scalar>::tabulate(CELL_TRI,9,inputPoints,expansions);
    ...
    case OPERATOR_D1:
      ...
      OrthogonalExpansions<Scalar>::tabulate(CELL_TRI,9,inputPoints,expansions);
      Teuchos::SerialDenseMatrix<int,Scalar> result(55,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> Dres(55,numPoints);
    ...
    case OPERATOR_D2:
      ...
      OrthogonalExpansions<Scalar>::tabulate(CELL_TRI,9,inputPoints,expansions);
      Teuchos::SerialDenseMatrix<int,Scalar> result(55,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> dtmp0(55,55);
      Teuchos::SerialDenseMatrix<int,Scalar> dtmp1(55,55);
      Teuchos::SerialDenseMatrix<int,Scalar> tmp2(55,numPoints);
...
void Basis_F0_TRI_C9_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  TEST_FOR_EXCEPTION( (true),
                      std::logic_error,
                      ">>> ERROR (Basis_F0_TRI_C9_FEM_FIAT): FEM Basis calling an FV/D member function");
}



template<class Scalar>
int Basis_F0_TRI_C9_FEM_FIAT<Scalar>::getNumLocalDof() const {
    return numDof_;   
}
...
template<class Scalar>
int Basis_F0_TRI_C9_FEM_FIAT<Scalar>::getLocalDofEnumeration(const LocalDofTag dofTag) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
LocalDofTag Basis_F0_TRI_C9_FEM_FIAT<Scalar>::getLocalDofTag(int id) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
const Teuchos::Array<LocalDofTag> & Basis_F0_TRI_C9_FEM_FIAT<Scalar>::getAllLocalDofTags() {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
ECell Basis_F0_TRI_C9_FEM_FIAT<Scalar>::getCellType() const {
  return CELL_TRI;
}
...
template<class Scalar>
EBasis Basis_F0_TRI_C9_FEM_FIAT<Scalar>::getBasisType() const {
...
template<class Scalar>
ECoordinates Basis_F0_TRI_C9_FEM_FIAT<Scalar>::getCoordinateSystem() const {
...
template<class Scalar>
int Basis_F0_TRI_C9_FEM_FIAT<Scalar>::getDegree() const {
  return 1;
}

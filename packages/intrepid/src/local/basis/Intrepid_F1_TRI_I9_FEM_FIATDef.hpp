...
/** \file   Intrepid_F1_TRI_I9_FEM_FIATDef.hpp
    \brief  Definition file for FEM basis functions of degree 8 for 1-forms on TRI cells.
...
void Basis_F1_TRI_I9_FEM_FIAT<Scalar>::initialize() {
...
template<class Scalar> 
void Basis_F1_TRI_I9_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  // Incomplete polynomial basis of degree 8 (I8) has 99 basis functions on a triangle that are 1-forms in 2D
  int    numFields = 99;
  ...
  for (countPt=0; countPt<numPoints; countPt++) {
    ...
    TEST_FOR_EXCEPTION( !MultiCell<Scalar>::inReferenceCell(CELL_TRI, inputPoints[countPt]),
                        ...
                        ">>> ERROR (Basis_F1_TRI_I9_FEM_FIAT): Evaluation point is outside the TRI reference cell");
  }
#endif
  switch(operatorType) {
    case OPERATOR_VALUE:   
    {
      ...
      Teuchos::SerialDenseMatrix<int,Scalar> result(99,numPoints);
    ...
    case OPERATOR_D10:
    {
      TEST_FOR_EXCEPTION( true , std::invalid_argument, ">>>ERROR F1_TRI_I9_FEM_FIAT: operator not implemented" );
    }
    break; 
    case OPERATOR_CURL:
    {
      Teuchos::SerialDenseMatrix<int,Scalar> expansions(55,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> tmp(55,55);
      Teuchos::SerialDenseMatrix<int,Scalar> curl(99,numPoints);
      OrthogonalExpansions<Scalar>::tabulate(CELL_TRI,9,inputPoints,expansions);
    ...
    default:
      TEST_FOR_EXCEPTION( ( (operatorType != OPERATOR_VALUE) &&
                          ...
                          std::invalid_argument,
                          ">>> ERROR (Basis_F1_TRI_I9_FEM_DEFAULT): Invalid operator type");
  }
...
void Basis_F1_TRI_I9_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  TEST_FOR_EXCEPTION( (true),
                      std::logic_error,
                      ">>> ERROR (Basis_F1_TRI_I9_FEM_FIAT): FEM Basis calling an FV/D member function");
}



template<class Scalar>
int Basis_F1_TRI_I9_FEM_FIAT<Scalar>::getNumLocalDof() const {
    return numDof_;   
}
...
template<class Scalar>
int Basis_F1_TRI_I9_FEM_FIAT<Scalar>::getLocalDofEnumeration(const LocalDofTag dofTag) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
LocalDofTag Basis_F1_TRI_I9_FEM_FIAT<Scalar>::getLocalDofTag(int id) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
const Teuchos::Array<LocalDofTag> & Basis_F1_TRI_I9_FEM_FIAT<Scalar>::getAllLocalDofTags() {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
ECell Basis_F1_TRI_I9_FEM_FIAT<Scalar>::getCellType() const {
  return CELL_TRI;
}
...
template<class Scalar>
EBasis Basis_F1_TRI_I9_FEM_FIAT<Scalar>::getBasisType() const {
...
template<class Scalar>
ECoordinates Basis_F1_TRI_I9_FEM_FIAT<Scalar>::getCoordinateSystem() const {
...
template<class Scalar>
int Basis_F1_TRI_I9_FEM_FIAT<Scalar>::getDegree() const {
  return 1;
}

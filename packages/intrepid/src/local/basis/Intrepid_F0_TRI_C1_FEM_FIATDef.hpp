...
/** \file   Intrepid_F0_TRI_C1_FEM_FIATDef.hpp
...
void Basis_F0_TRI_C1_FEM_FIAT<Scalar>::initialize() {
...
template<class Scalar> 
void Basis_F0_TRI_C1_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  switch(operatorType) {
    case OPERATOR_VALUE:
    {
      ...
      OrthogonalExpansions<Scalar>::tabulate(2,1,inputPoints,expansions);
    ...
    case OPERATOR_D1:
      ...
      OrthogonalExpansions<Scalar>::tabulate(2,1,inputPoints,expansions);
      Teuchos::SerialDenseMatrix<int,Scalar> result(3,numPoints);
      Teuchos::SerialDenseMatrix<int,Scalar> Dres(3,numPoints);
...
void Basis_F0_TRI_C1_FEM_FIAT<Scalar>::getValues(FieldContainer<Scalar>&                  outputValues,
  ...
  TEST_FOR_EXCEPTION( (true),
                      std::logic_error,
                      ">>> ERROR (Basis_F0_TRI_C1_FEM_FIAT): FEM Basis calling an FV/D member function");
}



template<class Scalar>
int Basis_F0_TRI_C1_FEM_FIAT<Scalar>::getNumLocalDof() const {
    return numDof_;   
}
...
template<class Scalar>
int Basis_F0_TRI_C1_FEM_FIAT<Scalar>::getLocalDofEnumeration(const LocalDofTag dofTag) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
LocalDofTag Basis_F0_TRI_C1_FEM_FIAT<Scalar>::getLocalDofTag(int id) {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
const Teuchos::Array<LocalDofTag> & Basis_F0_TRI_C1_FEM_FIAT<Scalar>::getAllLocalDofTags() {
  if (!isSet_) {
    initialize();
    isSet_ = true;
  }
...
template<class Scalar>
ECell Basis_F0_TRI_C1_FEM_FIAT<Scalar>::getCellType() const {
  return CELL_TRI;
}
...
template<class Scalar>
EBasis Basis_F0_TRI_C1_FEM_FIAT<Scalar>::getBasisType() const {
...
template<class Scalar>
ECoordinates Basis_F0_TRI_C1_FEM_FIAT<Scalar>::getCoordinateSystem() const {
...
template<class Scalar>
int Basis_F0_TRI_C1_FEM_FIAT<Scalar>::getDegree() const {
  return 1;
}

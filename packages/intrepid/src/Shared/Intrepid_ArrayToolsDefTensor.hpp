...
namespace Intrepid {
  ...
  void ArrayTools::matvecProductDataField(ArrayOutFields &       outputFields,
     ...
     */
    // (1) inputData is (C, P, D, D) and 2 <= D <= 3 is required  
    TEST_FOR_EXCEPTION( !requireRankRange(errmsg, inputData,  4,4), 
                        std::invalid_argument, errmsg);
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, inputData, 2,  2,3), 
                        std::invalid_argument, errmsg);
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, inputData, 3,  2,3), 
                        std::invalid_argument, errmsg);
    // (2) inputFields is (C, F, P, D) or (F, P, D) and 2 <= (D=dimension(rank - 1)) <= 3 is required. 
    TEST_FOR_EXCEPTION( !requireRankRange(errmsg, inputFields, 3,4), std::invalid_argument, errmsg);
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, inputFields, inputFields.rank()-1,  2,3), 
     ...
     *      (3) outputFields vs. inputFields
     *     
     *   Cross-check (2): outputFields(C,F,P,D) vs. inputData(C,P,D,D): dimensions C, P, D must match
     */
    TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg,
                                               outputFields, 0,2,3,3,
                                               inputData,    0,1,2,3),
                        std::invalid_argument, errmsg);    
    /*
     *   Cross-checks (1,3):
     */
    if( inputFields.rank() == 4) {
      // Cross-check (1): inputData(C,P,D,D) vs. inputFields(C,F,P,D):  dimensions  C, P, D must match
      TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg, 
                                                 inputData,    0,1,2,3, 
                                                 inputFields,  0,2,3,3),
                          std::invalid_argument, errmsg);  
      ...
      // Cross-check (1): inputData(C,P,D,D) vs. inputFields(F,P,D): dimensions  P, D must match
      TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg, 
                                                 inputData,    1,2,3, 
                                                 inputFields,  1,2,2),
                          std::invalid_argument, errmsg);      
        ...
        for(int field = 0; field < outputFields.dimension(1); field++){
          for(int point = 0; point < outputFields.dimension(2); point++){
            for(int row = 0; row < outputFields.dimension(3); row++){
              ...
              for(int col = 0; col < inputData.dimension(3); col++){
  ...
  void ArrayTools::matvecProductDataData(ArrayOutData &            outputData,
    ...
    /*
     *   Check array rank and spatial dimension range (if applicable)
     *      (1) inputDataLeft(C,P,D,D);    
     *      (2) inputDataRight(C,P,D) or (P,D);   
     *      (3) outputData(C,P,D)
     */
    // (1) inputDataLeft is (C, P, D, D) and 2 <= D <= 3 is required  
    TEST_FOR_EXCEPTION( !requireRankRange(errmsg, inputDataLeft,  4,4), 
                        std::invalid_argument, errmsg);
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, inputDataLeft, 2,  2,3), 
                        std::invalid_argument, errmsg);
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, inputDataLeft, 3,  2,3), 
                        std::invalid_argument, errmsg);
    // (2) inputDataRight is (C, P, D) or (P, D) and 2 <= (D=dimension(rank - 1)) <= 3 is required. 
    TEST_FOR_EXCEPTION( !requireRankRange(errmsg, inputDataRight,  2,3), 
                        std::invalid_argument, errmsg);
     ...
     *      (3) outputData    vs. inputDataRight 
     *
     *   Cross-check (2): outputData(C,P,D) vs. inputDataLeft(C,P,D,D): dimensions C, P, D must match
     */
    TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg,
                                               outputData,    0,1,2,2,
                                               inputDataLeft, 0,1,2,3),
                        std::invalid_argument, errmsg);    
    /*
     *   Cross-checks (1,3):
     */
    if( inputDataRight.rank() == 3) {
      // Cross-check (1): inputDataLeft(C,P,D,D) vs. inputDataRight(C,P,D):  dimensions  C, P, D must match
      TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg, 
                                                 inputDataLeft,  0,1,2,3, 
                                                 inputDataRight, 0,1,2,2),
                          std::invalid_argument, errmsg);  
      ...
      // Cross-check (1): inputDataLeft(C,P,D,D) vs. inputDataRight(P,D): dimensions  P, D must match
      TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg, 
                                                 inputDataLeft,  1,2,3, 
                                                 inputDataRight, 0,1,1),
                          std::invalid_argument, errmsg);      
      ...
      for(int cell = 0; cell < inputDataLeft.dimension(0); cell++){
        for(int point = 0; point < inputDataLeft.dimension(1); point++){
          for(int row = 0; row < inputDataLeft.dimension(2); row++){
            ...
            for(int col = 0; col < inputDataLeft.dimension(3); col++){
  ...
  void ArrayTools::matmatProductDataField(ArrayOutFields &       outputFields,
    ...
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, inputData, 3,  2,3), 
                        std::invalid_argument, errmsg);
    // (2) inputFields is (C,F,P,D,D) or (F,P,D,D) and 2 <= (dimension(rank-1), (rank-2)) <= 3 is required. 
    TEST_FOR_EXCEPTION( !requireRankRange(errmsg, inputFields, 4,5), std::invalid_argument, errmsg);
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, inputFields, inputFields.rank()-1,  2,3), 
                        std::invalid_argument, errmsg);
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, inputFields, inputFields.rank()-2,  2,3), 
                        std::invalid_argument, errmsg);
    // (3) outputFields is (C,F,P,D,D)
    TEST_FOR_EXCEPTION( !requireRankRange(errmsg, outputFields,  5,5), std::invalid_argument, errmsg);      
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, outputFields, 3,  2,3), 
     ...
     *      (3) outputFields vs. inputFields 
     *
     *   Cross-check (2): outputFields(C,F,P,D,D) vs. inputData(C,P,D,D): dimensions C, P, D must match
     */
    TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg,
                                               outputFields, 0,2,3,4,
                                               inputData,    0,1,2,3),
    ...
    if( inputFields.rank() == 5) {
      // Cross-check (1): inputData(C,P,D,D) vs. inputFields(C,F,P,D,D):  dimensions  C, P, D must match
      TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg, 
                                                 inputData,    0,1,2,3, 
                                                 inputFields,  0,2,3,4),
                          std::invalid_argument, errmsg);  
      ...
      // Cross-check (1): inputData(C,P,D,D) vs. inputFields(F,P,D,D): dimensions  P, D must match
      TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg, 
                                                 inputData,    1,2,3, 
                                                 inputFields,  1,2,3),
                          std::invalid_argument, errmsg);      
          ...
          for(int point = 0; point < outputFields.dimension(2); point++){
            for(int row = 0; row < outputFields.dimension(3); row++){
              for(int col = 0; col < outputFields.dimension(4); col++){
                ...
                for(int i = 0; i < inputData.dimension(3); i++){
                  outputFields(cell, field, point, row, col) += \
  ...
  void ArrayTools::matmatProductDataData(ArrayOutData &            outputData,
    ...
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, inputDataLeft, 3,  2,3), 
                        std::invalid_argument, errmsg);
    // (2) inputDataRight is (C,P,D,D) or (P,D,D) and 2 <= (D=dimension(rank-1),(rank-2)) <= 3 is required. 
    TEST_FOR_EXCEPTION( !requireRankRange(errmsg, inputDataRight,  2,3), 
                        std::invalid_argument, errmsg);
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, inputDataRight, inputDataRight.rank()-1,  2,3), 
                        std::invalid_argument, errmsg);
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, inputDataRight, inputDataRight.rank()-2,  2,3), 
                        std::invalid_argument, errmsg);
    // (3) outputData is (C,P,D,D)
    TEST_FOR_EXCEPTION( !requireRankRange(errmsg, outputData, 4, 4), std::invalid_argument, errmsg);      
    TEST_FOR_EXCEPTION( !requireDimensionRange(errmsg, outputData, 2,  2,3), 
     ...
     *      (3) outputData    vs. inputDataRight 
     *
     *   Cross-check (2): outputData(C,P,D,D) vs. inputDataLeft(C,P,D,D): all dimensions C, P, D must match
     */
    TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg, outputData, inputDataLeft),
                        std::invalid_argument, errmsg);    
    /*
     *   Cross-checks (1,3):
     */
    if( inputDataRight.rank() == 4) {
      // Cross-check (1): inputDataLeft(C,P,D,D) vs. inputDataRight(C,P,D,D):  all dimensions  C, P, D must match
      TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg, inputDataLeft, inputDataRight),
                          std::invalid_argument, errmsg);  
      ...
      // Cross-check (1): inputDataLeft(C,P,D,D) vs. inputDataRight(P,D,D): dimensions  P, D must match
      TEST_FOR_EXCEPTION( !requireDimensionMatch(errmsg, 
                                                 inputDataLeft,  1,2,3, 
        ...
        for(int point = 0; point < inputDataLeft.dimension(1); point++){
          for(int row = 0; row < inputDataLeft.dimension(2); row++){
            for(int col = 0; col < inputDataLeft.dimension(3); col++){
              ...
              for(int i = 0; i < inputDataLeft.dimension(3); i++){
                outputData(cell, point, row, col) += \

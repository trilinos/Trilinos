...
/** \file test_01.cpp
\brief  Unit tests for the Intrepid::Basis_F0_TET_C7_FEM_FIAT class.
...
int main(int argc, char *argv[]) {
  ...
  *outStream \
    << "===============================================================================\n" \
    << "|                                                                             |\n" \
    << "|                 Unit Test (Basis_F0_TET_C7_FEM_FIAT)                                   |\n" \
    << "|                                                                             |\n" \
  ...
  *outStream \
  ...
  try{
	...
	if ( ( (fabscur < 2.499872e+02 * INTREPID_FIAT_TOL)
	       && (std::fabs(vals[i]) > 2.499872e+02 * INTREPID_FIAT_TOL ) ) 
	     || ( ( fabscur >= 2.499872e+02 * INTREPID_FIAT_TOL )
		  && ( std::fabs( vals[i] - cur ) / fabscur ) >= 2.499872e+02 * INTREPID_FIAT_TOL  ) ) {
	  errorFlag++;
	  *outStream << std::setw(70) << "^^^^----FAILURE!" << "\n";
	...
	GRADfile >> cur;
	fabscur = std::fabs(cur);
	if ( ( (fabscur < 4.826302e+03 * INTREPID_FIAT_TOL)
	       && (std::fabs(vals[i]) > 4.826302e+03 * INTREPID_FIAT_TOL ) ) 
	     || ( ( fabscur >= 4.826302e+03 * INTREPID_FIAT_TOL )
		  && ( std::fabs( vals[i] - cur ) / fabscur ) >= 4.826302e+03 * INTREPID_FIAT_TOL  ) ) {
	  errorFlag++;
	  *outStream << std::setw(70) << "^^^^----FAILURE!" << "\n";
	  *outStream << std::setw(70) << " Wrong basis values" << "\n";
	  *outStream << i << " " << vals[i] << " " << cur << "\n";
	...
	D2file >> cur;
	fabscur = std::fabs(cur);
	if ( ( (fabscur < 9.317753e+04 * INTREPID_FIAT_TOL)
	       && (std::fabs(vals[i]) > 9.317753e+04 * INTREPID_FIAT_TOL ) ) 
	     || ( ( fabscur >= 9.317753e+04 * INTREPID_FIAT_TOL )
		  && ( std::fabs( vals[i] - cur ) / fabscur ) >= 9.317753e+04 * INTREPID_FIAT_TOL  ) ) {
	  errorFlag++;
	  *outStream << std::setw(70) << "^^^^----FAILURE!" << "\n";
	  *outStream << std::setw(70) << " Wrong basis values" << "\n";
	  *outStream << i << " " << vals[i] << " " << cur << "\n";
	...
	D3file >> cur;
	fabscur = std::fabs(cur);
	if ( ( (fabscur < 1.798904e+06 * INTREPID_FIAT_TOL)
	       && (std::fabs(vals[i]) > 1.798904e+06 * INTREPID_FIAT_TOL ) ) 
	     || ( ( fabscur >= 1.798904e+06 * INTREPID_FIAT_TOL )
		  && ( std::fabs( vals[i] - cur ) / fabscur ) >= 1.798904e+06 * INTREPID_FIAT_TOL  ) ) {
	  errorFlag++;
	  *outStream << std::setw(70) << "^^^^----FAILURE!" << "\n";
	  *outStream << std::setw(70) << " Wrong basis values" << "\n";
	  *outStream << i << " " << vals[i] << " " << cur << "\n";

  ...
  *outStream \
  ...
  try{
     ...
     for (int dim=3; dim>0; dim--) {
      ...
      for (int i=0; i<ma_x_x_d_d.getSize(); i++) {
        ma_x_x_d_d[i] = Teuchos::ScalarTraits<double>::random();
      }
      for (int i=0; i<va_x_x_d.getSize(); i++) {
      ...
      *outStream << ma_x_x_d_d << mb_x_x_d_d << mc_x_x_d_d;
      ...
      rst::subtract(&mc_x_x_d_d[0], &ma_x_x_d_d[0], ma_x_x_d_d.getSize()); // C = C - A ~= 0 

      if (rst::vectorNorm(&mc_x_x_d_d[0], mc_x_x_d_d.getSize(), NORM_ONE) > zero) {
      ...
      *outStream << detA_x_x << detB_x_x;
      ...
      if ( (rst::dot(&detA_x_x[0], &detB_x_x[0], detA_x_x.getSize()) - (double)(i0*i1)) > zero) {
        *outStream << "\n\nINCORRECT det\n\n" ;
        errorFlag = -1000;
      }

      *outStream << "\n det(A)*det(inv(A)) = " <<
                    rst::det(&ma_x_x_d_d[0], ma_x_x_d_d.getDimension(3))*rst::det(&mb_x_x_d_d[0], mb_x_x_d_d.getDimension(3))
                 << "\n";

      if ( (rst::det(&ma_x_x_d_d[0], ma_x_x_d_d.getDimension(3))*
            rst::det(&mb_x_x_d_d[0], mb_x_x_d_d.getDimension(3)) - (double)1) > zero) {
        *outStream << "\n\nINCORRECT det\n\n" ;
        errorFlag = -1000;
      }
    ...
    for (int dim=3; dim>0; dim--) {
      ...
      // fill with random numbers
      for (int i=0; i<ma_x_d_d.getSize(); i++) {
        ma_x_d_d[i] = Teuchos::ScalarTraits<double>::random();
      }
      for (int i=0; i<va_x_d.getSize(); i++) {
      ...
      *outStream << ma_x_d_d << mb_x_d_d << mc_x_d_d;
      ...
      rst::subtract(&mc_x_d_d[0], &ma_x_d_d[0], ma_x_d_d.getSize()); // C = C - A ~= 0 

      if (rst::vectorNorm(&mc_x_d_d[0], mc_x_d_d.getSize(), NORM_ONE) > zero) {
        *outStream << "\n\nINCORRECT inverse OR subtract OR vectorNorm\n\n";
        errorFlag = -1000;
      }
      ...
      *outStream << detA_x << detB_x;
      ...
      if ( (rst::dot(&detA_x[0], &detB_x[0], detA_x.getSize()) - (double)i0) > zero) {
        *outStream << "\n\nINCORRECT det\n\n" ;
        errorFlag = -1000;
      }

      *outStream << "\n det(A)*det(inv(A)) = " <<
                    rst::det(&ma_x_d_d[0], ma_x_d_d.getDimension(2))*rst::det(&mb_x_d_d[0], mb_x_d_d.getDimension(2))
                 << "\n";

      if ( (rst::det(&ma_x_d_d[0], ma_x_d_d.getDimension(2))*
            rst::det(&mb_x_d_d[0], mb_x_d_d.getDimension(2)) - (double)1) > zero) {
        *outStream << "\n\nINCORRECT det\n\n" ;
        errorFlag = -1000;
      }

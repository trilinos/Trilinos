...
void mexFunction(int nOutput, mxArray *pOutput[], int nInput, const mxArray *pInput[])
{
    ...
    std::string descriptor =
            ("\nintrepid_computeEdgeMeasure ..... MEX interface for the Intrepid (Trilinos) function Intrepid::FunctionSpaceTools::computeEdgeMeasure.\n\n"
                    ...
                    "\t<1-in/out> outMeasures = Output array with weighted face measures (2D array of size [#cubPoints x #cells])\n"
                    "\t<2-in>     inJacobians = Input array containing cell Jacobians (4D array of size [#spaceDim x #spaceDim x #cubPoints x #cells])\n"
                    "\t<3-in>     inWeights = Input integration weights (1D array of size [#cubPoints])\n"
                    "\t<4-in>     whichEdge = Index of the face subcell relative to the parent cell; defines the domain of integration (int)\n"
    ...
    try
    {
        FunctionSpaceTools::computeFaceMeasure<double>(cellMeasures, cellJac, cubWeights, whichEdge, *(cellTopo.get()));

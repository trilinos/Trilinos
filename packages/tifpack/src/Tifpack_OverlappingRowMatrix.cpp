...
Tifpack_OverlappingRowMatrix(const RCP<const Tpetra_RowMatrix>& Matrix_in,
                            int OverlapLevel_in)  :
  Matrix_(Matrix_in),
  OverlapLevel_(OverlapLevel_in)
{
  ...
  RCP<Tpetra_Map> TmpMap;
  RCP<Tpetra_CrsMatrix> TmpMatrix; 
  RCP<Tpetra_Import> TmpImporter;
  ...
  for (int overlap = 0 ; overlap < OverlapLevel_in ; ++overlap) {
    ...
    TmpMap = rcp( new Tpetra_Map(-1,count, &list[0],0,Comm()) ); 

    TmpMatrix = rcp( new Tpetra_CrsMatrix(Copy,*TmpMap,0) ); 

    TmpImporter = rcp( new Tpetra_Import(*TmpMap,A().RowMatrixRowMap()) ); 
  ...
  Map_ = rcp( new Tpetra_Map(-1, NumMyRowsA_ + ExtElements.size(),
	  			      &list[0], 0, Comm()) );
# ifdef TIFPACK_NODE_AWARE_CODE
  colMap_ = Map_;
# endif
  ...
  ExtMap_ = rcp( new Tpetra_Map(-1,ExtElements.size(),
					 &ExtElements[0],0,A().Comm()) );
  ExtMatrix_ = rcp( new Tpetra_CrsMatrix(Copy,*ExtMap_,*Map_,0) ); 

  ExtImporter_ = rcp( new Tpetra_Import(*ExtMap_,A().RowMatrixRowMap()) ); 
  ExtMatrix_->Import(A(),*ExtImporter_,Insert); 
  ExtMatrix_->FillComplete(A().OperatorDomainMap(),*Map_);

  Importer_ = rcp( new Tpetra_Import(*Map_,A().RowMatrixRowMap()) );
...
# ifdef HAVE_MPI
  const Tpetra_MpiComm *pComm = dynamic_cast<const Tpetra_MpiComm*>( &Comm() );
  assert(pComm != NULL);
  MPI_Comm nodeMPIComm;
  MPI_Comm_split(pComm->Comm(),nodeID,pComm->MyPID(),&nodeMPIComm);
  Tpetra_MpiComm *nodeComm = new Tpetra_MpiComm(nodeMPIComm);
# else
  Tpetra_SerialComm *nodeComm =  dynamic_cast<Tpetra_MpiComm*>( &(Matrix_in->RowMatrixRowMap().Comm()) );
# endif
...
*/
  ...
  for (int overlap = 0 ; overlap < OverlapLevel_in ; ++overlap)
  {
    ...
    //ghostTable = Tifpack::HashTable(3 * A().RowMatrixColMap().NumMyElements() );
    //colMapTable = Tifpack::HashTable(3 * A().RowMatrixColMap().NumMyElements() );

    
    // For each column ID, determine the owning node (as opposed to core)
    // ID of the corresponding row.
    Tpetra_IntVector colIdList( *ColMap );
    Tpetra_IntVector rowIdList(*DomainMap);
    rowIdList.PutValue(nodeID);  
    Teuchos::RCP<Tpetra_Import> nodeIdImporter = rcp(new Tpetra_Import( *ColMap, *DomainMap ));
...
#   endif //ifdef HAVE_MPI
    ...
    Tpetra_IntVector ov_colIdList( TmpMatrix->ColMap() );
    ov_colIdList.PutValue(-1);
    Tpetra_IntVector ov_rowIdList( TmpMatrix->RowMap() );
    ov_rowIdList.PutValue(nodeID);  
    Teuchos::RCP<Tpetra_Import> ov_nodeIdImporter = rcp(new Tpetra_Import( TmpMatrix->ColMap(), TmpMatrix->RowMap()));
    ...
    ov_colIdList.PutValue(-1);
    Tpetra_IntVector ArowIdList( A().RowMatrixRowMap() );
    ArowIdList.PutValue(nodeID);
    nodeIdImporter = rcp(new Tpetra_Import( TmpMatrix->ColMap(), A().RowMatrixRowMap() ));
...
*/
  ...
  Map_ = rcp( new Tpetra_Map(-1, NumMyRowsA_ + ghostElements.size(), &rowList[0], 0, Comm()) );
  ...
  //colMap_ = rcp( new Tpetra_Map(-1, colMapElements.size(), &colList[0], 0, Comm()) );
  ...
  // column map for the overlapped matrix (local + overlap)
  colMap_ = rcp( new Tpetra_Map(-1, A().RowMatrixColMap().NumMyElements() + colMapElements.size(), &colList[0], 0, Comm()) );
  ...
  try {
    ...
    RCP<Tpetra_Map> nodeMap_ = rcp( new Tpetra_Map(-1,NumMyRowsA_ + ghostElements.size(),&rowList[0],0,*nodeComm) );
    //Tpetra_Map* nodeMap_ = new Tpetra_Map(-1,NumMyRowsA_ + ghostElements.size(),&rowList[0],0,*nodeComm) ;
   ...
   /* The column map *must* be sorted: first locals, then ghosts.
    ...
    /* This is a little gotcha.  It appears that the ordering of the column map's local entries
       ...
       MakeColMap() in Tpetra_CrsGraph.cpp, line 1072. */
...
*/
    ...
    colMap_ = rcp( new Tpetra_Map(-1,numMyElts,&*mySortedGlobalElts,indexBase,Comm()) );
...
//EPETRA_CHK_ERR(EpetraExt::OperatorToMatlabFile("Ainv.dat", AInvOp));
    ...
    printf("** * Tifpack_OverlappingRowmatrix ctor: problem creating column map * **\n\n");
  }
...
*/
  ...
  // now build the map corresponding to all the external nodes
  // (with respect to A().RowMatrixRowMap().
  ExtMap_ = rcp( new Tpetra_Map(-1,ghostElements.size(), &ghostElements[0],0,Comm()) );
  ExtMatrix_ = rcp( new Tpetra_CrsMatrix(Copy,*ExtMap_,*colMap_,0) ); 
...
*/
  ...
  Importer_ = rcp( new Tpetra_Import(*Map_,A().RowMatrixRowMap()) ); //FIXME is this right?!
...
/*
  ...
  if (Comm().MyPID()==0)
    printf("=======================================\nTifpack_OverlappingRowMatrix Importer_\n=======================================\n"); fflush(stdout);
...
} //Tifpack_OverlappingRowMatrix() ctor for more than one core
#endif //ifdef TIFPACK_NODE_AWARE_CODE
...
Multiply(bool TransA, const Tpetra_MultiVector& X, Tpetra_MultiVector& Y) const
{
  ...
  for (int i = 0 ; i < NumMyRowsA_ ; ++i) {
    for (int k = 0 ; k < NumVectors ; ++k) {
      int Nnz;
      TIFPACK_CHK_ERR(A().ExtractMyRowCopy(i,MaxNumEntries_,Nnz, 
                                          &Val[0], &Ind[0]));
      for (int j = 0 ; j < Nnz ; ++j) {
  ...
  for (int i = 0 ; i < NumMyRowsB_ ; ++i) {
    for (int k = 0 ; k < NumVectors ; ++k) {
      int Nnz;
      TIFPACK_CHK_ERR(B().ExtractMyRowCopy(i,MaxNumEntries_,Nnz, 
                                          &Val[0], &Ind[0]));
      for (int j = 0 ; j < Nnz ; ++j) {
...
// ======================================================================
#ifndef TIFPACK_NODE_AWARE_CODE
Tpetra_RowMatrix& Tifpack_OverlappingRowMatrix::B() const
{
  return(*ExtMatrix_);
}
#endif
// ======================================================================
const Tpetra_BlockMap& Tifpack_OverlappingRowMatrix::Map() const
{
  return(*Map_);
}

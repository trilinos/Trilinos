...
//==============================================================================
Tifpack_ILU::Tifpack_ILU(Tpetra_RowMatrix* Matrix_in) :
...
//==========================================================================
int Tifpack_ILU::SetParameters(Teuchos::ParameterList& List)
{
...
//==========================================================================
int Tifpack_ILU::ComputeSetup() 
{

#ifdef ENABLE_TIFPACK_ILU_TEUCHOS_TIMERS
  TEUCHOS_FUNC_TIME_MONITOR("Tifpack_ILU::ComputeSetup");
#endif
  ...
  double *DV;
  TIFPACK_CHK_ERR(D_->ExtractView(&DV)); // Get view of diagonal

  // First we copy the user's matrix into L and U, regardless of fill level

  for (i=0; i< NumMyRows(); i++) {

    TIFPACK_CHK_ERR(Matrix().ExtractMyRowCopy(i, MaxNumEntries, NumIn, &InV[0], &InI[0])); // Get Values and Indices
	...
	DV[i] += Rthresh_ * InV[j] + EPETRA_SGN(InV[j]) * Athresh_; // Store perturbed diagonal in Tpetra_Vector D_
      ...
      else if (k < 0) {TIFPACK_CHK_ERR(-4);} // Out of range
	...
	TIFPACK_CHK_ERR(L_->InsertMyValues(i, NumL, &LV[0], &LI[0]));
      }
    }
	...
	TIFPACK_CHK_ERR(U_->InsertMyValues(i, NumU, &UV[0], &UI[0]));
      }
    }
  ...
  if (!ReplaceValues) {
    ...
    TIFPACK_CHK_ERR(L_->FillComplete((L_->RowMatrixColMap()), *L_RangeMap_));
    TIFPACK_CHK_ERR(U_->FillComplete(*U_DomainMap_, U_->RowMatrixRowMap()));
  }

  // At this point L and U have the values of A in the structure of L and U, and diagonal vector D
  int TotalNonzeroDiags = 0;
  TIFPACK_CHK_ERR(Graph().L_Graph().RowMap().Comm().SumAll(&NumNonzeroDiags, &TotalNonzeroDiags, 1));
...
//==========================================================================
int Tifpack_ILU::Initialize() 
{

#ifdef ENABLE_TIFPACK_ILU_TEUCHOS_TIMERS
  TEUCHOS_FUNC_TIME_MONITOR("Tifpack_ILU::Initialize");
#endif
  ...
  if (CrsMatrix == 0) {
    ...
    for (int i = 0 ; i < A_->NumMyRows() ; ++i) {
      ...
      TIFPACK_CHK_ERR(A_->ExtractMyRowCopy(i, size, NumEntries, 
					  &Values[0], &Indices[0]));
      // convert to global indices
      for (int j = 0 ; j < NumEntries ; ++j) {
	Indices[j] = A_->RowMatrixColMap().GID(Indices[j]); 
      }
      TIFPACK_CHK_ERR(CrsGraph_->InsertGlobalIndices(GlobalRow,NumEntries,
						   &Indices[0]));
    }
    
    TIFPACK_CHK_ERR(CrsGraph_->FillComplete(A_->RowMatrixRowMap(),
					  A_->RowMatrixRowMap()));

    // always overlap zero, wider overlap will be handled
    // by the AdditiveSchwarz preconditioner.
    Graph_ = rcp(new Tifpack_IlukGraph(*CrsGraph_, LevelOfFill_, 0));

  }
  else {
    // see comment above for the overlap.
    Graph_ = rcp(new Tifpack_IlukGraph(CrsMatrix->Graph(), LevelOfFill_, 0));
  }

  if (Graph_.get() == 0)
    TIFPACK_CHK_ERR(-5); // memory allocation error
  TIFPACK_CHK_ERR(Graph_->ConstructFilledGraph());
...
//==========================================================================
int Tifpack_ILU::Compute() 
{

#ifdef ENABLE_TIFPACK_ILU_TEUCHOS_TIMERS
  TEUCHOS_FUNC_TIME_MONITOR("Tifpack_ILU::Compute");
#endif
  ...
  for (i = 0; i < NumMyRows(); ++i) {
    ...
    NumIn = MaxNumEntries;
    TIFPACK_CHK_ERR(L_->ExtractMyRowCopy(i, NumIn, NumL, &InV[0], &InI[0]));
    LV = &InV[0];
    LI = &InI[0];
    ...
    TIFPACK_CHK_ERR(U_->ExtractMyRowCopy(i, NumIn-NumL-1, NumU, &InV[NumL+1], &InI[NumL+1]));
    NumIn = NumL+NumU+1;
    UV = &InV[NumL+1];
    UI = &InI[NumL+1];
    ...
    for (int jj=0; jj<NumL; jj++) {
      ...
      TIFPACK_CHK_ERR(U_->ExtractMyRowView(j, NumUU, UUV, UUI)); // View of row above
    ...
    if (NumL) {
      TIFPACK_CHK_ERR(L_->ReplaceMyValues(i, NumL, LV, LI));  // Replace current row of L
    }
    ...
    if (NumU) {
      TIFPACK_CHK_ERR(U_->ReplaceMyValues(i, NumU, UV, UI));  // Replace current row of L and U
    }
  ...
  TIFPACK_CHK_ERR(Graph().L_Graph().RowMap().Comm().SumAll(&current_flops, &total_flops, 1)); // Get total madds across all PEs
                      ...
                      Tpetra_MultiVector& Y) const 
{
...
#ifdef ENABLE_TIFPACK_ILU_TEUCHOS_TIMERS
  TEUCHOS_FUNC_TIME_MONITOR("Tifpack_ILU::ApplyInverse - Solve");
#endif
  ...
  if (!Trans) {
    ...
    TIFPACK_CHK_ERR(L_->Solve(Lower, Trans, UnitDiagonal, X, Y));
    // y = D*y (D_ has inverse of diagonal)
    TIFPACK_CHK_ERR(Y.Multiply(1.0, *D_, Y, 0.0)); 
    // Solve Uy = y
    TIFPACK_CHK_ERR(U_->Solve(Upper, Trans, UnitDiagonal, Y, Y)); 
  }
  else {
    // Solve Uy = y
    TIFPACK_CHK_ERR(U_->Solve(Upper, Trans, UnitDiagonal, X, Y)); 
    // y = D*y (D_ has inverse of diagonal)
    TIFPACK_CHK_ERR(Y.Multiply(1.0, *D_, Y, 0.0)); 
    TIFPACK_CHK_ERR(L_->Solve(Lower, Trans, UnitDiagonal, Y, Y));
  } 
				...
				Tpetra_MultiVector& Y) const 
{
...
#ifdef ENABLE_TIFPACK_ILU_TEUCHOS_TIMERS
  TEUCHOS_FUNC_TIME_MONITOR("Tifpack_ILU::Multiply");
#endif
  ...
  if (!Trans) {
    TIFPACK_CHK_ERR(U_->Multiply(Trans, X, Y)); 
    // Y1 = Y1 + X1 (account for implicit unit diagonal)
    TIFPACK_CHK_ERR(Y.Update(1.0, X, 1.0)); 
    // y = D*y (D_ has inverse of diagonal)
    TIFPACK_CHK_ERR(Y.ReciprocalMultiply(1.0, *D_, Y, 0.0)); 
    Tpetra_MultiVector Y1temp(Y); // Need a temp copy of Y1
    TIFPACK_CHK_ERR(L_->Multiply(Trans, Y1temp, Y));
    // (account for implicit unit diagonal)
    TIFPACK_CHK_ERR(Y.Update(1.0, Y1temp, 1.0)); 
  }
  else {

    TIFPACK_CHK_ERR(L_->Multiply(Trans, X, Y));
    // Y1 = Y1 + X1 (account for implicit unit diagonal)
    TIFPACK_CHK_ERR(Y.Update(1.0, X, 1.0)); 
    // y = D*y (D_ has inverse of diagonal)
    TIFPACK_CHK_ERR(Y.ReciprocalMultiply(1.0, *D_, Y, 0.0)); 
    Tpetra_MultiVector Y1temp(Y); // Need a temp copy of Y1
    TIFPACK_CHK_ERR(U_->Multiply(Trans, Y1temp, Y));
    // (account for implicit unit diagonal)
    TIFPACK_CHK_ERR(Y.Update(1.0, Y1temp, 1.0)); 
  } 
                             ...
                             Tpetra_MultiVector& Y) const
{
...
#ifdef ENABLE_TIFPACK_ILU_TEUCHOS_TIMERS
  TEUCHOS_FUNC_TIME_MONITOR("Tifpack_ILU::ApplyInverse");
#endif
  ...
  TIFPACK_CHK_ERR(Solve(Tifpack_ILU::UseTranspose(), *Xcopy, Y));
...
//=============================================================================
double Tifpack_ILU::Condest(const Tifpack_CondestType CT, 
                           const int MaxIters, const double Tol,
                              Tpetra_RowMatrix* Matrix_in)
{

#ifdef ENABLE_TIFPACK_ILU_TEUCHOS_TIMERS
  TEUCHOS_FUNC_TIME_MONITOR("Tifpack_ILU::Condest");
#endif
...
//=============================================================================
std::ostream&
Tifpack_ILU::Print(std::ostream& os) const
{
  if (!Comm().MyPID()) {
    os << endl;
    os << "================================================================================" << endl;
    os << "Tifpack_ILU: " << Label() << endl << endl;

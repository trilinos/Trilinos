                         ...
                         const Tpetra_MultiVector& X, const Tpetra_MultiVector&Y)
{
  ...
  TIFPACK_CHK_ERR(A.Multiply(false,X,RHS));
...
double Tifpack_FrobeniusNorm(const Tpetra_RowMatrix& A)
{
  ...
  for (int i = 0 ; i < A.NumMyRows() ; ++i) {
    ...
    int Nnz;
    TIFPACK_CHK_ERR(A.ExtractMyRowCopy(i,A.MaxNumEntries(),Nnz,
                                      &colVal[0],&colInd[0]));
...
int Tifpack_Analyze(const Tpetra_RowMatrix& A, const bool Cheap,
                   const int NumPDEEqns)
{
  ...
  double NormF   = Tifpack_FrobeniusNorm(A);
  ...
  if (Cheap == false) {
    ...
    for (int i = 0 ; i < NumMyRows ; ++i) {
      ...
      for (int j = 0 ; j < Nnz ; ++j) {
        ...
        if (AplusAT.SumIntoGlobalValues(1,&GRID,1,&GCID,&plus_val) != 0) {
          TIFPACK_CHK_ERR(AplusAT.InsertGlobalValues(1,&GRID,1,&GCID,&plus_val));
        }

        if (AplusAT.SumIntoGlobalValues(1,&GCID,1,&GRID,&plus_val) != 0) {
          TIFPACK_CHK_ERR(AplusAT.InsertGlobalValues(1,&GCID,1,&GRID,&plus_val));
        }

        if (AminusAT.SumIntoGlobalValues(1,&GRID,1,&GCID,&plus_val) != 0) {
          TIFPACK_CHK_ERR(AminusAT.InsertGlobalValues(1,&GRID,1,&GCID,&plus_val));
        }

        if (AminusAT.SumIntoGlobalValues(1,&GCID,1,&GRID,&minus_val) != 0) {
          TIFPACK_CHK_ERR(AminusAT.InsertGlobalValues(1,&GCID,1,&GRID,&minus_val));
        }
    ...
    NormF   = Tifpack_FrobeniusNorm(AplusAT);
    ...
    NormF   = Tifpack_FrobeniusNorm(AminusAT);
...
// ====================================================================== 
int Tifpack_PrintSparsity(const Tpetra_RowMatrix& A, const char* InputFileName, 
                         const int NumPDEEqns)
{
  ...
  if (NumGlobalRows != NumGlobalCols)
    TIFPACK_CHK_ERR(-1); // never tested

...
#ifndef TIFPACK_NODEFILTER_HPP
#define TIFPACK_NODEFILTER_HPP
...
//! Tifpack_NodeFilter a class for light-weight extraction of the submatrix corresponding to local rows and columns.

/*! Class Tifpack_NodeFilter enables a light-weight contruction of an
 ...
 */ 
class Tifpack_NodeFilter : public virtual Tpetra_RowMatrix {

public:
  //@{ \name Constructor.
  //! Constructor
  Tifpack_NodeFilter(const Teuchos::RCP<const Tpetra_RowMatrix>& Matrix, int nodeID);
  //Tifpack_NodeFilter(const Teuchos::RCP<const Tpetra_RowMatrix>& Matrix,const Tpetra_Comm *);
...
private:
  ...
  const Tifpack_OverlappingRowMatrix* ovA_;
#ifdef HAVE_MPI

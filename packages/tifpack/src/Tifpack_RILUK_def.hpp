...
void RILUK<MatrixType>::initialize() {
  ...
  if (isOverlapped_) {
    throw std::runtime_error("Error, Tifpack::RILUK::initialize: overlapping not yet supported.");
  ...
  for (GlobalOrdinal i=rowMap->getMinGlobalIndex(); i<=rowMap->getMaxGlobalIndex(); ++i) {
    ...
    for (size_t j=0; j< NumIn; j++) {
      ...
      if (k==i) {
        ...
        throw std::runtime_error("out of range (k<0) in Tifpack::RILUK::initAllValues");
      }
...
void RILUK<MatrixType>::compute() {
 ...
 // Fill InV, InI with current row of L, D and U combined
  ...
  if( !L_->isLowerTriangular() ) 
    throw std::runtime_error("Tifpack::RILUK::compute() ERROR, L isn't lower triangular.");
  if( !U_->isUpperTriangular() ) 
    throw std::runtime_error("Tifpack::RILUK::compute() ERROR, U isn't lower triangular.");
...
void RILUK<MatrixType>::generateXY(Teuchos::ETransp mode, 
  ...
  TEST_FOR_EXCEPTION(Xin.getNumVectors()!=Yin.getNumVectors(), std::runtime_error,
       "Tifpack::RILUK::GenerateXY ERROR: X and Y not the same size");

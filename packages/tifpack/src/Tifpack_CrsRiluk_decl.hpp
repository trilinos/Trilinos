...
#ifndef TIFPACK_CRSRILUK_DECL_HPP
...
\warning A Tpetra::Map<LocalOrdinal,GlobalOrdinal,Node> is required for the Tifpack_CrsRiluk constructor.

*/    


template<class MatrixType>
class CrsRiluk: public virtual Tifpack::Preconditioner<typename MatrixType::scalar_type,typename MatrixType::local_ordinal_type,typename MatrixType::global_ordinal_type,typename MatrixType::node_type> {
      
  // Give ostream << function some access to private and protected data/functions.

  friend ostream& operator << (ostream& os, const Tifpack::CrsRiluk<MatrixType>& A);
  ...
  */
  CrsRiluk(const Teuchos::RCP<const Tpetra::RowMatrix<Scalar,LocalOrdinal,GlobalOrdinal,Node> >& A_in);
  ...
  */
  ...
  //! Returns the result of a CrsRiluk forward/back solve on a Tpetra::MultiVector<Scalar,LocalOrdinal,GlobalOrdinal,Node> X in Y.
  /*! 
    \param In
  ...
  */
  ...
  const MatrixType& L() const {return(*L_);};
    
  //! Returns the D factor associated with this factored matrix.
  const Tpetra::Vector<Scalar,LocalOrdinal,GlobalOrdinal,Node> & D() const {return(*D_);};
    
  //! Returns the U factor associated with this factored matrix.
  const MatrixType& U() const {return(*U_);};
...
ostream& operator << (ostream& os, const Tifpack::CrsRiluk& A);

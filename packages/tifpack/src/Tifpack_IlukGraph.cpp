...
ostream& operator << (ostream& os, const Tifpack_IlukGraph& A)
{
/*  Tpetra_fmtflags olda = os.setf(ios::right,ios::adjustfield);
  Tpetra_fmtflags oldf = os.setf(ios::scientific,ios::floatfield);
  int oldp = os.precision(12); */
  int LevelFill = A.LevelFill();
  Tpetra_CrsGraph & L = (Tpetra_CrsGraph &) A.L_Graph();
  Tpetra_CrsGraph & U = (Tpetra_CrsGraph &) A.U_Graph();
  os.width(14);
  ...
  os <<  "     Graph of L = "; 
  os << endl;
  os << L; // Let Tpetra_CrsGraph handle the rest.

  os.width(14);
  os <<  "     Graph of U = "; 
  os << endl;
  os << U; // Let Tpetra_CrsGraph handle the rest.

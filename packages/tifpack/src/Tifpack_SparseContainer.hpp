...
#ifndef TIFPACK_SPARSECONTAINER_HPP
#define TIFPACK_SPARSECONTAINER_HPP
...
/*! 
\brief Tifpack_SparseContainer: a class for storing and solving linear systems
...
class Tifpack_DenseContainer, one can store the blocks as sparse
matrices (Tpetra_CrsMatrix), which can be advantageous when the 
blocks are large. Otherwise,
class Tifpack_DenseContainer is probably more appropriate.
...
used in Tifpack_DenseContainer). In SparseContainer, T must be
an Tifpack_Preconditioner derived class. The container will allocate
...
template<typename T>
class Tifpack_SparseContainer : public Tifpack_Container {

public:

  //@{ Constructors/Destructors.
  //! Constructor.
  Tifpack_SparseContainer(const int NumRows, const int NumVectors = 1);

  //! Copy constructor.
  Tifpack_SparseContainer(const Tifpack_SparseContainer<T>& rhs);

  //! Destructor.
  virtual ~Tifpack_SparseContainer();
  //@}

  //@{ Overloaded operators.

  //! Operator =
  Tifpack_SparseContainer& operator=(const Tifpack_SparseContainer<T>& rhs);
  //@}
  ...
  virtual int SetNumVectors(const int NumVectors_in)
  {
    if (NumVectors_ == NumVectors_in)
      return(0);
    TIFPACK_CHK_ERR(-99); // STILL TO DO
  }
...
//==============================================================================
template<typename T>
Tifpack_SparseContainer<T>::
Tifpack_SparseContainer(const int NumRows_in, const int NumVectors_in) :
  NumRows_(NumRows_in),
...
//==============================================================================
template<typename T>
Tifpack_SparseContainer<T>::~Tifpack_SparseContainer()
{
  Destroy();
}
...
//==============================================================================
template<typename T>
int Tifpack_SparseContainer<T>::NumRows() const
{
...
//==============================================================================
template<typename T>
int Tifpack_SparseContainer<T>::Initialize()
{
  ...
  Label_ = "Tifpack_SparseContainer";
...
//==============================================================================
template<typename T>
double& Tifpack_SparseContainer<T>::LHS(const int i, const int Vector)
{
...
//==============================================================================
template<typename T>
double& Tifpack_SparseContainer<T>::RHS(const int i, const int Vector)
{
...
//==============================================================================
template<typename T>
int Tifpack_SparseContainer<T>::
SetMatrixElement(const int row, const int col, const double value)
{
  if (!IsInitialized())
    TIFPACK_CHK_ERR(-3); // problem not shaped yet

  if ((row < 0) || (row >= NumRows())) {
    TIFPACK_CHK_ERR(-2); // not in range
  }
...
//==============================================================================
template<typename T>
int Tifpack_SparseContainer<T>::Compute(const Tpetra_RowMatrix& Matrix_in)
{
  ...
  TIFPACK_CHK_ERR(Extract(Matrix_in));
...
//==============================================================================
template<typename T>
int Tifpack_SparseContainer<T>::Apply()
{
  if (IsComputed() == false) {
    TIFPACK_CHK_ERR(-3); // not yet computed
  }
  
  TIFPACK_CHK_ERR(Matrix_->Apply(*RHS_, *LHS_));
...
//==============================================================================
template<typename T>
int Tifpack_SparseContainer<T>::ApplyInverse()
{
  if (!IsComputed())
    TIFPACK_CHK_ERR(-1);
  
  TIFPACK_CHK_ERR(Inverse_->ApplyInverse(*RHS_, *LHS_));
...
//==============================================================================
template<typename T>
int Tifpack_SparseContainer<T>::Destroy()
{
  IsInitialized_ = false;
  IsComputed_ = false;
  return(0);
...
//==============================================================================
template<typename T>
int& Tifpack_SparseContainer<T>::ID(const int i)
{
  return(GID_[i]);
}
...
template<typename T>
int Tifpack_SparseContainer<T>::Extract(const Tpetra_RowMatrix& Matrix_in)
{
  ...
  TIFPACK_CHK_ERR(Matrix_->FillComplete());
...
//==============================================================================
template<typename T>
ostream& Tifpack_SparseContainer<T>::Print(ostream & os) const
{
  os << "================================================================================" << endl;
  os << "Tifpack_SparseContainer" << endl;
...
#endif // TIFPACK_SPARSECONTAINER_HPP

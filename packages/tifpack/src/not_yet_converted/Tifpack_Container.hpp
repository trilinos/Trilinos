...
#ifndef TIFPACK_CONTAINER_HPP
#define TIFPACK_CONTAINER_HPP
...
//! Tifpack_Container: a pure virtual class for creating and solving local linear problems.

/*!
Class Tifpack_Container provides the abstract interfaces for 
...
<P>Containers are currently used by class Tifpack_BlockRelaxation.

<P>Tifpack_Container is a pure virtual class.
Two concrete implementations are provided in classes
Tifpack_SparseContainer (that stores matrices in sparse the format
Tpetra_CrsMatrix) and Tifpack_DenseContainer
...
class Tifpack_Container {

public:

  //! Destructor.
  virtual ~Tifpack_Container() {};
...
inline ostream& operator<<(ostream& os, const Tifpack_Container& obj)
{
  return(obj.Print(os));
}

#endif // TIFPACK_CONTAINER_HPP

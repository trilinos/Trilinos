#include "BuildTestProblems.h"
  ...
  int  BuildRDP_MatrixTests (Petra_RDP_MultiVector & C,
    ...
    // zero) filled Petra_RDP_MultiVector C, and allocated
    // Petra_RDP_MultiVectors A, B and C_GEMM this routine will generate values for 
    // Petra_RDP_MultiVectors A, B and C_GEMM such that, if A, B and (this) are 
    ...
    Petra_Map * A_Map = new Petra_Map(-1, A_nrows, A_IndexBase, A.Map().Comm());
    Petra_Map * B_Map = new Petra_Map(-1, B_nrows, B_IndexBase, B.Map().Comm());
...
int  BuildRDP_MultiVectorTests (Petra_RDP_MultiVector & C, const double alpha, 
				Petra_RDP_MultiVector& A, 
  ...
  // Petra_RDP_MultiVector (the this object), allocated double * arguments dotvec_AB, 
  // norm1_A, and norm2_A, and allocated Petra_RDP_MultiVectors A, sqrtA,
  // B, C_alpha, C_alphaAplusB and C_plusB, this method will generate values for 
  // Petra_RDP_MultiVectors A, B and all of the additional arguments on
  ...
  */
  ...
  for (i=0; i< A.NumVectors(); i++) 
    {
    norminf_A[i] = (double) (A.GlobalLength() * (i+1));
    minval_A[i] =  (double) (i+1);
    maxval_A[i] = (double) (A.GlobalLength() * (i+1));
  ...
  for (i=0; i< A.NumVectors(); i++) 
    {
      normw_A[i] = (double) (i*1)*A.GlobalLength();
      for (j=0; j<A_nrows; j++) Weightsp[i][j] = 1.0/Ap[i][j];
    }

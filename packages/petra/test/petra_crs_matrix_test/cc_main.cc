...
int check(Petra_RDP_CRS_Matrix& A, int NumMyRows1, int NumGlobalRows1, int NumMyNonzeros1,
	  int NumGlobalNonzeros1, int * MyGlobalElements, bool verbose);

int power_method(bool TransA, Petra_RDP_CRS_Matrix& A, 
		 Petra_RDP_Vector& q,
		 Petra_RDP_Vector& z, 
...
int main(int argc, char *argv[])
{
  ...
  Petra_Map& Map = *new Petra_Map(NumGlobalEquations, NumMyEquations, 0, Comm);
  ...
  Petra_RDP_CRS_Matrix & AA = *new Petra_RDP_CRS_Matrix(Copy, Map, 5);
  ...
  Petra_RDP_CRS_Matrix & B = *new Petra_RDP_CRS_Matrix(AA);
  delete &AA;
  ...
  if (verbose1) {
    ...
    Petra_RDP_CRS_Matrix& A1 = *new Petra_RDP_CRS_Matrix(Copy, Map1, NumNz1);

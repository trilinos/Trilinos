#ifndef _PETRA_EXPORT_H_
#define _PETRA_EXPORT_H_
...
/*! Petra_Import is used to construct a communication plan that can be called repeatedly by computational
    ...
    This class currently has one constructor, taking two Petra_Map or Petra_BlockMap objects.  The
...
class Petra_Export{
  ...
  //! Petra_Export constructor
  /*! Builds an import object that will transfer object built with SourceMap to objects built with TargetMap.

    A Petra_Export object categorizes the elements of the target map into three sets as follows:
    <ol>
...
Given the above information, the Petra_Export constructor builds a list of elements that must be communicated to other
...
With these two maps passed into the Petra_Export constructor, we get the following attribute definitions:
...
<b> Using Petra_Export Objects </b>

Once a Petra_Export object has been constructed, it can be used by any of the Petra classes that support distributed global
...
for each class is given in the appropriate class documentation.
  ...
  */ 
  ...
  //! Constructs a Petra_Export object from the source and target maps.
  Petra_Export( const Petra_BlockMap & SourceMap, const Petra_BlockMap & TargetMap );
  
  //! Petra_Export copy constructor. 
  Petra_Export(const Petra_Export& Exporter);
  
  //! Petra_Export destructor.
  
  virtual ~Petra_Export(void);
 ...
 friend ostream & operator<<( ostream & os, const Petra_Export & Exporter );

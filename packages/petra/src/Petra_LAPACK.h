#ifndef _PETRA_LAPACK_H_
#define _PETRA_LAPACK_H_

//! Petra_LAPACK:  The Petra LAPACK Wrapper Class.
/*! The Petra_LAPACK class is a wrapper that encapsulates LAPACK
    ...
    platforms.  The Petra_LAPACK class provides C++ wrappers for the LAPACK
    kernels in order to insulate the rest of Petra from the details of C++ to Fortran
    translation.
    A Petra_LAPACK object is essentially nothing, but allows access to the LAPACK wrapper
    functions.
  
    Petra_LAPACK is a serial interface only.  This is appropriate since the standard 
...
class Petra_LAPACK {
  ...
  //! Petra_LAPACK Constructor.
  /*! Builds an instance of a serial LAPACK object.
   */
  Petra_LAPACK(void);


  //! Petra_LAPACK Copy Constructor.
  /*! Makes an exact copy of an existing Petra_LAPACK instance.
  */
  Petra_LAPACK(const Petra_LAPACK& LAPACK);

  //! Petra_LAPACK Destructor.
  virtual ~Petra_LAPACK(void);
  //@}


  //@{ \name Symmetric Positive Definite linear system routines.
  
  //! Petra_LAPACK factorization for positive definite matrix (SPOTRF)
  void POTRF( char UPLO, int N, float * A, int LDA, int * INFO) const;
  //! Petra_LAPACK factorization for positive definite matrix (DPOTRF)
  void POTRF( char UPLO, int N, double * A, int LDA, int * INFO) const;

  //! Petra_LAPACK solve (after factorization) for positive definite matrix (SPOTRS)
  void POTRS( char UPLO, int N, int NRHS, float * A, int LDA, float * X, int LDX, int * INFO) const;
  //! Petra_LAPACK solve (after factorization) for positive definite matrix (DPOTRS)
  void POTRS( char UPLO, int N, int NRHS, double * A, int LDA, double * X, int LDX, int * INFO) const;

  //! Petra_LAPACK inversion  for positive definite matrix (SPOTRI)
  void POTRI( char UPLO, int N, float * A, int LDA, int * INFO) const;
  //! Petra_LAPACK inversion  for positive definite matrix (DPOTRI)
  void POTRI( char UPLO, int N, double * A, int LDA, int * INFO) const;

  //! Petra_LAPACK condition number estimator for positive definite matrix (SPOCON)
  void POCON( char UPLO, int N, float * A, int LDA, float ANORM, 
			  float * RCOND, float * WORK, int * IWORK, int * INFO) const;
  //! Petra_LAPACK condition number estimator for positive definite matrix (DPOCON)
  void POCON( char UPLO, int N, double * A, int LDA, double ANORM, 
			  double * RCOND, double * WORK, int * IWORK, int * INFO) const;

  //! Petra_LAPACK factor and solve for positive definite matrix (SPOSV)
  void POSV( char UPLO, int N, int NRHS, float * A, int LDA, float * X, int LDX, int * INFO) const;
  //! Petra_LAPACK factor and solve for positive definite matrix (DPOSV)
  void POSV( char UPLO, int N, int NRHS, double * A, int LDA, double * X, int LDX, int * INFO) const;

  //! Petra_LAPACK equilibration for positive definite matrix (SPOEQU)
  void POEQU(int N, float * A, int LDA, float * S, float * SCOND, float * AMAX, int * INFO) const;
  //! Petra_LAPACK equilibration for positive definite matrix (DPOEQU)
  void POEQU(int N, double * A, int LDA, double * S, double * SCOND, double * AMAX, int * INFO) const;

  //! Petra_LAPACK solve driver for positive definite matrix (SPOSVX)
  ...
  //! Petra_LAPACK solve driver for positive definite matrix (DPOSVX)
  ...
  //! Petra_LAPACK wrapper for computing the eigenvalues of a real upper Hessenberg matrix (DHSEQR)
  ...
  //! Petra_LAPACK wrapper for generating a real orthogonal matrix Q defined by elementary reflectors. (SORGHR)
  void ORGHR( int N, int ILO, int IHI, float * A, int LDA, float * TAU, float * WORK, int LWORK, int * INFO) const;
  //! Petra_LAPACK wrapper for generating a real orthogonal matrix Q defined by elementary reflectors. (DORGHR)
  void ORGHR( int N, int ILO, int IHI, double * A, int LDA, double * TAU, double * WORK, int LWORK, int * INFO) const;

  //! Petra_LAPACK wrapper for applying an orthogonal matrix in-place (SORMHR)
  void ORMHR( char SIDE, char TRANS, int M, int N, int ILO, int IHI, float * A, int LDA, 
	      float * TAU, float * C,
	      int LDC, float * WORK, int LWORK, int * INFO) const;
  //! Petra_LAPACK wrapper for applying an orthogonal matrix in-place (DORMHR)
  ...
  //! Petra_LAPACK wrapper for computing eigenvectors of a quasi-triangular/triagnular matrix (STREVC)
  ...
  //! Petra_LAPACK wrapper for computing eigenvectors of a quasi-triangular/triagnular matrix (DTREVC)
  /*! \warning HOWMNY = 'S" is not supported.
   */
  ...
  //! Petra_LAPACK wrapper for reordering the real-Schur/Schur factorization of a matrix (STREXC)
  void TREXC( char COMPQ, int N, float * T, int LDT, float * Q, int LDQ, int IFST, int ILST, 
	      float * WORK, int * INFO) const;
  //! Petra_LAPACK wrapper for reordering the real-Schur/Schur factorization of a matrix (DTREXC)
  ...
  //! Petra_LAPACK wrapper for DLAMCH routine that returns machine double precision floating point characteristics
  double DLAMCH( char CMACH) const; 
  //! Petra_LAPACK wrapper for SLAMCH routine that returns machine single precision floating point characteristics
...
// Petra_LAPACK constructor
inline Petra_LAPACK::Petra_LAPACK(void){}
// Petra_LAPACK constructor
inline Petra_LAPACK::Petra_LAPACK(const Petra_LAPACK& LAPACK){}
// Petra_LAPACK destructor
inline Petra_LAPACK::~Petra_LAPACK(){}

#endif /* _PETRA_LAPACK_H_ */

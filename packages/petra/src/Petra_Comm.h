#ifndef _PETRA_COMM_H_
#define _PETRA_COMM_H_

//! Petra_Comm:  The Petra Communication Class.
/*! The Petra_Comm class is a wrapper that encapsulates the general
  information and services needed for other Petra classes to run on a parallel computer.
  A Petra_Comm object is required for building all Petra Map objects, which
  in turn are required for all other Petra classes.
  
  Petra_Comm support both serial execution and (via MPI) parallel 
...
class Petra_Comm: public Petra_Object {
    
  public:
  //@{ \name Constructor/Destructor Methods
#ifdef PETRA_MPI
  //! Petra_Comm MPI Constructor.
  /*! Creates a Petra_Comm instance for use with MPI.  If no specialized
    MPI communicator is needed, this constuctor can be called with the
    argument MPI_COMM_WORLD.  
    \warning Note that Petra_Comm.cc must be built with
    -DPETRA_MPI on the compile line in order for this constructor to be
    available.
  */
  ...
  /*! Builds an instance of a serial communicator.  Even
    ...
    \param comm In
           An MPI communicator.  The user must call MPI_Init and create
	   a valid MPI communicator prior to constructin a Petra_Comm object.
  */
  Petra_Comm(void);


  //! Petra_Comm Copy Constructor.
  /*! Makes an exact copy of an existing Petra_Comm instance.
  */
  Petra_Comm(const Petra_Comm& Comm);


  //! Petra_Comm Destructor.
  /*! Completely deletes a Petra_Comm object.  
    \warning Note:  All objects that depend
    on a Petra_Comm instance should be destroyed prior to calling this
    function.
  */
  virtual ~Petra_Comm(void);
  //@}

  //@{ \name Barrier Methods
  //! Petra_Comm Barrier function.
  ...
  */
  ...
  //@{ \name Gather Methods
  //! Petra_Comm All Gather function.
  /*! A copy for a serial communicator.  For MPI, it 
  ...
  */
  ...
  //@{ \name Sum Methods
  //! Petra_Comm Global Sum function.
  /*! A copy for a serial communicator.  For MPI, it 
  ...
  */
  ...
  //@{ \name Max/Min Methods
  //! Petra_Comm Global Max function.
  /*! A copy for a serial communicator.  For MPI, it 
  ...
  */
  ...
  //! Petra_Comm Global Min function.
  /*! A copy for a serial communicator.  For MPI, it 
    take list of input values from all processors in the communicator, computes the max and returns the
    max to all processors.
...
#ifdef PETRA_MPI
  //! Extract MPI Communicator from a Petra_Comm object.
  /*! \warning Note that Petra_Comm.cc must be built with
    -DPETRA_MPI on the compile line in order for this constructor to be
    available.
  */
  ...
  /*! In MPI mode returns the rank of the calling process.  In serial mode
    returns 0.
  */
  int MyPID(void) const {return(rank_);};
  ...
  */
  ...
  /*! If SetMyThreadID was called to set a thread value, this function returns the thread ID of the calling process.  
      Otherwise returns 0.
  */
  int MyThreadID(void) const {return(ThreadID_);};
  ...
  */
  int MyNodeID(void) const {return(NodeID_);};
 ...
 protected:
  ...
  friend class Petra_RDP_DVBR_Matrix;
  friend class Petra_RDP_MultiVector;
  friend class Petra_RDP_Vector;
...
#endif
  bool Serial_; //!< \internal Serial true/false variable.
  int rank_;
  int size_;
  int ThreadID_;
  int NodeID_;
...
/*inline ostream& operator<<(ostream& os, const Petra_Comm& comm)
{
  os << comm.Label() << endl;
  comm.Print(os);
  return os;

...
int main (int argc, char *argv[]) {
  ...
  int r_val = 0;
  {
    // initialize both OpenMP and Pthreads spaces
    Kokkos::initialize();
    exec_space::initialize(nthreads, numa, core_per_numa);

    std::cout << "KokkosSpace::  "; Kokkos::DefaultExecutionSpace::print_configuration(std::cout, false);

    // Pthreads sleep and OpenMP awake
    if (!exec_space::sleep())
      std::cout << "Fail to toggle execution space" << std::endl;            

    // do application work with OpenMP

    // OpenMP sleep and Pthreads sleep
    if (!exec_space::wake())
      std::cout << "Fail to toggle execution space" << std::endl;            
    ...
    // Pthreads needs to be awake to get finalized
    if (!exec_space::wake())
      std::cout << "Fail to toggle execution space" << std::endl;            

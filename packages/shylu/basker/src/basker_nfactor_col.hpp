#ifndef BASKER_NFACTOR_COL_HPP
#define BASKER_NFACTOR_COL_HPP
...
namespace BaskerNS
{
    ...
    */
	...
	for(Int l = 0; l < lvl; ++l)
	  {	    
		...
		t_dense_blk_col_copy_atomic(kid, team_leader, lvl, 
				      l, k);
   ...
   )
  {
    ...
    if(kid == 8)
      {
	printf("8 what the hell! \n");
	INT_1DARRAY temppp = LL(X_col)(X_row).iws;
      }
    */
  ...
  template <class Int, class Entry, class Exe_Space>
  BASKER_INLINE
  int Basker<Int,Entry,Exe_Space>::t_back_solve_atomic(Int kid,
						 Int team_leader,
						 Int lvl, Int l,
						 Int k, Int top,
						       Int xnnz)
...
//Used for O(dim(S)*p) Atomics
  template <class Int, class Entry, class Exe_Space>
  BASKER_INLINE
  int Basker<Int, Entry, Exe_Space>::t_n_col_copy_atomic(Int kid,
						Int team_leader,
							 Int lvl,
							 Int l,
							 Int k)
  ...
  template <class Int, class Entry, class Exe_Space>
  BASKER_INLINE
  int Basker<Int,Entry,Exe_Space>::t_col_copy_atomic(Int kid,
						Int team_leader,
						Int lvl, 
						Int l, 
						Int k)

#ifndef BASKER_SFACTOR_OLD_HPP
#define BASKER_SFACTOR_OLD_HPP
...
namespace BaskerNS
{
  ...
  int Basker<Int,Entry,Exe_Space>::nontree_QR_symb()
  {
    //#ifdef BASKER_DEBUG_SFACTOR
	 ...
	 #endif
	 printf("done alloc LL \n");

	 //Malloc Inner U
	 MALLOC_MATRIX_1DARRAY(LU[i], LU_size[i]);
	 printf("done alloc LU %d \n", LU_size[i]);
	 ...
	 for(Int j=i; j!=A.ncol; j=tree.treetab[j])
	   {
	     ...
	     cout << "Init U(): " << i << " " << LU_count[j]
		  << " nnz " << Utemp[LU_count[j]].nnz << endl;
  ...
  template <class Int, class Entry, class Exe_Space>
  BASKER_INLINE
  Int Basker<Int, Entry, Exe_Space>::post_dfs(Int j, Int k, Int *head, 
  ...
  /* you don't need the transpose if the matrix is symmtric so use cheaper
     function call!!! */
  template <class Int, class Entry, class Exe_Space>
        ...
        if(parent[j] != A.ncol)
          {
            ...
            delta[parent[j]]--;}  /*j is not a root node*/
    ...
    for(Int k = 0; k < A.ncol; k++)
      {
        ...
        if(i < 0)
          {
            //cout << "ficitious row added " << endl;
            i = m2++;
          }
        lpinv[i] = k;
        if(--nq[k] <=0)
          {
            //cout << "Skiped, empty" << endl;
            continue;
          }
        count += nq[k];
        Int pa = parent[k];
  ...
  template <class Int, class Entry, class Exe_Space>
  BASKER_INLINE
  void Basker<Int,Entry, Exe_Space>::row_count(INT_1DARRAY &parent, 
					       INT_1DARRAY &post)
  {
    ...
    */
    ...
    for(Int l = 0; l < A.ncol; l++)
    {
      ...
      for(Int J = head[l]; J!= A.ncol; J = next[J])
        {
          //cout << "sub node "  << J <<  "size: " << At->col_ptr[J+1]-At->col_ptr[J] <<endl;
          for(Int p = At.col_ptr[J]; p < At.col_ptr[J+1]; p++)
            {
              Int i = At.row_idx[p];
              //cout << "i " << i << endl;
              ...
              if(jleaf)
                {
                  ...
                  //cout << "rowcount: " << i << " "  << rowcount[i] <<" " << (lvl[j] - lvl[q]) << endl;
                }
            }
        }
        if(parent[j] != A.ncol)

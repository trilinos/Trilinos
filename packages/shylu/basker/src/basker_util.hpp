...
namespace BaskerNS
{
  ...
  (INT_1DARRAY a, Int size, Int c)
  {
    //printf("init_value called: size: %d c: %d\n", size, c);
  ...
  BASKER_INLINE
  void Basker<Int,Entry,Exe_Space>::init_value
  (ENTRY_1DARRAY a, Int size, Entry c)
  {
    //printf("init_value called: size: %d c: %f\n", size, c);
    for(Int i=0; i < size; i++)
      {
	a(i) = c;
      }
	...
	if(kid%((Int)pow(2,lvl)) == 0)
	  {
	    ...
	    for(Int kk = 0; kk < LU(b)(LU_size(b)-1).ncol+1;
		kk++)
	      {
	    ...
	    */
		...
		for(Int kk = 0; kk < LU(U_col)(U_row).ncol+1; 
		    kk++)
		  {
...
;
 ...
 AVM(U_col)(U_row).convert2D(BTF_A, alloc, kid);
        ...
        for(Int k = 0; k < myL.ncol; k++)
          {
            ...
            fprintf(fp, "k=%d \n", k+myL.scol);
            //printf("test: %d %d %d \n",k , myL.col_ptr[k], myL.col_ptr[k+1]); 
  ...
  //Print MTX
  template <class Int, class Entry, class Exe_Space>
  void Basker<Int,Entry,Exe_Space>::printMTX(std::string fname,
					     BASKER_MATRIX &M)
  {

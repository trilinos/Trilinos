...
TEST( DenseLinearAlgebra, team_gemm_nn ) {
  ...
  for (int i=0;i<m;++i)
    for (int j=0;j<n;++j) 
      EXPECT_NEAR(std::abs(C1.h_view(i,j)), std::abs(C2.h_view(i,j)), eps);
  TEST_END;
}
...
TEST( DenseLinearAlgebra, team_gemv_n ) {
  ...
  const MagnitudeType eps = std::numeric_limits<MagnitudeType>::epsilon() * 1000;
  for (int i=0;i<m;++i)
    EXPECT_NEAR(std::abs(y1.h_view(i,0)), std::abs(y2.h_view(i,0)), eps);
  TEST_END;
}
...
#define TEAM_TRSV_TEST_BODY {                                           \
    ...
    for (int i=0;i<m;++i)                                               \
      EXPECT_NEAR(std::abs(b1.h_view(i,0)), std::abs(b2.h_view(i,0)), eps*std::abs(b2.h_view(i,0))); \
  }
...
#define TEAM_TRSM_TEST_BODY {                                           \
    ...
    for (int i=0;i<m;++i)                                               \
      for (int j=0;j<n;++j)                                             \
        EXPECT_NEAR(std::abs(B1.h_view(i,j)), std::abs(B2.h_view(i,j)), eps*std::abs(B2.h_view(i,j))); \
  }
...
TEST( DenseLinearAlgebra, team_chol_u ) {
  ...
  for (int i=0;i<m;++i)
    for (int j=0;j<m;++j) 
      EXPECT_NEAR(std::abs(A1.h_view(i,j)), std::abs(A2.h_view(i,j)), eps);
  TEST_END;
}

...
int main(int argc, char *argv[]) {
  ...
  const double eps = std::numeric_limits<double>::epsilon() * 10000;
  {
    ...
    typedef TaskFunctor_Chol<scheduler_type, DenseMatrixOfBlocksType, Uplo::Upper, Algo::ByBlocks> task_functor_chol;
    typedef TaskFunctor_Trsm<scheduler_type, double, DenseMatrixOfBlocksType, Side::Left, Uplo::Upper,
                             Trans::ConjTranspose, Diag::NonUnit, Algo::ByBlocks>
        task_functor_trsm;
    typedef TaskFunctor_Gemm<scheduler_type, double, DenseMatrixOfBlocksType, Trans::NoTranspose, Trans::NoTranspose,
                             Algo::ByBlocks>
        task_functor_gemm;
    typedef TaskFunctor_Herk<scheduler_type, double, DenseMatrixOfBlocksType, Uplo::Upper, Trans::ConjTranspose,
                             Algo::ByBlocks>
        task_functor_herk;

    const ordinal_type max_functor_size = 4 * sizeof(task_functor_gemm);

    Kokkos::DualView<value_type *, exec_space> a("a", mend * mend), a1("a1", mend * mend), a2("a2", mend * mend),
        b("b", mend * mend);

    const ordinal_type bmend = (mend / mb) + 1;
    Kokkos::DualView<DenseMatrixViewType *, exec_space> ha("ha", bmend * bmend), hb("hb", bmend * bmend),
        hc("hc", bmend * bmend);

    {
      const ordinal_type task_queue_capacity_tmp = 2 * bmend * bmend * bmend * max_functor_size, min_block_size = 16,
                         max_block_size = 4 * max_functor_size, num_superblock = 4,
      ...
      sched = scheduler_type(typename scheduler_type::memory_space(), (size_t)task_queue_capacity,
                             (unsigned)min_block_size, (unsigned)max_block_size, (unsigned)superblock_size);
    }
    ...
    for (ordinal_type m = mbeg; m <= mend; m += step) {
      ...
      // dense by blocks
      {
...

        {
          ...
          for (ordinal_type iter = dry; iter < niter; ++iter) {
            ...
            timer.reset();
            Kokkos::host_spawn(Kokkos::TaskSingle(sched, Kokkos::TaskPriority::High), task_functor_chol(DA));
            Kokkos::wait(sched);
...

      {
        ...
        printf("chol problem %10d, gflop %10.2f, gflop/s :: reference %10.2f, byblocks %10.2f\n", m, gflop,
               gflop / t_reference, gflop / t_byblocks);
        PRINT_TIMER;
      }
    }
    printf("\n\n");
    ...
    for (ordinal_type m = mbeg; m <= mend; m += step) {
      ...
      // reference
      {
        ...
        for (ordinal_type iter = dry; iter < niter; ++iter) {
          timer.reset();
          Trsm<Side::Left, Uplo::Upper, Trans::ConjTranspose, Algo::External>::invoke(Diag::NonUnit(), alpha, A, B);
          t_reference += (iter >= 0) * timer.seconds();
        }
        t_reference /= niter;
      }
      ...
      // dense by blocks
      {
...

        {
          ...
          for (ordinal_type iter = dry; iter < niter; ++iter) {
            timer.reset();
            Kokkos::host_spawn(Kokkos::TaskSingle(sched, Kokkos::TaskPriority::High), task_functor_trsm(alpha, DA, DB));
            Kokkos::wait(sched);
            t_byblocks += (iter >= 0) * timer.seconds();
          }
          t_byblocks /= niter;
    ...
    for (ordinal_type m = mbeg; m <= mend; m += step) {
      ...
      // reference
      {
        ...
        for (ordinal_type iter = dry; iter < niter; ++iter) {
          timer.reset();
          Gemm<Trans::NoTranspose, Trans::NoTranspose, Algo::External>::invoke(alpha, A, B, beta, C);
          t_reference += (iter >= 0) * timer.seconds();
        }
        t_reference /= niter;
      }
    ...
    for (ordinal_type m = mbeg; m <= mend; m += step) {
      ...
      // reference
      {
        ...
        for (ordinal_type iter = dry; iter < niter; ++iter) {
          timer.reset();
          Herk<Uplo::Upper, Trans::ConjTranspose, Algo::External>::invoke(alpha, A, beta, C);
          t_reference += (iter >= 0) * timer.seconds();
        }
        t_reference /= niter;
      }

...
template <typename value_type> int driver(int argc, char *argv[]) {
  ...
  int r_val = 0;
  {
    ...
    {
      if (verbose)
        printf("cublas/cusolver handle create begin\n");
      {
        const int status = cublasCreate(&handle_blas);
        if (status)
          printf("Nonzero error from cublasCreate %d\n", status);
      }
      {
        const int status = cusolverDnCreate(&handle_lapack);
        if (status)
          printf("Nonzero error from cusolverDnCreate %d\n", status);
      }
      if (verbose)
        printf("cublas/cusolver handle create end\n");
    }
#endif
    ...
    {
      if (verbose)
        printf("test problem randomization\n");
    ...
    {
      if (verbose)
        printf("test problem symmetrization\n");
      const value_type *Arand_ptr = Arand.data();
      value_type *A_ptr = A.data();

      timer.reset();
#if defined(KOKKOS_ENABLE_CUDA)
      Tacho::Blas<value_type>::gemm(handle_blas, CUBLAS_OP_N, CUBLAS_OP_T, m, m, m, one, Arand_ptr, m, Arand_ptr, m,
                                    zero, A_ptr, m);
#else
      Tacho::Blas<value_type>::gemm('N', 'T', m, m, m, one, Arand_ptr, m, Arand_ptr, m, zero, A_ptr, m);
#endif
      Kokkos::fence();
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
      Kokkos::deep_copy(A, Aback);
      {
...
#if defined(KOKKOS_ENABLE_CUDA)
        int lwork(0);
        Tacho::Lapack<value_type>::potrf_buffersize(handle_lapack, CUBLAS_FILL_MODE_UPPER, m, A_ptr, m, &lwork);
        ...
        Tacho::Lapack<value_type>::potrf(handle_lapack, CUBLAS_FILL_MODE_UPPER, m, A_ptr, m, W_ptr, lwork,
                                         dev_view.data());
        auto dev_host = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), dev_view);
        dev = dev_host();
#else
        Tacho::Lapack<value_type>::potrf('U', m, A_ptr, m, &dev);
#endif
        Kokkos::fence();
        const double t = timer.seconds();
        printf("Cholesky time %e\n", t);
        if (dev)
          printf("Cholesky returns non-zero dev info %d\n", dev);
      }
    }
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
      ...
      Kokkos::View<int *, Kokkos::LayoutLeft, device_type> ipiv("pivot", m);
      {
...
#if defined(KOKKOS_ENABLE_CUDA)
        int lwork(0);
        Tacho::Lapack<value_type>::sytrf_buffersize(handle_lapack, m, A_ptr, m, &lwork);
        printf("LDLt lwork %d\n", lwork);
        Kokkos::View<value_type *, Kokkos::LayoutLeft, device_type> W("W", lwork);
        value_type *W_ptr = W.data();

        Kokkos::View<int, device_type> dev_view("dev");
        Tacho::Lapack<value_type>::sytrf(handle_lapack, CUBLAS_FILL_MODE_LOWER, m, A_ptr, m, ipiv_ptr, W_ptr, lwork,
                                         dev_view.data());
        auto dev_host = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), dev_view);
        dev = dev_host();
#else
        ...
        Kokkos::View<value_type *, Kokkos::LayoutLeft, device_type> W("W", lwork);
        value_type *W_ptr = W.data();
        Tacho::Lapack<value_type>::sytrf('L', m, A_ptr, m, ipiv_ptr, W_ptr, lwork, &dev);
#endif
        Kokkos::fence();
        const double t = timer.seconds();
        ...
        printf("LDLt time %e\n", t);
        if (dev)
          printf("LDLt returns non-zero dev info %d\n", dev);
      }
    }
...
#if defined(KOKKOS_ENABLE_CUDA)
        int lwork(0);
        Tacho::Lapack<value_type>::getrf_buffersize(handle_lapack, m, m, A_ptr, m, &lwork);
        printf("LU lwork %d\n", lwork);
        Kokkos::View<value_type *, Kokkos::LayoutLeft, device_type> W("W", lwork);
        value_type *W_ptr = W.data();

        Kokkos::View<int, device_type> dev_view("dev");
        Tacho::Lapack<value_type>::getrf(handle_lapack, m, m, A_ptr, m, W_ptr, ipiv_ptr, dev_view.data());
        auto dev_host = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), dev_view);
        dev = dev_host();
#else
        Tacho::Lapack<value_type>::getrf(m, m, A_ptr, m, ipiv_ptr, &dev);
#endif
        Kokkos::fence();
        const double t = timer.seconds();
        printf("LU time %e\n", t);
        if (dev)
          printf("LU returns non-zero dev info %d\n", dev);
      }
    }
    ...
    {
      if (verbose)
        printf("cublas/cusolver handle destroy begin\n");
      {
        const int status = cublasDestroy(handle_blas);
        if (status)
          printf("Nonzero error from cublasDestroy %d\n", status);
      }
      {
        const int status = cusolverDnDestroy(handle_lapack);
        if (status)
          printf("Nonzero error from cusolverDnDestroy %d\n", status);
      }
      if (verbose)
        printf("cublas/cusolver handle destroy end\n");
    }
#endif
  }
  Kokkos::finalize();

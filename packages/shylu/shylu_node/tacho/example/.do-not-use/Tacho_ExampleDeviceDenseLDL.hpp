...
template <typename value_type> int driver_ldl(const int m, const bool verbose) {
  int max_iter = 1;
  ...
  int r_val = 0;
  {
    ...
    Kokkos::View<value_type **, Kokkos::LayoutLeft, device_type> A("A", m, m);
    {
      ...
      Kokkos::parallel_for(
          Kokkos::RangePolicy<typename device_type::execution_space>(0, m * m), KOKKOS_LAMBDA(const int ij) {
    ...
    Kokkos::View<value_type **, Kokkos::LayoutLeft, device_type> x("x", m, 1);
    {
      Kokkos::parallel_for(
          ...
          KOKKOS_LAMBDA(const int i) { x(i, 0) = i + 1; });
    }

    Kokkos::View<value_type **, Kokkos::LayoutLeft, device_type> b("b", m, 1);
    {
      Tacho::Gemv<Tacho::Trans::NoTranspose, Tacho::Algo::OnDevice>::invoke(handle_blas, one, A, x, zero, b);
      Kokkos::fence();
    }
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
      ...
      const int lwork = Tacho::LDL<Tacho::Uplo::Lower, Tacho::Algo::OnDevice>::invoke(handle_lapack, A, p, W);
      W = Kokkos::View<value_type *, Kokkos::LayoutLeft, device_type>("W", lwork);

      printf("LDLt lwork %d\n", lwork);
      Tacho::LDL<Tacho::Uplo::Lower, Tacho::Algo::OnDevice>::invoke(handle_lapack, A, p, W);
      // {
      ...
      Tacho::LDL<Tacho::Uplo::Lower, Tacho::Algo::OnDevice>::modify(exec_instance, A, p, D);
      Kokkos::fence();
      ...
      /// solve
      {
        ...
        /// copy and transpose
        Tacho::ApplyPermutation<Tacho::Side::Left, Tacho::Trans::NoTranspose, Tacho::Algo::OnDevice>::invoke(
            exec_instance, b, perm, x);
        ...
        /// trsv
        Tacho::Trsv<Tacho::Uplo::Lower, Tacho::Trans::NoTranspose, Tacho::Algo::OnDevice>::invoke(
            handle_blas, Tacho::Diag::Unit(), A, x);
        Kokkos::fence();
        {
          auto x_host = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), x);
        ...
        Tacho::Scale2x2_BlockInverseDiagonals<Tacho::Side::Left, Tacho::Algo::OnDevice>::invoke(exec_instance, p, D, x);
        Kokkos::fence();
        {
          auto x_host = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), x);
        ...
        Tacho::Trsv<Tacho::Uplo::Lower, Tacho::Trans::Transpose, Tacho::Algo::OnDevice>::invoke(
            handle_blas, Tacho::Diag::Unit(), A, x);
        Kokkos::fence();
        {
          auto x_host = Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace(), x);

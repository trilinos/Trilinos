  ...
  do {                                                                  \
    if (object == nullptr)                                              \
      object = (numeric_tools_base_type*) ::operator new (sizeof(numeric_tools_serial_type)); \
                                                                        \
    new (object) numeric_tools_serial_type(_method,                     \
                                           _m, _ap, _aj,                \
                                           _perm, _peri,                \
                                           _nsupernodes, _supernodes,   \
                                           _gid_ptr, _gid_colidx, \
                                           _sid_ptr, _sid_colidx, _blk_colidx, \
...
#define TACHO_NUMERIC_TOOLS_FACTORY_LEVELSET_BODY(numeric_tools_levelset_name) \
  ...
  new (object) numeric_tools_levelset_name(_method,                     \
    _m, _ap, _aj,                                                       \
    _perm, _peri,                                                       \
    _nsupernodes, _supernodes,                                          \
    _gid_ptr, _gid_colidx,                      \
  ...
  class NumericToolsFactory<ValueType,typename UseThisDevice<Kokkos::Serial>::type> {
  public:
    ...
    void setLevelSetMember(const ordinal_type variant,
                           const ordinal_type device_level_cut,
                           const ordinal_type device_factor_thres,
  ...
  template<typename ValueType>
  class NumericToolsFactory<ValueType,typename UseThisDevice<Kokkos::OpenMP>::type> {
  public:
    using value_type = ValueType;
  ...
  template<typename ValueType>
  class NumericToolsFactory<ValueType,typename UseThisDevice<Kokkos::Cuda>::type> {
  public:
    using value_type = ValueType;
    ...
    void createObject(numeric_tools_base_type *& object) {
      switch (_variant) {
      case 0: { TACHO_NUMERIC_TOOLS_FACTORY_LEVELSET_BODY(numeric_tools_levelset_var0_type); break;  }
      case 1: { TACHO_NUMERIC_TOOLS_FACTORY_LEVELSET_BODY(numeric_tools_levelset_var1_type); break;  }
      case 2: { TACHO_NUMERIC_TOOLS_FACTORY_LEVELSET_BODY(numeric_tools_levelset_var2_type); break;  }
      default: { TACHO_TEST_FOR_EXCEPTION(true, std::logic_error, "Invalid variant input"); break; }
      }
    }
  };
#endif

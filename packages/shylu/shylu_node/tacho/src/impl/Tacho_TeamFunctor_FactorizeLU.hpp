...
namespace Tacho {
  ...
  struct TeamFunctor_FactorizeLU {
  public:
  ...
  public:
    ...
    KOKKOS_INLINE_FUNCTION
    TeamFunctor_FactorizeLU(const supernode_info_type &info,
      ...
      if (m > 0) {
        ...
        if (n_m > 0) {
          ...
          Gemm<Trans::NoTranspose,Trans::NoTranspose,GemmAlgoType>
            ::invoke(member, minus_one, ABL, ATR, zero, ABR);
        }
      }
    }
          ...
          Kokkos::parallel_for
            (Kokkos::TeamThreadRange(member, srcsize), 
             [&, srcsize, src, tgt](const ordinal_type &j) { // Value capture is a workaround for cuda + gcc-7.2 compiler bug w/c++14
              ...
              Kokkos::parallel_for
                (Kokkos::ThreadVectorRange(member, srcsize), [&](const ordinal_type &i) {

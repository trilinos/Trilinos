...
template <> struct LDL<Uplo::Lower, Algo::External> {
  ...
  KOKKOS_INLINE_FUNCTION static int invoke(MemberType &member, const ViewTypeA &A, const ViewTypeP &P, const ViewTypeW &W) {
#if defined(KOKKOS_ACTIVE_EXECUTION_MEMORY_SPACE_HOST)
    int r_val = 0;
    r_val = invoke(A, P, W);
    return r_val;
  ...
  KOKKOS_INLINE_FUNCTION static int modify(MemberType &member, const ViewTypeA &A, const ViewTypeP &P, const ViewTypeD &D) {
#if defined(KOKKOS_ACTIVE_EXECUTION_MEMORY_SPACE_HOST)
    int r_val = 0;
    r_val = modify(A, P, D);
    return r_val;

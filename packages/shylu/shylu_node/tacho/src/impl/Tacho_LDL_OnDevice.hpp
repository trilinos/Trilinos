...
namespace Tacho {
    ...
    template<>
    struct LDL<Uplo::Lower,Algo::OnDevice> {
      ...
      cusolver_invoke(cusolverDnHandle_t &handle,
        ...
        if (m > 0) {
          ...
          int lwork = (W.span()-1);
          r_val = Lapack<value_type>::sytrf(handle_lapack,
      ...
      cusolver_buffer_size(cusolverDnHandle_t &handle,
        ...
        int r_val(0);
        if (m > 0) 
          r_val = Lapack<value_type>::sytrf_buffersize(handle,
                                                       m, 
                                                       A.data(), A.stride_1(),
                                                       lwork);
        return r_val;
      ...
      inline
      static int
      lapack_modify(const ViewTypeA &A,
                    const ViewTypeP &P,
                    const ViewTypeD &D) {
      ...
      cusolver_modify(ExecSpaceType &exec_instance,
        ...
        Kokkos::RangePolicy range_policy(exec_instance, 0, m);
        Kokkos::parallel_for
          ("PermutationSet", range_policy,
      ...
      modify(MemberType &member,
        ...
        static_assert(std::is_same<value_type,value_type_d>::value,
                      "A and D do not have the same value type.");

        static_assert(std::is_same<memory_space,memory_space_p>::value,
                      "A and P do not have the same memory space.");

        static_assert(std::is_same<memory_space,memory_space_d>::value,
                      "A and D do not have the same memory space.");

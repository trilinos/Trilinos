...
namespace Tacho {
    ...
    public:
      ...
      KOKKOS_INLINE_FUNCTION
      TaskFunctor_FactorizeCholByBlocks(const memory_pool_type &bufpool,
        ...
        case 1: {
          ...
          if (buf != NULL && bufsize > 0) {
            ...
            if (bn > 0) {
              ...
              Kokkos::single(Kokkos::PerTeam(member), [&]() {
                  Kokkos::respawn(this, sched.when_all(dep, bn2), Kokkos::TaskPriority::Regular);
                  for (ordinal_type k=0;k<static_cast<ordinal_type>(bn2);++k) (dep+k)->~future_type();
                  sched.queue().get_memory_pool().deallocate((void*)dep, depsize);
                });
            } else {
              Kokkos::single(Kokkos::PerTeam(member), [&]() {
                  if (_bufsize) {
                ...
                if (use_byblocks) {
                  for (ordinal_type i=0;i<_s.nchildren;++i) {
                    auto f = Kokkos::task_spawn(Kokkos::TaskTeam(sched, Kokkos::TaskPriority::Regular),
                                                ...
                                                (_bufpool, _info, _s.children[i], _mb));
                  ...
                  for (ordinal_type i=0;i<_s.nchildren;++i) {
                    auto f = Kokkos::task_spawn(Kokkos::TaskTeam(sched, Kokkos::TaskPriority::Regular),
                                                TaskFunctor_FactorizeChol<mat_value_type,scheduler_type>
                                                (_bufpool, _info, _s.children[i]));
                    TACHO_TEST_FOR_ABORT(f.is_null(), "task allocation fails");
                    dep[i] = f;
                  }
                }

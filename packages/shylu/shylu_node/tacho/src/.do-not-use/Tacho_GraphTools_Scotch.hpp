...
public:
  ...
  virtual ~GraphTools_Scotch() { SCOTCH_graphFree(&_graph); }
  ...
  void setTreeLevel(const unsigned int level = 0) { _level = level; }
  ...
  void reorder(const ordinal_type verbose = 0) {
    ...
    timer.reset();
    {
      ...
      if (_strat & SCOTCH_STRATLEVELMAX || _strat & SCOTCH_STRATLEVELMIN) {
        TACHO_TEST_FOR_EXCEPTION(_level == 0, std::logic_error,
      ...
      TACHO_TEST_FOR_EXCEPTION(ierr, std::runtime_error, "Failed in SCOTCH_stratInit");

      // if both are zero, do not build strategy
      if (_strat || _level) {
        ierr = SCOTCH_stratGraphOrderBuild(&stradat, straval, level, 0.2);
        TACHO_TEST_FOR_EXCEPTION(ierr, std::runtime_error, "Failed in SCOTCH_stratGraphOrderBuild");
      }
      ierr = SCOTCH_graphOrder(&_graph, &stradat, perm, peri, &_cblk, range, tree);
      TACHO_TEST_FOR_EXCEPTION(ierr, std::runtime_error, "Failed in SCOTCH_graphOrder");
  ...
  std::ostream &showMe(std::ostream &os, const bool detail = false) const {
    ...
    if (detail) {
      ...
      for (ordinal_type i = 0; i < m; ++i)
        os << std::setw(w) << _perm[i] << "   " << std::setw(w) << _peri[i] << "   " << std::setw(w) << _range[i]

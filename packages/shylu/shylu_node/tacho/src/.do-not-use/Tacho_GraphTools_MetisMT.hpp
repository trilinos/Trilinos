...
public:
  ...
  void setOption(const int id, const double value) { _options[id] = value; }
  ...
  void reorder(const ordinal_type verbose = 0) {
    ...
    timer.reset();
    ierr = MTMETIS_NodeND(&_nvts, xadj, adjncy, vwgt, _options, perm, peri);
    ...
    TACHO_TEST_FOR_EXCEPTION(ierr != MTMETIS_SUCCESS, std::runtime_error, "Failed in METIS_NodeND");
    _is_ordered = true;
  ...
  std::ostream &showMe(std::ostream &os, const bool detail = false) const {
    ...
    if (_is_ordered)
      os << " -- MetisMT Ordering -- " << std::endl << "  PERM     PERI     " << std::endl;
    else

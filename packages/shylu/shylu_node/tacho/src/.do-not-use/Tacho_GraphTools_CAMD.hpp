...
template <typename OrdinalType> class CAMD;

template <> class CAMD<int> {
public:
  static void run(int n, int Pe[], int Iw[], int Len[], int iwlen, int pfree, int Nv[], int Next[], int Last[],
                  int Head[], int Elen[], int Degree[], int W[], double Control[], double Info[], const int C[],
                  int BucketSet[]) {
    TACHO_SUITESPARSE(camd_2)
    (n, Pe, Iw, Len, iwlen, pfree, Nv, Next, Last, Head, Elen, Degree, W, Control, Info, C, BucketSet);
  }
};

template <> class CAMD<SuiteSparse_long> {
public:
  static void run(SuiteSparse_long n, SuiteSparse_long Pe[], SuiteSparse_long Iw[], SuiteSparse_long Len[],
                  SuiteSparse_long iwlen, SuiteSparse_long pfree, SuiteSparse_long Nv[], SuiteSparse_long Next[],
                  SuiteSparse_long Last[], SuiteSparse_long Head[], SuiteSparse_long Elen[], SuiteSparse_long Degree[],
                  SuiteSparse_long W[], double Control[], double Info[], const SuiteSparse_long C[],
                  SuiteSparse_long BucketSet[]) {
    TACHO_SUITESPARSE(camd_l2)
    (n, Pe, Iw, Len, iwlen, pfree, Nv, Next, Last, Head, Elen, Degree, W, Control, Info, C, BucketSet);
  }
};
...
public:
  ...
  void setConstraint(const ordinal_type nblk, const ordinal_type_array range, const ordinal_type_array peri) {
  ...
  void reorder(const ordinal_type verbose = 0) {
    ...
    TACHO_TEST_FOR_EXCEPTION(_nnz != pfree, std::logic_error,
                             ">> nnz in the graph does not match to nnz count (pfree)");

    ordinal_type *nv_ptr = reinterpret_cast<ordinal_type *>(_nv.data()); // 2) Nv
    ordinal_type *hd_ptr = swork_ptr;
    swork_ptr += (_m + 1);                                               // 3) Head
    ordinal_type *el_ptr = reinterpret_cast<ordinal_type *>(_el.data()); // 4) Elen
    ordinal_type *dg_ptr = swork_ptr;
    swork_ptr += _m; // 5) Degree
    ordinal_type *wk_ptr = swork_ptr;
    swork_ptr += (_m + 1); // 6) W
    ordinal_type *bk_ptr = swork_ptr;
    swork_ptr += _m; // 7) BucketSet

    const size_type iwlen = swlen - (4 * _m + 2);
    ordinal_type *iw_ptr = swork_ptr;
    swork_ptr += iwlen; // Iw
    ...
    CAMD<ordinal_type>::run(_m, pe_ptr, iw_ptr, lwork_ptr, iwlen, pfree,
                            // output
                            nv_ptr, next, perm, hd_ptr, el_ptr, dg_ptr, wk_ptr, _control, _info, cnst, bk_ptr);
    t_camd = timer.seconds();

    TACHO_TEST_FOR_EXCEPTION(_info[TRILINOS_CAMD_STATUS] != TRILINOS_CAMD_OK, std::runtime_error, "CAMD fails");
  ...
  std::ostream &showMe(std::ostream &os, const bool detail) const {
    ...
    const int w = 6;
    for (ordinal_type i = 0; i < _m; ++i)
      os << std::setw(w) << _cnst[i] << "   " << std::setw(w) << _perm[i] << "   " << std::setw(w) << _peri[i] << "   "
         << std::setw(w) << _pe[i] << "   " << std::setw(w) << _nv[i] << "   " << std::setw(w) << _next[i] << "   "
         << std::setw(w) << _el[i] << "   " << std::endl;

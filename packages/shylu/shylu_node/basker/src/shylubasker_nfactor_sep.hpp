...
#ifndef SHYLUBASKER_NFACTOR_SEP_HPP
#define SHYLUBASKER_NFACTOR_SEP_HPP
...
//#define BASKER_DEBUG_NFACTOR_SEP
    ...
    if((maxindex == BASKER_MAX_IDX) || (pivot == zero) )
    {
      if(Options.verbose == BASKER_TRUE)
      {
        ...
        if (Options.replace_tiny_pivot && normA_blk > STS::magnitude(zero) && maxindex != BASKER_MAX_IDX) {
          std::cout << "  + replace zero pivot with " << normA_blk * STS::squareroot(eps) << std::endl;
...
#undef BASKER_DEBUG_NFACTOR_SEP
#endif //end ifndef backer_nfactor_sep

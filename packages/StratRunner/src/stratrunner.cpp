...
double StratRunner::epetraNorm2( const Epetra_Vector &v )
{
  double norm[1] = { -1.0 };
  v.Norm2(&norm[0]);
  return norm[0];
...
int StratRunner::runStrat(QString xmlFileName, QString userMatrixFile, QTextEdit *outputDisplay){

	CustomStreamBuff myBuff(outputDisplay);
	std::cout.rdbuf(&myBuff);
  using Teuchos::rcp;
  using Teuchos::RCP;
...
//NEED TO REDFINE WHAT OUT IS AND REDIRECT IT TO MY OUTPUT DEVICE
  ...
  try {
    ...
    bool            showDoc                = true;*/
	std::string matrixFile = userMatrixFile.toStdString();
	double tol = 1e-5;
	bool onlyPrintOptions = false;
	bool printXmlFormat = false;
	bool showDoc = true;

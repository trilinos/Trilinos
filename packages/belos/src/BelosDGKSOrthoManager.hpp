...
#ifndef BELOS_BASIC_ORTHOMANAGER_HPP
#define BELOS_BASIC_ORTHOMANAGER_HPP
    ...
    do {
      ...
      if (rank == xc) {
        ...
        TEST_FOR_EXCEPTION( rank < oldrank, OrthoError,   
                            "Belos::DGKSOrthoManager::projectAndNormalize(): basis lost rank; this shouldn't happen");
    ...
    for (int j = xstart; j < xc; j++) {
      ...
      if (rankDef == true) {
        ...
        TEST_FOR_EXCEPTION( completeBasis, OrthoError, 
                            "Belos::DGKSOrthoManager::findBasis(): Unable to complete basis" );
        return j;
      }
...
#endif // BELOS_BASIC_ORTHOMANAGER_HPP

...
namespace Belos {
  ...
  /// \brief Error in TsqrOrthoManager or TsqrOrthoManagerImpl
  /// \author Mark Hoemmen
  ...
  /// and therefore by TsqrOrthoManager.  It is invoked right after
  /// discovering the need to reorthogonalize (for the first time),
  /// but before actually reorthogonalizing.  It is <i>only</i>
  /// invoked if reorthogonalization is necessary.  You can define
  /// your own callback by implementing this interface.
  ///
  ...
  /// implement TsqrOrthoManager and TsqrMatOrthoManager.
  ///
                           ...
                           Teuchos::ArrayView<Teuchos::RCP<const MV> > Q)
  {
    ...
    if (reorthogonalizeBlocks_) {
      ...
      DMT::nrm2(*DMT::Subview(*B_out, firstPassRank, firstPassRank), normsAfterFirstPass);
      ...
      if (reorthogonalize) {
        ...
        DMT::Multiply(false, false, SCT::one(), *B2, *B_copy, SCT::zero(), *B_out);
        ...
        for (int k = 0; k < num_Q_blocks; ++k) {
          // C[k] := C2[k]*B_copy + C[k].
          DMT::Multiply(false, false, SCT::one(), *C2[k], *B_copy, SCT::one(), *C[k]);
        }
        ...
        DMT::nrm2(*DMT::Subview(*B2, rank, rank), normsAfterSecondPass);
                ...
                Teuchos::RCP<DM> B) const
  {
    ...
    if (B.is_null()) {
      ...
      const int theNumRows = DMT::GetNumRows(*B);
      const int theNumCols = DMT::GetNumCols(*B;

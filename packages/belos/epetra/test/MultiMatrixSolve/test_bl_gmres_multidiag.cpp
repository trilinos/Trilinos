...
// @HEADER
//
// This test generates diagonal matrices for pseudo block GMRES to solve.
//
...
class Iterative_Inverse_Operator : public Vector_Operator
{
  ...
  Teuchos::RCP<LinearMultiShiftProblem<double,MV,OP> >   pProb;
  Teuchos::RCP<PseudoBlockGmresSolMgr<double,MV,OP> >      pBelos;
};
  ...
  timer(opString)
{
  ...
  pProb = Teuchos::rcp( new LinearMultiShiftProblem<double,MV,OP>() );
  ...
  pBelos = Teuchos::rcp( new PseudoBlockGmresSolMgr<double,MV,OP>(pProb, pList) );
}
...
int main(int argc, char *argv[])
{
  ...
  try {
    ...
    {
      Epetra_MultiVector Xi(Epetra_DataAccess::View, X, i, 1);
      Xi.PutScalar( 1.0 + i );
    }

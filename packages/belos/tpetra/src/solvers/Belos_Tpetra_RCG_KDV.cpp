...
void register_RCG_KDV_tmpl (const bool verbose)
{
  ...
  const char solverName[] = "RCG";
  {
    ...
    using solver_type = ::Belos::RCGSolMgr<SC, MV, OP, DM>;
    registerSolverSubclassForTypes<solver_type, SC, MV, OP, DM> (solverName);
  }
}

void register_RCG_KDV (const bool verbose)
{
  TPETRA_ETI_MANGLING_TYPEDEFS()

#ifdef BELOS_TPETRA_REGISTER_RCG_KDV
#  undef BELOS_TPETRA_REGISTER_RCG_KDV
#endif // BELOS_TPETRA_REGISTER_RCG_KDV
#define BELOS_TPETRA_REGISTER_RCG_KDV( SC, LO, GO, NT ) register_RCG_KDV_tmpl<SC, LO, GO, NT> (verbose);

  TPETRA_INSTANTIATE_SLGN_NO_ORDINAL_SCALAR( BELOS_TPETRA_REGISTER_RCG_KDV )

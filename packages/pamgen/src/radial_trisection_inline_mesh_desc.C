...
/*****************************************************************************/
{
  ...
  //Defaults for 3rd dimension of 2D mesh
  nel_tot[2] = 1;
  a_inline_n[2][0] = 1;
  c_inline_n[2][0] = 0;
  c_inline_n[2][1] = 1;
  ...
  for(long long k = 0; k < inline_b[2]; k ++){
    ...
    for(long long j = 0; j < inline_b[1]; j ++){
      for(long long i = 1; i < inline_b[0]; i ++){
        bl_ct = i + j * inline_b[0] + k * inline_b[0]*inline_b[1];
        cum_block_totals[bl_ct]=total_element_count;
        total_element_count += els_in_block[bl_ct];
      }
  ...
  if(enforce_periodic){
    Real ctotal_theta = c_block_dist[1][inline_b[1]];
    ...
    long long num_90_sections = (long long)(c_block_dist[1][inline_b[1]]/90.);
...
/****************************************************************************/
{
  ...
  Real total_theta = c_block_dist[1][inline_b[1]]-inline_gmin[1];
  ...
  for(long long i = 0; i < trisection_blocks; i ++){
   quads[i] = new Quad_Patch(Vector(0,0,0),
                              Vector(transition_radius*cos(deg_to_rad*(inline_gmin[1] + dtheta*(Real)(2*i+0))),
                                     transition_radius*sin(deg_to_rad*(inline_gmin[1] + dtheta*(Real)(2*i+0))), 0.),
                              Vector(transition_radius*cos(deg_to_rad*(inline_gmin[1] + dtheta*(Real)(2*i+1))),
                                     transition_radius*sin(deg_to_rad*(inline_gmin[1] + dtheta*(Real)(2*i+1))), 0.),
                              Vector(transition_radius*cos(deg_to_rad*(inline_gmin[1] + dtheta*(Real)(2*i+2))),
                                     transition_radius*sin(deg_to_rad*(inline_gmin[1] + dtheta*(Real)(2*i+2))), 0.));
  }
...
/****************************************************************************/
{
  ...
  for(setit = nodeset_list.begin(); setit != nodeset_list.end();setit++,nsct ++){
    ...
    if((!(*setit)->the_locs[ict].block_boundary_set) ||
       ((*setit)->the_locs[ict].block_boundary_set && (((*setit)->the_locs[ict].block_id-1)%inline_b[0] == 0))){
  ...
  //SIDESETS
  // Sidesets are defined by the element id and the id of the face to
  // which the set applies.

  // For the purposes of calculation the sideset definition process
  // starts by considering all elements to be numbered sequentially in
  // i then j then k across all blocks. This is not actually the case
  // since exodus requires elements to be numbered sequentially within
  // a block.

  // If we consider the elements to be numbered sequentially across
  // blocks in i,j, and k then we can use a loop limits type call to
  // get the i,j,and k index limits of the elements in the sideset. We
  // can then loop over these limits to calculate the index of the
  // element if it were numbered across blocks. It is then left to
  // calculate the index of this element if it were numbered
  // sequentially within blocks.  This is done by calculating the
  // global i,j,and k indices of the element within the entire domain,
  // calculating the indices of the block within which the element
  // resides, calculating the ordinal number of the block in which the
  // element resides, and the local i,j,and k indices of the element
  // within the block it resides.

  // These values are combined to calculate the index of the element
  // that corresponds to numbering the elements sequentially within
  // blocks.
  nsct = 0;

  // NOTE (mfh 06 Oct 2014) It costs linear time to compute the length
  // (size) of an std::list!  Don't do it more often than you need to!
  // In particular, don't use sideset_list.size() as the upper bound
  // of a for loop, because that will make the loop at least quadratic
  // time in the list length!
  const size_t sidesetListSize = sideset_list.size ();
  if (sidesetListSize != 0) {
    sideset_vectors = new std::vector<std::pair<long long, Topo_Loc > > [sidesetListSize];
    sideset_global_count = new long long [sidesetListSize];
    for (size_t ict = 0; ict < sidesetListSize; ++ict) {
      sideset_global_count[ict] = 0.;
    }
  }
  for (setit = sideset_list.begin(); setit != sideset_list.end(); setit++, nsct++) {
    // FIXME (mfh 06 Oct 2014) Should we use size_t instead of
    // unsigned here?  The STL's .size() methods usually return
    // size_t, which is 64 bits on a 64-bit machine, but unsigned may
    // be 32 bits (same as int).
    for (unsigned ict = 0; ict < (*setit)->the_locs.size (); ict++) {
      Topo_Loc the_location =  (*setit)->the_locs[ict].location;
      ...
      if (dimension == 3) {
        for (long long _nk_ = ll.ks; _nk_ < ll.ke; _nk_ ++) {
          for (long long _nj_ = ll.js; _nj_ < ll.je; _nj_ ++) {
            for (long long _ni_ = ll.is; _ni_ < ll.ie; _ni_ ++) {
              long long elnumber =
                get_element_number_from_l_i_j_k (trisection_blocks,
                                                 _ni_, _nj_, _nk_);
      ...
      if ((!(*setit)->the_locs[ict].block_boundary_set) ||
          ((*setit)->the_locs[ict].block_boundary_set &&
           (((*setit)->the_locs[ict].block_id - 1) % inline_b[0] == 0))) {
        // need to do trisect blocks now
        for(long long i = 0; i < trisection_blocks; i ++){
          Topo_Loc new_location;
          long long is_nodes = 0;
          LoopLimits tll =
            get_tri_block_limits ((*setit)->the_locs[ict].block_boundary_set,
                                  (*setit)->the_locs[ict].block_id, i,
                                  the_location, new_location, is_nodes);
...
/****************************************************************************/
{
  ...
  for(setit = nodeset_list.begin(); setit != nodeset_list.end();setit++){
    ...
    if((*setit)->the_locs[ict].block_boundary_set){
      ...
      {
        long long jind = 0;
        if(jord && (inline_b[0]-1)){
          jind = (jord-1)/(inline_b[0]-1);
        }
        long long iind = jord;
        if (jind){
          iind =1+ ((jord-1)-jind*(inline_b[0]-1));
        }
        (*setit)->the_locs[ict].block_id = 1 + iind + jind*inline_b[0] + kind * inline_b[0]*inline_b[1];
      }
    }
  }
  }
...
/****************************************************************************/
{
  ...
  long long mesh_block_id = Ibg + Jbg*inline_b[0] + Kbg*inline_b[0]*inline_b[1];

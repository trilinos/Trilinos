...
\code{FaceSets}, \code{EdgeSets},
\code{CommSets} defining the current model. These are all
...
and two sidesets (facesets) is shown in Figure~\ref{fig:block}.
\begin{figure}[htp]
\centering
...
Note that the \code{FaceSets} are further divided into one or more
\code{FaceBlocks} which contain entities of homogenous topology.
...
The \code{EdgeSet} and \code{FaceSet} classes provides the properties:
\begin{quote}
\begin{tabular}{ll}
\code{block\_count}         &(Integer) \\
\code{edge\_block\_count} &(Integer) (for EdgeSet) \\
\code{face\_block\_count} &(Integer) (for FaceSet) \\
\end{tabular}
\end{quote}
...
The \code{EdgeBlock} and \code{FaceBlock} classes provides the additional properties:
\begin{quote}
\begin{tabular}{ll}
\code{distribution\_factor\_count}     &(Integer) \\
\end{tabular}
...
\code{face\_set\_count}      &(Integer) \\
\code{edge\_set\_count}      &(Integer) \\
...
int  get\_field (const FaceBlock *fb, const Field \&field, void *data, size\_t data\_size)
int  get\_field (const EdgeBlock *fb, const Field \&field, void *data, size\_t data\_size)
int  get\_field (const NodeSet *ns, const Field \&field, void *data, size\_t data\_size)
int  get\_field (const EdgeSet *es, const Field \&field, void *data, size\_t data\_size)
int  get\_field (const FaceSet *fs, const Field \&field, void *data, size\_t data\_size)
...
int  put\_field (const FaceBlock *fb, const Field \&field, void *data, size\_t data\_size)
int  put\_field (const EdgeBlock *fb, const Field \&field, void *data, size\_t data\_size)
int  put\_field (const NodeSet *ns, const Field \&field, void *data, size\_t data\_size)
int  put\_field (const EdgeSet *es, const Field \&field, void *data, size\_t data\_size)
int  put\_field (const FaceSet *fs, const Field \&field, void *data, size\_t data\_size)
...
\item [\code{virtual bool  supports\_edge\_fields()}]--
true/false depending on whether this database type supports fields
written on edges in edgesets.
\item [\code{virtual bool  supports\_face\_fields()}]-- true/false

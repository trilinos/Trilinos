...
\begin{enumerate}
  ...
  numeric computations are done. In this step, column indices for $C$ are computed and used to construct a graph.
...
the amount of work done on each node does constant. We define weak-scaling efficiency as $Time_{serial}/Time_{parallel}$.
...
waste our limited computing resources. ML is not included in these results because it does not have a transpose
mode~\footnote{One could first explicitly transpose a matrix and then give it to the ML algorithm. We did not do this because
the point of this test was to compare EpetraExt and Tpetra.}
...
ML's matrix-matrix multiply routine has a complex hashing scheme in order to speed lookup times for 

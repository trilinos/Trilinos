    ...
    void Gemm(const int N) {
        ...
        for (int k=0;k<N*VectorLength;++k) {
          ...
          for (int i=0;i<BlkSize;++i)
            for (int j=0;j<BlkSize;++j) {
              a(k, i, j) = ValueType(k1 + 1, k1 + 6);
              b(k, i, j) = ValueType(k0 + 1, k0 + 6);
              c(k, i, j) = ValueType(0,0);
            }
        }
      ...
      ///
      {
        ...
        for (int k=0;k<N*VectorLength;++k) {
          ...
          for (int i=0;i<BlkSize;++i)
            for (int j=0;j<BlkSize;++j) {
              a_host(k, i, j) = ValueType(k1 + 1, k1 + 6);
              b_host(k, i, j) = ValueType(k0 + 1, k0 + 6);
              c_host(k, i, j) = ValueType(0,0);
            }
        }
...

        {
          ...
          for (int i=0;i<cref.dimension(0);++i)
            for (int j=0;j<cref.dimension(1);++j)
              for (int k=0;k<cref.dimension(2);++k)
                diff += Kokkos::abs(cref(i,j,k) - c_host(i,j,k));
      ...
      ///
      {
        ...
        for (int k0=0;k0<N;++k0)
          for (int k1=0;k1<VectorLength;++k1)
            for (int i=0;i<BlkSize;++i)
              for (int j=0;j<BlkSize;++j) {
                a_host(k0, i, j)[k1] = ValueType(k1 + 1, k1 + 6);
                b_host(k0, i, j)[k1] = ValueType(k0 + 1, k0 + 6);
                c_host(k0, i, j)[k1] = ValueType(0,0);
              }
...

        {
          ...
          for (int i=0;i<cref.dimension(0);++i)
            for (int j=0;j<cref.dimension(1);++j)
              for (int k=0;k<cref.dimension(2);++k)
                diff += Kokkos::abs(cref(i,j,k) - c_host(i/VectorLength,j,k)[i%VectorLength]);
...
int main(int argc, char *argv[]) {
  ...
  {    
    ...
    for (int i=0;i<ntest;++i) {
      ...
      // std::cout << "\n Testing SIMD2 complex and Algo::Gemm::Triple\n";
      // run<VectorTag<SIMD<Kokkos::complex<double> >,2>,Algo::Gemm::Triple>(N[i]/2);
      
      // std::cout << "\n Testing AVX256 complex and Algo::Gemm::Triple\n";
      // run<VectorTag<AVX<Kokkos::complex<double> >,2>,Algo::Gemm::Triple>(N[i]/2);
      
      //std::cout << "\n Testing SIMD2 complex and Algo::Gemm::Blocked\n";
      //run<VectorTag<SIMD<Kokkos::complex<double> >,2>,Algo::Gemm::Blocked>(N[i]/2);
      
      std::cout << "\n Testing AVX256 complex and Algo::Gemm::Blocked\n";
      run<VectorTag<AVX<Kokkos::complex<double> >,2>,Algo::Gemm::Blocked>(N[i]/2);
    }
#endif
    ...
    for (int i=0;i<ntest;++i) {
      ...
      std::cout << "\n Testing AVX512 complex and Algo::Gemm::Blocked\n";
      run<VectorTag<AVX<Kokkos::complex<double> >,4>,Algo::Gemm::Blocked>(N[i]/4);
    }
#endif
  }

  ...
  public:
      ...
      , ElementSpaceData_()
    {
      ...
      if (numGlobalElements < zero)
        throw reportError("numGlobalElements = " + toString(numGlobalElements) 
      ...
      ElementSpaceData_ = Teuchos::rcp(new ElementSpaceData<OrdinalType>(indexBase, numGlobalElements, 
                                         ...
                                         minMyGID, maxMyGID, lgMap, 
                                         glMap, true, platform_clone, comm));
      ...
      , ElementSpaceData_()
    {
      ...
      if(numGlobalElements < negOne) 
        throw reportError("numGlobalElements = " + toString(numGlobalElements) 
                  + ".  Should be >= " + toString(negOne) + ".", -1);
      if(numMyElements < zero) 
        throw reportError("numMyElements = " + toString(numMyElements) 
      ...
      if(numGlobalElements == negOne)
        ...
        throw reportError("Invalid numGlobalElements.  numGlobalElements = " 
                  + toString(numGlobalElements) + ".  Should equal " 
                  + toString(global_sum) + ", or be set to -1 to compute automatically", -3);
    ...
    ~ElementSpace()
    {
      if (data().haveDirectory_ == true) {
        Teuchos::RCP< Directory<OrdinalType> > dummy_dir;
        data().Directory_ = dummy_dir;
        data().haveDirectory_ = false;
      }
    }
    ...
    void getRemoteIDList(std::vector<OrdinalType> const& GIDList, 

...
// ***********************************************************************
// 
//     Tpetra: Tpetra Extended - Linear Algebra Services Package
...
#include <Tpetra_Transpose_RowMatrix.hpp>
...
//
template<Scalar S, LocalOrdinal T>
S sparsedot(S* u, T* u_ind,  size_t u_len,
		 S* v, T* v_ind, size_t v_len)
{
  S result = 0.0;
...
/*
template<Scalar S, LocalOrdinal T, GlobalOrdinal T2>
	     ...
	     CrsWrapper& C)
{
  ...
  size_t C_numCols = C_lastCol - C_firstCol + 1;
  size_t C_numCols_import = C_lastCol_import - C_firstCol_import + 1;

  if (C_numCols_import > C_numCols) C_numCols = C_numCols_import;
  S* dwork = new S[C_numCols];
  T* iwork = new int[C_numCols];

  S* C_row_i = dwork;
  T* C_cols = iwork;

  size_t C_row_i_length;
  T i, j, k;
  ...
  for(i=0; i<Aview.numRows; ++i) {
    ...
    T* Aindices_i = Aview.indices[i];
    S* Aval_i  = Aview.values[i];

    T2 global_row = Aview.rowMap->GID(i);
    ...
    for(k=0; k<Aview.numEntriesPerRow[i]; ++k) {
      T Ak = Bview.rowMap->LID(Aview.colMap->GID(Aindices_i[k]));
      S Aval = Aval_i[k];

      T* Bcol_inds = Bview.indices[Ak];
      S* Bvals_k = Bview.values[Ak];
  ...
  for(i=0; i<Aview.numRows; ++i) {
	...
	if (C_filled) {
	  //C.Filled()==true, and C doesn't have all the necessary nonzero
          ...
          //than 0 or non local).
          std::cerr << "Tpetra::MatrixMatrix::Multiply Warning: failed "
		       ...
		       CrsWrapper& C)
{
  ...
  const Tpetra_Map* Crowmap = &(C.RowMap());
...
int import_and_extract_views(const Tpetra_CrsMatrix& M,
			     const Tpetra_Map& targetMap,
			     CrsMatrixStruct& Mview)
{
  ...
  const Tpetra_Map& Mrowmap = M.RowMap();
  ...
  for(i=0; i<Mview.numRows; ++i) {
    ...
    if (mlid < 0) {
      ...
      TPETRA_CHK_ERR( M.ExtractMyRowView(mlid, Mview.numEntriesPerRow[i],
  ...
  if (numProcs < 2) {
    if (Mview.numRemote > 0) {
      cerr << "Tpetra::MatrixMatrix::Multiply ERROR, numProcs < 2 but "
  ...
  if (globalMaxNumRemote > 0) {
    ...
    Tpetra_Map MremoteRowMap(-1, Mview.numRemote, MremoteRows,
			     Mrowmap.IndexBase(), Mrowmap.Comm());

    //Create an importer with target-map MremoteRowMap and 
    //source-map Mrowmap.
    Tpetra_Import importer(MremoteRowMap, Mrowmap);

    //Now create a new matrix into which we can import the remote rows of M
    //that we need.
    Mview.importMatrix = new Tpetra_CrsMatrix(Copy, MremoteRowMap, 1);

    TPETRA_CHK_ERR( Mview.importMatrix->Import(M, importer, Insert) );

    TPETRA_CHK_ERR( Mview.importMatrix->FillComplete(M.DomainMap(), M.RangeMap()) );
	...
	TPETRA_CHK_ERR( Mview.importMatrix->ExtractMyRowView(importLID,
...
int distribute_list(const Tpetra_Comm& Comm,
                    int lenSendList,
                    const int* sendList,
                    int& maxSendLen,
                    int*& recvList)
...
Tpetra_Map* create_map_from_imported_rows(const Tpetra_Map* map,
					  int totalNumSend,
					  int* sendRows,
					  int numProcs,
					  int* numSendPerProc)
{
  ...
  Tpetra_Distributor* distributor = map->Comm().CreateDistributor();
  ...
  Tpetra_Map* import_rows = new Tpetra_Map(-1, numRecv, recvRows,
...
int form_map_union(const Tpetra_Map* map1,
		   const Tpetra_Map* map2,
		   const Tpetra_Map*& mapunion)
{
  //form the union of two maps

  if (map1 == NULL) {
    mapunion = new Tpetra_Map(*map2);
    return(0);
  }

  if (map2 == NULL) {
    mapunion = new Tpetra_Map(*map1);
    return(0);
  }
  ...
  mapunion = new Tpetra_Map(-1, union_offset, union_elements,
...
Tpetra_Map* find_rows_containing_cols(const Tpetra_CrsMatrix& M,
                                      const Tpetra_Map* colmap)
{
  ...
  if (numProcs < 2) {
    Tpetra_Map* result_map = NULL;

    int err = form_map_union(&(M.RowMap()), NULL,
                             (const Tpetra_Map*&)result_map);
    if (err != 0) {
      return(NULL);
    }
    return(result_map);
  ...
  const Tpetra_CrsGraph& Mgraph = M.Graph();
  const Tpetra_Map& Mrowmap = M.RowMap();
  const Tpetra_Map& Mcolmap = M.ColMap();
  ...
  for(int row=0; row<MnumRows; ++row) {
    ...
    for(int j=0; j<MnumCols; ++j) {
      ...
      for(int p=0; p<numProcs; ++p) {
        ...
        int insertPoint;
        int foundOffset = Tpetra_Util_binary_search(colGID, procCols[p],
  ...
  Tpetra_Map* recvd_rows =
    create_map_from_imported_rows(&Mrowmap, totalNumSend,
                                  procRows_1D, numProcs, procNumRows);

  Tpetra_Map* result_map = NULL;

  err = form_map_union(&(M.RowMap()), recvd_rows, (const Tpetra_Map*&)result_map);
  if (err != 0) {
    return(NULL);
  }
...
*/
int MatrixMatrix::Multiply(const Tpetra_CrsMatrix& A,
			   bool transposeA,
			   const Tpetra_CrsMatrix& B,
			   bool transposeB,
			   Tpetra_CrsMatrix& C,
                           bool call_FillComplete_on_result)
{
  ...
  //the domain-map.
  const Tpetra_Map* domainMap_A = &(A.DomainMap());
  const Tpetra_Map* domainMap_B = &(B.DomainMap());

  const Tpetra_Map* rowmap_A = &(A.RowMap());
  const Tpetra_Map* rowmap_B = &(B.RowMap());
  ...
  const Tpetra_Map* workmap1 = NULL;
  const Tpetra_Map* workmap2 = NULL;
  const Tpetra_Map* mapunion1 = NULL;
  ...
  const Tpetra_Map* targetMap_A = rowmap_A;
  const Tpetra_Map* targetMap_B = rowmap_B;
  ...
  TPETRA_CHK_ERR( import_and_extract_views(A, *targetMap_A, Aview) );

  //We will also need local access to all rows of B that correspond to the
  //column-map of op(A).
  if (numProcs > 1) {
    const Tpetra_Map* colmap_op_A = NULL;
    if (transposeA) {
    ...
    //local processor.
    if (transposeB) {
      TPETRA_CHK_ERR( form_map_union(colmap_op_A, domainMap_B, mapunion1) );
  ...
  TPETRA_CHK_ERR( import_and_extract_views(B, *targetMap_B, Bview) );
  ...
  if (!C.Filled()) {
    ...
    switch(scenario) {
    case 1:    TPETRA_CHK_ERR( mult_A_B(Aview, Bview, crsgraphbuilder) );
      break;
    case 2:    TPETRA_CHK_ERR( mult_A_Btrans(Aview, Bview, crsgraphbuilder) );
      break;
    case 3:    TPETRA_CHK_ERR( mult_Atrans_B(Aview, Bview, crsgraphbuilder) );
      break;
    case 4:    TPETRA_CHK_ERR( mult_Atrans_Btrans(Aview, Bview, crsgraphbuilder) );
      break;
    }
    ...
    if (call_FillComplete_on_result) {
      ...
      TPETRA_CHK_ERR( C.FillComplete(*domainmap, *rangemap) );
  ...
  CrsWrapper_Tpetra_CrsMatrix ecrsmat(C);

  switch(scenario) {
  case 1:    TPETRA_CHK_ERR( mult_A_B(Aview, Bview, ecrsmat) );
    break;
  case 2:    TPETRA_CHK_ERR( mult_A_Btrans(Aview, Bview, ecrsmat) );
    break;
  case 3:    TPETRA_CHK_ERR( mult_Atrans_B(Aview, Bview, ecrsmat) );
    break;
  case 4:    TPETRA_CHK_ERR( mult_Atrans_Btrans(Aview, Bview, ecrsmat) );
    break;
  }
...
/*
int MatrixMatrix::Add(const Tpetra_CrsMatrix& A,
                      bool transposeA,
                      double scalarA,
                      Tpetra_CrsMatrix& B,
                      double scalarB )
{
  ...
  if (!A.Filled() ) {
    std::cerr << "Tpetra::MatrixMatrix::Add ERROR, input matrix A.Filled() is false, it is required to be true. (Result matrix B is not required to be Filled())."<<std::endl;
    TPETRA_CHK_ERR(-1);
  }

  //explicit tranpose A formed as necessary
  Tpetra_CrsMatrix * Aprime = 0;
  Tpetra::RowMatrix_Transpose * Atrans = 0;
  if( transposeA )
  {
    Atrans = new Tpetra::RowMatrix_Transpose();
    Aprime = &(dynamic_cast<Tpetra_CrsMatrix&>(((*Atrans)(const_cast<Tpetra_CrsMatrix&>(A)))));
  }
  else
    Aprime = const_cast<Tpetra_CrsMatrix*>(&A);

  int MaxNumEntries = TPETRA_MAX( A.MaxNumEntries(), B.MaxNumEntries() );
  ...
  if( scalarA )
  {
    ...
    {
      Row = B.GRID(i);
      TPETRA_CHK_ERR( Aprime->ExtractGlobalRowCopy( Row, MaxNumEntries, A_NumEntries, A_Values, A_Indices ) );

      if (scalarB != 1.0) {
        if (!B.Filled()) {
          TPETRA_CHK_ERR( B.ExtractGlobalRowView( Row, B_NumEntries,
                                                  B_Values, B_Indices));
        }
        else {
          TPETRA_CHK_ERR( B.ExtractMyRowView( i, B_NumEntries,
                                              B_Values, B_Indices));
        }
    ...
    TPETRA_CHK_ERR( B.Scale(scalarB) );
  }
                      ...
                      Tpetra_CrsMatrix * & C)
{
  ...
  if (!A.Filled() || !B.Filled() ) {
     std::cerr << "Tpetra::MatrixMatrix::Add ERROR, input matrix A.Filled() or B.Filled() is false,"
               << "they are required to be true. (Result matrix C should be an empty pointer)" << std::endl;
     TPETRA_CHK_ERR(-1);
  }

  Tpetra_CrsMatrix * Aprime = 0, * Bprime=0;
  Tpetra::RowMatrix_Transpose * Atrans = 0,* Btrans = 0;
  ...
  if( transposeB ) {
     Btrans = new Tpetra::RowMatrix_Transpose();
     Bprime = &(dynamic_cast<Tpetra_CrsMatrix&>(((*Btrans)(const_cast<Tpetra_CrsMatrix&>(B)))));
  }
  else
     Bprime = const_cast<Tpetra_CrsMatrix*>(&B);

  // allocate or zero the new matrix
  if(C!=0)
     C->PutScalar(0.0);
  else
     C = new Tpetra_CrsMatrix(Copy,Aprime->RowMap(),0);

  // build arrays  for easy resuse
  int ierr = 0;
  Tpetra_CrsMatrix * Mat[] = { Aprime,Bprime};
  ...
  for(int k=0;k<2;k++) {
     ...
     for( int i = 0; i < NumMyRows; ++i ) {
        Row = Mat[k]->GRID(i);
        TPETRA_CHK_ERR( Mat[k]->ExtractGlobalRowCopy( Row, MaxNumEntries, NumEntries, Values, Indices));

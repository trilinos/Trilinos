	...
	*/
	...
	public:
		...
		*/
		...
		void setOutputManager(OutputObject const& outputObject) {
			om_ = outputObject.getOutputManager(); 
		}
  
		//! Set the output manager to default: no manager.
		void unsetOutputManager() {
			om_ = Teuchos::null;
		}
		...
		*/
		virtual void printSignature(ostream& os) const {
			os << "Replace this version of printSignature with your own." << endl; // Default is to print nothing
		}
		...
		*/
		virtual void printSummary(ostream& os) const {
			os << "Replace this version of printSummary with your own." << endl; // Default is to print nothing
		}
		...
		/*! Prints information about an object when the state of an object changes.
			...
			\param os (InOut) The ostream to which information will be printed.
			\warning Calls to this method should normally be surrounded by 
			         #ifdef HAVE_DEBUG...#endif since performance will be potential be impacted.
		*/

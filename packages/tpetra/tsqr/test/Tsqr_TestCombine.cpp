...
parseOptions(int argc,
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(params.benchmark && params.numTrials < 1,
                             ...
                             "If you set --benchmark, then the "
                             "number of trials must be positive, but you set --numTrials="
                                 << params.numTrials << ".");
#ifndef HAVE_TPETRATSQR_COMPLEX
  TEUCHOS_TEST_FOR_EXCEPTION(params.testComplex, std::invalid_argument,
                             "Complex "
...
int main(int argc, char* argv[]) {
  ...
  TEUCHOS_STANDARD_CATCH_STATEMENTS(actually_print_caught_exceptions, cerr, success);

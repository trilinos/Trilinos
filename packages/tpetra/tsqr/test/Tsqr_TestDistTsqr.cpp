 ...
 public:
  ...
  verify(const Ordinal numCols,
    ...
    if (debug_) {
      scalarComm_->barrier();
      if (myRank == 0) {
        err_ << "-- DistTsqr object initialized" << endl
             << endl;
      }
    }
    ...
    if (testFactorExplicit_) {
      ...
      if (printMatrices_) {
        if (myRank == 0) {
          err_ << std::endl
               << "Computed Q factor:" << std::endl;
        }
        printGlobalMatrix(err_, Q_local, scalarComm_.get(),
                          ordinalComm_.get());
        if (myRank == 0) {
          err_ << std::endl
               << "Computed R factor:" << std::endl;
...
parseOptions(int argc,
  ...
  try {
    ...
    const char docString[] =
        "This program tests TSQR::DistTsqr, which "
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(params.benchmark && params.numTrials < 1, std::invalid_argument,

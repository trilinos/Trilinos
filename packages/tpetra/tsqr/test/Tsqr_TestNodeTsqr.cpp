...
void printNodeTestParameters(std::ostream& out,
...
parseOptions(int argc,
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(params.numRows <= 0, std::invalid_argument,
                             "Number of "
                             "rows must be positive, but you set --numRows="
                                 << params.numRows << ".");
  TEUCHOS_TEST_FOR_EXCEPTION(params.numCols <= 0, std::invalid_argument,
                             "Number of "
                             "columns must be positive, but you set --numCols="
                                 << params.numCols << ".");
  TEUCHOS_TEST_FOR_EXCEPTION(params.numRows < params.numCols, std::invalid_argument,
                             "Number of rows must be >= number of columns, but you set "
                             "--numRows="
                                 << params.numRows << " and --numCols=" << params.numCols << ".");
  TEUCHOS_TEST_FOR_EXCEPTION(params.benchmark && params.numTrials < 1,
                             std::invalid_argument,
                             "Since you set --benchmark, the "
                             "number of trials must be positive, but you set --numTrials="
                                 << params.numTrials << ".");
  TEUCHOS_TEST_FOR_EXCEPTION(cacheSizeHintAsInt < 0, std::invalid_argument,
                             "Cache size "
...
getDeviceMatrixCopy(const MatView<LO, Scalar>& A,
  ...
  const size_t nrows(A.extent(0));
  const size_t ncols(A.extent(1));
  device_matrix_type A_dev(view_alloc(label, WithoutInitializing), nrows, ncols);
...
getNodeTsqr(const NodeTestParameters& p,
  ...
  const std::string nodeTsqrType = overrideNodeTsqrType == "" ? p.nodeTsqrType : overrideNodeTsqrType;
...
verifyNodeTsqrTmpl(std::ostream& out,
  ...
  if (relResidError > relResidBound) {
    success = false;
    if (verbose) {
      const std::string relResStr(results[2] == STM::zero() ? " / ||A||_F" : "");
...
bool verifyNodeTsqr(std::ostream& out,
                    const NodeTestParameters& p) {
  ...
  if (p.testComplex) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error,
                               "TSQR was not built with complex "
                               "arithmetic support.");
#endif  // HAVE_TPETRATSQR_COMPLEX
  }
  return success;
...
template <class Scalar>
void verifyLapackImplementations(std::ostream& out,
                                 std::vector<int>& iseed,
                                 const NodeTestParameters& p) {
...
void verifyLapack(std::ostream& out,
                  const NodeTestParameters& p) {
  ...
  if (p.testComplex) {
#ifdef HAVE_TPETRATSQR_COMPLEX
    verifyLapackImplementations<std::complex<float>>(out, iseed, p);
    verifyLapackImplementations<std::complex<double>>(out, iseed, p);
#else   // HAVE_TPETRATSQR_COMPLEX
    TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error,
                               "TSQR was not built with complex "
                               "arithmetic support.");
...
template <template <class SC> class LapackType, class Scalar>
void benchmarkLapackTmpl(std::ostream& out,
                         std::vector<int>& iseed,
                         LapackType<Scalar>& lapack,
                         const NodeTestParameters& params,
                         const std::string& lapackImplName) {
...
template <class Scalar>
void benchmarkLapackImplementations(std::ostream& out,
                                    std::vector<int>& iseed,
                                    const NodeTestParameters& p) {
#if defined(HAVE_TPETRATSQR_CUBLAS) && defined(HAVE_TPETRATSQR_CUSOLVER)
  {
...
void benchmarkLapack(std::ostream& out,
                     const NodeTestParameters& p) {
  std::vector<int> iseed{{0, 0, 0, 1}};
  if (p.testReal) {
...
template <class Scalar>
void benchmarkNodeTsqrTmpl(std::ostream& out,
                           std::vector<int>& iseed,
...
template <class Scalar>
void benchmarkNodeTsqrImplementation(std::ostream& out,
...
template <class Scalar>
void benchmarkNodeTsqrImplementations(std::ostream& out,
                                      std::vector<int>& iseed,
                                      const NodeTestParameters& p) {
...
void benchmarkNodeTsqr(std::ostream& out,
                       const NodeTestParameters& p) {
  using Teuchos::TypeNameTraits;
  ...
  if (p.testComplex) {
#ifdef HAVE_TPETRATSQR_COMPLEX
    benchmarkNodeTsqrImplementations<std::complex<float>>(out, iseed, p);
    benchmarkNodeTsqrImplementations<std::complex<double>>(out, iseed, p);
#else   // Don't HAVE_TPETRATSQR_COMPLEX
    TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error,
                               "TSQR was not built with complex arithmetic support.");
#endif  // HAVE_TPETRATSQR_COMPLEX

 ...
 private:
  ...
  getNodeTsqr(
    ...
    if (testParams->isType<size_t>(cacheSizeHintParamName)) {
      ...
      cacheSizeHint = static_cast<size_t>(testParams->get<int>(cacheSizeHintParamName));
      nodeTsqrParams->set(cacheSizeHintParamName, cacheSizeHint);
    }
  ...
  static bool verify(
    ...
    if (testFactorExplicit) {
      ...
      if (!found_nonzero_in_R) {
        success = false;
        if (myRank == 0) {
          const std::string prefix(verbose ? "  - *** " : "*** ");
    ...
    if (testRankRevealing) {
      ...
      const ordinal_type rank = [&]() {
        Scalar* Q_raw          = tsqr->wants_device_memory() ? reinterpret_cast<Scalar*>(Q_d.data()) : Q_local.data();
        const ordinal_type ldq = tsqr->wants_device_memory() ? Q_d.stride(1) : Q_local.stride(1);
      ...
      if (two_to_the_numCols > magnitude_type(10) * STM::eps()) {
        TEUCHOS_TEST_FOR_EXCEPTION(rank != numCols, std::logic_error, "The matrix of " << numCols << " columns should have full numerical rank, "
                                                                                                     "but Tsqr reports that it has rank "
                                                                                       << rank << ".  "
      ...
      for (int trialNum = 0; trialNum < numTrials; ++trialNum) {
        if (testFactorExplicit) {
          if (tsqr->wants_device_memory()) {
            ...
            Scalar* A_raw = contiguousCacheBlocks ? reinterpret_cast<Scalar*>(A_copy_d.data()) : reinterpret_cast<Scalar*>(A_d.data());
            Scalar* Q_raw = reinterpret_cast<Scalar*>(Q_d.data());
            tsqr->factorExplicitRaw(A_copy_d.extent(0),
                                    A_copy_d.extent(1),
                                    A_raw,
            ...
            Scalar* A_raw = contiguousCacheBlocks ? A_copy.data() : A_local.data();
            Scalar* Q_raw = Q_local.data();
            tsqr->factorExplicitRaw(A_copy.extent(0),
                                    A_copy.extent(1),
                                    A_raw,
          ...
          if (tsqr->wants_device_memory()) {
            ...
            const Scalar* A_raw = contiguousCacheBlocks ? reinterpret_cast<Scalar*>(A_copy_d.data()) : reinterpret_cast<Scalar*>(A_d.data());
            Scalar* Q_raw       = reinterpret_cast<Scalar*>(Q_d.data());
            tsqr->explicit_Q(numRowsLocal, numCols,
                             A_raw, A_copy_d.stride(1),
                             factorOutput, numCols,
            ...
            const Scalar* A_raw = contiguousCacheBlocks ? A_copy.data() : A_local.data();
            Scalar* Q_raw       = Q_local.data();
            tsqr->explicit_Q(numRowsLocal, numCols,
                             A_raw, A_copy.stride(1),
                             factorOutput, numCols,
...
class FullTsqrVerifierCaller {
 public:
  ...
  getValidParameterList() const {
    ...
    plist->set("NodeTsqr", nodeTsqr,
               "NodeTsqr subclass to use; "
  ...
  validateRandomSeed(const std::vector<int>& seed) {
    ...
    for (size_t k = 0; k < seed.size(); ++k) {
      TEUCHOS_TEST_FOR_EXCEPTION(seed[k] < 0 || seed[k] > 4095, std::invalid_argument,
                                 "seed[" << k << "]=" << seed[k] << " is invalid.  "
                                                                    "Each of the four seeds must be in [0, 4095].");
    }
    TEUCHOS_TEST_FOR_EXCEPTION(seed[3] % 2 != 1, std::invalid_argument, "seed[3]=" << seed[3] << " is invalid: it must be odd.");
    return seed;
  }

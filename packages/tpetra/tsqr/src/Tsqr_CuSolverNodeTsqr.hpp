...
#define TSQR_IMPL_CHECK_LAST_CUDA_ERROR(location)                                                                            \
  do {                                                                                                                       \
    ...
    if (errCode != cudaSuccess) {                                                                                            \
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, "At \"" << (location) << "\", "                                   \
                                                                                    "CUDA is in the following error state: " \
                                                                   << errorString);                                          \
    }                                                                                                                        \
  } while (false)
...
using kokkos_view_value_type = typename std::conditional<
    ...
    non_const_kokkos_value_type<Scalar>>::type;
...
template <class T>
void reallocDeviceVectorIfNeeded(device_vector_type<T>& vec,
                                 const char label[],
...
get_mat_view(const size_t nrows,
             ...
             Scalar A[],
             const size_t lda) {
  static_assert(!std::is_const<non_const_kokkos_value_type<Scalar>>::value,
                "non_const_kokkos_value_type is const.");
  using KVVT = kokkos_view_value_type<Scalar>;  // preserves const
  static_assert((std::is_const<Scalar>::value && std::is_const<KVVT>::value) ||
...
get_host_mat_view(const size_t nrows,
                  ...
                  Scalar A[],
                  const size_t lda) {
  return get_mat_view<Scalar, host_device_type>(nrows, ncols, A, lda);
}
...
get_host_mat_view(const MatView<Ordinal, Scalar>& A_host) {
  ...
  return get_mat_view<Scalar, host_device_type>(nrows, ncols, A_host.data(), lda);
}
...
get_contiguous_device_mat_view(device_vector_type<T>& storage,
  ...
  if (currentStorageSize < requiredStorageSize) {
    ...
    try {
      storage = device_vector_type<T>(view_alloc(std::string(label), WithoutInitializing),
                                      newStorageSize);
    } catch (std::exception& e) {
      TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, prefix << "Allocating rank-1 "
                                                                     "View of size "
                                                                  << newStorageSize << " to represent a " << numRows << " x " << numCols << " matrix threw: " << std::endl
                                                                  << e.what());
    }
  }
...
get_contiguous_host_mat_view(host_vector_type<T>& storage,
  ...
  if (currentStorageSize < requiredStorageSize) {
    ...
    try {
      storage = host_vector_type<T>(view_alloc(std::string(label), WithoutInitializing),
                                    newStorageSize);
    } catch (std::exception& e) {
      TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, prefix << "Allocating rank-1 "
                                                                     "host View of size "
                                                                  << newStorageSize << " to store a " << numRows << " x " << numCols << " matrix threw: " << std::endl
                                                                  << e.what());
    }
  }
...
class CuSolverNodeFactorOutput : public NodeFactorOutput<LocalOrdinal, Scalar> {
 public:
...
class SetDiagonalEntriesToOne {
 ...
 public:
  SetDiagonalEntriesToOne(const device_mat_view_type<ScalarType>& A)
    : A_(A) {}
...
void set_diagonal_entries_to_one(const device_mat_view_type<ScalarType>& A) {
  static_assert(!std::is_const<ScalarType>::value,
  ...
  Kokkos::parallel_for("set_diagonal_entries_to_one", range,
...
class CuSolverNodeTsqr : public NodeTsqr<LocalOrdinal, Scalar> {
 ...
 private:
  ...
  get_B_copy(const LocalOrdinal nrows_and_ncols,
    ...
    MatView<LocalOrdinal, Scalar> B_copy_matview(nrows_and_ncols, nrows_and_ncols, B_copy_raw, B_copy_stride);
    MatView<LocalOrdinal, const Scalar> B_matview(nrows_and_ncols, nrows_and_ncols, B, ldb);
  ...
  extract_R(const LocalOrdinal nrows,
    ...
    using a_view_type = decltype(get_device_mat_view<const Scalar>(nrows, ncols, A, lda));
    a_view_type A_view;
    try {
      A_view = get_device_mat_view<const Scalar>(nrows, ncols, A, lda);
    }
 ...
 private:
  ...
  get_my_factor_output(const factor_output_type& factor_output) const {
    ...
    if (output_ptr == nullptr) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(true, std::invalid_argument, prefix << "Input "
                                                                        "factor_output_type object was not created by the same "
                                                                        "type of CuSolverNodeTsqr object as this one.  This "
                                                                        "object has type "
                                                                     << this_name << " and its subclass of "
                                                                                     "factor_output_type has type "
                                                                     << factor_output_type_name << ", but the input factor_output_type object has dynamic "
                                                                                                   "type "
                                                                     << dynamic_type_name << ".");
    }
    return *output_ptr;
 ...
 public:
  ...
  copy_from_host(const MatView<LocalOrdinal, Scalar>& C_dev,
    ...
    auto C_dev_view  = Impl::get_device_mat_view<Scalar>(nrows, ncols, C_dev.data(), C_dev.stride(1));
    auto C_host_view = Impl::get_host_mat_view<const Scalar>(nrows, ncols, C_host.data(), C_host.stride(1));
    ...
    if (C_dev_view.stride(1) == C_dev_view.extent(0) &&
      ...
      try {
        ...
        TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, prefix << "Kokkos::deep_copy(C_dev_view, C_host_view) (both "
                                                                       "contiguous) threw: "
                                                                    << e.what());
      }
    } else {
      // We need to make a contiguous copy of host storage.
      auto C_host_copy = Impl::get_contiguous_host_mat_view(hostMatrixStorage_, nrows, ncols);
      TEUCHOS_ASSERT(C_host_copy.stride(1) ==
                     C_host_copy.extent(0));
      try {
        Kokkos::deep_copy(C_host_copy, C_host_view);
      } catch (std::exception& e) {
        TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, prefix << "Kokkos::deep_copy(C_host_copy, C_host_view) threw: " << e.what());
      }
      ...
      if (C_dev_view.stride(1) == C_dev_view.extent(0)) {
        try {
          ...
          TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, prefix << "Kokkos::deep_copy(C_dev_view, C_host_copy) threw: " << e.what());
        }
      } else {
        auto C_dev_copy = Impl::get_contiguous_device_mat_view(matrixStorage_, nrows, ncols);
        try {
          Kokkos::deep_copy(C_dev_copy, C_host_copy);
        } catch (std::exception& e) {
          TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, prefix << "Kokkos::deep_copy(C_dev_copy, C_host_copy) threw: " << e.what());
        }
        try {
          Kokkos::deep_copy(C_dev_view, C_dev_copy);
        } catch (std::exception& e) {
          TEUCHOS_TEST_FOR_EXCEPTION(true, std::runtime_error, prefix << "Kokkos::deep_copy(C_dev_view, C_dev_copy) threw: " << e.what());
        }
      }
    }
  }
  ...
  copy_to_host(const MatView<LocalOrdinal, Scalar>& C) const override {
    using LO = LocalOrdinal;
  ...
  void
  set_diagonal_entries_to_one(const MatView<LocalOrdinal, Scalar>& C) const override {
    auto C_view =

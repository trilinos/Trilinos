...
void deep_copy(const MatView<LO, SC>& tgt,
...
template <class LO, class SC, class SourceScalar>
void deep_copy(const MatView<LO, SC>& tgt, const SourceScalar& src) {
...
void deep_copy(const MatView<TargetOrdinal, TargetScalar>& tgt,
  ...
  if (tgt_nrows == ptrdiff_t(src.extent(0)) ||
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(true, std::invalid_argument,
                               "TSQR::deep_copy: dimensions "
                               "of tgt (output matrix) and src (input matrix) are not "
                               "compatible.  tgt is "
                                   << tgt.extent(0) << " x " << tgt.extent(1) << ", but src is " << src.extent(0) << " x "
                                   << src.extent(1) << ".");
  }
}
...
void copy_upper_triangle(const MatView<LO, TargetScalar>& R_out,

...
#define TSQR_IMPL_SYSTEMBLAS_IMPL(Scalar)                                                                                                 \
  void SystemBlas<Scalar>::                                                                                                               \
    ...
    const Teuchos::ETransp transa_enum =                                                                                                  \
        (transa == 'C' || transa == 'c') ? Teuchos::CONJ_TRANS : ((transa == 'T' || transa == 't') ? Teuchos::TRANS : Teuchos::NO_TRANS); \
    const Teuchos::ETransp transb_enum =                                                                                                  \
        (transb == 'C' || transb == 'c') ? Teuchos::CONJ_TRANS : ((transb == 'T' || transb == 't') ? Teuchos::TRANS : Teuchos::NO_TRANS); \
  ...
  void SystemBlas<Scalar>::                                                                                                               \
    ...
    const Teuchos::ESide side_enum =                                                                                                      \
        (side == 'L' || side == 'l') ? Teuchos::LEFT_SIDE : Teuchos::RIGHT_SIDE;                                                          \
    const Teuchos::EUplo uplo_enum =                                                                                                      \
        (uplo == 'U' || uplo == 'u') ? Teuchos::UPPER_TRI : ((uplo == 'L' || uplo == 'l') ? Teuchos::LOWER_TRI : Teuchos::UNDEF_TRI);     \
    const Teuchos::ETransp transa_enum =                                                                                                  \
        (transa == 'C' || transa == 'c') ? Teuchos::CONJ_TRANS : ((transa == 'T' || transa == 't') ? Teuchos::TRANS : Teuchos::NO_TRANS); \
    const Teuchos::EDiag diag_enum =                                                                                                      \
        (diag == 'U' || diag == 'u') ? Teuchos::UNIT_DIAG : Teuchos::NON_UNIT_DIAG;                                                       \

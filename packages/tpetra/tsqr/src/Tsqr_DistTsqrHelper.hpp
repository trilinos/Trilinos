...
class DistTsqrHelper : private Impl::CombineUser<LocalOrdinal, Scalar> {
 public:
  ...
  factor_pair(const ordinal_type ncols,
    ...
    const LO ldr    = ncols;
    MatView<LO, scalar_type> R_mine_view(ncols, ncols, R_mine.data(), ldr);
    vector<scalar_type> R_other(nelts);
    MatView<LO, scalar_type> R_other_view(ncols, ncols, R_other.data(), ldr);

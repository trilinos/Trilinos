...
void getPidGidPairs(const Tpetra::Import<LocalOrdinal, GlobalOrdinal, Node>& Importer,
                    Teuchos::Array<std::pair<int, GlobalOrdinal> >& gpids,
                    bool use_minus_one_for_local);

//! Like getPidGidPairs, but just gets the PIDs, ordered by the column Map.
template <typename LocalOrdinal, typename GlobalOrdinal, typename Node>
void getPids(const Tpetra::Import<LocalOrdinal, GlobalOrdinal, Node>& Importer,
...
template <typename LocalOrdinal, typename GlobalOrdinal, typename Node>
void getRemotePIDs(const Tpetra::Import<LocalOrdinal, GlobalOrdinal, Node>& Importer,
...
template <typename LocalOrdinal, typename GlobalOrdinal, typename Node>
bool checkImportValidity(const Tpetra::Import<LocalOrdinal, GlobalOrdinal, Node>& Importer) {
  using Teuchos::RCP;
  ...
  Teuchos::Array<int> remotePIDs;
  getRemotePIDs(Importer, remotePIDs);
  ...
  for (GlobalOrdinal i = minTargetGID; i < maxTargetGID; i++) {
    ...
    if (tlid == LO_INVALID)
      local_proc_mask[MyPID] = 0;
    else
      local_proc_mask[MyPID] = 1;
...
template <typename LocalOrdinal, typename GlobalOrdinal, typename Node>
bool checkAztecOOMLOrdering(const Tpetra::Import<LocalOrdinal, GlobalOrdinal, Node>& Importer) {
...
/* Check to see that the import object's target map respects AztecOO/ML ordering */
template <typename LocalOrdinal, typename GlobalOrdinal, typename Node>
bool checkBlockConsistency(const Tpetra::Map<LocalOrdinal, GlobalOrdinal, Node>& map, size_t block_size) {

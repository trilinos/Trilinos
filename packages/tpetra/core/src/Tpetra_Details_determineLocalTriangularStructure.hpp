...
class DetermineLocalTriangularStructure {
 public:
  ...
  join(result_type& dst,
    ...
    dst.maxNumRowEnt = (src.maxNumRowEnt > dst.maxNumRowEnt) ? src.maxNumRowEnt : dst.maxNumRowEnt;
  ...
  operator()(const typename LocalMapType::local_ordinal_type lclRow,
    ...
    if (numEnt != 0) {
      result.maxNumRowEnt = (numEnt > result.maxNumRowEnt) ? numEnt : result.maxNumRowEnt;

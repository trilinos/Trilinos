...
namespace Tpetra {
    ...
    directory_ (new Directory<LocalOrdinal, GlobalOrdinal, Node> ())
  {
    ...
    if (numLocalElements_ > 0) {
      ...
      // doing so, fill in the LID -> GID table.
      typename decltype (lgMap_)::non_const_type lgMap
        (view_alloc ("lgMap2", WithoutInitializing), numLocalElements_);
  ...
  bool
  Map<LocalOrdinal,GlobalOrdinal,Node>::
  getGlobalElements (const local_ordinal_type localIndices[], size_t numEntries, global_ordinal_type globalIndices[]) const
  {

...
void unpackAndCombine(const RowView& row_ptrs_beg,
  ...
  Kokkos::parallel_reduce(
      ...
      KOKKOS_LAMBDA(const LO i, size_t& running_max_num_ent) {
        const size_t num_packets_this_lid = num_packets_per_lid(i);
        const size_t num_ent              = (unpack_pids) ? num_packets_this_lid / 2 : num_packets_this_lid;
  ...
  if (num_items) {
    ...
    parallel_reduce(
        ...
        KOKKOS_LAMBDA(const LO lid, size_t& update) {
          update += static_cast<size_t>(local_graph.row_map[lid + 1] - local_graph.row_map[lid]);
        },
        kcnt);
    count += kcnt;
  }
...
void setupRowPointersForRemotes(
...
void copyDataFromSameIDs(
  ...
  parallel_for(
      ...
      KOKKOS_LAMBDA(const size_t i) {
        ...
        const size_t nsr = local_graph.row_map(src_lid + 1) - local_graph.row_map(src_lid);
...
void copyDataFromPermuteIDs(
  ...
  parallel_for(
      ...
      KOKKOS_LAMBDA(const size_t i) {
        ...
        size_t nsr = local_graph.row_map(src_lid + 1) - local_graph.row_map(src_lid);
        Kokkos::atomic_fetch_add(&new_start_row(tgt_lid), atomic_incr_type(nsr));
...
void unpackAndCombineIntoCrsArrays2(
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(gbl_err_count != 0,
                             std::invalid_argument, prefix << "Attempting to unpack PIDs, but num_ent is not even; this should never "
...
void unpackAndCombineIntoCrsArrays(
  ...
  parallel_for(
      ...
      KOKKOS_LAMBDA(const size_t i) {
        const LO tgt_lid    = static_cast<LO>(i);
        const LO src_lid    = static_cast<LO>(i);
        tgt_rowptr(tgt_lid) = local_graph.row_map(src_lid + 1) - local_graph.row_map(src_lid);
      });
  ...
  makeCrsRowPtrFromLengths(tgt_rowptr, new_start_row);
  {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(condition, std::invalid_argument,
                               prefix << "CRS_rowptr[last] = " << nth_tgt_rowptr_h << "!= mynnz = " << mynnz << ".");
  }
...
void unpackAndCombineIntoCrsArrays(
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(
      ...
      std::invalid_argument, prefix << "CRS_rowptr.size() = " << CRS_rowptr.size() << "!= TargetNumRows+1 = " << TargetNumRows + 1 << ".");
  ...
  Kokkos::View<const LO*, buffer_device_type> import_lids_d =
      create_mirror_view_from_raw_host_array(bufferOutputDevice, importLIDs.getRawPtr(),
                                             importLIDs.size(), true, "import_lids");

  Kokkos::View<const packet_type*, buffer_device_type> imports_d =
      create_mirror_view_from_raw_host_array(bufferOutputDevice, imports.getRawPtr(),

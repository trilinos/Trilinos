...
template <class LO, class GO, class NT>
bool Directory<LO, GO, NT>::initialized() const {
  return impl_ != NULL;
}

template <class LO, class GO, class NT>
void Directory<LO, GO, NT>::
    initialize(const Map<LO, GO, NT>& map,
               const Tpetra::Details::TieBreak<LO, GO>& tieBreak) {
  if (initialized()) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        impl_ == NULL, std::logic_error,
...
template <class LO, class GO, class NT>
void Directory<LO, GO, NT>::initialize(const Map<LO, GO, NT>& map) {
  if (initialized()) {
    TEUCHOS_TEST_FOR_EXCEPTION(
        impl_ == NULL, std::logic_error,
        "Tpetra::Directory::initialize: "

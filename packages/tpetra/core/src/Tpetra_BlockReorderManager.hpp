...
#ifndef TPETRA_BLOCKREORDERMANAGER_HPP_
#define TPETRA_BLOCKREORDERMANAGER_HPP_

/// \file Tpetra_BlockReorderManager.hpp
/// \brief Declaration of the Tpetra::BlockReorderManager class.
///
/// Direct port of Xpetra::BlockReorderManager.  This class (and its
/// BlockReorderLeaf subclass) is a linear-algebra-independent tree/string
/// utility describing how the blocks of a Tpetra::BlockedCrsMatrix should be
/// reordered/nested.  It carries no Scalar/Ordinal/Node template parameters, so
/// the port is a straight namespace rename (Xpetra -> Tpetra).
...
Teuchos::RCP<Tpetra::BlockReorderManager> blockedReorderFromTokens(const std::vector<std::string>& tokens);
 ...
 */
Teuchos::RCP<const Tpetra::BlockReorderManager> blockedReorderFromString(std::string reorder);
}  // namespace Tpetra

#endif /* TPETRA_BLOCKREORDERMANAGER_HPP_ */

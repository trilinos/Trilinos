...
Teuchos::RCP<Tpetra::BlockReorderManager> blockedReorderFromTokens(const std::vector<std::string>& tokens) {
  // base case
  if (tokens.size() == 1)
    return Teuchos::rcp(new Tpetra::BlockReorderLeaf(Teuchos::StrUtils::atoi(tokens[0])));

  // check first and last character
  TEUCHOS_ASSERT(*(tokens.begin()) == "[")
  TEUCHOS_ASSERT(*(tokens.end() - 1) == "]");

  std::vector<Teuchos::RCP<Tpetra::BlockReorderManager> > vecRMgr;
  ...
  Teuchos::RCP<Tpetra::BlockReorderManager> rMgr = Teuchos::rcp(new Tpetra::BlockReorderManager());
...
Teuchos::RCP<const Tpetra::BlockReorderManager> blockedReorderFromString(std::string reorder) {
  ...
  Teuchos::RCP<Tpetra::BlockReorderManager> mgr = blockedReorderFromTokens(tokens);

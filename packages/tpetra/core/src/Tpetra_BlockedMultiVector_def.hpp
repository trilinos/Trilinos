...
void BlockedMultiVector<Scalar, LocalOrdinal, GlobalOrdinal, Node>::
  ...
  for (size_t r = 0; r < map_->getNumMaps(); ++r) {
    ...
    Teuchos::RCP<const BlockedMap> nestedMap = map_->getBlockedSubMap(r);
    Teuchos::RCP<MultiVector> vv;
    if (nestedMap.is_null())
      vv = Teuchos::rcp(new MultiVector(
          map_->getMap(r, bRhs->getBlockedMap()->getThyraMode()), bRhs->getNumVectors(), true));
    else
      vv = Teuchos::rcp(new BlockedMultiVector(nestedMap, bRhs->getNumVectors(), true));
  ...
  numVectors_ = bRhs->getNumVectors();
}

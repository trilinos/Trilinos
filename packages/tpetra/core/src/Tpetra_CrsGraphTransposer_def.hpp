...
CrsGraphTransposer<LocalOrdinal, GlobalOrdinal, Node>::
  ...
  if (!matchingColMaps) {
    ...
    Tpetra::Details::makeColMap<LocalOrdinal, GlobalOrdinal, Node>(col_map_sym, domain_map, globalColindsSym);
...
CrsGraphTransposer<LocalOrdinal, GlobalOrdinal, Node>::
  ...
  if (exporter.is_null()) {
    ...
    return exportAndFillCompleteCrsGraph<crs_graph_type>(transGraphWithSharedRows, *exporter, Teuchos::null,

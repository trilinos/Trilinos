...
KOKKOS_INLINE_FUNCTION static T1 *lower_bound(T1 *first, T1 *last, const T2 &val,
                                              CompareType compare) {
  T1 *it;
...
KOKKOS_FORCEINLINE_FUNCTION static T1 *lower_bound(T1 *first, T1 *last, const T2 &val) {
  return lower_bound(first, last, val, [](T1 left, T2 right) { return left < right; });
}

template <typename T1, typename T2>
KOKKOS_INLINE_FUNCTION static void heapify(T1 *v, T2 n, T2 i) {
  while (true) {
    T2 largest = i;
    T2 l       = 2 * i + 1;
    T2 r       = 2 * i + 2;
...
template <typename T1, typename T2>
KOKKOS_INLINE_FUNCTION static void heap_sort(T1 *v, T2 n) {
...
// mesh database
template <typename T>
KOKKOS_INLINE_FUNCTION static T
...
struct MeshDatabase {
 ...
 public:
  ...
  MeshDatabase(Teuchos::RCP<const Teuchos::Comm<int> > comm,
    ...
    const local_ordinal_range_type face[6][3] = {{local_ordinal_range_type(remote_range_i.first, _owned._range_i.first), _owned._range_j, _owned._range_k},

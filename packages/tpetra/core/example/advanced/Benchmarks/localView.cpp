...
// stream \c out.
int checkCmdLineOpts(std::ostream& out, const CmdLineOpts& opts) {
  int err = 0;
...
makeEpetraComm(const Teuchos::Comm<int>& comm) {
  ...
  if (mpiComm == NULL) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(serialComm == NULL, std::logic_error,
                               "Input Teuchos::Comm "
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(rawMpiCommWrapped.is_null(), std::logic_error,
                               "The input "
...
getEpetraMatrix(const Teuchos::RCP<const Teuchos::Comm<int> >& comm,
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(gblSuccess != 1, std::logic_error,
                             "While filling Epetra_CrsGraph, "
  ...
  reduceAll<int, int>(*comm, REDUCE_MIN, lclSuccess, outArg(gblSuccess));
  TEUCHOS_TEST_FOR_EXCEPTION(gblSuccess != 1, std::logic_error,
                             "While filling Epetra_CrsMatrix, "
                             "error on one or more MPI processes.");
...
int main(int argc, char* argv[]) {
  ...
  Tpetra::ScopeGuard tpetraScope(&argc, &argv);
  {
    ...
    if (opts.testEpetra) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error, "Epetra not enabled; should never get here!");
#endif  // HAVE_TPETRACORE_EPETRA
    }
    ...
    if (opts.testEpetraLen) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(A_ptr.is_null(), std::logic_error,
                                 "getEpetraMatrix returned null!  "

...
void localApplyBlockNoTrans(Tpetra::BlockCrsMatrix<Scalar, LO, GO, Node>& A,
...
template <class Scalar, class LO, class GO, class Node>
bool compareLocalMatVec(Teuchos::FancyOStream& out,
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(numVecs != static_cast<LO>(X_mv.getNumVectors()),
...
void setCmdLineOpts(CmdLineOpts& opts,
  ...
  clp.setOption("blockSize", &(opts.blockSize),
                "Block size; number of rows "
                "/ columns per block");
  clp.setOption("runTest", "runBenchmark", &(opts.runTest),
                "Instead of "

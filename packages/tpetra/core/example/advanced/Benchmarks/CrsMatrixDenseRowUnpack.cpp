...
getTargetRowMapIndices(const LO lclNumRows,
  ...
  for (LO lid = 0; lid < lclNumRows; ++lid) {
    const LO offset = (lid % LO(2) == 0) ? lclNumRows - LO(1) - lid : lid;
...
getLclRowsToTest(const export_type& exporter) {
  ...
  Kokkos::View<int*, device_type> lclRowsToTest(view_alloc("lclRowsToTest", WithoutInitializing), tgtLclNumRows);
...
void fillSourceMatrixValues(const crs_matrix_type& A) {
...
int testTargetLocalMatrixValues(const crs_matrix_type::local_matrix_device_type& A,
                                const map_type::local_map_type& lclColMap,
...
void printLocalMatrix(std::ostream& out,
...
bool testTargetMatrixValues(const crs_matrix_type& A,
                            const Kokkos::View<const int*, typename crs_matrix_type::device_type>& lclRowsToTest,
                            const Tpetra::CombineMode combineMode) {
  using std::endl;
  const bool verbose = Tpetra::Details::Behavior::verbose();
...
void benchmark(const RCP<const Teuchos::Comm<int>>& comm,
  ...
  const LO numColsToFill = maxNumColsToFill > gblNumCols ? gblNumCols : maxNumColsToFill;
...
getAndValidateCmdLineArgs(CmdLineArgs& args,
  ...
  if (parseResult == CommandLineProcessor::PARSE_HELP_PRINTED) {
    ...
    return valid ? CmdLineArgsValidation::VALID : CmdLineArgsValidation::INVALID;
  }
}

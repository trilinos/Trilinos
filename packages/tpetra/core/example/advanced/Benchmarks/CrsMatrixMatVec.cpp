...
void setCmdLineOpts(CmdLineOpts& opts,
  ...
  clp.setOption("lclNumRows", &(opts.lclNumRows),
                "Number of rows per MPI "
                "process in the sparse graph.");
  clp.setOption("numEntPerRow", &(opts.numEntPerRow),
                "Number of entries "
                "per row in the sparse graph.");
  clp.setOption("warm-up", "no-warm-up", &(opts.warmUp),
                "Perform a first un-timed apply"
...
void printCmdLineOpts(Teuchos::FancyOStream& out,
                      const CmdLineOpts& opts) {
  using std::endl;

...
createRowAndColMap(RCP<const Teuchos::Comm<int> > comm,
  ...
  RCP<const map_type> rowAndColMap(new map_type(gblNumInds, lclNumInds, indexBase, comm));
  return rowAndColMap;
}
...
void populateCrsMatrix(crs_matrix_type& A,
...
void populateCrsGraph(crs_graph_type& G,
                      const LO numToInsert,
                      const LO lclColInds[]) {
...
void doSumIntoLocalValues(const std::string& label,
                          crs_matrix_type& A,
                          const LO numToInsert,
                          const LO lclColInds[],
                          const SC vals[],
  ...
  for (int trial = 0; trial < numTrials; ++trial) {
    for (LO lclRow = 0; lclRow < lclNumRows; ++lclRow) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(numInserted != numToInsert, std::logic_error,
...
void doKokkosSumIntoLocalValues(const std::string& label,
                                crs_matrix_type& A,
                                const LO numToInsert,
                                const LO lclColInds[],
                                const SC vals[],
  ...
  for (int trial = 0; trial < numTrials; ++trial) {
    for (LO lclRow = 0; lclRow < lclNumRows; ++lclRow) {
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(numInserted != numToInsert, std::logic_error,
                                 "doKokkosSumIntoLocalValues failed: numInserted=" << numInserted << " != numToInsert=" << numToInsert << ".");
    }
  }
}
...
void benchmarkCrsMatrixSumIntoLocalValues(const CmdLineArgs args) {
  ...
  // CrsMatrix with nonconst graph
  {
    ...
    doSumIntoLocalValues(
        "(nonconst graph) sumIntoLocalValues: "
  ...
  // CrsMatrix with const graph
  {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(!A.isStaticGraph(), std::logic_error,
                               "A does not have a "
    ...
    try {
      doSumIntoLocalValues(
          "(const graph) sumIntoLocalValues: "
          "after first fillComplete",
          A, LO(args.numToInsert), lclColInds.data(),
          vals.data(), args.numTrials);
    } catch (std::exception& e) {

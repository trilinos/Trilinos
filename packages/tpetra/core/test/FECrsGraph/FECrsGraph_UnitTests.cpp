...
bool compare_final_graph_structure(Teuchos::FancyOStream& out, Tpetra::CrsGraph<LO, GO, Node>& g1, Tpetra::CrsGraph<LO, GO, Node>& g2) {
  ...
  if (!g1.isFillComplete() || !g2.isFillComplete()) {
    out << "Compare: FillComplete failed" << endl;
    return false;
  }
  if (!g1.getRangeMap()->isSameAs(*g2.getRangeMap())) {
    out << "Compare: RangeMap failed" << endl;
    return false;
  }
  if (!g1.getRowMap()->isSameAs(*g2.getRowMap())) {
    out << "Compare: RowMap failed" << endl;
    return false;
  }
  if (!g1.getDomainMap()->isSameAs(*g2.getDomainMap())) {
    out << "Compare: DomainMap failed" << endl;
    return false;
  }
  if (!g1.getColMap()->isSameAs(*g2.getColMap())) {
    out << "Compare: ColMap failed" << endl;
    g1.describe(out, Teuchos::VERB_EXTREME);
    g2.describe(out, Teuchos::VERB_EXTREME);
    return false;
  }
  ...
  if (rowptr1.extent(0) != rowptr2.extent(0)) {
    out << "Compare: rowptr extent failed" << endl;
    return false;
  }
  if (colind1.extent(0) != colind2.extent(0)) {
    out << "Compare: colind extent failed: " << colind1.extent(0) << " vs " << colind2.extent(0) << endl;
  ...
  if (!success) {
    out << "Compare: rowptr match failed" << endl;
    return false;
  }

  TEST_COMPARE_ARRAYS(colind1, colind2);
  if (!success) {
    out << "Compare: colind match failed" << endl;
    return false;
  }
...
void generate_fem1d_graph(size_t numLocalNodes, RCP<const Comm<int> > comm, GraphPack<2, LO, GO, Node>& pack) {
  ...
  size_t numOverlapNodes = numLocalNodes;
  if (rank != numProc - 1) numOverlapNodes++;
  if (rank != 0) numOverlapNodes++;
  ...
  if (rank != 0) {
    overlapIndices[last] = overlapIndices[0] - 1;
    last++;
  }
  if (rank != numProc - 1) {
    overlapIndices[last] = overlapIndices[numLocalNodes - 1] + 1;
    last++;
  }
  ...
  Kokkos::parallel_for(
      Kokkos::RangePolicy<typename Node::execution_space>(0, numLocalElements), KOKKOS_LAMBDA(const size_t i) {
...
void generate_fem2d_q1_graph(size_t numCells1D, RCP<const Comm<int> > comm, GraphPack<4, LO, GO, Node>& pack) {
  ...
  auto k_e2n = pack.k_element2node;
  Kokkos::parallel_for(
      Kokkos::RangePolicy<typename Node::execution_space>(0, numMyCells),

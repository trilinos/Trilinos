...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(CrsGraph, DottedDiag, LO, GO, Node) {
  ...
  for (int T = 0; T < 4; ++T) {
    ...
    ddgraph.insertGlobalIndices(mymiddle, tuple<GO>(mymiddle));
    {
      ...
      ddgraph.getGlobalRowView(mymiddle - 1, myrow_gbl);
      TEST_EQUALITY(myrow_gbl.size(), 0);
      ddgraph.getGlobalRowView(mymiddle, myrow_gbl);
      TEST_COMPARE_ARRAYS(myrow_gbl, tuple<GO>(mymiddle));
      ddgraph.getGlobalRowView(mymiddle + 1, myrow_gbl);
      TEST_EQUALITY(myrow_gbl.size(), 0);
    }

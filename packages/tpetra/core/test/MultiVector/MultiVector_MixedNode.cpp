...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(MultiVector, mixed_node_basic, Scalar, LO, GO) {
  ...
  if (myRank == 0)
    map0 = rcp(new CudaMap(INVALID, numLocal, indexBase, comm));
  else
    map1 = rcp(new SerialMap(INVALID, numLocal, indexBase, comm));

  myOut << "Test MultiVector's usual constructor" << endl;
  if (myRank == 0)
    success = mv_test_basic<Scalar, CudaMap>(map0, numVecs, myOut);
  else
    success = mv_test_basic<Scalar, SerialMap>(map1, numVecs, myOut);

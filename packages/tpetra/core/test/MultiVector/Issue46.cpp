...
auto getHostViewOfVector(const VectorType& X) -> decltype(Kokkos::subview(X.getLocalViewHost(Tpetra::Access::ReadOnly), Kokkos::ALL(), 0)) {
...
auto getHostViewOfMultiVector(const MultiVectorType& X) -> decltype(X.getLocalViewHost(Tpetra::Access::ReadOnly)) {
...
template <class MultiVectorType>
bool multiVectorsEqual(const MultiVectorType& X, const MultiVectorType& Y) {
...
template <class MultiVectorType>
void printMultiVector(std::ostream& out, const MultiVectorType& X) {
  typedef typename MultiVectorType::local_ordinal_type LO;
...
void issue46Test(bool& success, Teuchos::FancyOStream& out) {
  ...
  out << "X2, which is a view of X1 with offset=" << offset2 << ", and thus "
                                                                "should equal X3, a view of X0 with offset="
      << (offset1 + offset2) << endl;

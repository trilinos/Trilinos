...
template <class GO>
void getFirstGID(GO& first_gid) {
  first_gid = 0;
}

template <>
void getFirstGID<long long>(long long& first_gid) {
...
TEUCHOS_UNIT_TEST_TEMPLATE_2_DECL(CrsGraphImportExport, doImport, LO, GO) {
  ...
  if (numProcs < 2) {
    out << "This test is only nontrivial if run with multiple MPI "
           "processes, but you have run it with only 1 process."
        << endl;
    return;
  }
  ...
  out << "Create a Map that is evenly distributed, "
         "and another that has all elements on Proc 0"
      << endl;
  try {
    OSTab tab2(out);
...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(CrsMatrixImportExport, doImport, LO, GO, Scalar) {
  ...
  if (numImages < 2) {
    out << "This test is only meaningful if running with multiple MPI "
           "processes, but you ran it with only 1 process."
        << endl;
    return;
  }
  ...
  try {
    ...
    const LO src_num_local_elements = (myImageID == 0) ? static_cast<LO>(numImages * tgt_num_local_elements) : static_cast<LO>(0);
...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(MultiVectorImport, doImport, LO, GO, Scalar) {
  ...
  for (int collectRank = 0; collectRank < 2; collectRank++) {
    ...
    const LO tgt_num_local_elements = (myImageID == collectRank) ? static_cast<LO>(numImages * src_num_local_elements) : static_cast<LO>(0);
...
test_with_matvec_reduced_maps(const CrsMatrixType& A,
  ...
  RCP<const MapType> Bmap = Bfullmap.getLocalNumElements() > 0 ? B.getDomainMap() : Teuchos::null;
...
template <class CrsMatrixType>
void build_test_matrix(const RCP<const Teuchos::Comm<int>>& Comm,
                       RCP<CrsMatrixType>& A) {
  typedef typename CrsMatrixType::local_ordinal_type LO;
  typedef typename CrsMatrixType::global_ordinal_type GO;
  typedef typename CrsMatrixType::scalar_type Scalar;
...
// ===============================================================================
template <class CrsMatrixType>
void build_test_matrix_wideband(const RCP<const Teuchos::Comm<int>>& Comm,
                                RCP<CrsMatrixType>& A) {
  typedef typename CrsMatrixType::local_ordinal_type LO;
  typedef typename CrsMatrixType::global_ordinal_type GO;
  typedef typename CrsMatrixType::scalar_type Scalar;
...
// ===============================================================================
template <class CrsMatrixType>
void build_test_matrix_with_row_overlap(const Teuchos::RCP<const Teuchos::Comm<int>>& comm,
  ...
  const LO NumLocalEquations   = (NumProc > 1) ? (2 * NumPrimaryEquations) : NumPrimaryEquations;
...
// ===============================================================================
template <class CrsMatrixType>
void build_test_prolongator(const Teuchos::RCP<const CrsMatrixType>& A,
...
void build_test_map(const Teuchos::RCP<const MapType>& oldMap, Teuchos::RCP<MapType>& newMap) {
  using Teuchos::rcp;
  typedef Tpetra::global_size_t GST;
...
void build_remote_only_map(const Teuchos::RCP<const ImportType>& Import,
...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(FusedImportExport, doImport, LO, GO, Scalar) {
  ...
  try {
    ...
    if (MyPID == 0)
      Map1 = rcp(new map_type(num_global, (size_t)num_global, 0, Comm));
    else
      Map1 = rcp(new map_type(num_global, (size_t)0, 0, Comm));
...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(Import_Util, UnpackAndCombineWithOwningPIDs, LO, GO, Scalar) {
  ...
  if (Importer != Teuchos::null) {
    ...
    if (A->getGraph()->getImporter() == Teuchos::null)
      SourcePids.assign(SourcePids.size(), MyPID);
    else
      Tpetra::Import_Util::getPids<LO, GO, Node>(*A->getGraph()->getImporter(), SourcePids, false);
    ...
    if (Bcolind.size() != as<size_t>(colind.size())) {
      test_err++;
      std::cout << "--colind mismatch" << std::endl;
    }
    if (Bvals.size() != as<size_t>(vals.size())) {
      test_err++;
...
TEUCHOS_UNIT_TEST_TEMPLATE_2_DECL(Import_Util, LowCommunicationMakeColMapAndReindex, LO, GO) {
...

  {
    ...
    if (Acolmap->getLocalNumElements() != Bcolmap->getLocalNumElements())
      test_err++;
    else {

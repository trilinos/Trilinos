...
void testComputeOffsetsTmpl(bool& success,
  ...
  out << "Test the case where counts and offsets do not alias one another, "
         "but live in the same memory space"
      << endl;
  {
    Teuchos::OSTab tab2(out);
    OffsetType computedTotal = 0;
...
void testComputeOffsets(bool& success,
...

  {
    ...
    if (!success) {
      out << "Test with "
          << "unsigned long long, " << countTypeName << ", "
          << deviceName<DeviceType>() << " failed; returning early" << endl;
      return;
    }
  }

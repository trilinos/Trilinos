...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(Bug8794, InsertDenseRows,
  ...
  // Initialize matrix and expected value of SpMV product
  {
    ...
    for (size_t i = 0; i < map->getLocalNumElements(); i++) {
      ...
      if (!denseRow) {  // sparse row
        ...
        if (gid + 1 < nrows) {
          cols[nz++] = gid + 1;
          expectedData[i] += Scalar(gid + 1);
        }
        if (gid + 2 < nrows) {
          cols[nz++] = gid + 2;
          expectedData[i] += Scalar(gid + 2);
        }
        if (gid - 1 >= 0) {
          cols[nz++] = gid - 1;
          expectedData[i] += Scalar(gid - 1);
  ...
  int ierr = 0;
  {
    ...
    for (size_t i = 0; i < map->getLocalNumElements(); i++) {
      if (Teuchos::ScalarTraits<Scalar>::magnitude(yData(i, 0) - expectedData(i, 0)) > 200 * Teuchos::ScalarTraits<Scalar>::magnitude(expectedData(i, 0)) * Teuchos::ScalarTraits<magnitude_t>::eps()) {
  ...
  if (gerr)
    out << "TEST FAILED with " << gerr << " errors" << std::endl;
  else

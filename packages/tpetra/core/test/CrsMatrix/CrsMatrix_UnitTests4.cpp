...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(CrsMatrix, ExtractBlockDiagonal, LO, GO, Scalar, Node) {
  ...
  RCP<MV> diag1 = rcp(new MV(map, 1));
  diag1->putScalar(SC_one);
  RCP<MV> diag2 = rcp(new MV(map, 2));
  diag2->putScalar(SC_one);
  RCP<MV> diag5 = rcp(new MV(map, 5));
...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(CrsMatrix, ScaleBlockDiagonal, LO, GO, Scalar, Node) {
  ...
  RCP<MV> toScale1 = rcp(new MV(map, 2));
  toScale1->putScalar(SC_one);
  RCP<MV> toScale2 = rcp(new MV(map, 2));
  toScale2->putScalar(SC_one);
  RCP<MV> toScale5 = rcp(new MV(map, 2));
  toScale5->putScalar(SC_one);
...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(CrsMatrix, ScaleBlockDiagonal_Forward, LO, GO, Scalar, Node) {
  ...
  RCP<MV> diag2 = rcp(new MV(map, 2));
  diag2->putScalar(SC_one);
  RCP<MV> diag4 = rcp(new MV(map, 4));
  diag4->putScalar(SC_one);
...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(CrsMatrix, ApplyHelpers, LO, GO, Scalar, Node) {
  ...
  Kokkos::parallel_for(
      range_policy(0, A1_values.extent(0)), KOKKOS_LAMBDA(const int i) {
  ...
  // Do a std::vector version
  {
    ...
    if (ST::isOrdinal) {
      TEST_COMPARE_ARRAYS(exact, norm);
    } else {
      TEST_ABSOLUTE_COMPARE_FLOATING_ARRAYS(exact, norm, absNorm * testingTol<Mag>());
    }
  ...
  // Do a std::vector version
  {
    ...
    Teuchos::Array<MAT *> matrices(2);
    matrices[0] = A1.get();
    matrices[1] = A2.get();
    Teuchos::Array<MV *> outvec(2);
    outvec[0] = &Y1b;
    outvec[1] = &Y2b;

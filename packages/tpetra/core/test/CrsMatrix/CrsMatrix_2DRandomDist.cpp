 ...
 public:
  ...
  void distributeAndApply(
    ...
    std::cout << comm->getRank()
              ...
              << "; import "
              << (Amat->getGraph()->getImporter() == Teuchos::null ? 0 : Amat->getGraph()->getImporter()->getNumExportIDs())
              << "; export "
              << (Amat->getGraph()->getExporter() == Teuchos::null ? 0 : Amat->getGraph()->getExporter()->getNumExportIDs())
              << std::endl;

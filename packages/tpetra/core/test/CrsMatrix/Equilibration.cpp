...
template <class SC, class LO, class GO, class NT>
void testCrsMatrixEquality(bool& success,
...
template <class SC, class LO, class GO, class NT>
void testEquilibration(Teuchos::FancyOStream& out,
                       bool& success,
  ...
  // Test computeLocalRowAndColumnOneNorms (CrsMatrix)
  {
...

    {
      ...
      os << "]" << endl
         << "Actual local rowDiagonalEntries: [";
      for (LO k = 0; k < lclNumRows; ++k) {
    ...
    if (!assumeSymmetric) {
      ...
      os << "]" << endl
         << "Actual local colDiagonalEntries: [";
      for (LO k = 0; k < lclNumCols; ++k) {
    ...
    if (!assumeSymmetric) {
      ...
      os << "]" << endl
         << "Actual local rowScaledColNorms: [";
      for (LO k = 0; k < lclNumCols; ++k) {
    ...
    if (!assumeSymmetric) {
      ...
      os << "]" << endl
         << "Actual global rowScaledColNorms: [";
      for (LO k = 0; k < lclNumCols; ++k) {
        os << rowScaledColNorms_h[k];
        if (k + LO(1) < lclNumCols) {
          os << ", ";
  ...
  // Test computeRowAndColumnOneNorms
  {
    ...
    Tpetra::Vector<mag_type, LO, GO, NT> colNormsVec =
        createVectorFromCopyOf1DView(test.A->getColMap(),
                                     result2.assumeSymmetric ? result2.colNorms : result2.rowScaledColNorms);
    {
      ...
      Tpetra::leftAndOrRightScaleCrsMatrix(*A_copy,
                                           result2.rowNorms,
                                           result2.assumeSymmetric ? result2.colNorms : result2.rowScaledColNorms,  // ignored
                                           true, false,
...
makeSymmetricPositiveDefiniteTridiagonalMatrixTest(Teuchos::FancyOStream& out,
  ...
  for (LO lclRow = 0; lclRow < lclNumRows; ++lclRow) {
    ...
    const mag_type expectedRowNorm =
        (gblRow == 0 || gblRow == gblNumRows - GO(1)) ? offDiagAbsVal + diagAbsVal : offDiagAbsVal + diagAbsVal + offDiagAbsVal;
  ...
  Teuchos::RCP<crs_matrix_type> A_leftScaled = [&]() {
    ...
    for (LO lclRow = 0; lclRow < lclNumRows; ++lclRow) {
      ...
      for (size_type k = ptr_h[lclRow]; k < ptr_h[lclRow + 1]; ++k) {
        ...
        const mag_type scalingFactor = assumeSymmetric ? KAM::sqrt(rowNorm) : rowNorm;
  ...
  if (assumeSymmetric) {
    out << "assumeSymmetric=true: Skip local (column norms, "
           "diagonal entries, and row-scaled column norms)"
        << endl;
  } else {
  ...
  std::vector<mag_type> lclColNorms(assumeSymmetric ? LO(0) : lclNumCols);
  std::vector<val_type> lclColDiagonalEntries(assumeSymmetric ? LO(0) : lclNumCols);
  std::vector<mag_type> lclRowScaledColNorms(assumeSymmetric ? LO(0) : lclNumCols);
  ...
  std::vector<mag_type> gblRowScaledColNorms(assumeSymmetric ? LO(0) : lclNumCols);
  ...
  Teuchos::RCP<crs_matrix_type> A_rightScaled = [&]() {
    ...
    for (LO lclRow = 0; lclRow < lclNumRows; ++lclRow) {
      ...
      for (size_type k = ptr_h[lclRow]; k < ptr_h[lclRow + 1]; ++k) {
        const LO lclCol        = ind_h[k];
        const mag_type colNorm = assumeSymmetric ? gblColNorms[lclCol] : gblRowScaledColNorms[lclCol];
        // const mag_type rowScalingFactor = assumeSymmetric ?
        //   KAM::sqrt (gblRowNorms[lclRow]) : gblRowNorms[lclRow];
        const mag_type colScalingFactor = assumeSymmetric ? KAM::sqrt(colNorm) : colNorm;
  ...
  Teuchos::RCP<crs_matrix_type> A_symScaled = [&]() {
    ...
    for (LO lclRow = 0; lclRow < lclNumRows; ++lclRow) {
      ...
      for (size_type k = ptr_h[lclRow]; k < ptr_h[lclRow + 1]; ++k) {
        const LO lclCol                 = ind_h[k];
        const mag_type rowNorm          = gblRowNorms[lclRow];
        const mag_type rowScalingFactor = assumeSymmetric ? KAM::sqrt(rowNorm) : rowNorm;
        const mag_type colNorm          = assumeSymmetric ? gblColNorms[lclCol] : gblRowScaledColNorms[lclCol];
        const mag_type colScalingFactor = assumeSymmetric ? KAM::sqrt(colNorm) : colNorm;
        const mag_type scalingFactor    = rowScalingFactor * colScalingFactor;
        val_h[k]                        = val_h[k] / scalingFactor;
...
makeMatrixTestWithExplicitZeroDiag(Teuchos::FancyOStream& out,
  ...
  out << "Create diagonal matrix problem with explicit zero "
         "diagonal entry on one process, not Process 0"
      << endl;
  Teuchos::OSTab tab0(out);
  ...
  if (!assumeSymmetric) {
    ...
    for (LO lclRow = 0; lclRow < lclNumRows; ++lclRow) {
      ...
      lclRowScaledColNorms[lclCol] = myRank == 1 ? NaughtyValues<mag_type>::quiet_NaN() : 1.0;
    }
  }  // ! assumeSymmetric
  ...
  if (!assumeSymmetric && colMap->getGlobalNumElements() != 0) {
    for (LO lclCol = 0; lclCol < lclNumCols; ++lclCol) {
      gblRowScaledColNorms[lclCol] = myRank == 1 ? NaughtyValues<mag_type>::quiet_NaN() : 1.0;
    }
  }
...
makeMatrixTestWithExplicitInfAndNan(Teuchos::FancyOStream& out,
  ...
  out << "Create diagonal matrix problem with an explicit Inf entry on "
         "one process, and an explicit NaN entry on another process"
      << endl;
  Teuchos::OSTab tab0(out);
...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(Equilibration, Test0, SC, LO, GO, NT) {
  ...
  using test_type =
      std::function<test_return_type(FancyOStream&,
                                     ...
                                     const bool)>;
  std::array<test_type, 4> tests{{makeSymmetricPositiveDefiniteTridiagonalMatrixTest<SC, LO, GO, NT>,

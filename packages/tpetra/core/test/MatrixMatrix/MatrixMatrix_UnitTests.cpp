...
add_test_results<typename Matrix_t::scalar_type> regular_add_test(
  ...
  toReturn.epsilon      = ST::magnitude(toReturn.correctNorm - toReturn.computedNorm);
...
template <class Matrix_t>
add_test_results<typename Matrix_t::scalar_type>
...
template <class Matrix_t>
add_test_results<typename Matrix_t::scalar_type> add_into_test(
    RCP<Matrix_t> A,
    RCP<Matrix_t> B,
    bool AT,
    RCP<Matrix_t> C,
...
mult_test_results<typename Matrix_t::scalar_type> multiply_test_manualfc(
  ...
  if (!myImport.is_null())
    results.isImportValid = Tpetra::Import_Util::checkImportValidity(*myImport);
  else
    results.isImportValid = true;

  return results;
}

template <class Matrix_t>
mult_test_results<typename Matrix_t::scalar_type> multiply_test_autofc(
    const std::string& name,
    RCP<Matrix_t> A,
    RCP<Matrix_t> B,
    bool AT,
...
template <class Matrix_t>
mult_test_results<typename Matrix_t::scalar_type> multiply_RAP_test_autofc(
    const std::string& name,
    RCP<Matrix_t> R,
    RCP<Matrix_t> A,
    RCP<Matrix_t> P,
...
template <class Matrix_t>
mult_test_results<typename Matrix_t::scalar_type> multiply_RAP_reuse_test(
    const std::string& name,
    RCP<Matrix_t> R,
    RCP<Matrix_t> A,
    RCP<Matrix_t> P,
...
template <class Matrix_t>
mult_test_results<typename Matrix_t::scalar_type> multiply_reuse_test(
    const std::string& name,
    RCP<Matrix_t> A,
    RCP<Matrix_t> B,
    bool AT,
  ...
  if (AT == false)
    As->leftScale(*leftScaling);
  else
    As->rightScale(*leftScaling);
...
template <class Matrix_t>
mult_test_results<typename Matrix_t::scalar_type> jacobi_test(
    const std::string& name,
    RCP<Matrix_t> A,
    RCP<Matrix_t> B,
    RCP<const Comm<int>> comm,
  ...
  // Error Check
  SC compNorm = C_check->getFrobeniusNorm();
  SC cNorm    = C->getFrobeniusNorm();
  mult_test_results<SC> results;
...
template <class Matrix_t>
mult_test_results<typename Matrix_t::scalar_type> jacobi_reuse_test(
    const std::string& name,
    RCP<Matrix_t> A,
    RCP<Matrix_t> B,
    RCP<const Comm<int>> comm,
...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(Tpetra_MatMat, operations_test, SC, LO, GO, NT) {
  ...
  RCP<FancyOStream> newOutP = (myRank == 0) ? Teuchos::getFancyOStream(Teuchos::rcpFromRef(std::cerr)) : Teuchos::getFancyOStream(rcp(new Teuchos::oblackholestream()));
  ...
  for (Teuchos::ParameterList::ConstIterator it = matrixSystems->begin();
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(!it->second.isList(), std::runtime_error,
                               ...
                               "ParameterList instances.  "
                                   << endl
                                   << "Bad tag's name: " << it->first << "Type name: " << it->second.getAny().typeName() << endl
                                   << endl);
  ...
  }
  {
    ...
    for (LO localrow = row_map_A->getMinLocalIndex();
      ...
      if (globalrow == 0) {
        cols_C.resize(2);
        cols_C[0] = globalrow;
        cols_C[1] = globalrow + 1;
      } else if (globalrow == 3) {
        cols_C.resize(2);
        cols_C[0] = globalrow - 1;
        cols_C[1] = globalrow;
      } else {
        cols_C.resize(3);
        cols_C[0] = globalrow - 1;
        cols_C[1] = globalrow;
        cols_C[2] = globalrow + 1;
      }
...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(Tpetra_MatMat, ATI_range_row_test, SC, LO, GO, NT) {
  ...
  newOut << "Tpetra sparse matrix-matrix multiply: Test A^T * I, "
            "where A's row Map and range Map differ"
         << endl;
  Teuchos::OSTab tab1(newOut);
    ...
    for (LO m = 0; m < 2; m++) {
      for (size_t row = 0; row < nrows; row++) {
        ...
        for (size_t entry = 0; entry < nnzPerRow; entry++) {
          rowVals[entry] = ((SC)rand()) / ((SC)RAND_MAX);
...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(Tpetra_MatMat, add_zero_rows, SC, LO, GO, NT) {
  ...
  RCP<crs_matrix_type> C2 = Tpetra::MatrixMatrix::add(one, false, *A, one, false, *B);

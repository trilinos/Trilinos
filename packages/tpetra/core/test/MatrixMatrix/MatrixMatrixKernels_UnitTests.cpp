...
mult_test_results multiply_test_kernel(
  ...
  for (int alg = 0; alg < (int)ALGORITHMS.size(); alg++) {
    ...
    for (size_t i = 0; i < (size_t)Real_rowptr.size() - 1; i++) {
      ...
      for (size_t j = 0; j < nnz; j++) {
        if (R_sorted[j].first != C_sorted[j].first) {
          has_mismatch = true;
          break;
        }
...
mult_test_results jacobi_test(
  ...
  typename Teuchos::ScalarTraits<SC>::magnitudeType omega = Teuchos::ScalarTraits<SC>::magnitude(Teuchos::ScalarTraits<SC>::one());
  ...
  Dinv.putScalar(omega * Teuchos::ScalarTraits<SC>::one());

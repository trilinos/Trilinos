...
TEUCHOS_UNIT_TEST_TEMPLATE_4_DECL(BlockMultiVector, BlockWiseMultiply, Scalar, LO, GO, Node) {
  ...
  Teuchos::RCP<Teuchos::FancyOStream> outPtr =
      debug ? Teuchos::getFancyOStream(Teuchos::rcpFromRef(std::cerr)) : Teuchos::rcpFromRef(out);
  ...
  Kokkos::View<IST**, host_layout_type, host_device_type> prototypeBlock("prototypeBlock", blockSize, blockSize);
  ...
  myOut << "Call Y.blockWiseMultiply(alpha, D, X) again, where Y has nonzero "
           "initial contents (the method should ignore those contents)"
        << endl;
  Y.putScalar(-five);
  Y.blockWiseMultiply(one, D, X);
  Kokkos::fence();

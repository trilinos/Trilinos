...
class TestReader {
 public:
  ...
  TestReader(
    ...
    nRow = A_baseline->getRowMap()->getMaxAllGlobalIndex() + 1;  // Since Trilinos' reader converts one-based to zero-based
    nCol = A_baseline->getColMap()->getMaxAllGlobalIndex() + 1;  // Since Trilinos' reader converts one-based to zero-based

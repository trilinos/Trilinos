...
TEUCHOS_STATIC_SETUP() {
  ...
  clp.setOption("debug", "release", &debug,
                "If true, print copious debugging "
                "output to stderr on all processes.");
}

// Input matrices must be fill complete.
template <class CrsMatrixType>
bool compareCrsMatrixMaps(const CrsMatrixType& A_orig, const CrsMatrixType& A, Teuchos::FancyOStream& out) {
...
template <class CrsMatrixType>
bool compareCrsMatrix(const CrsMatrixType& A_orig, const CrsMatrixType& A, Teuchos::FancyOStream& out) {
...
// (row, column, domain, and range) must be the same.
template <class CrsMatrixType>
bool compareCrsMatrixValues(const CrsMatrixType& A_orig,
...
bool testReadAndWriteFile(Teuchos::FancyOStream& out,
  ...
  if (matrixOutFile == "") {
    if (myRank == 0) {
      out << "Process 0: No CrsMatrix output file provided; storing output in "
             "memory."
          << endl
          << "WARNING: This may run out of memory if the input "

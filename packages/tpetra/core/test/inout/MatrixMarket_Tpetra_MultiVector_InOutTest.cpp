...
template <class MV>
void assertMultiVectorsEqual(const Teuchos::RCP<const MV>& X,
  ...
  if (badColumns.size() > 0) {
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(true, std::logic_error, "Column" << (numBad != 1 ? "s" : "") << " [" << os.str() << "] of X and Y have "
                                                                                                                   "norms that differ relatively by more than "
                                                                << tol << ".");
  }
}
...
// \param debug [in] Whether to print debugging output.
template <class MV>
void testWriteDenseFile(const std::string& outputFilename,
...
int main(int argc, char* argv[]) {
  ...
  cmdp.setOption("testToRun", &testToRun,
                 "Number of a specific test to run.  "
  ...
  cmdp.setOption("testDifferentIndexBase", "dontTestDifferentIndexBase",
                 &testDifferentIndexBase,
                 "Whether to test input and output "
  ...
  if (inputFilename != "") {
    ...
    if ((testToRun == 0 && testContiguousInputMap) ||
      ...
      if (verbose && myRank == 0) {
        cout << "Test " << testNum << ": Nonnull contiguous Map (same index "
                                      "base) on input to readDenseFile()"
             << endl;
      }
    ...
    if ((testToRun == 0 && testContiguousInputMap && testDifferentIndexBase) ||
      ...
      if (verbose && myRank == 0) {
        cout << "Test " << testNum << ": Nonnull contiguous Map (different "
                                      "index base) on input to readDenseFile()"
             << endl;
      }
      const Tpetra::global_size_t globalNumRows = X->getMap()->getGlobalNumElements();
      ...
      TEUCHOS_TEST_FOR_EXCEPTION(minIndexBase != maxIndexBase || minIndexBase != indexBase,
                                 std::logic_error,
                                 "Index base values do not match on all processes.  "
                                 "Min value is "
                                     << minIndexBase << " and max value is " << maxIndexBase
                                     << ".");
    ...
    if ((testToRun == 0 && testNoncontiguousInputMap) ||
      ...
      const size_t localNumRows = (as<size_t>(myRank) < remainder) ? (quotient + 1) : quotient;
    ...
    if ((testToRun == 0 && testNoncontiguousInputMap) ||
      ...
      if (verbose && myRank == 0) {
        cout << "Test " << testNum << ": Nonnull noncontiguous Map (same index "
                                      "base, GIDs not in 0 .. N-1) on input to readDenseFile()"
             << endl;
      }
      const Tpetra::global_size_t globalNumRows = X->getMap()->getGlobalNumElements();

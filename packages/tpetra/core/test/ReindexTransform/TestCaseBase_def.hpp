...
#ifndef TPETRA_CORE_TEST_REINDEX_TRANSFORM_TEST_CASE_BASE_DEF_HPP
#define TPETRA_CORE_TEST_REINDEX_TRANSFORM_TEST_CASE_BASE_DEF_HPP
...
template <class Scalar_t, class LocalId_t, class GlobalId_t, class Node_t>
bool TestCaseBase<Scalar_t, LocalId_t, GlobalId_t, Node_t>::baseCheckBeforeOrAfterTransform(Problem_t const *problem, std::string const &caseString) const {
  bool result(true);
  ...
  // Check Frobenius norm
  {
    ...
    Scalar_t relDiff(Teuchos::ScalarTraits<Scalar_t>::magnitude(1. - frobNorm / m_frobNorm));
...

  {
    ...
    for (size_t v(0); v < numVectors; ++v) {
      if (m_lhsNorms2[v] == 0.) {
        ...
        Scalar_t relDiff(Teuchos::ScalarTraits<Scalar_t>::magnitude(1. - allNorms2[v] / m_lhsNorms2[v]));
        if (relDiff > relDiffThershold) {
          std::stringstream msg;
          msg << "In TestCaseBase::baseCheckBeforeOrAfterTransform()"
              << "lhs, v = " << v
...

  {
    ...
    for (size_t v(0); v < numVectors; ++v) {
      if (m_rhsNorms2[v] == 0.) {
        ...
        Scalar_t relDiff(Teuchos::ScalarTraits<Scalar_t>::magnitude(1. - allNorms2[v] / m_rhsNorms2[v]));
        if (relDiff > relDiffThershold) {
          std::stringstream msg;
          msg << "In TestCaseBase::baseCheckBeforeOrAfterTransform()"
              << "rhs, v = " << v
...
template <class Scalar_t, class LocalId_t, class GlobalId_t, class Node_t>
bool TestCaseBase<Scalar_t, LocalId_t, GlobalId_t, Node_t>::baseCheckMapsAreDifferent(Matrix_t const &mat1, Matrix_t const &mat2) const {
  bool result(true);
...
#endif  // TPETRA_CORE_TEST_REINDEX_TRANSFORM_TEST_CASE_BASE_DEF_HPP

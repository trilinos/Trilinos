...
bool tpetraCommIsLocallyLegit(const Teuchos::Comm<int>* wrappedTpetraComm,
...
void testMain(bool& success, int argc, char* argv[]) {
  ...
  if (isMpiInitialized()) {
    ...
    cout << "TEST FAILED: MPI_Initialized claims MPI is "
            "initialized, before MPI_Init was called."
         << endl;
    return;
  }
  ...
  if (!isMpiInitialized()) {
    ...
    cout << "TEST FAILED: MPI_Initialized claims MPI is not "
            "initialized, even after MPI_Init was called."
         << endl;
    return;
  }
    ...
    if (!tpetraCommGloballyValid) {
      ...
      if (mySplitRank == 0) {
        cout << "TEST FAILED: Tpetra::getDefaultComm() returns "
                "an invalid comm on at least one process."
             << endl;
      }
    }
  ...
  if (!isMpiInitialized()) {
    ...
    cout << "TEST FAILED: Tpetra::ScopeGuard::~ScopeGuard seems to have "
            ...
            "responsible for initializing and finalizing MPI."
         << endl;
    return;
  }

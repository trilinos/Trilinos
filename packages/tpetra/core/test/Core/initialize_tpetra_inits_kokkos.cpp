...
void testMain(bool& success, int argc, char* argv[]) {
  ...
  if (Kokkos::is_initialized()) {
    ...
    cout << "Kokkos::is_initialized() is true, "
            "before Tpetra::initialize was called."
         << endl;
    return;
  }
  ...
  if (!Kokkos::is_initialized()) {
    success = false;
    cout << "Tpetra::finalize was not supposed to call Kokkos::finalize, "
         << "but apparently did. See issue #15294." << endl;
    return;
  }
  ...
  if (Tpetra::isInitialized()) {
    ...
    cout << "Tpetra::isInitialized() is true, "
            "even after Tpetra::finalize has been called"
         << endl;
  }
}

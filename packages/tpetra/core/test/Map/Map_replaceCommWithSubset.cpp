...
void testSubsetMapOverSubsetComm(int& gblSuccess,  // output argument; 0 means false
                                 std::ostream& out,
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(subsetComm.is_null(), std::logic_error,
                             "Input subsetComm is null.  "
  ...
  try {
    ...
    if (foundBadInd) {
      ...
      out << "The following local indices have global indices in the two Maps "
             "that do not match: "
          << Teuchos::toString(badLclInds)
          << endl
          << "Here are their corresponding global indices in "
             "(origMap, subsetMap): ";
...
template <class LO, class GO, class NT>
void testSubsetMapOverOrigComm(int& gblSuccess,  // output argument; 0 means false
                               std::ostream& out,
                               const Teuchos::RCP<const Tpetra::Map<LO, GO, NT> >& origMap,
                               const Teuchos::RCP<const Tpetra::Map<LO, GO, NT> >& subsetMap) {
  int lclSuccess = 1;  // to be modified below
...
// We want to collect all the test's output.
template <class LO, class GO, class NT>
void testSubsetMap(int& gblSuccess,  // output argument; 0 means false
                   std::ostream& out,
                   const Teuchos::RCP<const Tpetra::Map<LO, GO, NT> >& origMap,
...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(Map, replaceCommWithSubset, LO, GO, NT) {
  ...
  if (subsetMap.is_null()) {
    ...
    lclSuccess = 0;
    errStrm << "Process " << myRank << ": replaceCommWithSubset returned a "
                                       "nonnull Map with a null communicator"
            << endl;
  }

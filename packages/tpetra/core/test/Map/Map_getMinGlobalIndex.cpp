...
TEUCHOS_UNIT_TEST_TEMPLATE_2_DECL(Map, getMinGlobalIndex_nonuniform, LO, GO) {
  ...
  const GO myMinGID = (myRank == 0) ? std::numeric_limits<GO>::max() : static_cast<GO>((myRank - 1) * numLocal);
  const GO myMaxGID = (myRank == 0) ? std::numeric_limits<GO>::lowest() : static_cast<GO>(myRank * numLocal - 1);
...
TEUCHOS_UNIT_TEST_TEMPLATE_2_DECL(Map, getMinGlobalIndex_noncontiguous, LO, GO) {
  ...
  // create data to check validity of the map
  const GO myMinGID = (myRank == 0) ? std::numeric_limits<GO>::max() : static_cast<GO>(actualBase + myRank * numLocal);
  const GO myMaxGID = (myRank == 0) ? std::numeric_limits<GO>::lowest() : static_cast<GO>(actualBase + (myRank + 1) * numLocal - 1);
  GO minAllGIDs, maxAllGIDs;
  reduceAll<int, GO>(*comm, Teuchos::REDUCE_MIN, myMinGID, outArg(minAllGIDs));
  reduceAll<int, GO>(*comm, Teuchos::REDUCE_MAX, myMaxGID, outArg(maxAllGIDs));
  //

...
void Tpetra_Alltoallv(const void *sendbuf, const int *sendcounts,
    ...
    for (int source = 0; source < size; ++source) {
        MPI_Irecv(&rb[rdispls[source]], recvcounts[source], recvtype, source, ARBITRARY_TAG, comm, &rreqs[source]);
    } 
    for (int dest = 0; dest < size; ++dest) {
        MPI_Isend(&sb[sdispls[dest]], sendcounts[dest], sendtype, dest, ARBITRARY_TAG, comm, &sreqs[dest]);
    }
...
*/
void test_nothing(MPI_Comm comm,
    bool nullBufs,
    bool sameBufs,
    ...
    Tpetra_Alltoallv(sbuf, sendcounts.data(), senddispls.data(), MPI_BYTE, rbuf, recvcounts.data(), recvdispls.data(), MPI_BYTE, comm);

    // MPI Advance implementation should be okay
    MPI_Alltoallv(sbuf, sendcounts.data(), senddispls.data(), MPI_BYTE, rbuf, recvcounts.data(), recvdispls.data(), MPI_BYTE, comm);
...
*/
void test_random(MPI_Comm comm, int seed,
    Teuchos::FancyOStream& out,
    bool &success) {
    ...
    Tpetra_Alltoallv(sbuf.data(), sendcounts.data(), senddispls.data(), MPI_BYTE, exp.data(), recvcounts.data(), recvdispls.data(), MPI_BYTE, comm);
    MPI_Alltoallv(sbuf.data(), sendcounts.data(), senddispls.data(), MPI_BYTE, act.data(), recvcounts.data(), recvdispls.data(), MPI_BYTE, comm);
...
static MPI_Comm tpetra_default_comm_as_mpi_comm() {
    ...
    if (teuchosMpiComm) {
        ...
        throw std::runtime_error("couldn't convert tpetra::getDefaultComm() into MPI_Comm");
    }
}

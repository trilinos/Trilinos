...
bool falseOnSomeProcess(const Teuchos::Comm<int>& comm, const bool in) {
...
bool trueOnAllProcesses(const Teuchos::Comm<int>& comm, const bool in) {
  using Teuchos::outArg;
  using Teuchos::REDUCE_MIN;
  using Teuchos::reduceAll;
...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(ImportExport, IsLocallyComplete, LO, GO, NT) {
  ...
  if (numProcs > 1) {
    ...
    out << "Create map8 (like map6, but with one process (ideally in the "
           "middle, if numProcs > 2) with no indices)"
        << endl;

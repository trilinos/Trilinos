...
bool trueEverywhere(const bool localTruthValue,
...
void printMapCompactly(Teuchos::FancyOStream& out,
...
template <class LO, class GO, class NT>
bool importsLocallySame(Teuchos::FancyOStream& out,
...
template <class LO, class GO, class NT>
bool importsGloballySame(Teuchos::FancyOStream& out,
                         const Tpetra::Import<LO, GO, NT>& X,
                         const std::string& X_name,
                         const Tpetra::Import<LO, GO, NT>& Y,
                         const std::string& Y_name) {
...
makeTargetMapFromTestInput(const Issue2198TestInput<LO, GO, NT>& testInput,
  ...
  const std::vector<GO>& remoteGlobalIndices = optimized ? testInput.optimizedRemoteGlobalIndices : testInput.remoteGlobalIndices;
...
makeTest_A(const Teuchos::RCP<const Teuchos::Comm<int> >& comm) {
  ...
  TEUCHOS_TEST_FOR_EXCEPTION(numProcs < minNumProcsForTestA, std::invalid_argument,
                             "Test A needs at "
                             "least "
                                 << minNumProcsForTestA << "MPI processes, but the given "
                                                           "communicator has only "
                                 << numProcs
...
makeTest_B(const Teuchos::RCP<const Teuchos::Comm<int> >& comm) {
  const int numProcs = comm->getSize();
  TEUCHOS_TEST_FOR_EXCEPTION(numProcs < minNumProcsForTestB, std::invalid_argument,
                             "Test A needs at "
                             "least "
                                 << minNumProcsForTestB << "MPI processes, but the given "
                                                           "communicator has only "
                                 << numProcs
                                 << " process" << (numProcs != 1 ? "es" : "") << ".");
...
template <class LO, class GO, class NT>
void runTest(Teuchos::FancyOStream& out,
             bool& success,
...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(ImportExport, Issue2198, LO, GO, NT) {
  ...
  const int numProcs        = comm->getSize();
  constexpr int minNumProcs = minNumProcsForTestA < minNumProcsForTestB ? minNumProcsForTestA : minNumProcsForTestB;

...
void fillTargetVector(vec_type& tgtVector) {
  tgtVector.putScalar(baseValue());
}

void fillSourceVector(vec_type& srcVector) {
  ...
  const GO incrValue = (myRank % 2 == 0) ? GO(numProcs - (myRank / 2) - 1) : GO(myRank - 1) / GO(2);
...
void testVectorAbsMaxExport(bool& success,

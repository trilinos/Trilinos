...
void compareImports(bool& success, Teuchos::FancyOStream& out,
  ...
  if (numSame1 == numSame2) {
    ...
    if (sameTotals) {
      ...
      const size_t numSameDiff = (numSame1 > numSame2) ? (numSame1 - numSame2) : (numSame2 - numSame1);
...
TEUCHOS_UNIT_TEST_TEMPLATE_3_DECL(ImportUnion, ContigPlusContig, LocalOrdinalType, GlobalOrdinalType, NodeType) {
  ...
  Array<GO> tgtMap1Gids;
  {
    ...
    const GO lower = (n * myRank < 1) ? static_cast<GO>(0) : static_cast<GO>(n * myRank - 1);
  ...
  Array<GO> tgtMap2Gids;
  {
    ...
    const GO lower = (n * myRank < 2) ? static_cast<GO>(0) : static_cast<GO>(n * myRank - 2);
  ...
  out << "Testing whether union Import (1,2) works like "
         "expected Union Import with a Vector"
      << endl;
  {

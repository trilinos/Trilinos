...
createLaplace1D(const Teuchos::RCP<const Tpetra::Map<LocalOrdinalType, GlobalOrdinalType, NodeType> >& rowMap,
  ...
  Kokkos::parallel_for(
      "matrix fill", numRows, KOKKOS_LAMBDA(const LO& row) {
        if (row == 0) {
          // First row on proc
          LO row_start = 0;
...
bool compareCrsMatrix(const CrsMatrixType& A_orig, const CrsMatrixType& A) {
  using Teuchos::Array;
  using Teuchos::ArrayView;
  using Teuchos::Comm;
  using Teuchos::RCP;
...
size_t get_memory_usage_now() {
  ...
  if (proc_stat) {
    std::string buf;
    proc_stat >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> buf >> rss_pages;
    proc_stat.close();
  }
...
int main(int narg, char* arg[]) {
  ...
  int run_case = -1;
  cmdp.setOption("case", &run_case, "Which case to run");
  bool watchr_output = false;
  cmdp.setOption("watchr-output", "no-watchr-output", &watchr_output, "Output memory data for watchr");

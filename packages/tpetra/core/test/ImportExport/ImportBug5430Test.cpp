...
bool countFailures(const Teuchos::RCP<const Teuchos::Comm<int> >& teuchosComm,
                   // Epetra widgets
...
bool testMain(int argc, char* argv[]) {
  using std::cout;
  using std::endl;
  using Teuchos::ArrayRCP;
  ...
  if (verbose) {
    out << "Testing Import from owned Map to overlap Map:" << endl
        << endl;
  }
  ...
  if (testOtherDirections) {
    ...
    if (verbose) {
      out << "Testing Export from owned Map to overlap Map:" << endl
          << endl;
    }
    ...
    if (verbose) {
      out << "Testing Import from overlap Map to owned Map:" << endl
          << endl;
    }

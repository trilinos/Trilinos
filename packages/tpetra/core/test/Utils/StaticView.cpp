...
bool view1dSame(ViewType1 x, ViewType2 y) {
...
template <class ViewType1, class ViewType2>
bool view2dSame(ViewType1 x, ViewType2 y) {
  using execution_space = typename ViewType1::execution_space;
  using range_type      = Kokkos::RangePolicy<execution_space, size_t>;
...
void view1dIota(const Kokkos::View<ValueType*, DeviceType>& x,
...
void view2dIota(const ViewType& x,
...
template <class ValueType, class DeviceType>
void testStatic1dView(bool& success, Teuchos::FancyOStream& out,
  ...
  view_type referenceView(Kokkos::view_alloc("Reference View", Kokkos::WithoutInitializing),
                          maxSize);
...
void testStatic1dViewValues(bool& success, Teuchos::FancyOStream& out,
                            const std::string& deviceName) {
  testStatic1dView<double, DeviceType>(success, out, "double", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic1dView<float, DeviceType>(success, out, "float", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic1dView<Kokkos::complex<double>, DeviceType>(success, out, "complex<double>", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic1dView<Kokkos::complex<float>, DeviceType>(success, out, "complex<float>", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic1dView<size_t, DeviceType>(success, out, "size_t", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic1dView<int, DeviceType>(success, out, "int", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic1dView<long long, DeviceType>(success, out, "long long", deviceName);
}

void testStatic1dViewValuesAndDevices(bool& success, Teuchos::FancyOStream& out){
#ifdef KOKKOS_ENABLE_CUDA
    {using device_type = Kokkos::Device<Kokkos::Cuda, Kokkos::CudaUVMSpace>;
testStatic1dViewValues<device_type>(success, out, "(Cuda,CudaUVMSpace)");
if (!success) {
  out << "Stopping test early" << std::endl;
  return;
}
}
{
  ...
  testStatic1dViewValues<device_type>(success, out, "(Cuda,CudaSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
}
{
  ...
  testStatic1dViewValues<device_type>(success, out, "(Host,CudaHostPinnedSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
...
#ifdef KOKKOS_ENABLE_OPENMP
{
  ...
  testStatic1dViewValues<device_type>(success, out, "(OpenMP,HostSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
...
#ifdef KOKKOS_ENABLE_THREADS
{
  ...
  testStatic1dViewValues<device_type>(success, out, "(Threads,HostSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
...
#ifdef KOKKOS_ENABLE_SERIAL
{
  ...
  testStatic1dViewValues<device_type>(success, out, "(Serial,HostSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
...
template <class ValueType, class DeviceType>
void testStatic2dView(bool& success, Teuchos::FancyOStream& out,
                      const std::string& valueName,
                      const std::string& deviceName) {
  ...
  for (size_t row_size : row_sizes) {
    for (size_t col_size : col_sizes) {
      view_type view =
          Tpetra::Details::getStatic2dView<ValueType, DeviceType>(row_size, col_size);
...
template <class DeviceType>
void testStatic2dViewValues(bool& success, Teuchos::FancyOStream& out,
                            const std::string& deviceName) {
  testStatic2dView<double, DeviceType>(success, out, "double", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dView<float, DeviceType>(success, out, "float", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dView<Kokkos::complex<double>, DeviceType>(success, out, "complex<double>", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dView<Kokkos::complex<float>, DeviceType>(success, out, "complex<float>", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dView<size_t, DeviceType>(success, out, "size_t", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dView<int, DeviceType>(success, out, "int", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dView<long long, DeviceType>(success, out, "long long", deviceName);
}

void testStatic2dViewValuesAndDevices(bool& success, Teuchos::FancyOStream& out){
#ifdef KOKKOS_ENABLE_CUDA
    {using device_type = Kokkos::Device<Kokkos::Cuda, Kokkos::CudaUVMSpace>;
testStatic2dViewValues<device_type>(success, out, "(Cuda,CudaUVMSpace)");
if (!success) {
  out << "Stopping test early" << std::endl;
  return;
}
...
{
  using device_type = Kokkos::Device<Kokkos::Cuda, Kokkos::CudaSpace>;
  testStatic2dViewValues<device_type>(success, out, "(Cuda,CudaSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
}
{
  using device_type = Kokkos::Device<Kokkos::HostSpace::execution_space,
                                     Kokkos::CudaHostPinnedSpace>;
  testStatic2dViewValues<device_type>(success, out, "(Host,CudaHostPinnedSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
...
{
  using device_type = Kokkos::Device<Kokkos::OpenMP, Kokkos::HostSpace>;
  testStatic2dViewValues<device_type>(success, out, "(OpenMP,HostSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
...
{
  using device_type = Kokkos::Device<Kokkos::Threads, Kokkos::HostSpace>;
  testStatic2dViewValues<device_type>(success, out, "(Threads,HostSpace)");
}
#endif  // KOKKOS_ENABLE_THREADS
...
{
  using device_type = Kokkos::Device<Kokkos::Serial, Kokkos::HostSpace>;
  testStatic2dViewValues<device_type>(success, out, "(Serial,HostSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
...
template <class ValueType, class DeviceType>
void testStatic2dDualView(bool& success, Teuchos::FancyOStream& out,
                          const std::string& valueName,
                          const std::string& deviceName) {
  ...
  for (size_t row_size : row_sizes) {
    for (size_t col_size : col_sizes) {
      dual_view_type view =
          Tpetra::Details::getStatic2dDualView<ValueType, DeviceType>(row_size, col_size);
      TEST_ASSERT(size_t(view.extent(0)) == row_size);
      TEST_ASSERT(size_t(view.extent(1)) == col_size);
...
template <class DeviceType>
void testStatic2dDualViewValues(bool& success, Teuchos::FancyOStream& out,
                                const std::string& deviceName) {
  testStatic2dDualView<double, DeviceType>(success, out, "double", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dDualView<float, DeviceType>(success, out, "float", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dDualView<Kokkos::complex<double>, DeviceType>(success, out, "complex<double>", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dDualView<Kokkos::complex<float>, DeviceType>(success, out, "complex<float>", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dDualView<size_t, DeviceType>(success, out, "size_t", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dDualView<int, DeviceType>(success, out, "int", deviceName);
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

  testStatic2dDualView<long long, DeviceType>(success, out, "long long", deviceName);
}

void testStatic2dDualViewValuesAndDevices(bool& success, Teuchos::FancyOStream& out){
#ifdef KOKKOS_ENABLE_CUDA
    {using device_type = Kokkos::Device<Kokkos::Cuda, Kokkos::CudaUVMSpace>;
testStatic2dDualViewValues<device_type>(success, out, "(Cuda,CudaUVMSpace)");
if (!success) {
  out << "Stopping test early" << std::endl;
  return;
}
...
{
  using device_type = Kokkos::Device<Kokkos::Cuda, Kokkos::CudaSpace>;
  testStatic2dDualViewValues<device_type>(success, out, "(Cuda,CudaSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
}
{
  using device_type = Kokkos::Device<Kokkos::HostSpace::execution_space,
                                     Kokkos::CudaHostPinnedSpace>;
  testStatic2dDualViewValues<device_type>(success, out, "(Host,CudaHostPinnedSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
...
{
  using device_type = Kokkos::Device<Kokkos::OpenMP, Kokkos::HostSpace>;
  testStatic2dDualViewValues<device_type>(success, out, "(OpenMP,HostSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
...
{
  using device_type = Kokkos::Device<Kokkos::Threads, Kokkos::HostSpace>;
  testStatic2dDualViewValues<device_type>(success, out, "(Threads,HostSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }
...
{
  using device_type = Kokkos::Device<Kokkos::Serial, Kokkos::HostSpace>;
  testStatic2dDualViewValues<device_type>(success, out, "(Serial,HostSpace)");
  if (!success) {
    out << "Stopping test early" << std::endl;
    return;
  }

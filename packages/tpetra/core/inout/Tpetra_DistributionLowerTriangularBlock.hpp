 ...
 public:
  ...
  DistributionLowerTriangularBlock(size_t nrows_,
    ...
    TEUCHOS_TEST_FOR_EXCEPTION(nChunks * (nChunks + 1) != npnp, std::logic_error,
                               "Number of processors np = " << np << " must satisfy np = q(q+1)/2 for some q"
  ...
  bool Mine(gno_t i, gno_t j) {
    if (redistributed) {
      if (j > i)
        return false;  // Don't keep any upper triangular entries
      else
...
class LowerTriangularBlockOperator : public Tpetra::Operator<scalar_t, Tpetra::Map<>::local_ordinal_type,

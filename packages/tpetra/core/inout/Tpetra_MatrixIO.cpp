...
void Tpetra::Utils::readHBHeader(std::ifstream &fin, Teuchos::ArrayRCP<char> &Title, Teuchos::ArrayRCP<char> &Key, Teuchos::ArrayRCP<char> &Type,
  ...
  Ptrfmt.resize(16 + 1);
  std::fill(Ptrfmt.begin(), Ptrfmt.end(), '\0');
  Indfmt.resize(16 + 1);
  std::fill(Indfmt.begin(), Indfmt.end(), '\0');
  Valfmt.resize(20 + 1);
  std::fill(Valfmt.begin(), Valfmt.end(), '\0');
  Rhsfmt.resize(20 + 1);
  std::fill(Rhsfmt.begin(), Rhsfmt.end(), '\0');
  //
  ...
  try {
    ...
    if (readPatternOnly == false) {
      ...
      for (int lno = 0; lno < valCrd; ++lno) {
        ...
        if (valFlag == 'D') std::replace_if(
            lineBuf, lineBuf + MAXSIZE, [](const char c) { return c == 'D'; }, 'E');
        char *linePtr = lineBuf;

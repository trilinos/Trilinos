...
TEUCHOS_UNIT_TEST_TEMPLATE_1_DECL( ROpNorm1, unitStride, Scalar )
{
  ...
  out << "v="<<v<<"\n";
  SubVectorView<Scalar> sv = newSubVectorView<Scalar>(n, v);
  RTOpPack::ROpNorm1<Scalar> normInfOp;
  RCP<RTOpPack::ReductTarget> normInf = normInfOp.reduct_obj_create();
  normInfOp.apply_op(
    tuple<ConstSubVectorView<Scalar> >(sv)(),
    Teuchos::null,
  ...
  TEST_FLOATING_EQUALITY( normInfOp(*normInf), as<ScalarMag>(n*ST::magnitude(v)),
    as<ScalarMag>(ST::eps() * errorTolSlack) );
}
...
TEUCHOS_UNIT_TEST_TEMPLATE_1_DECL( ROpNorm1, reduct, Scalar )
{
  ...
  TEST_FLOATING_EQUALITY( scalarReduct2.get(), three+four+two,
    as<ScalarMag>(ST::eps() * errorTolSlack) );

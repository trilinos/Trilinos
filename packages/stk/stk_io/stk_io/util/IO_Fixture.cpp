...
void IO_Fixture::set_meta_data( Teuchos::RCP<stk::mesh::MetaData> arg_meta_data )
{
...
void IO_Fixture::initialize_meta_data( const std::string & base_filename, const std::string & mesh_type)
{
  ...
  m_fem_meta_data = Teuchos::rcp( new stk::mesh::MetaData());
...
void IO_Fixture::initialize_bulk_data()
{
  ...
  m_bulk_data = Teuchos::rcp( new stk::mesh::BulkData(meta_data(), m_comm));

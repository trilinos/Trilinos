...
{
  return stk_classic::mesh::put_field( meta_data.declare_field<VectorField>(s), stk_classic::mesh::fem::FEMMetaData::NODE_RANK , meta_data.universal_part() , n1 );
}
...
declare_vector_field_on_all_elements(
  stk_classic::mesh::fem::FEMMetaData & meta_data , const std::string & s , unsigned n1 )
{
  return stk_classic::mesh::put_field( meta_data.declare_field<VectorField>(s), get_element_rank(meta_data), meta_data.universal_part() , n1 );
}
...
void zero_field_data( stk_classic::mesh::BulkData & mesh , stk_classic::mesh::EntityRank type , const FieldType & field )
{
  ...
  const VectorField & node_velocity )
{
  ...
  for ( std::vector<stk_classic::mesh::Bucket*>::const_iterator k = buckets.begin() ; k != buckets.end() ; ++k ) {
    ...
    double * X     = stk_classic::mesh::field_data( node_model_corrdinates , bucket.begin() );

...
def setup_parser():
    ...
    # sierra options
    sierra_opts = parser.add_argument_group('sierra control')
    sierra_opts.add_argument('-x', '--bin-dir', default="", metavar='PATH_TO_EXE', required=True, help='you must supply a path to which stk_adapt_exe to use')
...
def get_cmd_str(args, close_string=True):
    ...
    cmd_str += " --bin-dir %s " % bin_dir

    # Find what args/options need to go to stk_adapt
    cmd_str += ' stk_adapt_exe '
    for arg in dir(args):
...
def run_or_print(cmd_str):
    global debug
    output = "debug...%s"%debug
    #print output
    if debug:
...
def run_or_print_par_0(cmd_str, i_pass, streaming_W, iW,  timeout=60):
    cmd_str1 = cmd_str + add_args(i_pass, streaming_W, iW)
    run_or_print(cmd_str1)

def run_or_print_par(cmd_str, i_pass, streaming_W, timeout=600):
    ...
    if False:
        result = map(run_or_print, argsl)
    else:
        # parallel run
        result = pool.map(run_or_print, argsl)
    ...
    if streaming_W == 0:
        ...
        for i_pass in range(-1, 4):
            ...
            if False:
                ...
                print "startparallel ..."
                run_or_print_par(cmd_str, i_pass, streaming_W)
                print "end parallel"

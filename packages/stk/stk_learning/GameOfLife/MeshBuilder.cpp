...
// Game of Life Mesh
MeshBuilder::MeshBuilder(stk::ParallelMachine comm, stk::topology elemType, unsigned spacialDim,
...
//private
void MeshBuilder::declare_fields()
{
    m_lifeField =
...
void MeshBuilder::put_fields_on_parts()
{
    int val = 0;
...
void MeshBuilder::declare_parts()
{
...
void MeshBuilder::declare_entities()
{
    for (unsigned index = 0; index < m_elemsOnProc; index++)
...
TwoDimensionalMeshBuilder::TwoDimensionalMeshBuilder(stk::ParallelMachine comm,
                                                     stk::topology elemType,
                                                     unsigned width, unsigned height,
                                                     stk::mesh::BulkData::AutomaticAuraOption
                                                     auraOption)
:MeshBuilder(comm, elemType, 2, auraOption), m_width(width), m_height(height),
...
// private
void TwoDimensionalMeshBuilder::declare_coordinate_field()
{
...
bool TwoDimensionalMeshBuilder::only_one_active_proc()
{
    return m_height < m_numProcs;
}
void TwoDimensionalMeshBuilder::share_nodes_between_processors()
{
...
void TwoDimensionalMeshBuilder::number_coordinate_field()
{
    for (size_t index = 0, numNodes = m_nodes.size(); index<numNodes; index++)
        number_coordinate_field_of_node(index);
}
void TwoDimensionalMeshBuilder::number_coordinate_field_of_node(unsigned nodeIndex)
{
...
void TwoDimensionalMeshBuilder::share_top_nodes()
{
...
void TwoDimensionalMeshBuilder::share_bottom_nodes()
{
...
void TwoDimensionalMeshBuilder::share_node_with_this_id_to_this_processor(unsigned nodeId,
                                                                          unsigned procNum)
{
...
TriangleMeshBuilder::TriangleMeshBuilder(stk::ParallelMachine comm, unsigned width,
                                         unsigned height,
                                         stk::mesh::BulkData::AutomaticAuraOption auraOption)
:TwoDimensionalMeshBuilder(comm, stk::topology::TRIANGLE_3, width, height, auraOption)
{
...
//private
void TriangleMeshBuilder::declare_element_nodes_ids()
{
...
void TriangleMeshBuilder::declare_node_ids_of_two_elements(unsigned index,
...
void TriangleMeshBuilder::declare_node_ids_of_two_first_row_elements(unsigned index,
                                                                     unsigned initialOffset)
{
...
void TriangleMeshBuilder::declare_node_ids_of_two_sucessive_row_elements(unsigned index)
{
    declare_node_ids_of_this_element(index);
    declare_node_ids_of_this_element(index+1);
}
void TriangleMeshBuilder::declare_node_ids_of_this_element(unsigned index)
{
    m_elemNodeIds[index].resize(3);
...
QuadMeshBuilder::QuadMeshBuilder(stk::ParallelMachine comm, unsigned width, unsigned height,
                                 stk::mesh::BulkData::AutomaticAuraOption auraOption)
:TwoDimensionalMeshBuilder(comm, stk::topology::QUAD_4, width, height, auraOption)
{
...
//private
void QuadMeshBuilder::declare_element_nodes_ids()
{
...
void QuadMeshBuilder::declare_node_ids_of_element(unsigned index, unsigned initialOffset)
{
    if (index < m_elemsPerRow)
...
void QuadMeshBuilder::declare_first_row_element_nodes(unsigned index, unsigned offset)
{
    m_elemNodeIds[index].resize(4);
    m_elemNodeIds[index] = {offset+1, offset+2,offset+m_nodesPerRow+2,offset+m_nodesPerRow+1};
}
void QuadMeshBuilder::declare_remaining_element_nodes(unsigned index)
{
    m_elemNodeIds[index].resize(4);
...
ThreeDimensionalMeshBuilder::ThreeDimensionalMeshBuilder(stk::ParallelMachine comm,
                                                         stk::topology elemType, unsigned width,
                                                         unsigned height, unsigned depth,
                                                         stk::mesh::BulkData::AutomaticAuraOption
                                                         auraOption)
:MeshBuilder(comm, elemType, 3, auraOption), m_width(width), m_height(height), m_depth(depth),
...
//private
void ThreeDimensionalMeshBuilder::declare_coordinate_field()
{
...
bool ThreeDimensionalMeshBuilder::only_one_active_proc()
{
    return m_depth < m_numProcs;
}
void ThreeDimensionalMeshBuilder::share_nodes_between_processors()
{
    if (m_numProcs-1 != m_procRank)
        share_nodes_in_back();
    if (0 != m_procRank)
...
void ThreeDimensionalMeshBuilder::share_nodes_in_back()
{
...
void ThreeDimensionalMeshBuilder::share_nodes_in_front()
{
    unsigned nodeOffset = m_nodesPerSlice*m_slicesPerProc*m_procRank;
    for (unsigned index = 1; index <= m_nodesPerSlice; index++)
        share_node_with_this_id_to_this_processor(index+nodeOffset, m_procRank-1);
}
void ThreeDimensionalMeshBuilder::share_node_with_this_id_to_this_processor(unsigned nodeId,
                                                                            unsigned procNum)
{
    stk::mesh::Entity node = bulk_data()->get_entity(stk::topology::NODE_RANK, nodeId);
    bulk_data()->add_node_sharing(node, procNum);
}
void ThreeDimensionalMeshBuilder::number_coordinate_field()
{
    for (unsigned index = 0, numNodes = m_nodes.size(); index < numNodes; index++)
        number_coordinate_field_for_node(index);
}
void ThreeDimensionalMeshBuilder::number_coordinate_field_for_node(unsigned nodeIndex)
{
    double* const coord = stk::mesh::field_data(*m_nodeCoords, m_nodes[nodeIndex]);
...
HexMeshBuilder::HexMeshBuilder(stk::ParallelMachine comm, unsigned width, unsigned height,
                               unsigned depth, stk::mesh::BulkData::AutomaticAuraOption
                               auraOption)
:ThreeDimensionalMeshBuilder(comm, stk::topology::HEX_8, width, height, depth, auraOption)
{
...
//private
void HexMeshBuilder::declare_element_nodes_ids()
{
    m_elemNodeIds.resize(m_elemsOnProc);
...
void HexMeshBuilder::declare_node_ids_of_element(unsigned index, unsigned offset)
{
...
void HexMeshBuilder::declare_first_slice_element_node_ids(stk::mesh::EntityIdVector& V,
...
void HexMeshBuilder::declare_remaining_element_node_ids(stk::mesh::EntityIdVector& newer,

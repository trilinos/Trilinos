...
protected:
  ...
  void run_decomp_with_method(const std::string& method, stk::mesh::Selector selector, stk::mesh::EntityRank primary_rank)
  {
    ...
    stk::mesh::EntityVector entitiesToBalance = stk::balance::internal::get_entities_to_balance(selector,
                                                                                                primary_rank,
                                                                                                get_bulk());
    stk::balance::internal::GeometricVertices vertexInfo(geometricBalanceSettings, get_bulk(), entitiesToBalance, {selector});
    for(size_t i=0;i<entitiesToBalance.size();++i)
    {

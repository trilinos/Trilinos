...
class AttributesInFile_legacy : public stk::unit_test_util::MeshFixture
{
protected:
...
TEST_F(AttributesInFile_legacy, reading_fieldsReturnedInOrder)
{
...
TEST_F(AttributesInFile_legacy, readWriteRead_fieldsReturnedInOrder)
{
    if (!std::ifstream(filename)) {
      std::cout<<"file "<<filename<<" doesn't exist, skipping test."<<std::endl;
      return;
    ...
    if(stk::parallel_machine_size(get_comm()) == 1)
    {
        ...
        std::shared_ptr<stk::mesh::BulkData> bulk = build_mesh_no_simple_fields(get_comm());

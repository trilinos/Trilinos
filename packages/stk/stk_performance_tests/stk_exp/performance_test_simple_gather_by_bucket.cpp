                   ...
                   std::vector<double> & avg_centroid)
{
  ...
  BOOST_FOREACH( bucket_key bucket, get_selected_buckets( element_select, mesh)) {
    ...
    size_t offset = 0;
    BOOST_FOREACH( entity_descriptor element, mesh.get_entities(bucket)) {
      BOOST_FOREACH( csr_mesh::relation_descriptor node_relation, mesh.get_relations(element,node_rank)) {
    ...
    stk::performance_tests::calculate_centroid_3d(num_elems, num_nodes,&elem_node_coords[0],&elem_centroid[0]);
...
STKUNIT_UNIT_TEST( csr_mesh, gears_gather_by_bucket)
{
  ...
  for(int t=0; t<num_iters; ++t) {
    simple_gather(gear.m_element_part, gear.m_coordinates, *mesh, avg_centroid);

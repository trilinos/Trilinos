...
{
  size_t num_elements = coord_ptrs.size();
  int num_nodes = 8;//!!HARD-WIRED!!
  std::vector<double> elem_centroid(num_elements*3,0);
  std::vector<double> elem_node_coords(num_elements*num_nodes*3,0);
  size_t offset = 0;
  ...
  stk::performance_tests::calculate_centroid_3d(num_elements, num_nodes,&elem_node_coords[0],&elem_centroid[0]);

  for(size_t i=0; i<num_elements; ++i) {
    //add this element-centroid to the avg_centroid vector, and
    //re-zero the element-centroid vector:
    avg_centroid[0] += elem_centroid[i*3 + 0];
    avg_centroid[1] += elem_centroid[i*3 + 1];
    avg_centroid[2] += elem_centroid[i*3 + 2];
  }
}

}

STKUNIT_UNIT_TEST(csr_mesh, fast_ptr_gather_by_bucket)
{

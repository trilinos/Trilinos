...
#include <performance_tests/mesh/bucket_traverse.hpp>
...
using sierra::mesh::csr_mesh;

TEST( Hex, CSR_Performance_on_Generic_Hex_Gather )
{
...
TEST( Hex, CSR_Performance_on_Generic_Hex_Gather_Bucket_Traverse )
{
  double start_time = stk::cpu_time();
  unsigned ex=100, ey=100, ez=100;
  unsigned num_elems = ex*ey*ez;
...
struct CentroidVisitor2: public BucketVisitor<Mesh>
{
  ...
  void finish_entity(entity_descriptor /* entity */, Mesh & /* mesh */) {
    calculate_centroid_3d(m_numNodes, &m_elementNodeCoordinate[0], &m_elementCentroid[0]);
...
TEST( Hex, CSR_Performance_on_Generic_Hex_Gather_Bucket_Traverse_2 )
{
  double start_time = stk::cpu_time();
  unsigned ex=100, ey=100, ez=100;
  unsigned num_elems = ex*ey*ez;

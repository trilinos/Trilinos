...
typedef typename std::array<REAL,DIM>::iterator iterator;
 ...
 public:
  ...
  bool ZeroLength() const; ///< Return true if vector is zero length
  ...
  REAL LengthSquared() const; ///< Magnitude squared (faster, avoids square root)
  Vec<REAL,DIM> UnitVector() const; ///< Return unit length vector leaving original unchanged
...
template<class REAL, unsigned DIM>
inline bool Vec<REAL,DIM>::ZeroLength() const
{
...
template<class REAL, unsigned DIM>
inline REAL Vec<REAL,DIM>::LengthSquared() const
{
  REAL length_squared = 0.0;
  for (unsigned i=0; i<DIM; ++i) length_squared += vec[i]*vec[i];
...
template<class REAL, unsigned DIM>
inline Vec<REAL,DIM> Vec<REAL,DIM>::UnitVector() const
{
  const REAL inv_length = 1.0/Length();

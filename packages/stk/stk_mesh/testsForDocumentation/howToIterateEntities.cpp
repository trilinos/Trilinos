...
TEST(StkMeshHowTo, iterateSidesetNodesMostEfficientlyForFieldDataAccess)
{
    ...
    stk::mesh::Field<double> &temperatureField = stkMeshMetaData.declare_field<stk::mesh::Field<double> >("temperature");
    ...
    for (size_t bucketIndex = 0; bucketIndex < boundaryNodeBuckets.size(); ++bucketIndex)
    {
        ...
        double *temperatureValues = stkMeshBulkData.nodal_field_data(temperatureField, nodeBucket);
...
TEST(StkMeshHowTo, iterateSidesetNodesWithFieldDataAccess)
{
    ...
    for (size_t nodeIndex = 0; nodeIndex < nodes.size(); ++nodeIndex)
    {
        ...
        double *temperatureValues = stkMeshBulkData.nodal_field_data(temperatureField, nodes[nodeIndex]);

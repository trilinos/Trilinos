...
Note the double shell between nodes 31, 36 and 36, 41
*/

GridMeshWithShellFixture::GridMeshWithShellFixture(ParallelMachine pm)
...
GridMeshWithShellFixture::~GridMeshWithShellFixture()
{ }

void GridMeshWithShellFixture::generate_grid()
{
  ...
  std::vector<Entity*> shell_faces;
  {
    ...
    for (unsigned i = 0; i < num_shell_faces; ++i) {
      Entity& new_shell =
        m_bulk_data.declare_entity(Face, m_shell_face_ids[i], shell_parts);
  ...
  // declare relationships
  {
    ...
    for (unsigned i = 0; i < num_shell_1_faces; ++i) {
      ...
      Entity& node1 = *all_entities[node_list_1[i] - 1];
      Entity& node2 = *all_entities[node_list_1[i+1] - 1];
    ...
    for (unsigned i = 0; i < num_shell_2_faces; ++i) {
      Entity& shell = *(shell_faces[i + num_shell_1_faces]);
      Entity& node1 = *all_entities[node_list_2[i] - 1];
      Entity& node2 = *all_entities[node_list_2[i+1] - 1];
      m_bulk_data.declare_relation(shell, node1, 0);
      m_bulk_data.declare_relation(shell, node2, 1);
    }
  }

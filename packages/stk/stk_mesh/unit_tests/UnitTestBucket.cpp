...
STKUNIT_UNIT_TEST(UnitTestingOfBucket, testDangerousIterationIsCaught)
{
// only run in debug
#ifdef NDEBUG
  return;
  ...
  for (int i = 0; i < 10; ++i) {
    bulk.declare_entity(stk::topology::ELEMENT_RANK, i + 1, entity_parts);
  }
  ...
  EntityId bad_mod_entity = 42;

  BucketVector const& buckets = bulk.buckets(stk::topology::ELEMENT_RANK);
  for (size_t b = 0, be = buckets.size(); b < be; ++b) {
    Bucket const& bucket = *buckets[b];

    for (size_t e = 0, ee = bucket.size(); e < ee; ++e) {
      if (e > 0) {
        STKUNIT_ASSERT_THROW(bucket[e], std::logic_error);
      }
      else {
        bulk.declare_entity(stk::topology::ELEMENT_RANK, bad_mod_entity, entity_parts);
      }
    }
    ...
    for (size_t e = 0; e < bucket.size(); ++e) {
      if (e > 0) {
        ...
        bulk.destroy_entity( bulk.get_entity(stk::topology::ELEMENT_RANK, bad_mod_entity) );
      }
    }

    Bucket::iterator begin_itr = bucket.begin();
    for (Bucket::iterator itr = begin_itr, end = bucket.end(); itr != end; ++itr) {
      if (itr != begin_itr) {
        Entity e;
        STKUNIT_ASSERT_THROW(e = *itr, std::logic_error);
        (void) e;
      }
      else {
        bulk.declare_entity(stk::topology::ELEMENT_RANK, bad_mod_entity + 1, entity_parts);
      }
    }
  }

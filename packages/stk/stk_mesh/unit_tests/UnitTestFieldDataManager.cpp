...
void testAddingSingleEntity(stk::mesh::MetaData &meshMetaData, stk::mesh::ContiguousFieldDataManager &fieldDataManager)
{
    ...
    EXPECT_EQ(NULL, part2FieldMetaDataVector[firstBucketIndex].m_data);
    EXPECT_EQ(NULL, fieldRawData[field2Ordinal]);
...
TEST(ContiguousFieldDataManagerTest, add_field_data_for_entity)
{
    ...
    if(numProcs == 1)
    {
        ...
        EXPECT_EQ(NULL, part1FieldMetaDataVector[firstBucketIndex].m_data);
        EXPECT_EQ(NULL, part2FieldMetaDataVector[firstBucketIndex].m_data);

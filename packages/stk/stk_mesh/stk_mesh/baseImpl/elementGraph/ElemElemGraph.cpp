    ...
    for(int proc_id=0; proc_id<m_bulk_data.parallel_size(); ++proc_id)
    {
        if (proc_id != m_bulk_data.parallel_rank())
        {
            while(comm.recv_buffer(proc_id).remaining())
            {
                ...
                elementData.m_sideNodes = convert_keys_to_entities(m_bulk_data, node_keys);
...
stk::mesh::EntityVector get_filtered_element_list_for_received_side_nodes(const stk::mesh::BulkData& bulk_data, const stk::mesh::impl::ConnectedElementDataVector & elemSideDataReceived )
{
    ...
    return localElementsConnectedToReceivedSideNodes;
}
                                                                               ...
                                                                               std::vector<impl::SharedEdgeInfo> &newlySharedEdges)
{
    ...
    stk::mesh::EntityVector localElementsAttachedToReceivedNodes = get_filtered_element_list_for_received_side_nodes(m_bulk_data, elemSideDataReceived);
            ...
            if (num_nodes_this_side == num_side_nodes_received_element)
            {
                ...
                for (const impl::ConnectedElementData & elemDataFromOtherProc : filteredCommunicatedElementData)
                {
                    ...
                    if (isRemoteElement && are_local_element_and_remote_element_connected_via_side_nodes)
                    {
                        ...
                        impl::LocalId negativeRemoteElemId = -1*elemDataFromOtherProc.m_elementIdentifier;
                        stk::mesh::GraphEdge graphEdge(local_elem_id, side_index, negativeRemoteElemId, elemDataFromOtherProc.m_sideIndex);
                        m_graph.add_edge(graphEdge);
...
std::vector<int> ElemElemGraph::get_exposed_sides(stk::mesh::impl::LocalId localId, int maxSidesThisElement)
{
    std::vector<int> exposedSides;
    impl::add_exposed_sides(localId, maxSidesThisElement, m_graph, exposedSides);
    if(m_air_selector != nullptr)
...
{
    int maxSidesThisElement = m_element_topologies[localId].num_sides();
    std::vector<int> exposedSides;
...
{
    std::vector<SideSetEntry> skinnedSideSet = extract_skinned_sideset();
...
void ElemElemGraph::create_interior_block_boundary_sides(const stk::mesh::PartVector& skinParts)
{
    ...
    for(const stk::mesh::Bucket* bucket : buckets)
    {
        if(bucket->owned())
        {
            for(size_t i=0;i<bucket->size();++i)
            {
                ...
                impl::LocalId elementId = get_local_element_id(element);
                for(size_t j = 0; j < m_graph.get_num_edges_for_element(elementId); ++j)
                {
                    ...
                    std::vector<stk::mesh::PartOrdinal> *part_ordinals = nullptr;
                    ...
                    if(isParallelEdge)
                    {
                        ...
                        part_ordinals = &parallel_edge_info.m_part_ordinals;
                    }
                    else
                    {
                    ...
                    if(!isParallelEdge && m_bulk_data.identifier(element) < otherEntityId)
                    {
                        should_add_side = !stk::mesh::impl::are_entity_element_blocks_equivalent(m_bulk_data, element, otherElement);
                    }
                    else if(isParallelEdge)
                    {
                        should_add_side = !stk::mesh::impl::are_entity_element_blocks_equivalent(m_bulk_data, element, *part_ordinals);
                    }
...
size_t ElemElemGraph::create_face_entities_per_element(size_t element_side_index, const std::vector<SideSetEntry> &skinnedSideSet, const stk::mesh::PartVector& skinParts, std::vector<stk::mesh::sharing_info> &sharedModified)
{
    std::vector<int> ordinals = get_side_ordinals_of_element(element_side_index, skinnedSideSet);
    stk::mesh::impl::LocalId localId = get_local_element_id(skinnedSideSet[element_side_index].element);
    create_side_entities(ordinals, localId, skinParts, sharedModified);
...
void ElemElemGraph::create_side_entities_given_sideset(const std::vector<SideSetEntry> &skinnedSideSet, const stk::mesh::PartVector& skinParts, std::vector<stk::mesh::sharing_info>& sharedModified)
{

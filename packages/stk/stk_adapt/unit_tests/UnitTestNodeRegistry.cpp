...
#define EXTRA_PRINT 0
        ...
        if (p_size == 2)
          {
            ...
            const stk::mesh::Entity* element_1_p = eMesh.getBulkData()->get_entity(stk::mesh::Element, elem_num);
            dw() << "P["<<p_rank<<"] element_1_p = " << element_1_p << DWENDL;
            NeededEntityType entity_rank(stk::mesh::Edge, 1u);
            //stk::mesh::EntityRank entity_rank = stk::mesh::Edge;
            unsigned iSubDimOrd = 0u;

            const stk::mesh::Entity& element_1 = *eMesh.getBulkData()->get_entity(stk::mesh::Element, elem_num);
        ...
        if (p_size == 2)
          {
            ...
            const stk::mesh::Entity* element_local_p = eMesh.getBulkData()->get_entity(stk::mesh::Element, elem_num_local);
            const stk::mesh::Entity* element_ghost_p = eMesh.getBulkData()->get_entity(stk::mesh::Element, elem_num_ghost);
      ...
      //======================================================================================================================      
      //======================================================================================================================      
      TEST(nodeRegistry, test_parallel_1)
      {
        EXCEPTWATCH;
        MPI_Barrier( MPI_COMM_WORLD );
      ...
      TEST(nodeRegistry, test_parallel_1_0)
      {
        ...
        mesh::Part& block_hex_20 = eMesh.getMetaData()->declare_part("block_hex_20", mesh::Element);
        /// set cell topology for the part block_hex_20
        mesh::set_cell_topology< shards::Hexahedron<20>  >( block_hex_20 );
        ...
        if (p_size <= 2)
          {
            ...
            const CellTopologyData *const cell_topo_data = mesh::get_cell_topology(block_hex_20);
      ...
      TEST(nodeRegistry, test_serial_hex8_tet4_24_1)
      {
        ...
        mesh::Part& block_tet_4 = eMesh.getMetaData()->declare_part("block_tet_4", mesh::Element);
        /// set cell topology for the part block_tet_4
        mesh::set_cell_topology< shards::Tetrahedron<4>  >( block_tet_4 );
        ...
        if (p_size <= 2)
          {
            ...
            needed_entity_ranks[0] = NeededEntityType(stk::mesh::Face, 1u);
            ...
            if (p_size == 2)
              {
                EXPECT_EQ(nodeIds_onSE_0[0], 25u);
              }
            ...
            if (p_rank == 0)
              {
                ...
                if (p_size == 2)
                  {
                    EXPECT_EQ(nodeIds_onSE_1[0], 19u);
                  }
              }
            //exit(1);
            // end_demo

...
TEST(samba_io, mesh_reader_generated_basic)
{
  Ioss::Region *region = make_region("generated", "3x4x5|sideset:xXyYzZ");
  ...
  EXPECT_EQ(mesh.num_elements(), 60u);
  EXPECT_EQ(mesh.num_nodes(), 120u);

  delete region;
}

#if 0
TEST(samba_io, mesh_reader_generated_coords_fields)
{
  Ioss::Region *region = make_region("generated", "3x4x5|sideset:xXyYzZ");
...
TEST(samba_io, mesh_reader_generated_sidesets)
{
  Ioss::Region *region = make_region("generated", "3x4x5|sideset:xXyYzZ");
  ...
  EXPECT_TRUE(entered);
  EXPECT_TRUE(check_cobound_count(cobound_counter, 22, 24, 8) );
...
TEST(samba_io, mesh_reader_fixture)
{
  samba::io::mesh_reader_fixture fixture(0, "generated", "3x4x5|sideset:xXyYzZ");

  std::vector<samba::entity_block_key> sidesets;
  fixture.m_mesh.get_entity_blocks(samba::entity_rank::face(), sidesets);

  // std::cout << fixture.m_mesh << std::endl;

  EXPECT_EQ(sidesets.size(), 6u);
  ...
  for (size_t i = 0; i < num_sidesets; ++i)
  {
    samba::entity_block_key sideset = sidesets[i];
    samba::mesh::entity_key_vector side_entities;
    fixture.m_mesh.get_entities(sideset & samba::entity_rank::face(), side_entities);
    ...
    {
      samba::entity_key side_face = side_entities[k];
      samba::entity_proxy side_face_proxy = fixture.m_mesh[side_face];
      ...
      size_t num_face_nodes = get_nodes(side_face_proxy, face_nodes);
      EXPECT_EQ(num_face_nodes, 4u);
      EXPECT_TRUE(check_aa_coplanar(face_nodes, fixture.m_node_coords));

      bool cobound_elt_found = update_cobound_count(side_face, fixture.m_mesh, cobound_counter);
      EXPECT_TRUE(cobound_elt_found);

...
#include <performance_tests/calculate_centroid.hpp>
...
struct gather_elem
{
  ...
  void operator()( samba::partition_proxy partition)
  {
    ...
    for(offset=0,end_offset=partition.size(); offset<end_offset; ++offset) {
      ...
      performance_tests::calculate_centroid_3d(num_nodes,&elem_node_coords[0],elem_centroid);

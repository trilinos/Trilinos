...
void disabled_unit_test()
{
  ...
  stk::rebalance::rebalance(bulk_data, selector, &coord_field, NULL, zoltan_partition, stk::mesh::MetaData::NODE_RANK);

  const double imbalance_threshold = stk::rebalance::check_balance(bulk_data, NULL, stk::mesh::MetaData::NODE_RANK, &selector);

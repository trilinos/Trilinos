                       ...
                       std::vector<TilingIndices> &tilesOccupied)
{
  ...
  //  if (decompRank == 36) {
  //    std::cout << "p_" << decompRank << ":  tile dimensions = {" << xTileLen << "," << yTileLen << ","
  //              << zTileLen << "}  box = {(" << boxXMin << "," << boxYMin << "," << boxZMin << ") ("
  //              << boxXMax << "," << boxYMax << "," << boxZMax << ") }" << std::endl
  //              << " discretized box =  {(" << firstXInd << "," << firstYInd << "," << firstZInd << ") ("
  //                  << lastXInd << "," << lastYInd << "," << lastZInd << ") }" << std::endl;
  //    std::cout << "p_" << decompRank << " occupies tiles { ";
  //    for (TilingIndices tile : tilesOccupied) {
  //      std::cout << "{" << tile.xIdx << "," << tile.yIdx << "," << tile.zIdx << "} ";
  //    }
  //    std::cout << std::endl;
  //  }
}
...
void witnessAndComputeNeighborRanks(MPI_Comm mpiComm, int pSize,
                                    ...
                                    const ResidencyMapT &myRangeResiddnts,
    ...
    SplitTimer &stopwatch)
{
  ...
  // Still need this in the spike version.  In the real version, we'll want the "average"
  // decomp rank's diameter, discounting grossly disconnected ranks.
  DomainBox globalDomBox = computeGlobalBoundingBox(boxA_proc.GetBox());

  timeBreakdown.computeProcBoundingVolume += stopwatch.split();

  // In the spike, we will just use a single tile in the tiling.  It's
  // sub-tiling will be used to define the Ghosting Regions
  ...
  // In the spike, we will just raster the AABBs.
  ...
  //  std::cout << "p_" << pRank << ": domain intersects " << myDomainOccupancies.size() << " Ghosting Regions"
  //            << std::endl;
  ...
  // std::cout << "p_" << pRank << ": has " << domainRanksNeighboringMyRange.size() << " neighboring domains"
  //           << std::endl;

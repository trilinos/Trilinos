...
T Hex8::vector_norm(std::array<T, N>& x)
{
  T norm_sq = 0.0;
...
bool Hex8::within_tol(const double& value, const double& tolerance)
{
   return (std::fabs(value) < tolerance);
}

double Hex8::parametric_distance(const std::array<double, 3>& x)
{
...
double Hex8::is_in_element(const double* elem_nodal_coor,  // (8,3)
...
const std::vector<double>& Hex8::coordinate_center()
{
  static const std::vector<double> C(3, 0.);
  return C;
}

void Hex8::interpolate_point(const double* par_coord, // (3)

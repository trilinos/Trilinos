    ...
    : m_processorCount(proc_count), m_myProcessor(my_proc)
{
  ...
  if (!params.empty()) {
    ...
    stk::unit_test_util::TextMeshParser parser(m_dimension);
...
unsigned TextMesh::get_part_id(const stk::unit_test_util::ElementData &elementData) const
{
  return m_data.partIds.get(elementData.partName);
}
...
int64_t TextMesh::element_count_proc(int64_t id) const
{
  ...
  for (const auto &elementData : m_data.elementDataVec) {
    if (get_part_id(elementData) == id && elementData.proc == myProc) {
      count++;
    }
  }
...
stk::topology TextMesh::get_topology_for_part(int64_t id) const
{
  unsigned partId = id;
  ...
  stk::topology topo = iter->second;
  return topo;
}

std::pair<std::string, int> TextMesh::topology_type(int64_t id) const
{
  stk::topology topo = get_topology_for_part(id);
...
int64_t TextMesh::communication_node_count_proc() const
{
  ...
  for (auto id : nodeIds) {
    if (m_data.procs_for_node(id).size() > 1) {
      count++;
    }
  }
  return count;
...
public:
  ...
  void fill_map_from_data(const stk::unit_test_util::TextMeshData& data)
  {
    m_fillIndex = 0;
...
private:
  ...
  void fill_map_for_node(int64_t id, const stk::unit_test_util::TextMeshData& data)
  {
...
unsigned TextMesh::get_node_index(stk::mesh::EntityId id) const
{
  auto iter = m_data.nodeIds.find(id);
  ThrowRequireMsg(iter != m_data.nodeIds.end(), "Could not locate node with ID " << id);
  return std::distance(m_data.nodeIds.begin(), iter);
}

#ifndef TEXTMESHUTILS_HPP_
#define TEXTMESHUTILS_HPP_
 ...
 private:
  void validate_num_coordinates(const Data &data, const std::vector<double> &coordinates)
  {
    if (coordinates.size() != data.nodeIds.size() * data.spatialDim) {
      std::ostringstream errmsg;
      errmsg << "Number of coordinates: " << coordinates.size() << ", Number of nodes: " << data.nodeIds.size()

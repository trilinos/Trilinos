...
#include <stk_unit_test_utils/TextMeshUtils.hpp>
...
class TextMesh
{
 ...
 private:
  ...
  unsigned get_node_index(stk::mesh::EntityId id) const;
  stk::topology get_topology_for_part(int64_t id) const;

  std::set<int64_t> get_local_element_ids_for_block(int64_t id) const;
  unsigned get_part_id(const stk::unit_test_util::ElementData &elementData) const;
  ...
  stk::unit_test_util::TextMeshData m_data;
  stk::unit_test_util::Coordinates m_coordinates;

  std::unordered_map<std::string, stk::topology> m_partToTopology;

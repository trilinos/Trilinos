...
namespace stk {
  namespace percept {
    ...
    void ReferenceMeshSmoother1::get_scale()
    {
      PerceptMesh *eMesh = m_eMesh;
      stk::mesh::FieldBase *cg_g_field    = eMesh->get_field("cg_g");
                ...
                for (unsigned i_node = 0; i_node < num_nodes_in_bucket; i_node++)
                  {
                      ...
                      //if ( m_use_hessian_scaling && spatialDim == 2 )
                    {
                      ...
                      if (d > 1.e-10)
                        {
                          HI = inverse(H);
                          double my_gn[] = { cg_g[0], cg_g[1] };
                          cg_g[0] = HI(0,0)*my_gn[0] + HI(0,1)*my_gn[1];
                          cg_g[1] = HI(1,0)*my_gn[0] + HI(1,1)*my_gn[1];
                        }
                      sum=0.0;
                      for (int idim=0; idim < spatialDim; idim++)
                        {
        ...
        if (!converged)
        ...
        else
          {
            ...
            if (0)
              {
                double an = alpha*1.5;
                //double fa = total_metric( alpha, 1.0, total_valid);
    ...
    double ReferenceMeshSmoother1::total_metric( double alpha, double multiplicative_edge_scaling, bool& valid, int* num_invalid)
    {

  ...
  // compute point
    ...
    cv = cv0 + j0*cv_stride0;
		for ( j1 = 0; j1 < order1; j1++ ) {
			c = N_0[j0]*N_1[j1];
			// Fixing this to work for rationals also, Matt Staten 9/7/2011
			w = is_rat ? cv[dim] : 1.0;
			j = dim;
			while (j--) 
				*P++ += w* c* *cv++;
    ...
    if ( der_count > 1 ) {
      ...
      if ( der_count > 2 ) 
      {
        ...
        // compute higher derivatives in slower generic loop
				...
				// This codes does not appear to work for rationals (is_rat = true)
				// I fixed computation of up to der_count == 2 to work for
				// rationals, but have not for der_count > 2.
				//  -Matt Staten, Sept. 7, 2011

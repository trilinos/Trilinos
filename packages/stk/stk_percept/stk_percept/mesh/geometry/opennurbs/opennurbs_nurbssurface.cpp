...
bool ON_NurbsSurface::GetClosestPoint( 
        const ON_3dPoint& P,
        double* u, double* v,  // parameters of local closest point returned here
        double maximum_distance,
        const ON_Interval* angle_sub_domain, // can be NULL
        const ON_Interval* curve_sub_domain  // can be NULL
        ) const
{
  // They did not provide a guess for u,v.  So loop through the knots attempt
  // to project using the value for the knot.  Do this until we find a guess
  // that converges.
  const int u_dir = 0;
  const int v_dir = 1;
  bool found = false;
  double min_dist = 0.0;
  int knot_count_u = this->KnotCount( u_dir );

  // We scan over the surface trying a grid of guess points.
  for ( int iknot_u = 1; iknot_u < knot_count_u; iknot_u++ )
  {
    double prev_knot_u = this->Knot( u_dir, iknot_u-1 );
    double this_knot_u = this->Knot( u_dir, iknot_u );
    if ( fabs( this_knot_u - prev_knot_u ) < 0.000001 )
      continue;

    double u_guess = ( prev_knot_u + this_knot_u ) * 0.5;

    int knot_count_v = this->KnotCount( v_dir );
    for ( int iknot_v = 1; iknot_v < knot_count_v; iknot_v++ )
    {
      double prev_knot_v = this->Knot( v_dir, iknot_v-1 );
      double this_knot_v = this->Knot( v_dir, iknot_v );
      if ( fabs( this_knot_v - prev_knot_v ) < 0.000001 )
        continue;

      double v_guess = ( prev_knot_v + this_knot_v ) * 0.5;
      double local_u, local_v;

      ON_3dPoint P_projected;
      if ( GetClosestPoint_optimization( P, u_guess, v_guess,
                                         local_u, local_v,
                                         P_projected ) )
      {
        // We converged on a point.  Save it to see if we get a closer one.
        double dist = P_projected.DistanceTo( P );
  ...
  if ( !found )
  {
    // We have not been able to converge on a point.  Try a brute force method.
    // We should only get here with surfaces which are very poorly defined with
    // unstable derivatives.  For example, if you have coincident control
    // points, which cause the derivatives to be unpredictable, making
    // optimization based methods difficult to control.
    ON_3dPoint P_projected;
    double local_u, local_v;
    if ( GetClosestPoint_bruteforce( P, local_u, local_v, P_projected ) )
    {
      found = true;
      *u = local_u;
      *v = local_v;
...
static bool compare_test_pts( TestPts *pt1, TestPts *pt2 )
{
    return (pt1->dist_sqr < pt2->dist_sqr);
}

bool ON_NurbsSurface::GetClosestPoint_bruteforce
(
  const ON_3dPoint& pt_to_project,
  double &u_output,
  double &v_output,
  ON_3dPoint &output_pt,
  int max_iter
) const
{
  ON_Interval intervals[2] = { this->Domain(0),
                               this->Domain(1) };
  double u_tol = intervals[0].Length() * 1e-5;
  double v_tol = intervals[1].Length() * 1e-5;

  const int dim = 50;
  const int num_test_pts = dim*dim;
  TestPts pts[num_test_pts];
  int num_near_pts = dim*2;
  int iter = 0;
  do
  {
    iter++;
    ...
    double deltas[2] = { intervals[0].Length()/(dim-1),
                         intervals[1].Length()/(dim-1) };
    int ipt = 0;
    std::vector<TestPts *> pts_vector(num_test_pts);
    for ( int iu = 0; iu < dim; iu++ )
    {
      double u = intervals[0].Min() + deltas[0]*iu;
      for ( int iv = 0; iv < dim; iv++ )
      {
        double v = intervals[1].Min() + deltas[1]*iv;
        double pt[3];
        Evaluate( u, v, 0, 3, pt );
        double dx = pt[0] - pt_to_project.x;
        double dy = pt[1] - pt_to_project.y;
        double dz = pt[2] - pt_to_project.z;
        pts[ipt].dist_sqr = dx*dx + dy*dy + dz*dz;
        pts[ipt].u = u;
        pts[ipt].v = v;
        pts_vector[ipt] = &(pts[ipt]);
        ipt++;
      }
    }

    std::sort( pts_vector.begin(), pts_vector.end(), compare_test_pts );
    double minu = pts_vector[0]->u;
    double maxu = pts_vector[0]->u;
    double minv = pts_vector[0]->v;
    double maxv = pts_vector[0]->v;
    for ( ipt = 1; ipt < num_near_pts; ipt++ )
    {
      double u = pts_vector[ipt]->u;
      double v = pts_vector[ipt]->v;
      if ( u < minu ) minu = u;
      if ( u > maxu ) maxu = u;
      if ( v < minv ) minv = v;
      if ( v > maxv ) maxv = v;
    ...
    if ( deltau < u_tol && deltav < v_tol )
    {
      u_output = pts_vector[0]->u;
      v_output = pts_vector[0]->v;
      double pt[3];
      Evaluate( u_output, v_output, 0, 3, pt );
      output_pt.Set( pt[0], pt[1], pt[2] );
      return true;
    }

    intervals[0].Set( minu, maxu );
    intervals[1].Set( minv, maxv );
  } while ( true );
}

bool ON_NurbsSurface::GetClosestPoint_optimization
(
  const ON_3dPoint& pt_to_project,
  const double &u_guess,
  const double &v_guess,
  double &u_output,
  double &v_output,
  ON_3dPoint &output_pt,
  int max_iter,
...
) const
{
  ...
  if ( debug )
  {
    printf( "GetClosestPoint_optimization\n" );
    printf( "create vertex %lf %lf %lf #pt_to_project\n",
            pt_to_project.x, pt_to_project.y, pt_to_project.z );
    fflush(stdout);
  }

  int iter = 0;
  
  double cur_param[2] = {u_guess, v_guess};
  ON_Interval intervals[2] = { this->Domain(0),
                               this->Domain(1) };
  int idir;
  for ( idir = 0; idir < 2; idir++ )
  {
    if( cur_param[idir] < intervals[idir].Min() ) cur_param[idir] = intervals[idir].Min();
    if( cur_param[idir] > intervals[idir].Max() ) cur_param[idir] = intervals[idir].Max();
  }

  double values[18];
  const double dist_sqrt_threshold = ((ON_NurbsSurface*)this)->GetProjectionTolSqr();
  do
  {
    iter++;
    if ( max_iter < iter )
    ...
    Evaluate( cur_param[0], cur_param[1], 2, 3, values );
    ON_3dPoint cur_location = ON_3dPoint( values[0], values[1], values[2] );
    ON_3dVector dxyz_du = ON_3dVector( values[3], values[4], values[5] );
    ON_3dVector dxyz_dv = ON_3dVector( values[6], values[7], values[8] );
    ON_3dVector dxyz2_du2 = ON_3dVector( values[9], values[10], values[11] );
    ON_3dVector dxyz2_dudv = ON_3dVector( values[12], values[13], values[14] );
    ON_3dVector dxyz2_dv2 = ON_3dVector( values[15], values[16], values[17] );

    if ( debug )
    {
      printf( "create vertex %lf %lf %lf\n",
              cur_location.x, cur_location.y, cur_location.z );
      fflush(stdout);
    }

    // See how far we are from the point to project
    ON_3dVector cur_offset = cur_location - pt_to_project;
    double dist_sqr = cur_offset.LengthSquared();
    if ( dist_sqr < dist_sqrt_threshold )
    {
      // We converged!  pt_to_project is within a tolerance of this position on the curve.
      u_output = cur_param[0];
      v_output = cur_param[1];
      output_pt = cur_location;
      return true;
    }

    // Do the u Direction first
    bool u_converged = false;
    bool v_converged = false;
    double new_param[2] = { cur_param[0], cur_param[1] };
    {
      double dot = dxyz_du * cur_offset;
      dot /= (dxyz_du.Length() * cur_offset.Length() );
      if ( fabs(dot) <= dot_tolerance )
      {
        u_converged = true;
      }
      else
      {
      //Compute a new location on the curve to try next.
        new_param[0] = cur_param[0] - (dxyz_du*cur_offset)/((dxyz2_du2*cur_offset)+dxyz_du.LengthSquared());
      }
    }

    // Do the v Direction second
    {
      double dot = dxyz_dv * cur_offset;
      dot /= (dxyz_dv.Length() * cur_offset.Length() );
      if ( fabs(dot) <= dot_tolerance )
      {
        v_converged = true;
      }
      ...
      {
      //Compute a new location on the curve to try next.
        new_param[1] = cur_param[1] - (dxyz_dv*cur_offset)/((dxyz2_dv2*cur_offset)+dxyz_dv.LengthSquared());
      }
    }

    if ( u_converged && v_converged )
    {
      // We converged!  The amount we would move is within the tolerance, so quit.
      u_output = cur_param[0];
      v_output = cur_param[1];
      output_pt = cur_location;
      return true;
    ...
    // Make sure we stay in the domain.
    for ( idir = 0; idir < 2; idir++ )
    {
      if( new_param[idir] < intervals[idir].Min() ) new_param[idir] = intervals[idir].Min();
      if( new_param[idir] > intervals[idir].Max() ) new_param[idir] = intervals[idir].Max();
    }

    if ( fabs( new_param[0] - cur_param[0] ) < ON_ZERO_TOLERANCE &&
         fabs( new_param[1] - cur_param[1] ) < ON_ZERO_TOLERANCE )
    {
      // We are not making any progress, and in fact stuck.
      return false;
    }

    //ON_3dVector scaled_dxyz_du = (new_param[0]-cur_param[0])*dxyz_du;
    //ON_3dVector scaled_dxyz_dv = (new_param[1]-cur_param[1])*dxyz_dv;
    //if ( scaled_dxyz_du.LengthSquared() < dist_sqrt_threshold &&
    //     scaled_dxyz_dv.LengthSquared() < dist_sqrt_threshold )
    //{
    //  // We converged!  The amount we would move is within the tolerance, so quit.
    //  u_output = cur_param[0];
    //  v_output = cur_param[1];
    ...
    cur_param[0] = new_param[0];
    cur_param[1] = new_param[1];
  } while ( true );
...
double ON_NurbsSurface::GetProjectionTolSqr( void )
{
  if ( m_proj_tol_sqr < 0 )
  {
    double min[3], max[3];
    ...
    double tol = dist * 1e-8;
    m_proj_tol_sqr = tol * tol;
  }
  return m_proj_tol_sqr;
}

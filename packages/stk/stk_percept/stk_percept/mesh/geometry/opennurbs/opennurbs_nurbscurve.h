...
public:
  ...
  double* m_cv;             // Control points.
                            ...
                            //           [ CV(i)[0], ..., CV(i)[m_dim] ].
                            //
  double m_proj_tol_sqr;    // Tolerance for GetClosestPoint.  The returned
                            // point will be within sqrt(m_proj_tol_sqr) of
                            // the projected point if the projectted point
                            // is already on the curve.

private:
  bool GetClosestPoint_GeneralCase( 
          const ON_3dPoint& P,
          double* t,
  ...
  bool GetClosestPoint_optimization(
          const ON_3dPoint &pt_to_project, // <in>  The point to project
          double initial_guess,            // <in>  A guess parametric location
          double &output_t,                // <out> The projected parametric
                                           //       location
          ON_3dPoint &output_pt,           // <out> The projected xyz location
          int max_iter=50,                 // <in>  The maximum number of
                                           //       iterations before failing
          double dot_tol=0.001             // <in>  The maximum deviation in
                                           //       the dot project of normal
                                           //       and tangent if pt_to_project
                                           //       is off the curve.
          ) const;
  double GetProjectionTolSqr( void );

        ...
        ) const
{
  ...
  return GetClosestPoint_GeneralCase(P,t,maximum_distance,sub_domain);
}

bool ON_NurbsCurve::GetClosestPoint_GeneralCase( 
        const ON_3dPoint& P,
        double* t,
        double maximum_distance,
        const ON_Interval* sub_domain
...
{
  // They did not provide a guess for t.  So loop through the knots attempt
  // to project using the value for the knot.  Do this until we find a guess
  // that converges.
  int knot_count = this->KnotCount();
  bool found = false;
  double min_dist = 0.0;
  for ( int iknot = 1; iknot < knot_count; iknot++ )
  {
    double prev_knot = this->Knot( iknot-1 );
    double this_knot = this->Knot( iknot );
    if ( fabs( this_knot - prev_knot ) < 0.000001 )
      continue;

    double t_guess = ( prev_knot + this_knot ) * 0.5;
    double local_t;

    ON_3dPoint P_projected;
    if ( GetClosestPoint_optimization( P, t_guess, local_t,
                                       P_projected ) )
    {
...
bool ON_NurbsCurve::GetClosestPoint_optimization
(
...
) const
{
  ...
  double cur_t = initial_guess;
  ON_Interval interval = this->Domain();
  if( cur_t < interval.Min() ) cur_t = interval.Min();
  if( cur_t > interval.Max() ) cur_t = interval.Max();

  double values[9];
  const double dist_sqrt_threshold = ((ON_NurbsCurve*)this)->GetProjectionTolSqr();
  do
  {
    iter++;
    if ( max_iter < iter )
    ...
    Evaluate( cur_t, 2, 3, values );
    ON_3dPoint cur_location = ON_3dPoint( values[0], values[1], values[2] );
    ON_3dVector first_deriv = ON_3dVector( values[3], values[4], values[5] );
    ON_3dVector second_deriv = ON_3dVector( values[6], values[7], values[8] );
    ...
    double dot = first_deriv * cur_offset;
    dot /= (first_deriv.Length() * cur_offset.Length() );
    if ( fabs(dot) <= dot_tolerance )
    {
      // We converged!  The pt_to_project is off the curve, but this is the closest point.
      output_t = cur_t;
      output_pt = cur_location;
      return true;
    }
    ...
    double new_t = cur_t - (first_deriv*cur_offset)/((second_deriv*cur_offset)+first_deriv.LengthSquared());
    if( new_t < interval.Min() ) new_t = interval.Min();
    if( new_t > interval.Max() ) new_t = interval.Max();

    //if ( fabs( new_t - cur_t ) < ON_ZERO_TOLERANCE )
    //{
    //  // We are not making any progress, and in fact stuck.
    //  return false;
    //}

    ON_3dVector scaled_1st_deriv = (new_t-cur_t)*first_deriv;
    if ( scaled_1st_deriv.LengthSquared() < dist_sqrt_threshold )
    {
...
double ON_NurbsCurve::GetProjectionTolSqr( void )
{
  if ( m_proj_tol_sqr < 0 )
  {
    ON_3dPoint pt;

    ...
    public:
        ...
        FunctionOperator(bulkData, (mesh::Part*)0), m_is_surface_norm(is_surface_norm), m_turboOpt(turboOpt), m_cubDegree(2)
      {
        ...
        for (int i = 0; i < partNames.dimension(0); i++)
          {
            mesh::Part * part = stk::mesh::MetaData::get(bulkData).get_part(partNames(i));
        ...
        if (1 || typeid(integrand) == typeid(FieldFunction))
          {
            ...
            PerceptMesh eMesh(&mesh::MetaData::get(m_bulkData), &m_bulkData);
            ...
            const stk::mesh::Part& locally_owned_part = mesh::MetaData::get(m_bulkData).locally_owned_part();

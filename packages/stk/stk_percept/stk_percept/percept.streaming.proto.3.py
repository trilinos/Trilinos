### only for python 3.... due to concurrent.futures
#!/usr/netpub/python-3.2/bin/python
...
def run_if_par(cmd_str, i_pass, streaming_W, debug, timeout=60):

    with concurrent.futures.ThreadPoolExecutor(max_workers=streaming_W) as executor:
        print "here 1"
        future_to_iW = dict((executor.submit(run_if_par_0, cmd_str, i_pass, streaming_W, iW, debug), iW)
                         for iW in range(0,streaming_W))
        print "here 2"

        for future in concurrent.futures.as_completed(future_to_iW):
            iW = future_to_iW[future]
            print "iW= " , iW
            if future.exception() is not None:
                print('%r generated an exception: %s' % (iW,
                                                         future.exception()))
            else:
                print('%r page is %d bytes' % (iW, len(future.result())))

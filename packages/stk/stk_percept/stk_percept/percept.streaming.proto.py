...
def setup_parser():
    ...
    streaming.add_argument('-M', '--num-virtual-proc', type=int, default=0, metavar='NM', help='number of virtual processors in streaming mode = M in file.e.M.iM decomposition (default: %(default)s)')
...
def main():
    ...
    if not args:
        ...
        num_virtual_proc = args.num_virtual_proc
        # loop over virtual procs - we expect to see file.e.M.iM in the current directory FIXME
        M = num_virtual_proc
        for iM in range(0,M):
            ...
            for arg in dir(args):
                if (not arg.startswith("_") and arg not in skip_list):
                    cmd_piece = " %s" % convert_to_stk_adapt_option(args, arg)
                    #print "arg = " , arg, " cmd_piece= ", cmd_piece
                    if arg == "infile" or arg == "outfile":
                        cmd_piece += ".%d.%d" % (M, iM)
                    cmd_str += cmd_piece

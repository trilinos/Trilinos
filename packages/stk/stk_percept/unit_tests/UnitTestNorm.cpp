...
#define EXTRA_PRINT 0
      ...
      TEST(norm, volume)
      {
        ...
        for (int iter=0; iter < niter; iter++)
          {
            EXPECT_DOUBLE_EQ_APPROX(1.0, eval(random01(), random01(), random01(), 0.0, sqrt_volume));
          }
...
#define DO_IO_TESTING 1
      ...
      TEST(norm, string_function)
      {
        ...
        for (int iter=0; iter < niter; iter++)
          {
            EXPECT_DOUBLE_EQ_APPROX( sfx_expect, eval(random01(), random01(), random01(), 0.0, sfx_res));
          }

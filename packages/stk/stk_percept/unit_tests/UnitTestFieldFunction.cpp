...
STKUNIT_UNIT_TEST(function, fieldFunction_demo_2)
{
  ...
  eMesh.add_field("coords_mag_field", mesh::MetaData::NODE_RANK, vectorDimension);
  eMesh.commit();
  ...
  stk::mesh::FieldBase* coords_mag_field = eMesh.get_field("coords_mag_field");
...
{
  stk::mesh::FieldBase* pressure_field =  eMesh.get_field("pressure");
  unsigned p_rank = eMesh.get_bulk_data()->parallel_rank();
  unsigned p_size = eMesh.get_bulk_data()->parallel_size();
  (void)p_size;
        ...
        for (unsigned iEntity = 0; iEntity < num_elements_in_bucket; iEntity++)
          {
            ...
            if (bucket.owned())
            ...
            else if (bucket.shared())
              {
                ...
                p[0] = -double((eMesh.owner_rank(entity)+1)*100+id);
                type=Shared;
              }
            else
              {
            ...
            out << "P["<<p_rank<<"] id= " << m_eMesh.identifier(entity) << " p= " << p[0] << " type= " << types[type] << std::endl;
...

  {
    ...
    if (sync_aura) stk::mesh::communicate_field_data(eMesh.get_bulk_data()->shared_aura(), fields);
        ...
        for (unsigned iEntity = 0; iEntity < num_elements_in_bucket; iEntity++)
          {
            ...
            if (bucket.owned())
              {
                STKUNIT_ASSERT_NEAR(p[0], p_e, 1.e-6);
              }
            else if (bucket.shared())
              {
                //p_e = (p_rank?100+id:200+id);
                p_e = ((eMesh.owner_rank(entity)+1)*100+id);
                type = Shared;
                if (sync_shared)
                  {
            ...
            out1 << "P["<<p_rank<<"] after id= " << m_eMesh.identifier(entity) << " p= " << p[0] << " type= " << types[type] << std::endl;
        ...
        for (unsigned iEntity = 0; iEntity < num_elements_in_bucket; iEntity++)
          {
            ...
            if (bucket.owned())
              ...
              {
                p[0] = -((eMesh.owner_rank(entity)+1)*100+id);
                type=Shared;
              }
            else
              {
...
STKUNIT_UNIT_TEST(function, fieldFunction_field_sync)
{
  EXCEPTWATCH;
  ...
  // add a new field
  int vectorDimension = 0;  // signifies a scalar field
  stk::mesh::FieldBase* pressure_field =  eMesh.add_field("pressure", mesh::MetaData::NODE_RANK, vectorDimension);
  PressureFieldType& p_field = fixture.meta_data.declare_field<PressureFieldType>("p");
  stk::mesh::put_field( p_field , mesh::MetaData::NODE_RANK , fixture.meta_data.universal_part());

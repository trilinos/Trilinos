...
{
  std::string filename = stk::unit_test_util::simple_fields::get_option("-i", "ARefLA.e");
  std::string subdir = stk::unit_test_util::simple_fields::get_option("-s", ".");

  bool useAutoFaceTolerance = stk::unit_test_util::simple_fields::get_command_line_option<bool>("-useAutoFaceTolerance", false);
  double toleranceForFaceSearch = stk::unit_test_util::simple_fields::get_command_line_option<double>("-fs", 0.0001);
  double toleranceForParticleSearch  =  stk::unit_test_util::simple_fields::get_command_line_option<double>("-ps",3);
  double edgeWeightForSearch = stk::unit_test_util::simple_fields::get_command_line_option<double>("-ew",15);
  std::string decompMethod = stk::unit_test_util::simple_fields::get_option("-d","parmetis");
  double vertexWeightMultiplierForVertexInSearch = stk::unit_test_util::simple_fields::get_command_line_option<double>("-vwm",5.0);
  bool multiCriteriaFlag = stk::unit_test_util::simple_fields::get_command_line_option<bool>("-mc", false);
  bool doRebalance = stk::unit_test_util::simple_fields::get_command_line_option<bool>("-dorebal", true);
  bool setContactWeightFromTopology = stk::unit_test_util::simple_fields::get_command_line_option<bool>("-setContactWeightFromTopology", false);
  double contactWeight = stk::unit_test_util::simple_fields::get_command_line_option<double>("-contactWeight", 1.0);
  double nonContactWeight = stk::unit_test_util::simple_fields::get_command_line_option<double>("-nonContactWeight", 1.0);
  bool includeSearchResults = stk::unit_test_util::simple_fields::get_command_line_option<bool>("-useSearch", true);

  std::shared_ptr<stk::mesh::BulkData> bulkPtr = build_mesh(MPI_COMM_WORLD);
  stk::mesh::BulkData& bulk = *bulkPtr;
  stk::mesh::MetaData& meta = bulk.mesh_meta_data();
  stk::mesh::Field<double> &nonContactCriteria = meta.declare_field<double>(stk::topology::ELEM_RANK, "non contact criteria");
  stk::mesh::Field<double> &contactCriteria = meta.declare_field<double>(stk::topology::ELEM_RANK, "contact criteria");

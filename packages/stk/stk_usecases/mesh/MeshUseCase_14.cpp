...
#include <mesh/MeshUseCase_14.hpp>
...
//--------------------------------------------------------------------
//
// main driver for use-case 14: element internal force.
//

void use_case_14_driver( MPI_Comm comm , bool performance_test )
{
  ...
  if ( ! performance_test ) {
    ...
    int num_trials = 1;
    use_case_14_driver( comm , "use_case_14.exo" , a_box_size , a_box_sides , num_trials );
  }
  else {
    ...
    use_case_14_driver( comm , "use_case_14a.exo" , a_box_size , a_box_sides , num_trials );
...
//     use_case_14_driver( comm , "use_case_14b.exo" , b_box_size , b_box_sides , num_trials );
  }
}
...
void zero_field_data( stk::mesh::BulkData & mesh , unsigned type , const FieldType & field )
{
  ...
  const unsigned num_trials )
{
...

  {
    ...
    ElementNodePointerField & coord_gather    = declare_element_node_pointer_field( mesh_meta_data , "coord_gather" , coordinates_field );
    ElementNodePointerField & velocity_gather = declare_element_node_pointer_field( mesh_meta_data , "velocity_gather" , velocity_field );
    ElementNodePointerField & fint_gather     = declare_element_node_pointer_field( mesh_meta_data , "fint_gather" , fint_field );
  ...
  for(unsigned n=0; n<num_trials; ++n) {
    ...
    zero_field_data( mesh_bulk_data , stk::mesh::MetaData::NODE_RANK , fint_field );
  ...
  const CartesianField & node_velocity )
{
  ...
  const std::vector<stk::mesh::Bucket*> & buckets = mesh.buckets( stk::mesh::MetaData::NODE_RANK );

  for ( std::vector<stk::mesh::Bucket*>::const_iterator k = buckets.begin() ; k != buckets.end() ; ++k ) {
    stk::mesh::Bucket & bucket = **k ;
    const unsigned length   = bucket.size();

    double * X     = stk::mesh::field_data( node_model_coordinates , bucket.begin() );
  ...
  const unsigned shell_side[][2] )
{
  ...
  if ( 0 == parallel_rank ) {
    std::cout << "On " << parallel_size << " processors, meshed "
              ...
              << entity_count[ stk::mesh::MetaData::NODE_RANK ] << " nodes "
              << " in " << dt << " sec"
              << std::endl ;
    std::cout.flush();
  }

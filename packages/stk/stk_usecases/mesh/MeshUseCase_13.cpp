...
void use_case_13_driver( MPI_Comm comm )
{
...

  {
    ...
    mesh::set_cell_topology(block_hex, hex_top );
    ...
    mesh_meta_data.declare_field_relation(
      elem_node_coord ,
      mesh::get_element_node_stencil(SpatialDim) ,
      coordinates_field );
...

    {
      ...
      std::cout << "  P" << p_rank << ": Uses {"
                << " Node = " << count[ stk::mesh::MetaData::NODE_RANK ]
                          ...
                          const unsigned side_ord )
{
  ...
  const mesh::PairIterRelation side_nodes = side.relations( mesh::MetaData::NODE_RANK );

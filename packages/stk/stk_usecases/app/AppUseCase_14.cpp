                        ...
                        bool performance_test)
{
...

  {
    ...
    {
      stk::mesh::Property<double>& delta_t = meta_data.declare_property<double>("dt");
      stk::mesh::Property<lame::MatProps>& mprops =
        meta_data.declare_property<lame::MatProps>("materialProperties");
	...
	if ( part->primary_entity_rank() == stk::mesh::MetaData::ELEMENT_RANK) {
	  ...
	  if (topology->key == shards::Hexahedron<8>::key) {
	    ...
	    meta_data.put_property( delta_t, *part);
	    double* delta_t_ptr = stk::mesh::property_data(delta_t, *part);
	    *delta_t_ptr = dt;

	    meta_data.put_property(mprops, *part);
    ...
    stk::mesh::get_parts_with_topology<shards::Hexahedron<8> >(bulk_data, hex_parts, skip_topology_root_parts);
    ...
    for(int n=0; n<num_trials; ++n) {
      ...
      fill( *alg_runner, bulk_data , stk::mesh::MetaData::NODE_RANK , *fields.fint_field, 0.0 );

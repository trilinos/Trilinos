  ...
  const unsigned num_trials )
{
...

  {
    ...
    VectorFieldType & coordinates_field =
      stk::mesh::put_field(fem_meta.declare_field< VectorFieldType >( "coordinates" ) , stk::mesh::MetaData::NODE_RANK , universal , SpatialDim );

    VectorFieldType & displacements_field =
      stk::mesh::put_field(fem_meta.declare_field< VectorFieldType >( "displacements" ) , stk::mesh::MetaData::NODE_RANK , universal , SpatialDim );

    VectorFieldType & residual_field =
      stk::mesh::put_field(fem_meta.declare_field< VectorFieldType >( "residual" ) , stk::mesh::MetaData::NODE_RANK , universal , SpatialDim );

    //--------------------------------
    // rotation_field only exists on the shell-nodes:

    VectorFieldType & rotation_field =
      stk::mesh::put_field(fem_meta.declare_field< VectorFieldType >( "rotation" ), stk::mesh::MetaData::NODE_RANK , block_quad_shell , SpatialDim );
    ...
    const std::vector< stk::mesh::Bucket * >& node_buckets = mesh_bulk_data.buckets( stk::mesh::MetaData::NODE_RANK );
    ...
    for ( unsigned i = 0 ; i < num_trials ; ++i ) {
      ...
      double result = dot(*alg_runner , mesh_bulk_data , stk::mesh::MetaData::NODE_RANK , residual_field , displacements_field);

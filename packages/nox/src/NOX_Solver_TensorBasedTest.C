...
bool NOX::Solver::TensorBasedTest::performLinesearch(
...
double NOX::Solver::TensorBasedTest::selectLambda(double newf, double oldf,
						   double oldfprime,
						   double lambda)
{
  double lambdaRet;
					       ...
					       const string& errorMsg) const
{
  if (utils.isPrintProcessAndType(NOX::Utils::Error))
    cerr << "NOX::Solver::TensorBasedTest::" << functionName
	 << " - " << errorMsg << endl;
  throw "NOX Error";
}

...
{
  Teuchos::RCP<CRSM> mat = Teuchos::rcp(new CRSM(map, 3));

  Scalar posOne = ST::one();
  Scalar negOne = -1.0*posOne;
  Scalar two = 2.0*posOne;
  for (std::size_t i = 0; i < map->getNodeNumElements(); ++i) {
    ...
    if (row == 0) {
      mat->insertGlobalValues(row,
                              ...
                              Teuchos::tuple(two, posOne));
    } else if (row == map->getGlobalNumElements()-1) {
      mat->insertGlobalValues(row,
                              Teuchos::tuple(row-1, row),
                              Teuchos::tuple(negOne, two));
    } else {
      mat->insertGlobalValues(row,
                              Teuchos::tuple(row-1, row, row+1),
                              Teuchos::tuple(negOne, two, posOne));
    }
  }
...
TEUCHOS_UNIT_TEST(Tpetra_OpTests, VecNoTrans)
{
  ...
  Scalar beta = -1.0*one;
  op_tpetra->apply(*z_tpetra, *x_tpetra, Teuchos::NO_TRANS, alpha, beta);

  // Apply op through Thyra interface
  Thyra::apply(*op, Thyra::NOTRANS, *z, y.ptr(), alpha, beta);
...
TEUCHOS_UNIT_TEST(Tpetra_OpTests, VecTrans)
{
  ...
  Scalar alpha = static_cast<Scalar>(0.5);
  Scalar beta = -1.0*one;
  op_tpetra->apply(*z_tpetra, *x_tpetra, Teuchos::TRANS, alpha, beta);

  // Apply op through Thyra interface
  Thyra::apply(*op, Thyra::TRANS, *z, y.ptr(), alpha, beta);

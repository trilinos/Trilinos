...
int main(int argc, char *argv[])
{
  ...
  nlParams.sublist("Trust Region").
    setParameter("Inner Iteration Method", "Inexact Trust Region");
  ...
  lsParams.setParameter("Max Age Of Prec", 1);
  ...
  if (solver.getParameterList().sublist("Output").getParameter("Nonlinear Iterations", 0) != 17)
    status = 3;
  ...
  if (solver.getParameterList().sublist("Trust Region").sublist("Output").getParameter("Number of Cauchy Steps", 0) != 3)
    status = 5;
  // 6. Number of Newton steps (14)
  if (solver.getParameterList().sublist("Trust Region").sublist("Output").getParameter("Number of Newton Steps", 0) != 14)
    status = 6;

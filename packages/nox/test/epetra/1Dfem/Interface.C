...
bool Interface::computeF(const Epetra_Vector& x, 
				    Epetra_Vector& FVec, 
				    NOX::Epetra::Interface::FillType fillType)
{
					   ...
					   Epetra_Operator& Jac)
{
  ...
  if (Jacobian == NULL) {
    cout << "ERROR: Interface::computeJacobian() - The supplied"
	 << "Epetra_Operator is NOT an Epetra_RowMatrix!" << endl;
    throw "Interface Error";
  }
  return evaluate(NOX::Epetra::Interface::Jacobian, &x, 0, Jacobian);
}
					     ...
					     Epetra_RowMatrix& M)
{
  ...
  if (precMatrix == NULL) {
    cout << "ERROR: Interface::computePreconditioner() - The "
	 << "supplied Epetra_Operator is NOT an Epetra_RowMatrix!" << endl;
    throw "Interface Error";
  }
  return evaluate(NOX::Epetra::Interface::Preconditioner, &x, 0, precMatrix);
}
...
bool Interface::evaluate(NOX::Epetra::Interface::FillType flag, 
				    ...
				    Epetra_RowMatrix* tmp_matrix)
{
  ...
  if (tmp_matrix != 0) {
    fillMatrix = true;
    Jacobian = dynamic_cast<Epetra_CrsMatrix*> (tmp_matrix);
  }
  ...
  if (flag == NOX::Epetra::Interface::F) {
    // Do nothing for now
  }
  else if (flag == NOX::Epetra::Interface::Jacobian) {
    // Do nothing for now
  }
  else if (flag == NOX::Epetra::Interface::Preconditioner) {
    // Do nothing for now
  }
  else if (flag == NOX::Epetra::Interface::MatrixFreeF) {
    // Do nothing for now
  }
  else if (flag == NOX::Epetra::Interface::FiniteDifferenceF) {
    // Do nothing for now
  }

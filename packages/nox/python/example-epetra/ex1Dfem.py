...
def main():
    ...
    #if options.verbosity > 2: print "Constructing nlParams ParameterList"
    #nlParams = Teuchos.ParameterList()
    #nlParams.set("Nonlinear Solver","Line Search Based")
    #if options.verbosity > 2: print "Constructing sublists"
    #printParams  = nlParams.sublist("Printing")
    #lineParams   = nlParams.sublist("Line Search")
    #dirParams    = nlParams.sublist("Direction")
    #newtonParams = nlParams.sublist("Newton")
    #lsParams     = nlParams.sublist("Linear Solver")
    #solverParams = nlParams.sublist("Solver Options")
    ...
    #if options.verbosity > 2: print "Setting sublist parameters"
    #printParams.set( "MyPID"                 , myPID                  )
    #printParams.set( "Output Precision"      , 3                      )
    #printParams.set( "Output Processor"      , 0                      )
    #printParams.set( "Output Information"    , outputInfo             )
    printParams["Output Information"] = outputInfo
    #lineParams.set(  "Method"                , "Full Step"            )
    ...
    #solverParams.set("Status Test Check Type", NOX.StatusTest.Complete)
    #printing = NOX.Utils(printParams)
    #ppo = UserPrePostOperator(printing)
    #nlParams["Solver Options"]["User Defined Pre/Post Operator"] = ppo
    ...
    if options.verbosity > 2: print "Converting nlParams from dict to ParameterList"
    nlParams = Teuchos.ParameterList(nlParams)
    if options.verbosity > 2: print repr(nlParams)

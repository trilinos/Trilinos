...
int main(int argc, char *argv[])
{
  ...
  // The initial condition is also established in the constructor.
  FiniteElementProblem Problem(NumGlobalElements, Comm);

  // Get the initialized solution vector from the Problem
  Epetra_Vector& soln = Problem.getSolution();

  // Initialize old solution to initial solution
  Problem.reset(soln);
  //soln.PutScalar(0.1);
  ...
  while(timeStep < maxTimeSteps) {
    ...
    for (i=0; i<NumMyElements; i++)
      fprintf(ifp, "%d  %E  %E  %E\n", soln.Map().MinMyGID()+i, 
                                   Problem.getMesh()[i], finalSolution[i],
                                   exactSolution[i]);
    fclose(ifp);

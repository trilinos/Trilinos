...
Equation_B::Equation_B(int numGlobalNodes, Epetra_Comm& comm) :
...
bool Equation_B::evaluate(FillType f, 
                                    ...
                                    NOX::Epetra::Interface::FillType fillFlag)
{
  ...
  if (flag == F_ONLY) {
    ...
    cout << "ERROR: Equation_B::fillMatrix() - FillType flag is broken" << endl;
    throw;
  }
	    ...
	    (*rhs)[StandardMap->LID(OverlapMap->GID(ne+i))]+=
	      ...
	      *((basis.uu - basis.uuold)/dt * basis.phi[i] 
	      +(1.0/(basis.dx*basis.dx))*Dcoeff*basis.duu*basis.dphide[i]
              + basis.phi[i] * ( -beta*basis.aaux
                 + basis.aaux*basis.aaux*basis.uu) );
	  }
	}
	    ...
	    if (StandardMap->MyGID(row)) {
	      ...
	      jac=basis.wt*basis.dx*(
                      ...
                      +(1.0/(basis.dx*basis.dx))*Dcoeff*basis.dphide[j]*
                                                        basis.dphide[i]
                      + basis.phi[i] * basis.aaux*basis.aaux*basis.phi[j] );

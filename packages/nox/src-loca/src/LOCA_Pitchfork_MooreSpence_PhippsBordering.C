...
{
  borderedSolver = 
    globalData->locaFactory->createBorderedSystemStrategy(topParams,
							  solverParams);
}
	 ...
	 const Teuchos::RefCountPtr<const NOX::Abstract::Vector>& dJndp_)
{
  ...
  borderedSolver->setMatrixBlocksMultiVecConstraint(group, JnMultiVector, 
						    nullMultiVector, 
						    Teuchos::null);
	   ...
	   bool isContiguous) const
{
  ...
  if (isContiguous) {
    ...
    // Expand input, result by 2 columns to store solves of psi and (Jv)_x v

...
#include "NOX_BLAS_Group.H"	// class definition
#include "blas.h"
#include "lapack.h"
  ...
  problemInterface(source.problemInterface)	
{
  ...
  default:
    cerr << "NOX:BLAS::Group - invalid CopyType for copy constructor." << endl;
    throw "NOX BLAS Error";
  }
		     ...
		     double step) 
{
  ...
  const Group& blasgrp = dynamic_cast<const Group&> (grp);
  const Vector& blasd = dynamic_cast<const Vector&> (d);
  return computeX(blasgrp, blasd, step); 
}
...
bool Group::computeGradient() 
{
  ...
  if (!isF()) {
    cerr << "ERROR: NOX::BLAS::Group::computeGrad() - F is out of date wrt X!" << endl;
    return false;
  }

  if (!isJacobian()) {
    cerr << "ERROR: NOX::BLAS::Group::computeGrad() - Jacobian is out of date wrt X!" << endl;
    return false;
  }
...
{
  const Vector& blasinput = dynamic_cast<const Vector&> (input);
  Vector& blasresult = dynamic_cast<Vector&> (result);
  return applyJacobian(blasinput, blasresult);
}
...
bool Group::applyJacobianTranspose(const Abstract::Vector& input, Abstract::Vector& result) const
{
  const Vector& blasinput = dynamic_cast<const Vector&> (input);
  Vector& blasresult = dynamic_cast<Vector&> (result);
  return applyJacobianTranspose(blasinput, blasresult);
}
...
bool Group::applyJacobianInverse(NOX::Parameter::List& p, 
 ...
 const Vector& blasinput = dynamic_cast<const Vector&> (input);
 Vector& blasresult = dynamic_cast<Vector&> (result); 
 return applyJacobianInverse(p, blasinput, blasresult);
}
				 ...
				 const Vector& input, Vector& result) const 
{
  ...
  if (!isF()) {
    cerr << "ERROR: NOX::BLAS::Group::applyJacobianInverse() - invalid F" << endl;
    throw "NOX Error";
  }

  if (!isJacobian()) {
    cerr << "ERROR: NOX::BLAS::Group::applyJacobianInverse() - invalid Jacobian" << endl;
    throw "NOX Error";
  }

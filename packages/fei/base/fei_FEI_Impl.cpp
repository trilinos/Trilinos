...
int fei::FEI_Impl::initCoefAccessPattern(int patternID,
					  int numRowIDs,
					  ...
					  int interleaveStrategy)
{
  //define two patterns, patternID+1000 and patternID+2000 which will
  //describe the shape of these contributions.

  int rowPatternID = patternID+1000;
  int colPatternID = patternID+2000;

  int num = numRowIDs;
  if (num < numColIDsPerRow) {
    num = numColIDsPerRow;
  }

  std::vector<int> rowfields;
  rowfields.reserve(4000);
  std::vector<int> idTypes(num, nodeIDType_);

  int i, j;
  for(i=0; i<numRowIDs; ++i) {
    for(j=0; j<numFieldsPerRow[i]; ++j) {
      rowfields.push_back(rowFieldIDs[i][j]);
    }
  }

  std::vector<int> colfields;
  colfields.reserve(4000);
  for(i=0; i<numColIDsPerRow; ++i) {
    for(j=0; j<numFieldsPerCol[i]; ++j) {
      colfields.push_back(colFieldIDs[i][j]);
    }
  }

  matGraph_->definePattern(rowPatternID,
			   numRowIDs,
			   &idTypes[0],
			   numFieldsPerRow, &rowfields[0]);

  matGraph_->definePattern(colPatternID,
			   numColIDsPerRow,
			   &idTypes[0],
			   numFieldsPerCol, &colfields[0]);

  return(0);
}

int fei::FEI_Impl::initCoefAccess(int patternID,
				   ...
				   const GlobalID* colIDs)
{
  ...
  CHK_ERR( matGraph_->initConnectivity(patternID+1000,
				       rowIDs,
				       patternID+2000,
				       colIDs) );
...
int fei::FEI_Impl::sumIntoMatrix(int patternID,
		     const int* rowIDTypes,
                     const GlobalID* rowIDs,
		     const int* colIDTypes,
                     const GlobalID* colIDs,
                     const double* const* matrixEntries)
{
  ...
  int numRowIndices = 0, numColIndices = 0;
...
int fei::FEI_Impl::sumIntoRHS(int patternID,
		  const int* IDTypes,
                  const GlobalID* IDs,
...
int fei::FEI_Impl::putIntoMatrix(int patternID,
		     const int* rowIDTypes,
                     const GlobalID* rowIDs,
		     const int* colIDTypes,
                     const GlobalID* colIDs,
...
int fei::FEI_Impl::putIntoRHS(int patternID,
		  const int* IDTypes,
                  const GlobalID* IDs,
                  const double* coefficients)
{
...
int fei::FEI_Impl::getFromMatrix(int patternID,
				  const int* rowIDTypes,
				  const GlobalID* rowIDs,
				  const int* colIDTypes,
				  const GlobalID* colIDs,
...
int fei::FEI_Impl::getFromRHS(int patternID,
			       const int* IDTypes,
			       const GlobalID* IDs,
			       double* coefficients)
{

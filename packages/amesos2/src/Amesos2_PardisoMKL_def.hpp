...
#   include <mkl_pardiso.h>
  ...
  {
    // We set the parameter if in the list, otherwise leave it alone

    iparm_[1]  = parameterList->get<int_t>("IPARM(2)" , iparm_[1]);
    iparm_[3]  = parameterList->get<int_t>("IPARM(4)" , iparm_[3]);
    iparm_[7]  = parameterList->get<int_t>("IPARM(8)" , iparm_[7]);
    iparm_[9]  = parameterList->get<int_t>("IPARM(10)", iparm_[9]);
    iparm_[17] = parameterList->get<int_t>("IPARM(18)", iparm_[17]);
    iparm_[23] = parameterList->get<int_t>("IPARM(24)", iparm_[23]);
    iparm_[24] = parameterList->get<int_t>("IPARM(25)", iparm_[24]);
    iparm_[59] = parameterList->get<int_t>("IPARM(60)", iparm_[59]);
  }
  ...
  PardisoMKL<Matrix,Vector>::getValidParameters_impl() const
  {
    ...
    if( is_null(valid_params) ){
      ...
      void *pt_dummy[64];
      PMKL::_INTEGER_t mtype_temp;
      PMKL::_INTEGER_t iparm_temp[64];
      PMKL::pardisoinit(pt_dummy,
			const_cast<PMKL::_INTEGER_t*>(&mtype_temp),
	                const_cast<PMKL::_INTEGER_t*>(iparm_temp));

      pl->set("IPARM(2)" , iparm_temp[1] , "Fill-in reducing ordering for the input matrix");
      pl->set("IPARM(4)" , iparm_temp[3] , "Preconditioned CGS/CG");
      pl->set("IPARM(8)" , iparm_temp[8] , "Iterative refinement step");
      pl->set("IPARM(10)", iparm_temp[9] , "Pivoting perturbation");
      pl->set("IPARM(18)", iparm_temp[17], "Report the number of non-zero elements in the factors");
      pl->set("IPARM(24)", iparm_temp[23], "Parallel factorization control");
      pl->set("IPARM(25)", iparm_temp[24], "Parallel forward/backward solve control");
      pl->set("IPARM(60)", iparm_temp[59], "PARDISO mode (OOC mode)");

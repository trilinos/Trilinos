...
void BlockedMultiVector<Scalar, LocalOrdinal, GlobalOrdinal, Node>::
  ...
  typedef typename tpetra_blockedmultivector_type::dot_type tpetra_dot_type;
  Teuchos::Array<tpetra_dot_type> tpetraDots(dots.size());
  vec_->dot(*BlockedMultiVectorDetails::unwrapMultiVector(Teuchos::rcpFromRef(A)), tpetraDots());
  for (typename Teuchos::ArrayView<Scalar>::size_type i = 0; i < dots.size(); ++i)
    dots[i] = static_cast<Scalar>(tpetraDots[i]);
}
...
void BlockedMultiVector<Scalar, LocalOrdinal, GlobalOrdinal, Node>::
  ...
  typedef typename tpetra_blockedmultivector_type::impl_scalar_type tpetra_impl_scalar_type;
  Teuchos::Array<tpetra_impl_scalar_type> tpetraMeans(means.size());
  vec_->meanValue(tpetraMeans());
  for (typename Teuchos::ArrayView<Scalar>::size_type i = 0; i < means.size(); ++i)
    means[i] = static_cast<Scalar>(tpetraMeans[i]);
}

...
namespace Xpetra {
  ...
  public:
    ...
    void fillComplete(const RCP< const Map< LocalOrdinal, GlobalOrdinal, Node > > &domainMap, const RCP< const Map< LocalOrdinal, GlobalOrdinal, Node > > &rangeMap, OptimizeOption os=DoOptimizeStorage) { XPETRA_MONITOR("EpetraCrsMatrix::fillComplete"); mtx_->FillComplete(toEpetra(domainMap), toEpetra(rangeMap), toEpetra(os)); }

    //! Signal that data entry is complete.
    void fillComplete(OptimizeOption os=DoOptimizeStorage) { XPETRA_MONITOR("EpetraCrsMatrix::fillComplete"); mtx_->FillComplete(toEpetra(os)); }

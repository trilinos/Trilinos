...
template <class LocalOrdinal, class GlobalOrdinal, class Node>
void setNestedTpetraBlockedMaps(
    const Teuchos::RCP<Tpetra::BlockedMap<LocalOrdinal, GlobalOrdinal, Node>>& tpetraMap,
    const std::vector<Teuchos::RCP<const Xpetra::Map<LocalOrdinal, GlobalOrdinal, Node>>>& maps) {
  for (size_t i = 0; i < maps.size(); ++i) {
    Teuchos::RCP<const Xpetra::BlockedMap<LocalOrdinal, GlobalOrdinal, Node>> nested =
        Teuchos::rcp_dynamic_cast<const Xpetra::BlockedMap<LocalOrdinal, GlobalOrdinal, Node>>(maps[i]);
    if (!nested.is_null())
      tpetraMap->setBlockedSubMap(i, nested->getTpetra_BlockedMap());
  }
}
}  // namespace BlockedMapDetails
...
BlockedMap<LocalOrdinal, GlobalOrdinal, Node>::
  ...
  Teuchos::RCP<tpetra_blockedmap_type> tpetraMap =
      Teuchos::rcp(new tpetra_blockedmap_type(toTpetra(fullmap),
  ...
  if (bThyraMode)
    ...
    mapsXpetra_ = maps;
    // In Xpetra mode, the caller-provided full map is the real full map.  In
    // Thyra mode it is only used to select the underlying linear algebra
    // library/index base; the Tpetra BlockedMap builds the real full map from
    // the Thyra sub-maps, so leave fullmapXpetra_ empty and wrap that lazily.
...
BlockedMap<LocalOrdinal, GlobalOrdinal, Node>::
  ...
  fullmapXpetra_   = input.fullmapXpetra_;
  mapsXpetra_      = input.mapsXpetra_;
  thyraMapsXpetra_ = input.thyraMapsXpetra_;
}

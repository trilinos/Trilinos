...
void
CDFEM_Options_Parser::parse(const YAML::Node & region_node, RegionInterface & region)
{
  const YAML::Node cdfem_node = YAML_Parser::get_map_if_present(region_node, "cdfem_options");
  if ( cdfem_node )
  {
    ...
    if (YAML_Parser::get_if_present(cdfem_node, "cdfem_edge_degeneracy_handling", cdfem_edge_degeneracy_handling_string))
    {
      ...
      {
        stk::RuntimeDoomedAdHoc() << "Invalid cdfem_edge_degeneracy_handling type: " << YAML_Parser::info(cdfem_node);
      }
      else
      {
    ...
    if (YAML_Parser::get_if_present(cdfem_node, "cdfem_edge_tolerance", cdfem_edge_tol))
    {
    ...
    if (YAML_Parser::get_if_present(cdfem_node, "cdfem_simplex_generation_method", cdfem_simplex_generation_method_string))
    {
      ...
      if (it == valid_entries.end())
      {
        stk::RuntimeDoomedAdHoc() << "Invalid cdfem_simplex_generation_method type: " << YAML_Parser::info(cdfem_node);
      }
      else
      {
    ...
    if (YAML_Parser::get_if_present(cdfem_node, "number_of_initial_decomposition_cycles", num_init_decomp_cycles))
    {
    ...
    if(YAML_Parser::get_if_present(cdfem_node, "cdfem_interface_minimum_refinement_level", interface_minimum_refinement_level))
    {
      interface_refinement_specified = true;
    }

    int interface_maximum_refinement_level = 0;
    if(YAML_Parser::get_if_present(cdfem_node, "cdfem_interface_maximum_refinement_level", interface_maximum_refinement_level))
    {
      interface_refinement_specified = true;
    }
    ...
    if (YAML_Parser::get_if_present(cdfem_node, "cdfem_nonconformal_adaptivity_levels", nonconformal_adapt_levels))
    {
      cdfem_support.activate_nonconformal_adaptivity( nonconformal_adapt_levels );
    }

    int post_adapt_refine_levels = 0;
    if (YAML_Parser::get_if_present(cdfem_node, "post_adaptivity_uniform_refinement_levels", post_adapt_refine_levels))
    {
    ...
    if (YAML_Parser::get_if_present(cdfem_node, "nonconformal_adaptivity_target_element_count", nonconformal_adapt_target_element_count))
    {
    ...
    if (YAML_Parser::get_if_present(cdfem_node, "post_cdfem_refinement_levels", post_cdfem_refinement_levels))
    {
      cdfem_support.set_post_cdfem_refinement_levels( post_cdfem_refinement_levels );
    }

    const YAML::Node post_cdfem_refinement_blocks_seq = YAML_Parser::get_sequence_if_present(cdfem_node, "post_cdfem_refinement_blocks");

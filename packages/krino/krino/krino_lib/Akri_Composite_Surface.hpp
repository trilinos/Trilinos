...
public:
  ...
  virtual void add( Surface * surf ) { my_subsurfaces.emplace_back(surf); }
  ...
  virtual void set_transformation(Transformation * trans) { Surface::set_transformation(trans); for (auto && subsurf : my_subsurfaces) subsurf->set_transformation(trans);}
  ...
  double point_signed_distance(const Vector3d &x, const double narrow_band) const
  {
    ...
    return point_signed_distance(x, narrow_band, get_signed_narrow_band_size(narrow_band));
  }
  virtual double point_signed_distance(const Vector3d &x, const double truncation_length, const double far_field_value) const override;

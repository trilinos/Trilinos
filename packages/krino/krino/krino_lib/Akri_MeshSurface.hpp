...
protected:
  ...
  void add_facet2d(Vector3d & p0, Vector3d & p1);
  void add_facet3d(Vector3d & p0, Vector3d & p1, Vector3d & p2);
...
private:
  virtual void read_file(const std::vector<BoundingBox> & proc_bboxes) override;
  void read_points(std::vector<Vector3d> & points);
  void read_facets(const int batch_size, const int num_facets, const std::vector<Vector3d> & points);
...
private:
  ...
  void read_points(const int num_points, std::vector<Vector3d> & points);
  void read_facets(const int batch_size, const std::vector<Vector3d> & points);

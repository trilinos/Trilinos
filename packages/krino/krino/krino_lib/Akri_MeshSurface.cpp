...
void FACSurface::read_points(std::vector<Vector3d> & points)
{
  std::ifstream & input = my_reader.input();
  int num_points;
  input >> num_points;
...
void FACSurface::read_facets(const int batch_size, const int num_facets, const std::vector<Vector3d> & points)
{
  std::ifstream & input = my_reader.input();
...
void PLYSurface::read_points(const int num_points, std::vector<Vector3d> & points)
{
  std::ifstream & input = my_reader.input();
  points.clear();
  points.reserve(num_points);
...
void PLYSurface::read_facets(const int batch_size, const std::vector<Vector3d> & points)
{
  std::ifstream & input = my_reader.input();
...
static std::unique_ptr<Facet> build_facet(const std::array<Vector3d,3> & pts, const int sign)
{
  unsigned p0 = 0;
  unsigned p1 = 1;
  unsigned p2 = 2;
...
static bool is_finite_normal(const Vector3d normal)
{
...
static std::unique_ptr<Facet> read_ascii_facet(std::ifstream & input, const Vector3d & scale, const int sign)
{
...
static Vector3d read_binary_vector(std::ifstream& input)
{
...
static std::unique_ptr<Facet> read_binary_facet(std::ifstream & input, const Vector3d & scale, const int sign)
{
  const Vector3d normal = read_binary_vector(input);

  std::array<Vector3d,3> pts;
  for (auto && pt : pts)
  {
    const Vector3d vec = read_binary_vector(input);
    pt = Vector3d(vec[0]*scale[0], vec[1]*scale[1], vec[2]*scale[2]);
  }
...
BoundingBox STLSurface::get_ascii_facet_bounding_box()
{
  ...
  while (!done)
  {
    std::unique_ptr<Facet> facet = read_ascii_facet(input, my_scale, my_dist_sign);
    if (facet)
      bbox.accommodate(facet->bounding_box());
      ...
      if ( 0 == stk::EnvData::parallel_rank() )
      {
        ...
        for ( int e = 0; e < current_batch_size; e++ )
        {
          for ( int j = 0; j < nodes_per_elem; j++ )
          {
            ...
            pt[j] = Vector3d( nodeX*my_scale[0], nodeY*my_scale[1], nodeZ*my_scale[2] );
          }
...
MeshSurface::add_facet3d(const stk::mesh::BulkData& mesh, stk::mesh::Entity side, unsigned p0, unsigned p1, unsigned p2)
{
  ...
  pt[0] = Vector3d(stk::mesh::field_data(my_coord_ref, side_nodes[p0]));
  pt[1] = Vector3d(stk::mesh::field_data(my_coord_ref, side_nodes[p1]));
  pt[2] = Vector3d(stk::mesh::field_data(my_coord_ref, side_nodes[p2]));
...
MeshSurface::add_quad3d(const stk::mesh::BulkData& mesh, stk::mesh::Entity side, unsigned p0, unsigned p1, unsigned p2, unsigned p3)
{
  ...
  pt[1] = Vector3d(stk::mesh::field_data(my_coord_ref, side_nodes[p1]));
  pt[2] = Vector3d(stk::mesh::field_data(my_coord_ref, side_nodes[p2]));
  pt[3] = Vector3d(stk::mesh::field_data(my_coord_ref, side_nodes[p3]));

...
bool
find_intersection_of_three_planes(const Plane3d & plane0, const Plane3d & plane1, const Plane3d & plane2, Vector3d & point)
{
  const Vector3d & normal0 = plane0.normal();
  const Vector3d & normal1 = plane1.normal();
  const Vector3d & normal2 = plane2.normal();
...
Vector3d
triangle_parametric_coordinates_of_projected_point(const std::array<Vector3d,3> & triCoords, const Vector3d & pt)
{
  const Vector3d v1 = triCoords[1] - triCoords[0];
  const Vector3d v2 = triCoords[2] - triCoords[0];
  const Vector3d normal = Cross(v1,v2);
  const double invNormalMag2 = 1./normal.length_squared();
  const Vector3d diff = pt - triCoords[0];
  Vector3d paramPt;
...
static bool within_tet_bounds(const Vector3d & pt)
{
...
bool find_intersection_of_three_planes_within_tet(const Plane3d & plane0, const Plane3d & plane1, const Plane3d & plane2, Vector3d & intersectionPoint)
{
  return find_intersection_of_three_planes(plane0, plane1, plane2, intersectionPoint) && within_tet_bounds(intersectionPoint);
}

static bool
find_intersection_of_two_planes_and_coordinate_plane(const int coordinatePlane, const Plane3d & plane0, const Plane3d & plane1, Vector3d & point)
{
  const Vector3d & normal0 = plane0.normal();
  const Vector3d & normal1 = plane1.normal();
...
bool find_intersection_of_two_planes_and_side_of_tet(const int side, const Plane3d & plane0, const Plane3d & plane1, Vector3d & intersectionPoint)
{
  ...
  if (side == 1)
  {
    const Plane3d side1Plane{Vector3d{1.,0.,0.},Vector3d{0.,0.,1.},Vector3d{0.,1.,0.}};

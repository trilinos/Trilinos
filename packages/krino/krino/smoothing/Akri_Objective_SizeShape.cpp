    ...
    std::vector<double> & elemGradContrib) const
{
  ...
  if (spatialDim == 2)
  {
    const auto forces = compute_tri3_2d_element_pseudo_forces(refSize, elemNodeCoords);
  ...
  {
    const auto forces = compute_tet4_element_pseudo_forces(refSize, elemNodeCoords);
    elemGradContrib.resize(npe*spatialDim);
...
template std::array<stk::math::Vector3d, 4> SizeShapeObjective::compute_tet4_element_pseudo_forces(const double refSize, const std::array<stk::math::Vector3d, 4> & current_coords);
template std::array<stk::math::Vector3d, 4> SizeShapeObjective::compute_tet4_element_pseudo_forces(const double refSize, const CONSTPTRCONSTPTR & current_coords);

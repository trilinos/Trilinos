...
ordinal_type
Stokhos::GMRESDivisionExpansionStrategy<ordinal_type,value_type,node_type>::
GMRES(const Teuchos::SerialDenseMatrix<ordinal_type, value_type> & aA, 
      ...
      ordinal_type adiag)
{
  ...
  Teuchos::SerialDenseMatrix<ordinal_type, value_type> r0(Teuchos::Copy,aB);
  r0-=Ax;
  ...
  while (resid > tolerance && k < max_iter){
    ...
    if (aPrecNum == 1){
      ...
      precond.ApplyInverse(vk,z,aprec_iter);
    }
    else if (aPrecNum == 2){
      ...
      Stokhos::GSPreconditioner<ordinal_type, value_type> precond(aM,1);
      precond.ApplyInverse(vk,z,1);
    }
    else if (aPrecNum == 4){
      Stokhos::SchurPreconditioner<ordinal_type, value_type> precond(aM, order, dim, adiag);
      precond.ApplyInverse(vk,z,aprec_iter);
    }

    w.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS, 1, aA, z, 0.0);
  ...
  Teuchos::SerialDenseMatrix<ordinal_type, value_type> ans(aX);
  V.reshape(n,k-1);
  ans.multiply(Teuchos::NO_TRANS, Teuchos::NO_TRANS, 1.0, V, bb, 0.0);
  if (aPrecNum == 1){
    Stokhos::DiagPreconditioner<ordinal_type, value_type> precond(aM);
    precond.ApplyInverse(ans,ans,aprec_iter);
  }
  else if (aPrecNum == 2){
    Stokhos::JacobiPreconditioner<ordinal_type, value_type> precond(aM);

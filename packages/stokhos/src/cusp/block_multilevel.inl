...
::solve(const Array1& b, Array2& x, Monitor& monitor)
{
    ...
    cusp::array2d<ValueType,MemorySpace> update(n, numRHS);
    cusp::array2d<ValueType,MemorySpace> residual(n, numRHS);
...
::_solve(const Array1& b, Array2& x, const size_t i)
{
    ...
    if (i + 1 == levels.size())
    {
        // coarse grid solve
        // copy to host first
	cusp::array2d<ValueType,cusp::host_memory> temp_b(b);
	cusp::array2d<ValueType,cusp::host_memory> temp_x(x.num_rows, numRHS);

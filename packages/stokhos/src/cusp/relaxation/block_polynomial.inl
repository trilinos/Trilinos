...
void block_chebyshev_polynomial_coefficients( const ValueType rho,
...
block_polynomial<ValueType,MemorySpace>
::block_polynomial()
{
}
...
block_polynomial<ValueType,MemorySpace>
::block_polynomial(const block_polynomial<ValueType,MemorySpace2>& A) : default_coefficients(A.default_coefficients), residual(A.residual), h(A.h), y(A.y)
{
}
...
void block_polynomial<ValueType,MemorySpace>
::operator()(const MatrixType& A, const VectorType1& b, VectorType2& x) const
{
    CUSP_PROFILE_SCOPED();

    block_polynomial<ValueType,MemorySpace>::operator()(A,b,x,default_coefficients);
}
...
::operator()(const MatrixType& A, const VectorType1& b, VectorType2& x, VectorType3& coefficients)
{
    ...
    else
    {
        // compute residual <- b - A*x
        cusp::MVmultiply(A, x, residual);
        cusp::blas::axpby(b.values, residual.values, residual.values, ValueType(1), ValueType(-1));
    }

    ValueType scale_factor = coefficients[0];
    cusp::blas::axpby(residual.values, h.values, h.values, scale_factor, ValueType(0));
    ...
    for( size_t i = 1; i<coefficients.size(); i++ )
    {
        ...
        cusp::MVmultiply(A, h, y);
        cusp::blas::axpby(y.values, residual.values, h.values, ValueType(1.0), scale_factor);
    }

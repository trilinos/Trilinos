...
template <typename IndexType,
          typename ValueType>
__global__ void
axpby_kernel(const ValueType a, const ValueType b, const IndexType num_rows,
                        const IndexType num_cols,
                       const ValueType * x,
			const ValueType * y, 
                             ValueType * z)
{
    for (IndexType row = threadIdx.y + blockDim.y * blockIdx.x; row < num_rows; row += gridDim.x * blockDim.y) {
...
row_spmm_dense_diag_kernel(const IndexType Anum_rows, 
			const IndexType Anum_cols,
                       ...
                       const ValueType * x,
                             ValueType * y)
{
    for (IndexType row = threadIdx.y + blockDim.y * blockIdx.x; row < Anum_rows; row += gridDim.x * blockDim.y) {
                             ...
                             ValueType * temp)
{
    ...
    for (IndexType row = threadIdx.y + blockDim.y * blockIdx.x; row < Anum_rows; row += gridDim.x * blockDim.y) {
	for (IndexType col = threadIdx.x; col < Anum_cols; col += blockDim.x){
		sdata[col + Anum_cols*threadIdx.y] +=  Aval[col + Anum_cols * row] * x[col + Anum_cols * row];
	}
    }
...
template <typename IndexType,
          typename ValueType>
__global__ void
reduce_kernel(const IndexType num_rows,
                        const IndexType num_cols,
                    ...
                    Vector3& y, cusp::row_major)
{
	...
	while (num_blocks > 0){
		...
		reduce_kernel<IndexType,ValueType> <<<sum_grid, sum_block, sum_shared>>> 
        	(temp.num_rows, temp.num_cols, thrust::raw_pointer_cast(&temp.values[0]), thrust::raw_pointer_cast(&sum_temp.values[0]));
                    ...
                    Vector2& z)
{
        ...
        const size_t BLOCK_SIZE = 32;
        const size_t ROWS_PER_BLOCK = 6;
        const size_t MAX_BLOCKS = cusp::detail::device::arch::max_active_blocks(axpby_kernel<IndexType, ValueType>, BLOCK_SIZE, (size_t) 0);
        const size_t NUM_BLOCKS = std::min(MAX_BLOCKS, DIVIDE_INTO(x.num_rows, ROWS_PER_BLOCK));
        dim3 block(BLOCK_SIZE, ROWS_PER_BLOCK);
        dim3 grid(NUM_BLOCKS, 1);

        axpby_kernel<IndexType,ValueType> <<<grid, block>>>

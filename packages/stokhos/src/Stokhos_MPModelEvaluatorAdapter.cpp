...
createOutArgs() const
{
  ...
  outArgs.set_Np_Ng_mp(me_outargs.Np(), me_outargs.Ng());
  for (int j=0; j<me_outargs.Np(); j++)
...
evalModel(const InArgs& inArgs, const OutArgs& outArgs) const
{
  ...
  Teuchos::Array<mp_const_vector_t> p_mp(inArgs.Np_mp());
  mp_vector_t f_mp;
  mp_operator_t W_mp;
  Teuchos::Array<MPDerivative> dfdp_mp(outArgs.Np_mp());
  Teuchos::Array<mp_vector_t> g_mp(outArgs.Ng_mp());
  Teuchos::Array<MPDerivative> dgdx_mp(outArgs.Ng_mp());
  Teuchos::Array<MPDerivative> dgdx_dot_mp(outArgs.Ng_mp());
  Teuchos::Array< Teuchos::Array<MPDerivative> > dgdp_mp(outArgs.Ng_mp());
  int num_mp;
  ...
  for (int i=0; i<outArgs.Ng_mp(); i++) {
    ...
    dgdp_mp[i].resize(outArgs.Np_mp());
    for (int j=0; j<outArgs.Np_mp(); j++) {

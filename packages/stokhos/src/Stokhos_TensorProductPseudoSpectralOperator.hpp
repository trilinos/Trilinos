...
namespace Stokhos {
  ...
  public:
    ...
    TensorProductPseudoSpectralOperator(
      ...
      // Check if we can use PST if it was requested
      if (use_pst) {
	...
	TEUCHOS_TEST_FOR_EXCEPTION(
	  tb == NULL, std::logic_error,
	  "PST approach only works with tensor product basis " <<
	  "using lexographic coefficient ordering");
      }
    ...
    void apply(
      ...
      if (use_pst)
	apply_pst(alpha, input, result, beta, trans);
      else
  ...
  protected:
    ...
    //! Apply pseudo-spectral quadrature operator using direct method
    /*!
     * \c input is a vector storing values of a function at the quadrature
     * points, and \c result will contain the resulting polynomial chaos
     * coefficients.  \c input and \c result can have multiple columns for
    ...
    //! Apply tensor product quadrature operator using PST method
    /*!
     * \c input is a vector storing values of a function at the quadrature
     * points, and \c result will contain the resulting polynomial chaos
     * coefficients.  \c input and \c result can have multiple columns for
	...
	m = result.numRows();
      ...
      TEUCHOS_ASSERT(n == point_size());
      TEUCHOS_ASSERT(m == coeff_size());
  ...
  protected:
    ...
    //! Matrix mapping point to coefficients for each dimension for PST

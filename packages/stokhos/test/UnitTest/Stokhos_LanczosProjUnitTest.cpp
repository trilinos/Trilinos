...
struct Lanczos_PCE_Setup {
  ...
  Teuchos::RCP<const Stokhos::LanczosProjPCEBasis<OrdinalType,ValueType> > st_1d_basis;
...
namespace LanczosCosTest {
  ...
  Lanczos_PCE_Setup< Lanczos_Cos_Func<int,double> > setup(true);

  // Tests mapping from Lanczos basis to original is correct
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, CosMap ) {
   Stokhos::OrthogPolyApprox<int,double> u2(setup.basis);
   setup.st_1d_basis->transformCoeffsFromLanczos(setup.u_st.coeff(), 
						   u2.coeff());
   success = Stokhos::comparePCEs(setup.u, "u", u2, "u2", setup.rtol, setup.atol, out);
  }

  // Tests Lanczos basis is orthogonal
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, CosOrthog ) {
  ...
  // Tests PCE computed from Lanczos basis is same as original
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, CosPCE ) {
  ...
  // Tests mean computed from Lanczos basis is same as original
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, CosMean ) {
  ...
  // Tests standard deviation from Lanczos basis is same as original
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, CosStandardDeviation ) {
...
namespace LanczosSinTest {
  ...
  Lanczos_PCE_Setup< Lanczos_Sin_Func<int,double> > setup(true);

  // Tests mapping from Lanczos basis to original is correct
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, SinMap ) {
   Stokhos::OrthogPolyApprox<int,double> u2(setup.basis);
   setup.st_1d_basis->transformCoeffsFromLanczos(setup.u_st.coeff(), 
						   u2.coeff());
   success = Stokhos::comparePCEs(setup.u, "u", u2, "u2", setup.rtol, setup.atol, out);
  ...
  // Tests Lanczos basis is orthogonal
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, SinOrthog ) {
    const Teuchos::Array<double>& norms = setup.st_1d_basis->norm_squared();
    const Teuchos::Array<double>& weights = setup.st_quad->getQuadWeights();
    const Teuchos::Array< Teuchos::Array<double> >& values = 
      setup.st_quad->getBasisAtQuadPoints();
  ...
  // Tests PCE computed from Lanczos basis is same as original
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, SinPCE ) {
    Stokhos::OrthogPolyApprox<int,double> v2(setup.basis);
    lanczos_pce_quad_func quad_func(setup.v_st, *setup.st_basis);
    setup.exp->unary_op(quad_func, v2, setup.u);
    success = comparePCEs(setup.v, "v", v2, "v2", setup.rtol, setup.atol, out);
  ...
  // Tests mean computed from Lanczos basis is same as original
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, SinMean ) {
    success = Teuchos::testRelErr("v.mean()", setup.v.mean(), 
				  "v_st.mean()", setup.v_st.mean(),
				  "rtol", setup.rtol, 
				  "rtol", setup.rtol, 
  ...
  // Tests standard deviation from Lanczos basis is same as original
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, SinStandardDeviation ) {
    success = Teuchos::testRelErr("v.standard_deviation()", 
				  setup.v.standard_deviation(), 
				  "v_st.standard_devaition()", 
				  setup.v_st.standard_deviation(),
...
namespace LanczosExpTest {
  ...
  Lanczos_PCE_Setup< Lanczos_Exp_Func<int,double> > setup(false);

  // Tests mapping from Lanczos basis to original is correct
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, ExpMap ) {
   Stokhos::OrthogPolyApprox<int,double> u2(setup.basis);
   setup.st_1d_basis->transformCoeffsFromLanczos(setup.u_st.coeff(), 
						   u2.coeff());
   success = Stokhos::comparePCEs(setup.u, "u", u2, "u2", setup.rtol, setup.atol, out);
  ...
  // Tests Lanczos basis is orthogonal
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, ExpOrthog ) {
    const Teuchos::Array<double>& norms = setup.st_1d_basis->norm_squared();
    const Teuchos::Array<double>& weights = setup.st_quad->getQuadWeights();
    const Teuchos::Array< Teuchos::Array<double> >& values = 
      setup.st_quad->getBasisAtQuadPoints();
  ...
  // Tests PCE computed from Lanczos basis is same as original
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, ExpPCE ) {
    Stokhos::OrthogPolyApprox<int,double> v2(setup.basis);
    lanczos_pce_quad_func quad_func(setup.v_st, *setup.st_basis);
    setup.exp->unary_op(quad_func, v2, setup.u);
    success = comparePCEs(setup.v, "v", v2, "v2", setup.rtol, setup.atol, out);
  ...
  // Tests mean computed from Lanczos basis is same as original
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, ExpMean ) {
    success = Teuchos::testRelErr("v.mean()", setup.v.mean(), 
				  "v_st.mean()", setup.v_st.mean(),
				  "rtol", setup.rtol, 
				  "rtol", setup.rtol, 
  ...
  // Tests standard deviation from Lanczos basis is same as original
  TEUCHOS_UNIT_TEST( Stokhos_LanczosProjPCEBasis, ExpStandardDeviation ) {
    success = Teuchos::testRelErr("v.standard_deviation()", 
				  setup.v.standard_deviation(), 
				  "v_st.standard_devaition()", 
				  setup.v_st.standard_deviation(),

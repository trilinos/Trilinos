...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    // Kapil
    RCP<Epetra_Vector> b = rcp( new Epetra_Vector( *BaseMap, 1 ) );
    std::cout << "----b constructed------" << endl;
    /*
    ...
    */
    ...
    RCP<Epetra_Operator> MLPrecWrap = rcp( MLPrec );
    RCP<Belos::EpetraPrecOp> belosPrec = rcp( new Belos::EpetraPrecOp( MLPrecWrap ) );
    ...
    RCP<Epetra_Vector> x2 = rcp( new Epetra_Vector( *BaseMap, 1 ) );
    std::cout << "----x2 constructed------" << endl;
    /*
    ...
    */
    ...
    Belos::LinearProblem<double,MV,OP> problem(B_k, x2, b);
    std::cout << "----Problem constructed------" << endl;

    // Create an iterative solver manager.
    RCP< Belos::SolverManager<double,MV,OP> > newSolver
        = rcp( new Belos::RCGSolMgr<double,MV,OP>(rcp(&problem,false), rcp(&belosList,false)));
    std::cout << "----solver mgr created------" << endl;
    ...
    */
    ...
    for( std::size_t sp_idx = 0; sp_idx < quad_points.size(); sp_idx++){
      ...
      for(int i = 0; i<NumMyElements; i++){
         if(bcIndices[i] == 0){
	   // Kapil
	   b->operator[](i) = RHS_function(x[(MyGlobalElements[i]%n2)%x.size()], x[(MyGlobalElements[i]%n2)/x.size()],xi);
	   /*
      ...
      // std::cout << "----b filled------" << endl;

      B_k->FillComplete();
      AssemblyTimer.stop();

      problem.setOperator( B_k );
      problem.setRHS ( b );
      ...
      // std::cout << "----Problem constructed with set problem------" << endl;
      ...
      iters = iters + newSolver->getNumIters();

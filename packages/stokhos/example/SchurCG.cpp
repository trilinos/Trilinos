...
#include "Stokhos_Diag_Preconditioner.hpp"


//Implementing the Schur Complement preconditioner
int Schur(const  Teuchos::SerialDenseMatrix<int, double> &  K) 
{ 
  int N; //polynomial order
  int M; //# of random variables
  //subdivide the matrix 
  size=
  Teuchos::SerialDenseMatrix<int, double> A(Teuchos::Copy, K,(n-1+m)!/((n-1)!m!),);
...
//CG
int CG(const  Teuchos::SerialDenseMatrix<int, double> &  A, Teuchos::SerialDenseMatrix<int,double>   X,const Teuchos::SerialDenseMatrix<int,double> &   B, int max_iter, double tolerance)

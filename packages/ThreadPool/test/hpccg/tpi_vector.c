...
{
  const int chunk = ( n + work->work_count - 1 ) / work->work_count ;

  *iEnd = chunk * ( work->work_rank + 1 );
  *iBeg = chunk * ( work->work_rank );
...
static void tpi_work_fill( TPI_Work * work )
{
  const struct tpi_work_vector * const h =
    (struct tpi_work_vector *) work->shared ;
...
static void tpi_work_dot_reduce( void * dest , const void * src  )
{
...
static void tpi_work_crs_matrix_apply( TPI_Work * work )
{
  const struct tpi_crs_matrix * const h =
    (struct tpi_crs_matrix *) work->shared ;
  ...
  const int chunk = ( nRow + work->work_count - 1 ) / work->work_count ;

  int rowEnd = chunk * ( work->work_rank + 1 );
  int row    = chunk * work->work_rank ;

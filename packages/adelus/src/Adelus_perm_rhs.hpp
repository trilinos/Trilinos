...
namespace Adelus {
  ...
  void permute_rhs(HandleType& ahandle, ZViewType& RHS, PViewType& permute) {
    ...
    if (!(ahandle.get_gpuawarempi_behavior()) && isOnDeviceSpace) {
      h_tmpr = ViewVectorHostPinnType( Kokkos::view_alloc(Kokkos::WithoutInitializing, "h_tmpr"), RHS.extent(1) );
      h_tmps = ViewVectorHostPinnType( Kokkos::view_alloc(Kokkos::WithoutInitializing, "h_tmps"), RHS.extent(1) );
    }

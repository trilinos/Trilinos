...
int tLSCStabilized::runTest(int verbosity, std::ostream& stdstrm, std::ostream& failstrm,
                            int& totalrun) {
  bool allTests = true;
  bool status;
  int failcount = 0;
...
bool tLSCStabilized::test_diagonal(int verbosity, std::ostream& os) {
  ...
  eb[0] = 1.0;
  eb[1] = 3.0;
  ef[0] = 0.407268709825528;
  ef[1] = 1.560553633217993;
  eg[0] = -0.058181244260790;
  eg[1] = -0.265138408304498;
  Thyra::apply(*precOp, Thyra::NOTRANS, *x, y.ptr());
  TEST_ASSERT((diff = Teko::Test::Difference(y, z) / Thyra::norm_2(*z->col(0))) < tolerance_,
              "   tLSCStabilized::test_diagonal "
  ...
  eb[0] = 7.0;
  eb[1] = 9.0;
  ef[0] = 0.850880968778696;
  ef[1] = 5.181660899653979;
  eg[0] = -0.407268709825528;
  eg[1] = -0.795415224913495;
  Thyra::apply(*precOp, Thyra::NOTRANS, *x, y.ptr());
  TEST_ASSERT((diff = Teko::Test::Difference(y, z) / Thyra::norm_2(*z->col(0))) < tolerance_,
              "   tLSCStabilized::test_diagonal "
                  << toString(status) << ":(y=inv(A)*x) != z (|y-z|_2/|z|_2 = " << diff
  ...
  ef[1] = -1.767589388696655;
  eg[0] = 0.000000000000000;
  eg[1] = 0.441897347174164;
  Thyra::apply(*precOp, Thyra::NOTRANS, *x, y.ptr());
  TEST_ASSERT((diff = Teko::Test::Difference(y, z) / Thyra::norm_2(*z->col(0))) < tolerance_,
              "   tLSCStabilized::test_diagonal "
                  << toString(status) << ":(y=inv(A)*x) != z (|y-z|_2/|z|_2 = " << diff
  ...
  eb[0] = 6.0;
  eb[1] = 12.0;
  ef[0] = 6.443612258953168;
  ef[1] = 2.242214532871971;
  eg[0] = -0.349087465564738;
  eg[1] = -1.060553633217993;
  Thyra::apply(*precOp, Thyra::NOTRANS, *x, y.ptr());
  TEST_ASSERT((diff = Teko::Test::Difference(y, z) / Thyra::norm_2(*z->col(0))) < tolerance_,
              "   tLSCStabilized::test_diagonal "
                  << toString(status) << ":(y=inv(A)*x) != z (|y-z|_2/|z|_2 = " << diff
...
bool tLSCStabilized::test_strategy(int verbosity, std::ostream& os) {
  ...
  // build inverse strategy
  {
    ...
    TEST_ASSERT(result, std::endl
                            << "   tLSCStabilized::test_strategy " << toString(status)

...
void tExplicitOps::initializeTest() {
  ...
  v->Random();
  RCP<Thyra::VectorBase<double> > tV =
      Thyra::create_Vector(v, Thyra::create_VectorSpace(rcpFromRef(epetraF.RowMap())));
  D_ = Thyra::diagonal(tV);
}

int tExplicitOps::runTest(int verbosity, std::ostream& stdstrm, std::ostream& failstrm,
                          int& totalrun) {
  bool allTests = true;
  bool status;
  int failcount = 0;
...
bool tExplicitOps::test_mult_diagScaleMatProd(int verbosity, std::ostream& os) {
...

  {
    ...
    TEST_ASSERT(result, std::endl
                            << "   tExplicitOps::test_diagScaleMatProd "
...
bool tExplicitOps::test_mult_modScaleMatProd(int verbosity, std::ostream& os) {
...

  {
    ...
    const bool result = tester.compare(*thyOp, *expOp, Teuchos::ptrFromRef(fos));
    TEST_ASSERT(result, std::endl
                            << "   tExplicitOps::test_modScaleMatProd "
                            << ": Testing triple matrix product");
    if (not result || verbosity >= 10) os << ss.str();
  }
  ...
  Teuchos::rcp_dynamic_cast<Epetra_CrsMatrix>(Thyra::get_Epetra_Operator(*F_))
      ->ExtractMyRowView(3, numEntries, values);
  for (int i = 0; i < numEntries; i++) values[i] *= values[i] * double(i + 1) * 0.92;

  // do some random violence (oh my brothers) to one row
  Teuchos::rcp_dynamic_cast<Epetra_CrsMatrix>(Thyra::get_Epetra_Operator(*F_))
      ->ExtractMyRowView(7, numEntries, values);
  for (int i = 0; i < numEntries; i++) values[i] *= values[i] * double(i + 1) * 0.92;

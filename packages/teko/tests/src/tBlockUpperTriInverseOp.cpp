...
const RCP<const Thyra::LinearOpBase<double> > build2x2(const RCP<const Epetra_Map> & map,double a,double b,double c,double d)
{
   int indicies[2];
   double row0[2];
   double row1[2];
...
int tBlockUpperTriInverseOp::runTest(int verbosity,std::ostream & stdstrm,std::ostream & failstrm,int & totalrun)
{
   ...
   status = allTests;
   if(verbosity >= 10) {
      PB_TEST_MSG(failstrm,0,"tUpperTriInverseOp...PASSED","tUpperTriInverseOp...FAILED");
   }
   else {// Normal Operatoring Procedures (NOP)
      PB_TEST_MSG(failstrm,0,"...PASSED","tUpperTriInverseOp...FAILED");
   }

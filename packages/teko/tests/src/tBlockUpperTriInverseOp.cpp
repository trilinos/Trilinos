...
int tBlockUpperTriInverseOp::runTest(int verbosity, std::ostream& stdstrm, std::ostream& failstrm,
                                     int& totalrun) {
  bool allTests = true;
  bool status;
  int failcount = 0;
...
bool tBlockUpperTriInverseOp::test_alphabeta(int verbosity, std::ostream& os) {
  ...
  TEST_ASSERT(diff <= 0.0, std::endl
                               << "   tBlockUpperTriInverseOp::test_apply " << toString(status)
  ...
  TEST_ASSERT(diff <= tolerance_, std::endl
                                      << "   tBlockUpperTriInverseOp::test_apply "
                                      << toString(status)
  ...
  TEST_ASSERT(diff <= tolerance_, std::endl
                                      << "   tBlockUpperTriInverseOp::test_apply "
                                      << toString(status) << ": apply operation (relerr=" << diff
                                      << " <= " << tolerance_ << ")");
...
bool tBlockUpperTriInverseOp::test_applyTranspose(int verbosity, std::ostream& os) {
  ...
  RCP<Thyra::PhysicallyBlockedLinearOpBase<double> > U = getUpperTriBlocks(A_);
  RCP<const Thyra::LinearOpBase<double> > invTri =
      Thyra::transpose(createBlockUpperTriInverseOp(U, invDiag_));

...
void tStridedEpetraOperator::initializeTest() { tolerance_ = 1e-14; }

int tStridedEpetraOperator::runTest(int verbosity, std::ostream& stdstrm, std::ostream& failstrm,
                                    int& totalrun) {
  bool allTests = true;
  bool status;
  int failcount = 0;
  ...
  status = allTests;
  if (verbosity >= 10) {
    Teko_TEST_MSG(failstrm, 0, "tStridedEpetraOperator...PASSED",
                  "tStridedEpetraOperator...FAILED");
...
bool tStridedEpetraOperator::test_numvars_constr(int verbosity, std::ostream& os) {
  ...
  TEST_ASSERT(max >= min, "\n   tStridedEpetraOperator::test_numvars_constr: "
                              << toString(status) << ": "
  ...
  TEST_ASSERT(max >= min, "\n   tStridedEpetraOperator::test_numvars_constr (rebuild): "
                              << toString(status) << ": "
                              << "sanity checked - " << max << " >= " << min);
  TEST_ASSERT(max <= tolerance_, "\n   tStridedEpetraOperator::test_numvars_constr (rebuild): "
                                     << toString(status) << ": "
...
bool tStridedEpetraOperator::test_vector_constr(int verbosity, std::ostream& os) {
  ...
  TEST_ASSERT(max >= min, "\n   tStridedEpetraOperator::test_vector_constr (rebuild): "
                              << toString(status) << ": "
                              << "sanity checked - " << max << " >= " << min);
  TEST_ASSERT(max <= tolerance_, "\n   tStridedEpetraOperator::test_vector_constr (rebuild): "
                                     << toString(status) << ": "
...
bool tStridedEpetraOperator::test_reorder(int verbosity, std::ostream& os, int total) {
  ...
  TEST_ASSERT(max >= min, "   tStridedEpetraOperator::test_reorder"
                              << tstr << ": " << toString(status) << ": "
                              << "sanity checked - " << max << " >= " << min);
  TEST_ASSERT(max <= tolerance_, "   tStridedEpetraOperator::test_reorder"
                                     << tstr << ": " << toString(status) << ": "
                                     << "testing tolerance over many matrix vector multiplies ( "
                                     << max << " <= " << tolerance_ << " )");

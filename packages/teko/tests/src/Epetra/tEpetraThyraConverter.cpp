...
double compareEpetraMVToThyra(const Epetra_MultiVector& eX,
                              const Teuchos::RCP<const Thyra::MultiVectorBase<double> >& tX,
                              int verbosity, std::ostream& os, int indexStart = -1,
                              int indexEnd = -1);
...
double compareEpetraMVToThyra(const Epetra_MultiVector& eX,
  ...
  for (int i = 0; i < prodVS->numBlocks(); i++) {
    ...
    double val = compareEpetraMVToThyra(eX, prodX->getMultiVectorBlock(i), verbosity, os,
...
int tEpetraThyraConverter::runTest(int verbosity, std::ostream& stdstrm, std::ostream& failstrm,
                                   int& totalrun) {
  bool allTests = true;
  bool status;
  int failcount = 0;
  ...
  Teko_TEST_MSG(stdstrm, 1, "   \"blockThyraToEpetra\" ... PASSED",
                "   \"blockThyraToEpetra\" ... FAILED");
  allTests &= status;
  failcount += status ? 0 : 1;
  totalrun++;

  status = test_single_blockThyraToEpetra(verbosity, failstrm);
  Teko_TEST_MSG(stdstrm, 1, "   \"single_blockThyraToEpetra\" ... PASSED",
                "   \"single_blockThyraToEpetra\" ... FAILED");
  allTests &= status;
  failcount += status ? 0 : 1;
  totalrun++;

  status = test_blockEpetraToThyra(verbosity, failstrm);
  Teko_TEST_MSG(stdstrm, 1, "   \"blockEpetraToThyra\" ... PASSED",
                "   \"blockEpetraToThyra\" ... FAILED");
  allTests &= status;
  failcount += status ? 0 : 1;
  totalrun++;

  status = test_single_blockEpetraToThyra(verbosity, failstrm);
  Teko_TEST_MSG(stdstrm, 1, "   \"single_blockEpetraToThyra\" ... PASSED",
                "   \"single_blockEpetraToThyra\" ... FAILED");
  allTests &= status;
  failcount += status ? 0 : 1;
  totalrun++;
...
bool tEpetraThyraConverter::test_blockThyraToEpetra(int verbosity, std::ostream& os) {
  ...
  TEST_ASSERT(eX != Teuchos::null, "\n   tEpetraThyraConverter::test_blockThyraToEpetra "
                                       << toString(status)
...
bool tEpetraThyraConverter::test_single_blockThyraToEpetra(int verbosity, std::ostream& os) {
  ...
  TEST_ASSERT(eX != Teuchos::null, "\n   tEpetraThyraConverter::test_single_blockThyraToEpetra: "
                                       << toString(status)
                                       << ": blockThyraToEpetra returns not null");
...
bool tEpetraThyraConverter::test_blockEpetraToThyra(int verbosity, std::ostream& os) {
  ...
  double result = compareEpetraMVToThyra(eX, tX, verbosity, os);
  TEST_ASSERT(result == 0.0, "\n   tEpetraThyraConverter::test_blockEpetraToThyra: "
                                 << toString(status)
                                 << ": Epetra MV is compared to Thyra MV (maxdiff = " << result
                                 << ")");
...
bool tEpetraThyraConverter::test_single_blockEpetraToThyra(int verbosity, std::ostream& os) {
  ...
  TEST_ASSERT(tX != Teuchos::null, "\n   tEpetraThyraConverter::test_single_blockEpetraToThyra: "
                                       << toString(status)

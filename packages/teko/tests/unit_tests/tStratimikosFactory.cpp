...
TEUCHOS_UNIT_TEST(tStratimikosFactory, test_Defaults) {
  ...
  // build tpetra operator
  RCP<Tpetra::Operator<ST, LO, GO, NT> > tpetraA = buildSystem(comm, 5);
  RCP<Thyra::LinearOpBase<double> > tA           = Thyra::tpetraLinearOp<ST, LO, GO, NT>(
      Thyra::tpetraVectorSpace<ST, LO, GO, NT>(tpetraA->getRangeMap()),
      Thyra::tpetraVectorSpace<ST, LO, GO, NT>(tpetraA->getDomainMap()), tpetraA);
  ...
  RCP<const Teko::InverseFactory> invFact = invLib->getInverseFactory("Amesos2");
...
TEUCHOS_UNIT_TEST(tStratimikosFactory, test_MultipleCalls) {
  ...
  // build tpetra operator
  RCP<Tpetra::Operator<ST, LO, GO, NT> > tpetraA = buildStridedSystem(comm, 5);
  RCP<Thyra::LinearOpBase<double> > tA           = Thyra::tpetraLinearOp<ST, LO, GO, NT>(
      Thyra::tpetraVectorSpace<ST, LO, GO, NT>(tpetraA->getRangeMap()),
      Thyra::tpetraVectorSpace<ST, LO, GO, NT>(tpetraA->getDomainMap()), tpetraA);
...
TEUCHOS_UNIT_TEST(tStratimikosFactory, test_BlockGaussSeidel) {
  ...
  Teko::LinearOp testOp;
  {
    ...
    RCP<Tpetra::Operator<ST, LO, GO, NT> > strided_tpetraA =
        Teuchos::rcp(new Teko::TpetraHelpers::StridedTpetraOperator(2, tpetraA));
    ...
    RCP<Teko::TpetraHelpers::InverseFactoryOperator> invFactOp =
        Teuchos::rcp(new Teko::TpetraHelpers::InverseFactoryOperator(invFact));
    invFactOp->buildInverseOperator(strided_tpetraA);

    testOp = Thyra::tpetraLinearOp<ST, LO, GO, NT>(
        Thyra::tpetraVectorSpace<ST, LO, GO, NT>(invFactOp->getRangeMap()),
        Thyra::tpetraVectorSpace<ST, LO, GO, NT>(invFactOp->getDomainMap()),
        Teuchos::rcp_dynamic_cast<Tpetra::Operator<ST, LO, GO, NT> >(invFactOp));
  }

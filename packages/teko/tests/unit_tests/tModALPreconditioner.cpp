...
TEUCHOS_UNIT_TEST(tModALPreconditioner, test)
{
   ...
   Epetra_Map mapVel(numVel, 0, comm), mapPre(numPre, 0, comm), mapAll(numVel * dim + numPre, 0, comm);
   ...
   // Build AL operator.
   double gamma = 0.05;
   Teuchos::RCP<Teko::NS::ALOperator>
      al = Teuchos::rcp(new Teko::NS::ALOperator(blockedVec, mat, lpMp, gamma));

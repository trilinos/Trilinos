// Teko_KrylovSurrogate.hpp — adaptive block-preconditioning orchestration
// around the Krylov-informed reduced model.
//
// The reduced-model construction (C_hat / b_hat from the Krylov state, via
// per-block Gram-matrix thin SVDs) lives in Teko_KrylovReducedModel.hpp, which
// this header includes. Everything here is the machinery around it: the JSON
// request/reconfig/convergence/solved file formats, the watcher handshake, the
// reconfigured-preconditioner rebuild, and the Belos adaptive hook itself.
//
...
#include "Teko_KrylovReducedModel.hpp"  // type aliases + CHatData/computeCHat (the surrogate math)

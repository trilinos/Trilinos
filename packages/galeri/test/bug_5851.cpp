    ...
    RCP<Galeri::Xpetra::Problem<Tpetra::Map<lno_t, gno_t>,
        ...
        Pr =
            Galeri::Xpetra::BuildProblem<scalar_t, lno_t, gno_t,
                                         ...
                                         Tpetra::CrsMatrix<scalar_t, lno_t, gno_t>,
                                         Tpetra::MultiVector<scalar_t, lno_t, gno_t> >(params.GetMatrixType(), map, params.GetParameterList());
    if (comm->getRank() == 0)
...
int main(int narg, char **arg) {
  ...
  if (comm->getRank() == 0 && !ierr)
    cout << "SUCCESS for scalar_t == DOUBLE" << endl
         << endl
         << endl;
  ...
  if (comm->getRank() == 0 && !jerr)
    cout << "SUCCESS for scalar_t == FLOAT" << endl
         << endl
         << endl;

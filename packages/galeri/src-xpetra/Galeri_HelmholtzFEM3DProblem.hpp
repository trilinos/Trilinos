...
class HelmholtzFEM3DProblem : public Problem_Helmholtz<Map, Matrix, MultiVector> {
 public:
  HelmholtzFEM3DProblem(Teuchos::ParameterList& list, const Teuchos::RCP<const Map>& map)
    ...
    PMLwidthz_ = std::max(LBz_, Dz_ - RBz_);
    if (PMLwidthz_ == 0) {
      PMLwidthz_ = 1.0;
    }
    nDim = 3;
...
void HelmholtzFEM3DProblem<Scalar, LocalOrdinal, GlobalOrdinal, Map, Matrix, MultiVector>::EvalBasis(Point& quadPoint, std::vector<double>& vals, std::vector<double>& dxs, std::vector<double>& dys, std::vector<double>& dzs) {
  ...
  double valy2 = y / hy_;
  double dy2   = 1 / hy_;
  double valz1 = (hz_ - z) / hz_;
  double dz1   = -1 / hz_;
  double valz2 = z / hz_;
  double dz2   = 1 / hz_;
  // linear function values, derivatives in x, y, and z directions
  vals[0] = valx1 * valy1 * valz1;
  dxs[0]  = dx1 * valy1 * valz1;
  dys[0]  = valx1 * dy1 * valz1;
  dzs[0]  = valx1 * valy1 * dz1;
  vals[1] = valx2 * valy1 * valz1;
  dxs[1]  = dx2 * valy1 * valz1;
  dys[1]  = valx2 * dy1 * valz1;
  dzs[1]  = valx2 * valy1 * dz1;
  vals[2] = valx1 * valy2 * valz1;
  dxs[2]  = dx1 * valy2 * valz1;
  dys[2]  = valx1 * dy2 * valz1;
  dzs[2]  = valx1 * valy2 * dz1;
  vals[3] = valx2 * valy2 * valz1;
  dxs[3]  = dx2 * valy2 * valz1;
  dys[3]  = valx2 * dy2 * valz1;
  dzs[3]  = valx2 * valy2 * dz1;
  vals[4] = valx1 * valy1 * valz2;
  dxs[4]  = dx1 * valy1 * valz2;
  dys[4]  = valx1 * dy1 * valz2;
  dzs[4]  = valx1 * valy1 * dz2;
  vals[5] = valx2 * valy1 * valz2;
  dxs[5]  = dx2 * valy1 * valz2;
  dys[5]  = valx2 * dy1 * valz2;
  dzs[5]  = valx2 * valy1 * dz2;
  vals[6] = valx1 * valy2 * valz2;
  dxs[6]  = dx1 * valy2 * valz2;
  dys[6]  = valx1 * dy2 * valz2;
  dzs[6]  = valx1 * valy2 * dz2;
  vals[7] = valx2 * valy2 * valz2;
  dxs[7]  = dx2 * valy2 * valz2;
  dys[7]  = valx2 * dy2 * valz2;
  dzs[7]  = valx2 * valy2 * dz2;
}
...
void HelmholtzFEM3DProblem<Scalar, LocalOrdinal, GlobalOrdinal, Map, Matrix, MultiVector>::EvalStretch(double& shiftx, double& shifty, double& shiftz, const std::vector<Point>& quadPoints, std::vector<Scalar>& sx, std::vector<Scalar>& sy, std::vector<Scalar>& sz, std::vector<Scalar>& cs) {
  ...
  for (unsigned int i = 0; i < quadPoints.size(); i++) {
    ...
    if (curz < LBz_) {
      sigz = delta_ * pow((curz - LBz_) / PMLwidthz_, 2);
    } else if (curz > RBz_) {
      sigz = delta_ * pow((curz - RBz_) / PMLwidthz_, 2);
    } else {
      sigz = 0.0;
    }

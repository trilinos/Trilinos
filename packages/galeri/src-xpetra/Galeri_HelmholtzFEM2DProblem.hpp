...
class HelmholtzFEM2DProblem : public Problem_Helmholtz<Map, Matrix, MultiVector> {
 public:
  HelmholtzFEM2DProblem(Teuchos::ParameterList& list, const Teuchos::RCP<const Map>& map)
    : Problem_Helmholtz<Map, Matrix, MultiVector>(list, map) {
    // parameters
    ...
    PMLwidthx_ = std::max(LBx_, Dx_ - RBx_);
    if (PMLwidthx_ == 0) {
      PMLwidthx_ = 1.0;
    }
    PMLwidthy_ = std::max(LBy_, Dy_ - RBy_);
    if (PMLwidthy_ == 0) {
      PMLwidthy_ = 1.0;
    }
    nDim = 2;
...
void HelmholtzFEM2DProblem<Scalar, LocalOrdinal, GlobalOrdinal, Map, Matrix, MultiVector>::EvalBasis(Point& quadPoint, std::vector<double>& vals, std::vector<double>& dxs, std::vector<double>& dys) {
  ...
  double valx1 = (hx_ - x) / hx_;
  double dx1   = -1 / hx_;
  double valx2 = x / hx_;
  double dx2   = 1 / hx_;
  double valy1 = (hy_ - y) / hy_;
  double dy1   = -1 / hy_;
  double valy2 = y / hy_;
  double dy2   = 1 / hy_;
...
void HelmholtzFEM2DProblem<Scalar, LocalOrdinal, GlobalOrdinal, Map, Matrix, MultiVector>::EvalStretch(double& shiftx, double& shifty, const std::vector<Point>& quadPoints, std::vector<Scalar>& sx, std::vector<Scalar>& sy, std::vector<Scalar>& cs) {
  ...
  for (unsigned int i = 0; i < quadPoints.size(); i++) {
    ...
    double sigx, sigy;
    if (curx < LBx_) {
      sigx = delta_ * pow((curx - LBx_) / PMLwidthx_, 2);
    } else if (curx > RBx_) {
      sigx = delta_ * pow((curx - RBx_) / PMLwidthx_, 2);
    } else {
      sigx = 0.0;
    }
    if (cury < LBy_) {
      sigy = delta_ * pow((cury - LBy_) / PMLwidthy_, 2);
    } else if (cury > RBy_) {
      sigy = delta_ * pow((cury - RBy_) / PMLwidthy_, 2);
    } else {
      sigy = 0.0;
    }

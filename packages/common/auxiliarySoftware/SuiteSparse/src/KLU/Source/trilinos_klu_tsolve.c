...
/* Solve A'x=b using the symbolic and numeric objects from TRILINOS_KLU_analyze
...
#pragma omp parallel for schedule(guided) private(tid, nr, k, block, k1, k2, nk, pend, p, s, i, Bz, X, x, offik, rs) 
	...
	for (block = 0 ; block < nblocks ; block++)
	{
	    ...
	    {
		TRILINOS_KLU_utsolve (nk, Uip + k1, Ulen + k1, LUbx [block],
			Udiag + k1, nr,
#ifdef COMPLEX
			conj_solve,
#endif
			X + nr*k1) ;
		TRILINOS_KLU_ltsolve (nk, Lip + k1, Llen + k1, LUbx [block], nr,
#ifdef COMPLEX
			conj_solve,
#endif
			X + nr*k1) ;

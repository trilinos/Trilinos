...
*
*\Description:
*	KOKKOS_DCRSSM performs one of the triangular solve operations
*
*    x = T^{-1}*x or x = A^{-T}*x.
*
*    where x is a vector, T is an n-by-n triangular.  The matrix
...
*    each row in T.  Each element of the array rowind contains
*    the column index of the corresponding element of T.
*
*\Usage:
*     call DCRSSM(  iuplo, itrans, udiag, n, val, indx, profile, x, ldx, nrhs )
*
*
*    iuplo Integer (input)
*           On entry, iuplo specifies whether or not T is upper/lower triangular.
*           If itrans = 0, T is lower triangular.
*           If itrans = 1, T is upper triangular..
*           If itrans is any other value, then no operation is
*           performed.
*           The iuplo argument is unchanged on exit.
*
...
*\Enddoc
      ...
      subroutine kokkos_dcrsmm( iuplo, itrans, udiag, m, n, val, indx, 
     &                          profile, 
     &                          x, ldx, y, ldy, nrhs)
...
*     ----------------------------
      implicit none
      integer iuplo, itrans, udiag, m, n, ldx, ldy
...
*     Strip mine nrhs by fives
      ...
      do 10 irhs = 1, nrhs, 5
         if (itrans.eq.0) then
            call kokkos_scrssm5(udiag, m, n, val, indx, profile,
     &                          x(ix), ldx, y(iy), ldy, nrhs1)
         else
            call kokkos_sccssm5(udiag, m, n, val, indx, profile,
     &                          x(ix), ldx, y(iy), ldy, nrhs1)
         endif
         ix = ix + nrhs1*ldx
         iy = iy + nrhs1*ldy
 ...
 10   continue
      ...
      subroutine kokkos_sccssm5( udiag, m, n, val, indx, profile, 
     &                           x, ldx, y, ldy, nrhs)
*
...
*                               x = T^{-1}*x
*
*     where x is a vector and T is a sparse matrix stored
...
*     ----------------------------
      implicit none
      integer iuplo, udiag, m, n, nrhs, ldx, ldy

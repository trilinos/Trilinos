...
#include "Kokkos_Map.hpp"
#include "Kokkos_Import.hpp"
#include "Kokkos_Export.hpp"
#include "Kokkos_Vector.hpp"
#include "Kokkos_MultiVector.hpp"
#include "Kokkos_Comm.hpp"
#include "Kokkos_Distributor.hpp"

//==============================================================================
Kokkos_CrsMatrix::Kokkos_CrsMatrix(Kokkos_DataAccess CV, const Kokkos_Map& RowMap, int* NumEntriesPerRow) 
  : Kokkos_DistObject(RowMap, "Kokkos::CrsMatrix"),
    Kokkos_CompObject(),
    Kokkos_BLAS(),
    Graph_(0),
    Allocated_(false),
    ...
    CV_(CV)
{
  Graph_ = new Kokkos_CrsGraph(CV, RowMap, NumEntriesPerRow);
  InitializeDefaults();
  Allocate();
}
...
//==============================================================================
Kokkos_CrsMatrix::Kokkos_CrsMatrix(Kokkos_DataAccess CV, const Kokkos_Map& RowMap, 
				   const Kokkos_Map& ColMap, int* NumEntriesPerRow) 
  : Kokkos_DistObject(RowMap, "Kokkos::CrsMatrix"),
    Kokkos_CompObject(),
    Kokkos_BLAS(),
    Graph_(0),
    ...
    CV_(CV)
{
  Graph_ = new Kokkos_CrsGraph(CV, RowMap, ColMap, NumEntriesPerRow);
  InitializeDefaults();
  Allocate();
}
...
//==============================================================================
Kokkos_CrsMatrix::Kokkos_CrsMatrix(Kokkos_DataAccess CV, const Kokkos_CrsGraph& Graph) 
  : Kokkos_DistObject(Graph.Map(), "Kokkos::CrsMatrix"),
    Kokkos_CompObject(),
    Kokkos_BLAS(),
    Graph_((Kokkos_CrsGraph*) &Graph),
    Allocated_(false),
    StaticGraph_(true),
...
//==============================================================================
Kokkos_CrsMatrix::Kokkos_CrsMatrix(const Kokkos_CrsMatrix& Matrix) 
  : Kokkos_DistObject(Matrix),
    ...
    CV_(Copy)
{
  Graph_ = new Kokkos_CrsGraph(Matrix.Graph());
  Allocate();
...
//==============================================================================
void Kokkos_CrsMatrix::InitializeDefaults() { // Initialize all attributes that have trivial default values
...
//==============================================================================
Kokkos_CrsMatrix::~Kokkos_CrsMatrix(){
...
//==============================================================================
int Kokkos_CrsMatrix::PutScalar(double ScalarConstant) 
{
  for(int i=0; i<NumMyRows_; i++) {
    int NumEntries = NumEntriesPerRow_[i];
...
//==============================================================================
int Kokkos_CrsMatrix::Scale(double ScalarConstant) 
{
  for(int i=0; i<NumMyRows_; i++) {
    int NumEntries = NumEntriesPerRow_[i];
    for(int j=0; j< NumEntries; j++) 
...
//==========================================================================
int Kokkos_CrsMatrix::InsertGlobalValues(int Row, int NumEntries,
					 double* Values,
					 int* Indices)
{
  if(IndicesAreLocal()) 
    KOKKOS_CHK_ERR(-2); // Cannot insert global values into local graph
  if(IndicesAreContiguous()) 
    KOKKOS_CHK_ERR(-3); // Indices cannot be individually deleted and newed
  Graph_->SetIndicesAreGlobal(true);
  Row = Graph_->LRID(Row); // Find local row number for this global row index

  KOKKOS_CHK_ERR( InsertValues(Row, NumEntries, Values, Indices) );

  return(0);
}

//==========================================================================
int Kokkos_CrsMatrix::InsertMyValues(int Row, int NumEntries,
				     double* Values,
				     int* Indices)
{
  if(IndicesAreGlobal()) 
    KOKKOS_CHK_ERR(-2); // Cannot insert global values into filled graph
  if(IndicesAreContiguous()) 
    KOKKOS_CHK_ERR(-3); // Indices cannot be individually deleted and new
...
//==========================================================================
int Kokkos_CrsMatrix::InsertValues(int Row, int NumEntries,
				   double* Values,
				   int* Indices)
{
  int j;
  ...
  if(Row < 0 || Row >= NumMyRows_) 
    KOKKOS_CHK_ERR(-1); // Not in Row range
  ...
  if(CV_ == View) {
	...
	testRow = Graph_->LRID( Row );
      KOKKOS_CHK_ERR(Graph_->ExtractMyRowView(testRow, testNumEntries, testIndices));
    ...
    if(StaticGraph()) 
      KOKKOS_CHK_ERR(-2); // If the matrix graph is fully constructed, we cannot insert new values
  ...
  if(!StaticGraph()) {
    KOKKOS_CHK_ERR(Graph_->InsertIndices(Row, NumEntries, Indices));
  }
...
//==========================================================================
int Kokkos_CrsMatrix::ReplaceGlobalValues(int Row, int NumEntries, double * Values, int *Indices) {
...
//==========================================================================
int Kokkos_CrsMatrix::ReplaceMyValues(int Row, int NumEntries, double * Values, int *Indices) {

  if (!IndicesAreLocal()) 
    KOKKOS_CHK_ERR(-4); // Indices must be local.
...
//==========================================================================
int Kokkos_CrsMatrix::SumIntoGlobalValues(int Row, int NumEntries, double * Values, int *Indices) {
...
//==========================================================================
int Kokkos_CrsMatrix::SumIntoMyValues(int Row, int NumEntries, double * Values, int *Indices) {
...
//==========================================================================
int Kokkos_CrsMatrix::FillComplete() {
  KOKKOS_CHK_ERR(FillComplete(RowMap(), RowMap()));
  return(0);
}

//==========================================================================
int Kokkos_CrsMatrix::FillComplete(const Kokkos_Map& DomainMap, const Kokkos_Map& RangeMap) {
  if (!StaticGraph()) 
    KOKKOS_CHK_ERR(Graph_->MakeIndicesLocal(DomainMap, RangeMap));
  SortEntries();  // Sort column entries from smallest to largest
  MergeRedundantEntries(); // Get rid of any redundant index values
  if (!StaticGraph()) 
    KOKKOS_CHK_ERR(Graph_->FillComplete(DomainMap, RangeMap));

  return(0);
}

//==========================================================================
int Kokkos_CrsMatrix::TransformToLocal() {
  KOKKOS_CHK_ERR(FillComplete());
  return(0);
}

//==========================================================================
int Kokkos_CrsMatrix::TransformToLocal(const Kokkos_Map* DomainMap, const Kokkos_Map* RangeMap) {
  KOKKOS_CHK_ERR(FillComplete(*DomainMap, *RangeMap));
  return(0);
}

//==========================================================================
int Kokkos_CrsMatrix::SortEntries() {
...
//==========================================================================
int Kokkos_CrsMatrix::MergeRedundantEntries() {
  ...
  if(NoRedundancies()) 
    return(0);
  if(!Sorted()) 
    KOKKOS_CHK_ERR(-1);  // Must have sorted entries
  ...
  KOKKOS_CHK_ERR(Graph_->RemoveRedundantIndices()); // Remove redundant indices and then return
  return(0);
}

//==========================================================================
int Kokkos_CrsMatrix::OptimizeStorage() {
  ...
  if ((CV_==View) && !Contiguous) 
    KOKKOS_CHK_ERR(-1);  // This is user data, it's not contiguous and we can't make it so.
...
//==========================================================================
int Kokkos_CrsMatrix::ExtractGlobalRowCopy(int Row, int Length, int & NumEntries, double * Values,
					   int * Indices) const 
{
  ...
  KOKKOS_CHK_ERR(ExtractGlobalRowCopy(Row, Length, NumEntries, Values));
  return(0);
}
//==========================================================================
int Kokkos_CrsMatrix::ExtractMyRowCopy(int Row, int Length, int & NumEntries, double * Values,
				       int * Indices) const 
{
  ...
  KOKKOS_CHK_ERR(ExtractMyRowCopy(Row, Length, NumEntries, Values));
  return(0);
}
//==========================================================================
int Kokkos_CrsMatrix::NumMyRowEntries(int Row, int & NumEntries) const 
{

  if (!MyLRID(Row)) 
    KOKKOS_CHK_ERR(-1); // Not in the range of local rows
  NumEntries = NumMyEntries(Row);
  return(0);
}
//==========================================================================
int Kokkos_CrsMatrix::ExtractGlobalRowCopy(int Row, int Length, int & NumEntries, double * Values) const 
{

  int Row0 = Graph_->RowMap().LID(Row); // Normalize row range

  KOKKOS_CHK_ERR(ExtractMyRowCopy(Row0, Length, NumEntries, Values));
  return(0);
}


//==========================================================================
int Kokkos_CrsMatrix::ExtractMyRowCopy(int Row, int Length, int & NumEntries, double * Values) const 
{
  int j;
  ...
  if (Length < NumEntries) 
    KOKKOS_CHK_ERR(-2); // Not enough space for copy. Needed size is passed back in NumEntries
...
//==============================================================================
int Kokkos_CrsMatrix::ExtractDiagonalCopy(Kokkos_Vector & Diagonal) const {
	
  if(!Filled()) 
    KOKKOS_CHK_ERR(-1); // Can't get diagonal unless matrix is filled (and in local index space)
  if(!RowMap().SameAs(Diagonal.Map())) 
    KOKKOS_CHK_ERR(-2); // Maps must be the same
...
//==============================================================================
int Kokkos_CrsMatrix::ReplaceDiagonalValues(const Kokkos_Vector & Diagonal) {
	
  if(!Filled())
    KOKKOS_CHK_ERR(-1); // Can't replace diagonal unless matrix is filled (and in local index space)
  if(!RowMap().SameAs(Diagonal.Map())) 
    KOKKOS_CHK_ERR(-2); // Maps must be the same
...
//==========================================================================
int Kokkos_CrsMatrix::ExtractGlobalRowView(int Row, int & NumEntries, double *& Values, int *& Indices) const 
{
  ...
  KOKKOS_CHK_ERR(ExtractGlobalRowView(Row, NumEntries, Values));
  return(0);
}
//==========================================================================
int Kokkos_CrsMatrix::ExtractMyRowView(int Row, int & NumEntries, double *& Values, int *& Indices) const 
{
  int ierr = Graph_->ExtractMyRowView(Row, NumEntries, Indices);
  if (ierr) 
    KOKKOS_CHK_ERR(ierr);

  KOKKOS_CHK_ERR(ExtractMyRowView(Row, NumEntries, Values));
  return(0);
}
//==========================================================================
int Kokkos_CrsMatrix::ExtractGlobalRowView(int Row, int & NumEntries, double *& Values) const 
{

  int Row0 = Graph_->RowMap().LID(Row); // Normalize row range

  KOKKOS_CHK_ERR(ExtractMyRowView(Row0, NumEntries, Values));
  return(0);
}

//==========================================================================
int Kokkos_CrsMatrix::ExtractMyRowView(int Row, int & NumEntries, double *& Values) const 
{
...
//=============================================================================
int Kokkos_CrsMatrix::Multiply(bool TransA, const Kokkos_Vector& x, Kokkos_Vector& y) const {
  //
  // This function forms the product y = A * x or y = A' * x
  //

  if(!Filled()) 
    KOKKOS_CHK_ERR(-1); // Matrix must be filled.
  ...
  if(!TransA) {
	...
	if(ImportVector_->NumVectors() != 1) { 
      ...
      if(ImportVector_ == 0) 
	ImportVector_ = new Kokkos_MultiVector(ColMap(),1); // Create import vector if needed
      KOKKOS_CHK_ERR(ImportVector_->Import(x, *Importer(), Insert));
	...
	if(ExportVector_->NumVectors() != 1) { 
      ...
      if(ExportVector_ == 0) 
	ExportVector_ = new Kokkos_MultiVector(RowMap(),1); // Create Export vector if needed
    ...
    if(Exporter() != 0) {
      ...
      KOKKOS_CHK_ERR(y.Export(*ExportVector_, *Exporter(), Add)); // Fill y with Values from export vector
    }
  }
	...
	if(ExportVector_->NumVectors() != 1) { 
      ...
      if(ExportVector_ == 0) 
	ExportVector_ = new Kokkos_MultiVector(RowMap(),1); // Create Export vector if needed
      KOKKOS_CHK_ERR(ExportVector_->Import(x, *Exporter(), Insert));
    ...
    if(Importer() != 0) {
      y.PutScalar(0.0); // Make sure target is zero
      KOKKOS_CHK_ERR(y.Export(*ImportVector_, *Importer(), Add)); // Fill y with Values from export vector
    }
  }
...
//=============================================================================
int Kokkos_CrsMatrix::Multiply(bool TransA, const Kokkos_MultiVector& X, Kokkos_MultiVector& Y) const {
  //
  ...
  if((X.NumVectors() == 1) && (Y.NumVectors() == 1)) {
    ...
    Kokkos_Vector x(View, X.Map(), xp);
    Kokkos_Vector y(View, Y.Map(), yp);
    KOKKOS_CHK_ERR(Multiply(TransA, x, y));
    return(0);
  }
  if(!Filled()) {
    KOKKOS_CHK_ERR(-1); // Matrix must be filled.
  ...
  if (!TransA) {
	...
	if (ImportVector_->NumVectors()!=NumVectors) { 
      ...
      if (ImportVector_==0) 
	ImportVector_ = new Kokkos_MultiVector(ColMap(),NumVectors); // Create import vector if needed
      KOKKOS_CHK_ERR(ImportVector_->Import(X, *Importer(), Insert));
	...
	if (ExportVector_->NumVectors()!=NumVectors) { 
      ...
      if (ExportVector_==0) 
	ExportVector_ = new Kokkos_MultiVector(RowMap(),NumVectors); // Create Export vector if needed
...
//=============================================================================
int Kokkos_CrsMatrix::Solve(bool Upper, bool Trans, bool UnitDiagonal,
			    const Kokkos_Vector& x, Kokkos_Vector& y) const
{
  //
  ...
  if ((!UnitDiagonal) && (NoDiagonal())) 
    KOKKOS_CHK_ERR(-4); // If matrix has no diagonal, we must use UnitDiagonal
  if ((!UnitDiagonal) && (NumMyDiagonals()<NumMyRows_)) 
    KOKKOS_CHK_ERR(-5); // Need each row to have a diagonal
...
//=============================================================================
int Kokkos_CrsMatrix::Solve(bool Upper, bool Trans, bool UnitDiagonal, const Kokkos_MultiVector& X, Kokkos_MultiVector& Y) const {
  //
  ...
  if((X.NumVectors() == 1) && (Y.NumVectors() == 1)) {
    ...
    Kokkos_Vector x(View, X.Map(), xp);
    Kokkos_Vector y(View, Y.Map(), yp);
    KOKKOS_CHK_ERR(Solve(Upper, Trans, UnitDiagonal, x, y));
    return(0);
  }
  if(!Filled()) 
    KOKKOS_CHK_ERR(-1); // Matrix must be filled.
...
//=============================================================================
int Kokkos_CrsMatrix::InvRowSums(Kokkos_Vector& x) const {
  //
  // Put inverse of the sum of absolute values of the ith row of A in x[i].
  //

  if (!Filled()) KOKKOS_CHK_ERR(-1); // Matrix must be filled.
  if (!Graph().RowMap().SameAs(x.Map())) KOKKOS_CHK_ERR(-2); // x must have the same distribution as the range of A
  int ierr = 0;
  int i, j;
  int * NumEntriesPerRow = NumEntriesPerRow_;
  double ** Values = Values_;
...
//=============================================================================
//=============================================================================
int Kokkos_CrsMatrix::InvColSums(Kokkos_Vector& x) const {
  //
  ...
  if(!Graph().DomainMap().SameAs(x.Map())) 
    KOKKOS_CHK_ERR(-2); // x must have the same distribution as the domain of A
  
  double* xp = (double*)x.Values();
  Kokkos_Vector* x_tmp = 0;
  int NumMyCols_ = NumMyCols();
  

  // If we have a non-trivial importer, we must export elements that are permuted or belong to other processors
  if(Importer() != 0) {
    x_tmp = new Kokkos_Vector(ColMap()); // Create import vector if needed
...
//=============================================================================
int Kokkos_CrsMatrix::LeftScale(const Kokkos_Vector& x) {
  //
  ...
  if(!Graph().RangeMap().SameAs(x.Map())) 
    KOKKOS_CHK_ERR(-2); // x must have the same distribution as the range of A
...
//=============================================================================
int Kokkos_CrsMatrix::RightScale(const Kokkos_Vector& x) {
  //
  ...
  double* xp = (double*)x.Values();
  Kokkos_MultiVector* x_tmp = 0;

  // If we have a non-trivial importer, we must import elements that are permuted or are on other processors
  if(Importer() != 0) {
    x_tmp = new Kokkos_Vector(ColMap()); // Create import vector if needed
    KOKKOS_CHK_ERR(x_tmp->Import(x,*Importer(), Insert)); // x_tmp will have all the values we need
    xp = (double*)x_tmp->Values();
  }
...
//=============================================================================
double Kokkos_CrsMatrix::NormInf() const {
  ...
  for (int i=0; i < NumMyRows_; i++) {
    ...
    Local_NormInf = KOKKOS_MAX(Local_NormInf, sum);
  }
...
//=============================================================================
double Kokkos_CrsMatrix::NormOne() const {
  ...
  Kokkos_Vector* x = new Kokkos_Vector(RowMap()); // Need temp vector for column sums
  ...
  // If we have a non-trivial importer, we must export elements that are permuted or belong to other processors
  if(Importer() != 0) {
    x_tmp = new Kokkos_Vector(ColMap()); // Create temporary import vector if needed
    xp = (double*)x_tmp->Values();
  }
  int i, j;
  int* NumEntriesPerRow = NumEntriesPerRow_;
  ...
  if(Importer() != 0) {
    x->PutScalar(0.0);
    KOKKOS_CHK_ERR(x->Export(*x_tmp, *Importer(), Add)); // Fill x with Values from temp vector
  }
...
//=========================================================================
int Kokkos_CrsMatrix::CheckSizes(const Kokkos_SrcDistObject & Source) {
  try {
    const Kokkos_CrsMatrix & A = dynamic_cast<const Kokkos_CrsMatrix &>(Source);
    if (!A.Graph().GlobalConstantsComputed()) KOKKOS_CHK_ERR(-1); // Must have global constants to proceed
  }
  catch (...) {
...
//=========================================================================
int Kokkos_CrsMatrix::CopyAndPermute(const Kokkos_SrcDistObject & Source,
				     int NumSameIDs, 
  ...
  try {
    const Kokkos_CrsMatrix & A = dynamic_cast<const Kokkos_CrsMatrix &>(Source);
    KOKKOS_CHK_ERR(CopyAndPermuteCrsMatrix(A, NumSameIDs, NumPermuteIDs, PermuteToLIDs,
					   PermuteFromLIDs));
  }
  catch (...) {
    try {
      const Kokkos_RowMatrix & A = dynamic_cast<const Kokkos_RowMatrix &>(Source);
      KOKKOS_CHK_ERR(CopyAndPermuteRowMatrix(A, NumSameIDs, NumPermuteIDs, PermuteToLIDs,
					     PermuteFromLIDs));
    }
    catch (...) {
      KOKKOS_CHK_ERR(-1); // Incompatible SrcDistObject
    }
  }
...
//=========================================================================
int Kokkos_CrsMatrix::CopyAndPermuteCrsMatrix(const Kokkos_CrsMatrix & A, int NumSameIDs, 
					      int NumPermuteIDs, int * PermuteToLIDs,
					      int *PermuteFromLIDs) {
  ...
  if (NumSameIDs>0) {
	...
	Row = GRID(i);
	KOKKOS_CHK_ERR(A.ExtractGlobalRowCopy(Row, MaxNumEntries, NumEntries, Values, Indices)); // Set pointers
	// Place into target matrix.  Depends on Kokkos_DataAccess copy/view and static/dynamic graph.
	...
	if (ierr<0) KOKKOS_CHK_ERR(ierr);
      }
    }
    else { // A.IndiceAreGlobal()
      for (i=0; i<NumSameIDs; i++) {
	Row = GRID(i);
	KOKKOS_CHK_ERR(A.ExtractGlobalRowView(Row, NumEntries, Values, Indices)); // Set pointers
	// Place into target matrix.  Depends on Kokkos_DataAccess copy/view and static/dynamic graph.
	if (StaticGraph() || IndicesAreLocal())
	  ierr = ReplaceGlobalValues(Row, NumEntries, Values, Indices); 
	else
  ...
  if (NumPermuteIDs>0) {
	...
	KOKKOS_CHK_ERR(A.ExtractGlobalRowCopy(FromRow, MaxNumEntries, NumEntries, Values, Indices)); // Set pointers
	// Place into target matrix.  Depends on Kokkos_DataAccess copy/view and static/dynamic graph.
	if (StaticGraph() || IndicesAreLocal())
	...
	FromRow = A.GRID(PermuteFromLIDs[i]);
	ToRow = GRID(PermuteToLIDs[i]);
	KOKKOS_CHK_ERR(A.ExtractGlobalRowView(FromRow, NumEntries, Values, Indices)); // Set pointers
	// Place into target matrix.  Depends on Kokkos_DataAccess copy/view and static/dynamic graph.
	if (StaticGraph() || IndicesAreLocal())
	  ierr = ReplaceGlobalValues(ToRow, NumEntries, Values, Indices); 
	else
...
//=========================================================================
int Kokkos_CrsMatrix::CopyAndPermuteRowMatrix(const Kokkos_RowMatrix & A, int NumSameIDs, 
					      int NumPermuteIDs, int * PermuteToLIDs,
					      int *PermuteFromLIDs) {
  ...
  const Kokkos_Map & RowMap = A.RowMatrixRowMap();
  const Kokkos_Map & ColMap = A.RowMatrixColMap();

  // Do copy first
  if (NumSameIDs>0) {
    for (i=0; i<NumSameIDs; i++) {
      KOKKOS_CHK_ERR(A.ExtractMyRowCopy(i, MaxNumEntries, NumEntries, Values, Indices));
      // Place into target matrix.  Depends on Kokkos_DataAccess copy/view and static/dynamic graph.
      if (StaticGraph() || IndicesAreLocal())
  ...
  if (NumPermuteIDs>0) {
    for (i=0; i<NumPermuteIDs; i++) {
      ...
      KOKKOS_CHK_ERR(A.ExtractMyRowCopy(FromRow, MaxNumEntries, NumEntries, Values, Indices));
      // Place into target matrix.  Depends on Kokkos_DataAccess copy/view and static/dynamic graph.
      if (StaticGraph() || IndicesAreLocal())
	ierr = ReplaceGlobalValues(ToRow, NumEntries, Values, Indices);
      else {
...
//=========================================================================
int Kokkos_CrsMatrix::PackAndPrepare(const Kokkos_SrcDistObject & Source, 
  ...
  for (int i=0; i<NumExportIDs; i++) {
    ...
    *intptr = FromRow;
    Values = valptr; 
    Indices = intptr + 2;
    KOKKOS_CHK_ERR(A.ExtractMyRowCopy(ExportLIDs[i], GlobalMaxNumEntries, NumEntries, Values, Indices));
...
//=========================================================================
int Kokkos_CrsMatrix::UnpackAndCombine(const Kokkos_SrcDistObject & Source, 
				       int NumImportIDs, int * ImportLIDs, 
				       char * Imports, int & SizeOfPacket, 
				       Kokkos_Distributor & Distor, 
				       Kokkos_CombineMode CombineMode) {
  ...
  if (   CombineMode != Add
	 && CombineMode != Insert
	 && CombineMode != Zero )
    KOKKOS_CHK_ERR(-1); //Unsupported CombineMode, defaults to Zero
...
void Kokkos_CrsMatrix::Print(ostream& os) const {
  int MyPID = RowMap().Comm().MyPID();
  int NumProc = RowMap().Comm().NumProc();

  for (int iproc=0; iproc < NumProc; iproc++) {
    if (MyPID==iproc) {
      /*      const Kokkos_fmtflags olda = os.setf(ios::right,ios::adjustfield);
	      const Kokkos_fmtflags oldf = os.setf(ios::scientific,ios::floatfield);

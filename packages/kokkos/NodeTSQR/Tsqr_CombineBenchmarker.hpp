...
namespace TSQR {
  namespace Test {
    ...
    class CombineBenchmarkCalibrater {
    public:
    ...
    public:
      ...
      ///
      CombineBenchmarkCalibrater (const double timerRes,
	...
	TSQR::Test::verifyTimerConcept<timer_type> ();
      ...
      ///   routines) for requirements on the seed values.
      ///
      CombineBenchmarkCalibrater (const std::vector<int>& iseed) : 
	normGenS_ (iseed), 
	normGenM_ (iseed),
    ...
    /// \class CombineBenchmarker
    /// \brief Benchmarker for arbitrary TSQR::Combine implementations.
    /// \author Mark Hoemmen
    ///
    template< class Ordinal, class Scalar, class CombineType, class TimerType >
    class CombineBenchmarker {

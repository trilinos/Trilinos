...
if [ 1 ] ; then
  ...
  CMAKE_CONFIGURE="${CMAKE_CONFIGURE} -D Trilinos_ENABLE_KokkosClassic:BOOL=ON"
  CMAKE_CONFIGURE="${CMAKE_CONFIGURE} -D Trilinos_ENABLE_KokkosCompat:BOOL=ON"
  CMAKE_CONFIGURE="${CMAKE_CONFIGURE} -D Trilinos_ENABLE_KokkosMpiComm:BOOL=ON"
  ...
  if [ -n "${USE_CUDA_ARCH}" ] ; then
    ...
    CMAKE_CONFIGURE="${CMAKE_CONFIGURE} -D KokkosClassic_ENABLE_Thrust:BOOL=ON"
    CMAKE_CONFIGURE="${CMAKE_CONFIGURE} -D KokkosClassic_ENABLE_CUDA_DOUBLE:BOOL=ON"
    CMAKE_CONFIGURE="${CMAKE_CONFIGURE} -D KokkosClassic_ENABLE_Cusp:BOOL=OFF"

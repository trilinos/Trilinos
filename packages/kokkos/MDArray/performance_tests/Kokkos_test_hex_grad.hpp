...
struct TestHexGrad {
  ...
  DeviceMap map ;
  Kokkos::MDArrayView< Scalar , DeviceMap > coord;
  Kokkos::MDArrayView< Scalar , DeviceMap > grad;

  double init_time;
  double reduce_time;

  TestHexGrad(int parallel_work_length) : map( parallel_work_length )
  {

  ...
  const bool check )
{
  ...
  matrix.graph = Kokkos::create_crsarray<graph_type>( std::string("test crs graph") , fem_graph );
  ...
  if (check)
  {
    const double tol = Kokkos::Impl::is_same<double,VectorScalar>::value ? 1.0e-13 : 1.0e-5 ;

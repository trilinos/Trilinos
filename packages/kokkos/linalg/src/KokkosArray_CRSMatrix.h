...
template<class RangeVector,class CrsMatrix,class DomainVector, int doalpha = 1,int dobeta = 0>
struct MultiplyFunctor
{
  ...
  typedef typename DomainVector::scalar_type          domain_scalar_type ;
  typedef typename RangeVector::scalar_type            range_scalar_type ;


  range_scalar_type beta;
  scalar_type alpha;
  CrsMatrix  m_A ;
  DomainVector  m_x ;
  RangeVector  m_y ;
  ...
  void operator()( const size_type iRow ) const
  {
    ...
    if(dobeta == 1)
      m_y(iRow) += sum ;
    else m_y(iRow) = beta*m_y(iRow) + sum ;
  }
};

template<class RangeVector,class CrsMatrix,class DomainVector>
void Multiply( const typename RangeVector::scalar_type &beta, const RangeVector & y, const typename CrsMatrix::scalar_type& alpha,
		const CrsMatrix & A , const DomainVector & x)
{
	if(beta == 0.0)
	{
	  if(alpha == 1.0)
	  {
	    MultiplyFunctor<RangeVector,CrsMatrix,DomainVector,0,0> op ;
	  ...
	  {
	    MultiplyFunctor<RangeVector,CrsMatrix,DomainVector,1,0> op ;
        const typename CrsMatrix::ordinal_type nrow = A.numRows;
        op.m_A = A ;
        op.m_x = x ;
        op.m_y = y ;
	...
	else if(beta == 1.0)
	{
	  if(alpha == 1.0)
	  {
	    MultiplyFunctor<RangeVector,CrsMatrix,DomainVector,0,1> op ;
        const typename CrsMatrix::ordinal_type nrow = A.numRows;
        op.m_A = A ;
        op.m_x = x ;
        op.m_y = y ;
	  ...
	  {
	    MultiplyFunctor<RangeVector,CrsMatrix,DomainVector,1,1> op ;
        const typename CrsMatrix::ordinal_type nrow = A.numRows;
        op.m_A = A ;
        op.m_x = x ;
        op.m_y = y ;
	...
	else
	{
	  if(alpha == 1.0)
	  {
	    MultiplyFunctor<RangeVector,CrsMatrix,DomainVector,0,2> op ;
        const typename CrsMatrix::ordinal_type nrow = A.numRows;
        op.m_A = A ;
        op.m_x = x ;
        op.m_y = y ;
	  ...
	  {
	    MultiplyFunctor<RangeVector,CrsMatrix,DomainVector,1,2> op ;
        const typename CrsMatrix::ordinal_type nrow = A.numRows;
        op.m_A = A ;
        op.m_x = x ;
        op.m_y = y ;

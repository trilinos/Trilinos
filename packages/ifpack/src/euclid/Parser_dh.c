...
void
Parser_dhCreate (Parser_dh * p)
{
  START_FUNC_DH OptionsNode * ptr;

  /* allocate storage for object */
  struct _parser_dh *tmp =
    (struct _parser_dh *) MALLOC_DH (sizeof (struct _parser_dh));
  CHECK_V_ERROR;
  *p = tmp;

  /* consruct header node */
  tmp->head = tmp->tail = (OptionsNode *) MALLOC_DH (sizeof (OptionsNode));
  CHECK_V_ERROR;
  ptr = tmp->head;
  ptr->next = NULL;
  ptr->name = (char *) MALLOC_DH (6 * sizeof (char));
  CHECK_V_ERROR;
  ptr->value = (char *) MALLOC_DH (6 * sizeof (char));
  CHECK_V_ERROR;
...
void
Parser_dhDestroy (Parser_dh p)
{
  START_FUNC_DH OptionsNode * ptr2 = p->head, *ptr1 = ptr2;
  if (ptr1 != NULL)
    {
      do
	{
...
void
Parser_dhUpdateFromFile (Parser_dh p, char *filename)
{
  START_FUNC_DH_2 char line[80], name[80], value[80];
  FILE *fp;
...
*/
  ...
  for (j = 1; j < argc; ++j)
      ...
      if (strcmp (argv[j], "-db_filename") == 0)
	{
	  ...
	  if (j < argc)
	    {
	      Parser_dhUpdateFromFile (p, argv[j]);
	      CHECK_V_ERROR;
	    }
	}
    }
...
bool
Parser_dhHasSwitch (Parser_dh p, char *s)
{
  START_FUNC_DH_2 bool has_switch = false;
  OptionsNode *node;
...
bool
Parser_dhReadInt (Parser_dh p, char *in, int *out)
{
  START_FUNC_DH_2 bool has_switch = false;
  OptionsNode *node;
...
bool
Parser_dhReadDouble (Parser_dh p, char *in, double *out)
{
  START_FUNC_DH_2 bool optionExists = false;
  OptionsNode *node;
...
bool
Parser_dhReadString (Parser_dh p, char *in, char **out)
{
  START_FUNC_DH_2 bool optionExists = false;
  OptionsNode *node;
...
void
Parser_dhPrint (Parser_dh p, FILE * fp, bool allPrint)
{
  START_FUNC_DH_2 OptionsNode * ptr = p->head;
...
void
Parser_dhInsert (Parser_dh p, char *option, char *value)
{
  START_FUNC_DH_2 OptionsNode * node;
  int length;

  if (p == NULL)
    goto PARSER_NOT_INITED;
  ...
  if (find (p, option, &node))
	...
	{
	  FREE_DH (node->value);
	  node->value = (char *) MALLOC_DH (length * sizeof (char));
	  CHECK_V_ERROR;
	}
  ...
  /* otherwise, add a new node to the list */
    ...
    {
      node = p->tail;
      p->tail = node->next = (OptionsNode *) MALLOC_DH (sizeof (OptionsNode));
      CHECK_V_ERROR;
      node = node->next;
      length = strlen (option) + 1;
      node->name = (char *) MALLOC_DH (length * sizeof (char));
      CHECK_V_ERROR;
...
bool
find (Parser_dh p, char *option, OptionsNode ** ptr)
{
  START_FUNC_DH_2 OptionsNode * tmpPtr = p->head;
  bool foundit = false;
  while (tmpPtr != NULL)
    {
...
void
init_from_default_settings_private (Parser_dh p)
{
  START_FUNC_DH_2
    /* default is to intercept certain signals
       (floating point error, segmentation violation, etc.)
     */
    Parser_dhInsert (p, "-sig_dh", "1");
  CHECK_V_ERROR;

  /* used by MetGenFD */
  Parser_dhInsert (p, "-px", "1");
  CHECK_V_ERROR;
  Parser_dhInsert (p, "-py", "1");
  CHECK_V_ERROR;
  Parser_dhInsert (p, "-pz", "0");
  CHECK_V_ERROR;
  Parser_dhInsert (p, "-m", "4");
  CHECK_V_ERROR;

  Parser_dhInsert (p, "-xx_coeff", "-1.0");
  CHECK_V_ERROR;
  Parser_dhInsert (p, "-yy_coeff", "-1.0");
  CHECK_V_ERROR;
  Parser_dhInsert (p, "-zz_coeff", "-1.0");
  CHECK_V_ERROR;

  Parser_dhInsert (p, "-level", "1");
  CHECK_V_ERROR;

  Parser_dhInsert (p, "-printStats", "0");
  CHECK_V_ERROR;
END_FUNC_DH_2}

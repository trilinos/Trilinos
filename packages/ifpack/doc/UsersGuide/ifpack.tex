...
\choicebox{\tt block: print level}{[{\tt int}] Level of print, from 0 (silent)
  to 10 (verbose). Default: {\tt 0}.}
...
The parameters recognized by \verb~Ifpack_AdditiveSchwarz~ are reported below.
...
The parameters that can influence class \verb!Ifpack_CrsAdditiveSchwarz! are
reported below\footnote{Also all parameters for class \tt
  Ifpack\_AdditiveSchwarz are recognized.}.

\choicebox{\tt partitioner: overlap level}{[{\tt int}]. Specifies the
  level of overlap among the
{\sl blocks}. The overlap among the subdomains (or processes) is defined
in the constructor phase (value \verb~OverlapLevel~ in the previous examples).
These two parameters can differ. Default: {\tt 0}.}.

\choicebox{\tt partitioner: type}{[{\tt string}]. Specifies the
  \ifpack\ partitioner to use to decompose the local graph. It can be:
  {\tt linear}, {\tt greedy}, {\tt metis}, {\tt equation}. Default:
  {\tt metis}.}

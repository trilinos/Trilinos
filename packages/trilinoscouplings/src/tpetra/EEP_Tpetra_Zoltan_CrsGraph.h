// @HEADER
// *****************************************************************************
//           Trilinos: An Object-Oriented Solver Framework
//
// Copyright 2001-2024 NTESS and the Trilinos contributors.
// SPDX-License-Identifier: BSD-3-Clause
// *****************************************************************************
// @HEADER

#ifndef TPETRA_ZOLTAN_CRSGRAPH_H
#define TPETRA_ZOLTAN_CRSGRAPH_H

#include <EpetraExt_Transform.h>

#include <Teuchos_RCP.hpp>
#include <Teuchos_ParameterList.hpp>

class Epetra_CrsGraph;
class Epetra_Map;

namespace EpetraExt {

///
/** Generates an Epetra_CrsGraph based on the repartitioning algorithms of Isorropia
 */
class Zoltan_CrsGraph : public StructuralSameTypeTransform<Epetra_CrsGraph> {

  // Parameter list to specify partitioning algorithm for Isorropia.
  Teuchos::ParameterList partitionList_;

  // New graph for the repartitioned matrix. "String added by EEP, for testing"
  Teuchos::RCP<Epetra_CrsGraph> NewGraph_;

 public:

  ///
  /* Destructor
   */
  ~Zoltan_CrsGraph();

  ///
  /* Constructor
   * input param part_method - type of Isorropia partitioning to use
   */
  Zoltan_CrsGraph( Teuchos::ParameterList& partitionList )
  : partitionList_(partitionList)
  {
    std::cout << "EEP Passing through trilinoscouplings/src/tpetra/EEP_Tpetra_Zoltan_CrsGraph.h Zoltan_CrsGraph::constructor()..." << std::endl;
  }

  ///
  /* Generates the Isorropia partitioned Epetra_CrsGraph from the input object.
   */
  NewTypeRef operator()( OriginalTypeRef orig );

};

} //namespace Tpetra

#endif // TPETRA_ZOLTAN_CRSGRAPH_H

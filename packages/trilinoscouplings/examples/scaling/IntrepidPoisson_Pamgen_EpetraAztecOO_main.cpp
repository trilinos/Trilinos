  ...
  *out << "||B - A*X_exact||_2 = " << norms[0] << endl
  ...
  RCP<operator_type> M;
  {
    ...
    if (prec_type == "ML") {
      ...
      RCP<Xpetra::CrsMatrix<ST, int, int> > mueluA_ = rcp(new Xpetra::EpetraCrsMatrix(A));
      RCP<Xpetra::Matrix <ST, int, int> > mueluA  = rcp(new Xpetra::CrsMatrixWrap<ST, int, int>(mueluA_));

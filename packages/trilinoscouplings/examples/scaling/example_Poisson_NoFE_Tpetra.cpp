...
typedef Map::node_type::execution_space execution_space;
...
int main(int argc, char* argv[]) {
  ...
  if (argc >= 2)
    xmlMeshInFileName = string(argv[1]);
  else
  ...
  ParameterList iso_paramlist = inputMeshList.sublist("Isorropia Input");
  if (MyPID == 0) {
  ...
  Kokkos::DynRankView<double, memory_space> nodeCoord("nodeCoord", numNodes, dim);
  ...
  for (int b = 0; b < numElemBlk; b++) {
    ...
    if (inputMeshList.isParameter(sigmaBlock.str()))
      ...
      if (!sigmaRTC[b].addVar("double", "x")) {
        printf("ERROR: sigmaRTC.addVar(x) failed\n");
        exit(-1);
      }
      if (!sigmaRTC[b].addVar("double", "y")) {
        printf("ERROR: sigmaRTC.addVar(y) failed\n");
        exit(-1);
      }
      if (!sigmaRTC[b].addVar("double", "z")) {
        printf("ERROR: sigmaRTC.addVar(z) failed\n");
        exit(-1);
      }
      if (!sigmaRTC[b].addVar("double", "time")) {
        printf("ERROR: sigmaRTC.addVar(time) failed\n");
        exit(-1);
      }
      if (!sigmaRTC[b].addVar("double", "sigma")) {
        printf("ERROR: sigmaRTC.addVar(sigma) failed\n");
        exit(-1);
      }
      if (!sigmaRTC[b].addBody(mystr)) {
        printf("ERROR: sigmaRTC[%d].addBody failed\n", b);
        exit(-1);
      }
      useSigmaRTC[b] = true;
    } else
  ...
  telct = 0;
  Kokkos::DynRankView<double, memory_space> sigmaVal("sigmaVal", numElems);
  for (long long b = 0; b < numElemBlk; b++) {
    for (long long el = 0; el < elements[b]; el++) {
  ...
  Kokkos::DynRankView<double, memory_space> nodeOnBoundary("nodeOnBoundary", numNodes);
  ...
  for (int workset = 0; workset < numWorksets; workset++) {
    ...
    // Now we know the actual workset size and can allocate the array for the cell nodes
    worksetSize = worksetEnd - worksetBegin;
    Kokkos::View<double***, memory_space> cellWorkset("cellWorkset", worksetSize, numNodesPerElem, spaceDim);
    ...
    Intrepid2::ScalarView<double,memory_space> worksetJacobInv("worksetJacobianInv",worksetSize, numCubPoints, spaceDim, spaceDim);
    Intrepid2::ScalarView<double,memory_space> worksetJacobDet("worksetDet",worksetSize, numCubPoints);
    ...
    Intrepid2::ScalarView<double,memory_space> worksetMaterialVals("worksetMaterialVales",worksetSize, numCubPoints, spaceDim, spaceDim);
    ...
    Intrepid2::ScalarView<double,memory_space> weightedMeasureSigma("weightedMeasureSigma",worksetSize, numCubPoints);
    ...
    Intrepid2FSTools::HGRADtransformGRAD( worksetHGBGrads,worksetJacobInv, HGBGrads);

    // Compute integration measure for workset cells: Det(DF)*w = J*w
    Intrepid2FSTools::computeCellMeasure( worksetCubWeights,worksetJacobDet, cubWeights);
    ...
    Intrepid2FSTools::multiplyMeasure(worksetHGBGradsWeighted,
  ...
  Belos::ReturnType ret;
  {
    ...
    if (set == false) {
      cout << endl
           << "ERROR: Belos::LinearProblem failed to set up correctly!" << endl;
      return -1;
    }

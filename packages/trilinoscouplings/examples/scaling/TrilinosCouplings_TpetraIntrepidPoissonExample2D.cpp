                            ...
                            const bool debug)
{
  ...
  *out << "Generating mesh (tpetra)" << endl;
  ...
  for(long long b = 0; b < numElemBlk; b++){
    for(long long el = 0; el < elements[b]; el++){
      std::vector<double> centercoord(dim,0);
      for (int j=0; j<numNodesPerElem; j++) {
...
/**********************************************************************************/
  ...
  *out << "Getting quadrature" << endl;

  // Get numerical integration points and weights
  DefaultCubatureFactory<ST>  cubFactory;
  int cubDegree = 2;
  RCP<Cubature<ST> > myCub = cubFactory.create (cellType, cubDegree);
  ...
  *out << "Getting basis" << endl;
  ...
  // Define basis
  Basis_HGRAD_QUAD_C1_FEM<ST, FieldContainer<ST> > HGradBasis;

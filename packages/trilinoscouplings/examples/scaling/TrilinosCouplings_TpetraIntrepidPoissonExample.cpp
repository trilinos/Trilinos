...
/**********************************************************************************/
    ...
    for (int inode = 0; inode < numNodes; ++inode) {
      if (nodeIsOwned[inode]) {
        ...
        if (nodeOnBoundary (inode)) {
          ...
          ++indbc;
          // mfh 19 Apr 2012: We assume that the exact solution is
          // defined on the boundary, and evaluate the exact
          // solution function to get boundary values.
  ...
  *out << "Exporting matrix and right-hand side from overlapped to owned Map" << endl;
    ...
    TimeMonitor::getNewTimer ("Export from overlapped to owned");
  {
    ...
    // setAllToScalar(0.0); replace mode export will clobber values.
  ...
  *out << "Adjusting matrix and right-hand side for BCs" << endl;
    ...
    TimeMonitor::getNewTimer ("Adjust owned matrix and RHS for BCs");
  {
    ...
    Teuchos::ArrayRCP<const ST> vArrayRCP = v->getData(0);
    ...
    //
    // Zero out rows and columns of stiffness matrix corresponding
    // to Dirichlet edges and add one to diagonal.  The following is
    // the Tpetra analog of Apply_OAZToMatrix().
    //

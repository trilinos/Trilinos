...
/*
  ...
  FieldContainer<double> uhGValsTrans(desiredWorksetSize,numFieldsG, numCubPointsErr);
  FieldContainer<double> uhGrads(numFieldsG, numCubPointsErr, spaceDim);
  FieldContainer<double> uhGradsTrans(desiredWorksetSize, numFieldsG, numCubPointsErr, spaceDim);
  ...
  for (int k=0; k<numElems; k++){
    ...
    FieldContainer<double> cellWorksetEr(desiredWorksetSize,numNodesPerElem, spaceDim);

    for (int i=0; i<numNodesPerElem; i++) {
      cellWorksetEr(0,i,0) = nodeCoord(elemToNode(k,i),0);
      cellWorksetEr(0,i,1) = nodeCoord(elemToNode(k,i),1);
      cellWorksetEr(0,i,2) = nodeCoord(elemToNode(k,i),2);
    }

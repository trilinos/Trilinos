...
static void run(benchmark::State& state) {
  ...
  Kokkos::View<Scalar*, Device> beta(Kokkos::view_alloc(Kokkos::WithoutInitializing, "beta"), n);
  ...
  Kokkos::fill_random(beta, pool, 10.0);
  ExecSpace space;

  Kokkos::fence();
  for (auto _ : state) {
    KokkosBlas::axpby(space, alpha, x, beta, y);
    space.fence();
  }

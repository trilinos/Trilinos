...
nrm2w(const execution_space& space, const XVector& x, const XVector& w,
      typename std::enable_if<
          Kokkos::is_execution_space<execution_space>::value, int>::type = 0) {
  ...
  Impl::Nrm2w<execution_space, RVector_Internal, XVector_Internal>::nrm2w(
      space, R, X, W, true);
  space.fence();
  return result;
}
...
void nrm2w(const execution_space& space, const RV& R, const XMV& X,
           const XMV& W,
  ...
  Impl::Nrm2w<execution_space, RV_Internal, XMV_Internal>::nrm2w(
      space, R_internal, X_internal, W_internal, true);
}

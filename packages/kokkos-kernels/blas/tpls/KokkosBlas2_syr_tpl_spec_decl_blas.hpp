...
#define KOKKOSBLAS2_DSYR_BLAS(LAYOUT, ETI_SPEC_AVAIL)                                                            \
  template <typename ExecSpace>                                                                                  \
    requires(std::is_same_v<typename ExecSpace::memory_space, Kokkos::HostSpace>)                                \
  struct SYR<ExecSpace, Kokkos::View<const double*, LAYOUT, ExecSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged>>, \
    ...
    using AViewType = Kokkos::View<SCALAR**, LAYOUT, ExecSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged>>;        \
                                                                                                                 \
    static void syr(const ExecSpace& space, const char trans[], const char uplo[],                               \
      ...
      if (A_is_ll) {                                                                                             \
        ...
        SYR<ExecSpace, XViewType, AViewType, false, ETI_SPEC_AVAIL>::syr(space, trans, uplo, alpha, X, A);       \
      }                                                                                                          \
      Kokkos::Profiling::popRegion();                                                                            \
    }                                                                                                            \
  };

#define KOKKOSBLAS2_SSYR_BLAS(LAYOUT, ETI_SPEC_AVAIL)                                                           \
  template <typename ExecSpace>                                                                                 \
    requires(std::is_same_v<typename ExecSpace::memory_space, Kokkos::HostSpace>)                               \
  struct SYR<ExecSpace, Kokkos::View<const float*, LAYOUT, ExecSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged>>, \
...
#define KOKKOSBLAS2_ZSYR_BLAS(LAYOUT, ETI_SPEC_AVAIL)                                                                  \
  template <typename ExecSpace>                                                                                        \
    requires(std::is_same_v<typename ExecSpace::memory_space, Kokkos::HostSpace>)                                      \
  struct SYR<ExecSpace,                                                                                                \
...
#define KOKKOSBLAS2_CSYR_BLAS(LAYOUT, ETI_SPEC_AVAIL)                                                                  \
  template <typename ExecSpace>                                                                                        \
    requires(std::is_same_v<typename ExecSpace::memory_space, Kokkos::HostSpace>)                                      \
  struct SYR<ExecSpace,                                                                                                \
...
KOKKOSBLAS2_DSYR_BLAS(Kokkos::LayoutLeft, true)
KOKKOSBLAS2_DSYR_BLAS(Kokkos::LayoutLeft, false)
KOKKOSBLAS2_DSYR_BLAS(Kokkos::LayoutRight, true)
KOKKOSBLAS2_DSYR_BLAS(Kokkos::LayoutRight, false)

KOKKOSBLAS2_SSYR_BLAS(Kokkos::LayoutLeft, true)
KOKKOSBLAS2_SSYR_BLAS(Kokkos::LayoutLeft, false)
KOKKOSBLAS2_SSYR_BLAS(Kokkos::LayoutRight, true)
KOKKOSBLAS2_SSYR_BLAS(Kokkos::LayoutRight, false)

KOKKOSBLAS2_ZSYR_BLAS(Kokkos::LayoutLeft, true)
KOKKOSBLAS2_ZSYR_BLAS(Kokkos::LayoutLeft, false)
KOKKOSBLAS2_ZSYR_BLAS(Kokkos::LayoutRight, true)
KOKKOSBLAS2_ZSYR_BLAS(Kokkos::LayoutRight, false)

KOKKOSBLAS2_CSYR_BLAS(Kokkos::LayoutLeft, true)
KOKKOSBLAS2_CSYR_BLAS(Kokkos::LayoutLeft, false)
KOKKOSBLAS2_CSYR_BLAS(Kokkos::LayoutRight, true)
KOKKOSBLAS2_CSYR_BLAS(Kokkos::LayoutRight, false)

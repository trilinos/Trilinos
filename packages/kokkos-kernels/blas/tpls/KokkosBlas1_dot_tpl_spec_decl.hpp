...
namespace KokkosBlas {
namespace Impl {
#define KOKKOSBLAS1_DOT_TPL_SPEC_DECL_BLAS(LAYOUT, KOKKOS_TYPE, TPL_TYPE,      \
                                           MEMSPACE, ETI_SPEC_AVAIL)           \
  template <class ExecSpace>                                                   \
  struct Dot<ExecSpace,                                                        \
             Kokkos::View<KOKKOS_TYPE, LAYOUT, Kokkos::HostSpace,              \
    ...
    typedef Kokkos::View<KOKKOS_TYPE, LAYOUT, Kokkos::HostSpace,               \
                         Kokkos::MemoryTraits<Kokkos::Unmanaged> >             \
        RV;                                                                    \
    typedef Kokkos::View<const KOKKOS_TYPE*, LAYOUT,                           \
                         Kokkos::Device<ExecSpace, MEMSPACE>,                  \
                         Kokkos::MemoryTraits<Kokkos::Unmanaged> >             \
        XV;                                                                    \
    ...
    static void dot(const ExecSpace& space, RV& R, const XV& X, const XV& Y) { \
      Kokkos::Profiling::pushRegion("KokkosBlas::dot[TPL_BLAS," +              \
      ...
      if (numElems < static_cast<size_type>(INT_MAX)) {                        \
        ...
        int N   = numElems;                                                    \
        int one = 1;                                                           \
        R()     = HostBlas<TPL_TYPE>::dot(                                     \
            N, reinterpret_cast<const TPL_TYPE*>(X.data()), one,           \
...
#define KOKKOSBLAS1_DOT_TPL_SPEC_DECL_BLAS_EXT(ETI_SPEC_AVAIL)           \
  ...
  KOKKOSBLAS1_DOT_TPL_SPEC_DECL_BLAS(Kokkos::LayoutLeft, double, double, \
                                     Kokkos::HostSpace, ETI_SPEC_AVAIL)  \
  KOKKOSBLAS1_DOT_TPL_SPEC_DECL_BLAS(                                    \
...
namespace KokkosBlas {
namespace Impl {
#define KOKKOSBLAS1_DOT_TPL_SPEC_DECL_CUBLAS(LAYOUT, KOKKOS_TYPE, TPL_TYPE,    \
                                             EXECSPACE, MEMSPACE, TPL_DOT,     \
                                             ETI_SPEC_AVAIL)                   \
  template <>                                                                  \
  struct Dot<EXECSPACE,                                                        \
             Kokkos::View<KOKKOS_TYPE, LAYOUT, Kokkos::HostSpace,              \
    ...
    typedef typename XV::size_type size_type;                                  \
                                                                               \
    static void dot(const EXECSPACE& space, RV& R, const XV& X, const XV& Y) { \
      Kokkos::Profiling::pushRegion("KokkosBlas::dot[TPL_CUBLAS," +            \
                                    Kokkos::ArithTraits<KOKKOS_TYPE>::name() + \
                                    "]");                                      \
      const size_type numElems = X.extent(0);                                  \
      ...
      if (numElems <=                                                          \
        ...
        KOKKOS_CUBLAS_SAFE_CALL_IMPL(                                          \
            TPL_DOT(s.handle, N, reinterpret_cast<const TPL_TYPE*>(X.data()),  \
                    1, reinterpret_cast<const TPL_TYPE*>(Y.data()), 1,         \
...
#define KOKKOSBLAS1_DOT_TPL_SPEC_DECL_CUBLAS_EXT(ETI_SPEC_AVAIL)           \
  KOKKOSBLAS1_DOT_TPL_SPEC_DECL_CUBLAS(Kokkos::LayoutLeft, float, float,   \
                                       Kokkos::Cuda, Kokkos::CudaSpace,    \
                                       cublasSdot, ETI_SPEC_AVAIL)         \
  KOKKOSBLAS1_DOT_TPL_SPEC_DECL_CUBLAS(Kokkos::LayoutLeft, double, double, \
                                       Kokkos::Cuda, Kokkos::CudaSpace,    \
                                       cublasDdot, ETI_SPEC_AVAIL)         \
  KOKKOSBLAS1_DOT_TPL_SPEC_DECL_CUBLAS(                                    \
  ...
  KOKKOSBLAS1_DOT_TPL_SPEC_DECL_CUBLAS(                                    \
      ...
      Kokkos::Cuda, Kokkos::CudaSpace, cublasZdotc, ETI_SPEC_AVAIL)
...
namespace KokkosBlas {
namespace Impl {
#define KOKKOSBLAS1_DOT_TPL_SPEC_DECL_ROCBLAS(LAYOUT, KOKKOS_TYPE, TPL_TYPE,   \
                                              EXECSPACE, MEMSPACE, TPL_DOT,    \
                                              ETI_SPEC_AVAIL)                  \
  template <>                                                                  \
  struct Dot<EXECSPACE,                                                        \
    ...
    typedef typename XV::size_type size_type;                                  \
                                                                               \
    static void dot(const EXECSPACE& space, RV& R, const XV& X, const XV& Y) { \
      Kokkos::Profiling::pushRegion("KokkosBlas::dot[TPL_ROCBLAS," +           \
                                    Kokkos::ArithTraits<KOKKOS_TYPE>::name() + \
                                    "]");                                      \
      const size_type numElems = X.extent(0);                                  \
      if (numElems <=                                                          \
...
#define KOKKOSBLAS1_DOT_TPL_SPEC_DECL_ROCBLAS_EXT(ETI_SPEC_AVAIL)           \
  KOKKOSBLAS1_DOT_TPL_SPEC_DECL_ROCBLAS(Kokkos::LayoutLeft, float, float,   \
                                        Kokkos::HIP, Kokkos::HIPSpace,      \
                                        rocblas_sdot, ETI_SPEC_AVAIL)       \
  KOKKOSBLAS1_DOT_TPL_SPEC_DECL_ROCBLAS(Kokkos::LayoutLeft, double, double, \
                                        Kokkos::HIP, Kokkos::HIPSpace,      \
                                        rocblas_ddot, ETI_SPEC_AVAIL)       \
  KOKKOSBLAS1_DOT_TPL_SPEC_DECL_ROCBLAS(                                    \
...
namespace KokkosBlas {
namespace Impl {
#define KOKKOSBLAS1_DOT_TPL_SPEC_DECL_ONEMKL(LAYOUT, KOKKOS_TYPE, TPL_TYPE,    \
                                             EXECSPACE, MEMSPACE, TPL_DOT,     \
                                             ETI_SPEC_AVAIL)                   \
  template <>                                                                  \
  struct Dot<EXECSPACE,                                                        \
    ...
    static void dot(const EXECSPACE& exec, RV& R, const XV& X, const XV& Y) {  \
      Kokkos::Profiling::pushRegion("KokkosBlas::dot[TPL_ONEMKL," +            \
                                    Kokkos::ArithTraits<KOKKOS_TYPE>::name() + \
                                    "]");                                      \
      const size_type numElems = X.extent(0);                                  \
      if (numElems <=                                                          \
        ...
        TPL_DOT(exec.sycl_queue(), N,                                          \
                reinterpret_cast<const TPL_TYPE*>(X.data()), 1,                \
                reinterpret_cast<const TPL_TYPE*>(Y.data()), 1,                \
                reinterpret_cast<TPL_TYPE*>(&R()));                            \
      } else {                                                                 \
        Dot<EXECSPACE, RV, XV, XV, 1, 1, false, ETI_SPEC_AVAIL>::dot(exec, R,  \
                                                                     X, Y);    \
      }                                                                        \
      Kokkos::Profiling::popRegion();                                          \
    }                                                                          \
...
#define KOKKOSBLAS1_DOT_TPL_SPEC_DECL_ONEMKL_EXT(ETI_SPEC_AVAIL)            \
  KOKKOSBLAS1_DOT_TPL_SPEC_DECL_ONEMKL(                                     \

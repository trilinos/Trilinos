...
void impl_test_batched_axpy(const int N, const int BlkSize, const int N_team) {
  ...
  Functor_TestBatchedTeamAxpy<DeviceType, ViewType, alphaViewType, true>(alpha, X1, Y1, N_team).run();
  Functor_TestBatchedTeamAxpy<DeviceType, ViewType, alphaViewType, false>(alpha, X1, Y2, 1).run();
...
TEST_F(TestCategory, batched_scalar_team_axpy_nt_fcomplex_fcomplex) {
  test_batched_team_axpy<TestDevice, Kokkos::complex<float>, Kokkos::complex<float>>();
}

TEST_F(TestCategory, batched_scalar_team_axpy_nt_fcomplex_float) {
  test_batched_team_axpy<TestDevice, Kokkos::complex<float>, float>();
}
#endif

...
c
c  mydsaupd  is usually called iteratively to solve one of the
...
c          Not referenced if IPARAM(4) is not equal to zero.
c          See remark 7 below.
c
...
c
c          IPARAM(4) = USRCHK: a non-zero value denotes that the user will
c          check convergence. See remark 7 below.
c
c          IPARAM(5) = NCONV: number of "converged" Ritz values.
c          If USRCHK is equal to zero, then, upon exit (IDO=99)     
c          NCONV is the number of Ritz values that satisfy
c          the convergence criterion as determined by dsaupd.f
...
c     satisfy the users' convergence criterion before dsaupd.f is
...
c
c\Routines called:
c     mydsaup2   ARPACK routine that implements the Implicitly Restarted
c             Arnoldi Iteration.
...
c
     ...
     &     ipntr, workd, workl, lworkl, info, verbose )
c
c     %----------------------------------------------------%
...
c     %----------------------%
c
      external   mydsaup2 ,  dvout , ivout, second, dstats 
c
c     %--------------------%

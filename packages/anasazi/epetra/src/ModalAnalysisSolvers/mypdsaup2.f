...
c
c VERBOSE  Flag to print out the status of computation per iteration (or
c          equivalently restart).
c
c USRCHK   Integer. (INPUT) A nonzero value implies that reverse
c          communication is done so that the user can check convergence.
c
c UNCONV   Integer. (INPUT) Only used if USRCHK is not equal to zero.
c
c
c\EndDoc
c
...
c-----------------------------------------------------------------------
c
      subroutine mypdsaup2
     &   ( comm, ido, bmat, n, which, nev, np, tol, resid, mode, iupd, 
     &     ishift, mxiter, v, ldv, h, ldh, ritz, bounds, 
     &     q, ldq, workl, ipntr, workd, info, verbose, usrchk, unconv )
c
      include   'mpif.h'
c
c     %---------------%
...
c
c     %---------------------------------%
c     | Back from checking convergence. | 
c     %---------------------------------%
c
      if (cnvchk)  go to 110
c 
...
c
	   ...
	   if (usrchk .eq. zero) then
            call dsconv (nev, ritz(np+1), workl(np+1), tol, nconv)
	   else
c
c           %--------------------------------------------------%
c           | reverse communication so that the user can check |
c           | convergence.                                     |
c           %--------------------------------------------------%
c
	      cnvchk = .true.

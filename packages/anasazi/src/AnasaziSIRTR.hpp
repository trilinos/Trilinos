...
namespace Anasazi {
  ...
  public:
    ...
    /*! \brief This method performs %SIRTR iterations until the status test
     ...
     * one of the SIRTR-specific exceptions.
     *
    */
    void iterate();
...
#define SIRTR_RELEASE_TEMP_MV(mv,workspace) \
  ...
  void SIRTR<ScalarType,MV,OP>::iterate() {
    ...
    while (this->tester_->checkStatus(this) != Passed) {
      ...
      if (this->om_->isVerbosity( Debug ) ) {
        ...
        // get temp storage for second order decrease of model: will store Aeta and Beta
        //
  ...
  template <class ScalarType, class MV, class OP>
  void 
  SIRTR<ScalarType,MV,OP>::currentStatus(ostream &os) 
  {

...
typedef double                             ST;
typedef ScalarTraits<ST>::magnitudeType MagnitudeType;
...
void checks( RCP<BlockDavidson<ST,MV,OP> > solver, int blocksize, int numblocks,
             RCP<Eigenproblem<ST,MV,OP> > problem,
             RCP<MatOrthoManager<ST,MV,OP> > ortho,
             SolverUtils<ST,MV,OP> &msutils) {
  BlockDavidsonState<ST,MV> state = solver->getState();
...
void testsolver( RCP<BasicEigenproblem<ST,MV,OP> > problem,
                 ...
                 BlockDavidsonState<ST,MV> initstate, bool invalidinit)
{
  // create a status tester to run for two iterations
  RCP< StatusTest<ST,MV,OP> > tester = rcp( new StatusTestMaxIters<ST,MV,OP>(2) );

  const int blocksize = pls.get<int>("Block Size",problem->getNEV());
  const int numblocks = pls.get<int>("Num Blocks",2);

  // create the solver
  RCP< BlockDavidson<ST,MV,OP> > solver;
  try {
    solver = rcp( new BlockDavidson<ST,MV,OP>(problem,sorter,printer,tester,ortho,pls) );
...
main (int argc, char *argv[])
{
  ...
  using Teuchos::rcp;
  typedef double                              ST;
  typedef ScalarTraits<ST>::magnitudeType     MT;
  typedef Tpetra::MultiVector<ST>             MV;
  typedef Tpetra::Operator<ST>                OP;
  ...
  RCP< SortManager<MT> > sorter = rcp( new BasicSort<MT>("LR") );
  ...
  try
  {
    BlockDavidsonState<ST,MV> istate;
    ...
    istate.KK = rcp( new SerialDenseMatrix<int,ST>(3,3) );
    ...
    istate.KK = rcp( new SerialDenseMatrix<int,ST>(5,5) );
    testsolver(probstd,printer,orthostd,sorter,pls,false,istate,true);
    ...
    try {
      RCP< BlockDavidson<ST,MV,OP> > solver
        = rcp( new BlockDavidson<ST,MV,OP>(Teuchos::null,sorter,printer,dumtester,orthostd,pls) );
    ...
    try {
      RCP< BlockDavidson<ST,MV,OP> > solver
        = rcp( new BlockDavidson<ST,MV,OP>(probstd,Teuchos::null,printer,dumtester,orthostd,pls) );
      TEUCHOS_TEST_FOR_EXCEPTION(true,get_out,"Instantiating with invalid parameters failed to throw exception.");
    }
    catch (const std::invalid_argument &ia) {
      // caught expected exception
    ...
    try {
      RCP< BlockDavidson<ST,MV,OP> > solver
        = rcp( new BlockDavidson<ST,MV,OP>(probstd,sorter,Teuchos::null,dumtester,orthostd,pls) );
      TEUCHOS_TEST_FOR_EXCEPTION(true,get_out,"Instantiating with invalid parameters failed to throw exception.");
    }
    catch (const std::invalid_argument &ia) {
      // caught expected exception
    ...
    try {
      RCP< BlockDavidson<ST,MV,OP> > solver
        = rcp( new BlockDavidson<ST,MV,OP>(probstd,sorter,printer,Teuchos::null,orthostd,pls) );
      TEUCHOS_TEST_FOR_EXCEPTION(true,get_out,"Instantiating with invalid parameters failed to throw exception.");
    }
    catch (const std::invalid_argument &ia) {
      // caught expected exception
    ...
    try {
      RCP< BlockDavidson<ST,MV,OP> > solver
        = rcp( new BlockDavidson<ST,MV,OP>(probstd,sorter,printer,dumtester,Teuchos::null,pls) );
      TEUCHOS_TEST_FOR_EXCEPTION(true,get_out,"Instantiating with invalid parameters failed to throw exception.");
    }
    catch (const std::invalid_argument &ia) {
      // caught expected exception

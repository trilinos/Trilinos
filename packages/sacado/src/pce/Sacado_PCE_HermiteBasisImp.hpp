...
template <typename T>
Sacado::PCE::HermiteBasis<T>::
HermiteBasis(const Sacado::PCE::HermiteBasis<T>& b) :
  d(b.d),
  basis(b.basis),
  norms(b.norms)
{
...
Sacado::PCE::HermiteBasis<T>& 
Sacado::PCE::HermiteBasis<T>::
operator=(const Sacado::PCE::HermiteBasis<T>& b)
{
  if (this != &b) {
    d = b.d;
    basis = b.basis;
...
project(const Sacado::PCE::StandardPoly<T>& p, std::vector<T>& coeffs) const
{
  ...
  coeffs[1] = T(0.5)*p.coeff(dp);
  unsigned int dc = 1;

  for (int k=dp-2; k>=0; --k) {

    // Multiply by t
    h[0] = coeffs[1] + p.coeff(k);
    for (unsigned int i=1; i<=dc-1; i++) {
      h[i] = T(0.5)*coeffs[i-1] + T(i+1)*coeffs[i+1];
    }
    h[dc] = T(0.5)*coeffs[dc-1];
    h[dc+1] = T(0.5)*coeffs[dc];

...
#ifndef _tinyfadseventeen_h_
#define _tinyfadseventeen_h_
...
template <class T> class TinyFad<17,T> {
public:
...
public:
  ...
  TinyFad(const TinyFad<17,T> & in) : n(0), val_(in.val_) {
     dx0_ = in.dx0_;
     dx1_ = in.dx1_;
     dx2_ = in.dx2_;
     dx3_ = in.dx3_;
  ...
  TinyFad<17,T> & operator = (const T & in){
     val_ = in;
  ...
  TinyFad<17,T> & operator += (const T & in){     val_ += in;
...
template <class T> inline TinyFad<17,T> operator + (const TinyFad<17,T>& in)
{
  return TinyFad<17,T>(in);
}
...
TinyFad<17,typename NumericalTraits<L,R>::promote >
operator +(const TinyFad<17,L>& un, const TinyFad<17,R>& deux) {

  typedef typename NumericalTraits<L,R>::promote value_type;

  No_Initialization nothing;
  TinyFad<17,value_type> tmp( nothing );
...
template <class L, class R> inline
TinyFad<17,typename NumericalTraits<L,R>::promote>
operator +(const TinyFad<17,L>& un, const R& deux) {
...
template <class L, class R> inline
TinyFad<17,typename NumericalTraits<L,R>::promote >
operator +(const L& un, const TinyFad<17,R>& deux) {
...
operator /(const TinyFad<17,L> & un, const TinyFad<17,R> & deux) {
  ...
  if (deux.val() == 0.) error("TinyFad & TinyFad::operator /(const TinyFad<17,L> & un, const TinyFad<17,R> & deux), dividing by 0");
...
operator /(const L & un, const TinyFad<17,R> & deux) {
  ...
  if (deux.val() == 0.) error("TinyFad & TinyFad::operator /(const L & un, const TinyFad<17,R> & deux), dividing by 0");
...
operator /(const TinyFad<17,L> & un, const R & deux) {
  ...
  if (deux == 0.) error("TinyFad & TinyFad::operator /(const TinyFad<17,L> & un, const R & deux), dividing by 0");
...
template <class T> TinyFad<17,T> exp (const TinyFad<17,T>& in)
{
  TinyFad<17,T> tmp(exp(in.val()));
...
template <class T> TinyFad<17,T> log (const TinyFad<17,T>& in)
{
  if ( in.val() <= 0) error("TinyFad log (const TinyFad& in) : zero or negative value");
  TinyFad<17,T> tmp(log(in.val()));
...
template <class T> TinyFad<17,T> sqrt (const TinyFad<17,T>& in)
{
  if ( in.val() < 0. ) error("TinyFad<17,T> sqrt (const TinyFad& in) : negative value");
  TinyFad<17,T> tmp(sqrt(in.val()));
  ...
  if ( in.val() == T(0.) ){
    ...
    if ( !test )
      error("TinyFad<17,T> sqrt (const TinyFad& in) : null value");
  }
  else {
...
template <class T> TinyFad<17,T> sin (const TinyFad<17,T>& in)
{
  TinyFad<17,T> tmp( sin(in.val()) );
...
template <class T> TinyFad<17,T> cos (const TinyFad<17,T>& in)
{
  TinyFad<17,T> tmp(cos(in.val()));
...
template <class T> TinyFad<17,T> tan (const TinyFad<17,T>& in)
{
  if ( in.val() == 0) error("TinyFad tan (const TinyFad& in) undiefined in 0.");
  TinyFad<17,T> tmp(tan(in.val()));
...
template <class T> TinyFad<17,T> pow (const TinyFad<17,T>& in, double e)
{
  TinyFad<17,T> tmp(pow(in.val(), e));
...
template <class T> TinyFad<17,T> pow (const TinyFad<17,T>& un, const TinyFad<17,T>& deux)
{
  if (un.val() == 0) error("TinyFad pow (const TinyFad& un, const TinyFad& deux), un = 0. ");
  TinyFad<17,T> tmp(pow(un.val(), deux.val()));
...
template <class T> TinyFad<17,T> pow (const TinyFad<17,T>& in, const int e)
{
  TinyFad<17,T> tmp( std::pow((double)in.val(), (double)e) );
...
template <class T> TinyFad<17,T> abs (const TinyFad<17,T>& in)
{
...
template <class T> std::ostream& operator << (std::ostream& os, const TinyFad<17,T>& a)
{

...
#ifndef _tinyfadseven_h_
#define _tinyfadseven_h_
...
template <class T> class TinyFad<7,T> {
public:
...
public:
  ...
  TinyFad(const TinyFad<7,T> & in) : n(0), val_(in.val_) {
     dx0_ = in.dx0_;
     dx1_ = in.dx1_;
     dx2_ = in.dx2_;
     dx3_ = in.dx3_;
  ...
  TinyFad<7,T> & operator = (const T & in){
     val_ = in;
  ...
  TinyFad<7,T> & operator += (const T & in){     val_ += in;
...
template <class T> inline TinyFad<7,T> operator + (const TinyFad<7,T>& in)
{
  return TinyFad<7,T>(in);
}
...
TinyFad<7,typename NumericalTraits<L,R>::promote >
operator +(const TinyFad<7,L>& un, const TinyFad<7,R>& deux) {

  typedef typename NumericalTraits<L,R>::promote value_type;

  No_Initialization nothing;
  TinyFad<7,value_type> tmp( nothing );
...
template <class L, class R> inline
TinyFad<7,typename NumericalTraits<L,R>::promote>
operator +(const TinyFad<7,L>& un, const R& deux) {
...
template <class L, class R> inline
TinyFad<7,typename NumericalTraits<L,R>::promote >
operator +(const L& un, const TinyFad<7,R>& deux) {
...
operator /(const TinyFad<7,L> & un, const TinyFad<7,R> & deux) {
  ...
  if (deux.val() == 0.) error("TinyFad & TinyFad::operator /(const TinyFad<7,L> & un, const TinyFad<7,R> & deux), dividing by 0");
...
operator /(const L & un, const TinyFad<7,R> & deux) {
  ...
  if (deux.val() == 0.) error("TinyFad & TinyFad::operator /(const L & un, const TinyFad<7,R> & deux), dividing by 0");
...
operator /(const TinyFad<7,L> & un, const R & deux) {
  ...
  if (deux == 0.) error("TinyFad & TinyFad::operator /(const TinyFad<7,L> & un, const R & deux), dividing by 0");
...
template <class T> TinyFad<7,T> exp (const TinyFad<7,T>& in)
{
  TinyFad<7,T> tmp(exp(in.val()));
...
template <class T> TinyFad<7,T> log (const TinyFad<7,T>& in)
{
  if ( in.val() <= 0) error("TinyFad log (const TinyFad& in) : zero or negative value");
  TinyFad<7,T> tmp(log(in.val()));
...
template <class T> TinyFad<7,T> sqrt (const TinyFad<7,T>& in)
{
  if ( in.val() < 0. ) error("TinyFad<7,T> sqrt (const TinyFad& in) : negative value");
  TinyFad<7,T> tmp(sqrt(in.val()));
  ...
  if ( in.val() == T(0.) ){
    ...
    if ( !test )
      error("TinyFad<7,T> sqrt (const TinyFad& in) : null value");
  }
  else {
...
template <class T> TinyFad<7,T> sin (const TinyFad<7,T>& in)
{
  TinyFad<7,T> tmp( sin(in.val()) );
...
template <class T> TinyFad<7,T> cos (const TinyFad<7,T>& in)
{
  TinyFad<7,T> tmp(cos(in.val()));
...
template <class T> TinyFad<7,T> tan (const TinyFad<7,T>& in)
{
  if ( in.val() == 0) error("TinyFad tan (const TinyFad& in) undiefined in 0.");
  TinyFad<7,T> tmp(tan(in.val()));
...
template <class T> TinyFad<7,T> pow (const TinyFad<7,T>& in, double e)
{
  TinyFad<7,T> tmp(pow(in.val(), e));
...
template <class T> TinyFad<7,T> pow (const TinyFad<7,T>& un, const TinyFad<7,T>& deux)
{
  if (un.val() == 0) error("TinyFad pow (const TinyFad& un, const TinyFad& deux), un = 0. ");
  TinyFad<7,T> tmp(pow(un.val(), deux.val()));
...
template <class T> TinyFad<7,T> pow (const TinyFad<7,T>& in, const int e)
{
  TinyFad<7,T> tmp( std::pow((double)in.val(), (double)e) );
...
template <class T> TinyFad<7,T> abs (const TinyFad<7,T>& in)
{
...
template <class T> std::ostream& operator << (std::ostream& os, const TinyFad<7,T>& a)
{
